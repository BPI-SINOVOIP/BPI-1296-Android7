...
unsigned char gRadioBaseMac[CW_MAX_RADIOS_PER_WTP][6]; // note: this include optional Wireless field
			// Babylon: Each Radio's Base Address, We currently don't set, assume that AC knows the informatin.
...
{
	CW_COPY_MEMORY(buf, &msgPtr->msg[msgPtr->offset], len);
	msgPtr->offset += len;
}
...
{ 
	CW_FREE_OBJECT(pAddWlan);
}

void CWProtocolAddWLANVlauePrint(const CWProtocolAddWLANValues *pAddWlan)
{
	int i;
	CWDebugLog("[AddWlan radio_id: %u, wlan_id %u (%p)]", 
		 pAddWlan->radio_id, pAddWlan->wlan_id, pAddWlan);
	CWDebugLog("\tcapability: %04x", pAddWlan->capability.u16);
	CWDebugLog("\tkey_index: %u", pAddWlan->key_index);
	CWDebugLog("\tkey_status: %u", pAddWlan->key_status);
	CWDebugLog("\tkey_len: %u", pAddWlan->key_len);
	CWDebugLog("\tkey:");
	for (i=0; i<pAddWlan->key_len; i++)
	{
		CWDebugLog(" %02x", pAddWlan->key[i]);
	}
	CWDebugLog("\tgroup_TSC:");
	for (i=0; i<6; i++)
	{
		CWDebugLog(" %02x", pAddWlan->group_TSC[i]);
	}
	CWDebugLog("\tnon_WMM_sta_qos: %u", pAddWlan->non_WMM_sta_qos);
	CWDebugLog("\tauth_type: %u", pAddWlan->auth_type);
	CWDebugLog("\tmac_mode: %u", pAddWlan->mac_mode);
	CWDebugLog("\ttunnel_mode: %u", pAddWlan->tunnel_mode);
	CWDebugLog("\tsuppress_ssid: %u", pAddWlan->suppress_ssid);
	CWDebugLog("\tssid: %s", pAddWlan->ssid);
}
#endif
...
// TODO Babylon: It seems that radio_mac_present is always 0...
CWBool CWAssembleTransportHeader(CWProtocolMessage *transportHdrPtr, CWProtocolTransportHeaderValues *valuesPtr) {
	...
	if(radio_mac_present){
		...
		CWThreadMutexLock(&gRADIO_MAC_mutex);		
			CWProtocolStoreRawBytes(transportHdrPtr,gRadioBaseMac[0],6);
...
CWBool CWAssembleMessage(CWProtocolMessage **completeMsgPtr, int *fragmentsNumPtr, int PMTU, int seqNum, int msgTypeValue, CWProtocolMessage *msgElems, const int msgElemNum, CWProtocolMessage *msgElemsBinding, const int msgElemBindingNum, int is_crypted) {
	...
	if(!(CWAssembleControlHeader(&controlHdr, &controlVal))) {
		...
		CWLog("Fail to Assemble Message");
...
//		transportVal.last = 1;
		...
		for(i = 0; i < *fragmentsNumPtr; i++) { // for each fragment to assemble
			...
			if(!(CWAssembleTransportHeader(&transportHdr, &transportVal))) {
				...
				CW_FREE_PROTOCOL_MESSAGE(transportHdr);
				CW_FREE_OBJECT(completeMsgPtr);
				CWLog("Fail to Assemble Message Fragments");
				return CW_FALSE; // will be handled by the caller
			}
...
CWBool CWProtocolParseFragment(unsigned char *buf, int readBytes, CWList *fragmentsListPtr, CWProtocolMessage *reassembledMsg, CWBool *dataFlagPtr, char *RadioBaseMac) {
	...
	if(!CWParseTransportHeader(&msg, &values, dataFlagPtr, RadioBaseMac)){
...
CWBool CWParseTransportHeader(CWProtocolMessage *msgPtr, CWProtocolTransportHeaderValues *valuesPtr, CWBool *dataFlagPtr, char *RadioBaseMAC) {
...
//	CWDebugLog("FRAGMENT_OFFSET: %d", valuesPtr->fragmentOffset);
	...
	if (*dataFlagPtr == CW_TRUE){
		...
		if( m && rid == 0){
			...
			if (!CWParseTransportHeaderMACAddress(msgPtr, RadioBaseMAC)){
				if(valuesPtr->bindingValuesPtr!=NULL)
				{
					//printf("%s:%d free valuesPtr->bindingValuesPtr!!!!\n",__FUNCTION__,__LINE__);
					CW_FREE_OBJECT(valuesPtr->bindingValuesPtr);
				}
				return CW_FALSE;
			}
...
CWBool CWAssembleMsgElemImageIdentifier(CWProtocolMessage *msgPtr, int vendor_indentifier, const char *version)
{
	CW_CREATE_PROTOCOL_MESSAGE(*msgPtr, 4+strlen(version)+1, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
//printf("(%s)vendor_indentifier=%d, version=%s\n", __FUNCTION__, vendor_indentifier, version);
	CWProtocolStore32(msgPtr, vendor_indentifier);
	CWProtocolStoreRawBytes(msgPtr, (const void *)version, strlen(version)+1);
...
{
	CW_CREATE_PROTOCOL_MESSAGE(*msgPtr, DEL_STA_ELEM_LEN, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
...
CWBool CWAssembleMsgElemFreeStation(int radioID, CWProtocolMessage *msgPtr,unsigned char* StationMacAddr)
{
	CW_CREATE_PROTOCOL_MESSAGE(*msgPtr, FREE_STA_ELEM_LEN, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	CWProtocolStore8(msgPtr, radioID);
	CWProtocolStoreRawBytes(msgPtr,StationMacAddr,6);
	
	return CWAssembleMsgElem(msgPtr, CW_MSG_ELEMENT_FREE_STATION_CW_TYPE);
}
CWBool CWAssembleMsgElemSilentStation(int radioID, CWProtocolMessage *msgPtr,unsigned char* StationMacAddr)
{
	CW_CREATE_PROTOCOL_MESSAGE(*msgPtr, SILENT_STA_ELEM_LEN, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	CWProtocolStore8(msgPtr, radioID);
	CWProtocolStoreRawBytes(msgPtr,StationMacAddr,6);
	
	return CWAssembleMsgElem(msgPtr, CW_MSG_ELEMENT_SILENT_STATION_CW_TYPE);
}
CWBool CWAssembleMsgElemUnsilentStation(int radioID, CWProtocolMessage *msgPtr,unsigned char* StationMacAddr)
{
	CW_CREATE_PROTOCOL_MESSAGE(*msgPtr, UNSILENT_STA_ELEM_LEN, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	CWProtocolStore8(msgPtr, radioID);
	CWProtocolStoreRawBytes(msgPtr,StationMacAddr,6);
	
	return CWAssembleMsgElem(msgPtr, CW_MSG_ELEMENT_UNSILENT_STATION_CW_TYPE);
}
CWBool CWAssembleMsgElemDot11vStation(int radioID,CWProtocolMessage *msgPtr,unsigned char* StationMacAddr)
{
	CW_CREATE_PROTOCOL_MESSAGE(*msgPtr, DOT11V_STA_ELEM_LEN, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	CWProtocolStore8(msgPtr, radioID);
	CWProtocolStoreRawBytes(msgPtr,StationMacAddr,6);
	
	return CWAssembleMsgElem(msgPtr, CW_MSG_ELEMENT_DOT11V_STATION_CW_TYPE);
}
CWBool CWAssembleMsgElemDualStation(int radioID,CWProtocolMessage *msgPtr,unsigned char *StationMacAddr)
{
	CW_CREATE_PROTOCOL_MESSAGE(*msgPtr, DUAL_STA_ELEM_LEN, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY,NULL););
	CWProtocolStore8(msgPtr, radioID);
	CWProtocolStoreRawBytes(msgPtr,StationMacAddr,6);

	return CWAssembleMsgElem(msgPtr, CW_MSG_ELEMENT_DUAL_STATION_CW_TYPE);
}
CWBool CWAssembleMsgElemAddStation(CWProtocolMessage *msgPtr, int len, unsigned char *str, int type)
{
	...
	CWProtocolStoreRawBytes(msgPtr, str, len);
...
{
	CW_CREATE_PROTOCOL_MESSAGE(*msgPtr, sizeof(CWProtocolRtkWlanConfigValues), return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););

	CWProtocolStore8(msgPtr, valPtr->radio_id);
	CWProtocolStore8(msgPtr, valPtr->wlan_id);
	CWProtocolStoreRawBytes(msgPtr,valPtr->bssid, 6);	
	CWProtocolStoreRawBytes(msgPtr,valPtr->ssid, CW_MAX_SSID_SIZE+1); // TODO
	CWProtocolStore8(msgPtr,valPtr->key_type);
	CWProtocolStore8(msgPtr,valPtr->psk_format);
	CWProtocolStoreRawBytes(msgPtr, valPtr->key, CW_MAX_KEY_SIZE*2+1); // TODO

	return CWAssembleMsgElem(msgPtr, CW_MSG_ELEMENT_RTK_WLAN_CFG_CW_TYPE);
}
...
CWBool CWAssembleMsgElemRtkChannel(CWProtocolMessage *msgPtr, const CWProtocolRtkChannelValues *valPtr)
{
	...
	CWProtocolStore8(msgPtr, valPtr->radio_id);
	CWProtocolStore8(msgPtr, valPtr->channel);
	return CWAssembleMsgElem(msgPtr, CW_MSG_ELEMENT_RTK_CHANNEL_CW_TYPE);
}
...
CWBool CWAssembleMsgElemRtkPowerScale(CWProtocolMessage *msgPtr, const CWProtocolRtkPowerScaleValues *valPtr)
{
	CW_CREATE_PROTOCOL_MESSAGE(*msgPtr, 2, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	CWProtocolStore8(msgPtr, valPtr->radio_id);
	CWProtocolStore8(msgPtr, valPtr->power_scale);
	return CWAssembleMsgElem(msgPtr, CW_MSG_ELEMENT_RTK_POWER_SCALE_CW_TYPE);
}
#endif
...
CWBool CWAssembleMsgElemRtkSiteSurveyRequest(CWProtocolMessage *msgPtr)
{
	msgPtr->offset = 0;
	msgPtr->msg = NULL;
	return CWAssembleMsgElem(msgPtr, CW_MSG_ELEMENT_RTK_SITE_SURVEY_REQUEST_CW_TYPE);
}
...
CWBool CWAssembleMsgElemRtkStationRequest(CWProtocolMessage *msgPtr)
{
	msgPtr->offset = 0;
	msgPtr->msg = NULL;
	return CWAssembleMsgElem(msgPtr, CW_MSG_ELEMENT_RTK_STATION_REQUEST_CW_TYPE);
}
#endif


CWBool CWParseMsgElemACName(CWProtocolMessage *msgPtr, int len, char **valPtr) {	
...
CWBool CWParseMsgElemWTPRadioAdminState (CWProtocolMessage *msgPtr, int len, CWRadioAdminInfoValues *valPtr)
{
	CWParseMessageElementStart();
...
CWBool CWParseMsgElemWTPRadioOperationalState (CWProtocolMessage *msgPtr, int len, CWRadioOperationalInfoValues *valPtr)
{
	CWParseMessageElementStart();
...
CWBool CWParseMsgElemResultCode(CWProtocolMessage *msgPtr, int len, CWProtocolResultCode *valPtr) {
	CWParseMessageElementStart();
...
uint8_t *CWParseMsgElemSessionID(CWProtocolMessage *msgPtr, int len)
{
	return CWProtocolRetrieveRawBytes(msgPtr,16);
}


CWBool CWParseMsgElemWTPRadioInformation(CWProtocolMessage *msgPtr, int len, unsigned char *valPtr) {	
...
{
		image_identifier->vendor_id = (int)CWProtocolRetrieve32(msgPtr); 
		CWProtocolCopyRawBytes(image_identifier->imageIdentifier, msgPtr, len-sizeof(int));
		image_identifier->imageIdentifier[len-sizeof(int) -1] = '\0'; // make sure it is zero terminated. 
//printf("(%s) vendor_id=%d, imageIdentifier=%s\n", __FUNCTION__, image_identifier->vendor_id, image_identifier->imageIdentifier);
		return CW_TRUE;
}
...
CWBool CWParseMsgElemWTPConfiguration(CWProtocolMessage *msgPtr, int len, CWWTPWlanConfiguration *valPtr)
{
	...
	valPtr->wlan_num = CW_MAX_RADIOS_PER_WTP;
	int wlan_id = CWProtocolRetrieve32(msgPtr);	
	valPtr->wlan[wlan_id].ID = wlan_id;	
	CWProtocolCopyRawBytes(valPtr->wlan[wlan_id].mac_addr, msgPtr, 6);	
...
{
	if(len > CW_MAX_STR_BUFFER_SIZE)	return CW_FALSE;

	CWProtocolCopyRawBytes(str, msgPtr, len);
...
{
	if (len < DEL_STA_ELEM_LEN) return CW_FALSE;

	del_station->radio_id = CWProtocolRetrieve8(msgPtr);
	CWProtocolCopyRawBytes(del_station->mac_addr, msgPtr, 6);
...
{
	if (len < FREE_STA_ELEM_LEN) return CW_FALSE;

	free_sta->radio_id = CWProtocolRetrieve8(msgPtr);
	CWProtocolCopyRawBytes(free_sta->mac_addr, msgPtr, 6);
...
{
	if (len < SILENT_STA_ELEM_LEN) return CW_FALSE;

	silent_sta->radio_id = CWProtocolRetrieve8(msgPtr);
	CWProtocolCopyRawBytes(silent_sta->mac_addr, msgPtr, 6);
...
{
	if (len < UNSILENT_STA_ELEM_LEN) return CW_FALSE;

	unsilent_sta->radio_id = CWProtocolRetrieve8(msgPtr);
	CWProtocolCopyRawBytes(unsilent_sta->mac_addr, msgPtr, 6);
...
{
	if (len < DOT11V_STA_ELEM_LEN) return CW_FALSE;

	dot11v_sta->radio_id = CWProtocolRetrieve8(msgPtr);
	CWProtocolCopyRawBytes(dot11v_sta->mac_addr, msgPtr, 6);
...
{
	if(len < DUAL_STA_ELEM_LEN)		return CW_FALSE;

	dual_sta->radio_id = CWProtocolRetrieve8(msgPtr);
	CWProtocolCopyRawBytes(dual_sta->mac_addr, msgPtr, 6);

	return CW_TRUE;
}
#endif
#ifdef RTK_CAPWAP
CWBool CWParseMsgElemRtkWlanConfiguration(CWProtocolMessage *msgPtr, int len, CWProtocolRtkWlanConfigValues *valPtr)
{
	//int key_len;
	CWParseMessageElementStart();
	valPtr->radio_id = CWProtocolRetrieve8(msgPtr);
	valPtr->wlan_id = CWProtocolRetrieve8(msgPtr);
	CWProtocolCopyRawBytes(valPtr->bssid, msgPtr,6);
	CWProtocolCopyRawBytes(valPtr->ssid, msgPtr, CW_MAX_SSID_SIZE+1); // TODO

	valPtr->key_type = CWProtocolRetrieve8(msgPtr);
	valPtr->psk_format = CWProtocolRetrieve8(msgPtr);
	CWProtocolCopyRawBytes(valPtr->key, msgPtr, CW_MAX_KEY_SIZE*2+1); // TODO
...
CWBool CWParseMsgElemRtkChannel(CWProtocolMessage *msgPtr, int len, CWProtocolRtkChannelValues *valPtr)
{
	CWParseMessageElementStart();
	valPtr->radio_id = CWProtocolRetrieve8(msgPtr);
	valPtr->channel = CWProtocolRetrieve8(msgPtr);
	CWParseMessageElementEnd();
}
...
CWBool CWParseMsgElemRtkPowerScale(CWProtocolMessage *msgPtr, int len, CWProtocolRtkPowerScaleValues *valPtr)
{
	CWParseMessageElementStart();
	valPtr->radio_id = CWProtocolRetrieve8(msgPtr);
	valPtr->power_scale = CWProtocolRetrieve8(msgPtr);
	CWParseMessageElementEnd();
}
#endif
