...
#define CW_SETTINGS_FILE 	"/etc/config/settings.wtp.txt"
#else
...
CWBool CWParseSettingsFile()
{
	...
	while((line = (char*)CWGetCommand(gSettingsFile)) != NULL) 
	{
		...
		if (!strncmp(startTag+1, "MESH_PORTAL", endTag-startTag-1))
		{
			char* startValue=NULL;
			char* endValue=NULL;
			int offset = 0;
			...
			strncpy(portal_str, startValue, offset);
			portal_str[offset] ='\0';
			gMesh_portal = atoi(portal_str);
			CWLog("gMesh_portal: %d",gMesh_portal);
			CW_FREE_OBJECT(line);
			continue;
		}
	...
	//printf("%s:%d ####fclose(gSettingsFile)!!!!\n",__FUNCTION__,__LINE__);
	fclose(gSettingsFile);
	return CW_TRUE;
}
