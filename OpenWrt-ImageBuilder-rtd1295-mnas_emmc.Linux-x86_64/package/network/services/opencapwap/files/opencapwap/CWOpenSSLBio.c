...
typedef struct
{
	...
	volatile CWBool terminateRxWait;
} BIO_memory_data;
...
BIO* BIO_new_memory(CWSocket sock, CWNetworkLev4Address* pSendAddress, CWSafeList* pRecvAddress)
{
	...
	pData->terminateRxWait = CW_FALSE;
...
static int memory_read(BIO *b, char *out, int outl)
{
	...
	while (CWGetCountElementFromSafeList(pData->pRecvAddress) == 0 && !pData->terminateRxWait){
...
static long memory_ctrl(BIO *b, int cmd, long num, void *ptr)
{
	...
	switch (cmd)
	{
		...
		case BIO_CTRL_CAPWAP_TERMINATE_RX_WAIT:
			CWLog("BIO_CTRL_CAPWAP_TERMINATE_RX_WAIT");
			pData->terminateRxWait = CW_TRUE;
			CWSignalElementSafeList(pData->pRecvAddress);
			break;
