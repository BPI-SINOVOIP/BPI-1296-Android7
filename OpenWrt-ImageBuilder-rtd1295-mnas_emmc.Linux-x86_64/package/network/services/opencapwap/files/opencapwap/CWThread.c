...
CWBool CWCreateThread(CWThread *newThread, CW_THREAD_FUNCTION threadFunc, void *arg) {
	...
	int s = pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, &cancelable_state);
	if (s != 0) {
		CWDebugLog("Thread can't set cancel! (old_state=%d)\n", cancelable_state);
	} else {
		CWDebugLog("Thread is cancelable! (old_state=%d)\n", cancelable_state);
	}

	return CW_TRUE;
}

CWBool CWDestroyThread(CWThread t)
{
	CWBool ret;
	CWDebugLog("Destroy a Thread..\n");
	ret = (pthread_cancel(t) == 0);
	if (!ret) {		
		CWDebugLog("Error: Can't destory thread!");
	}
	return ret;
}
...
CWBool CWWaitThreadConditionTimeout(CWThreadCondition *theCondition, CWThreadMutex *theMutex, struct timespec* pTimeout) 
{
	int r;
	if(theCondition == NULL || theMutex == NULL) return CWErrorRaise(CW_ERROR_WRONG_ARG, NULL);
	CWLog("%s waiting...", __FUNCTION__);
	r = pthread_cond_timedwait(theCondition, theMutex, pTimeout);
	CWLog("%s waked up...", __FUNCTION__);
...
/* Babylon: this situation must be the bug created from programmer, you should let it crash!
...
/* Babylon: if this situation happens, it must be the bug created from programmer. You should let it crash!
	if(theMutex == NULL) return;
*/
...
CWBool CWThreadCreateSem(CWThreadSem *semPtr, int value) {
	...
	#ifdef CW_USE_NAMED_SEMAPHORES
	{
		...
		snprintf(name, 32, "/CWSem-%d-%4.4d", rtk_getpid(), semCount++);
...
void CWHandleTimer(CWTimerArg arg) {
	...
	CWThreadSendSignal(requestedThreadPtr, signalToRaise);
 	CWDebugLog("Timer Expired, Sent Signal(%d) to Thread: %p", signalToRaise, requestedThreadPtr);
...
CWBool CWTimerRequest(int sec, int usec, CWThread *threadPtr, CWTimerID *idPtr, int signalToRaise) {

	CWThreadTimerArg *arg;

	//CWDebugLog("Timer Request");
	if(sec < 0 || usec < 0 || threadPtr == NULL || idPtr == NULL) return CWErrorRaise(CW_ERROR_WRONG_ARG, NULL);
	...
	if ((*idPtr = timer_add(sec, usec, &CWHandleTimer, arg)) == -1) {
