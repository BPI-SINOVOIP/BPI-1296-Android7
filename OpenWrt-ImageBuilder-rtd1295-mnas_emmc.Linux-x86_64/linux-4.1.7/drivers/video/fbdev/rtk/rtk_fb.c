...
static int rtk_fb_memory_alloc(struct rtk_fb_memory ** ppMem, int width, int height, int byte_per_pixel ,int align, int count, int flags)
{
    uint32_t allocate_size = MEMORY_ALIGN((width * height * byte_per_pixel * count), align);
    uint32_t ion_heap_mask = RTK_PHOENIX_ION_HEAP_MEDIA_MASK;
...
static int rtk_fb_set_size (struct rtk_fb *fb, int width, int height, int count, int _byte_per_pixel, int flags)
{
    int ret = -1;
    struct rtk_fb_memory * pMem = NULL;
    ...
    if (rtk_fb_update_by_memory(fb, pMem) != 0) {
