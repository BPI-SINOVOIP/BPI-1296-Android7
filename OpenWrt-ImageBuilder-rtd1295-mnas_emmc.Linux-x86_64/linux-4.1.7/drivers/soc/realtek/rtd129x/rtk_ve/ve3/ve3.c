                            ...
                            unsigned long arg)
{
	...
	case HANTRODEC_SET_CLK_RATE:
	{
		...
		if (get_user(value, (u32 __user *) arg))
			break;

		if (ve3_clk_pll_set_rate(s_ve3_clk, value))
			break;
		ve3_wrapper_setup(base_address);
	}
	break;
	case HANTRODEC_GET_CLK_RATE:
	{
		...
		value = (unsigned int)clk_get_rate(s_ve3_clk);
...
static struct clk *ve3_clk_get(struct device *dev)
{
	struct clk * clk = clk_get(dev, "clk_ve3");
	
	if (IS_ERR_OR_NULL(clk)) {
		dev_warn(dev, "[VE3] failed to get clk_ve3\n");
		clk = NULL;
	}
	return clk;
}
