...
void _rtk_cpu_power_up(int cpu){
    ...
    if (unlikely(((tmp >> (16 + cpu)) & 1) != 1)) {
        WARN_ON(1);
        printk(KERN_ERR "Waiting for CPU%d SMPEN to be set\n", cpu);
    }
...
void rtk_cpu_power_up(int cpu)
{
    ...
    // 1. Assert nCPUPORESET LOW. Ensure DBGPWRDUP is held LOW to prevent any
    ...
    // 2. Apply power to the PDCPU power domain. Keep the state of the signals
    //    nCPUPORESET and DBGPWRDUP LOW.
    ...
    if((unlikely((tmp >> cpu) & 0x1) == 0x1)) {
        WARN_ON(1);
        printk(KERN_ERR "%s: L1 cache is in power-saving mode.\n", __func__);
        printk(KERN_ERR "%s: 0x9801D534 = %x\n", __func__, readl(scpu_wrap_addr + 0x534));
    }
...
void rtk_cpu_power_down(int cpu)
{
    ...
    if (unlikely(((tmp >> cpu) & 0x1) != 0x1)) {
        WARN_ON(1);
        printk(KERN_ERR "%s: Processor is not in WFI standby mode.\n", __func__);
        printk(KERN_ERR "%s: 0x9801D104 = %x\n", __func__, readl(scpu_wrap_addr + 0x104));
    }

    // 7. Dessert DBGPWRDUP LOW. This prevents any external debug access to the core.
    tmp = readl(scpu_wrap_addr + 0x100);
    tmp &= ~(1UL << (24 + cpu));
    writel(tmp, scpu_wrap_addr + 0x100);
