...
extern int32 rtl865xC_setAsicEthernetForceModeRegs(uint32 port,
	uint32 enForceMode, uint32 forceLink, uint32 forceSpeed,
	uint32 forceDuplex);
...
extern int32 rtl8651_setAsicEthernetPHYAdvCapality(uint32 port,
	uint32 capality);
...
extern int rtl8367b_getAsicPHYReg(uint32 phyNo, uint32 phyAddr,
	uint32 *pRegData);
...
#if defined(CONFIG_RTD_1295_HWNAT)
extern void __iomem *rtl_hwnat_mmio;
extern unsigned int rtl_hwnat_base;
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
...
static struct proc_dir_entry *vlan_entry, *netif_entry, *l2_entry, *arp_entry,
	...
	*acl_entry, *storm_control, *sw_netif_entry, *sw_nexthop_entry,
	*sw_l3_entry,
#if defined(CONFIG_RTL_MULTIPLE_WAN)
	*advRt_entry,
#endif
...
static struct proc_dir_entry *mmd_entry, *mem_entry, *diagnostic_entry,
	*asicCnt_entry, *phyReg_entry, *port_status_entry, *mac_entry,
	*fc_threshold_entry, *stats_debug_entry
...
extern int rtl_mdio_read(unsigned int mdio_phyaddr, unsigned int reg,
	unsigned int *pdata);
extern int rtl_mdio_write(unsigned int mdio_phyaddr, unsigned int reg,
	unsigned int data);
#endif
...
static struct proc_dir_entry *l3v6_entry, *arp6_entry, *nexthop6_entry,
	*mcast6_entry, *ip6rd_entry, *dslite_entry, *sw_dslite_entry,
	*sw_arp6_entry;
#endif
...
void memDump2(void *start, uint32 size)
{
...
#if defined(CONFIG_RTD_1295_HWNAT)
	const uintptr_t mem_base = (uintptr_t) rtl_hwnat_mmio - rtl_hwnat_base;
	uint32 phys_addr;
	uintptr_t addr = 0;
#else /* defined(CONFIG_RTD_1295_HWNAT) */
	uint32 addr = 0;
	...
	if (addr > (uintptr_t) rtl_hwnat_mmio
		&& (addr - (uintptr_t) rtl_hwnat_mmio) < 0x170000) {
		/* rbus virtual address */
		phys_addr = addr - mem_base;
	} else if (addr > rtl_hwnat_base && (addr - rtl_hwnat_base) < 0x170000) {
		/* rbus physical address */
		phys_addr = addr;
		addr += mem_base;
	} else {
		phys_addr = virt_to_phys((void *)addr);
	}
	rtlglue_printf("physical memory 0x%08x : virtual memory 0x%p\n",
		phys_addr, start);
	if (phys_addr < 0x80000000 || phys_addr > 0xbfffffff) {
		return;
	}
#else /* defined(CONFIG_RTD_1295_HWNAT) */
	addr = (uint32)start;
...
#if defined(CONFIG_RTD_1295_HWNAT)
	if ((phys_addr + size) < rtl_hwnat_base
		|| phys_addr >= (rtl_hwnat_base + 0x170000))
#else /* defined(CONFIG_RTD_1295_HWNAT) */
	if (addr < 0xb8000000)
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
	{
...
#if defined(CONFIG_RTD_1295_HWNAT)
			rtlglue_printf("%08lX: ", (uintptr_t) p);
#else /* defined(CONFIG_RTD_1295_HWNAT) */
...
#if defined(CONFIG_RTD_1295_HWNAT)
void memDump3(void *start, uint32 size, int8 *strHeader)
{
	int32 row, column, index, index2, max;
	uint32 buffer[5];
	uint8 *buf, *line, ascii[17];
	...
	if (!start || (size == 0))
		return;
	line = (uint8 *)start;
	phys_addr = (uintptr_t) start - mem_base;
...
#else
		/* for un-alignment access */
		buffer[0] = ntohl(READ_MEM32((((uintptr_t) line) & ~3) + 0));
		buffer[1] = ntohl(READ_MEM32((((uintptr_t) line) & ~3) + 4));
		buffer[2] = ntohl(READ_MEM32((((uintptr_t) line) & ~3) + 8));
		buffer[3] = ntohl(READ_MEM32((((uintptr_t) line) & ~3) + 12));
		buffer[4] = ntohl(READ_MEM32((((uintptr_t) line) & ~3) + 16));
		buf = ((uint8 *)buffer) + (((uintptr_t) line) & 3);
#endif
...
extern int32 rtl865x_perf_proc_write(struct file *file, const char *buffer,
	unsigned long count, void *data);
#else
static struct proc_dir_entry *perf_dump;
extern int32 rtl865x_perf_proc_read(char *page, char **start, off_t off,
...
extern int32 rtl865x_sw_napt_proc_read(char *page, char **start, off_t off,
	int count, int *eof, void *data);
extern int32 rtl865x_sw_napt_proc_write(struct file *filp, const char *buff,
...
extern int32 rtl865x_sw_l2_proc_write(struct file *filp, const char *buff,
	unsigned long len, void *data);
#else
static struct proc_dir_entry *sw_l2_entry;
extern int32 rtl865x_sw_l2_proc_read(char *page, char **start, off_t off,
	int count, int *eof, void *data);
extern int32 rtl865x_sw_l2_proc_write(struct file *filp, const char *buff,
	unsigned long len, void *data);
#endif
...
#else
extern int32 sw_arp6_read(char *page, char **start, off_t off, int count,
	int *eof, void *data);
#endif
extern int32 sw_arp6_write(struct file *filp, const char *buff,
	unsigned long len, void *data);
...
static int32 vlan_write(struct file *filp, const char *buff, unsigned long len,
	void *data)
{
	char tmpbuf[256];
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		if (tmpbuf[0] == '1') {
			...
			rtl865x_add_acl(&rule, RTL_DRV_LAN_NETIF_NAME,
			...
			if (!memcmp(cmd_addr, "dump", 4)) {
				...
				rtl865x_addVlanPortMember2(vid & 4095, portMask,
					taggedPortMask);
				...
				rtl865x_addVlanPortMember2(vid & 4095,
					portMask & 0x1FF,
					untagPortMask & 0x1FF);
				...
				/* update vlanconfig */
				update_vlanconfig(vid & 4095, portMask & 0x1FF,
					untagPortMask & 0x1FF);
			}
			return len;
errout:
	...
	for (i = 0; i < RTL865XC_NETIFTBL_SIZE; i++) {
		...
		if (intf.valid) {
			...
			seq_printf(s, "[%d]  VID[%d] %x:%x:%x:%x:%x:%x",
				i, intf.vid, mac[0], mac[1], mac[2], mac[3],
				mac[4], mac[5]);
			...
			if (RTL8651_ACLTBL_DROP_ALL <= intf.inAclStart) {
				...
				seq_printf(s, "ACL %d-%d, ", intf.inAclStart,
					intf.inAclEnd);
			...
			if (RTL8651_ACLTBL_DROP_ALL <= intf.outAclStart) {
				...
				seq_printf(s, "ACL %d-%d, ", intf.outAclStart,
					intf.outAclEnd);
...
#else
static int32 netif_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	int len;
	int8 *pst[] = { "DIS/BLK", "LIS", "LRN", "FWD" };
	...
	for (i = 0; i < RTL865XC_NETIFTBL_SIZE; i++) {
		...
		if (intf.valid) {
			...
			len += sprintf(page + len,
				"[%d]  VID[%d] %x:%x:%x:%x:%x:%x", i, intf.vid,
			...
			if (RTL8651_ACLTBL_DROP_ALL <= intf.inAclStart) {
				...
				len += sprintf(page + len, "ACL %d-%d, ",
					intf.inAclStart, intf.inAclEnd);
			...
			if (RTL8651_ACLTBL_DROP_ALL <= intf.outAclStart) {
				...
				len += sprintf(page + len, "ACL %d-%d, ",
					intf.outAclStart, intf.outAclEnd);
...
static int32 netif_write(struct file *filp, const char *buff, unsigned long len,
	void *data)
{
	return len;
}
...
#else
extern int32 sw_netif_read(char *page, char **start, off_t off, int count,
	int *eof, void *data);

#endif
static int32 sw_netif_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	return len;
}
...
#if 0
static int32 acl_read(char *page, char **start, off_t off, int count, int *eof,
	void *data)
{
	int len;

	int8 *actionT[] = {
			"permit", "redirect to ether", "drop", "to cpu",
			"legacy drop",
			"drop for log", "mirror", "redirect to pppoe",
			"default redirect", "mirror keep match",
			"drop rate exceed pps", "log rate exceed pps",
			"drop rate exceed bps", "log rate exceed bps",
			"priority "
	};
#ifdef CONFIG_RTL_LAYERED_DRIVER
...
again:
		...
		for (; acl_start <= acl_end; acl_start++) {
			...
			case RTL865X_ACL_MAC:
				...
				len += sprintf(page + len,
					"\tether type: %x   ether type mask: %x\n",
					asic_acl.typeLen_,
					asic_acl.typeLenMask_);
				len += sprintf(page + len,
					"\tDMAC: %x:%x:%x:%x:%x:%x  DMACM: %x:%x:%x:%x:%x:%x\n",
					asic_acl.dstMac_.octet[0],
					asic_acl.dstMac_.octet[1],
					asic_acl.dstMac_.octet[2],
					asic_acl.dstMac_.octet[3],
					asic_acl.dstMac_.octet[4],
					asic_acl.dstMac_.octet[5],
					asic_acl.dstMacMask_.octet[0],
					asic_acl.dstMacMask_.octet[1],
					asic_acl.dstMacMask_.octet[2],
					asic_acl.dstMacMask_.octet[3],
					asic_acl.dstMacMask_.octet[4],
					asic_acl.dstMacMask_.octet[5]
					);

				len += sprintf(page + len,
					"\tSMAC: %x:%x:%x:%x:%x:%x  SMACM: %x:%x:%x:%x:%x:%x\n",
					asic_acl.srcMac_.octet[0],
					asic_acl.srcMac_.octet[1],
					asic_acl.srcMac_.octet[2],
					asic_acl.srcMac_.octet[3],
					asic_acl.srcMac_.octet[4],
					asic_acl.srcMac_.octet[5],
					asic_acl.srcMacMask_.octet[0],
					asic_acl.srcMacMask_.octet[1],
					asic_acl.srcMacMask_.octet[2],
					asic_acl.srcMacMask_.octet[3],
					asic_acl.srcMacMask_.octet[4],
					asic_acl.srcMacMask_.octet[5]
					);
				break;
			...
			case RTL865X_ACL_IP:
				len += sprintf(page + len,
					...
					actionT[asic_acl.actionType_]);
				len += sprintf(page + len,
					"\tdip: %d.%d.%d.%d dipM: %d.%d.%d.%d\n",
					(asic_acl.dstIpAddr_ >> 24),
					((asic_acl.dstIpAddr_ & 0x00ff0000) >> 16),
					((asic_acl.dstIpAddr_ & 0x0000ff00) >> 8),
					(asic_acl.dstIpAddr_ & 0xff),
					(asic_acl.dstIpAddrMask_ >> 24),
					((asic_acl.dstIpAddrMask_ & 0x00ff0000) >> 16),
					((asic_acl.dstIpAddrMask_ & 0x0000ff00) >> 8),
					(asic_acl.dstIpAddrMask_ & 0xff)
					);
				len += sprintf(page + len,
					"\tsip: %d.%d.%d.%d sipM: %d.%d.%d.%d\n",
					(asic_acl.srcIpAddr_ >> 24),
					((asic_acl.srcIpAddr_ & 0x00ff0000) >> 16),
					((asic_acl.srcIpAddr_ & 0x0000ff00) >> 8),
					(asic_acl.srcIpAddr_ & 0xff),
					(asic_acl.srcIpAddrMask_ >> 24),
					((asic_acl.srcIpAddrMask_ & 0x00ff0000) >> 16),
					((asic_acl.srcIpAddrMask_ & 0x0000ff00) >> 8),
					(asic_acl.srcIpAddrMask_ & 0xff)
					);
				len += sprintf(page + len,
					"\tTos: %x   TosM: %x   ipProto: %x   ipProtoM: %x   ipFlag: %x   ipFlagM: %x\n",
					asic_acl.tos_, asic_acl.tosMask_,
					asic_acl.ipProto_,
					asic_acl.ipProtoMask_, asic_acl.ipFlag_,
					asic_acl.ipFlagMask_);

				len += sprintf(page + len,
					"\t<FOP:%x> <FOM:%x> <http:%x> <httpM:%x> <IdentSdip:%x> <IdentSdipM:%x> \n",
					asic_acl.ipFOP_, asic_acl.ipFOM_,
					asic_acl.ipHttpFilter_,
					asic_acl.ipHttpFilterM_,
					asic_acl.ipIdentSrcDstIp_,
			...
			case RTL865X_ACL_ICMP:
				...
				len += sprintf(page + len,
					"\tTos: %x   TosM: %x   type: %x   typeM: %x   code: %x   codeM: %x\n",
					asic_acl.tos_, asic_acl.tosMask_,
					asic_acl.icmpType_,
					asic_acl.icmpTypeMask_,
					asic_acl.icmpCode_,
					asic_acl.icmpCodeMask_);
				break;
			...
			case RTL865X_ACL_IGMP:
				...
				len += sprintf(page + len,
					"\tTos: %x   TosM: %x   type: %x   typeM: %x\n",
					asic_acl.tos_, asic_acl.tosMask_,
					asic_acl.igmpType_,
					asic_acl.igmpTypeMask_);
				break;
			...
			case RTL865X_ACL_TCP:
				...
				len += sprintf(page + len,
					"\tTos:%x  TosM:%x  sportL:%d  sportU:%d  dportL:%d  dportU:%d\n",
					asic_acl.tos_, asic_acl.tosMask_,
					asic_acl.tcpSrcPortLB_,
					asic_acl.tcpSrcPortUB_,
					asic_acl.tcpDstPortLB_,
					asic_acl.tcpDstPortUB_);
				len += sprintf(page + len,
					"\tflag: %x  flagM: %x  <URG:%x> <ACK:%x> <PSH:%x> <RST:%x> <SYN:%x> <FIN:%x>\n",
					asic_acl.tcpFlag_,
					asic_acl.tcpFlagMask_, asic_acl.tcpURG_,
					asic_acl.tcpACK_, asic_acl.tcpPSH_,
					asic_acl.tcpRST_, asic_acl.tcpSYN_,
					asic_acl.tcpFIN_);
				break;
			...
			case RTL865X_ACL_UDP:
				...
				len += sprintf(page + len,
					"\tTos:%x  TosM:%x  sportL:%d  sportU:%d  dportL:%d  dportU:%d\n",
					asic_acl.tos_, asic_acl.tosMask_,
					asic_acl.udpSrcPortLB_,
					asic_acl.udpSrcPortUB_,
					asic_acl.udpDstPortLB_,
					asic_acl.udpDstPortUB_);
				break;
			...
			case RTL865X_ACL_SRCFILTER:
				...
				len += sprintf(page + len,
					"\tSMAC: %x:%x:%x:%x:%x:%x  SMACM: %x:%x:%x:%x:%x:%x\n",
					asic_acl.srcFilterMac_.octet[0],
					asic_acl.srcFilterMac_.octet[1],
					asic_acl.srcFilterMac_.octet[2],
					asic_acl.srcFilterMac_.octet[3],
					asic_acl.srcFilterMac_.octet[4],
					asic_acl.srcFilterMac_.octet[5],
					asic_acl.srcFilterMacMask_.octet[0],
					asic_acl.srcFilterMacMask_.octet[1],
					asic_acl.srcFilterMacMask_.octet[2],
					asic_acl.srcFilterMacMask_.octet[3],
					asic_acl.srcFilterMacMask_.octet[4],
					asic_acl.srcFilterMacMask_.octet[5]
					);
				len += sprintf(page + len,
					"\tsvidx: %d   svidxM: %x   sport: %d   sportM: %x   ProtoType: %x\n",
					asic_acl.srcFilterVlanIdx_,
					asic_acl.srcFilterVlanIdxMask_,
					asic_acl.srcFilterPort_,
				...
				len += sprintf(page + len,
					...
					((asic_acl.srcFilterIpAddr_ & 0x00ff0000) >> 16),
					((asic_acl.srcFilterIpAddr_ & 0x0000ff00) >> 8),
					(asic_acl.srcFilterIpAddr_ & 0xff),
					(asic_acl.srcFilterIpAddrMask_ >> 24),
					((asic_acl.srcFilterIpAddrMask_ & 0x00ff0000) >> 16),
					((asic_acl.srcFilterIpAddrMask_ & 0x0000ff00) >> 8),
			...
			case RTL865X_ACL_DSTFILTER:
				...
				len += sprintf(page + len,
					"\tDMAC: %x:%x:%x:%x:%x:%x  DMACM: %x:%x:%x:%x:%x:%x\n",
					asic_acl.dstFilterMac_.octet[0],
					asic_acl.dstFilterMac_.octet[1],
					asic_acl.dstFilterMac_.octet[2],
					asic_acl.dstFilterMac_.octet[3],
					asic_acl.dstFilterMac_.octet[4],
					asic_acl.dstFilterMac_.octet[5],
					asic_acl.dstFilterMacMask_.octet[0],
					asic_acl.dstFilterMacMask_.octet[1],
					asic_acl.dstFilterMacMask_.octet[2],
					asic_acl.dstFilterMacMask_.octet[3],
					asic_acl.dstFilterMacMask_.octet[4],
					asic_acl.dstFilterMacMask_.octet[5]
					);
				len += sprintf(page + len,
					"\tdvidx: %d   dvidxM: %x  ProtoType: %x   dportL: %d   dportU: %d\n",
					asic_acl.dstFilterVlanIdx_,
					asic_acl.dstFilterVlanIdxMask_,
					(asic_acl.dstFilterIgnoreL3L4_ == TRUE ? 2 : (asic_acl.dstFilterIgnoreL4_ == 1 ? 1 : 0)),
					asic_acl.dstFilterPortLowerBound_,
					asic_acl.dstFilterPortUpperBound_);
				len += sprintf(page + len,
					"\tdip: %d.%d.%d.%d   dipM: %d.%d.%d.%d\n",
					(asic_acl.dstFilterIpAddr_ >> 24),
					((asic_acl.dstFilterIpAddr_ & 0x00ff0000) >> 16),
					((asic_acl.dstFilterIpAddr_ & 0x0000ff00) >> 8),
					(asic_acl.dstFilterIpAddr_ & 0xff),
					(asic_acl.dstFilterIpAddrMask_ >> 24),
					((asic_acl.dstFilterIpAddrMask_ & 0x00ff0000) >> 16),
					((asic_acl.dstFilterIpAddrMask_ & 0x0000ff00) >> 8),
			...
			case RTL865X_ACL_MIRROR_KEEP_MATCH:
				len += sprintf(page + len,
					"\tnetifIdx: %d   pppoeIdx: %d   l2Idx:%d  ",
					asic_acl.netifIdx_, asic_acl.pppoeIdx_,
					asic_acl.L2Idx_);
				break;

			case RTL865X_ACL_PRIORITY:
				len += sprintf(page + len, "\tprioirty: %d   ",
					asic_acl.priority_);
				break;

			case RTL865X_ACL_DEFAULT_REDIRECT:
				len += sprintf(page + len, "\tnextHop:%d  ",
					asic_acl.nexthopIdx_);
				break;
...
#else
		for (; acl_start <= acl_end; acl_start++) {
			if (rtl8651_getAsicAclRule(acl_start,
					&asic_acl) == FAILED)
				rtlglue_printf("=============%s(%d): get asic acl rule error!\n",
					__FUNCTION__, __LINE__);
			...
			case RTL8651_ACL_MIRROR_KEEP_MATCH:	/* 0x09 */
				len += sprintf(page + len,
					"\tdvidx: %d   hp: %d   pppoeIdx: %d   nxtHop:%d  ",
...
static int32 acl_write(struct file *filp, const char *buff, unsigned long len,
	void *data)
{
...
#else
static int32 advRt_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	int len = 0;
...
static int32 advRt_write(struct file *filp, const char *buff, unsigned long len,
	void *data)
{
	char tmpbuf[32];
...
int acl_show(struct seq_file *s, void *v)
{
	...
	for (vid = 0; vid < 8; vid++) {
		...
		acl_start = asic_intf.inAclStart;
		acl_end = asic_intf.inAclEnd;
		seq_printf(s, "\nacl_start(%d), acl_end(%d)", acl_start,
			acl_end);
again:
		if (outRule == FALSE)
			seq_printf(s,
		...
		for (; acl_start <= acl_end; acl_start++) {
			if (_rtl865x_getAclFromAsic(acl_start,
					&asic_acl) == FAILED)
				seq_printf(s,
					"=============%s(%d): get asic acl rule error!\n",
					__FUNCTION__, __LINE__);
			...
			case RTL865X_ACL_IPV6_RANGE:
				...
				if (acl_temp <= acl_end) {	/* it should be less than or equal to acl_end. */
					memset(&asic_acl2, 0x00,
						sizeof(rtl865x_AclRule_t));
					if (_rtl865x_getAclFromAsic(acl_temp,
							&asic_acl2) == FAILED)
						seq_printf(s,
							"=============%s(%d): get asic acl rule error!\n",
							__FUNCTION__, __LINE__);
					if ((!asic_acl2.ipv6EntryType_)
						&& asic_acl.ipv6EntryType_) {
						asic_acl.dstIpV6Addr_.v6_addr32[3] =
							asic_acl2.dstIpV6Addr_.v6_addr32[3];
						asic_acl.dstIpV6Addr_.v6_addr32[2] =
							asic_acl2.dstIpV6Addr_.v6_addr32[2];
						asic_acl.dstIpV6Addr_.v6_addr32[1] =
							asic_acl2.dstIpV6Addr_.v6_addr32[1];
						asic_acl.dstIpV6Addr_.v6_addr32[0] =
							asic_acl2.dstIpV6Addr_.v6_addr32[0];
						...
						asic_acl.ipv6TrafficClass_ =
							asic_acl2.ipv6TrafficClass_;
						asic_acl.ipv6TrafficClassM_ =
							asic_acl2.ipv6TrafficClassM_;
						asic_acl.ipv6NextHeader_ =
							asic_acl2.ipv6NextHeader_;
						asic_acl.ipv6NextHeaderM_ =
							asic_acl2.ipv6NextHeaderM_;
						asic_acl.ipv6HttpFilter_ =
							asic_acl2.ipv6HttpFilter_;
						asic_acl.ipv6HttpFilterM_ =
							asic_acl2.ipv6HttpFilterM_;
						asic_acl.ipv6IdentSrcDstIp_ =
							asic_acl2.ipv6IdentSrcDstIp_;
						asic_acl.ipv6IdentSrcDstIpM_ =
							asic_acl2.ipv6IdentSrcDstIpM_;
						/* ActionType and ActionField useless in entry0 */
						asic_acl.actionType_ =
							asic_acl2.actionType_;
						switch (asic_acl.actionType_) {
						...
						if (RTL865X_ACL_IPV6 ==
							asic_acl.ruleType_) {
							memset((void *)
								&start_ip.v6_addr32[0],
								0x00,
								sizeof(start_ip));
							memset((void *)&end_ip.v6_addr32[0],
								0x00,
								sizeof(end_ip));
							ntohl_array((uint32 *)
								&asic_acl.srcIpV6Addr_.v6_addr32[0],
								(void *)&start_ip.v6_addr32[0],
								4);
							ntohl_array((uint32 *)
								&asic_acl.srcIpV6AddrMask_.v6_addr32[0],
								(void *)&end_ip.v6_addr32[0],
								4);
							seq_printf(s,
								"\tsip: %x:%x:%x:%x:%x:%x:%x:%x  sipM: %x:%x:%x:%x:%x:%x:%x:%x\n",
								htons(start_ip.v6_addr16[0]),
								htons(start_ip.v6_addr16[1]),
								htons(start_ip.v6_addr16[2]),
								htons(start_ip.v6_addr16[3]),
								htons(start_ip.v6_addr16[4]),
								htons(start_ip.v6_addr16[5]),
								htons(start_ip.v6_addr16[6]),
								htons(start_ip.v6_addr16[7]),
								htons(end_ip.v6_addr16[0]),
								htons(end_ip.v6_addr16[1]),
								htons(end_ip.v6_addr16[2]),
								htons(end_ip.v6_addr16[3]),
								htons(end_ip.v6_addr16[4]),
								htons(end_ip.v6_addr16[5]),
								htons(end_ip.v6_addr16[6]),
								htons(end_ip.v6_addr16[7])
								);
							...
							ntohl_array((uint32 *)
								...
								4);
							seq_printf(s,
								"\tdip: %x:%x:%x:%x:%x:%x:%x:%x  dipM: %x:%x:%x:%x:%x:%x:%x:%x\n",
								htons(start_ip.v6_addr16[0]),
								htons(start_ip.v6_addr16[1]),
								htons(start_ip.v6_addr16[2]),
								htons(start_ip.v6_addr16[3]),
							...
							memset((void *)&end_ip.v6_addr32[0],
								...
								sizeof(end_ip));
							ntohl_array((uint32 *)
								&asic_acl.srcIpV6AddrLB_.v6_addr32[0],
								(void *)
								&start_ip.v6_addr32[0],
								4);
							ntohl_array((uint32 *)
								&asic_acl.srcIpV6AddrUB_.v6_addr32[0],
								(void *)&end_ip.v6_addr32[0],
								4);
							seq_printf(s,
								"\tsipLB: %x:%x:%x:%x:%x:%x:%x:%x  sipUB: %x:%x:%x:%x:%x:%x:%x:%x\n",
								htons(start_ip.v6_addr16[0]),
								htons(start_ip.v6_addr16[1]),
								htons(start_ip.v6_addr16[2]),
								htons(start_ip.v6_addr16[3]),
							...
							memset((void *)&end_ip.v6_addr32[0],
								...
								sizeof(end_ip));
							ntohl_array((uint32 *)
								&asic_acl.dstIpV6AddrLB_.v6_addr32[0],
								(void *)
								&start_ip.v6_addr32[0],
								4);
							ntohl_array((uint32 *)
								&asic_acl.dstIpV6AddrUB_.v6_addr32[0],
								(void *)&end_ip.v6_addr32[0],
								4);
							seq_printf(s,
								"\tdipLB: %x:%x:%x:%x:%x:%x:%x:%x  dipUB: %x:%x:%x:%x:%x:%x:%x:%x\n",
								htons(start_ip.v6_addr16[0]),
								htons(start_ip.v6_addr16[1]),
								htons(start_ip.v6_addr16[2]),
								htons(start_ip.v6_addr16[3]),
						...
						seq_printf(s,
							"\tFlowLabel: 0x%x   FlowLabelM: 0x%x\n",
							asic_acl.ipv6FlowLabel_,
							asic_acl.ipv6FlowLabelM_);
						seq_printf(s,
							"\tInvert: %d   Combine1: %d   Combine2: %d   IPtunnel: %d\n",
							asic_acl.ipv6Invert_,
							asic_acl.ipv6Combine_,
							asic_acl2.ipv6Combine_,
							asic_acl.ipv6IPtunnel_);
						seq_printf(s,
							...
							asic_acl.ipv6NextHeaderM_);
						seq_printf(s,
							"\tHTTPP: %d   HTTPM: %d   IdentSDIPP: %d   IdentSDIPM: %d\n",
							asic_acl.ipv6HttpFilter_,
							asic_acl.ipv6HttpFilterM_,
							asic_acl.ipv6IdentSrcDstIp_,
							asic_acl.ipv6IdentSrcDstIpM_);
				...
				if (acl_temp <= acl_end) {	/* it should be less than or equal to acl_end. */
					...
					if ((!asic_acl2.ipv6EntryType_)
						...
						seq_printf(s,
							...
							actionT[asic_acl.actionType_]);
						seq_printf(s,
							"\tsportU: %d sportL: %d dportU: %d dportL: %d protocol: %d\n",
							asic_acl.un_ty.PM._spub,
							asic_acl.un_ty.PM._splb,
							asic_acl.un_ty.PM._dpub,
							asic_acl.un_ty.PM._dplb,
							asic_acl.un_ty.PM._protocol);
						seq_printf(s,
							"\tOffset0: %d Pattern0: 0x%04x PatternMask0: 0x%04x OR0: %d\n",
							asic_acl.un_ty.PM._offset0,
							asic_acl.un_ty.PM._pattern0,
							asic_acl.un_ty.PM._pm0,
							asic_acl.un_ty.PM._or0);
						seq_printf(s,
							"\tOffset1: %d Pattern1: 0x%04x PatternMask1: 0x%04x OR1: %d\n",
							asic_acl.un_ty.PM._offset1,
							asic_acl.un_ty.PM._pattern1,
							asic_acl.un_ty.PM._pm1,
							asic_acl.un_ty.PM._or1);
						seq_printf(s,
							"\tOffset2: %d Pattern2: 0x%04x PatternMask2: 0x%04x OR2: %d\n",
							asic_acl.un_ty.PM._offset2,
							asic_acl.un_ty.PM._pattern2,
							asic_acl.un_ty.PM._pm2,
							asic_acl.un_ty.PM._or2);
						seq_printf(s,
							"\tOffset3: %d Pattern3: 0x%04x PatternMask3: 0x%04x OR3: %d\n",
							asic_acl.un_ty.PM._offset3,
							asic_acl.un_ty.PM._pattern3,
							asic_acl.un_ty.PM._pm3,
							asic_acl.un_ty.PM._or3);

						/* output entry1's valid field */
						seq_printf(s,
							"\tOffset4: %d Pattern4: 0x%04x PatternMask4: 0x%04x OR4: %d\n",
							asic_acl2.un_ty.PM._offset4,
							asic_acl2.un_ty.PM._pattern4,
							asic_acl2.un_ty.PM._pm4,
							asic_acl2.un_ty.PM._or4);
						seq_printf(s,
							"\tOffset5: %d Pattern5: 0x%04x PatternMask5: 0x%04x OR5: %d\n",
							asic_acl2.un_ty.PM._offset5,
							asic_acl2.un_ty.PM._pattern5,
							asic_acl2.un_ty.PM._pm5,
							asic_acl2.un_ty.PM._or5);
						seq_printf(s,
							"\tOffset6: %d Pattern6: 0x%04x PatternMask6: 0x%04x OR6: %d\n",
							asic_acl2.un_ty.PM._offset6,
							asic_acl2.un_ty.PM._pattern6,
							asic_acl2.un_ty.PM._pm6,
							asic_acl2.un_ty.PM._or6);
						seq_printf(s,
							"\tOffset7: %d Pattern7: 0x%04x PatternMask7: 0x%04x OR7: %d\n",
							asic_acl2.un_ty.PM._offset7,
							asic_acl2.un_ty.PM._pattern7,
							asic_acl2.un_ty.PM._pm7,
							asic_acl2.un_ty.PM._or7);
						seq_printf(s,
							"\tSpecialOP: %d SrcPortPattern: %d SrcPortMask: %d PPPCTL: %d PPPCTLM: %d PPPCTLOR: %d\n",
							asic_acl2.un_ty.PM._specialop,
							asic_acl2.un_ty.PM._spa,
							asic_acl2.un_ty.PM._spam,
							asic_acl2.un_ty.PM._pppctl,
							asic_acl2.un_ty.PM._pppctlm,
							asic_acl2.un_ty.PM._pppctlor);
						seq_printf(s,
							"\tInvert: %d   Combine1: %d   Combine2: %d   IPtunnel1: %d IPtunnel2: %d IpfragApply: %d\n",
							asic_acl2.ipv6Invert_,
							asic_acl2.ipv6Combine_,
							asic_acl2.ipv6Combine_,
							asic_acl.ipv6IPtunnel_,
							asic_acl2.ipv6IPtunnel_,
							asic_acl2.ipfrag_apply_);
						/* update acl index */
						acl_start = acl_temp;
					}
			...
			case RTL865X_ACL_PRIORITY:
				seq_printf(s, "\tprioirty: %d   ",
					asic_acl.priority_);
				break;

			case RTL865X_ACL_DEFAULT_REDIRECT:
				seq_printf(s, "\tnextHop:%d  ",
					asic_acl.nexthopIdx_);
				break;
			...
			case RTL865X_ACL_DROP_RATE_EXCEED_BPS:
			case RTL865X_ACL_LOG_RATE_EXCEED_BPS:
				seq_printf(s, "\tratelimitIdx: %d  ",
					asic_acl.ratelimtIdx_);
				break;
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
			case RTL865X_ACL_VID:
				seq_printf(s, "\taclvid: %d  ",
					asic_acl.aclvid_);
				break;
#endif
			default:
			...
			if (!((asic_acl.ruleType_ == RTL865X_ACL_IPV6)
					|| (asic_acl.ruleType_ ==
						RTL865X_ACL_IPV6_RANGE)
#if defined(CONFIG_RTL_8197F)
					|| (asic_acl.ruleType_ ==
...
#if defined(CONFIG_RTL_LOCAL_PUBLIC) || defined(CONFIG_RTL_MULTIPLE_WAN) || defined(CONFIG_RTL_HW_VLAN_SUPPORT)
	{
		...
		outRule = FALSE;
		rtl865x_getDefACLForNetDecisionMiss(&defInAclStart,
			&defInAclEnd, &defOutAclStart, &defOutAclEnd);
...
static int32 hs_read(struct seq_file *s, void *v)
{
	...
	virtualMacGetHsb(hsb_r);
	{
		...
		/* Protocol contents */
		seq_printf(s, "\ttagif:%d\tpppoeId:%d", hsb_r->tagif,
			hsb_r->pppoeid);
		seq_printf(s, "\tethrtype:0x%04x\n", hsb_r->ethtype);
		seq_printf(s, "\tllc_other:%d\tsnap:%d\n", hsb_r->llcothr,
			hsb_r->snap);
		seq_printf(s, "\tda:%02x-%02x-%02x-%02x-%02x-%02x",
			hsb_r->da[0], hsb_r->da[1], hsb_r->da[2], hsb_r->da[3],
			hsb_r->da[4], hsb_r->da[5]);
		seq_printf(s, "\tsa:%02x-%02x-%02x-%02x-%02x-%02x\n",
			hsb_r->sa[0], hsb_r->sa[1], hsb_r->sa[2], hsb_r->sa[3],
			hsb_r->sa[4], hsb_r->sa[5]);
		...
		seq_printf(s,
			"\tsip6:\t%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n",
		...
		seq_printf(s,
			"\tdip6:\t%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n",
	...
	virtualMacGetHsa(hsa_r);
	{
		...
		seq_printf(s, "\tmac:%02x-%02x-%02x-%02x-%02x-%02x\n",
			hsa_r->nhmac[0], hsa_r->nhmac[1], hsa_r->nhmac[2],
			hsa_r->nhmac[3], hsa_r->nhmac[4], hsa_r->nhmac[5]);
...
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
		seq_printf(s, "\tspa:%d \tipv4_1st:%d", hsa_r->spao,
			hsa_r->ipv4_1st);
#else
...
#else
static int32 hs_read(char *page, char **start, off_t off, int count, int *eof,
	void *data)
{
	int len;
	hsb_param_t *hsb_r, dummy_hsb_r;
	...
	virtualMacGetHsb(hsb_r);
	{
		...
		/* Protocol contents */
		len += sprintf(page + len, "\ttagif:%d\tpppoeId:%d",
			hsb_r->tagif, hsb_r->pppoeid);
		len += sprintf(page + len, "\tethrtype:0x%04x\n",
			hsb_r->ethtype);
		len += sprintf(page + len, "\tllc_other:%d\tsnap:%d\n",
			hsb_r->llcothr, hsb_r->snap);
		len += sprintf(page + len, "\tda:%02x-%02x-%02x-%02x-%02x-%02x",
			hsb_r->da[0], hsb_r->da[1], hsb_r->da[2], hsb_r->da[3],
			hsb_r->da[4], hsb_r->da[5]);
		len += sprintf(page + len,
			"\tsa:%02x-%02x-%02x-%02x-%02x-%02x\n", hsb_r->sa[0],
			hsb_r->sa[1], hsb_r->sa[2], hsb_r->sa[3], hsb_r->sa[4],
			hsb_r->sa[5]);

		addr = ntohl(hsb_r->sip);
		inet_ntoa_r(addr, addr_s);
		len += sprintf(page + len, "\tsip:%s(hex:%08x)   ", addr_s,
			hsb_r->sip);
		len += sprintf(page + len, "\tsprt:%d (hex:%x)\n ",
			(int)hsb_r->sprt, hsb_r->sprt);
		addr = ntohl(hsb_r->dip);
		inet_ntoa_r(addr, addr_s);
		len += sprintf(page + len, "\tdip:%s(hex:%08x) ", addr_s,
			hsb_r->dip);
		...
		len += sprintf(page + len,
			"\tl3csok:%d\tl4csok:%d\tipfragif:%d\n", hsb_r->l3csok,
			hsb_r->l4csok, hsb_r->ipfo0_n);
		...
		len += sprintf(page + len, "\thop_limit:0x%x",
			hsb_r->hop_limit);
	...
	virtualMacGetHsa(hsa_r);
	{
		...
		addr = ntohl(hsa_r->trip);
		inet_ntoa_r(addr, addr_s);
		len += sprintf(page + len, "\ttrip:%s(hex:%08x)", addr_s,
			hsa_r->trip);
		...
		len += sprintf(page + len, "\tdvid:%d(0x%x)", hsa_r->dvid,
			hsa_r->dvid);
...
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
		len += sprintf(page + len, "\tspa:%d \tipv4_1st:%d",
			hsa_r->spao, hsa_r->ipv4_1st);
#else
...
*/
		...
		len += sprintf(page + len, "\tmltcst_v6:0x%x",
			hsa_r->mltcst_v6);
...
static int32 hs_write(struct file *filp, const char *buff, unsigned long len,
	void *data)
{
	return len;
}
...
static int32 rxRing_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	char tmpbuf[256];
	char *strptr;
...
#if defined(CONFIG_RTD_1295_HWNAT)
ssize_t rxRing_single_write(struct file *file, const char __user *userbuf,
	size_t count, loff_t *off)
{
	return (ssize_t) rxRing_write(file, userbuf, count, off);
}
#else /* defined(CONFIG_RTD_1295_HWNAT) */
...
#else
static int32 pvid_read(char *page, char **start, off_t off, int count, int *eof,
	void *data)
{
	uint32 vidp[9];
	int32 i;
...
static int32 pvid_write(struct file *filp, const char *buff, unsigned long len,
	void *data)
{
	char tmpbuf[256];
	char *strptr;
...
#else
static int32 mirrorPort_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	uint32 mirrorRx, mirrorTx, mirrorPort;
	int len;
...
static int32 mirrorPort_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	char tmpbuf[64];
...
static int32 l2_read(struct seq_file *s, void *v)
{
	...
	for (row = 0x0; row < RTL8651_L2TBL_ROW; row++) {
		for (col = 0; col < RTL8651_L2TBL_COLUMN; col++) {
			...
			if (rtl8651_getAsicL2Table(row, col,
					&asic_l2) == FAILED) {
				continue;
			}

			if (asic_l2.isStatic && asic_l2.ageSec == 0
				&& asic_l2.cpu && asic_l2.memberPortMask == 0
				&& asic_l2.auth == 0) {
				continue;
			}

			seq_printf(s,
				"%4d.[%3d,%d] %02x:%02x:%02x:%02x:%02x:%02x FID:%x mbr(",
				m, row, col, asic_l2.macAddr.octet[0],
				asic_l2.macAddr.octet[1],
				asic_l2.macAddr.octet[2],
				asic_l2.macAddr.octet[3],
				asic_l2.macAddr.octet[4],
				asic_l2.macAddr.octet[5], asic_l2.fid);
			...
			seq_printf(s, ")");
			seq_printf(s, "%s %s %s %s age:%d ",
				asic_l2.cpu ? "CPU" : "FWD",
				asic_l2.isStatic ? "STA" : "DYN",
				asic_l2.srcBlk ? "BLK" : "",
				asic_l2.nhFlag ? "NH" : "", asic_l2.ageSec);
...
#else
static int32 l2_read(char *page, char **start, off_t off, int count, int *eof,
	void *data)
{
	int len;
	rtl865x_tblAsicDrv_l2Param_t asic_l2;
	...
	for (row = 0x0; row < RTL8651_L2TBL_ROW; row++) {
		for (col = 0; col < RTL8651_L2TBL_COLUMN; col++) {
			...
			len += sprintf(page + len, ")");
			len += sprintf(page + len, "%s %s %s %s age:%d ",
				asic_l2.cpu ? "CPU" : "FWD",
				asic_l2.isStatic ? "STA" : "DYN",
				asic_l2.srcBlk ? "BLK" : "",
				asic_l2.nhFlag ? "NH" : "", asic_l2.ageSec);
...
#endif
static int32 l2_write(struct file *filp, const char *buff, unsigned long len,
	void *data)
{
	return len;
}
...
static int32 rtl865x_proc_hw_mcast_read(struct seq_file *s, void *v)
{
	...
	for (entry = 0; entry < RTL8651_MULTICASTTBL_SIZE; entry++) {
		if (rtl8651_getAsicIpMulticastTable(entry, &asic) != SUCCESS) {
#if 0
			seq_printf(s,
				"\t[%d]  (INVALID)dip(%d.%d.%d.%d) sip(%d.%d.%d.%d) mbr(0x%x)\n",
				entry, asic.dip >> 24,
				(asic.dip & 0x00ff0000) >> 16,
				(asic.dip & 0x0000ff00) >> 8, (asic.dip & 0xff),
				asic.sip >> 24, (asic.sip & 0x00ff0000) >> 16,
				(asic.sip & 0x0000ff00) >> 8, (asic.sip & 0xff),
				asic.mbr);
			seq_printf(s,
				"\t       svid:%d, spa:%d, extIP:%d, age:%d, cpu:%d\n",
				asic.svid, asic.port, asic.extIdx, asic.age,
				asic.cpu);
#endif
			continue;
		} else {
			...
			seq_printf(s,
				"\t[%d]  (OK)dip(%d.%d.%d.%d) sip(%d.%d.%d.%d) mbr(0x%x)\n",
				entry, ucdip[0], ucdip[1], ucdip[2], ucdip[3],
	...
	for (entry = 0; entry < RTL8651_MULTICASTTBL_SIZE; entry++) {
		...
		seq_printf(s,
			"\t[%d]  (OK)dip(%d.%d.%d.%d) sip(%d.%d.%d.%d) mbr(%x)\n",
			entry, asic.dip >> 24, (asic.dip & 0x00ff0000) >> 16,
			(asic.dip & 0x0000ff00) >> 8, (asic.dip & 0xff),
			asic.sip >> 24, (asic.sip & 0x00ff0000) >> 16,
			(asic.sip & 0x0000ff00) >> 8, (asic.sip & 0xff),
			asic.mbr);
		seq_printf(s,
			"\t       svid:%d, spa:%d, extIP:%d, age:%d, cpu:%d\n",
			asic.svid, asic.port, asic.extIdx, asic.age, asic.cpu);
	}
	seq_printf(s,
		"\n\t TotalOpCnt:AddMcastOpCnt:%d\tDelMcastOpCnt:%d\tForceAddMcastOpCnt:%d\t \n",
		_rtl865x_getAddMcastOpCnt(), _rtl865x_getDelMcastOpCnt(),
	...
	for (entry = 0; entry < RTL8651_MULTICASTTBL_SIZE; entry++) {
		...
		rtl_periodic_watchdog_kick(entry, WATCHDOG_NUM_OF_TIMES);
#endif
		if (rtl8198C_getAsicIpMulticastv6Table(entry,
				&asic6) != SUCCESS) {
#if 0
			seq_printf(s,
				"\t[%d]  (INVALID)dip(%d.%d.%d.%d) sip(%d.%d.%d.%d) mbr(0x%x)\n",
			...
			seq_printf(s,
				"\t          spa:%d, age:%d, cpu:%d, mbr(0x%x)\n",
				asic6.port, asic6.age, asic6.cpu, asic6.mbr);
		}
	}
	seq_printf(s,
		"\n\t TotalOpCntv6:AddMcastOpCntv6:%d\tDelMcastOpCntv6:%d\tForceAddMcastOpCntv6:%d\t \n",
		_rtl8198C_getAddMcastv6OpCnt(), _rtl8198C_getDelMcastv6OpCnt(),
		_rtl8198C_getForceAddMcastv6OpCnt());
#endif
	return 0;
}
#else
static int32 rtl865x_proc_hw_mcast_read(char *page, char **start, off_t off,
	int count, int *eof, void *data)
{
	int len = 0;
	rtl865x_tblAsicDrv_multiCastParam_t asic;
	...
	for (entry = 0; entry < RTL8651_MULTICASTTBL_SIZE; entry++) {
		...
		if (rtl8198C_getAsicIpMulticastv6Table(entry,
			...
			len += sprintf(page + len,
				...
				entry, asic.dip >> 24,
				(asic.dip & 0x00ff0000) >> 16,
				(asic.dip & 0x0000ff00) >> 8,
				(asic.dip & 0xff), asic.sip >> 24,
				(asic.sip & 0x00ff0000) >> 16,
				(asic.sip & 0x0000ff00) >> 8,
				(asic.sip & 0xff), asic.mbr);
			len += sprintf(page + len,
				"\t		 svid:%d, spa:%d, extIP:%d, age:%d, cpu:%d\n",
			...
			len += sprintf(page + len,
				"\t[%d]	(OK)dip:%08x-%08x-%08x-%08x\n",
				entry, asic6.dip.v6_addr32[0],
				asic6.dip.v6_addr32[1],
				asic6.dip.v6_addr32[2], asic6.dip.v6_addr32[3]);
			len += sprintf(page + len,
				"\t			sip:%08x-%08x-%08x-%08x\n",
				asic6.sip.v6_addr32[0],
				asic6.sip.v6_addr32[1],
				asic6.sip.v6_addr32[2], asic6.sip.v6_addr32[3]);
	...
	for (entry = 0; entry < RTL8651_MULTICASTTBL_SIZE; entry++) {
		...
		len += sprintf(page + len,
			"\t[%d]  dip(%d.%d.%d.%d) sip(%d.%d.%d.%d) mbr(%x)\n",
			entry, asic.dip >> 24, (asic.dip & 0x00ff0000) >> 16,
			(asic.dip & 0x0000ff00) >> 8, (asic.dip & 0xff),
			asic.sip >> 24, (asic.sip & 0x00ff0000) >> 16,
			(asic.sip & 0x0000ff00) >> 8, (asic.sip & 0xff),
...
static int32 rtl865x_proc_hw_mcast_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	char tmpbuf[512];
	char *strptr;
...
#else
static int32 rtl865x_proc_sw_mcast_read(char *page, char **start, off_t off,
	int count, int *eof, void *data)
{
	int len = 0;
...
static int32 rtl865x_proc_sw_mcast_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
...
static int32 arp_read(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < RTL8651_ARPTBL_SIZE; i++) {
		...
		for (j = 0; j < RTL8651_ROUTINGTBL_SIZE; j++) {
			if (rtl8651_getAsicRouting(j, &asic_l3) == FAILED
				|| asic_l3.process != 0x02 /*RT_ARP */)
				continue;
			if (asic_l3.arpStart <= (i >> 3)
				&& (i >> 3) <= asic_l3.arpEnd) {
				...
				ipAddr = (asic_l3.ipAddr & asic_l3.ipMask) +
					(i - (asic_l3.arpStart << 3));

				/* 8197F: change to little endian format for display (inet_ntoa_r) */
				ipAddr = ntohl(ipAddr);

				if (rtl8651_getAsicL2Table_Patch(asic_arp.nextHopRow,
						asic_arp.nextHopColumn,
						&asic_l2) == FAILED) {
					inet_ntoa_r(ipAddr, ipBuf);
					seq_printf(s, "%-16s [%3d,%d] ", ipBuf,
						asic_arp.nextHopRow,
						asic_arp.nextHopColumn);
				} else {
					inet_ntoa_r(ipAddr, ipBuf);
					seq_printf(s,
						...
						ipBuf, asic_l2.macAddr.octet[0],
						asic_l2.macAddr.octet[1],
						asic_l2.macAddr.octet[2],
						asic_l2.macAddr.octet[3],
						asic_l2.macAddr.octet[4],
						asic_l2.macAddr.octet[5]);
					for (port = 0;
						port <
						RTL8651_PORT_NUMBER +
						rtl8651_totalExtPortNum;
						port++) {
						if (asic_l2.memberPortMask & (1 << port))
							seq_printf(s, "%d ", port);
						else
		...
		seq_printf(s, " ARP:%3d  L2:%3d,%d,aging:%d\n", i,
			asic_arp.nextHopRow, asic_arp.nextHopColumn,
			asic_arp.aging);
...
#else
static int32 arp_read(char *page, char **start, off_t off, int count, int *eof,
	void *data)
{
	int len;
	rtl865x_tblAsicDrv_routingParam_t asic_l3;
...
static int32 arp_write(struct file *filp, const char *buff, unsigned long len,
	void *data)
{
	return len;
}
...
static int32 nexthop_read(struct seq_file *s, void *v)
{
	...
	for (idx = 0; idx < RTL8651_NEXTHOPTBL_SIZE; idx++) {
		...
		if (rtl8651_getAsicNextHopTable(idx, &asic_nxthop) == FAILED)
			continue;
		seq_printf(s,
			"  [%d]  type(%s) IPIdx(%d) dstVid(%d) pppoeIdx(%d) nextHop([%d:%d]%d) rf(%d) rt(%d)\n",
			idx,
			(asic_nxthop.isPppoe == TRUE ? "pppoe" : "ethernet"),
			asic_nxthop.extIntIpIdx, asic_nxthop.dvid,
			asic_nxthop.pppoeIdx, asic_nxthop.nextHopRow,
			asic_nxthop.nextHopColumn,
			(asic_nxthop.nextHopRow << 2) +
			asic_nxthop.nextHopColumn, refcnt, rt_flag);
	}

	return 0;
}
#else
static int32 nexthop_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	int len;
	rtl865x_tblAsicDrv_nextHopParam_t asic_nxthop;
	...
	for (idx = 0; idx < RTL8651_NEXTHOPTBL_SIZE; idx++) {
		...
		if (rtl8651_getAsicNextHopTable(idx, &asic_nxthop) == FAILED)
			continue;
		len += sprintf(page + len,
			"  [%d]  type(%s) IPIdx(%d) dstVid(%d) pppoeIdx(%d) nextHop(%d) rf(%d) rt(%d)\n",
			idx,
			(asic_nxthop.isPppoe == TRUE ? "pppoe" : "ethernet"),
			asic_nxthop.extIntIpIdx, asic_nxthop.dvid,
			asic_nxthop.pppoeIdx,
			(asic_nxthop.nextHopRow << 2) +
			asic_nxthop.nextHopColumn, refcnt, rt_flag);
	}
...
static int32 nexthop_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	return len;
}
...
static int32 l3_read(struct seq_file *s, void *v)
{
	...
	for (idx = 0; idx < RTL8651_ROUTINGTBL_SIZE; idx++) {
		...
		if (idx == RTL8651_ROUTINGTBL_SIZE - 1)
			...
			for (mask = 32; !(asic_l3.ipMask & 0x01);
				asic_l3.ipMask = asic_l3.ipMask >> 1)
				mask--;
...
/* mask, str[asic_l3.process],strNetType[netIdx],asic_l3.DSLEG,asic_l3.DSL_IDX); */
...
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
		seq_printf(s,
			"\t[%d]  %d.%d.%d.%d/%d process(%s) %s DSLEG(%d),DSL_IDX(%d)\n",
			idx, ucp[0], ucp[1], ucp[2], ucp[3], mask,
			str[asic_l3.process], strNetType[netIdx], asic_l3.DSLEG,
			asic_l3.DSL_IDX);
#else
		seq_printf(s, "\t[%d]  %d.%d.%d.%d/%d process(%s) %s \n", idx,
			ucp[0], ucp[1], ucp[2], ucp[3], mask,
			str[asic_l3.process], strNetType[netIdx]);
#endif
		switch (asic_l3.process) {
		case 0x00:	/* PPPoE */
			seq_printf(s,
				"\t           dvidx(%d)  pppidx(%d) nxthop(%d)\n",
				asic_l3.vidx, asic_l3.pppoeIdx,
				(asic_l3.nextHopRow << 2) +
				asic_l3.nextHopColumn);
			break;
		...
		case 0x02:	/* ARP */
			seq_printf(s,
				"             dvidx(%d) ARPSTA(%d) ARPEND(%d) IPIDX(%d)\n",
				asic_l3.vidx, asic_l3.arpStart << 3,
				asic_l3.arpEnd << 3, asic_l3.arpIpIdx);
			break;
		...
		case 0x05:	/* NAPT Next Hop */
			seq_printf(s,
				"              NHSTA(%d) NHNUM(%d) NHNXT(%d) NHALGO(%d) IPDOMAIN(%d)\n",
				asic_l3.nhStart, asic_l3.nhNum, asic_l3.nhNxt,
				asic_l3.nhAlgo, asic_l3.ipDomain);
			break;
...
#else
static int32 l3_read(char *page, char **start, off_t off, int count, int *eof,
	void *data)
{
	int len;
	rtl865x_tblAsicDrv_routingParam_t asic_l3;
	...
	for (idx = 0; idx < RTL8651_ROUTINGTBL_SIZE; idx++) {
		...
		netIdx = asic_l3.internal << 1 | asic_l3.DMZFlag;
		len += sprintf(page + len,
			"\t[%d]  %d.%d.%d.%d/%d process(%s) %s \n", idx,
			(asic_l3.ipAddr >> 24),
			((asic_l3.ipAddr & 0x00ff0000) >> 16),
			((asic_l3.ipAddr & 0x0000ff00) >> 8),
			(asic_l3.ipAddr & 0xff), mask, str[asic_l3.process],
			strNetType[netIdx]);
...
static int32 l3_write(struct file *filp, const char *buff, unsigned long len,
	void *data)
{
	return len;
}
...
static int32 ip_read(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < RTL8651_IPTABLE_SIZE; i++) {
		...
		seq_printf(s,
			"  [%d] intip(%-14s) extip(%-14s) type(%s) nhIdx(%d)\n",
			i, intIpBuf, extIpBuf,
			(asic_ip.localPublic == TRUE ? "LP" : (asic_ip.nat ==
					TRUE ? "NAT" : "NAPT")),
			asic_ip.nhIndex);
	}
...
#else
static int32 ip_read(char *page, char **start, off_t off, int count, int *eof,
	void *data)
{
	int len;
...
static int32 ip_write(struct file *filp, const char *buff, unsigned long len,
	void *data)
{
	return len;
}
...
static int32 pppoe_read(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < RTL8651_PPPOETBL_SIZE; i++) {
		...
		seq_printf(s, "\t[%d]  sessionID(%d)  ageSec(%d)\n", i,
			asic_pppoe.sessionId, asic_pppoe.age);
	}
	return 0;
}
#else
static int32 pppoe_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	int len;
	rtl865x_tblAsicDrv_pppoeParam_t asic_pppoe;
...
static int32 pppoe_write(struct file *filp, const char *buff, unsigned long len,
	void *data)
{
	return len;
}
...
/* asic_tcpudp.isCollision, asic_tcpudp.isCollision2, asic_tcpudp.isStatic, asic_tcpudp.isTcp ); */
			asic_tcpudp.insideLocalIpAddr =
				ntohl(asic_tcpudp.insideLocalIpAddr);
			ucp = (unsigned char *)&(asic_tcpudp.insideLocalIpAddr);
			seq_printf(s,
				"[%4d] %d.%d.%d.%d:%d {V,D}={%d,%d} col1(%d) col2(%d) static(%d) tcp(%d)\n",
				idx, ucp[0], ucp[1], ucp[2], ucp[3],
				asic_tcpudp.insideLocalPort,
				asic_tcpudp.isValid, asic_tcpudp.isDedicated,
				asic_tcpudp.isCollision,
				asic_tcpudp.isCollision2, asic_tcpudp.isStatic,
				asic_tcpudp.isTcp);

			seq_printf(s,
				"   age(%d) offset(%d) tcpflag(%d) SelEIdx(%d) SelIPIdx(%d) priValid:%d pri(%d)\n",
				asic_tcpudp.ageSec, asic_tcpudp.offset << 10,
				asic_tcpudp.tcpFlag, asic_tcpudp.selEIdx,
				asic_tcpudp.selExtIPIdx, asic_tcpudp.priValid,
				asic_tcpudp.priority);
...
extern int32 rtl865x_sw_napt_seq_write(struct file *filp, const char *buff,
...
static int32 port_bandwidth_read(struct seq_file *s, void *v)
{
	...
	for (port = 0; port <= CPU; port++) {
		...
		if (port & 1) {
			data0 = (regData & IBWC_ODDPORT_MASK) >>
				IBWC_ODDPORT_OFFSET;
		} else {
			data0 = (regData & IBWC_EVENPORT_MASK) >>
				IBWC_EVENPORT_OFFSET;
		}
		...
		pBandwidth_high =
			(regData2 & (IBCR3_PORT_MASK <<
				IBCR3_PORT_OFFSET(port))) >>
			IBCR3_PORT_OFFSET(port);
		data0 = (pBandwidth_high << IBCR3_HIGH_OFFSET) | data0;

		if (data0) {
			seq_printf(s, "Port%d Ingress:[%d.%03dMbps]	", port,
				(data0 << 10) / 1000000,
				((data0 << 10) % 1000000) / 1000);
		} else {
		...
		if (data0) {
			if (data0 < 64)
				seq_printf(s, "Port%d Ingress:[%dKbps]	", port,
					(data0 << 4) / 1000);
			else
				seq_printf(s,
					"Port%d Ingress:[%d.%03dMbps]	",
					port, (data0 << 14) / 1000000,
					((data0 << 14) % 1000000) / 1000);
		} else {
			seq_printf(s, "Port%d Ingress:FullSpeed	", port);
		}
...
#else
static int32 port_bandwidth_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	int len;
	uint32 regData;
	...
	for (port = 0; port <= CPU; port++) {
		...
		if (data0) {
			len += sprintf(page + len,
				"Port%d Ingress:[%d.%03dMbps]	", port,
				(data0 << 10) / 1000000,
				((data0 << 10) % 1000000) / 1000);
		} else {
			len += sprintf(page + len,
				"Port%d Ingress:FullSpeed	", port);
		}
...
static int32 port_bandwidth_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	return len;
}
...
static int32 queue_bandwidth_read(struct seq_file *s, void *v)
{
	...
	for (port = PHY0; port <= CPU; port++) {
		...
		for (queue = QUEUE0; queue <= QUEUE5; queue++) {
			rtl8651_getAsicQueueFlowControlConfigureRegister(port,
				queue, &data0);
			seq_printf(s, "	==>Queue%d FC %s | ", queue,
			...
			if (data0 != (APR_MASK >> APR_OFFSET)) {
				...
				if (data0 < 16)
					seq_printf(s,
						"Engress: avgRate[%dKbps], peakRate[%d]\n",
						data0 << 6, 1 << data1);
				else
					seq_printf(s,
						"Engress: avgRate[%d.%3dMbps], peakRate[%d]\n",
						data0 >> 4, (data0 & 0xf) << 6,
						1 << data1);

			} else
				seq_printf(s,
					"Egress: avgRate & peakRateFullSpeed\n");
...
#else
static int32 queue_bandwidth_read(char *page, char **start, off_t off,
	int count, int *eof, void *data)
{
	int len;
	uint32 regData;
	...
	for (port = PHY0; port <= CPU; port++) {
		...
		for (queue = QUEUE0; queue <= QUEUE5; queue++) {
			rtl8651_getAsicQueueFlowControlConfigureRegister(port,
				queue, &data0);
			len += sprintf(page + len, "	==>Queue%d FC %s | ",
				queue, data0 ? "Enabled" : "Disabled");
...
static int32 queue_bandwidth_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		queue_bandwidth_record_portmask =
			(uint32)simple_strtol(tokptr, NULL, 0);
...
static int32 priority_decision_read(struct seq_file *s, void *v)
{
	...
	for (queue = 0; queue < RTL8651_OUTPUTQUEUE_SIZE; queue++) {
		if (queue < 6) {
			...
			seq_printf(s,
				"Piority[0~7] Mapping to Queue[ %d %d %d %d %d %d %d %d ]\n",
				regData & 0x7, (regData >> 3) & 0x7,
				(regData >> 6) & 0x7, (regData >> 9) & 0x7,
				(regData >> 12) & 0x7, (regData >> 15) & 0x7,
				(regData >> 18) & 0x7, (regData >> 21) & 0x7);
		} else {
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
...
#else
static int32 priority_decision_read(char *page, char **start, off_t off,
	int count, int *eof, void *data)
{
	int len;
	uint32 regData;
	...
	len += sprintf(page + len,
		"NAPT[%d] ACL[%d] DSCP[%d] 8021Q[%d] PortBase[%d]\n",
		(regData & NAPT_PRI_MASK) >> NAPT_PRI_OFFSET,
		(regData & ACL_PRI_MASK) >> ACL_PRI_OFFSET,
		(regData & DSCP_PRI_MASK) >> DSCP_PRI_OFFSET,
		(regData & BP8021Q_PRI_MASK) >> BP8021Q_PRI_OFFSET,
...
static int32 priority_decision_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	return len;
}
...
static int32 rate_limit_read(struct seq_file *s, void *v)
{
	...
	for (entry = 0; entry < RTL8651_RATELIMITTBL_SIZE; entry++) {
		if (rtl8651_getAsicRateLimitTable(entry, &asic_rl) == SUCCESS) {
			seq_printf(s,
				" [%d]  Token(%u)  MaxToken(%u)  remainTime Unit(%u)  \n\trefillTimeUnit(%u)  refillToken(%u)\n",
				entry, asic_rl.token, asic_rl.maxToken,
				asic_rl.t_remainUnit, asic_rl.t_intervalUnit,
				asic_rl.refill_number);
		} else
...
#else
static int32 rate_limit_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	int len;
	rtl865x_tblAsicDrv_rateLimitParam_t asic_rl;
...
static int32 rate_limit_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	return len;
}
...
static int32 storm_read(struct seq_file *s, void *v)
{
	...
	for (port = 0; port < RTL8651_PORT_NUMBER + totalExtPortNum; port++) {
		...
		seq_printf(s, "port%d, %s BCSC, %s BC, %s MC\n", port,
			regData & ENBCSC ? "enable" : "disable",
			regData & BCSC_ENBROADCAST ? "enable" : "disable",
			regData & BCSC_ENMULTICAST ? "enable" : "disable");
	}
	return 0;
}
#else
static int32 storm_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	int len;
	uint32 regData;
	...
	for (port = 0; port < RTL8651_PORT_NUMBER + totalExtPortNum; port++) {
		...
		len += sprintf(page + len, "port%d, %s BCSC, %s BC, %s MC\n",
			port, regData & ENBCSC ? "enable" : "disable",
			regData & BCSC_ENBROADCAST ? "enable" : "disable",
			regData & BCSC_ENMULTICAST ? "enable" : "disable");
	}
...
#ifdef CONFIG_RTL_PROC_NEW
#if defined(CONFIG_RTD_1295_HWNAT)
extern atomic_t rtl_rxTxDoneCnt;
extern atomic_t rtl_devOpened;
extern int rtl_rx_tasklet_running;
extern int rtl_tx_tasklet_running;
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
...
#if defined(CONFIG_RTL_8197F) &&  defined(CONFIG_FINETUNE_RUNOUT_IRQ)
	if (REG32(CPUIIMR) & (PKTHDR_DESC_RUNOUT_IP_ALL |
...
#endif
#if defined(CONFIG_RTD_1295_HWNAT)
	seq_printf(s, "    rtl_rxTxDoneCnt:	%d\n",
		atomic_read(&rtl_rxTxDoneCnt));
	seq_printf(s, "    rtl_devOpened:	%d\n",
		atomic_read(&rtl_devOpened));
	seq_printf(s, "    rtl_rx_tasklet_running:	%d\n",
		rtl_rx_tasklet_running);
	seq_printf(s, "    rtl_tx_tasklet_running:	%d\n",
		rtl_tx_tasklet_running);
#endif /* defined(CONFIG_RTD_1295_HWNAT) */

	return 0;
}
#else
static int32 stats_debug_entry_read(char *page, char **start, off_t off,
	int count, int *eof, void *data)
{
	int len;
...
#if defined(CONFIG_RTL_ETH_PRIV_SKB)
	len += sprintf(page + len,
		"    eth_skb_free_num:	%d (pool %d, rtk_que %d)\n",
		get_buf_in_poll() + get_buf_in_rx_skb_queue(),
		get_buf_in_poll(), get_buf_in_rx_skb_queue());
#endif
	...
	unsigned long len, void *data)
{
	...
	if (buffer && !copy_from_user(tmpBuf, buffer, len)) {
		...
		if (strncmp(cmd_addr, "clear", 5) == 0) {
			...
			if ((start < 0) || (start >= CP3ROMEPERF_INDEX_MAX)
				|| (end < 0) || (end >= CP3ROMEPERF_INDEX_MAX)
				|| (start > end)) {
				printk("Bad index!\n");
				return -1;
			}
			...
			for (i = start; i <= end; i++) {
				avrgCycle[i] =
					div64_u64(cp3romePerfStat[i].accCycle[0],
					cp3romePerfStat[i].executedNum);
				...
				avrgCycle[i] =
					div64_u64(cp3romePerfStat[i].accCycle[1],
					cp3romePerfStat[i].executedNum);
...
{
	int cpu_queue_cnt, rxRing_cnt, txRing_cnt, wlan_txRing_cnt,
		wlan_txRing_cnt1, mbuf_pending_cnt;
	...
	seq_printf(s,
		"cpu completion cnt(%d)\nnic rxring cnt(%d)\nnic txring cnt(%d)\nwlan0 txring cnt(%d)\nwlan1 txring cnt(%d)\nrx skb queue cnt(%d)\npool buf cnt(%d)\nmbuf pending times(%d)\nsum(%d)\n",
		cpu_queue_cnt, rxRing_cnt, txRing_cnt, wlan_txRing_cnt,
		wlan_txRing_cnt1, rx_skb_queue_cnt, poll_buf_cnt,
		mbuf_pending_cnt,
		cpu_queue_cnt + rxRing_cnt + txRing_cnt + wlan_txRing_cnt +
		wlan_txRing_cnt1 + rx_skb_queue_cnt + poll_buf_cnt);
...
#else
static int32 rtl819x_proc_privSkbInfo_read(char *page, char **start, off_t off,
	int count, int *eof, void *data)
{
	int len;
	int cpu_queue_cnt, rxRing_cnt, txRing_cnt, wlan_txRing_cnt,
...
static int32 rtl819x_proc_privSkbInfo_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	return len;
}
...
static int32 diagnostic_read(struct seq_file *s, void *v)
{
	...
	seq_printf(s, "DescRunOut: %s TotalDescFC: %s ShareBufFC: %s\n",
		(regData & DSCRUNOUT) ? "YES" : "NO",
		(regData & TotalDscFctrl_Flag) ? "YES" : "NO",
		(regData & SharedBufFCON_Flag) ? "YES" : "NO");
	...
	for (regIdx = 0; regIdx < 2; regIdx++) {
		...
		for (port = 0; port < 4; port++) {
			...
			if (regData1 == 0)
				...
				seq_printf(s,
					"Port%d queue congestion mask 0x%x\n",
...
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
		seq_printf(s,
			"Port%d each queue used descriptor: Queue[0~7]: [ ",
			port);
#else
		seq_printf(s,
			"Port%d each queue used descriptor: Queue[0~5]: [ ",
			port);
#endif
		for (regIdx = 0; regIdx < 3; regIdx++) {
			regData =
				READ_MEM32(P0_DCR0 + (port << 4) +
				(regIdx << 2));
			seq_printf(s, "%d %d ",
				((regData & Pn_EQDSCR_MASK) >>
					Pn_EVEN_OQDSCR_OFFSET),
				((regData & Pn_OQDSCR_MASK) >>
					Pn_ODD_OQDSCR_OFFSET));
		}
...
#else
static int32 diagnostic_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	int len;
	uint32 regData, regData1;
	...
	regData = READ_MEM32(GDSR0);
	len += sprintf(page + len,
		"MaxUsedDescriptor: %d CurUsed Descriptor: %d\n",
		(regData & MaxUsedDsc_MASK) >> MaxUsedDsc_OFFSET,
		(regData & USEDDSC_MASK) >> USEDDSC_OFFSET);
	len += sprintf(page + len,
		"DescRunOut: %s TotalDescFC: %s ShareBufFC: %s\n",
		(regData & DSCRUNOUT) ? "YES" : "NO",
		(regData & TotalDscFctrl_Flag) ? "YES" : "NO",
		(regData & SharedBufFCON_Flag) ? "YES" : "NO");
...
static int32 diagnostic_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	return len;
}
...
{
#if defined(CONFIG_RTD_1295_HWNAT)
	rtlglue_printf("Dump HW NAT registers from 0x%08x to 0x%08x\n",
		rtl_hwnat_base, rtl_hwnat_base + 0x170000 - 1);
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
	return PROC_READ_RETURN_VALUE;
}
#else
static int32 proc_mem_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	return PROC_READ_RETURN_VALUE;
}
...
static int32 proc_mem_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	char tmpbuf[64];
	uint32 *mem_addr, mem_data, mem_len;
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		if (!memcmp(cmd_addr, "readb", 5)) {
...
#if defined(CONFIG_RTD_1295_HWNAT)
		else if (!memcmp(cmd_addr, "readp", 5)) {
			phys_addr = simple_strtol(tokptr, NULL, 0);
			mem_addr = (uint32 *)(mem_base + phys_addr);
			rtlglue_printf("physical memory 0x%08x : virtual memory 0x%p\n",
				phys_addr, mem_addr);
			tokptr = strsep(&strptr, " ");
			if (tokptr == NULL) {
			...
			mem_len = simple_strtol(tokptr, NULL, 0);
			memDump3(mem_addr, mem_len, "");
		} else if (!memcmp(cmd_addr, "writep", 6)) {
			phys_addr = simple_strtol(tokptr, NULL, 0);
			mem_addr = (uint32 *)(mem_base + phys_addr);
			rtlglue_printf("physical memory 0x%08x : virtual memory 0x%p\n",
				phys_addr, mem_addr);
			...
			rtlglue_printf("Write memory 0x%08x data 0x%x: 0x%x\n",
				phys_addr, mem_data, READ_MEM32(mem_addr));
		}
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
...
static int32 port_status_read(struct seq_file *s, void *v)
{
	...
	for (port = PHY0; port <= PHY4; port++) {
		...
		if ((rtk_port_macStatus_get(port, &sts)) == 0) {
			...
			seq_printf(s, "	RXPause %s | ",
				(sts.rxpause) ? "Enabled" : "Disabled");
			seq_printf(s, "TXPause %s\n",
				(sts.txpause) ? "Enabled" : "Disabled");
			seq_printf(s, "	Duplex %s | ",
				(sts.duplex) ? "Enabled" : "Disabled");
			seq_printf(s, "Speed %s\n\n",
				(sts.speed) ==
				PortStatusLinkSpeed100M ? "100M" : ((sts.speed) ==
					PortStatusLinkSpeed1000M ? "1G" : ((sts.speed) ==
	...
	for (port = PHY0; port <= CPU; port++) {
		...
		if (port < 5) {
			extern rtl8651_tblAsic_ethernet_t
				rtl8651AsicEthernetTable[];
			int phyid;
			uint32 data1;
...
#else
		data0 = (regData & (PortEEEStatus_MASK)) >>
			PortEEEStatus_OFFSET;
	...
	for (port = PHY0; port < PHY4; port++) {
		extern int rtk_port_phyStatus_get(int port, int *pLinkStatus,
			int *pSpeed, int *pDuplex);
		int status, speed, duplex;

		if ((rtk_port_phyStatus_get(port, &status, &speed,
					&duplex)) == 0) {
			seq_printf(s, "Port%d ", port);
			...
			seq_printf(s, "	%s-Duplex | ",
				duplex ? "Full" : "Half");

			seq_printf(s, "Speed %s\n\n",
				speed ==
				PortStatusLinkSpeed100M ? "100M" : (speed ==
					PortStatusLinkSpeed1000M ? "1G" : (speed
						==
						PortStatusLinkSpeed10M ? "10M" :
						"Unkown")));
		}
...
#else
static int32 port_status_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	int len, port;
...
static int32 port_status_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	char tmpbuf[64];
	uint32 port_mask;
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		if (!memcmp(cmd_addr, "port", 4)) {
			...
			for (port = 0; port < EXT_SWITCH_MAX_PHY_PORT; port++) {
				extern int set_83XX_speed_mode(int port,
					int type);
				if (((1 << port) & port_mask)
					&& (type != HALF_DUPLEX_1000M)) {
			...
			if (type == DUPLEX_10M) {
				for (port = 0; port < CPU; port++) {
					if ((1 << port) & port_mask) {
						...
						rtl8651_setAsicEthernetPHYAdvCapality(port,
							((1 << HALF_DUPLEX_10M) |
							(1 << DUPLEX_10M)));
						REG32(PCRP0 + (port * 4)) =
							(REG32(PCRP0 + (port * 4)) |
				...
				for (port = 0; port < CPU; port++) {
					...
					if ((1 << port) & port_mask) {
						if (type == PORT_DOWN) {
							...
							rtl865xC_setAsicEthernetForceModeRegs(port,
								forceMode,
								forceLink,
								forceLinkSpeed,
								forceDuplex);
...
static int32 proc_rgmii_read(struct seq_file *s, void *v)
{
	...
	seq_printf(s, "Dump RGMII Configuration:\n\n");

	/* P0 RGMII */
	regData = READ_MEM32(P0GMIICR);
	seq_printf(s, "Port0 : 0x%08X\n", regData);

	seq_printf(s, "# TX CPU tag: %s\n",
		(regData & CFG_TX_CPUC_TAG) ? "Enabled" : "Disabled");
	seq_printf(s, "# CPU tag: %s\n",
		(regData & CFG_CPUC_TAG) ? "Enabled" : "Disabled");

	seq_printf(s, "# MAC interface mode: ");
	temp = (regData & CFG_GMAC_MASK);
	switch (temp) {
	case CFG_GMAC_RGMII:
		seq_printf(s, "RGMII mode\n");
		break;
	case CFG_GMAC_GMII_MII_MAC:
		seq_printf(s, "GMII/MII MAC mode\n");
		break;
	case CFG_GMAC_GMII_MII_PHY:
		seq_printf(s, "GMII/MII PHY mode\n");
		break;
	case CFG_GMAC_Reserved:
	default:
		seq_printf(s, "Reserved\n");
	}

	temp = ((regData & CF_SEL_RGTXC_MASK) >> CF_SEL_RGTXC_OFFSET) |
		((regData & (1 << 4)) ? (1 << 2) : 0);
	seq_printf(s, "# TX delay: %d x 0.5ns\n", temp);
	seq_printf(s, "# RX delay: %d x 0.5ns\n", (regData & 7));
	seq_printf(s, "# Config done: %s\n\n",
		(regData & Conf_done) ? "Yes" : "No");

	/* P5 RGMII */
	regData = READ_MEM32(P5GMIICR);
	seq_printf(s, "Port5 : 0x%08X\n", regData);
...
static int32 proc_rgmii_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	char tmpbuf[512];
	char *strptr;
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		if (strncmp(cmdptr, "port", 4) == 0) {
			...
			switch (portNum) {
			case PHY0:
				regAddr = (uint32 *)P0GMIICR;
				break;
			case PHY5:
				regAddr = (uint32 *)P5GMIICR;
				break;
			default:
				pr_err("RGMII is not existed in port %d\n", portNum);
				goto errout;
			}
			...
			if (strncmp(cmdptr, "delay", 5) == 0) {
				/* port 0/5 delay [tx delay] [rx delay] */
				cmdptr = strsep(&strptr, " ");
				if (cmdptr == NULL) {
					goto errout;
				}
				txDelay = simple_strtol(cmdptr, NULL, 0);
				if (txDelay > 7) {
					pr_err("valid tx delay is between 0 and 7\n");
					goto errout;
				}
				...
				rxDelay = simple_strtol(cmdptr, NULL, 0);
				if (rxDelay > 7) {
					pr_err("valid rx delay is between 0 and 7\n");
					goto errout;
				}

				regData = READ_MEM32(regAddr) &
					~((7 << 0) | (1 << 4) | CF_SEL_RGTXC_MASK);
				regData |= (txDelay & 3) << CF_SEL_RGTXC_OFFSET |
					   ((txDelay & (1 << 2)) ? (1 << 4) : 0) |
					   rxDelay;
				pr_err("Port %d RGMII new tx delay %d, rx delay %d\n"
					"old data 0x%08X, new data 0x%08X\n",
					portNum, txDelay, rxDelay,
					READ_MEM32(regAddr), regData);
...
static int32 rtl865x_proc_mibCounter_read(struct seq_file *s, void *v)
{
	...
	for (i = 0; i <= RTL8651_PORT_NUMBER; i++) {
		...
		seq_printf(s,
			"	 Rcv %llu bytes, TpPortInDiscard %u, DropEvent %u",
	...
	defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
		seq_printf(s, ", QmDiscard  %u",
			rtl8651_returnAsicCounter(OFFSET_QMDISCARDCNT_P0 +
				addrOffset_fromP0));
#endif
		seq_printf(s,
			...
			(OFFSET_DOT3STATSFCSERRORS_P0 +
				addrOffset_fromP0),
			rtl8651_returnAsicCounter
			(OFFSET_DOT3STATSSYMBOLERRORS_P0 +
				addrOffset_fromP0),
			rtl8651_returnAsicCounter
			(OFFSET_ETHERSTATSFRAGMEMTS_P0 +
				addrOffset_fromP0),
			rtl8651_returnAsicCounter
			(OFFSET_ETHERSTATSJABBERS_P0 + addrOffset_fromP0));
		seq_printf(s,
			"	 Unicast %u pkts, Multicast %u pkts, Broadcast %u pkts\n",
			...
			(OFFSET_ETHERSTATSBROADCASTPKTS_P0 +
				addrOffset_fromP0));
		seq_printf(s,
			"	 < 64: %u pkts, 64: %u pkts, 65 -127: %u pkts, 128 -255: %u pkts\n",
			rtl8651_returnAsicCounter
			(OFFSET_ETHERSTATSUNDERSIZEPKTS_P0 + addrOffset_fromP0),
			rtl8651_returnAsicCounter
			(OFFSET_ETHERSTATSPKTS64OCTETS_P0 + addrOffset_fromP0),
			rtl8651_returnAsicCounter
			...
			(OFFSET_ETHERSTATSPKTS128TO255OCTETS_P0 +
				addrOffset_fromP0));
		seq_printf(s,
			"	 256 - 511: %u pkts, 512 - 1023: %u pkts, 1024 - 1518: %u pkts\n",
			rtl8651_returnAsicCounter
			(OFFSET_ETHERSTATSPKTS256TO511OCTETS_P0 +
				addrOffset_fromP0),
			rtl8651_returnAsicCounter
			...
			(OFFSET_ETHERSTATSPKTS1024TO1518OCTETS_P0 +
				addrOffset_fromP0));
		seq_printf(s,
			"	 oversize: %u pkts, Control unknown %u pkts, Pause %u pkts\n",
			rtl8651_returnAsicCounter
			(OFFSET_ETHERSTATSOVERSIZEPKTS_P0 + addrOffset_fromP0),
			rtl8651_returnAsicCounter
		...
		seq_printf(s,
			"	 Snd %llu bytes, Unicast %u pkts, Multicast %u pkts\n",
			...
			rtl8651_returnAsicCounter(OFFSET_IFOUTMULTICASTPKTS_P0 +
				addrOffset_fromP0));
		seq_printf(s,
			"	 Broadcast %u pkts, Late collision %u, Deferred transmission %u\n",
			rtl8651_returnAsicCounter(OFFSET_IFOUTBROADCASTPKTS_P0 +
				addrOffset_fromP0),
			rtl8651_returnAsicCounter
			(OFFSET_DOT3STATSLATECOLLISIONS_P0 + addrOffset_fromP0),
			rtl8651_returnAsicCounter
			(OFFSET_DOT3STATSDEFERREDTRANSMISSIONS_P0 +
				addrOffset_fromP0));
		seq_printf(s,
			"	 Collisions %u, Single collision %u, Multiple collision %u, pause %u\n",
...
#else
static int32 rtl865x_proc_mibCounter_read(char *page, char **start, off_t off,
	int count, int *eof, void *data)
{
	int len = 0;
...
void display_8370_port_stat(uint32 port, rtk_stat_port_cntr_t *p)
{
	...
	rtlglue_printf("   Rcv %llu bytes, Unicast %u pkts, Multicast %u pkts, Broadcast %u pkts\n",
		p->ifInOctets, p->ifInUcastPkts, p->etherStatsMcastPkts,
		p->etherStatsBcastPkts);
	rtlglue_printf("   64: %u pkts, 65 -127: %u pkts, 128 -255: %u pkts\n",
		p->etherStatsPkts64Octets, p->etherStatsPkts65to127Octets,
		p->etherStatsPkts128to255Octets);
	rtlglue_printf("   256 - 511: %u pkts, 512 - 1023: %u pkts, 1024 - max: %u pkts\n",
		p->etherStatsPkts256to511Octets,
		p->etherStatsPkts512to1023Octets,
		p->etherStatsPkts1024toMaxOctets);
	rtlglue_printf("   Drop: %u, FCSError: %u, Fragment: %u, JabberErr: %u\n",
		p->etherStatsDropEvents, p->dot3StatsFCSErrors,
		p->etherStatsFragments, p->etherStatsJabbers);
	...
	rtlglue_printf("   PortInDiscard: %u, SymbolErr: %u, Pause: %u\n",
		p->dot1dTpPortInDiscards, p->dot3StatsSymbolErrors,
		p->dot3InPauseFrames);
	rtlglue_printf("   etherStatsOctets  %llu\n", p->etherStatsOctets);
	...
	rtlglue_printf("   Snd %llu bytes, Unicast %u pkts, Multicast %u pkts, Broadcast %u pkts\n",
		p->ifOutOctets, p->ifOutUcastPkts, p->ifOutMulticastPkts,
		p->ifOutBrocastPkts);
	rtlglue_printf("   Collision: %u, SingleCol: %u, MultipleCol: %u, LateCol: %u\n",
		...
		p->dot3StatsLateCollisions);
	rtlglue_printf("   ExcessiveCol: %u, DeferredTx: %u, DelayExceededDiscard: %u\n",
		p->dot3StatsExcessiveCollisions,
		p->dot3StatsDeferredTransmissions,
...
static int32 rtl865x_proc_mibCounter_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	char tmpbuf[512];
	char *strptr;
		...
		if (strncmp(cmdptr, "clear", 5) == 0) {
			...
			if (strncmp(cmdptr, "8367", 4) == 0) {
				...
				for (portNum = 0; portNum < 7; portNum++) {
					...
					memset(&port_cntrs, 0,
						sizeof(rtk_stat_port_cntr_t));
					rtk_stat_port_getAll(portNum,
						&port_cntrs);
					display_8367r_port_stat(portNum,
						&port_cntrs);
				}
				return len;
			}
			...
			if (strncmp(cmdptr, "8370", 4) == 0) {	/* 8370 or 83700 ~ 83709 */
				...
				if ((cmdptr[4] >= '0') && (cmdptr[4] <= '9')) {
					memset(&port_cntrs, 0,
						sizeof(rtk_stat_port_cntr_t));
					rtk_stat_port_getAll((cmdptr[4] - '0'),
						&port_cntrs);
					display_8370_port_stat((cmdptr[4] -
							'0'), &port_cntrs);
				} else {
					for (portNum = 0; portNum <= 9;
						portNum++) {
						memset(&port_cntrs, 0,
							sizeof
						...
						rtk_stat_port_getAll(portNum,
							&port_cntrs);
						display_8370_port_stat(portNum,
							&port_cntrs);
					}
				}
				return len;
			...
			if (strncmp(cmdptr, "8325", 4) == 0) {	/* 8325 or 83250 ~ 8325g */
				...
				if ((cmdptr[4] >= '0') && (cmdptr[4] <= '9')) {
					rtl8325d_mibCounter_dump((cmdptr[4] -
							'0'));
				} else if ((cmdptr[4] >= 'a')
					&& (cmdptr[4] <= 'f')) {
					rtl8325d_mibCounter_dump((cmdptr[4] -
					...
					for (portNum = 0; portNum < 16;
						portNum++) {
						rtl8325d_mibCounter_dump
							(portNum);
					}
					rtl8325d_mibCounter_dump(24);
				}
			...
			if ((portNum >= 0) && (portNum <= RTL8651_PORT_NUMBER)) {
				extern uint32 rtl8651_returnAsicCounter(uint32
					offset);
				extern uint64
					rtl865xC_returnAsicCounter64(uint32
					offset);
				uint32 addrOffset_fromP0 =
					portNum * MIB_ADDROFFSETBYPORT;
	...
	defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
				...
				rtlglue_printf("   < 64: %u pkts, 64: %u pkts, 65 -127: %u pkts, 128 -255: %u pkts\n",
					rtl8651_returnAsicCounter
					(OFFSET_ETHERSTATSUNDERSIZEPKTS_P0 +
						addrOffset_fromP0),
					rtl8651_returnAsicCounter
				...
				rtlglue_printf("   Snd %llu bytes, Unicast %u pkts, Multicast %u pkts\n",
					rtl865xC_returnAsicCounter64
					(OFFSET_IFOUTOCTETS_P0 +
						addrOffset_fromP0),
					rtl8651_returnAsicCounter
					(OFFSET_IFOUTUCASTPKTS_P0 +
						addrOffset_fromP0),
					rtl8651_returnAsicCounter
...
extern int rtl_get_portRate(unsigned int port, unsigned long *rxRate,
...
#else
static int32 rtl865x_proc_portRate_read(char *page, char **start, off_t off,
	int count, int *eof, void *data)
{
	int len;
...
static int32 rtl865x_proc_portRate_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	return len;
}
...
#else
static int32 proc_phyReg_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	return PROC_READ_RETURN_VALUE;
}
...
#else
static int32 proc_mmd_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	return PROC_READ_RETURN_VALUE;
}
 ...
 */
static int32 proc_mmd_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	char tmpbuf[64];
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		if (!memcmp(cmd_addr, "read", 4)) {
			...
			/* confirm result */
			rtl8651_setAsicEthernetPHYReg(phyId, 13,
				(devId | 0x4000));
			ret = rtl8651_getAsicEthernetPHYReg(phyId, 14,
				&dataReadBack);
...
static int32 proc_phyReg_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	char tmpbuf[64];
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		if (!memcmp(cmd_addr, "read", 4)) {
			...
			if (gpio_simulate_mdc_mdio) {
				if ((8 > phyId) && (phyId > 4))
					ret = rtl_mdio_read(phyId, regId,
						&regData);
				else
					ret = rtl8651_getAsicEthernetPHYReg
						(phyId, regId, &regData);
			} else
#endif
			{
				ret = rtl8651_getAsicEthernetPHYReg(phyId,
					regId, &regData);
			}
			...
			if (gpio_simulate_mdc_mdio) {
				if ((8 > phyId) && (phyId > 4))
					ret = rtl_mdio_write(phyId, regId,
						regData);
				else
					ret = rtl8651_setAsicEthernetPHYReg
						(phyId, regId, regData);
			} else
#endif
			{
				ret = rtl8651_setAsicEthernetPHYReg(phyId,
					regId, regData);
			}
			if (ret == SUCCESS) {
			...
			extern int rtl8367b_getAsicReg(unsigned int reg,
				unsigned int *pValue);
			tokptr = strsep(&strptr, " ");
			if (tokptr == NULL) {
				goto errout;
			...
			extern int rtl8367b_setAsicReg(unsigned int reg,
				unsigned int pValue);
			tokptr = strsep(&strptr, " ");
			if (tokptr == NULL) {
				goto errout;
			...
			extern int rtl8367b_getAsicPHYReg(uint32 phyNo,
			...
			extern int rtl8367b_setAsicPHYReg(uint32 phyNo,
			...
			extern int rtk_port_phyTestMode_set(uint32 port,
				int mode);
			...
			if (tokptr[0] == 'm') {
				extern int rtk_port_phyMdx_set(uint32 port,
					int mode);
			...
			extern int rtl8370_getAsicPHYReg(uint32 phyNo,
				uint32 phyAddr, uint32 *pRegData);
			...
			extern int rtl8370_setAsicPHYReg(uint32 phyNo,
				uint32 phyAddr, uint32 phyData);
			...
			int32 rtlglue_reg32_read_data(uint32 reg,
				uint32 *value);
			...
			extern int32 phy_reg_read(uint32 phyaddr,
				uint32 regaddr, uint32 page, uint16 *reg_data);
			uint16 data;
			...
			extern int32 phy_reg_write(uint32 phyaddr,
				uint32 regaddr, uint32 page, uint16 reg_data);
			...
			if (pageId >= 31) {
				...
				rtl8651_setAsicEthernetPHYReg(phyId, 30,
					pageId);
			} else
#endif
#if defined(CONFIG_RTL_8197F) && defined(CONFIG_RTL_8211F_SUPPORT)
			if (gpio_simulate_mdc_mdio) {
				if ((8 > phyId) && (phyId > 4)) {
					...
					/* change back to page 0 */
					rtl8651_setAsicEthernetPHYReg(phyId, 31,
						0);
				}
			} else
#endif
			 ...
			 */
			extern void Sram98C(uint32 phyid, uint32 oper,
				uint32 RegAddr, uint32 sramdata);
			...
			for (i = 0; i < 5; i++) {
				if (REG32(PSRP0 + (i * 4)) & PortStatusLinkUp) {
					for (j = 0, sum = 0; j < 10; j++) {
						rtl8651_getAsicEthernetPHYReg(i,
							29, &regData);
						sum += regData;
						mdelay(10);
					}
			...
			for (port = 0; port < 5; port++) {
				...
				for (page = 0; page < 4; page++) {
					/* change page num */
					rtl8651_setAsicEthernetPHYReg(port, 31,
						page);
					for (reg = 0; reg < 32; reg++) {
...
#else
static int32 mac_config_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	int len;
	uint32 regData;
	...
	if (data0)
		...
		len += sprintf(page + len,
			"Maximum of 128 consecutive pause frames\n");
...
static int32 mac_config_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	return len;
}
...
static int32 fc_threshold_read(struct seq_file *s, void *v)
{
	...
	for (port = PHY0; port <= CPU; port++) {
		...
		data0 = (regData & P_MaxDSC_FCOFF_MASK) >>
			P_MaxDSC_FCOFF_OFFSET;
		...
		/* if(port<CPU) */
		{
			for (group = GR0; group <= GR2; group++) {
				...
				regData =
					READ_MEM32(QDBFCRP0G0 + ((port) * 0xC) +
					((group) << 2));
				/* seq_printf(s,"address:%0x",(QDBFCRP0G0+((port)<<2)+((group)<<2))); */
				data0 = (regData & QG_DSC_FCOFF_MASK) >>
					QG_DSC_FCOFF_OFFSET;
				seq_printf(s, "   QG_DSC_FCOFF:%d, ", data0);
				data0 = (regData & QG_DSC_FCON_MASK) >>
					QG_DSC_FCON_OFFSET;
				...
				regData =
					READ_MEM32(QPKTFCRP0G0 +
					((port) * 0xC) + ((group) << 2));
				/* seq_printf(s,"address:%0x",(QPKTFCRP0G0+((port)<<2)+((group)<<2))); */
				data0 = (regData & QG_QLEN_FCOFF_MASK) >>
					QG_QLEN_FCOFF_OFFSET;
				seq_printf(s, "QG_QLEN_FCOFF:%d, ", data0);
				data0 = (regData & QG_QLEN_FCON_MASK) >>
					QG_QLEN_FCON_OFFSET;
...
#else
static int32 fc_threshold_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	...
	regData = READ_MEM32(QNUMCR);
	len += sprintf(page + len,
		"The number of output queue for port(0~6) :\n");
	...
	for (port = PHY0; port <= CPU; port++) {
		...
		/* if(port<CPU) */
		{
			for (group = GR0; group <= GR2; group++) {
				 ...
				 * - Queue-Descriptor=Based Flow Control Threshold for Port 0 Group 0 */
				regData =
					READ_MEM32(QDBFCRP0G0 + ((port) << 2) +
					((group) << 2));
				/* len+=sprintf(page+len,"address:%0x",(QDBFCRP0G0+((port)<<2)+((group)<<2))); */
				data0 = (regData & QG_DSC_FCOFF_MASK) >>
					QG_DSC_FCOFF_OFFSET;
				len += sprintf(page + len,
					"   QG_DSC_FCOFF:%d, ", data0);
				data0 = (regData & QG_DSC_FCON_MASK) >>
					QG_DSC_FCON_OFFSET;
				len += sprintf(page + len, "QG_DSC_FCON:%d, ",
					data0);
				 ...
				 * QPKTFCRP5G0,QPKTFCRP5G1,QPKTFCRP5G2
				 - Queue-Packet-Based Flow Control Register for Port 0 Group 0 */
				regData =
					READ_MEM32(QPKTFCRP0G0 + ((port) << 2) +
					((group) << 2));
				/* len+=sprintf(page+len,"address:%0x",(QPKTFCRP0G0+((port)<<2)+((group)<<2))); */
				data0 = (regData & QG_QLEN_FCOFF_MASK) >>
					QG_QLEN_FCOFF_OFFSET;
				len += sprintf(page + len, "QG_QLEN_FCOFF:%d, ",
					data0);
				data0 = (regData & QG_QLEN_FCON_MASK) >>
					QG_QLEN_FCON_OFFSET;
...
static int32 fc_threshold_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	return len;
}
...
static int32 l3v6_read(struct seq_file *s, void *v)
{
	...
	for (idx = 0; idx < RTL8198C_ROUTINGV6TBL_SIZE; idx++) {
		...
		ntohl_array((uint32 *)&asic_l3v6.ipAddr.v6_addr32[0],
			(void *)&tmp_ip.v6_addr32[0], 4);
		mask = asic_l3v6.ipMask;

		seq_printf(s,
			"\t[%d]  %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x/%d process(%s) \n",
			idx, htons(tmp_ip.v6_addr16[0]),
			htons(tmp_ip.v6_addr16[1]), htons(tmp_ip.v6_addr16[2]),
			htons(tmp_ip.v6_addr16[3]), htons(tmp_ip.v6_addr16[4]),
			htons(tmp_ip.v6_addr16[5]), htons(tmp_ip.v6_addr16[6]),
			htons(tmp_ip.v6_addr16[7]), mask,
			str[asic_l3v6.process]);

		switch (asic_l3v6.process) {
		case 0x00:	/* PPPoE */
			seq_printf(s,
				"\t           dvidx(%d)  pppidx(%d) nxthop(%d)\n",
				asic_l3v6.vidx, asic_l3v6.pppoeIdx,
				(asic_l3v6.nextHopRow << 2) +
				asic_l3v6.nextHopColumn);
			break;
		...
		case 0x05:	/* NAPT Next Hop */
			seq_printf(s,
				"              NHSTA(%d) NHNUM(%d) NHNXT(%d) NHALGO(%d)\n",
				asic_l3v6.nhStart, asic_l3v6.nhNum,
				asic_l3v6.nhNxt, asic_l3v6.nhAlgo);
			break;
...
#else
static int32 l3v6_read(char *page, char **start, off_t off, int count, int *eof,
	void *data)
{
	int len;
	uint32 idx, mask;
	...
	for (idx = 0; idx < RTL8198C_ROUTINGV6TBL_SIZE; idx++) {
		...
		len += sprintf(page + len,
			"\tsix_rd_eg :0x%x\tsix_rd_idx:0x%x\n",
			asic_l3v6.six_rd_eg, asic_l3v6.six_rd_idx);
	}
...
static int32 l3v6_write(struct file *filp, const char *buff, unsigned long len,
	void *data)
{
	return len;
}
...
static int32 arp6_read(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < RTL8198C_ARPV6TBL_SIZE; i++) {
		...
		seq_printf(s,
			" ARPv6: idx:%3d  L2:(%3d,%d), subnet_idx:%d HOSTID==>(%x:%x:%x:%x) aging:%d\n",
			i, asic_arpv6.nextHopRow, asic_arpv6.nextHopColumn,
			asic_arpv6.subnet_id, asic_arpv6.hostid.v6_addr32[0],
			asic_arpv6.hostid.v6_addr32[1],
			asic_arpv6.hostid.v6_addr32[2],
			asic_arpv6.hostid.v6_addr32[3], asic_arpv6.aging);
	}
#endif
	return 0;
}
#else
static int32 arp6_read(char *page, char **start, off_t off, int count, int *eof,
	void *data)
{
	int len = 0, i;
	rtl8198C_tblAsicDrv_arpV6Param_t asic_arpv6;
	...
	for (i = 0; i < RTL8198C_ARPV6TBL_SIZE; i++) {
		...
		len += sprintf(page + len,
			" ARPv6: idx:%3d  L2:(%3d,%d), subnet_idx:%d HOSTID==>::%04x:%04x:%04x:%04x aging:%d\n",
			i, asic_arpv6.nextHopRow, asic_arpv6.nextHopColumn,
			asic_arpv6.subnet_id, asic_arpv6.hostid.v6_addr16[4],
			asic_arpv6.hostid.v6_addr16[5],
			asic_arpv6.hostid.v6_addr16[6],
...
static int32 arp6_write(struct file *filp, const char *buff, unsigned long len,
	void *data)
{
	return len;
}
...
static int32 nexthop6_read(struct seq_file *s, void *v)
{
	...
	for (idx = 0; idx < RTL8198C_NEXTHOPV6TBL_SIZE; idx++) {
		if (rtl8198C_getAsicNextHopTableV6(idx,
				&asic_nxthopv6) == FAILED)
			continue;

		seq_printf(s,
			"  [%d]  type(%s) dstVid(%d) pppoeIdx(%d) nextHop(%d)(0x%x)\n",
			idx,
			(asic_nxthopv6.isPppoe == TRUE ? "pppoe" : "ethernet"),
			asic_nxthopv6.dvid, asic_nxthopv6.pppoeIdx,
			(asic_nxthopv6.nextHopRow << 2) +
			asic_nxthopv6.nextHopColumn,
			(asic_nxthopv6.nextHopRow << 2) +
			asic_nxthopv6.nextHopColumn);
	}
#endif
...
#else
static int32 nexthop6_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	int len;
	uint32 idx;
...
static int32 nexthop6_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	return len;
}
...
static int32 mcast6_read(struct seq_file *s, void *v)
{
	...
	for (entry = 0; entry < RTL8198C_MULTICASTV6TBL_SIZE; entry++) {
		...
		if (rtl8198C_getAsicIpMulticastv6Table(entry, &asic) != SUCCESS) {
			...
			seq_printf(s,
				"\t[%d]  (OK)dip(%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x) sip(%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x) mbr(0x%x)\n",
				entry, asic.dip.v6_addr16[0],
				asic.dip.v6_addr16[1], asic.dip.v6_addr16[2],
				asic.dip.v6_addr16[3], asic.dip.v6_addr16[4],
				asic.dip.v6_addr16[5], asic.dip.v6_addr16[6],
				asic.dip.v6_addr16[7], asic.sip.v6_addr16[0],
				asic.sip.v6_addr16[1], asic.sip.v6_addr16[2],
				asic.sip.v6_addr16[3], asic.sip.v6_addr16[4],
				asic.sip.v6_addr16[5], asic.sip.v6_addr16[6],
				asic.sip.v6_addr16[7], asic.mbr);
			seq_printf(s,
				"\t       spa:%d, age:%d, cpu:%d, 6rd_eg_en:%d, 6rd_eg_idx:%d\n",
				asic.port, asic.age, asic.cpu, asic.six_rd_eg,
				asic.six_rd_idx);
		}
	}
...
#else
static int32 mcast6_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	int len = 0;
	uint32 entry;
...
static int32 mcast6_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	return len;
}
...
static int32 ip6rd_read(struct seq_file *s, void *v)
{
	...
	for (idx = 0; idx < RTL8198C_6RDTBL_SIZE; idx++) {
		...
		ntohl_array((uint32 *)&asic.six_rd_prefix.v6_addr32[0],
			(void *)&tmp_ip.v6_addr32[0], 4);

		seq_printf(s,
			"  [%d]  ceip(%d.%d.%d.%d)/ceipm:(%d) 6RD prefix=> ::%04x:%04x:%04x:%04x/%d\n",
			idx, (asic.ce_ip_addr >> 24),
			((asic.ce_ip_addr & 0x00ff0000) >> 16),
			((asic.ce_ip_addr & 0x0000ff00) >> 8),
			(asic.ce_ip_addr & 0xff), asic.ce_ip_mask_len,
			htons(tmp_ip.v6_addr16[0]), htons(tmp_ip.v6_addr16[1]),
			htons(tmp_ip.v6_addr16[2]), htons(tmp_ip.v6_addr16[3]),
			asic.six_rd_prefix_len);
		seq_printf(s,
			"        brip(%d.%d.%d.%d)/bripm:(%d) 6RD MTU:%d(0x%x) valid:%d\n",
			(asic.br_ip_addr >> 24),
			((asic.br_ip_addr & 0x00ff0000) >> 16),
			((asic.br_ip_addr & 0x0000ff00) >> 8),
			(asic.br_ip_addr & 0xff), asic.br_ip_mask_len, asic.mtu,
			asic.mtu, asic.valid);
	}
#endif
	return 0;
...
#else
static int32 ip6rd_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	int len;
	uint32 idx;
	...
	for (idx = 0; idx < RTL8198C_6RDTBL_SIZE; idx++) {
		...
		len += sprintf(page + len,
			...
			((asic.ce_ip_addr & 0x0000ff00) >> 8),
			(asic.ce_ip_addr & 0xff), asic.ce_ip_mask_len,
			asic.six_rd_prefix.v6_addr16[0],
			asic.six_rd_prefix.v6_addr16[1],
			asic.six_rd_prefix.v6_addr16[2],
			asic.six_rd_prefix.v6_addr16[3],
			asic.six_rd_prefix_len);
		len += sprintf(page + len,
			"        brip(%d.%d.%d.%d)/bripm:(%d) 6RD MTU:%d(0x%x) valid:%d\n",
			(asic.br_ip_addr >> 24),
...
static int32 ip6rd_write(struct file *filp, const char *buff, unsigned long len,
	void *data)
{
	return len;
}
...
static int32 dslite_read(struct seq_file *s, void *v)
{
	...
	for (idx = 0; idx < RTL8198C_DSLITETBL_SIZE; idx++) {
		...
		ntohl_array((uint32 *)&asic.host_ipv6_addr.v6_addr32[0],
			(void *)&tmp_ip.v6_addr32[0], 4);

		seq_printf(s,
			"\t[%d]  HOSTIP/MASK:%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x/%d \n",
			idx, htons(tmp_ip.v6_addr16[0]),
			htons(tmp_ip.v6_addr16[1]), htons(tmp_ip.v6_addr16[2]),
			htons(tmp_ip.v6_addr16[3]), htons(tmp_ip.v6_addr16[4]),
			htons(tmp_ip.v6_addr16[5]), htons(tmp_ip.v6_addr16[6]),
			htons(tmp_ip.v6_addr16[7]), asic.host_ipv6_mask);
		memset((void *)&tmp_ip.v6_addr32[0], 0x00, sizeof(tmp_ip));
		ntohl_array((uint32 *)&asic.aftr_ipv6_addr.v6_addr32[0],
			(void *)&tmp_ip.v6_addr32[0], 4);

		seq_printf(s,
			"\t      AFTRIP/MASK:%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x/%d MTU:%d(0x%x) valid:%d\n",
			htons(tmp_ip.v6_addr16[0]), htons(tmp_ip.v6_addr16[1]),
			htons(tmp_ip.v6_addr16[2]), htons(tmp_ip.v6_addr16[3]),
			htons(tmp_ip.v6_addr16[4]), htons(tmp_ip.v6_addr16[5]),
			htons(tmp_ip.v6_addr16[6]), htons(tmp_ip.v6_addr16[7]),
...
#else
static int32 dslite_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	int len = 0;
...
static int32 dslite_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	return len;
}
...
static int32 sw_dslite_read(struct seq_file *s, void *v)
{
	...
	for (idx = 0; idx < RTL8198C_DSLITETBL_SIZE; idx++) {
		...
		seq_printf(s,
			"\t[%d]  HOSTIP/MASK:%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x/%d \n",
			idx, entry.host_ipv6_addr.v6_addr16[0],
			entry.host_ipv6_addr.v6_addr16[1],
			entry.host_ipv6_addr.v6_addr16[2],
			entry.host_ipv6_addr.v6_addr16[3],
			entry.host_ipv6_addr.v6_addr16[4],
			entry.host_ipv6_addr.v6_addr16[5],
			entry.host_ipv6_addr.v6_addr16[6],
			entry.host_ipv6_addr.v6_addr16[7],
			entry.host_ipv6_mask);
		seq_printf(s,
			"\t      AFTRIP/MASK:%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x/%d MTU:%d(0x%x) valid:%d\n",
			entry.aftr_ipv6_addr.v6_addr16[0],
			entry.aftr_ipv6_addr.v6_addr16[1],
			entry.aftr_ipv6_addr.v6_addr16[2],
			entry.aftr_ipv6_addr.v6_addr16[3],
			entry.aftr_ipv6_addr.v6_addr16[4],
			entry.aftr_ipv6_addr.v6_addr16[5],
			entry.aftr_ipv6_addr.v6_addr16[6],
			entry.aftr_ipv6_addr.v6_addr16[7], entry.aftr_ipv6_mask,
			entry.mtu, entry.mtu, entry.valid);
	}
	return 0;
}
#else
...
static int32 sw_dslite_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	return len;
}
...
static ssize_t perf_dump_single_write(struct file *file,
...
static ssize_t sw_netif_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t storm_control_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t mirrorPort_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t hwMCast_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t swMCast_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t nexthop_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
extern int32 sw_nexthop_write(struct file *filp, const char *buff,
	unsigned long len, void *data);
...
static ssize_t sw_nexthop_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
extern int32 sw_l3_write(struct file *filp, const char *buff, unsigned long len,
	void *data);
...
static ssize_t port_bandwidth_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t queue_bandwidth_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t priority_decision_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t rateLimit_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t eventMgr_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t privSkbInfo_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t memory_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t diagnostic_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t port_status_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t phyReg_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
/*rgmii*/
int rgmii_single_open(struct inode *inode, struct file *file)
{
	return (single_open(file, proc_rgmii_read, NULL));
}

static ssize_t rgmii_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
	return proc_rgmii_write(file, userbuf, count, off);
}

struct file_operations rgmii_proc_fops = {
	.open = rgmii_single_open,
	.write = rgmii_single_write,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
static ssize_t asicCounter_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t fc_threshold_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t portRate_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t sw_arp6_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t nexthop6_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t mcast6_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t dslite_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t sw_dslite_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
#ifdef CONFIG_RTL_PROC_DEBUG
extern int32 sw_nexthop_read(char *page, char **start, off_t off, int count,
	int *eof, void *data);
extern int32 sw_nexthop_write(struct file *filp, const char *buff,
	unsigned long len, void *data);
extern int32 sw_l3_read(char *page, char **start, off_t off, int count,
	int *eof, void *data);
extern int32 sw_l3_write(struct file *filp, const char *buff, unsigned long len,
	void *data);
#endif
...
static int32 vlan_8367_single_show(struct seq_file *s, void *v)
{
	...
	rtlglue_printf("%s\n", "ASIC VLAN Table:");
	{
		...
		for (i = 0; i < 4095; i++) {
			...
			if (rtk_vlan_get(i, &pMbrmsk, &pUntagmsk,
					&pFid) == FAILED)
				continue;
...
int32 rtl865x_proc_debug_init(void)
{
	...
	if (rtl865x_proc_dir) {
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("stats", 0, rtl865x_proc_dir,
				&stats_proc_fops, NULL);
#else
			stats_debug_entry =
				create_proc_entry("stats", 0, rtl865x_proc_dir);
			if (stats_debug_entry != NULL) {
				stats_debug_entry->read_proc =
					stats_debug_entry_read;
				stats_debug_entry->write_proc =
					stats_debug_entry_write;
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("vlan", 0, rtl865x_proc_dir,
				&vlan_single_seq_file_operations, NULL);
#else
			vlan_entry =
				create_proc_entry("vlan", 0, rtl865x_proc_dir);
			if (vlan_entry != NULL) {
				/* vlan_entry->read_proc = vlan_read; */
				/* vlan_entry->write_proc= vlan_write; */
				vlan_entry->proc_fops =
					&vlan_single_seq_file_operations;
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("netif", 0, rtl865x_proc_dir,
				&netif_proc_fops, NULL);
#else
			netif_entry =
				create_proc_entry("netif", 0, rtl865x_proc_dir);
			if (netif_entry != NULL) {
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("sw_netif", 0, rtl865x_proc_dir,
				&sw_netif_proc_fops, NULL);
#else
			sw_netif_entry =
				create_proc_entry("sw_netif", 0,
				rtl865x_proc_dir);
...
#if defined(CONFIG_RTL_8367R_SUPPORT) && defined(CONFIG_OPENWRT_SDK)	/* mark_bb */
		{
			proc_create_data("8367_vlan", 0, rtl865x_proc_dir,
				&vlan_8367_single_seq_file_operations, NULL);

		}
		/*pvid */
		{
			proc_create_data("8367_pvid", 0, rtl865x_proc_dir,
				&vlan_8367_pvid_single_seq_file_operations,
				NULL);
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("acl", 0, rtl865x_proc_dir,
				&acl_proc_fops, NULL);
#else
			acl_entry =
				create_proc_entry("acl", 0, rtl865x_proc_dir);
			if (acl_entry != NULL) {
				 ...
				 */
				acl_entry->proc_fops =
					&acl_single_seq_file_operations;
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("advRt", 0, rtl865x_proc_dir,
				&advRt_proc_fops, NULL);
#else
			advRt_entry =
				create_proc_entry("advRt", 0, rtl865x_proc_dir);
			if (advRt_entry != NULL) {
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("storm_control", 0, rtl865x_proc_dir,
				&storm_control_proc_fops, NULL);
#else
			storm_control =
				create_proc_entry("storm_control", 0,
				rtl865x_proc_dir);
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("soft_aclChains", 0, rtl865x_proc_dir,
				&aclChains_single_seq_file_operations, NULL);
#else
			acl_chains_entry =
				create_proc_entry("soft_aclChains", 0,
				rtl865x_proc_dir);
			if (acl_chains_entry != NULL) {
				acl_chains_entry->proc_fops =
					&aclChains_single_seq_file_operations;
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("soft_qosRules", 0, rtl865x_proc_dir,
				&qosRule_single_seq_file_operations, NULL);
#else
			qos_rule_entry =
				create_proc_entry("soft_qosRules", 0,
				rtl865x_proc_dir);
			if (qos_rule_entry != NULL) {
				 ...
				 * acl_entry->owner = THIS_MODULE;
				 */
				qos_rule_entry->proc_fops =
					&qosRule_single_seq_file_operations;
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("hs", 0, rtl865x_proc_dir,
				&hs_proc_fops, NULL);
#else
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("nic_mbuf", 0, rtl865x_proc_dir,
				&nic_mbuf_single_seq_file_operations, NULL);
#else
			nic_skb_buff =
				create_proc_entry("nic_mbuf", 0,
				rtl865x_proc_dir);
			if (nic_skb_buff != NULL) {
				nic_skb_buff->proc_fops =
					&nic_mbuf_single_seq_file_operations;
				retval = SUCCESS;
			} else {
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("rxRing", 0, rtl865x_proc_dir,
				&rxRing_single_seq_file_operations, NULL);
#else
			rxRing_entry =
				create_proc_entry("rxRing", 0,
				rtl865x_proc_dir);
			if (rxRing_entry != NULL) {
				rxRing_entry->proc_fops =
					&rxRing_single_seq_file_operations;
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("txRing", 0, rtl865x_proc_dir,
				&txRing_single_seq_file_operations, NULL);
#else
			txRing_entry =
				create_proc_entry("txRing", 0,
				rtl865x_proc_dir);
			if (txRing_entry != NULL) {
				txRing_entry->proc_fops =
					&txRing_single_seq_file_operations;
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("mbufRing", 0, rtl865x_proc_dir,
				&mbuf_single_seq_file_operations, NULL);
#else
			mbuf_entry =
				create_proc_entry("mbufRing", 0,
				rtl865x_proc_dir);
			if (mbuf_entry != NULL) {
				mbuf_entry->proc_fops =
					&mbuf_single_seq_file_operations;
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("pvid", 0, rtl865x_proc_dir,
				&pvid_proc_fops, NULL);
#else
			pvid_entry =
				create_proc_entry("pvid", 0, rtl865x_proc_dir);
			if (pvid_entry != NULL) {
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("mirrorPort", 0, rtl865x_proc_dir,
				&mirrorPort_proc_fops, NULL);
#else
			mirrorPort_entry =
				create_proc_entry("mirrorPort", 0,
				rtl865x_proc_dir);
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("l2", 0, rtl865x_proc_dir,
				&l2_proc_fops, NULL);
#else
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("hwMCast", 0, rtl865x_proc_dir,
				&hwMCast_proc_fops, NULL);
#else
			hwMCast_entry =
				create_proc_entry("hwMCast", 0,
				rtl865x_proc_dir);
			if (hwMCast_entry != NULL) {
				hwMCast_entry->read_proc =
					rtl865x_proc_hw_mcast_read;
				hwMCast_entry->write_proc =
					rtl865x_proc_hw_mcast_write;
				retval = SUCCESS;
			}
#endif
		}
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("swMCast", 0, rtl865x_proc_dir,
				&swMCast_proc_fops, NULL);
#else
			swMCast_entry =
				create_proc_entry("swMCast", 0,
				rtl865x_proc_dir);
			if (swMCast_entry != NULL) {
				swMCast_entry->read_proc =
					rtl865x_proc_sw_mcast_read;
				swMCast_entry->write_proc =
					rtl865x_proc_sw_mcast_write;
				retval = SUCCESS;
			}
#endif
		}
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("arp", 0, rtl865x_proc_dir,
				&arp_proc_fops, NULL);
#else
			arp_entry =
				create_proc_entry("arp", 0, rtl865x_proc_dir);
			if (arp_entry != NULL) {
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("nexthop", 0, rtl865x_proc_dir,
				&nexthop_proc_fops, NULL);
#else
			nexthop_entry =
				create_proc_entry("nexthop", 0,
				rtl865x_proc_dir);
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("sw_nexthop", 0, rtl865x_proc_dir,
				&sw_nexthop_proc_fops, NULL);
#else
			sw_nexthop_entry =
				create_proc_entry("sw_nexthop", 0,
				rtl865x_proc_dir);
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("l3", 0, rtl865x_proc_dir,
				&l3_proc_fops, NULL);
#else
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("sw_l3", 0, rtl865x_proc_dir,
				&sw_l3_proc_fops, NULL);
#else
			sw_l3_entry =
				create_proc_entry("sw_l3", 0, rtl865x_proc_dir);
			if (sw_l3_entry != NULL) {
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("ip", 0, rtl865x_proc_dir,
				&ip_proc_fops, NULL);
#else
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("pppoe", 0, rtl865x_proc_dir,
				&pppoe_proc_fops, NULL);
#else
			pppoe_entry =
				create_proc_entry("pppoe", 0, rtl865x_proc_dir);
			if (pppoe_entry != NULL) {
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("napt", 0, rtl865x_proc_dir,
				&napt_single_seq_file_operations, NULL);
#else
			napt_entry =
				create_proc_entry("napt", 0, rtl865x_proc_dir);
			if (napt_entry != NULL) {
#if 1
				napt_entry->proc_fops =
					&napt_single_seq_file_operations;
#else
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("sw_napt", 0, rtl865x_proc_dir,
				&sw_napt_single_seq_file_operations, NULL);
#else
			sw_napt_entry =
				create_proc_entry("sw_napt", 0,
				rtl865x_proc_dir);
			if (sw_napt_entry != NULL) {
#ifdef CONFIG_RTL_LAYERED_DRIVER
				sw_napt_entry->proc_fops =
					&sw_napt_single_seq_file_operations;
#else
				sw_napt_entry->read_proc =
					rtl865x_sw_napt_proc_read;
				sw_napt_entry->write_proc =
					rtl865x_sw_napt_proc_write;
#endif
				retval = SUCCESS;
			} else {
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("sw_l2", 0, rtl865x_proc_dir,
				&sw_l2_proc_fops, NULL);
#else
			sw_l2_entry =
				create_proc_entry("sw_l2", 0, rtl865x_proc_dir);
			if (sw_l2_entry != NULL) {
				sw_l2_entry->read_proc =
					rtl865x_sw_l2_proc_read;
				sw_l2_entry->write_proc =
					rtl865x_sw_l2_proc_write;
				retval = SUCCESS;
			} else {
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("port_bandwidth", 0, rtl865x_proc_dir,
				&port_bandwidth_proc_fops, NULL);
#else
			port_bandwidth_entry =
				create_proc_entry("port_bandwidth", 0,
				rtl865x_proc_dir);
			if (port_bandwidth_entry != NULL) {
				port_bandwidth_entry->read_proc =
					port_bandwidth_read;
				port_bandwidth_entry->write_proc =
					port_bandwidth_write;
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("queue_bandwidth", 0, rtl865x_proc_dir,
				&queue_bandwidth_proc_fops, NULL);
#else
			queue_bandwidth_entry =
				create_proc_entry("queue_bandwidth", 0,
				rtl865x_proc_dir);
			if (queue_bandwidth_entry != NULL) {
				queue_bandwidth_entry->read_proc =
					queue_bandwidth_read;
				queue_bandwidth_entry->write_proc =
					queue_bandwidth_write;
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("priority_decision", 0,
				rtl865x_proc_dir, &priority_decision_proc_fops,
				NULL);
#else
			priority_decision_entry =
				create_proc_entry("priority_decision", 0,
				rtl865x_proc_dir);
			if (priority_decision_entry != NULL) {
				priority_decision_entry->read_proc =
					priority_decision_read;
				priority_decision_entry->write_proc =
					priority_decision_write;
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("rateLimit", 0, rtl865x_proc_dir,
				&rateLimit_proc_fops, NULL);
#else
			rateLimit_entry =
				create_proc_entry("rateLimit", 0,
				rtl865x_proc_dir);
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("eventMgr", 0, rtl865x_proc_dir,
				&eventMgr_proc_fops, NULL);
#else
			eventMgr_entry =
				create_proc_entry("eventMgr", 0,
				rtl865x_proc_dir);
			if (eventMgr_entry != NULL) {
				eventMgr_entry->read_proc =
					rtl865x_event_proc_read;
				eventMgr_entry->write_proc =
					rtl865x_event_proc_write;
				retval = SUCCESS;
			}
#endif
		}
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("igmp", 0, rtl865x_proc_dir,
				&igmp_single_seq_file_operations, NULL);
#else
			igmp_entry =
				create_proc_entry("igmp", 0, rtl865x_proc_dir);
			if (igmp_entry != NULL) {
				/*
				 ...
				 */
				igmp_entry->proc_fops =
					&igmp_single_seq_file_operations;
				retval = SUCCESS;
			}
#endif
		}
...
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("perf_dump", 0, rtl865x_proc_dir,
				&perf_dump_proc_fops, NULL);
#else
			perf_dump =
				create_proc_entry("perf_dump", 0,
				rtl865x_proc_dir);
			if (perf_dump != NULL) {
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("privSkbInfo", 0, rtl865x_proc_dir,
				&privSkbInfo_proc_fops, NULL);
#else
			prive_skb_debug_entry =
				create_proc_entry("privSkbInfo", 0,
				rtl865x_proc_dir);
			if (prive_skb_debug_entry != NULL) {
				prive_skb_debug_entry->read_proc =
					rtl819x_proc_privSkbInfo_read;
				prive_skb_debug_entry->write_proc =
					rtl819x_proc_privSkbInfo_write;
				retval = SUCCESS;
			}
#endif
		}
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("memory", 0, rtl865x_proc_dir,
				&memory_proc_fops, NULL);
#else
			mem_entry =
				create_proc_entry("memory", 0,
				rtl865x_proc_dir);
			if (mem_entry != NULL) {
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("diagnostic", 0, rtl865x_proc_dir,
				&diagnostic_proc_fops, NULL);
#else
			diagnostic_entry =
				create_proc_entry("diagnostic", 0,
				rtl865x_proc_dir);
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("port_status", 0, rtl865x_proc_dir,
				&port_status_proc_fops, NULL);
#else
			port_status_entry =
				create_proc_entry("port_status", 0,
				rtl865x_proc_dir);
			if (port_status_entry != NULL) {
				port_status_entry->read_proc = port_status_read;
				port_status_entry->write_proc =
					port_status_write;
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("phyReg", 0, rtl865x_proc_dir,
				&phyReg_proc_fops, NULL);
#else
			phyReg_entry =
				create_proc_entry("phyReg", 0,
				rtl865x_proc_dir);
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("asicCounter", 0, rtl865x_proc_dir,
				&asicCounter_proc_fops, NULL);
#else
			asicCnt_entry =
				create_proc_entry("asicCounter", 0,
				rtl865x_proc_dir);
			if (asicCnt_entry != NULL) {
				asicCnt_entry->read_proc =
					rtl865x_proc_mibCounter_read;
				asicCnt_entry->write_proc =
					rtl865x_proc_mibCounter_write;
				retval = SUCCESS;
			}
#endif
		}
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("portRate", 0, rtl865x_proc_dir,
				&portRate_proc_fops, NULL);
#else
			portRate_entry =
				create_proc_entry("portRate", 0,
				rtl865x_proc_dir);
			if (portRate_entry != NULL) {
				portRate_entry->read_proc =
					rtl865x_proc_portRate_read;
				portRate_entry->write_proc =
					rtl865x_proc_portRate_write;
				retval = SUCCESS;
			}
#endif
		}
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("mmd", 0, rtl865x_proc_dir,
				&mmd_proc_fops, NULL);
#else
			mmd_entry =
				create_proc_entry("mmd", 0, rtl865x_proc_dir);
			if (mmd_entry != NULL) {
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("mac", 0, rtl865x_proc_dir,
				&mac_proc_fops, NULL);
#else
			mac_entry =
				create_proc_entry("mac", 0, rtl865x_proc_dir);
			if (mac_entry != NULL) {
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("fc_threshold", 0, rtl865x_proc_dir,
				&fc_threshold_proc_fops, NULL);
#else
			fc_threshold_entry =
				create_proc_entry("fc_threshold", 0,
				rtl865x_proc_dir);
			if (fc_threshold_entry != NULL) {
				fc_threshold_entry->read_proc =
					fc_threshold_read;
				fc_threshold_entry->write_proc =
					fc_threshold_write;
				retval = SUCCESS;
			} else {
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("l3v6", 0, rtl865x_proc_dir,
				&l3v6_proc_fops, NULL);
#else
			l3v6_entry =
				create_proc_entry("l3v6", 0, rtl865x_proc_dir);
			if (l3v6_entry != NULL) {
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("arp6", 0, rtl865x_proc_dir,
				&arp6_proc_fops, NULL);
#else
			arp6_entry =
				create_proc_entry("arp6", 0, rtl865x_proc_dir);
			if (arp6_entry != NULL) {
...
/*sw-arp6*/
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("sw-arp6", 0, rtl865x_proc_dir,
				&sw_arp6_proc_fops, NULL);
#else
			sw_arp6_entry =
				create_proc_entry("sw_arp6", 0,
				rtl865x_proc_dir);
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("nexthop6", 0, rtl865x_proc_dir,
				&nexthop6_proc_fops, NULL);
#else
			nexthop6_entry =
				create_proc_entry("nexthop6", 0,
				rtl865x_proc_dir);
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("mcast6", 0, rtl865x_proc_dir,
				&mcast6_proc_fops, NULL);
#else
			mcast6_entry =
				create_proc_entry("mcast6", 0,
				rtl865x_proc_dir);
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("6rd", 0, rtl865x_proc_dir,
				&sixrd_proc_fops, NULL);
#else
			ip6rd_entry =
				create_proc_entry("6rd", 0, rtl865x_proc_dir);
			if (ip6rd_entry != NULL) {
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("dslite", 0, rtl865x_proc_dir,
				&dslite_proc_fops, NULL);
#else
			dslite_entry =
				create_proc_entry("dslite", 0,
				rtl865x_proc_dir);
		...
		{
#ifdef CONFIG_RTL_PROC_NEW
			proc_create_data("sw_dslite", 0, rtl865x_proc_dir,
				&sw_dslite_proc_fops, NULL);
#else
			sw_dslite_entry =
				create_proc_entry("sw_dslite", 0,
				rtl865x_proc_dir);
...
int32 rtl865x_proc_debug_cleanup(void)
{
	if (rtl865x_proc_dir) {
		...
		if (priority_decision_entry) {
			remove_proc_entry("priority_decision",
				rtl865x_proc_dir);
		}
#endif
#if defined(CONFIG_RTL_LAYERED_DRIVER_L4)
