...
rtk_uint32  smi_SCK = MDC_GPIO;    /* GPIO used for SMI Clock Generation */
rtk_uint32  smi_SDA = MDIO_GPIO;    /* GPIO used for SMI Data signal */
rtk_uint32  smi_SCK_status = 0;
rtk_uint32  smi_SDA_status = 0;
...
#define GPIO_DIRECTION_SET(gpioID, direction)   \
                                    if(direction) {  \
                                        ...
                                        if (gpioID == smi_SDA) \
                                            gpio_direction_output(gpioID, smi_SDA_status); \
                                        else \
                                            gpio_direction_output(gpioID, smi_SCK_status); \
                                    }
...
#define GPIO_DATA_GET(gpioID, pData)  *pData = gpio_get_value(gpioID);
