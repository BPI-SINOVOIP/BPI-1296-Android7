...
#if defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_8370_SUPPORT) || defined(CONFIG_RTL_8325D_SUPPORT)
...
#if defined(CONFIG_RTL819X_GPIO) && defined(CONFIG_RTL_8367R_SUPPORT) && !defined(CONFIG_OPENWRT_SDK)
...
void init_8197f_p0(void)
{
	...
	REG32(P0GMIICR) = (REG32(P0GMIICR) & ~((1<<4)|(7<<0))) | ((0<<4) | (5<<0) | (3<<CF_SEL_RGTXC_OFFSET));
...
void initAsic_PHY(void)
{
	...
	uint32 phyId, phyData;
#endif //defined(CONFIG_FPGA_V6_HWNAT)
	uint32 p0_type_cfg;
	uint32 p0_phyId;

	rtl8651AsicEthernetTable[1].phyId = 8;
	rtl8651AsicEthernetTable[2].phyId = 9;
	rtl8651AsicEthernetTable[3].phyId = 10;
	/*** PHY ID is assigned by device tree ****
	rtl8651AsicEthernetTable[0].phyId = 1;
	rtl8651AsicEthernetTable[4].phyId = 3;
	rtl8651AsicEthernetTable[5].phyId = 6;
	 ******************************************/
	...
	if ((get_rtd129x_cpu_revision() != RTD129x_CHIP_REVISION_A00) &&
		...
		p0_phyId = 11 /* unused PHYID */;
	WRITE_MEM32(EPIDR, 	(Port_embPhyID(rtl8651AsicEthernetTable[4].phyId, 4) |
				Port_embPhyID(rtl8651AsicEthernetTable[3].phyId, 3) |
	...
	//turn on all advertise abilities except 1000M,
	//enable PHY interface of MAC0
	WRITE_MEM32(PCRP0, 	(READ_MEM32(PCRP0) & ~(EnForceMode | PollLinkStatus | NwayAbility1000MF)) |
				NwayAbility100MF |
				NwayAbility100MH |
				NwayAbility10MF |
				NwayAbility10MH |
	...
	//enable auto negotiation,
	//turn on all advertise abilities except 1000M,
	//enable PHY interface of MAC4
	WRITE_MEM32(PCRP4, 	(READ_MEM32(PCRP4) & ~(EnForceMode | PollLinkStatus | NwayAbility1000MF)) |
				NwayAbility100MF |
				NwayAbility100MH |
				NwayAbility10MF |
				NwayAbility10MH |
	...
	//arrange PHY ID for MAC5=PHY6,
	//enable auto negotiation,
	//turn on all advertise abilities except 1000M,
	//enable PHY interface of MAC5
	WRITE_MEM32(PCRP5, 	(READ_MEM32(PCRP5) & ~(ExtPHYID_MASK | EnForceMode | PollLinkStatus | NwayAbility1000MF)) |
				(rtl8651AsicEthernetTable[5].phyId << ExtPHYID_OFFSET) |
				NwayAbility100MF |
				NwayAbility100MH |
				NwayAbility10MF |
				NwayAbility10MH |
	...
	rtl865xC_setAsicEthernetMIIMode(5, LINK_MII_MAC);
	REG32(P5GMIICR) |= Conf_done;

	//enable auto negotiation,
	//turn off all advertise abilities,
	//disable PHY interface of MAC1
	WRITE_MEM32(PCRP1, 	(READ_MEM32(PCRP1) & ~(EnForceMode | PollLinkStatus | NwayAbility1000MF |
				NwayAbility100MF | NwayAbility100MH | NwayAbility10MF | NwayAbility10MH |
				EnablePHYIf)));

	//enable auto negotiation,
	//turn off all advertise abilities,
	//disable PHY interface of MAC2
	WRITE_MEM32(PCRP2, 	(READ_MEM32(PCRP2) & ~(EnForceMode | PollLinkStatus | NwayAbility1000MF |
				NwayAbility100MF | NwayAbility100MH | NwayAbility10MF | NwayAbility10MH |
				EnablePHYIf)));

	//enable auto negotiation,
	//turn off all advertise abilities,
	//disable PHY interface of MAC3
	WRITE_MEM32(PCRP3, 	(READ_MEM32(PCRP3) & ~(EnForceMode | PollLinkStatus | NwayAbility1000MF |
				NwayAbility100MF | NwayAbility100MH | NwayAbility10MF | NwayAbility10MH |
				EnablePHYIf)));
	...
	phyId = rtl8651AsicEthernetTable[4].phyId;
	rtl8651_getAsicEthernetPHYReg( phyId, 9, &phyData);
	phyData &= ~(CAPABLE_1000BASE_TX_FD | CAPABLE_1000BASE_TX_HD);
	rtl8651_setAsicEthernetPHYReg(phyId, 9, phyData);

	phyId = rtl8651AsicEthernetTable[5].phyId;
	rtl8651_getAsicEthernetPHYReg( phyId, 9, &phyData);
	phyData &= ~(CAPABLE_1000BASE_TX_FD | CAPABLE_1000BASE_TX_HD);
	rtl8651_setAsicEthernetPHYReg(phyId, 9, phyData);
	...
	if (hwnat_mac0_enable == 1 && hwnat_mac0_mode == 0 /*RGMII*/) {
		...
		/* write default value */
		WRITE_MEM32(PCRP0,
				(0x10 << ExtPHYID_OFFSET) |
				AutoNegoSts_MASK |
				PauseFlowControl_MASK |
				STP_PortST_MASK |
				MacSwReset);
	}
	...
	//turn on all advertise abilities,
	//enable PHY interface of MAC4
	WRITE_MEM32(PCRP4, 	(READ_MEM32(PCRP4) &
				~(EnForceMode | PollLinkStatus |
				PauseFlowControl_MASK | AcptMaxLen_16K)) |
				NwayAbility1000MF |
				NwayAbility100MF |
	...
	//turn off all advertise abilities,
	//disable PHY interface of MAC1
	WRITE_MEM32(PCRP1, 	(READ_MEM32(PCRP1) & ~(PollLinkStatus | NwayAbility1000MF |
				NwayAbility100MF | NwayAbility100MH | NwayAbility10MF | NwayAbility10MH |
	...
	//turn off all advertise abilities,
	//disable PHY interface of MAC2
	WRITE_MEM32(PCRP2, 	(READ_MEM32(PCRP2) & ~(PollLinkStatus | NwayAbility1000MF |
				NwayAbility100MF | NwayAbility100MH | NwayAbility10MF | NwayAbility10MH |
				EnablePHYIf)) | EnForceMode);
...
#elif defined(CONFIG_RTL_8197F) && defined(CONFIG_RTL_8367R_SUPPORT)
	init_8197f_p0();
...
#if (defined(CONFIG_RTL_8211DS_SUPPORT)&&defined(CONFIG_RTL_8197D)) || defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_8370_SUPPORT) || defined(CONFIG_RTL_8325D_SUPPORT)
	P0phymode = 3;
...
/* 	system clock = 50MHz --> token=0x40, tick=0x0b, l2=3*token=0xC0
	...
	#if !defined(CONFIG_RTD_1295_HWNAT)
	#if defined(PATCH_GPIO_FOR_LED) && !defined(CONFIG_RTL_8367R_SUPPORT)
...
int32 rtl865x_set_jumbo_frame_size(uint32 size, int op_mode)
{
	...
	WRITE_MEM32(PCRP5, (READ_MEM32(PCRP5)&(~AcptMaxLen_16K)) | MacSwReset ); /* Jumbo Frame */
	#else /* CONFIG_RTD_1295_HWNAT */
	WRITE_MEM32(PCRP0, (READ_MEM32(PCRP0)&(~AcptMaxLen_16K)) | MacSwReset ); /* Jumbo Frame */
	...
	switch(size){
		case 1552:
			...
			WRITE_MEM32(PCRP5, READ_MEM32(PCRP5) | (AcptMaxLen_1552|MacSwReset) ); /* Jumbo Frame */
			#else /* CONFIG_RTD_1295_HWNAT */
			WRITE_MEM32(PCRP0, READ_MEM32(PCRP0) | (AcptMaxLen_1552|MacSwReset) ); /* Jumbo Frame */
		...
		case 9000:
			...
			WRITE_MEM32(PCRP5, READ_MEM32(PCRP5) | (AcptMaxLen_9K|MacSwReset) ); /* Jumbo Frame */
			#else /* CONFIG_RTD_1295_HWNAT */
			WRITE_MEM32(PCRP0, READ_MEM32(PCRP0) | (AcptMaxLen_9K|MacSwReset) ); /* Jumbo Frame */
...
int32 rtl8651_setAsicEthernetPHYTxPadDriving(uint32 port)
{
	...
	/* change to page 0x0d41 */
	rtl8651_setAsicEthernetPHYReg(phyid, 31, 0x0d41);

	/* read current PHY reg 16 */
	...
	/* write PHY reg 16 */
	rtl8651_setAsicEthernetPHYReg(phyid, 16, data);
...
void rtl8651_resetAsicVlanRemark(void)
{
	...
	WRITE_MEM32(RMCR1P, 0xFAC688);

	/* clear vlan priority assignment */
	#if	defined(CONFIG_RTL_QOS_8021P_SUPPORT)
	WRITE_MEM32(LPTM8021Q, 0);
	#else
	WRITE_MEM32(LPTM8021Q, 0xFAC642);
	#endif
}
...
void rtl8651_resetAsicDscpRemark(void)
{
	...
	WRITE_MEM32(DSCPRM0, 0x2E000000);
	WRITE_MEM32(DSCPRM1, 0x2EBAE);

	/* clear dscp priority assignment */
	WRITE_MEM32(DSCPCR0,0);
	WRITE_MEM32(DSCPCR1,0);
	WRITE_MEM32(DSCPCR2,0);
	WRITE_MEM32(DSCPCR3,0);
...
int32 rtl8651_setAsicCPUPriorityToQIDMappingTable( enum PORTID port, enum PRIORITYVALUE priority, enum QUEUEID qid )
{
	...
	#if defined(CONFIG_RTD_1295_HWNAT)
	reg = (uintptr_t)(((uint32*)CPUQIDMCR0) + (port-CPU));
	#else //defined(CONFIG_RTD_1295_HWNAT)
