/*
 *  irq-rtk.c -  Realtek IRQ chip driver
 *
...
// interrupt flag map to interrupt enable
...
static void mux_irq_handle(unsigned int irq, struct irq_desc *desc)
{
    ...
    if (check_status == status) {
		if(count > 1)
			printk(KERN_ERR "[IRQ MUX] %s irq status is not change. clear it! (st:0x%08x en:0x%08x)\n", mux_data->index?"ISO":"MISC", status, enable);
		else
			count++;
