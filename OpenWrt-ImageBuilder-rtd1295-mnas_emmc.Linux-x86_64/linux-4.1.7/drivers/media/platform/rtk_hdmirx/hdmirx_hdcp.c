...
void HdmiRx_save_tx_ksv(struct hdcp_ksvlist_info *tx_ksv)
{
	...
	HDMI_PRINTF("Bksv(0x%02x %02x %02x %02x %02x)\n",
		tx_ksv_info.Bksv[4],tx_ksv_info.Bksv[3],tx_ksv_info.Bksv[2],tx_ksv_info.Bksv[1],tx_ksv_info.Bksv[0]);
...
void Hdmi_Hdcp1xRepeaterAuth(void)
{
	...
	HDMIRX_INFO("HDCP Bstatus:Byte1=0x%02x Byte2=0x%02x",Hdmi_HdcpPortRead(0x41),Hdmi_HdcpPortRead(0x42));//Readback Bstatus
	...
	for(k=7;k>=0;k--)
		An = (An<<8) | an_reg[k];
	//pr_info("[HDMI RX] HDCP An = 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x ,0x%llx\n",an_reg[0] ,an_reg[1],an_reg[2],an_reg[3],an_reg[4],an_reg[5],an_reg[6],an_reg[7],An);
	...
	*/
	...
	//Dump V'
	//for(k=0;k<20;k++)
		//pr_info("[HDMI RX] ReadBack V'(%u):0x%02x\n",k,Hdmi_HdcpPortRead(0x20+k));
...
static void hdcp1x_repeater_work_fun(struct work_struct *work)
{
	...
	hdmi_rx_reg_mask32(HDCP_FLAG2, ~HDCP_FLAG2_irq_aksv_en_mask, HDCP_FLAG2_irq_aksv_en_mask, HDMI_RX_MAC);//Enable AKSV interrupt
}
...
void Hdmi_HdcpFSM(void)
{
	...
	if (HDCP_FLAG1_get_wr_aksv_flag(hdcp_status_flag))//Tx write Aksv
	{
#ifdef CONFIG_RTK_HDCPRX_2P2
		// Disable HDCP 2.2
	...
	if(hdmi_ioctl_struct.hdcp_state==HDCPRX_STATE_1PX_DISCOVER)
	{
		...
		{
			hdmi_ioctl_struct.hdcp_state=HDCPRX_STATE_1PX_ON;// HDCP 1.x nego success
		}
	}
}
...
void HDMI_HDCP1p4_TEE_LOAD_KEY(void)
{
	...
	*/
	 ...
	 //pr_err("BKSV = %x ,%x ,%x ,%x ,%x ", rx_key.BKsv[0] , rx_key.BKsv[1] , rx_key.BKsv[2] , rx_key.BKsv[3] , rx_key.BKsv[4] );
	 }
	 /*
...
void Hdmi_HdcpInit(void)
{
	...
	hdmi_rx_reg_mask32(HDCP_PCR , ~HDCP_PCR_km_clk_sel_mask, HDCP_PCR_km_clk_sel_mask, HDMI_RX_MAC);
...
#ifdef CONFIG_RTK_HDCP1x_REPEATER
	Hdmi_HdcpPortWrite(0x40, HDCP_BCAPS_REPEATER);//Set Bcaps, repeater mode
	hdmi_rx_reg_mask32(HDCP_CR,~(_BIT7|_BIT0|_BIT6),(_BIT7|_BIT0|_BIT6), HDMI_RX_MAC);//BIT6:repeater
	Hdmi_HdcpPortWrite(0xc4,0x00); // keep old setting
	hdmi_rx_reg_mask32(HDCP_FLAG2, ~HDCP_FLAG2_irq_aksv_en_mask, HDCP_FLAG2_irq_aksv_en_mask, HDMI_RX_MAC);//Enable AKSV interrupt
#else
	Hdmi_HdcpPortWrite(0x40,HDCP_BCAPS_HDMI);//Set Bcaps
	// Enable HDCP function for all
	hdmi_rx_reg_mask32(HDCP_CR,~(_BIT7|_BIT0),(_BIT7|_BIT0), HDMI_RX_MAC);//for New_AC_Mode Enable,fix simplay bug
#endif
