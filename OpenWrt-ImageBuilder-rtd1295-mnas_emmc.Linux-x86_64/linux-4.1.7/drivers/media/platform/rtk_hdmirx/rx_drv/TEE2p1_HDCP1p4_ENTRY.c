...
unsigned char TA_HDCP1p4_Load_flag =0;
...
*/
//========================================================================
//pr_err("UUID = %x ,%x ,%x \n ",arg_HDCP1p4.uuid[0],arg_HDCP1p4.uuid[1],arg_HDCP1p4.uuid[2]);
...
void HDCP1p4_ta_deinit(void)
{
      int result;

	result = tee_client_close_session(ctx_HDCP1p4,arg_HDCP1p4.session);

	//pr_err("ta_deinit result =%x ",result);
}


unsigned char  T_KEY[5];
void  CA_hdcp1p4_RX_SET_KEY(void)
{
...
void CA_hdcp1p4_load(void)
{
       ...
       {
          //pr_err("CA_hdcp1p4_load \n");
          HDCP1p4_ta_init();
         CA_hdcp1p4_RX_SET_KEY();
	  HDCP1p4_ta_deinit();	 

        TA_HDCP1p4_Load_flag =1;
      	}
}
