...
void KS_Setting(unsigned char* bKs)
{
	...
	HDMIRX_DEBUG("[HDCP2.2]KS_Setting bKs  = 0x%x , 0x%x ,0x%x ,0x%x ,0x%x ,0x%x ,0x%x ,0x%x",(*bKs),*(bKs+1),*(bKs+2),*(bKs+3),*(bKs+4),*(bKs+5),*(bKs+6),*(bKs+7));
}
...
void Riv_Setting(unsigned char* bRiv)
{
	...
	HDMIRX_DEBUG("[HDCP2.2]Riv_Setting bRiv = 0x%x , 0x%x ,0x%x ,0x%x ,0x%x ,0x%x ,0x%x ,0x%x",(*bRiv),*(bRiv+1),*(bRiv+2),*(bRiv+3),*(bRiv+4),*(bRiv+5),*(bRiv+6),*(bRiv+7));
