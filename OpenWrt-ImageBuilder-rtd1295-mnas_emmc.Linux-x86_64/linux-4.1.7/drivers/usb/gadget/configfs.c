		...
		const char *page, size_t len)
{
	...
	if (!strlen(name) || strcmp(name, "none") == 0) {
		...
		if (usb_gadget_find_udc(name) == -ENODEV) {
			pr_info("%s: udc_name=%s usb_udc_attach_driver --> gadget device disconnected\n", __func__, name);
			rtk_dwc3_set_and_get_usb_gadget_driver(&gi->composite.gadget_driver);
			ret = 0;
		}
#endif
		if (ret)
