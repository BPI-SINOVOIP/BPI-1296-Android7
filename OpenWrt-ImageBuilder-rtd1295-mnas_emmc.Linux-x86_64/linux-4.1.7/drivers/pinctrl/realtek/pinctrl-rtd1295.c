...
static void RTK_pmx_set(struct pinctrl_dev *pctldev,unsigned pin,u8 config)
{
			  ...
			  unsigned offset)
{
	...
	{
		case PMUX_BASE_SB2:
			addr = pctl->sb2_membase + pin_regmap[offset].pmux_regoff;
			break;
		case PMUX_BASE_DISP:
			addr = pctl->disp_membase + pin_regmap[offset].pmux_regoff;
			break;
		case PMUX_BASE_CR:
			addr = pctl->cr_membase + pin_regmap[offset].pmux_regoff;
			break;
		case PMUX_BASE_ISO:
...
static struct of_device_id RTK_pinctrl_match[] = {
	{ .compatible = "rtk119x,rtk119x-pinctrl", .data = (void *)&rtk129x_pinctrl_data },
	{ /* Sentinel */ },
};
...
static int RTK_pinctrl_suspend(struct device *dev)
{
...
static int RTK_pinctrl_resume(struct device *dev)
{
	struct platform_device *pdev;
	struct RTK_pinctrl *pctl;
	void __iomem *addr;
...
static const struct dev_pm_ops rtk_pinctrl_pm_ops = {
