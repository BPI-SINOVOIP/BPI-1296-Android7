		...
		void *buffer, unsigned int flags, size_t size, int offset)
{
	...
	if(!vma){
		printk(KERN_ERR " Can't not find VMA\n");
	}
	...
	if (vma) {
		...
		if (!(vma->vm_flags & (VM_IO | VM_PFNMAP))){
			printk(KERN_ERR " vma->vm_flags error\n");
		...
		shm->paddr = PFN_PHYS(pfn);

		printk(KERN_ERR " vma->vm_start : %lx\n", vma->vm_start);
		printk(KERN_ERR " vma->vm_end : %lx\n", vma->vm_end);

		if (vma->vm_end - vma->vm_start < size) {
			dev_err(_DEV(tee), "%s %p:%lx not big enough: %lx - %d < %lx\n",
					__func__, buffer, (unsigned long)shm->paddr,
		...
		*/
		...
		int i;
		printk(KERN_ERR "It's not a VMA\n");
		...
		for (i = 1; i < nb_page; i++) {
			...
			if (pa != phys_base + i*PAGE_SIZE) {
				spin_unlock(&mm->page_table_lock);
				dev_err(_DEV(tee), "%s %p:%x not big enough: %lx - %d < %lx\n",
...
#endif
out:
	//dev_dbg(_DEV(tee), "%s: < %d shm=%p vaddr=%p paddr=%lx\n",
	//		__func__, ret, (void *)shm, buffer, (unsigned long)shm->paddr);
	return ret;
}
#endif
