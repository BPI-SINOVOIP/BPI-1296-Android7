...
#ifdef ISR_POLLING
#define ISR_POLLING_INTERVAL        (HZ>>4)
...
void mars_scd_fsm_active(mars_scd* p_this)
{
    ...
    if (event & SC_RCV_INT)  //RX received at least one character
   {
   	SC_INFO("RX Receive~~  \n");
...
{   
    ...
    p_this->pin_cmd_vcc      = 11;

    p_this->cmd_vcc_en       = dts_info.cmd_vcc_en;
    p_this->cmd_vcc_polarity = dts_info.cmd_vcc_polarity;
    p_this->pin_pwr_sel      = 12;
