...
local AVAILABLE_FMTS = {
	...
	fat32 = { path = "/usr/sbin/mkfs.vfat", option = "-F 32 -I" },
...
function index()
	...
	if executables_all_existed then
		...
		entry({"admin", "system", "disk"}, cbi("disk"), _("Disk Management"), 55)
...
function getdevices()
	...
	for i, device in pairs(target_devnames) do
		...
		if not isSystemMMC(device) and size > 0 then
			...
			table.insert(devices, device_info)
		end
	end
	return devices
end
...
function getRAIDdevices()
	...
	for line in mdstat:lines() do
		...
		if mdpath then
			...
			table.insert(raid_devices, device_info)
		end
	end
	mdstat:close()
