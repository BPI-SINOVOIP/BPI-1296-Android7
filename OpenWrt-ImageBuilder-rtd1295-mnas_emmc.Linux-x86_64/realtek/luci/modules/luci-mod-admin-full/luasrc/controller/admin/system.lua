...
function index()
	...
	entry({"admin", "system", "flashops", "upcomplete"}, call("action_isupcomplete"), nil).leaf = true
...
function action_flashops()
	...
	local reset_avail   = os.execute([[grep 'squashfs' /proc/mounts > /dev/null 2>&1]]) == 0
	...
	local function image_supported()
		local keep_alive = (not not luci.http.formvalue("keep_alive")) and "-k" or ""
		return (os.execute("sysupgrade -T %s %q >/dev/null" % {keep_alive, image_tmp}) == 0)
	end
	...
	elseif luci.http.formvalue("image") or luci.http.formvalue("step") then
		...
		if step == 1 then
			if image_supported() then
				luci.template.render("admin_system/upgrade", {
					...
					keep_alive = (not not luci.http.formvalue("keep_alive"))
				})
			else
				fs.unlink(image_tmp)
		...
		elseif step == 2 then
			...
			local keep_alive = (luci.http.formvalue("keep_alive") == "1")
			local uci = require("luci.model.uci").cursor()
			uci:set_confdir("/rom/etc/config")
			local ipaddr = uci:get("network","lan","ipaddr")
			luci.template.render("admin_system/applyreboot", {
				title = luci.i18n.translate("Flashing..."),
				msg   = luci.i18n.translate("The system is flashing now.<br /> DO NOT POWER OFF THE DEVICE!<br /> Wait a few minutes before you try to reconnect. It might be necessary to renew the address of your computer to reach the device again, depending on your settings."),
				addr  = (#keep > 0) and ipaddr or nil,
				polling = keep_alive
			})
			if keep_alive then
				fork_exec("touch /tmp/dbupgrade; /sbin/sysupgrade -k %s %q" %{ keep, image_tmp })
			else
	...
	elseif reset_avail and luci.http.formvalue("reset") then
		...
		fork_exec("ubus call system upgrade; killall dropbear uhttpd; rm -rf /overlay/*; sync; reboot -f")
	else
		--
		-- Overview
		--
...
function action_isupcomplete()
	local fs = require "nixio.fs"
	local rv = {}

	if fs.access("/tmp/install.log") then
		local install_log = fs.readfile("/tmp/install.log")

		start, _ = install_log:find("=== INSTALL SUCCESS ===")
		if start then rv["status"] = "success" end

		start, _ = install_log:find("=== INSTALL FAIL ===")
		if start then rv["status"] = "fail" end

		rv["install_log"] = install_log
	end

	-- Upgrading is done
	if not rv["status"] and not fs.access("/tmp/dbupgrade") then
		rv["status"] = "complete"
	end
