...
local cmssid_en = (uci:get("wireless", "cmssid", "enabled") == "1") and true or false
local cmssid = uci:get("wireless", "cmssid", "ssid")
...
function m.parse(map)
	if cmssid_en and (wnet:get("guest") ~= "enable") and (wnet:get("mode") == "ap") then
		wnet:set("ssid", cmssid)
		nw:commit("wireless")
	end
...
if found_sta then
	...
	if wdev:get("doth") == "1" then
		ch.freqlist = iw.freqlist
	else
		ch.freqlist = iw.no_dfs_freqlist
	end
...
if hwtype == "mac80211" then
	...
	bcn_int = s:taboption("advanced", Value, "beacon_int", translate("Beacon Interval"),
			translate("100-1024ms"))
	bcn_int.datatype = "and(uinteger,range(100,1024))"

	s:taboption("advanced", Flag, "doth", translate("802.11h"))
end
...
essid = s:taboption("general", Value, "ssid", translate("<abbr title=\"Extended Service Set Identifier\">ESSID</abbr>"))

guest = s:taboption("general", ListValue, "guest", translate("<abbr title=\"Guest Network\">Guest Network</abbr>"))
guest.override_values = true
guest.override_depends = true
guest:depends({mode="ap"})
guest:value("disable", translate("Disable"))
guest:value("enable", translate("Enable"))

function essid.write(self, section, value)
	local is_guest = tostring(self.map:get(section, "guest"))
	local ssid = (cmssid_en and (is_guest ~= "enable")) and cmssid or value
	wnet:set("ssid", ssid)
	nw:commit("wireless")
	Value.write(self, section, ssid)
end

function guest.write(self, section, value)
	ListValue.write(self, section, value)
	m.uci:set("wireless", section, "guest", value)
	
	local curssid = wnet:get("ssid")
	local newssid = (cmssid_en and (value ~= "enable")) and cmssid or curssid
	wnet:set("ssid", newssid)
	nw:commit("wireless")
end
...
if hwtype == "mac80211" then
	...
	if fs.access("/usr/bin/pathsel") then
		mode:value("rtkmesh", "rtk-mesh")
	end
	...
	txbf = s:taboption("general", Flag, "txbf", translate("Tx Beamforming"))
	txbf.default = txbf.disabled

	txbf_mu = s:taboption("general", Flag, "txbf_mu", translate("MU-MIMO"))
	txbf_mu.default = txbf_mu.disable
	...
	tdls = s:taboption("general", Flag, "tdls_prohibited", translate("TDLS Prohibited"))
	tdls:depends({mode="ap"})
	tdls:depends({mode="ap-wds"})
	tdls.default = tdls.disabled

	tdlscs = s:taboption("general", Flag, "tdls_cs_prohibited", translate("TDLS Channel Switch Prohibited"))
	tdlscs:depends({mode="ap"})
	tdlscs:depends({mode="ap-wds"})
	tdlscs.default = tdlscs.disabled

	ieee80211k = s:taboption("general", Flag, "ieee80211k", translate("802.11k Support"))
	ieee80211k:depends({mode="ap"})
	ieee80211k:depends({mode="ap-wds"})
	ieee80211k.default = ieee80211k.disabled

	smr = s:taboption("general", Flag, "smart_roaming", translate("Realtek Smart Roaming"))
	smr:depends({mode="rtkmesh"})
	smr.default = smr.disabled

	meshportal = s:taboption("general", Flag, "mesh_portal", translate("Mesh Portal(MPP)"))
	meshportal:depends({mode="rtkmesh"})
	meshportal.default = meshportal.disabled

	smlinktime = s:taboption("general", Value, "sm_linktime", translate("Smart Roaming Minimal Station Link Time"),
			translate("0-200s"))
	smlinktime.datatype = "and(uinteger,range(0,200))"
	smlinktime:depends({mode="rtkmesh"})
	smlinktime.default = 180
end
...
cipher:value("auto", translate("auto"), {mode="ap"}, {mode="sta"}, {mode="adhoc"}, {mode="ahdemo"}, {mode="ap-wds"}, {mode="sta-wds"})
cipher:value("ccmp", translate("Force CCMP (AES)"))
cipher:value("tkip", translate("Force TKIP"), {mode="ap"}, {mode="sta"}, {mode="adhoc"}, {mode="ahdemo"}, {mode="ap-wds"}, {mode="sta-wds"})
cipher:value("tkip+ccmp", translate("Force TKIP and CCMP (AES)"), {mode="ap"}, {mode="sta"}, {mode="adhoc"}, {mode="ahdemo"}, {mode="ap-wds"}, {mode="sta-wds"})
...
if hwtype == "atheros" or hwtype == "mac80211" or hwtype == "prism2" then
	...
	if hostapd and supplicant then
		...
		encr:value("psk2", "WPA2-PSK", {mode="ap"}, {mode="sta"}, {mode="ap-wds"}, {mode="sta-wds"}, {mode="rtkmesh"})
	...
	elseif hostapd and not supplicant then
		...
		encr:value("psk2", "WPA2-PSK", {mode="ap"}, {mode="ap-wds"}, {mode="rtkmesh"})
...
if hwtype == "atheros" or hwtype == "mac80211" or hwtype == "prism2" then
	...
	if hostcli and wpasupplicant then
		...
		wps.disabled = "0"
		wps.rmempty = false
		wps:depends({mode="ap", encryption="psk2", cipher="auto"})
		wps:depends({mode="ap", encryption="psk2", cipher="ccmp"})
		wps:depends({mode="ap", encryption="psk2", cipher="tkip+ccmp"})
		wps:depends({mode="ap", encryption="psk-mixed", cipher="auto"})
		wps:depends({mode="ap", encryption="psk-mixed", cipher="ccmp"})
		wps:depends({mode="ap", encryption="psk-mixed", cipher="tkip+ccmp"})

		wps:depends({mode="ap-wds", encryption="psk2", cipher="auto"})
		wps:depends({mode="ap-wds", encryption="psk2", cipher="ccmp"})
		wps:depends({mode="ap-wds", encryption="psk2", cipher="tkip+ccmp"})
		wps:depends({mode="ap-wds", encryption="psk-mixed", cipher="auto"})
		wps:depends({mode="ap-wds", encryption="psk-mixed", cipher="ccmp"})
		wps:depends({mode="ap-wds", encryption="psk-mixed", cipher="tkip+ccmp"})

		pin = s:taboption("encryption", Flag, "wps_label", translate("Enable WPS pin, requires WPA(2)-PSK"))
		pin.enabled = "1"
		pin.disabled = "0"
		pin.rmempty = false
		pin:depends({mode="ap", encryption="psk2", cipher="auto"})
		pin:depends({mode="ap", encryption="psk2", cipher="ccmp"})
		pin:depends({mode="ap", encryption="psk2", cipher="tkip+ccmp"})
		pin:depends({mode="ap", encryption="psk-mixed", cipher="auto"})
		pin:depends({mode="ap", encryption="psk-mixed", cipher="ccmp"})
		pin:depends({mode="ap", encryption="psk-mixed", cipher="tkip+ccmp"})

		pin:depends({mode="ap-wds", encryption="psk2", cipher="auto"})
		pin:depends({mode="ap-wds", encryption="psk2", cipher="ccmp"})
		pin:depends({mode="ap-wds", encryption="psk2", cipher="tkip+ccmp"})
		pin:depends({mode="ap-wds", encryption="psk-mixed", cipher="auto"})
		pin:depends({mode="ap-wds", encryption="psk-mixed", cipher="ccmp"})
		pin:depends({mode="ap-wds", encryption="psk-mixed", cipher="tkip+ccmp"})
	end
end
