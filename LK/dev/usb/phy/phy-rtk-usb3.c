...
static int do_usb3_phy_init(struct usb3_phy *usb3phy)
{
	...
	for (i = 0; i < phy_data_size; i++) {
		if (rtk_usb3_phy_write(usb3phy, phy_data[i].addr, phy_data[i].data)) {
			LTRACEF("[%s:%d], write_phy_addr = 0x%x PHY setting Error : addr = 0x%x, value = 0x%x\n",
					__func__, __LINE__, usb3phy->write_phy_addr,
					phy_data[i].addr,
					phy_data[i].data);
			return -1;
		} else
			LTRACEF("[%s:%d], write_phy_addr = 0x%x PHY setting : addr = 0x%x, value = 0x%x\n",
					__func__, __LINE__, usb3phy->write_phy_addr,
					phy_data[i].addr,
					phy_data[i].data);
	}
