...
//#define MCP_DEBUG
// FIXME : temporary remove this hard-code addr
#if 0
...
int reverse_rsa_signature(unsigned int addr)
{
        ...
        for(i=0;i<4;i++)
                backup_val[i] = swap_endian(rtd_inl(addr+(i*4)));
        rtd_outl(addr, swap_endian(rtd_inl(addr+28)));
        rtd_outl(addr+4, swap_endian(rtd_inl(addr+24)));
        rtd_outl(addr+8, swap_endian(rtd_inl(addr+20)));
        rtd_outl(addr+12, swap_endian(rtd_inl(addr+16)));
        rtd_outl(addr+16, backup_val[3]);
        rtd_outl(addr+20, backup_val[2]);
        rtd_outl(addr+24, backup_val[1]);
        rtd_outl(addr+28, backup_val[0]);
...
//FIXME : disable it since hard-code address is removed
#if 0 
int AES_ECB_encrypt(unsigned char * src_addr, unsigned int length, unsigned char * dst_addr, unsigned int key[4])
{
	...
	arch_clean_cache_range((addr_t)MEM_SRC_ADDR, length);
	#ifdef MCP_DEBUG
	rtk_hexdump( "mem_src :", (unsigned char*)MEM_SRC_ADDR, 16 );
	#endif
	dscpt->src_addr = (unsigned int)MEM_SRC_ADDR;
	dscpt->dst_addr = (unsigned int)MEM_DST_ADDR;
	dscpt->length = length;
	//flush_cache((unsigned int) MEM_DST_ADDR, length);
	arch_clean_cache_range((addr_t)MEM_DST_ADDR, length);
	...
	if (ret == 0)
	{
		//flush_cache((unsigned int) MEM_DST_ADDR, length);
		arch_clean_cache_range((addr_t)MEM_DST_ADDR, length);
		memcpy(dst_addr, (void*)MEM_DST_ADDR, length);
		//flush_cache((unsigned int) dst_addr, length);
		arch_clean_cache_range((addr_t)dst_addr, length);
		#ifdef MCP_DEBUG
		rtk_hexdump( "mem_dst :", (unsigned char*)MEM_DST_ADDR, 16 );
		#endif
		memset((void*)MEM_DST_ADDR, 0x00, length);
	}
...
int AES_hash(unsigned char * src_addr, unsigned int length, unsigned char * dst_addr, unsigned int block_size)
{
	...
	res = 0;
	while (length > 0) {
		dscpt->src_addr = PTR_TO_U32(src_ptr);
...
#if 1 // FIXME : Build fail using OTP related functions
...
int do_RSA(unsigned char* signature_addr, unsigned char* rsa_addr, unsigned char *output_addr)
{
	...
	if (rsa_addr == NULL)
        	...
        	copy_memory(0x9804CC00, rsa_addr, RSA_SIGNATURE_LENGTH);
    #endif
	...
	hexdump("rsa pub : ", (volatile unsigned int*) 0x9804cc00, 16);
        ...
        copy_memory(0x9804C780, signature_addr, RSA_SIGNATURE_LENGTH);	//msg.bin
	#ifdef MCP_DEBUG
	hexdump("c780 : ", 0x9804c780, 16);
	#endif
        copy_memory(0x9804C900, signature_addr + RSA_SIGNATURE_LENGTH, RSA_SIGNATURE_LENGTH); //RRModN.bin
	#ifdef MCP_DEBUG
	hexdump("c900 : ", 0x9804c900, 16);
	#endif
        //comment first, probably the value of experiment only
        rtd_outl(0x9804cf84, (unsigned int)swap_endian((unsigned int)(*(unsigned int*)(signature_addr+RSA_SIGNATURE_LENGTH+RSA_SIGNATURE_LENGTH))));  //np_inv32.bin
	#ifdef MCP_DEBUG
...
// FIXME : take it out since LK currently doesn't verify boot image
#if 0
...
//	sys_dcache_flush_all();
	...
	// using DDR
    rtd_outl(K_MCP_DES_COUNT, 0x00000001);
	// disable interrupt
	...
	rtd_outl(K_MCP_BASE, PTR_TO_U32(dscpt));
	rtd_outl(K_MCP_LIMIT, PTR_TO_U32(dscpt) + sizeof(t_mcp_descriptor) * 2);
	rtd_outl(K_MCP_RDPTR, PTR_TO_U32(dscpt));
	rtd_outl(K_MCP_WRPTR, PTR_TO_U32(dscpt) + sizeof(t_mcp_descriptor)); // writer pointer cannot be equal to limit

	//flush_cache((unsigned long)dscpt, sizeof(t_mcp_descriptor));
	arch_clean_cache_range((addr_t)dscpt, sizeof(t_mcp_descriptor));
	//flush_cache(dscpt->dst_addr, CONFIG_SYS_CACHELINE_SIZE);
	arch_clean_invalidate_cache_range((addr_t)dscpt->dst_addr, CACHE_LINE);
...
static t_mcp_descriptor *alloc_mcp_descriptor(void)
{
	...
	prints("\n");
#endif
	if (mcp_dscpt_addr_base == 0)
		mcp_dscpt_addr_base = mcp_dscpt_addr = memalign(256, CP_DSCPT_POOL_SIZE);
...
//FIXME : take it away since no need here
#if 0
void set_memory(void *dst, unsigned char value, unsigned int size)
{
        unsigned int i;
        for (i=0; i<size; i++)
                REG8(((unsigned int)dst) + i) = value;
}
