...
#include <platform/rbus/crt_reg.h>
#include <platform/rbus/edp_reg.h>
#include <platform/rbus/lvds_aif_reg.h>
#include <platform/rbus/lvds_pif_reg.h>
#include "rtk_rpc.h"
#include "../hdmitx/rtk_edid.h"
...
//		mdelay(5);
	...
	if((data[0] | data[1]<<4)==dpinfo.ltp1_check)
...
//		mdelay(5);
	...
	if((data[0] | data[1]<<4)==dpinfo.ltp2_check)
...
**
*************************************************************************/
static int get_one_step_info(void)
{
	...
	if (factory_read(BOOT_TVSYSTEM_FILE_NAME, &dst_addr, &dst_length)) {
		...
		{				
			memcpy(&checksum_blk0, dst_addr_chk ,sizeof(unsigned char));
			memcpy(&checksum_blk1, dst_addr_chk+sizeof(unsigned char),sizeof(unsigned char));	
			if(checksum_blk0==0 && checksum_blk1==0)
				return 0;			
		}
...
static int dptx_set_tv_system(int vid)
{
	...
	boot_info.tv_sys.videoInfo.pedType = SWAPEND32(VO_STANDARD_DP_FORMAT_1280_720P_60);
//	boot_info.tv_sys.videoInfo.pedType = SWAPEND32(VO_STANDARD_DP_FORMAT_1920_1080P_60);
	pedType = SWAPEND32(boot_info.tv_sys.videoInfo.pedType);
	if(pedType == VO_STANDARD_DP_FORMAT_1920_1080P_60)
		dpinfo.resolution = DP_FORMAT_1080P_60;
	else if(pedType == VO_STANDARD_DP_FORMAT_2160P_30)
...
static void dptx_pll_setting(void)
{
	unsigned int reg;
	int i;
	...
	mdelay(1);
	rtd_outl(CRT_BASE+0x604, 0x0000F000);
	mdelay(1);
