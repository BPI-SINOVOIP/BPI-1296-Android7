...
int IR_config_wakeup_keys(void *addr)
{
	...
	tbl->key_tbl[0].protocol = SWAPEND32(IR_PROTOCOL);
	tbl->key_tbl[0].ir_scancode_mask = SWAPEND32(IR_SCANCODE_MASK);
	tbl->key_tbl[0].ir_wakeup_scancode = SWAPEND32(IR_WAKEUP_SCANCODE);
	tbl->key_tbl[0].ir_cus_mask = SWAPEND32(IR_CUS_MASK);
	tbl->key_tbl[0].ir_cus_code = SWAPEND32(IR_CUS_CODE);
	return 0;
}
