...
int pwm_set_freq(int pwm_number, int freq)
{
	if (pwm_number < 0 || pwm_number > 3) {
...
int pwm_set_duty_rate(int pwm_number, int duty_rate)
{
	if (pwm_number < 0 || pwm_number > 3) {
...
{
	u32 value = 0;
	printf("%s default value %u\n", __func__, value); // Dummy to prevent compile error
...
{
	printf("\t show                      --- show current setting of PWM 0 ~ 3\n"
...
int do_pwm(int argc, const cmd_args *argv)
{
	ulong id;
	unsigned int value;
	int cmd_err = 0;
	...
	if (strcmp(argv[1].str, "show") == 0) {
		rtd129x_pwm_show();
	} else if (strcmp(argv[1].str, "enable") == 0) {
		if (argc == 4) {
			id = strtoul(argv[2].str, NULL, 10);
			value = strtoul(argv[3].str, NULL, 10);
			if (pwm_enable(id, value))
				return -1;
		} else {
			cmd_err = 1;
		}
	} else if (strcmp(argv[1].str, "duty") == 0) {
		if (argc == 4) {
			id = strtoul(argv[2].str, NULL, 10);
			value = strtoul(argv[3].str, NULL, 10);
			if (pwm_set_duty_rate(id, value))
				return -1;
		} else if (argc == 3) {
			id = strtoul(argv[2].str, NULL, 10);
			if (pwm_get_duty_rate(id, &value) == 0)
				printf("PWM %ld duty rate %d%%\n", id, value);
		} else {
			cmd_err = 1;
		}
	} else if (strcmp(argv[1].str, "freq") == 0) {
		if (argc == 4) {
			id = strtoul(argv[2].str, NULL, 10);
			value = strtoul(argv[3].str, NULL, 10);
			if (pwm_set_freq(id, value))
				return -1;
		} else if (argc == 3) {
			id = strtoul(argv[2].str, NULL, 10);
			if (pwm_get_freq(id, &value) == 0) {
				printf("PWM %ld output frequency %d HZ\n", id, value);
				return 0;
			}
		} else {
			cmd_err = 1;
...
STATIC_COMMAND("pwm", "Control PWM 0,1,2,3", (console_cmd)&do_pwm)
