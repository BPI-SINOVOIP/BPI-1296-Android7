...
static int rtk_rpmb_read_block(void *buff ,bnum_t block , uint count){
...
static int rtk_rpmb_write_block( void *buff , bnum_t block , uint count){
...
//	 }
	...
	if(memcmp(on_chip_kf,compare_kf,KF_LEN)!=0){
	 ...
	 for(index =0;index<16;index++){
		printf("kf[%d]:%02x\n",index ,(unsigned char)on_chip_kf[index]);
	}
	 return 0;
...
static void print_rpmb_usage(void){
	printf("rpmb write buff_addr,dest_block,block_count \n");
	printf("rpmb read  buff_addr,start_block,block_count \n");
...
int do_rtkRpmb(int argc, const cmd_args *argv){
	...
	if(!strncmp(argv[1].str,"write",5)){

			emmc_switch_partition(MMC_PART_RPMB);
			rtk_rpmb_write_block(buff_addr,dest_block,count);
			emmc_switch_partition(MMC_PART_UDA);

	}else if(! strncmp(argv[1].str, "read",4)){

			emmc_switch_partition(MMC_PART_RPMB);
			rtk_rpmb_read_block(buff_addr,dest_block,count);
			emmc_switch_partition(MMC_PART_UDA);
