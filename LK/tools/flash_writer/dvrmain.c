...
int dvrmain	( int argc, char * const argv[] )
{
	...
	if( flash_magicno == FLASH_MAGICNO_NAND )
	{
		...
		if ((*do_identify)(&device) < 0)
		{
			...
			NF_dump_flash(device, 0x03000000, 0, 0x440);//dump 15 block, fsbl block 14
...
program_backup_copy_of_hwsetting:
...
#endif
#if defined(Config_FSBL_OS_TRUE) && !defined(VMX_3RD_PARTY) && !defined(VMX_BL)
        //fsbl os
...
#endif
#if defined(Config_BL31_TRUE) && !defined(VMX_3RD_PARTY) && !defined(VMX_BL)
        //bl31
...
#endif
#if defined(Config_Secure_Improve_TRUE) && !defined(VMX_3RD_PARTY) && !defined(VMX_BL)
        //Kpublic_fw
	...
	else if ( flash_magicno == FLASH_MAGICNO_EMMC)
	{
		...
		#ifdef VMX_BL
		block_no = EMMC_HW_SETTING_BLK+(0x16800/512); //3rd party bl is 90KB
		#else
...
/* =======================================
 *        custome for humax usage
 * =========================================
 */
#ifdef BootPartition_StartUp  //which means run boot partition flow

		/*  make up bootpartiton header info */
		t_boot_header boot_part_header;
		set_memory(&boot_part_header,0x00,sizeof(struct boot_partition_header)); //clean
		boot_part_header.header_size   = 512;//header size
		boot_part_header.bootcode_size = programmed_img_size     ; //bootcode size
 ...
 #if defined(Config_Secure_Improve_TRUE)
		boot_part_header.RSA_FW_size   = programmed_Kpublic_fw_size + programmed_Kpublic_fw_sig_size;
	 	boot_part_header.RSA_TEE_size  = programmed_Kpublic_tee_size + programmed_Kpublic_tee_sig_size;
 #endif
 ...
 */
		...
		if(verify_after_write){
			if( EXIT_FAILURE == BootPartOps.verify_data(device,(unsigned char *)DATA_TMP_ADDR,boot_part_dest_block,programmed_img_size)){
				prints("verify bootcode  fail\n");
				return -6;
			}else
				prints("verify bootcode  OK\n");
				prints("write to block:");
...
/* ========================================= */
		...
		if(( programmed_fsbl_size != 0 )&&( programmed_fsbl_sig_size != 0 )) {
			...
			#ifdef BootPartition_StartUp  //which means run boot partition flow
			block_no += 1;	//according to hw setting header,we setup bootcode area to 1 block,so does it here
			#else
...
#if !defined(VMX) && !defined(VMX_3RD_PARTY) && !defined(VMX_BL)
		/***********************************************************************
...
#endif  // VMX,VMX_3RD_PARTY,VMX_BL end
