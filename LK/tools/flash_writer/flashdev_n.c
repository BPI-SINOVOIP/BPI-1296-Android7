...
//20170124
UINT32 nf_page_size;
#define DVR_init_erase_block_2k_page   74    // modify for image layout 
#define DVR_init_erase_block_4k_page   50    // modify for image layout 
//-----------------------------------------//
...
retry_all_one:
	...
	#ifdef Board_CPU_RTD1395 //hercules
		if ((REG_DMA_CONF&0xf000)==NF_SWC_REG)
		{
		rtd_outl(REG_DMA_CONF,(0x12));  //DMA => DDR
		rtd_outl(DMA_CTL4,0x01);
		rtd_outl(DMA_CTL5,0x10);
		rtd_outl(DMA_CTL6,0x01);
		}
		else  if ((REG_DMA_CONF&0xf000)==NF_NWC_REG)  //NSW
		{
	...
	{
	      //prints("SWC REG \n");
		while( (rtd_inl(REG_DMA_CONF) & 0x1) ==0x00 );
	}
	else  if ((REG_DMA_CONF&0xf000)==NF_NWC_REG)  //NSW
	{
	  //prints("NWC REG \n");
...
static int nf_write_phys_page( n_device_type *device, UINT8 chip_sel, UINT32 page_no, UINT8 *buf)
{
	...
	sync();
		...
		if ((REG_DMA_CONF&0xf000)==NF_SWC_REG)
		{
		//rtd_outl(REG_DMA_CONF,(rtd_inl(REG_DMA_CONF)&(~0x13))|0x00);
		rtd_outl(REG_DMA_CONF,(0x10));
		rtd_outl(DMA_CTL4,0x01);
		rtd_outl(DMA_CTL5,0x10);
		rtd_outl(DMA_CTL6,0x01);
		}
		...
		for (idx = 0; idx < DEV_SIZE_N; idx++)
		{
			...
			*dev = (void *)&n_device[idx];
			...
			blocks_per_flash += die_size / n_device[idx].BlockSize;
                    nf_page_size = n_device[idx].PageSize;
			prints("pages size  = ");
			print_hex(nf_page_size);
			prints("\n");
...
int do_init_n(void *dev)
{
	...
	if (nf_page_size =2048)     erase_block_bound = DVR_init_erase_block_2k_page;
	else                                    erase_block_bound = DVR_init_erase_block_4k_page;
	#if 1
		// erase all.
