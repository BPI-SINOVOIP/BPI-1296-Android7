...
void Generate_HwSettingHeader_for_BootPartition(){
	 ...
	 *    ---------------
	 *  | bootcode size   | ==> shrink to 1 block
	 *    ---------------
	 *  | fsbl size       | ==> real size
	 *   -----------------
	...
	printf("bootcode size : %08x ,set to 1\n", uboot_size);
	...
	if (ptr_Rescue) //maybe it's outdated
...
//0:succeess  1:fail
void Generate_HwSettingHeader_for_VMX(){
	 ...
	 *  | hwsetting size  | ==> real size
	 *    ---------------
	 *  | bootcode size   | ==> 100 byte (zero filled data)
	 *    ---------------
	 *  | fsbl size       | ==> real size
	 *   -----------------
	 *  | tee os size     | ==> 0 byte
	...
	unsigned int padding_len=0;
#ifdef VMX
	printf("generate header for VMX : %s \n", Hwsetting_header_file);	
#elif VMX_3RD_PARTY
	printf("generate header for VMX 3RD PARTY BL : %s \n", Hwsetting_header_file);	
#elif VMX_BL
	printf("generate header for VMX BL : %s \n", Hwsetting_header_file);	
#endif
	char buf[1024]={0};
	memset(buf,0x00,1024);
	tmp_r=tmp_w=0;
	...
	w_ret += fwrite(buf, 4, 1, ptr_HWsetting_header);
	printf("tee size : %08x ,set to 0\n", secure_fsbl_os_size);

	w_ret += fwrite(buf, 4, 1, ptr_HWsetting_header);
	printf("bl31 size : %08x ,set to 0\n", bl31_size);

	w_ret += fwrite(buf, 4, 1, ptr_HWsetting_header);
	printf("rsa_bin_fw size : %08x ,set to 0\n", secure_rsa_bin_fw_size);

	w_ret += fwrite(buf, 4, 1, ptr_HWsetting_header);
	printf("rsa_bin_tee size : %08x ,set to 0\n", secure_rsa_bin_tee_size);
...
*/
	...
	#elif VMX
		Generate_HwSettingHeader_for_VMX();
	#elif VMX_3RD_PARTY
		Generate_HwSettingHeader_for_VMX();
	#elif VMX_BL
		Generate_HwSettingHeader_for_VMX();
