...
static struct ehci_ctrl ehcic[USB_MAX_CONTROLLER_COUNT];

#define ALIGN_END_ADDR(type, ptr, size)			\
	((uint32_t)(unsigned long)0 /*(ptr)*/ + roundup((size) * sizeof(type), USB_DMA_MINALIGN))
...
static int ehci_td_buffer(struct qTD *td, void *buf, size_t sz)
{
	...
	arch_clean_invalidate_cache_range(addr, ALIGN(0 /*addr*/ + sz, ARCH_DMA_MINALIGN));
	idx = 0;
		   ...
		   int length, struct devrequest *req)
{
	...
	struct QH *qh;
        qh = (struct QH *)memalign(64 ,1*sizeof(struct QH));
...
#define PKT_ALIGN	512
	...
	if (length > 0 || req == NULL) {
		 ...
		 */
		if ((unsigned int)(unsigned long)buffer & (PKT_ALIGN - 1))
			xfr_sz--;
...
//#endif
	 ...
	 */
	qh->qh_link = cpu_to_hc32((uint32_t)(unsigned long)&ctrl->qh_list | QH_LINK_TYPE_QH);
	...
	if (req != NULL) {
		...
		/* Update previous qTD! */
		*tdp = cpu_to_hc32((uint32_t)(unsigned long)&qtd[qtd_counter]);
	...
	if (length > 0 || req == NULL) {
		...
		do {
			 ...
			 */
			xfr_bytes -= (uint32_t)(unsigned long)buf_ptr & (EHCI_PAGE_SIZE - 1);
			/*
	...
	ctrl->qh_list.qh_link = cpu_to_hc32((uint32_t)(unsigned long)qh | QH_LINK_TYPE_QH);

	/* Flush dcache */
	arch_clean_invalidate_cache_range((uint32_t)(unsigned long)&ctrl->qh_list,
		ALIGN_END_ADDR(struct QH, &ctrl->qh_list, 1));
	arch_clean_invalidate_cache_range((uint32_t)(unsigned long)qh, ALIGN_END_ADDR(struct QH, qh, 1));
	arch_clean_invalidate_cache_range((uint32_t)(unsigned long)qtd,
			   ALIGN_END_ADDR(struct qTD, qtd, qtd_count));

	/* Set async. queue head pointer. */
	ehci_writel(&ctrl->hcor->or_asynclistaddr, (uint32_t)(unsigned long)&ctrl->qh_list);
	...
	ts = current_time();//get_timer(0);
	vtd = &qtd[qtd_counter - 1];
	timeout = ts + USB_TIMEOUT_MS(pipe);
	do {
		/* Invalidate dcache */
		arch_invalidate_cache_range((uintptr_t)(unsigned long)&ctrl->qh_list,
			ALIGN_END_ADDR(struct QH, &ctrl->qh_list, 1));
		arch_invalidate_cache_range((uintptr_t)(unsigned long)qh,
			ALIGN_END_ADDR(struct QH, qh, 1));
		arch_invalidate_cache_range((uintptr_t)(unsigned long)qtd,
			ALIGN_END_ADDR(struct qTD, qtd, qtd_count));
		...
		if (!(QT_TOKEN_GET_STATUS(token) & QT_TOKEN_STATUS_ACTIVE))
			break;
		//WATCHDOG_RESET();
	} while (/*get_timer(ts)*/current_time() < timeout);
	 ...
	 */
	arch_invalidate_cache_range((uintptr_t)(unsigned long)buffer,
		ALIGN((uint32_t)(unsigned long)/*buffer*/0 + length, ARCH_DMA_MINALIGN));
		 ...
		 int length, struct devrequest *req)
{
	...
	struct ehci_ctrl *ctrl = dev->controller;
        uint8_t dtbuf1[] = "\4\3\1\0";
        uint8_t dtbuf2[] = "\16\3u\0-\0b\0o\0o\0t\0";
 	uint8_t dtbuf3[] = "\52\3E\0H\0C\0I\0 "
			 "\0H\0o\0s\0t\0 "
	...
	case DeviceRequest | USB_REQ_GET_DESCRIPTOR:
		...
		case USB_DT_STRING:
			...
			case 0:	/* Language */
				srcptr = dtbuf1; //"\4\3\1\0";
				srclen = 4;
				break;
			case 1:	/* Vendor */
				srcptr = dtbuf2; //"\16\3u\0-\0b\0o\0o\0t\0";
				srclen = 14;
				break;
			case 2:	/* Product */
				srcptr = dtbuf3; //"\52\3E\0H\0C\0I\0 "
					 //"\0H\0o\0s\0t\0 "
					 //"\0C\0o\0n\0t\0r\0o\0l\0l\0e\0r\0";
				srclen = 42;
				break;
...
int ehci_lowlevel_init(int index, void **controller)
{
	uint32_t reg;
	uint32_t cmd;
	struct QH *qh_list;
	...
	if (ehci_readl(&ehcic[index].hccr->cr_hccparams) & 1)
		ehci_writel((unsigned long)ehcic[index].hcor->or_ctrldssegment, 0);

	qh_list = &ehcic[index].qh_list;

	/* Set head of reclaim list */
	memset(qh_list, 0, sizeof(*qh_list));
	qh_list->qh_link = cpu_to_hc32((uint32_t)(unsigned long)qh_list | QH_LINK_TYPE_QH);
	...
	arch_clean_invalidate_cache_range((uintptr_t)qh_list,
			   ALIGN_END_ADDR(struct QH, qh_list, 1));

	/* Set async. queue head pointer. */
	ehci_writel(&ehcic[index].hcor->or_asynclistaddr, (uint32_t)(uintptr_t)qh_list);
	...
	arch_clean_invalidate_cache_range((uintptr_t)periodic,
	...
	if (!ehcic[index].periodic_list)
		return -ENOMEM;
	for (i = 0; i < 1024; i++) {
		ehcic[index].periodic_list[i] = (uint32_t)(uintptr_t)periodic
						| QH_LINK_TYPE_QH;
	}

	arch_clean_invalidate_cache_range((uintptr_t)ehcic[index].periodic_list,
			   ALIGN_END_ADDR(uint32_t, ehcic[index].periodic_list,
					  1024));

	/* Set periodic list base address */
	ehci_writel(&ehcic[index].hcor->or_periodiclistbase,
		(uint32_t)(uintptr_t)ehcic[index].periodic_list);
	...
	ehcic[index].ctrl_type = CTRL_TYPE_EHCI;
		   ...
		   int length, struct devrequest *setup)
{
	...
	debug("%s %d usb_pipedevice(pipe)= %lu , ctrl->rootdev=%d \n", __func__,__LINE__,usb_pipedevice(pipe), ctrl->rootdev);//hcy added 

	if (usb_pipedevice(pipe) == (unsigned int)ctrl->rootdev) {
		if (!ctrl->rootdev)
...
#define NEXT_QH(qh) (struct QH *)(uintptr_t)((qh)->qh_link & ~0x1f)
		 ...
		 int elementsize, void *buffer)
{
	...
	for (i = 0; i < queuesize; i++) {
		...
		qh->qh_link = (uint32_t)(uintptr_t)(qh+1) | QH_LINK_TYPE_QH;
		if (i == queuesize - 1)
			qh->qh_link = QH_LINK_TERMINATE;

		qh->qh_overlay.qt_next = (uint32_t)(uintptr_t)td;
		...
		td->qt_token = (elementsize << 16) |
			...
			0x80; /* active */
		td->qt_buffer[0] = (uint32_t)(uintptr_t)buffer + i * elementsize;
	...
	arch_clean_invalidate_cache_range((uintptr_t)buffer,
			   ALIGN_END_ADDR(char, buffer,
					  queuesize * elementsize));
 	arch_clean_invalidate_cache_range((uintptr_t)result->first,
			   ALIGN_END_ADDR(struct QH, result->first,
					  queuesize));
	arch_clean_invalidate_cache_range((uintptr_t)result->tds,
	...
	list->qh_link = (uint32_t)(uintptr_t)result->first | QH_LINK_TYPE_QH;

	arch_clean_invalidate_cache_range((uintptr_t)result->last,
			   ALIGN_END_ADDR(struct QH, result->last, 1));
	arch_clean_invalidate_cache_range((uintptr_t)list,
...
void *poll_int_queue(struct usb_device *dev, struct int_queue *queue)
{
	...
	/* still active */
	arch_invalidate_cache_range((uintptr_t)cur,
...
destroy_int_queue(struct usb_device *dev, struct int_queue *queue)
{
	...
	timeout = current_time() + 500; /* abort after 500ms */
	       ...
	       int length, int interval)
{
	...
	timeout = current_time() + USB_TIMEOUT_MS(pipe);
	...
	if (backbuffer != buffer) {
		debug("got wrong buffer back (%x instead of %x)\n",
		      (uint32_t)(unsigned long)backbuffer, (uint32_t)(unsigned long)buffer);
		return -EINVAL;
	}

	arch_invalidate_cache_range((uintptr_t)buffer,
