...
static struct xhci_ctrl xhcic[USB_MAX_CONTROLLER_COUNT];
...
static int xhci_configure_endpoints(struct usb_device *udev, bool ctx_change)
{
	...
	xhci_flush_cache((addr_t)in_ctx->bytes, in_ctx->size);
...
static int xhci_set_configuration(struct usb_device *udev)
{
	...
	xhci_inval_cache((addr_t)out_ctx->bytes, out_ctx->size);
...
static int xhci_address_device(struct usb_device *udev)
{
	...
	ctrl_ctx->drop_flags = 0;
	xhci_flush_cache((addr_t)ctrl_ctx, virt_dev->in_ctx->size);
	...
	xhci_inval_cache((addr_t)virt_dev->out_ctx->bytes,
...
void xhci_release_device(struct usb_device *udev)
{
	xhci_release_virt_device(udev);
}
		...
		u16 wIndex, volatile uint32_t *addr, u32 port_status)
{
	...
	case USB_PORT_FEAT_C_RESET:
		status = PORT_RC;
		strcpy(port_change_bit , "reset");
		break;
	case USB_PORT_FEAT_C_CONNECTION:
		status = PORT_CSC;
		strcpy(port_change_bit , "connect");
		break;
	case USB_PORT_FEAT_C_OVER_CURRENT:
		status = PORT_OCC;
		strcpy(port_change_bit , "over-current");
		break;
	case USB_PORT_FEAT_C_ENABLE:
		status = PORT_PEC;
		strcpy(port_change_bit ,  "enable/disable");
		break;
	case USB_PORT_FEAT_C_SUSPEND:
		status = PORT_PLC;
		strcpy(port_change_bit , "suspend/resume");
		break;
	default:
		/* Should never happen */
		return;
...
extern void rtk_usb3_phy_state(uint8_t state);//hcy added
/**
			...
			void *buffer, struct devrequest *req)
{
	...
	uint8_t dtbuf1[] = "\4\3\11\4"; 
        uint8_t dtbuf2[] = "\16\3u\0-\0b\0o\0o\0t\0";
        uint8_t dtbuf3[] = "\52\3X\0H\0C\0I\0 "
				 "\0H\0o\0s\0t\0 "
	...
	case DeviceRequest | USB_REQ_GET_DESCRIPTOR:
		...
		case USB_DT_STRING:
			...
			case 1:	/* Vendor String  */
				...
				break;
			case 2:	/* Product Name */
				srcptr = dtbuf3; //"\52\3X\0H\0C\0I\0 "
					 //"\0H\0o\0s\0t\0 "
					 //"\0C\0o\0n\0t\0r\0o\0l\0l\0e\0r\0";
				srclen = 42;
				break;
					...
					int length, struct devrequest *setup)
{
	...
	debug("%s %d usb_pipedevice(pipe)= %d , ctrl->rootdev=%d \n", __func__,__LINE__,(int)usb_pipedevice(pipe), (int)ctrl->rootdev);//hcy added 
 ...
 */
int xhci_lowlevel_init(int index, void **controller)
{
	uint32_t val;
	uint32_t val2;
	uint32_t reg;
	...
	xhcic[index].ctrl_type = CTRL_TYPE_XHCI;
