...
int usb_init(void)
{
	...
	for (i = 0; i < usb_max_controller_count; i++) {
		...
		//printf("delay 2sec for device ready...\n", i);
		//mdelay(2000);
...
#ifdef CONFIG_USB_UPDATE_WHEN_AC_ON
	if (is_usb_ac_on && ((dev_index - start_index) > 2)) {
			...
			void *data, unsigned short size, int timeout)
{
	...
	struct devrequest *setup_packet;
        setup_packet = (struct devrequest *)memalign(64 ,1024*sizeof(struct devrequest));
	...
	debug("usb_control_msg: request: 0x%.2X, requesttype: 0x%.2X, " \
	      "value 0x%.4X index 0x%.2X length 0x%.2X\n",
			...
			void *data, int len, int *actual_length, int timeout)
{
	if (len < 0)
		return -1;
	dev->status = USB_ST_NOT_PROC; /*not yet processed */
	if (submit_int_msg(dev, pipe, data, len, timeout) < 0)
		return -1;
	while (timeout--) {
		if (!((volatile unsigned long)dev->status & USB_ST_NOT_PROC))
			break;
...
{
	//ALLOC_CACHE_ALIGN_BUFFER(unsigned char, mybuf, 1024);
	unsigned char *tbuf;
	int err;
	unsigned int u, idx;
	unsigned char  *mybuf;
        mybuf = (unsigned char*)memalign(64 ,1024*sizeof(unsigned char));
...
void usb_free_device(void)
{
	...
	usb_release_device(&usb_dev[dev_index]);
...
int usb_new_device(struct usb_device *dev)
{
	int addr, err;
	int tmp;
	//ALLOC_CACHE_ALIGN_BUFFER(unsigned char, tmpbuf, 1024);
	unsigned char  *tmpbuf;
        tmpbuf = (unsigned char*)memalign(64 ,1024*sizeof(unsigned char));
	...
	case 8:
		debug("%s %d PACKET_SIZE_8\n",__func__,__LINE__);//hcy added

		dev->maxpacketsize  = PACKET_SIZE_8;
		break;
	case 16:
		debug("%s %d PACKET_SIZE_16\n",__func__,__LINE__);//hcy added

		dev->maxpacketsize = PACKET_SIZE_16;
		break;
	case 32:
		debug("%s %d PACKET_SIZE_32\n",__func__,__LINE__);//hcy added

		dev->maxpacketsize = PACKET_SIZE_32;
		break;
	case 64:
		debug("%s %d PACKET_SIZE_64\n",__func__,__LINE__);//hcy added
