...
//#define XHCI_DEBUG_TRB

#define BUG()	panic("xhci die\n");
#define BUG_ON(condition) do { if (condition) BUG(); } while(0)
#define get_timer(ts)  current_time() - ts 
					  ...
					  unsigned int *trb_fields)
{
	...
	printf("#@# %s(%d) 0x%.8x - 0x%.8x 0x%.8x 0x%.8x 0x%.8x\n", __func__, __LINE__,(unsigned int) (uintptr_t)trb, trb->field[0], trb->field[1], trb->field[2], trb->field[3]);
#endif // add by cfyeh
...
#ifdef XHCI_DEBUG_TRB // add by cfyeh
	printf("#@# %s(%d) 0x%.8x - 0x%.8x 0x%.8x 0x%.8x 0x%.8x\n", __func__, __LINE__, (unsigned int) (uintptr_t)start_trb, start_trb->field[0], start_trb->field[1], start_trb->field[2], start_trb->field[3]);
#endif // add by cfyeh
...
union xhci_trb *xhci_wait_for_event(struct xhci_ctrl *ctrl, trb_type expected)
{
	...
	if (expected == TRB_TRANSFER){
		printf(" XHCI expected == TRB_TRANSFER \n"); 
		return NULL;
	}
...
static void abort_td(struct usb_device *udev, int ep_index)
{
	...
	BUG_ON(TRB_TO_EP_INDEX(field) != (unsigned int)ep_index);
...
{
	udev->act_len = min(length, length -(int)
			...
			int length, void *buffer)
{
	...
	BUG_ON(TRB_TO_SLOT_ID(field) != (unsigned int)slot_id);
	BUG_ON(TRB_TO_EP_INDEX(field) != (unsigned int)ep_index);
