...
void xhci_flush_cache(uint32_t addr, u32 len)
{
	...
	arch_clean_invalidate_cache_range(addr & ~(CACHELINE_SIZE - 1),
				ALIGN(/*addr*/0 + len, CACHELINE_SIZE));
}
...
void xhci_inval_cache(uint32_t addr, u32 len)
{
	...
	arch_invalidate_cache_range(addr & ~(CACHELINE_SIZE - 1),
				ALIGN(/*addr*/0 + len, CACHELINE_SIZE));
}
...
void xhci_release_virt_device(struct usb_device *udev)
{
	...
	xhci_flush_cache((uintptr_t)ctrl->dcbaa,
...
#else
	ptr = memalign(64/*0x400*/, ALIGN(size, 64/*0x400*/));
#endif // add by cfyeh
	memset(ptr, '\0', size);
				...
				struct xhci_segment *next, bool link_trbs)
{
	...
	if (link_trbs) {
		...
		arch_clean_invalidate_cache_range( (uintptr_t)&prev->trbs[TRBS_PER_SEGMENT-1], sizeof(struct xhci_generic_trb));
	}
}
...
int xhci_alloc_virt_device(struct usb_device *udev)
{
	...
	xhci_flush_cache((uintptr_t)ctrl->dcbaa, sizeof(struct xhci_device_context_array));
	xhci_flush_cache((uintptr_t)&ctrl->dcbaa->dev_context_ptrs[slot_id],
							sizeof(__le64));
	return 0;
}
...
void xhci_setup_addressable_virt_dev(struct usb_device *udev)
{
	...
	case USB_SPEED_LOW:
		...
		break;
	default:
		printf("Speed was set earlier, this shouldn't happen. \n");
		return ;
	...
	xhci_flush_cache(trb_64, sizeof(struct xhci_generic_trb));  
	xhci_flush_cache((uintptr_t)ep0_ctx, sizeof(struct xhci_ep_ctx));
	xhci_flush_cache((uintptr_t)slot_ctx, sizeof(struct xhci_slot_ctx));

	//hexdump(ep0_ctx, sizeof(struct xhci_ep_ctx));
	//printf("---------------------------------------------\n");
	//hexdump(slot_ctx, sizeof(struct xhci_slot_ctx));
