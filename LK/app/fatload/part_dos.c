...
{
	//MY_CLR_ALIGN_BUFFER();
	//MY_ALLOC_CACHE_ALIGN_BUFFER(unsigned char, buffer, dev_desc->blksz);
	unsigned char  *buffer;
        buffer = (unsigned char*)memalign(64 , dev_desc->blksz*sizeof(unsigned char));
				 ...
				 int which_part, disk_partition_t *info)
{
	...
	if (buffer[DOS_PART_MAGIC_OFFSET] != 0x55 ||
        ...
        /* Get MBR disk signature */
	unsigned int *disk_sig = (unsigned int*) (buffer + DOS_DISK_SIG_OFFSET);
        /* Translate 4byte MBR disk signature into 8 chars */
        snprintf(info->uuid, sizeof(info->uuid), "%08x-%02d", *disk_sig, which_part);
#endif
