...
extern int submit_int_msg(struct usb_device *dev, unsigned long pipe, void *buffer,
...
{
	int len;
//	ALLOC_CACHE_ALIGN_BUFFER(unsigned char, result, 1024);
        unsigned char *result;
        result = (unsigned char*)memalign(64 ,1024*sizeof(unsigned char));
...
#ifdef CONFIG_USB_UPDATE_WHEN_AC_ON
			if (is_usb_ac_on && max_lun) {
				printf("\n\n#@# %s(%d) max_lun %d, bypass this device.\n\n", __func__, __LINE__, max_lun);
				continue;
			}
#endif /* CONFIG_USB_UPDATE_WHEN_AC_ON */
			for (lun = 0;
...
static int usb_stor_BBB_comdat(ccb *srb, struct us_data *us)
{
	...
	int dir_in;
	unsigned int pipe;
	//ALLOC_CACHE_ALIGN_BUFFER(umass_bbb_cbw_t, cbw, 1024);
        umass_bbb_cbw_t *cbw;
        cbw = (umass_bbb_cbw_t *)memalign(64 ,1024*sizeof(umass_bbb_cbw_t));
...
static int usb_stor_CBI_get_status(ccb *srb, struct us_data *us)
{
	...
	/*debug*/printf("Got interrupt data 0x%x, transfered %d status 0x%lX\n",
...
static int usb_stor_BBB_transport(ccb *srb, struct us_data *us)
{
	...
	//ALLOC_CACHE_ALIGN_BUFFER(umass_bbb_csw_t, csw, 1024);
	umass_bbb_csw_t *csw;
        csw = (umass_bbb_csw_t *)memalign(64 ,1024*sizeof(umass_bbb_csw_t));
...
again:
	...
	if (CSWSIGNATURE != le32_to_cpu(csw->dCSWSignature)) {
		...
		usb_stor_BBB_reset(us);
		return USB_STOR_TRANSPORT_FAILED;
	} else if (data_actlen > (int)srb->datalen) {
...
do_retry:
	...
	memset(&psrb->cmd[0], 0, CBWCDBLENGTH);
...
static int usb_inquiry(ccb *srb, struct us_data *ss)
{
	...
	retry = 5;
	do {
		memset(&srb->cmd[0], 0, CBWCDBLENGTH);
		srb->cmd[0] = SCSI_INQUIRY;
		srb->cmd[1] = srb->lun << 5;
		srb->cmd[4] = 36;
		srb->datalen = 36;
			    ...
			    lbaint_t blkcnt, void *buffer)
{
	...
	do {
		...
		if (blks > (lbaint_t)usb_max_xfer_blk/*USB_MAX_XFER_BLK*/)
...
retry_it:
		if (smallblks == usb_max_xfer_blk/*USB_MAX_XFER_BLK*/)
			usb_show_progress();
		debug("usb_dev_desc[device].blksz = %d \n", (int)usb_dev_desc[device].blksz);
		debug("smallblks = %d \n", smallblks);
	...
	usb_disable_asynch(0); /* asynch transfer allowed */
	if (blkcnt >= (lbaint_t) usb_max_xfer_blk/*USB_MAX_XFER_BLK*/)
		debug("\n");
	return blkcnt;
}
...
/*
	// add by cfyeh
	if (dev->config.desc.bmAttributes & 0x40) {
		printf("\n#@# %s(%d) skip self powered device(0x%x) !\n",
			__func__, __LINE__, dev->config.desc.bmAttributes);
		return 0;
	}

	// add by cfyeh
	if (dev->descriptor.bcdUSB > 0x200) {
		printf("\n#@# %s(%d) skip usb3 device(0x%x) !\n",
			__func__, __LINE__, dev->descriptor.bcdUSB);
		return 0;
	}

	// add by cfyeh
	if (dev->descriptor.bcdUSB < 0x200) {
		printf("\n#@# %s(%d) skip usb1 device(0x%x) !\n",
			__func__, __LINE__, dev->descriptor.bcdUSB);
		return 0;
	}
*/
		      ...
		      block_dev_desc_t *dev_desc)
{
	...
	//ALLOC_CACHE_ALIGN_BUFFER(unsigned long, cap, 1024);
	unsigned int *cap;
        cap = (unsigned int *)memalign(64 ,1024*sizeof(unsigned int));

	//ALLOC_CACHE_ALIGN_BUFFER(unsigned char, usb_stor_buf, 1024);
	unsigned char *usb_stor_buf;
        usb_stor_buf = (unsigned char *)memalign(64 ,1024*sizeof(unsigned char));
	...
	ss->flags &= ~USB_READY;
	debug("Read Capacity returns: 0x%x, 0x%x\n", (unsigned int)cap[0], (unsigned int)cap[1]);
#if 0
