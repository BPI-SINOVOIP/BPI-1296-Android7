...
#define DELAY 20
  ...
  while (!uart_tstc (DEBUG_UART) && (counter < xyzModem_CHAR_TIMEOUT * 1000 / DELAY))
    {
      spin (DELAY);
      counter++;
    }
  if (uart_tstc (DEBUG_UART))
    {
      *c = uart_getc (DEBUG_UART, true);
      return 1;
    }
  return 0;
}
...
/* Parse (scan) a number */
static bool
parse_num (char *s, unsigned long *val, char **es, const char *delim)
{
  bool first = true;
  int radix = 10;
  char c;
