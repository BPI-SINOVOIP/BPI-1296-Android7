...
int I2C_StartXfer_EEPROM(unsigned char Id, unsigned short Addr,unsigned short nSubAddr, unsigned char *pSubAddr, unsigned short nData, unsigned char *pData, unsigned int flag);
 ...
 * Parm : Id          : i2c bus id
 *        Addr        : Device address
 *        nSubaddr    : address mode, currently, in read command, it must be 1 or 2, in write command, write Data length + address mode length
 *        pSubAddr    : read-from or write-to adress
 *        nData       : read Data length, only available in read command
 *        pData       : read data buffer, only available in read command
 *        flag        : NO_READ(write command) or NON_STOP(read command)
 *
 * Retn : S_OK / S_FALSE
 * 2017/03/27 added
 *==============================================================================================================================================*/
...
{
    return I2C_StartXfer_EEPROM(Id, Addr, nSubAddr, pSubAddr, nData, pData, flag);
}

int I2C_Write_EEPROM(
    unsigned char           Id,
    unsigned short          Addr,
    unsigned short          nData,
    unsigned char*          pData,
...
{
    return I2C_StartXfer_EEPROM(Id, Addr, nData, pData, 0, NULL, flag);
}

int I2C_StartXfer_EEPROM( unsigned char Id,
		    unsigned short Addr,
		    unsigned short nSubAddr,
		    unsigned char *pSubAddr,
		    unsigned short nData,
		    ...
		    )
{
    ...
    if(flag==NO_READ)	//EEPROM spec. indicates that two eeprom write command's interval cannot be less than 5 milli seconds, set 7 for safer prupose
	udelay(7000);
...
#define RxComplete()        (rx_len >= nData)
	...
	//udelay(1000);
        // check status
        event = GET_IC_INTR_STAT(i);
	//event = readl(MIS_IC5_RAW_INTR_STAT);
        if (event & TX_ABRT_BIT)
        {
            i2c_ppprint("tx abort!!!!\n");
            break;
        ...
        while(!TxComplete() && NOT_TXFULL(i))
        {
            if (tx_len < nSubAddr)
            {
                if( flag & NON_STOP)
                {
                    if(nData==0) {      //read command first part, write the "will be read" address
                        if(nSubAddr==1) {
                                SET_IC_DATA_CMD(i, pSubAddr[tx_len]|(0x1<<10));
                        }
                        else if(nSubAddr==2){
                                SET_IC_DATA_CMD(i, pSubAddr[tx_len]);
                                udelay(200);
                                SET_IC_DATA_CMD(i, pSubAddr[tx_len+1]|(0x1<<9));
                                udelay(200);
                        }
                        else {  //in the future, if larger adress mode 3 bytes, 4 bytes...etc. put in here
                                printf("The address mode must be 1 or 2...\n");
                                goto done;
                        }
                        udelay(50);
                        goto done;
                        ...
                        //do nothing, we send the read address in previous write command
                    }
                }
                else if(flag & NO_READ) //write data command and the parameter must be NO_READ
                {
                    if(tx_len == (nSubAddr-1))
                    {
                        SET_IC_DATA_CMD(i, pSubAddr[tx_len]|(0x1<<9));// last byte, STOP
                    ...
                    else
                    {
                        ...
                        printf("Invalid flag passing...\n");
                        printf("Read command uses NON_STOP flag and write command uses NO_READ flag\n");
                        goto done;
                }
            }
            else
            {
                if(flag & NON_STOP) {   //read command second part, issue read command to fifo
                    if( tx_len == (nSubAddr+nData-1))   //the last read request
                        SET_IC_DATA_CMD(i, (READ_CMD|(0x1<<9)) );
                    else {
                        SET_IC_DATA_CMD(i, READ_CMD);  //send read command to rx fifo
		    }
