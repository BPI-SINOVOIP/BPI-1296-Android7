...
static int do_pmic(int argc, const cmd_args *argv)
{
	if (!gd) {
	...
	case 's':
		...
		reg_access(argv[2].str, argv[3].str);
		break;
...
STATIC_COMMAND("pmic", "PMIC commands", (console_cmd)&do_pmic)
STATIC_COMMAND_END(pmic);


static void cmd_pmic_init(const struct app_descriptor *app)
{
	int i;
...
static void cmd_pmic_entry(const struct app_descriptor *app, void *args)
{
	pmic_setup_pmic();
}
