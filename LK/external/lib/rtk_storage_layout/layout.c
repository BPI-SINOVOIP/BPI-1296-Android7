...
struct fw_info *get_fw_info_next(struct fw_info *fw)
{
	...
	LTRACEF_LEVEL(SPEW, "Find next fw info from %s index %d\n",
		    fw->name, fw->index);
	index = fw->index + 1;
	if (index < MAX_FW_INFO) {
		LTRACEF_LEVEL(SPEW, "Get next fw info %s index %d\n",
...
struct part_info *get_part_info_by_mount(char *mount)
{
	...
	LTRACEF_LEVEL(SPEW, "Find part info by mount %s\n",
		    mount);
	for (index = 0; index < MAX_PART_INFO; index++) {
		info = &g_part_info[index];
		if (!strcmp(mount, info->mount)) {
			LTRACEF_LEVEL(SPEW, "Get part info %s by mount %s\n",
...
int adjust_partition_size(void)
{
	...
	if (system_flash->total_size >= EMMC_8G_SIZE) {
		LTRACEF_LEVEL(ALWAYS, "For 8G emmc adjust partition (size=0x%llx)\n",
			system_flash->total_size);
	...
	for (i = MAX_PART_INFO - 1; 0 <= i; i--) {
		...
		if (pinfo->id == PART_DATA &&
			    pinfo->bytes_size > (size_t)system_flash->total_size) {
			LTRACEF_LEVEL(CRITICAL, "data image size (0x%lx) > 0x%x, fix it\n",
		...
		LTRACEF_LEVEL(ALWAYS, "Partition %s start addr 0x%llx size 0x%lx\n",
...
int adjust_gpt_partition_size(void)
{
	...
	if (system_flash->total_size >= EMMC_GPT_8G_MAX) {
		LTRACEF_LEVEL(ALWAYS, "For 8G emmc adjust partition (size=0x%llx)\n",
			system_flash->total_size);
...
static void debug_dump_fw_info(void)
{
	...
	for (i = 0; i < MAX_FW_INFO; i++) {
		...
		LTRACEF("fw_info: start_addr=0x%lx, start_block=0x%lx, "
				"bytes_size=0x%lx, block_size=0x%lx, load_addr=0x%lx\n",
...
size_t dump_fw_info(char *buf, size_t buf_sz)
{
	...
	for (i = 0; i < MAX_FW_INFO; i++) {
		...
		count += snprintf(buf + count, buf_sz - count,
			    ...
			    i, fw->id, fw->name, fw->path);
		count += snprintf(buf + count, buf_sz - count,
			    "fw_info: start_addr=0x%lx, start_block=0x%lx,\n "
			    "bytes_size=0x%lx, block_size=0x%lx, load_addr=0x%lx\n",
			    fw->start_addr, fw->start_block, fw->bytes_size,
			    fw->block_size, fw->load_addr);
	}
...
static int parse_config_file(void *file, unsigned file_size)
{
	...
	LTRACEF_LEVEL(SPEW,"%s", config_txt);
	...
	while (ptr) {
		...
		if (!strncmp(newline, "fw", 2)) {
			...
			if (fw) {
				...
				newline_p += len;
				newline_p += 3;
				fw->load_addr = (addr_t)strtol(newline_p ,0, 16);
				fw->is_use = true;
				LTRACEF_LEVEL(ALWAYS, "Set fw info id %d, name %s, path %s, "
						"load to load_addr 0x%lx\n",
			...
			if (part) {
				...
				part->bytes_size = (size_t)strtol(newline_p ,0, 10);
				part->length = part->bytes_size;
				part->is_use = true;
				LTRACEF_LEVEL(ALWAYS, "Set part info id %d, name %s, path %s, "
						"size 0x%lx bytes\n",
		...
		void *data, unsigned sz)
{
	...
	if (part) {
		LTRACEF_LEVEL(INFO, "flash part image %s (data size 0x%08x "
				"@0x%08llx/0x%08llx)\n",
	...
	fw = get_fw_info(arg);
	if (fw) {
		LTRACEF_LEVEL(INFO, "flash fw image %s (data size 0x%08x "
				"@0x%08llx/0x%08llx)\n",
...
int storage_write(uint64_t start_addr, uint64_t data_size, void *data)
{
	...
	ret = BDEV_WRITE(flash_use_block_op, system_flash, data,
		    bdev_addr, bdev_size);

	LTRACEF("start_addr 0x%llx bdev_addr 0x%lx data_size 0x%llx "
		    "bdev_size 0x%lx ==> ret 0x%x (bytes_size = 0x%x)\n",
		    start_addr, bdev_addr, data_size, bdev_size,
		    ret, ret * flash_blk_size);
...
int storage_read(uint64_t start_addr, uint64_t data_size, void *data)
{
	...
	LTRACEF("start_addr 0x%llx block_addr 0x%lx data_size 0x%llx "
		    "bdev_size 0x%lx ==> ret 0x%x (bytes_size = 0x%x)\n",
		    start_addr, bdev_addr, data_size, bdev_size,
		    ret, ret * flash_blk_size);
	/* transfer to byte size */
