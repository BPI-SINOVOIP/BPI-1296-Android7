...
extern struct gpt_partition_entry *partition_entries;
...
int read_part_info_from_gpt(struct part_info *p_infos)
{
	...
	if(!partition_entries) {
		LTRACEF("no gpt partition\n");
	} else {
		partition_count = partition_get_num();
		for (i=0; i<partition_count; i++) {
			idx = get_part_info_index(partition_entries[i].name, p_infos);
			if(idx >= 0) {
				LTRACEF("partition index = %d\n", idx);
			} else {
				LTRACEF("can't find partition index\n");
				continue;
			}
			p_infos[idx].is_extend = false;
			p_infos[idx].start_block = partition_entries[i].first_lba;
			p_infos[idx].start_addr = p_infos[idx].start_block * flash_blk_size;
			p_infos[idx].block_size = partition_entries[i].last_lba - partition_entries[i].first_lba + 1;
			p_infos[idx].bytes_size = p_infos[idx].block_size * flash_blk_size; 
		}
	}
	return 0;
}
