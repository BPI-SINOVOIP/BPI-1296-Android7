...
static addr_t fw_desc_table_start = 0; // Start address in block device
static void *fw_desc_table_ddr_base = 0; // load address in DDR
static size_t fw_desc_table_max_size = 32*1024; // 32kB

static int get_fwtbl_start_address(addr_t *fwtb_start_addr)
{
	*fwtb_start_addr = 0;
	...
	if (boot_flash_type == EMMC_FLASH) {
		...
		/* NAS with NAND flash use 4 blocks as factory region instead
		   of static size. */
		if (factory_blocks == 0)
			factory_blocks = ALIGN(FACTORY_SIZE, flash_blk_size)
					 / flash_blk_size;

		*fwtb_start_addr = NAND_TOTAL_RESERVE_BLOCKS * flash_blk_size
				 + NAND_TOTAL_RESERVE_SIZE;
		/* Add factory region offset */
		*fwtb_start_addr += factory_blocks * flash_blk_size;

		if (boot_mode == BOOT_GOLD_MODE)
			*fwtb_start_addr += 1 * flash_blk_size; // Add normal FWTBL offset
	} else {
		LTRACEF_LEVEL(CRITICAL, "boot_flash_type %d not support!!\n",
				boot_flash_type);
		return -1;
	}
	    ...
	    fw_desc_table_v1_t *fw_desc_table_v1)
{
	...
	if (count >= buf_sz)
		LTRACEF_LEVEL(CRITICAL, "[ERR] count %d >= buf_sz %d\n",
...
void dump_fw_table(void)
{
	...
	if (!fw_desc_table_ddr_base) {
		LTRACEF_LEVEL(CRITICAL, "[ERROR] No init fw_desc_table_ddr_base!\n");
		return;
	}
	...
	if (LOCAL_TRACE >= SPEW) {
		...
		if (fw_desc_table_v1->part_list_len == 0) {
			LTRACEF_LEVEL(CRITICAL, "[ERROR] No partition found!\n");
...
static size_t get_fw_entry_size(fw_desc_table_v1_t *fw_desc_table_v1)
{
	...
	switch (fw_desc_table_v1->version) {
		...
		case FW_DESC_TABLE_V2_T_VERSION_22:
			...
			break;
		default:
			LTRACEF_LEVEL(CRITICAL, "unsupport FW DESC TBL version 0x%x\n",
					fw_desc_table_v1->version);
			return RTK_PLAT_ERR_PARSE_FW_DESC;
			break;
	}
...
static int read_fw_table(void)
{
	...
	LTRACEF_LEVEL(SPEW, "[DEBUG] fw_desc_table_start = 0x%08lx "
	...
	if (strncmp((const char *)fw_desc_table_v1.signature,
		...
		LTRACEF_LEVEL(CRITICAL, "[ERROR] Signature(%s) error!\n",
				 fw_desc_table_v1.signature);
		return RTK_PLAT_ERR_PARSE_FW_DESC;
	}
	...
	if (fw_desc_table_v1.checksum != checksum) {
		LTRACEF_LEVEL(CRITICAL, "[ERROR] Checksum not match(0x%x != 0x%x)\n",
...
int build_fw_table(struct fw_info *fw_infos, struct part_info *p_infos)
{
	...
	LTRACEF_LEVEL(SPEW, "[DEBUG] fw_desc_table_v1 @%p\n", fw_desc_table_v1);
	...
	for (i = 0; i< MAX_FW_INFO; i++) {
		...
		LTRACEF_LEVEL(INFO, "Build fw_desc %s offset=0x%x, "
...
int update_fw_table(struct fw_info *fw)
{
	...
	LTRACEF("[DEBUG] update fw table fw_desc_table_start 0x%lx "
	...
	ret = storage_write(fw_desc_table_start, fw_desc_table_v1->paddings,
		    fw_desc_table_ddr_base);
	if (ret < 0) {
		LTRACEF_LEVEL(CRITICAL, "[ERROR] update fw table "
		    "fw_desc_table_start %lx "
		    "fw_desc_table_size 0x%x fw_desc_table_ddr_base %p\n",
		    fw_desc_table_start, (int) fw_desc_table_v1->paddings,
		    fw_desc_table_ddr_base);
	}
	...
	/* For Golden fw table */
	LTRACEF("[DEBUG] update golden fw table fw_desc_table_start 0x%lx "
		    "fw_desc_table_size 0x%x fw_desc_table_ddr_base %p\n",
	...
	ret = storage_write(fw_desc_table_start + FW_TABLE_SIZE,
		    ...
		    fw_desc_table_ddr_base);
	if (ret < 0) {
		LTRACEF_LEVEL(CRITICAL, "[ERROR] update golden fw table "
		    "fw_desc_table_start %lx "
		    "fw_desc_table_size 0x%x fw_desc_table_ddr_base %p\n",
		    fw_desc_table_start + FW_TABLE_SIZE,
		    (int) fw_desc_table_v1->paddings,
...
int read_fw_info(struct fw_info *fw_infos, struct part_info *part_infos)
{
	...
	for (i = 0 ; i < part_count; i++) {
		...
		LTRACEF_LEVEL(INFO, "#%d p_entry@%p Find pinfo for mount %s\n",
	...
	if (fw_desc_table_v1->fw_list_len) {
		...
		LTRACEF_LEVEL(SPEW,
			    "[DEBUG] fw_entry start: %8p size: 0x%x count: %d\n",
			    fw_entry, (int)fw_entry_size, fw_entry_num);
		...
		for (i = 0; i < fw_entry_num; i++) {
			for (j = 0; j < MAX_FW_INFO; j ++) {
				...
				LTRACEF_LEVEL(INFO, "From fw read fw_desc %s offset=0x%x, "
					    "paddings=0x%x, length=0x%x\n",
...
static size_t relayout_fw_img(struct fw_info *fw,
	    void *img_ddr_base, addr_t img_offset)
{
	int ret;
	size_t update_size = 0;
	addr_t offset = img_offset;
...
//	void *fw_update_ddr_base = memalign(CACHE_LINE, FW_IMG_UPDATE_SIZE_MAX);
	...
	if (ret < (int)fw_update_size) {
		TRACEF("[ERR] Read from storage! ret=0x%x "
			    "(fw_update_start 0x%lx, fw_update_size 0x%lx,"
	...
	arch_clean_cache_range((addr_t)fw_update_ddr_base, file_size);

#if HW_MCP
	arch_disable_ints();
	MCP_SHA256_hash(UINT32_PTR(fw_update_ddr_base), file_size,
		    fw->sha_hash, NULL);
	arch_enable_ints();
#else
	SHA256_hash(UINT32_PTR(fw_update_ddr_base), file_size,
		    fw->sha_hash);
#endif
	...
	if (fw_update_size > FW_IMG_UPDATE_SIZE_MAX) {
		LTRACEF_LEVEL(CRITICAL, "[ERROR] fw_update_size 0x%x > 0x%x\n",
	...
	if (ret < 0) {
		LTRACEF_LEVEL(CRITICAL, "[ERROR] update fw image fail %s "
