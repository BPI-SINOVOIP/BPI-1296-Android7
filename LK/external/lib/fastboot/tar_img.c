...
#include <tar.h> /* module: external/lib/tar */
...
static struct image_info cur_img;
...
static int dump_memory(void *buffer, size_t len)
{
	...
	LTRACEF_LEVEL(ALWAYS, "** dump_memory@%p **\n", buffer);

	for(i = 0;i < (int)len + 16;i += 16) {
		LTRACEF_LEVEL(ALWAYS, "0x%08x: 0x%08x 0x%08x 0x%08x 0x%08x\n",
			i,
			*(ptr + i / 4),
			*(ptr + i / 4 + 1),
			*(ptr + i / 4 + 2),
				...
				void *data, unsigned size))
{
	...
	if (cur_img.process_file_name) {
		...
		LTRACEF_LEVEL(CRITICAL, "ERROR!! Not tar header!! "
				"data@%p len 0x%lx\n", *data, len);
		return 0;
//		if (LOCAL_TRACE >= INFO)
				...
				void *data, unsigned size))
{
	...
	while (len > 0) {
		...
		file_sz = strtol(header->size ,0,8);

		LTRACEF_LEVEL(ALWAYS, "processing file: %s size 0x%llx (data@%p)\n",
