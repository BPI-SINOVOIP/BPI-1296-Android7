...
static int dump_memory(void *buffer)
{
	...
	if (buffer == NULL) {
		LTRACEF_LEVEL(CRITICAL, "[ERR] buffer addr is NULL\n");
		return -1;
	}
		...
		void **image)
{
	...
	default:
		LTRACEF_LEVEL(CRITICAL, "Unknown chunk type: %x\n",
		       chunk->chunk_type);
			...
			void *buffer, unsigned size))
{
	...
	if (!sparse_header) {
		LTRACEF_LEVEL(ALWAYS, "Not sparse image, pass to flash_image\n");
	...
	for (chunk = 1; chunk <= sparse_header->total_chunks; chunk++) {
		...
		if (!chunk_header) {
			LTRACEF_LEVEL(CRITICAL, "chunk #%d/%d Unknown type buffer@%p\n",
		...
		case CHUNK_TYPE_RAW:
			if (chunk_header->total_sz
				...
				LTRACEF_LEVEL(CRITICAL, "chunk #%d/%d CHUNK_TYPE_RAW: ERROR!! "
						"total_sz %d != %d\n",
			...
			arch_clean_invalidate_cache_range((addr_t)raw_buf, chunk_data_sz);
		...
		case CHUNK_TYPE_FILL:
			if (chunk_header->total_sz
				...
				LTRACEF_LEVEL(CRITICAL, "chunk #%d/%d CHUNK_TYPE_FILL: ERROR~~ "
						"total_sz %d != %ld\n",
						chunk, sparse_header->total_chunks,
						chunk_header->total_sz,
						sparse_header->chunk_hdr_sz + sizeof(uint32_t));
			...
			arch_clean_invalidate_cache_range((addr_t)fill_buf, max_fill_size);
			...
			while (chunk_count) {
				...
				LTRACEF_LEVEL(SPEW, "chunk #%d CHUNK_TYPE_FILL: fill "
				...
				LTRACEF_LEVEL(SPEW, "chunk #%d count %d remainder chunk_count "
		...
		default:
			LTRACEF_LEVEL(CRITICAL, "chunk #%d/%d Unkown chunk type: %x\n",
					chunk, sparse_header->total_chunks,
	...
	if (total_blocks != sparse_header->total_blks) {
		LTRACEF_LEVEL(CRITICAL, "sparse image write failure\n");
