...
int tar_dump_posix_header(posix_header *header)
{
	...
	if (header == NULL)
	{
		printf("[ERR] tar_dump_posix_header: header is NULL\n");
		return 1;
	}

	printf("** tar_dump_posix_header **\n");
...
void tar_build_header(char *cheader, const char* filename, int length, char filetype ) {
	...
	strcpy(header->mode, "000777");
	strcpy(header->uid, "0001000");
	strcpy(header->gid, "0001000");
	strcpy(header->mtime, "11720412623");
	header->typeflag = filetype;

	memset(header->linkname, 0, sizeof(header->linkname));
	sprintf(header->size,  "%o", length);
    ...
    unsigned int *dst_length )
{
	...
	for (;;)
	{
		...
		if (!strncmp(s,header,100))
		{
			...
			TAR_DEBUG("dump %p %p", dst_addr, dst_length);
			TAR_DEBUG("dump %p %p", header + 512, (void*)strtol(header + 124,0,8));
