...
/**
 * following options are related with hal behavior, do not modify them
 */
#if 1
#define HDMI_GRALLOC_USAGE    GRALLOC_USAGE_HW_TEXTURE | \
...
#define HDMI_GRALLOC_USAGE    GRALLOC_USAGE_HW_RENDER | \
                              GRALLOC_USAGE_SW_READ_OFTEN | \
                              GRALLOC_USAGE_SW_WRITE_OFTEN | \
                              GRALLOC_USAGE_HW_ION
#endif
...
protected:
    ...
    int32_t         mGrallocUsage;

    // keep tracking with hal buffers
    KeyedVector<int32_t, size_t>    mBufferMap;
    ...
    void makeFPSTable(cam_fps_range_t* fpsTable,
        size_t size,
        size_t max_size,
    ...
    // keep tracking with yuv buffers
    void saveBufInfo2Map(uint32_t phyAddr, size_t bufferSize);
