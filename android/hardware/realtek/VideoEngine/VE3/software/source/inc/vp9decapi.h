...
#if 1 /* RTK extend function */
    enum DecRet Vp9DecAddRefQueue(Vp9DecInst dec_inst, u32 n);

    enum DecRet Vp9DecRemoveRefQueue(Vp9DecInst dec_inst, u32 n);

    enum DecRet Vp9DecFlushRefQueue(Vp9DecInst dec_inst);

    enum DecRet Vp9DecHasFrame(Vp9DecInst dec_inst, u32 nPicId);

    enum DecRet Vp9DecFreeRefFrame(Vp9DecInst dec_inst, u32 bOnlyPic);

    enum DecRet Vp9DecFreeBufferQueue(Vp9DecInst dec_inst);

    int Vp9DecGetShowFrame(Vp9DecInst dec_inst);

    u32 Vp9DecSetState(Vp9DecInst dec_inst, int state);

    void Vp9DecReConfig(Vp9DecInst dec_inst, struct Vp9DecConfig *dec_cfg);
    u32 Vp9CheckCurrentKeyFrame(Vp9DecInst dec_inst, const struct Vp9DecInput *input);
#endif
