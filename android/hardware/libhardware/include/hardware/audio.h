...
#define AUDIO_HARDWARE_MODULE_ID_SCO "sco"
#endif
...
//rtk begin
    /**
     * Return the audio hardware driver real latency in milliseconds.
     */
    uint32_t (*get_fw_latency)(const struct audio_stream_out *stream);
//rtk end
};
...
struct audio_stream_in {
     ...
     * Return the audio hardware driver real latency in milliseconds.
     */
    uint32_t (*get_input_latency)(struct audio_stream_in *stream);

    /**
     *Return the timestamp of HDMI RX audio capture driver
     */
    int64_t (*get_input_timestamp)(struct audio_stream_in *stream);
    //rtk end
