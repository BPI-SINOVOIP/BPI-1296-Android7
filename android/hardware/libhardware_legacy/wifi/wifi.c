...
static const char RTK_EXT_ACTIONS[]     = "rtk.net.wifi.ext.actions";
...
#define MAX_WIFI 20
#define MAX_LEN  20
static char** WiFiModuleName=NULL;
static int  WiFiModuleNum=0;

static int detect_mod_names()
{
    if (strlen(DRIVER_MODULE_NAME) == 0)
        return -1;

    int init_mapper = init_wifi_mod_mapper();
    if(!init_mapper)
    {
        int i = 0;
        if(WiFiModuleName==NULL){
            
            ALOGD("malloc WiFiModuleName");
            WiFiModuleName = (char**)malloc(sizeof(char*) * MAX_WIFI);
            for (i = 0; i < MAX_WIFI; ++i){
                WiFiModuleName[i] = malloc(MAX_LEN);
            }
        }
        for (i = 0; i < MAX_WIFI; ++i)
            memset(WiFiModuleName[i], 0, sizeof(char)*MAX_LEN);
        WiFiModuleNum=0;

        WiFiModuleNum = get_insmod_names((char**)WiFiModuleName);
        ALOGD("wifiModCount=%d", WiFiModuleNum);
        if (WiFiModuleNum>0 && WiFiModuleName != NULL) {
            init_mapper = 0;
        } else {
            init_mapper = 1;
        }
...
int is_wifi_driver_loaded() {
    ...
    snprintf(DRIVER_MODULE_TAG, PROPERTY_VALUE_MAX,"%s ",WiFiModuleName[WiFiModuleNum-1]);
 ...
 */
void rtk_wifi_extension_actions()
{
    property_set(RTK_EXT_ACTIONS, "1");
    ALOGE("Realtek wireless extension actions");
}

int is_openwrt_wifi(){
    char isOpenWRT[PATH_MAX]={0};
    char isOpenWRTOttWifi[PATH_MAX]={0};
    __system_property_get("persist.rtk.net.openwrt", isOpenWRT);
    __system_property_get("persist.rtk.net.openwrt.ottwifi", isOpenWRTOttWifi);
    return !strncmp(isOpenWRT, "true", 4) && !strncmp(isOpenWRTOttWifi, "false", 5);
}

int wifi_load_driver()
{
    if(is_openwrt_wifi()) return 0;
#ifdef WIFI_DRIVER_MODULE_PATH
    char driver_status[PROPERTY_VALUE_MAX];
    int count = 100; /* wait at most 20 seconds for completion */

#ifdef WIFI_MULTI_MODULE
    char WiFiModulePath[50] = {0};
    int i=0;
#endif //WIFI_MULTI_MODULE
    ...
    if (!WiFiModuleNum)
	return -1;
    for (i=0; i < WiFiModuleNum; i++) {
        sprintf(WiFiModulePath, "%s/%s.ko", DRIVER_MODULE_PATH, WiFiModuleName[i]);
        ALOGE("%s: module path: %s", __func__, WiFiModulePath);
        insmod(WiFiModulePath, DRIVER_MODULE_ARG);
    }

#else //WIFI_MULTI_MODULE
...
int wifi_unload_driver()
{
    ...
    for (i=0; i < WiFiModuleNum; i++) {
        rmmod(WiFiModuleName[i]);
    }
    ALOGD("free WiFiModuleName");
    for(i=0;i<MAX_WIFI;i++){
        free(WiFiModuleName[i]);
    }
    free(WiFiModuleName);
    WiFiModuleName = NULL;
    WiFiModuleNum = 0;
...
int wifi_start_supplicant(int p2p_supported)
{
    ...
    if(is_openwrt_wifi()){
        property_set(supplicant_prop_name, "running");
    }
...
int wifi_connect_to_supplicant()
{
    ...
    if(is_openwrt_wifi()){
        snprintf(path, sizeof(path), "@abstract:wpa_%s", "wlan0");
        ALOGD("%s: path=%s", __func__, path);
    } else if (access(IFACE_DIR, F_OK) == 0) {
