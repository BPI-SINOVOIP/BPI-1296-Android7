...
static int ap_set_hdmirx_state(struct audio_policy *pol, bool state)
{
    struct legacy_audio_policy *lap = to_lap(pol);
    return lap->apm->setHDMIRXstate(state);
}

static int ap_set_rtk_audio_active(struct audio_policy *pol, bool state)
{
    struct legacy_audio_policy *lap = to_lap(pol);
    return lap->apm->setRTKAudioActive(state);
}

static bool ap_is_rtk_audio_active(struct audio_policy *pol)
{
    struct legacy_audio_policy *lap = to_lap(pol);
    return lap->apm->isRTKAudioActive();
}
                            ...
                            struct audio_policy **ap)
{
    ...
    lap->policy.set_hdmirx_state = ap_set_hdmirx_state;
    lap->policy.set_rtk_audio_active = ap_set_rtk_audio_active;
    lap->policy.is_rtk_audio_active = ap_is_rtk_audio_active;
