...
void AudioPolicyManagerBase::setupAudioRedirect()
{
    ...
    if(binder == NULL) {
        ...
        if (!mAudioRedirect) {
            ...
            if (isHDMIRXConnected) {
                ...
                ALOGD("AUDIO_DEVICES_CONNECTED:SetHDMIRXtoBTUsb = 1 in setAudioRedirect");
            }
...
void AudioPolicyManagerBase::stopAudioRedirect()
{
    ALOGD("%s", __FUNCTION__);
    ...
    if(binder == NULL) {
        ...
        if (!isBTConnected && !isUSBConnected && mAudioRedirect) {
            if (isHDMIRXConnected) {
                ...
                ALOGD("AUIDO_DEVICES_NO_CONNECTED:SetHDMIRXtoBTUsb = 0 in stopAudioRedirect");
            }
...
{
    //ALOGV("\033[0;33;33m setDeviceConnectionState() device: %x, state %d, address %s\033[m", device, state, device_address);
    ...
    if (audio_is_output_device(device)) {
        ...
        case AudioSystem::DEVICE_STATE_AVAILABLE:
            // for usb_audio support multi cards case
            if (mAvailableOutputDevices & device && (device !=AUDIO_DEVICE_OUT_ANLG_DOCK_HEADSET )) {
...
status_t AudioPolicyManagerBase::setHDMIRXstate(bool state)
{
...
//rtk begin
status_t AudioPolicyManagerBase::setRTKAudioActive(bool state)
{
    mRTKAudioActive = state;
    return NO_ERROR;
}

bool AudioPolicyManagerBase::isRTKAudioActive()
{
...
AudioPolicyManagerBase::AudioPolicyManagerBase(AudioPolicyClientInterface *clientInterface)
    ...
    mAudioRedirect(false), isUSBConnected(false), isBTConnected(false),
    mStreamVolumeIndex(11), isHDMIMuted(false), isHDMIRXConnected(false),
    mRTKAudioActive(false),
    mHDMIRXAudioAOID(0),mHDMIRXAudioAIID(0),mHDMIRXAudioPIN(0)
{
                                            ...
                                            audio_devices_t device)
{
    ...
    if (stream == AUDIO_STREAM_MUSIC) {
        ...
        if(!isHDMIMuted && device == AUDIO_DEVICE_OUT_SPEAKER)
        {
                                                   ...
                                                   bool force)
{
    ...
    if ((stream == AudioSystem::VOICE_CALL && mForceUse[AudioSystem::FOR_COMMUNICATION] == AudioSystem::FORCE_BT_SCO) ||
        ...
        if(!isHDMIMuted)
        {
            //ALOGD("\033[0;33;33m setAudioHdmiMute false line %d\033[m\n", __LINE__);
            SetAudioHdmiMute(false);
        }
        return INVALID_OPERATION;
    }
