...
struct ext4_super_block {
 ...
 unsigned int s_obso_log_frag_size;
 ...
 unsigned char s_reserved_char_pad2;
 unsigned short s_reserved_pad;
 unsigned long long s_kbytes_written;
 unsigned int s_reserved[160];
};
...
static int CreateStash(State* state, int maxblocks, const char* blockdev, std::string& base) {
	...
	int res;
	res = stat("/cache/recovery", &sb);
	if (res == -1 && errno != ENOENT) {
				ErrorAbort(state, kStashCreationFailure, "stat cache/recovery failed: %s\n",
			   strerror(errno));
	return -1;
	} else if (res != 0) {
	fprintf(stderr, "creating stash cache/recovery\n");
	res= mkdir("/cache/recovery/", STASH_DIRECTORY_MODE);
	
	if (res != 0) {
		ErrorAbort(state, kStashCreationFailure, "mkdir cache/recovery failed: %s\n",
					strerror(errno));
		return -1;
	}
	}
...
static int rtk_burn_ext4_metadata_hash(char* system_blk_device)
{
      int device_fd;
      int i;
      struct ext4_super_block *system_super_block = NULL;
      unsigned int  *table_len = NULL;
      unsigned int  system_offset;
      unsigned int  system_block_size;
      unsigned long long system_blocks;       
      unsigned long long  system_real_size; 
      char salt_data[64];
      unsigned char system_sha256_metadata[32];
	  
    device_fd = TEMP_FAILURE_RETRY(open(system_blk_device, O_RDONLY | O_CLOEXEC));

    if (device_fd == -1) {
         fprintf(stderr, "Error opening block device (%s)\n",system_blk_device);
        return -1;
    }
    if (TEMP_FAILURE_RETRY(lseek64(device_fd, 1024, SEEK_SET)) < 0) {
         fprintf(stderr, "Error seeking to superblock \n");
        close(device_fd);
        return -1;
    }
    system_super_block = (struct ext4_super_block *)malloc(sizeof(struct ext4_super_block));

    if (TEMP_FAILURE_RETRY(read(device_fd, system_super_block, sizeof(struct ext4_super_block))) != sizeof(struct ext4_super_block)) {
         fprintf(stderr, "Error reading superblock \n");
        close(device_fd);
        return -1;
    }
       system_block_size = 1024<<system_super_block->s_log_block_size;
       system_blocks = (system_super_block->s_blocks_count_hi<<32)+system_super_block->s_blocks_count_lo; 

       system_real_size = system_block_size * system_blocks;

       fprintf(stderr, "---the system_block_size is %10u---\r\n",system_block_size);
       fprintf(stderr, "---the system_blocks is %10llu---\r\n",system_blocks);
       fprintf(stderr, "---the system_real_size is %10llu---\r\n",system_real_size);


      system_offset = system_real_size + 264;


    if (TEMP_FAILURE_RETRY(lseek64(device_fd, system_offset, SEEK_SET)) < 0) {
         fprintf(stderr, "Error seeking to verity table length \n");
        close(device_fd);
        return -1;
    }
    
      table_len = (unsigned int *)malloc(sizeof(unsigned int));
    

    if (TEMP_FAILURE_RETRY(read(device_fd,table_len, sizeof(unsigned int))) != sizeof(unsigned int)) {
        fprintf(stderr, "Error reading verity table tength \n");
        close(device_fd);
        return -1;
    }
    
    fprintf(stderr, "----the table len is %06u---\r\n",*table_len);

    system_offset =system_real_size + 268 + *table_len - 64;

    if (TEMP_FAILURE_RETRY(lseek64(device_fd, system_offset, SEEK_SET)) < 0) {
         fprintf(stderr, "Error seeking to slat data\n");
        close(device_fd);
        return -1;
    }

    if (TEMP_FAILURE_RETRY(read(device_fd,salt_data,sizeof(salt_data))) != sizeof(salt_data)) {
        fprintf(stderr, "Error reading salt data \n");
        close(device_fd);
        return -1;
    }

      fprintf(stderr,"-----the salt data is:");
      for( i=0;i<64;i++)
      {
       
       fprintf(stderr,"%c",salt_data[i]);
      }
      fprintf(stderr,"-----\r\n");

      rtk_file_checksum(salt_data,sizeof(salt_data),system_sha256_metadata);

      //TODO: use the rpmb burn function to burn system_sha256_metadata



      close(device_fd);
      free(table_len);
      free(system_super_block);
...
static Value* PerformBlockImageUpdate(const char* name, State* state, int /* argc */, Expr* argv[],
    ...
    if (params.canwrite) {
        ...
        if(rtk_burn_ext4_metadata_hash(blockdev_filename->data)!=0)
        fprintf(stderr, "---burn ext4 system metadata sha256 error!!---\r\n");
