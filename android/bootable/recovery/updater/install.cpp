...
Value* FormatFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    else if (strcmp(fs_type, "ext4") == 0)
    {
        int status = -1;
#ifdef RTK_EXTENSION
        if (strncmp(mount_point, "/nas", 4) == 0)
        {
        printf("[FormatFn]make_ext4fs Force Format start.\n");
        status = make_ext4fs(location, atoll(fs_size), mount_point, NULL);
        }
        else
#endif
        {
        printf("[FormatFn]make_ext4fs Format start.\n");
    ...
    {
        printf("[FormatFn]ubifs Format start.\n");
        char cmd[256] = {0};
        snprintf(cmd, sizeof(cmd), "/sbin/busybox rm -rf %s/*", mount_point);
        if((rtk_command(cmd, __LINE__, __FILE__, 1)) < 0)
        {
            install_debug("Exec command fail\r\n");
            result = strdup("");
...
Value* WriteRawImageEMMCFn(const char* name, State* state, int argc, Expr* argv[])
{
...
#if 1
    f = fopen(partition_value->data, "wb");
    if (f == NULL)
    {
        ErrorAbort(state, "unable open partition (%s) device", partition_value->data);
        goto done;
    }

    count = fwrite(contents->data, contents->size, 1, f);
    if (count <= 0)
    {
        ErrorAbort(state,"Failed writing %s\n(%s)\n", partition_value->data, strerror(errno));
        goto done;
    }
    else
    {
    ...
    if (fclose(f) != 0)
    {
        ErrorAbort(state,"Failed closing %s\n(%s)\n", partition_value->data, strerror(errno));
        result = strdup("");
        goto done;
    }
#endif
...
Value* WriteRawRTKImageEMMCFn(const char* name, State* state, int argc, Expr* argv[])
{
    int ret;  //1: success; 0:fail
    bool success = true;
    char* result = NULL;
    char* tmpBuffer=NULL;
    long imgSize = 0;
    const ZipEntry* bootImgEntry = NULL;
    const ZipEntry* videoImgEntry = NULL;
    const ZipEntry* audioImgEntry = NULL;
    ZipArchive* za = ((UpdaterInfo*)(state->cookie))->package_zip;
    Value* retVal;
    CRTKImgDesc rtkimgdesc;
    memset(&rtkimgdesc, 0, sizeof(CRTKImgDesc));


    Rtk_fill_imgdesc_meminfo(&rtkimgdesc, MTD_EMMC);
    Rtk_load_config(&rtkimgdesc, za);
#ifdef NFLASH_LAOUT
    if (!Rtk_load_fwdesc_emmc(&rtkimgdesc, state))
    {
        success = false;
        goto end;
    }
#endif
    Rtk_fill_imgdesc_layout(&rtkimgdesc, za);

    ret = Rtk_burn_fw_emmc(&rtkimgdesc, za, za->updateZipPath, state);
    success = success & ret;
    ret = Rtk_burn_fwdesc_emmc(&rtkimgdesc, state);
    success = success & ret;
    Rtk_burn_factory(&rtkimgdesc, za, state);
end:
    result = success ? strdup("") : strdup("write raw rtk image wrong");

    return StringValue(result);

}

Value* WriteRawRTKImageNANDFn(const char* name, State* state, int argc, Expr* argv[])
{
    int ret;  //1: success; 0:fail
    bool success = true;
    char* result = NULL;
    ...
    Rtk_fill_imgdesc_meminfo(&rtkimgdesc, MTD_NANDFLASH);
    Rtk_load_config(&rtkimgdesc, za);
    Rtk_fill_imgdesc_layout(&rtkimgdesc, za);

    ret = Rtk_burn_fw_nand(&rtkimgdesc, za, za->updateZipPath, state);
    success = success & ret;
    ret = Rtk_burn_fwdesc_nand(&rtkimgdesc, state);
    success = success & ret;
    Rtk_burn_factory(&rtkimgdesc, za, state);
...
// start upgrade input 1, status=n; finish input 0, status = y
Value* UpdateStatusCtlFn(const char* name, State* state, int argc, Expr* argv[])
{
    int result = 0;
    char* ctl_str;
    char cmd[128] = {'\0'};
    ...
    if (ReadArgs(state, argv, 1, &ctl_str) < 0)
    {
        return NULL;
    }
    int status = atoi(ctl_str);
    printf("prepare set burnstatus :%d\n", status);
    memset(&temp, 0, sizeof(temp));
    snprintf(cmd, sizeof(cmd), "/sbin/factory load");
    rtk_command(cmd, __LINE__, __FILE__, 1);
    FILE* pF = fopen("/tmp/factory/recovery", "rb");
    int count;
    if (pF)
    {
        count = fread(&temp, sizeof(temp), 1, pF);
        if (count != 1)
        {
            printf("read BCB from factory seems stranged!(count:%d)\n", count);
        }
        fclose(pF);
        pF= NULL;
    }
    ...
    {
        printf("Failed open recovery file r\n(%s)\n", strerror(errno));
    }
    if (status == 1)
    {
        temp.status[0] = 'n';
        printf("set temp.status[0] = n");
    }
    else
    {
        temp.status[0] = 'y';
        printf("set temp.status[0] = y");
    }
    printf("write (cmd:%s, status:%s, recovery:%s)\n", temp.command, temp.status, temp.recovery);
    pF=fopen("/tmp/factory/recovery", "wb");
    if (pF)
    {
        count = fwrite(&temp, sizeof(temp), 1, pF);
        if (count != 1)
        {
            printf("Failed write recovery status\n(%s)\n", strerror(errno));
        }
        fclose(pF);
        pF=NULL;
    }
    ...
    {
        printf("Failed open recovery file w\n(%s)\n", strerror(errno));
    }
    snprintf(cmd, sizeof(cmd), "/sbin/factory save");
    rtk_command(cmd, __LINE__, __FILE__, 1);
    return StringValue(ctl_str);
}

// package_extract_tarfile(package_path, destination_path)
Value* PackageExtractTarFileFn(const char* name, State* state,
                           int argc, Expr* argv[])
{
    if (argc != 2)
    {
    ...
    char* tarfile_path;
    char* dest_path;
    if (ReadArgs(state, argv, 2, &tarfile_path, &dest_path) < 0) return NULL;
    ...
    bool success = mzExtractRecursive(za, tarfile_path, dest_path,
                                      MZ_EXTRACT_FILES_ONLY, &timestamp,
                                      NULL, NULL, sehandle);*/

    int ret = -1;
    char cmd[256] = {0};
    snprintf(cmd, sizeof(cmd), "/sbin/busybox unzip -p %s %s | /sbin/busybox tar -C %s --overwrite --numeric-owner -xpf -"
            , za->updateZipPath
            , tarfile_path
            , dest_path);
...
void RegisterInstallFunctions() {
    ...
    RegisterFunction("write_raw_rtkimage_emmc", WriteRawRTKImageEMMCFn);
    RegisterFunction("write_raw_rtkimage_nand", WriteRawRTKImageNANDFn);
    RegisterFunction("update_status", UpdateStatusCtlFn);
    RegisterFunction("package_extract_tarfile", PackageExtractTarFileFn);
#endif
