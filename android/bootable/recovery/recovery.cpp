...
#include "adb.h"
#include "ver.h"
#ifdef RECOVERY_UPGRADEBUILTINOTA_WHENFAIL
#include "builtinOTA.h"
#endif
...
static const struct option OPTIONS[] = {
  ...
  { "noreboot", no_argument, NULL, 'b' },  // extended by rtk
...
static const char *LOG_FILE_USB = "/mnt/usb/recovery/log";
static const char *LAST_INSTALL_FILE_USB = "/mnt/usb/recovery/last_install";
...
static const char *LAST_LOG_FILE_USB = "/mnt/usb/recovery/last_log";
...
static void copy_logs() {
    ...
    struct stat st;
    if (stat("/mnt/usb", &st) == 0)
    {
        if (stat("/mnt/usb/recovery", &st) != 0)
            system("mkdir -p /mnt/usb/recovery");
        copy_log_file(TEMPORARY_LOG_FILE, LOG_FILE_USB, true);
        copy_log_file(TEMPORARY_LOG_FILE, LAST_LOG_FILE_USB, false);
        copy_log_file(TEMPORARY_INSTALL_FILE, LAST_INSTALL_FILE_USB, false);
        chmod(LOG_FILE_USB, 0600);
        chown(LOG_FILE_USB, 1000, 1000);   // system user
        chmod(LAST_LOG_FILE_USB, 0640);
        chmod(LAST_INSTALL_FILE_USB, 0644);
        sync();
    }
}
...
finish_recovery(const char *send_intent)
{
    ...
    {
#ifdef RECOVERY_UPGRADEBUILTINOTA_WHENFAIL
        LOGI("update fail! Enter builtIn OTA!");
        memset(&boot.command, '\0', sizeof(boot.command));
        memset(&boot.recovery, '\0', sizeof(boot.recovery));
        sprintf(boot.command, "boot-recovery");
        sprintf(boot.recovery,"recovery\n--update_package=%s\n--locale=zh_CN",RECOVERY_UPGRADEBUILTINOTA_PATH);
        //set_bootloader_message(&boot);
#else
        LOGI("Update fail! We won't erase BCB, so next boot will enter OTA!");
#endif
    }
    else
    {
        ...
        //printf("run finish_recovery 1.1.\n");
    }
    } //RTK end
    ...
    if (locale != NULL) {
        ...
        if (has_cache) {
          ...
          if (fp == NULL) {
            printf("fopen LOCALE_FILE failed.");
          } else {
            fwrite(locale, 1, len, fp);
            fflush(fp);
            fsync(fileno(fp));
            check_and_fclose(fp, LOCALE_FILE);
            //printf("check_and_fclose done.\n");
          }
        }
    }

    copy_logs();
    printf("finish_recovery: copy_logs to /cache done.\n");
    ...
    if (has_cache) {
        ...
        {
            //printf("finish_recovery: ensure_path_mounted.\n");
            if (boot.status[0] != 'n')
            {
                unlink(COMMAND_FILE);
            }
            ...
            {
                LOGW("Can't unlink %s because we need upgrade again\n", COMMAND_FILE);
            }
        }
        else
        {
...
static bool wipe_data(int should_confirm, Device* device) {
    ...
    bool success =
        device->PreWipeData() &&
#ifndef RECOVERY_WIPEDATA_NOFORMAT
        erase_volume("/data") &&
#endif
...
static int apply_from_sdcard(Device* device, bool* wipe_cache) {
    ...
    for (int i = 0; i < SDCARD_INSTALL_TIMEOUT; ++i) {
		...
		LOGE("size of FUSE_SIDELOAD_HOST_PATHNAME: %lld\n",sb.st_size);
...
int main(int argc, char **argv) {
    ...
    printf("[%.03f] Starting recovery (pid %d) on %s", now(), getpid(), ctime(&start));

    //printf("Starting recovery on [date:%s, time:%s] \r\n", __DATE__, __TIME__);
#if 0
    load_volume_table();
#else
    printf("[%.03f] => execFactoryUtil(FAC_LOAD)\n", now());
    execFactoryUtil(FAC_LOAD); //FIXME: bootloader.cpp is gone
    printf("[%.03f] => load_volume_table_byLayoutFile\n", now());
    if (load_volume_table_byLayoutFile() == -1)
    {
        char *fstabPath = "/etc/recovery.fstab";
        printf("[%.03f] => load_volume_table\n", now());
        load_volume_table(fstabPath);
    }
    printf("[%.03f] => execFactoryUtil(FAC_SAVE)\n", now());
    execFactoryUtil(FAC_SAVE);
    printf("[%.03f] => dump_volume_table\n", now());
    dump_volume_table();
    printf("[%.03f] => ensure_path_mounted(/res)\n", now());

    ensure_path_mounted("/res");
#endif
    printf("[%.03f] => get_args\n", now());
    get_args(&argc, &argv);
    ...
    bool noreboot = false;  // extended by rtk
    ...
    if (locale == nullptr && has_cache) {
        printf("[%.03f] => load_locale_from_cache\n", now());
        load_locale_from_cache();
    }
    printf("[%.03f] locale is [%s]\n", now(), locale);
    printf("[%.03f] stage is [%s]\n", now(), stage);
    printf("[%.03f] reason is [%s]\n", now(), reason);
    ...
    if (update_package)
    {
        ...
        // "/cache/foo".
        //printf("[DEBUG] update_package path = %s .\n", update_package);
        ...
        else if (strncmp(update_package, "USB:", 4) == 0)
        {
            int len = strlen(update_package) + 10;
            char* modified_path = (char*)malloc(len);
            strlcpy(modified_path, "/mnt/usb/", len);
            strlcat(modified_path, update_package+4, len);
            printf("(replacing path \"%s\" with \"%s\")\n",
                   update_package, modified_path);
            update_package = modified_path;
        }
        else if (strncmp(update_package, "BACKUP:", 7) == 0)
        {
            int len = strlen(update_package) + 10;
            char* modified_path = (char*)malloc(len);
            strlcpy(modified_path, "/backup/", len);
    ...
    //TODO: property function not work, it's must use android's rootfs not kernel's rootfs.
#if 1
    property_list(print_property, NULL);
    printf("\n");
#else
    strcpy(recovery_version, "1.0.0");
    ...
    if (update_package != NULL) {
        ...
        if (!is_battery_ok()) {
            ...
            else //RTK begin
            {
                ...
                if (stat("/data/hash.txt", &st) == 0)
                    system("rm -rf /data/hash.txt");
                if (stat("/data/property/persist.security.dmcheck", &st) == 0)
                    system("rm -rf /data/property/persist.security.dmcheck");
                ensure_path_unmounted("/data");
            }    //RTK end
        }
    } else if (should_wipe_data) {
        printf("Recovery: wipe_data.\n");
    ...
    if (!sideload_auto_reboot && (status == INSTALL_ERROR || status == INSTALL_CORRUPT)) {
        printf("Recovery: status == INSTALL_ERROR || status == INSTALL_CORRUPT.\n");
        copy_logs();
        ui->SetBackground(RecoveryUI::ERROR);
    }
    ...
    printf("[DEBUG] run finish_recovery.\n");
    finish_recovery(send_intent);
    printf("[DEBUG] finish_recovery done.\n");
    //noreboot = true; //for test, don't reboot system
    if (noreboot)
        return EXIT_SUCCESS; //rtk: just exit without reboot
    ...
    switch (after) {
        ...
        default:
            ...
            android_reboot(ANDROID_RB_RESTART, 0, 0); //rtk: force reboot
            break;
    }
    while (true) {
      pause();
