                  ...
                  std::vector<std::string>& log_buffer, int retry_count)
{
    ...
    *wipe_cache = false;
	...
	printf("try_update_binary: INSTALL_SUCCESS.\n");
                       ...
                       std::vector<std::string>& log_buffer, int retry_count)
{
	...
	if( strcmp(path,"/sideload/package.zip" ) !=0){
		//workaround for sd card case
		add_externDev_volume_table(path);
	...
	printf("ready to start verify package\n");
    ...
    #else //added by WST, for test manual gen update.zip
    int err;
    #endif
                ...
                bool needs_mount, int retry_count)
{
    ...
    if (setup_install_mounts() != 0) {
    	...
    	printf("really_install_package start.\n");
