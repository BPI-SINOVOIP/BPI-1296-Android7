...
/*
 * Read one scanline off data from the file, and convert to RGBA form.
 */


int
TIFFReadRGBAScanline(TIFF* tif, uint32 row, uint32 * raster )

{
    ...
    if( TIFFIsTiled( tif ) )
    {
      TIFFErrorExt(tif->tif_clientdata, TIFFFileName(tif),
                  "Can't use TIFFReadRGBAScanline() with tiled file.");
      return (0);
    }
    ...
    if (TIFFRGBAImageOK(tif, emsg) && TIFFRGBAImageBegin(&img, tif, 0, emsg)) {
        ...
        {
          TIFFErrorExt(tif->tif_clientdata, TIFFFileName(tif),
                       "Row passed to TIFFReadRGBAScanline() is out of image.");
          return (0);
        }
	...
	ok = TIFFRGBAImageGet(&img, raster, img.width, 1);
