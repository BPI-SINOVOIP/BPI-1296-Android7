...
#define ABSTRCT_UNIX_DOMAIN_SOCKET

static inline void dump_chunks(server *srv, chunkqueue *cq){
    chunk *c;

    log_error_write(srv, __FILE__, __LINE__, "so", "chunkqueue length:", chunkqueue_length(cq));
    for(c = cq->first; (NULL != c); c = c->next) {
        log_error_write(srv, __FILE__, __LINE__, "sd", "chunk type:", c->type);
        if(c->type == MEM_CHUNK)
            log_error_write(srv, __FILE__, __LINE__, "sd", "mem", c->mem->used);
        else if(c->type == FILE_CHUNK)
            log_error_write(srv, __FILE__, __LINE__, "ssso", "file name", c->file.name->ptr, "length", c->file.length);
    }
}
...
#endif
#ifdef ABSTRCT_UNIX_DOMAIN_SOCKET
        fcgi_addr_un.sun_path[0] = 0;
#endif
		socket_type = AF_UNIX;
...
static int fcgi_demux_response(server *srv, handler_ctx *hctx) {
	...
	while (fin == 0) {
		...
		if (0 != fastcgi_get_packet(srv, hctx, &packet)) {
			/* no full packet */
			//log_error_write(srv, __FILE__, __LINE__, "s", "no packets available");
			break;
		}
		...
		case FCGI_STDOUT:
			...
			if (0 == con->file_started) {
				...
				//log_error_write(srv, __FILE__, __LINE__, "sd", "allow_xsendfile:", host->allow_xsendfile);
				if (host->allow_xsendfile && hctx->send_content_body &&
					...
					//log_error_write(srv, __FILE__, __LINE__, "s", "got X-Sendfile");
					if (HANDLER_ERROR != stat_cache_get_entry(srv, con, ds->value, &sce)) {
						data_string *dcls;
						//log_error_write(srv, __FILE__, __LINE__, "ssso", "stat_cache_get_entry fn:", ds->value->ptr, "st_size:", sce->st.st_size);
						if (NULL == (dcls = (data_string *)array_get_unused_element(con->response.headers, TYPE_STRING))) {
							dcls = data_response_init();
						}
						/* found */
						//log_error_write(srv, __FILE__, __LINE__, "so", "chunkqueue length:", chunkqueue_length(con->write_queue));
						http_chunk_append_file(srv, con, ds->value, 0, sce->st.st_size);
						//dump_chunks(srv, con->write_queue);
						hctx->send_content_body = 0; /* ignore the content */
						joblist_append(srv, con);
						//log_error_write(srv, __FILE__, __LINE__, "sd", "is fifo:", S_ISFIFO(sce->st.st_mode));
						if(!S_ISFIFO(sce->st.st_mode)){
						    buffer_copy_string_len(dcls->key, "Content-Length", sizeof("Content-Length")-1);
				...
				//log_error_write(srv, __FILE__, __LINE__, "sx", "wq:", con->write_queue);
