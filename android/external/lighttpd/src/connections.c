...
static int connection_handle_write_prepare(server *srv, connection *con) {
	...
	if (con->file_finished) {
		...
		if ((!(con->parsed_response & HTTP_CONTENT_LENGTH)) &&
			...
			}else if(NULL != array_get_element(con->response.headers, "X-Sendfile")){
			    //20140415 workaround: don't write content length in header because we may reading from FIFO
