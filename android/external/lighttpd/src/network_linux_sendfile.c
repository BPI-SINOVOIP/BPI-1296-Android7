...
#define USE_SPLICE_SYSCALL //20140421 charleslin, used for reading from FIFO
#ifdef USE_SPLICE_SYSCALL
...
int network_write_chunkqueue_linuxsendfile(server *srv, connection *con, int fd, chunkqueue *cq, off_t max_bytes) {
	...
	//log_error_write(srv, __FILE__, __LINE__, "s", "network_write_chunkqueue_linuxsendfile");
	...
	for(c = cq->first; (max_bytes > 0) && (NULL != c); c = c->next) {
		...
		case FILE_CHUNK: {
			...
			if(S_ISFIFO(st.st_mode))
				r = splice(c->file.fd, NULL, fd, NULL, max_bytes, 0);
			else
#endif
