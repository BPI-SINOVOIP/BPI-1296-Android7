...
#ifdef RTK_PLATFORM
long long ChkElementSizeIsValid(IMkvReader* pReader, long long pos, long& len)
{
    ...
    if (total >= 0 && (avail > total || pos > total))
        return E_FILE_FORMAT_INVALID;
    ...
    if (id_ != libwebm::kMkvCluster && id_ != libwebm::kMkvCues)
    {
        ALOGE("[%s][%d] ChkElementSizeIsValid return Error id::%llx \n",__FUNCTION__,__LINE__, id_);
        return E_FILE_FORMAT_INVALID;
    }
...
long long GetStringBySize(IMkvReader* pReader, long long pos, long long size, unsigned char *buf)
{
    if(buf == NULL)
        return E_FILE_FORMAT_INVALID;
...
#ifdef RTK_PLATFORM
      ContentCompSettings_len = buflen;
#endif
    }
...
#ifdef RTK_PLATFORM
      m_ContentCompSettings_len = content_encoding->ContentCompSettings_len;
      //ALOGD("\033[0;32m [%d]  m_ContentCompSettings_len= %d\033[m\n", __LINE__, (int)m_ContentCompSettings_len);
#endif
...
long Cluster::Load(long long& pos, long& len) const {
  ...
  {
      long len_back = len;
      long long pos_back = pos;
      pos = cluster_stop;

      if(ChkElementSizeIsValid(pReader, pos, len) != 0)
      {
          pos = pos_back;
          len = len_back;
          ALOGW("[%s][%d] ChkElementSizeIsValid fail need check m_element_size.\n",__FUNCTION__,__LINE__);
          ALOGI("[%s][%d] m_element_size::%lld m_element_start::%llx \n",__FUNCTION__,__LINE__,m_element_size,m_element_start);
          int dIsValid = 0;
          long long dpos = 0;
          unsigned char buf[4] = {0, 0, 0, 0};
          const char str1[4] = {0x1F,0x43,0xB6,0x75};
          while(dIsValid == 0)
          {
              pos++;
              dpos++;
              if(GetStringBySize(pReader, pos, 4, buf) != 0)
                  break;
              if(dpos > (2*1024*1024))
                  break;

              if(strncmp((const char *)buf, str1, 4) == 0)
              {
                  if(ChkElementSizeIsValid(pReader, pos, len) == 0)
                      dIsValid = 1;
              }
          ...
          if(dIsValid == 1)
          {
              ALOGW("[%s][%d] dIsValid 1 set m_element_size dpos::%lld\n",__FUNCTION__,__LINE__,dpos);
              m_element_size = dpos + 0x10;
          }
          pos = pos_back;
          len = len_back;
      }
