...
#define PARTITION_TYPE_NORMAL 0
#define PARTITION_TYPE_NTFS   1
///////////////////// iconv
...
int special_ntfs_utf16_to_utf8(char *inbuf, size_t inlen, char *outbuf, size_t outlen) {
    ALOGD("special_ntfs_utf16_to_utf8...");
    char szUTF16[129]={0};
    char szEncodeHex[3]={0,0,0};
    int nIdx, nCIdx;
    for(nIdx=0, nCIdx=0; nIdx<(int)inlen; nIdx+=2,nCIdx++){
        szEncodeHex[0] = inbuf[nIdx];
        szEncodeHex[1] = inbuf[nIdx+1];
        int ich = strtol(szEncodeHex, NULL, 16);
        szUTF16[nCIdx]=(char)ich;
    }
    return code_convert("UTF-16", "utf-8", szUTF16, inlen/2, outbuf, outlen);
}
...
int ansi_to_utf8(char *inbuf, size_t inlen, char *outbuf, size_t outlen) {
    ...
    else if(strcmp(szSysLocale, "zh-TW")==0 || strcmp(szSysLocale, "zh-HK") == 0)  // taiwan/hk
         strcpy(szFromLang, "BIG5-HKSCS:2004");
    else if(strcmp(szSysLocale, "ja-JP")==0)    // japan
         strcpy(szFromLang, "SHIFT_JIS");
    else if(strcmp(szSysLocale, "ko-KR")==0)    // korea
 ...
 */
static void safe_print(const char *cp, int len, int iPartitionType, int iConv)
{
    char out[200] = "";
    int iReturnCode = 0;
    ...
    if(iConv){
        ...
        case PARTITION_TYPE_NTFS:
            iReturnCode = special_ntfs_utf16_to_utf8((char*)cp, strlen(cp), out, 200); break;
        case PARTITION_TYPE_NORMAL:
        default:
...
static void print_tags(blkid_dev dev, char *show[], int numtag, int output)
{
	...
	// First identify the disk format
    int iPartitionType = PARTITION_TYPE_NORMAL;

    iter = blkid_tag_iterate_begin(dev);
    while (blkid_tag_next(iter, &type, &value) == 0) {
        ALOGD("GetType:%s", type);
        ALOGD("Value:%s", value);
        if (strcmp(type, "TYPE") == 0) {
            if (strcmp(value, "ntfs") == 0) {
                iPartitionType = PARTITION_TYPE_NTFS;
                break;
            }
            else{
                iPartitionType = PARTITION_TYPE_NORMAL;
                break;
            }
        }
    }
    blkid_tag_iterate_end(iter);
    ALOGD("partition format: %d", iPartitionType);
	...
	while (blkid_tag_next(iter, &type, &value) == 0) {
		...
		if (output & OUTPUT_VALUE_ONLY) {
			...
			if(strcmp(type, "LABEL")==0)
			        safe_print(value, -1, iPartitionType, 1);
			else safe_print(value, -1, iPartitionType, 0);
			fputs("\" ", stdout);
		}
	}
	blkid_tag_iterate_end(iter);
