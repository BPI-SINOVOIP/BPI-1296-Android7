	...
	while (1) {
		...
		if (attr_type == MFT_RECORD_ATTR_VOLUME_NAME) {
            ...
            for(i=0; i<val_len; i+=2){
                ...
                char szEncodeHex[5]={0};
                sprintf(szEncodeHex, "%02X%02X", val[1], val[0]); // encode to hex string.
                strncat(label_str, szEncodeHex, 5);
            }
            //ALOGD("label encode string:%s", label_str);
		}
	}
