...
BUILD_STATIC_RESIZE2FS := true
ifeq ($(BUILD_STATIC_RESIZE2FS),true)
include $(CLEAR_VARS)
LOCAL_SRC_FILES := $(resize2fs_src_files)
LOCAL_C_INCLUDES := $(resize2fs_c_includes)
LOCAL_CFLAGS := $(resize2fs_cflags)
LOCAL_STATIC_LIBRARIES := $(resize2fs_shared_libraries) $(resize2fs_system_shared_libraries)
LOCAL_FORCE_STATIC_EXECUTABLE := true
LOCAL_MODULE_PATH := $(TARGET_ROOT_OUT_SBIN)
LOCAL_UNSTRIPPED_PATH := $(TARGET_ROOT_OUT_SBIN_UNSTRIPPED)
LOCAL_MODULE := resize2fs_static_prebuilt
LOCAL_MODULE_TAGS := optional
include $(BUILD_EXECUTABLE)
endif
