			   ...
			   const u8 *cat, struct wpabuf *buf)
{
	...
	u8 *ampe_ie = NULL, *pos = NULL, *mic_payload;
...
skip_keys:
	...
	/* encrypt after MIC */
	mic_payload = (u8 *) wpabuf_put(buf, 2 + len + AES_BLOCK_SIZE);
			  ...
			  const u8 *start, size_t elems_len)
{
	...
	/* GTKdata shall not be include in Mesh Peering Confirm */
	if (cat[1] != PLINK_OPEN) {
		if (end > pos) {
