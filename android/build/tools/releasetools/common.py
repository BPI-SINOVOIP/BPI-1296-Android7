...
def LoadInfoDict(input_file, input_dir=None):
  ...
  #  recovery_fstab_path = "BOOT/RAMDISK/etc/recovery.fstab"
    ...
    recovery_fstab_path = "BOOT/RAMDISK/fstab.kylin.emmc"
    d["fstab"] = LoadRecoveryFSTab(read_helper, d["fstab_version"],
        recovery_fstab_path, system_root_image)
  else:
    d["fstab"] = None
...
def CopyImageFilesToOut(input_zip, tree_subdir, output_zip=None):
  """Copies image files underneath tree_subdir/ in the input zip to the output
  zip.
  """
  image_files = []
  for info in input_zip.infolist():
    patternZip = re.compile("zip")
    patternTar = re.compile("tar")
    bmatchZipFile = patternZip.findall(info.filename)
    bmatchTarFile = patternTar.findall(info.filename)
    if info.filename.startswith(tree_subdir) and info.file_size != 0 and (bmatchZipFile == [] or bmatchTarFile != []):
      basefilename = info.filename[len(tree_subdir)+1:]
      print "CopyImageFilesToOut- check %s" % (basefilename, )
      info2 = copy.copy(info)
      info2.filename = basefilename
      image_files.append(basefilename)
