  ...
  def WriteRawImage(self, mount_point, fn, mapfn=None):
    ...
    if fstab:
      ...
      if partition_type == "MTD":
        ...
        self.script.append(
              'write_raw_image_emmc(package_extract_file("%(fn)s"), "%(device)s");' % args)
      else:
        raise ValueError(
            "don't know how to write \"%s\" partitions" % p.fs_type)

  def WriteRTKImage(self):
    """ Write RTK related image. The rule are hidden in the native code now"""
    self.script.append('write_raw_rtkimage_emmc();')

  def UpdateBurnStatus(self, flag):
    """Update Upgrade Status for erase BCB judgement"""
    print "UpdateBurnStatus"
    self.script.append("update_status(%d);" % (flag,))
