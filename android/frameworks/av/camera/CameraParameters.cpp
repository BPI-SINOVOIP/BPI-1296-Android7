...
#if 1
const char CameraParameters::KEY_PREVIEW_ENABLED[] = "preview-enabled";
#endif
...
const char CameraParameters::PIXEL_FORMAT_H264[] = "h264";
...
#if 1
bool CameraParameters::changed(const CameraParameters &origParams, const char *key) const
{
    String8 k(key);
    ssize_t idx = mMap.indexOfKey(k);
    ssize_t origidx = origParams.mMap.indexOfKey(k);
    if(idx < 0 || origidx < 0){ //not found
        if(idx < 0 && origidx < 0)
            return false;
        else
            return true;
    ...
    String8 v = mMap.valueAt(idx);
    String8 origv = origParams.mMap.valueAt(origidx);
    if(origv != v)
        return true;
    else
        return false;
...
void CameraParameters::compare(const CameraParameters &origParams) const
{
    for(size_t i = 0; i < mMap.size(); i++){
        String8 k, v, origv;
        k = mMap.keyAt(i);
        ...
        origv = origParams.mMap.valueFor(k);
        if(origv != v)
            ALOGI("compare: [%s] changed: '%s' => '%s'", k.string(), origv.string(), v.string());
    }
}
#endif
