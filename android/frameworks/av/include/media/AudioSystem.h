...
//rtk begin
    // set/query RTKAudio state
    static status_t setRTKAudioActive(bool state);
    static bool isRTKAudioActive();
//rtk end
    ...
    static status_t getOutputFrameCount(size_t* frameCount,
            audio_stream_type_t stream);
//rtk begin
    static status_t getOutputFwLatency(uint32_t* latency,
           audio_stream_type_t stream);
//rtk end
    // FIXME This API assumes a route, and so should be deprecated.
...
//rtk begin
    // return the number of input latency by HAL implementation, or 0 if the handle is invalid
    static uint32_t getInputLatency(audio_io_handle_t ioHandle);
    // Returns the timestamp of HDMI RX in nanoseconds(systemtime)
    static int64_t getInputTimeStamp(audio_io_handle_t ioHandle);
//rtk end
    ...
    static status_t setHDMIRXstate(bool state);
    static status_t setHDMIRXvolume(int volume);
