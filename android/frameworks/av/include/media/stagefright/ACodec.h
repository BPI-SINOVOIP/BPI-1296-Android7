...
//#define ENABLE_CTS_WORKAROUND_ATM_457 /* The rootcause was CTS code flow, disable now */
...
private:
    ...
    status_t setupDTSHDCodec(bool encoder, int32_t numChannels, int32_t sampleRate);

    status_t setupMLPCodec(bool encoder, int32_t numChannels, int32_t sampleRate);

    status_t setupWMACodec(bool encoder, int32_t numChannels, int32_t sampleRate, bool isWMA);

    status_t setupADPCMCodec(bool encoder, int32_t numChannels, int32_t sampleRate);

    status_t setupDSDCodec(bool encoder, int32_t numChannels, int32_t sampleRate);

    status_t setupFLACCodec(bool encoder, int32_t numChannels, int32_t sampleRate);
    ...
    status_t setupALACCodec(const sp<AMessage> &msg, bool encoder);
