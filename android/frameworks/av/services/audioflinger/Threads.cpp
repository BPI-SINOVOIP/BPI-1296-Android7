...
// 250 * ~10msecs = 2.5 second
static const int kMaxTrackRetries = 250;
static const int kMaxTrackStartupRetries = 250;
//rtk end
...
//rtk begin
uint32_t AudioFlinger::PlaybackThread::fwlatency() const
{
    Mutex::Autolock _l(mLock);
    if (initCheck() == NO_ERROR) {
        if (mOutput->stream->get_fw_latency != NULL)
            return mOutput->stream->get_fw_latency(mOutput->stream);
    }
...
#ifdef USE_RTK_NO_STADBY
        //after resume[SCREEN_ON] need to call dostandby in HAL
        if (AudioFlinger::mStandbyTimeInNsecs == RESUME_STANDBY_TIME && !(screenState & 1)) {
            mStandbyTimeNs = systemTime() + AudioFlinger::mStandbyTimeInNsecs;
                if (shouldStandby_l()) {
                    ALOGD("do threadLoop_standby screenState = %u", screenState);
                    if (mOutDevice == AUDIO_DEVICE_OUT_BLUETOOTH_A2DP) {
                        AudioFlinger::mStandbyTimeInNsecs = kDefaultStandbyTimeInNsecs;
                    } else {
                        AudioFlinger::mStandbyTimeInNsecs = kRTKStandbyTimeInNsecs;
                        threadLoop_standby();
                        mStandby = true;
                    }
                    ALOGI("[%s:%d] %u mSec as standby time.", __FUNCTION__, __LINE__,
                        (uint32_t)(AudioFlinger::mStandbyTimeInNsecs / 1000000));
                }
        }
        //ALOGD("mStandbyTimeInNsecs = %llu screenState = %u", mStandbyTimeInNsecs,screenState);
        //mStandbyDelayNs = AudioFlinger::mStandbyTimeInNsecs;
#endif
...
#if 0 // rtk debug
        ALOGI("Write buffer %p with size %d", (char *)mSinkBuffer + offset, mBytesRemaining);
        FILE *fp1;
        if((fp1 = fopen("/data/test-audio-flinger.pcm", "ab")) != NULL)
        {
            fwrite((char *)mSinkBuffer + offset, mBytesRemaining, 1, fp1);
            fclose(fp1);
        }
#endif
    ...
    while (!exitPending())
    {
        ...
        { // scope for mLock
            ...
            if (mFramesWritten != lastFramesWritten) {
                //rtk modify for ATM-968
                serverLocationUpdate = false;
...
#ifdef USE_RTK_NO_STADBY
            //force into threadLoop_standby to closed pcm devices
            if (AudioFlinger::mStandbyTimeInNsecs == RESUME_STANDBY_TIME && mStandby != true) {
                mStandbyTimeNs = systemTime() - 1 ;
            }
#endif
            ...
            if ((!mActiveTracks.size() && systemTime() > mStandbyTimeNs) ||
                ...
                if (shouldStandby_l()) {
                    ...
                    threadLoop_standby();
#ifdef USE_RTK_NO_STADBY
                    ALOGD( " to standby Output device: %#x (%s)\n", mOutDevice, devicesToString(mOutDevice).string());
                    if (mOutDevice == AUDIO_DEVICE_OUT_BLUETOOTH_A2DP)
                       AudioFlinger::mStandbyTimeInNsecs = kDefaultStandbyTimeInNsecs;
                    else
                       AudioFlinger::mStandbyTimeInNsecs = kRTKStandbyTimeInNsecs;
    ...
    while (frameCount) {
        ...
        if (status != NO_ERROR || buffer.raw == NULL) {
            ...
            if (audio_has_proportional_frames(mFormat)) {
                 ...
                 * Rtk begin
                 * Fix DHCHERC-469
                 * No need to pad with 0 for pcm hw av sync
                 */
                if (!usesHwAvSync())
                    memset(curBuf, 0, frameCount * mFrameSize);
                else
                    ALOGW("[%s %d] HwAvSync: Disable padding 0", __func__, __LINE__);
                /* rtk end */
            }
            break;
        }
...
//rtk begin
uint32_t AudioFlinger::RecordThread::getInputLatency()
{
    Mutex::Autolock _l(mLock);
    if (initCheck() == NO_ERROR) {
        if (mInput->stream->get_input_latency != NULL)
            return mInput->stream->get_input_latency(mInput->stream);
    }

    return 0;
}

int64_t AudioFlinger::RecordThread::getInputTimeStamp()
{
    Mutex::Autolock _l(mLock);
    if (initCheck() == NO_ERROR) {
        if (mInput->stream->get_input_timestamp != NULL)
            return mInput->stream->get_input_timestamp(mInput->stream);
    }
