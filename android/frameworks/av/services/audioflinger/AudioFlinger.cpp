...
void AudioFlinger::onFirstRef()
{
    ...
    //if (property_get("persist.rtk.ctstest", val_str, NULL) && (!strcmp(val_str, "1") || !strcasecmp(val_str, "true"))) {
    if (1) {
        mStandbyTimeInNsecs = kRTKStandbyTimeInNsecs;
        ALOGI("RTK Using  %u mSec as standby time.",
                (uint32_t)(mStandbyTimeInNsecs / 1000000));
    }
#endif
...
//rtk begin
uint32_t AudioFlinger::fwlatency(audio_io_handle_t output) const
{
    Mutex::Autolock _l(mLock);
    PlaybackThread *thread = checkPlaybackThread_l(output);
    if (thread == NULL) {
...
status_t AudioFlinger::setMasterVolume(float value)
{
    ...
    //ALOGD("\033[0;33;33m %s value %f \033[m\n", __FUNCTION__, value);
...
/* Since RTK do NOT support Master Volume, to remove it.
#ifdef USE_RTK_AOUT_UTIL
    SetAudioMute(muted);
#endif
*/
...
//set Audio Volume in audio policy
#if 0//def USE_RTK_AOUT_UTIL
    #define MAX_RTK_VOLUME 31
    #define MIN_RTK_VOLUME 0
    if (stream == AUDIO_STREAM_MUSIC) {
        //ALOGD("\033[0;33;33m %s stream %d value %f \033[m\n", __FUNCTION__, stream, value);
        int vol = (int) (MAX_RTK_VOLUME * value);
        //SetAudioVolume(vol);
        SetAudioDecVolume(vol);
    }
...
#ifdef USE_RTK_NO_STADBY
            if (isOff) {
                mStandbyTimeInNsecs = RESUME_STANDBY_TIME;
            }
            ALOGD((isOff)? "setParameters SCREEN_OFF" : "setParameters SCREEN_ON");
#endif
...
//rtk begin
uint32_t AudioFlinger::getInputLatency(audio_io_handle_t ioHandle) const
{
    Mutex::Autolock _l(mLock);

    RecordThread *recordThread = checkRecordThread_l(ioHandle);
    if (recordThread != NULL) {
        return recordThread->getInputLatency();
    }
    return 0;
}

int64_t AudioFlinger::getInputTimeStamp(audio_io_handle_t ioHandle) const
{
    Mutex::Autolock _l(mLock);
    RecordThread *recordThread = checkRecordThread_l(ioHandle);
    if (recordThread != NULL) {
        return recordThread->getInputTimeStamp();
    }
                                                            ...
                                                            audio_output_flags_t flags)
{
    ...
    if (status == NO_ERROR) {
         ...
         * Use direct thread for HwAvSync
         */
        if ((flags & AUDIO_OUTPUT_FLAG_HW_AV_SYNC) && (flags & AUDIO_OUTPUT_FLAG_DIRECT)){
            thread = new DirectOutputThread(this, outputStream, *output, devices, mSystemReady);
            ALOGV("openOutput_l() created direct hw sync output: ID %d thread %p", *output, thread);
        /* Rtk end */
...
#ifdef USE_RTK_NO_STADBY
    if (*devices == AUDIO_DEVICE_OUT_BLUETOOTH_A2DP) {
        ALOGD("device AUDIO_DEVICE_OUT_BLUETOOTH_A2DP open");
        mStandbyTimeInNsecs = kDefaultStandbyTimeInNsecs;
    }
