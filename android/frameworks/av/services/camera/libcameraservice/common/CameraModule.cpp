...
#define FORCE_REFRESH_CAMERA_INFO   (1)
...
void CameraModule::deriveCameraCharacteristicsKeys(
    ...
    // Realtek Hal3 handling start
    const camera_metadata_t *buf = chars.getAndLock();
    bool breakNullBuf = false;
    if(buf == NULL){ breakNullBuf = true;}
    chars.unlock(buf);
    if(breakNullBuf){ return; }
    // Realtek Hal3 handling end
...
int CameraModule::getCameraInfo(int cameraId, struct camera_info *info) {
    ...
    if (index == NAME_NOT_FOUND) {
        ...
        ATRACE_BEGIN("camera_module->get_camera_info");
        ret = mModule->get_camera_info(cameraId, &rawInfo);
        ATRACE_END();
#if 0
        if (ret != 0) {
            return ret;
...
int CameraModule::updateNumberOfCameras() {
    int numCameras;
    if (mModule->update_number_of_cameras){
        ATRACE_BEGIN("camera_module->update_number_of_cameras");
        numCameras = mModule->update_number_of_cameras();
        ATRACE_END();
    } else{
        ALOGI("Function update_number_of_cameras is not implemented, use get_number_of_cameras instead.");
        numCameras = mModule->get_number_of_cameras();
    }
    return numCameras;
}
