...
status_t CameraClient::setPreviewWindow(const sp<IBinder>& binder,
    ...
    LOG1("setPreviewWindow E (pid %d)", getCallingPid());
    Mutex::Autolock lock(mLock);
...
#ifdef USE_SIDEBAND
    char nosb[PROPERTY_VALUE_MAX];
    char tunnel[PROPERTY_VALUE_MAX];
    property_get("rtk.feature-tunneled-playback", tunnel, "0");
    property_get("rtk.disable.sideband", nosb, "0");
    if(strcmp("0", nosb) == 0 && strcasecmp("1", tunnel) == 0){
        mSbClient = new SidebandClient();
        CHECK(mSbClient != NULL);

        mSbControl = mSbClient->createSurface(String8("Camera Surface"), 100, 100, PIXEL_FORMAT_RGB_888);
        if(mSbControl == NULL){ //fallback
            ALOGD("fallback to sf mode");
            mSbClient = NULL;
        }else{
            ALOGD("switch to sb mode");
            mNativeHandle = NULL;
            CHECK(mSbControl->getSidebandStream(&mNativeHandle) == 0);
            CHECK(mNativeHandle != NULL);

            mSourceHandle= NativeHandle::create(mNativeHandle, false);
            static_cast<Surface*>(window.get())->setSidebandStream(mSourceHandle);
            mSbPreviewWindow = static_cast<ANativeWindow *>(mSbControl->getSurface().get());
        }
    }
#endif
    ...
    if (mHardware->previewEnabled()) {
...
#ifdef USE_SIDEBAND
            if(mSbPreviewWindow != NULL)
                result = mHardware->setPreviewWindow(mSbPreviewWindow);
            else
#endif
        ...
        {
            /*
             * It's really weird. Camera2 apk set preview window as NULL
             * on preview enabled, so the original preview window
             * is disconnected below then Camera HAL will crash.
             * Stop preview here to avoid this problem
             */
            ALOGE("\033[31mstop preview on setting preview window to NULL\033[m");
            mHardware->stopPreview();
        }
#endif
    }
...
status_t CameraClient::startCameraMode(camera_mode mode) {
    LOG1("startCameraMode(%d) => %s", mode, mode ? "RECORD" : "PREVIEW");
    Mutex::Autolock lock(mLock);
    status_t result = checkPidAndHardware();
    if (result != NO_ERROR) return result;
...
#ifdef USE_SIDEBAND
    if(mSbPreviewWindow != NULL)
        mHardware->setPreviewWindow(mSbPreviewWindow);
    else
#endif
        mHardware->setPreviewWindow(mPreviewWindow);
...
void CameraClient::releaseRecordingFrame(const sp<IMemory>& mem) {
    LOG1("releaseRecordingFrame (pid %d)", getCallingPid());
    Mutex::Autolock lock(mLock);
    if (checkPidAndHardware() != NO_ERROR) return;
    if (mem == nullptr) {
...
String8 CameraClient::getParameters() const {
    LOG1("getParameters (pid %d)", getCallingPid());
    Mutex::Autolock lock(mLock);
...
bool CameraClient::lockIfMessageWanted(int32_t msgType) {
    ...
    while (mMsgEnabled & msgType) {
        if (mLock.tryLock() == NO_ERROR) {
            LOG1("lockIfMessageWanted (pid %d)", getCallingPid());
            if (sleepCount > 0) {
...
void CameraClient::handleGenericDataTimestamp(nsecs_t timestamp,
    ...
    if (c != 0 && dataPtr != nullptr) {
        ALOGV("handleGenericDataTimestamp: pointer:%p size:%zx", dataPtr->pointer(), dataPtr->size());
