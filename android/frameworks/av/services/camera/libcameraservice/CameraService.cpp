...
//#define IGNORE_CHECK_FOR_1195_DEMO
...
void CameraService::onFirstRef()
{
    ...
    ALOGV("In %s: Number of cameras = %d", __func__, mNumberOfCameras);
...
void CameraService::onDeviceStatusChanged(camera_device_status_t  cameraId,
    ...
    if (newStatus == CAMERA_DEVICE_STATUS_NOT_PRESENT) {
        ...
        sp<BasicClient> clientToDisconnect;
        {
            ...
            if(clientToDisconnect.get() == NULL) {
                return;
            }
...
Status CameraService::getNumberOfCameras(int32_t type, int32_t* numCameras) {
    ...
    switch (type) {
        case CAMERA_TYPE_BACKWARD_COMPATIBLE:
            mNumberOfNormalCameras = mModule->updateNumberOfCameras();
            ALOGV("In %s:%d, Number of normal cameras = %d", __func__, __LINE__, mNumberOfNormalCameras);
            *numCameras = mNumberOfNormalCameras;
            break;
        case CAMERA_TYPE_ALL:
            mNumberOfCameras = mModule->updateNumberOfCameras();
            ALOGV("In %s:%d, Number of cameras = %d", __func__, __LINE__, mNumberOfCameras);
...
Status CameraService::getCameraInfo(int cameraId,
    ...
    struct camera_info info;
    int rst = mModule->getCameraInfo(cameraId, &info);
    Status rc = filterGetInfoErrorCode(rst);
    if (rc.isOk() || rst == -EINVAL) {

        if(rst == -EINVAL) {
            ALOGW("[%s] camera %d is not available",__FUNCTION__,cameraId);
            rc = filterGetInfoErrorCode(NO_ERROR);
        }
...
#else
    ALOGW("Warning: loadSound feature is disabled!");
...
#ifdef IGNORE_CHECK_FOR_1195_DEMO
	ALOGE("!@#.ignore check!res=%d.AppOpsManager::MODE_ALLOWED=%d.",res,AppOpsManager::MODE_ALLOWED);
#else
