...
//rtk begin
status_t AudioPolicyService::setHDMIRXstate(bool state)
{
    if (mAudioPolicyManager == NULL) {
        return NO_INIT;
    }
    return mAudioPolicyManager->setHDMIRXstate(state);
}

status_t AudioPolicyService::setHDMIRXvolume(int volume)
{
    if (mAudioPolicyManager == NULL) {
        return NO_INIT;
    }
    return mAudioPolicyManager->setHDMIRXvolume(volume);
}

status_t AudioPolicyService::setRTKAudioActive(bool state)
{
    if (mAudioPolicyManager == NULL) {
        return NO_INIT;
    }
    return mAudioPolicyManager->setRTKAudioActive(state);
}

bool AudioPolicyService::isRTKAudioActive()
{
    if (mAudioPolicyManager == NULL) {
        return 0;
    }
    Mutex::Autolock _l(mLock);
    return mAudioPolicyManager->isRTKAudioActive();
}
//rtk end
