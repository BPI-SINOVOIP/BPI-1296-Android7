...
status_t Engine::setUSBState(bool isUSBconnected)
{
    ALOGD("setUSBState to %d", isUSBconnected);
    if (isUSBconnected == mUSBState ) {
        ALOGW("setUSBState() setting same state %d", isUSBconnected);
        return BAD_VALUE;
    }
    mUSBState = isUSBconnected;
    return NO_ERROR;
}
...
audio_devices_t Engine::getDeviceForInputSource(audio_source_t inputSource) const
{
    ...
    case AUDIO_SOURCE_VOICE_COMMUNICATION:
        ...
        case AUDIO_POLICY_FORCE_SPEAKER:
        //rtk begin
        //to support bluretooth voice bee device && USB MIC
    ...
    case AUDIO_SOURCE_HOTWORD:
        if (mForceUse[AUDIO_POLICY_FORCE_FOR_RECORD] == AUDIO_POLICY_FORCE_BT_SCO &&
            ...
            device = AUDIO_DEVICE_IN_USB_DEVICE;
        //rtk begin
        //If USB Headset is connected, to choose USB MIC first.
        //IF NO USB Headset , than to choose bluretooth voice bee device [AUDIO_DEVICE_IN_BACK_MIC].
        } else if (availableDeviceTypes & AUDIO_DEVICE_IN_BACK_MIC) {
            if (mUSBState) {
                device = AUDIO_DEVICE_IN_BUILTIN_MIC;
            } else {
    ...
    case AUDIO_SOURCE_CAMCORDER:
        //rtk add
        //to support bluretooth voice bee device
        if (availableDeviceTypes & AUDIO_DEVICE_IN_USB_DEVICE) {
            device = AUDIO_DEVICE_IN_USB_DEVICE;
        } else if (availableDeviceTypes & AUDIO_DEVICE_IN_BACK_MIC) {
            if (mUSBState) {
    ...
    //rtk: default audio in setting: I2S="1" or ADC="3"
    char value[PROPERTY_VALUE_MAX];
    property_get("persist.rtk.audio.in", value, "0");
    if (atoi(value) && device == AUDIO_DEVICE_IN_BACK_MIC) {
        device = AUDIO_DEVICE_IN_BUILTIN_MIC;
        ALOGD("getDeviceForInputSource()input source %d, device %08x for %s", inputSource, device, atoi(value)==1?"I2S":"ADC");
    }
