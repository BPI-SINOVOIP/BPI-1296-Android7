...
#include "libaudioRedirect/RedirectCommand.h"

#ifdef USE_RTK_AOUT_UTIL
#include <AudioUtil.h>
#define MAX_RTK_VOLUME 31
#define MIN_RTK_VOLUME 0
#endif
//rtk end
...
enum
{
    AUIDO_DEVICES_NO_CONNECTED = 0,
    AUDIO_DEVICES_CONNECTED
};

void AudioPolicyManager::setupAudioRedirect()
{
    ...
    if(binder == NULL) {
        ALOGE("failed to get AudioRedirect Service\n");
    } else {
        if (!mAudioRedirect) {
        ...
        if (mAudioRedirect) {
            ...
            if (isBTConnected) {
                ALOGD("send BT MODE\n");
                data.writeInt32(AUDIO_BT_CONNECTED);
                binder->transact(SET_AUDIO_MODE, data, &reply);
            } else if (isUSBConnected) {
                ALOGD("send USB MODE\n");
                data.writeInt32(AUDIO_USB_CONNECTED);
                binder->transact(SET_AUDIO_MODE, data, &reply);
            }
        }
    }
...
void AudioPolicyManager::stopAudioRedirect()
{
    ALOGD("%s", __FUNCTION__);
    ...
    if(binder == NULL) {
        ...
        if (!isBTConnected && !isUSBConnected && mAudioRedirect) {
            binder->transact(STOP_AUDIO_CAP, data, &reply);
            mAudioRedirect = false;
        }
    }
}
//endif
...
{
    //rtk start
    ALOGD("setDeviceConnectionState %08x %x %s", device, state,
    ...
    if (device == AUDIO_DEVICE_IN_REMOTE_SUBMIX && device_address) {
        ...
        if (parameters.getInt(String8("force"), forceValue) == OK) {
            ...
            ALOGD("mForceSubmixInputSelection = %d", mForceSubmixInputSelection);
            return 0;
        }
    }
    //rtk end
                                                         ...
                                                         const char *device_name)
{
    ...
    //DHCKYLIN-701:to set again to avoid HDMI and BT/USB output at the same time after suspend/resume.
#ifdef USE_RTK_AOUT_UTIL
    if(isBTConnected || isUSBConnected)
    {
        isHDMIMuted = true;
        SetAudioHdmiMute(true);
        setupAudioRedirect();
    }
#endif
    ...
    if (audio_is_output_device(device)) {
        ...
        case AUDIO_POLICY_DEVICE_STATE_AVAILABLE: {
            if (index >= 0){
                //rtk begin for multi usb_audio case
                //when more than one usb_audio cards are plugged, to clean the DeviceDescriptor first than to add again.
                if (device == AUDIO_DEVICE_OUT_ANLG_DOCK_HEADSET) {
                    ALOGW("setDeviceConnectionState() more than one AUDIO_DEVICE_OUT_ANLG_DOCK_HEADSET ");
...
//rtk begin
            if((device & AUDIO_DEVICE_OUT_ALL_A2DP) /*|| (device & AUDIO_DEVICE_OUT_ALL_SCO)*/) {
                ALOGV("\033[0;33;33m connecting BT device %x\033[m\n", device);
                isBTConnected = true;
                setupAudioRedirect();
            }
            else if((device & AUDIO_DEVICE_OUT_ANLG_DOCK_HEADSET) || (device & AUDIO_DEVICE_OUT_DGTL_DOCK_HEADSET)) {
                ALOGV("\033[0;33;33m connecting DOCK HEADSET %x\033[m\n", device);
                isUSBConnected = true;
                mEngine->setUSBState(true);
                setupAudioRedirect();
            }
#ifdef USE_RTK_AOUT_UTIL
            if(isBTConnected || isUSBConnected)
...
//rtk begin
            if((device & AUDIO_DEVICE_OUT_ALL_A2DP) /*|| (device & AUDIO_DEVICE_OUT_ALL_SCO)*/) {
                ALOGV("\033[0;33;33m disconnecting BT device %x\033[m\n", device);
                isBTConnected = false;
                stopAudioRedirect();
            }
            else if((device & AUDIO_DEVICE_OUT_ANLG_DOCK_HEADSET) || (device & AUDIO_DEVICE_OUT_DGTL_DOCK_HEADSET)) {
                ALOGV("\033[0;33;33m disconnecting DOCK HEADSET %x\033[m\n", device);
                isUSBConnected = false;
                mEngine->setUSBState(false);
                stopAudioRedirect();
            }
#ifdef USE_RTK_AOUT_UTIL
            if(!isBTConnected && !isUSBConnected)
            {
                //we set hdmi unmute later to avoid POP sound
                //SetAudioHdmiMute(false);
                isHDMIMuted = false;
            }
#endif
//rtk end
...
//rtk modify
    if (((flags & AUDIO_OUTPUT_FLAG_DIRECT) == 0 && (flags & AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD) == 0) && audio_is_linear_pcm(format)) {
        ALOGD(" pcm_stream flags %x non_direct_output ", flags);
        goto non_direct_output;
    }
//rtk end
...
//rtk modify
        if ((flags & AUDIO_OUTPUT_FLAG_HW_AV_SYNC) == 0) {
            outputDesc->mFlags = (audio_output_flags_t)(outputDesc->mFlags &(~AUDIO_OUTPUT_FLAG_HW_AV_SYNC));
            ALOGD("stream flag %x is no AUDIO_OUTPUT_FLAG_HW_AV_SYNC, outputDesc->mFlags %x ", flags, outputDesc->mFlags);
        }
//rtk end
...
//rtk begin
            //[Workaround] If audio HAL has create_audio_patch(), we should check it.
            //setOutputDevice(outputDesc, newDevice, false, outputDesc->latency()*2);
            setOutputDevice(outputDesc, newDevice, false, 0);
//rtk end
    ...
    for (size_t i = 0; i < mOutputs.size(); i++) {
        ...
        for (int curStream = 0; curStream < AUDIO_STREAM_FOR_POLICY_CNT; curStream++) {
            ...
            if (!(desc->isStreamActive((audio_stream_type_t)curStream) ||
                ...
                //rtk modify:DHCHERC-706
                if (curStream == AUDIO_STREAM_MUSIC && (curDevice & (AUDIO_DEVICE_OUT_HDMI|AUDIO_DEVICE_OUT_SPEAKER)) ) {
                    ALOGD("to set AUDIO_STREAM_MUSIC volume curDevice %x ", curDevice);
                } else {
                    continue;
                }
                //rtk end
...
//rtk begin
status_t AudioPolicyManager::setHDMIRXvolume(int volume)
{
    if (volume < 0 || volume > MAX_RTK_VOLUME) {
        ALOGE("setHDMIRXvolume error with volume %d", volume);
        return BAD_VALUE;
    }

    ALOGD("to set setHDMIRXvolume %d", volume);
    mHDMIRXvolume = volume;

    if (mHDMIRXvolume != MAX_RTK_VOLUME) {
        SetAudioHdmiRxVolume(mHDMIRXvolume, mHDMIRXAudioAOID, mHDMIRXAudioAIID, mHDMIRXAudioPIN);
    } else {
        float value = (float)mStreamVolumeIndex / 15;
        int vol = (int) (MAX_RTK_VOLUME * value);
        SetAudioHdmiRxVolume(vol, mHDMIRXAudioAOID, mHDMIRXAudioAIID, mHDMIRXAudioPIN);
    }
    return NO_ERROR;

}

status_t AudioPolicyManager::setHDMIRXstate(bool state)
{
#if 0
    /* Fix DHCKYLIN-3166: RtkHDMIRxManager assume AudioPolicyManager will unmute Rx */
    if (isHDMIRXConnected == state)
        return NO_ERROR;
#endif

    isHDMIRXConnected = state;
#ifdef USE_RTK_AOUT_UTIL
    if (isHDMIRXConnected) {
        char propvalue[PROPERTY_VALUE_MAX];
        if (property_get("rtk.hdmirx.config.aoid", propvalue, "0")) {
            mHDMIRXAudioAOID = atoi(propvalue);
            ALOGV(" get AudioAOID = %ld", mHDMIRXAudioAOID);
        }

        if (property_get("rtk.hdmirx.config.aiid", propvalue, "0")) {
            mHDMIRXAudioAIID = atoi(propvalue);
            ALOGV("get AudioAIID = %ld", mHDMIRXAudioAIID);
        }

        if (property_get("rtk.hdmirx.config.pinid", propvalue, "0")) {
            mHDMIRXAudioPIN = atoi(propvalue);
            ALOGV("get AudioPIN = %ld", mHDMIRXAudioPIN);
        }

        if (mHDMIRXAudioPIN == 0) {
            isHDMIRXConnected = false;
            ALOGE("HDMI RX audio is NOT ready");
            return INVALID_OPERATION;
        }

        //SetAudioMic1Volume(vol);
        if (mHDMIRXvolume != MAX_RTK_VOLUME) {
            SetAudioHdmiRxVolume(mHDMIRXvolume, mHDMIRXAudioAOID, mHDMIRXAudioAIID, mHDMIRXAudioPIN);
            ALOGD("isHDMIRXConnected = %d mHDMIRXvolume = %d",isHDMIRXConnected, mHDMIRXvolume);
        } else {
            float value = (float)mStreamVolumeIndex / 15;
            int vol = (int) (MAX_RTK_VOLUME * value);
            SetAudioHdmiRxVolume(vol, mHDMIRXAudioAOID, mHDMIRXAudioAIID, mHDMIRXAudioPIN);
            ALOGD("isHDMIRXConnected = %d vol = %d",isHDMIRXConnected, vol);
        }

        /*to info audio fw to src the sample rate "HDMI RX IN" to 48K*/
        if (isBTConnected || isUSBConnected) {
            SetHDMIRXtoBTUsb(AUDIO_DEVICES_CONNECTED,mHDMIRXAudioAIID);
            ALOGD("AUDIO_DEVICES_CONNECTED:SetHDMIRXtoBTUsb = 1 in setHDMIRXstate");
        }
    } else {
        SetAudioHdmiRxVolume(0,mHDMIRXAudioAOID,mHDMIRXAudioAIID,mHDMIRXAudioPIN); //to mute
        ALOGD("isHDMIRXConnected = %d mute",isHDMIRXConnected);
        if (isBTConnected || isUSBConnected) {
            SetHDMIRXtoBTUsb(AUIDO_DEVICES_NO_CONNECTED,mHDMIRXAudioAIID);
            ALOGD("AUIDO_DEVICES_NO_CONNECTED:SetHDMIRXtoBTUsb = 0 in setHDMIRXstate");
        }
    }
    ALOGD("AudioAOID =%ld  AudioAIID = %ld AudioPIN=%ld", mHDMIRXAudioAOID ,mHDMIRXAudioAIID ,mHDMIRXAudioPIN);
#endif
    return NO_ERROR;
}

status_t AudioPolicyManager::setRTKAudioActive(bool state)
{
    //ALOGD("%s %d state %d", __FUNCTION__, __LINE__, state);
    mRTKAudioActive = state;
    return NO_ERROR;
}

bool AudioPolicyManager::isRTKAudioActive()
{
    //ALOGD("%s %d", __FUNCTION__, __LINE__);
    return mRTKAudioActive;
}
//rtk end
...
//rtk modify
    if (property_get("rtk.hdmi.output", propValue, "1")) {
        hdmiMode = atoi(propValue);
        //ALOGD(" hdmiMode %d", hdmiMode);
    }
    if (property_get("rtk.spdif.output", propValue, "1")) {
        spdifMode =  atoi(propValue);
        //ALOGD("spdifMode %d", spdifMode);
    }

    if ((hdmiMode == 0) || (hdmiMode == 3) || (spdifMode == 0)) {
        ALOGD("enable offload hdmiMode %d spdifMode %d", hdmiMode, spdifMode);
    } else {
        ALOGD("disable offload hdmiMode %d spdifMode %d", hdmiMode, spdifMode);
        return false;
    }
    ...
    const bool allowOffloadWithVideo =
            property_get_bool("audio.offload.video", false );
...
//rtk begin
    ...
    mHDMIRXAudioAIID(0),
    mHDMIRXAudioPIN(0),
    mStreamVolumeIndex(11),
#endif
    mRTKAudioActive(false)
//rtk end
{
    mUidCached = getuid();
...
//rtk begin
    ...
    memset(propvalue, 0x0, sizeof(propvalue));
    if (mBootComplete == false){
        if (property_get("sys.boot_completed", propvalue, "0") && atoi(propvalue) == 1) {
                mBootComplete = true;
                ALOGD(" get mBootComplete = %d", mBootComplete);
            }
    }

    if (stream == AUDIO_STREAM_MUSIC) {
        //We do not call SetAudioVolume when the volume changes from mute to unmute,
        //because it will cause noise. (set default_stream_volume(11) first and set index 0 after)
        //However, it has the same procedure when android boot up.
        //Therefore, we always keep the index value in mStreamVolumeIndex.
        if(mStreamVolumeIndex == 0 && index == 11/*DEFAULT_STREAM_VOLUME*/) {
            //ALOGD("\033[0;33;33m %s change unmute to mute\033[m\n", __FUNCTION__);
        } else if (mBootComplete) {
            float value = (float)index / 15;
            ALOGD("\033[0;33;33m %s device %d stream %d index %d value %f\033[m\n", __FUNCTION__, device, stream, index, value);
            int vol = (int) (MAX_RTK_VOLUME * value);
            //SetAudioVolume(vol);
            //Set Dvdplayer volume
            SetAudioDecVolume(vol);
            //Set HDMI Rx volume
            if (isHDMIRXConnected) {
                //SetAudioMic1Volume(vol);
                if (mHDMIRXvolume != MAX_RTK_VOLUME) {
        ...
        if(!isHDMIMuted && device == AUDIO_DEVICE_OUT_SPEAKER && mBootComplete)
        {
            //ALOGD("\033[0;33;33m setAudioHdmiMute false\033[m\n");
            memset(propvalue, 0x0, sizeof(propvalue));
            property_get("persist.rtk.hdmi.audio", propvalue, NULL);
            if (!strcmp(propvalue, "on")) {
                SetAudioHdmiMute(false);
            }
        }

        mStreamVolumeIndex = index;
    }
#endif
//rtk end
