...
bool DrmManagerService::isProtectedCallAllowed(drm_perm_t perm) {
...
#ifdef ENABLE_DRM_PLUGIN_SERVICE
    ALOGI("Recevied uid:%d pid:%d perm:%d want to use DRM", uid, spid, perm);
    return true;
#else /* else of ENABLE_DRM_PLUGIN_SERVICE */
    ALOGI("\033[0;32m Recevied uid:%d want to use DRM maybe return false\033[m", uid);
    ...
    return false;
#endif /* end of ENABLE_DRM_PLUGIN_SERVICE */
}
