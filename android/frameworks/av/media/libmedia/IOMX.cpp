...
//[ RTK begin ]
    virtual status_t setBSRingBuffer(
        ...
        int32_t *kSetBSRingParams = (int32_t *)params;
        /*
           kSetBSRingParams[0] = rvsd_BSringPhyaddr;
           kSetBSRingParams[1] = rvsd_BSringSize;
           kSetBSRingParams[2] = rvsd_ipc_BSsharefd;
           kSetBSRingParams[3] = rvsd_BSHDRAddr;
           kSetBSRingParams[4] = rvsd_BSHDRSize;
           kSetBSRingParams[5] = rvsd_ipc_BSHDRsharefd;
           kSetBSRingParams[6] = rvsd_RefClockSize;
           kSetBSRingParams[7] = rvsd_ipc_RefClocksharefd;
           int i;
           for (i=0; i<8; i++)
           {
           ALOGD("%s:%d [=====] kSetBSRingParams[%d] = 0x%.8x\n", __func__, __LINE__, i, kSetBSRingParams[i]);
           }
        */
        data.writeInt32(kSetBSRingParams[0]);
        data.writeInt32(kSetBSRingParams[1]);
        data.writeDupFileDescriptor(kSetBSRingParams[2]);
        data.writeInt32(kSetBSRingParams[3]);
        data.writeInt32(kSetBSRingParams[4]);
        data.writeDupFileDescriptor(kSetBSRingParams[5]);
        data.writeInt32(kSetBSRingParams[6]);
        data.writeDupFileDescriptor(kSetBSRingParams[7]);
        remote()->transact(SET_BSRING_BUFFER, data, &reply);

        return reply.readInt32();
    }

    virtual status_t getIonFd(
            node_id node, OMX_INDEXTYPE index,
        void *params, size_t size, OMX_S32 *shareFd) {
        Parcel data, reply;
        data.writeInterfaceToken(IOMX::getInterfaceDescriptor());
        data.writeInt32((int32_t)node);
        data.writeInt32(index);
        data.writeInt64(size);
        data.write(params, size);
        remote()->transact(GET_ION_FD, data, &reply);
        ...
        *shareFd = 0;
        ...
        reply.read(params, size);
        OMX_PLUS_EXTRADATATYPE *rtkHeader = (OMX_PLUS_EXTRADATATYPE *)params;
        if (rtkHeader->shareFd > 0 && rtkHeader->enableCrypto > 0)
        {
            *shareFd = dup(reply.readFileDescriptor());
            ALOGV("\033[0;32m ---- In %s:%d recevied shareFd:%d \033[m", __FUNCTION__, __LINE__, *shareFd);
            /* Note : need close shareFd, current close in MediaCodec.cpp */
        }
        return OK;
    }

    virtual void setReduceOMXMode(node_id node, bool enableReduceOmx) {
        Parcel data, reply;
        data.writeInterfaceToken(IOMX::getInterfaceDescriptor());
        data.writeInt32((int32_t)node);
        data.writeInt32(enableReduceOmx);
        remote()->transact(SET_REDUCE_OMX_MEMCPY, data, &reply);
    }
    ...
    switch (code) {
        ...
        case ALLOC_SECURE_BUFFER:
        {
            ...
            if (!isSecure(node) || port_index != 0 /* kPortIndexInput */) {
                ALOGV("\033[0;35m @@@@@@@@ In %s:%d isReduceOMXMode:%d @@@@@@\033[m", __FUNCTION__, __LINE__, isReduceOMXMode(node));
                if (!isReduceOMXMode(node))
                {
                    ALOGE("b/24310423");
                    reply->writeInt32(INVALID_OPERATION);
                    return NO_ERROR;
        ...
        case SET_BSRING_BUFFER:
        {
            ...
            void *params = malloc(sizeof(int32_t) + size);
            int32_t *kSetBSRingParams = &((int32_t *)params)[1];
            kSetBSRingParams[0] = data.readInt32();
            kSetBSRingParams[1] = data.readInt32();
            kSetBSRingParams[2] = dup(data.readFileDescriptor());
            kSetBSRingParams[3] = data.readInt32();
            kSetBSRingParams[4] = data.readInt32();
            kSetBSRingParams[5] = dup(data.readFileDescriptor());
            kSetBSRingParams[6] = data.readInt32();
            kSetBSRingParams[7] = dup(data.readFileDescriptor());
            ((int32_t *)params)[0] = size;

            status_t err;
            err = setBSRingBuffer(node, index, params, size);
        ...
        case GET_ION_FD:
        {
            ...
            if (err == OK) {
                reply->write(params, size);
                OMX_PLUS_EXTRADATATYPE *rtkHeader = (OMX_PLUS_EXTRADATATYPE *)params;
                ALOGV("\033[0;32m ----- in %s:%d shareFd %d enableCrypto %d-----\033[m", __FUNCTION__, __LINE__, rtkHeader->shareFd, rtkHeader->enableCrypto);
                if (rtkHeader->shareFd > 0 && rtkHeader->enableCrypto > 0)
                    reply->writeDupFileDescriptor(rtkHeader->shareFd);
            }
        ...
        /* Reduce omx input-port memcpy for Camera, some APP */
        case SET_REDUCE_OMX_MEMCPY:
        {
            CHECK_OMX_INTERFACE(IOMX, data, reply);
            ...
            ALOGV("\033[0;32m $$$$$ In %s:%d call setReduceOMXMode, option:%d ##### \033[m", __FUNCTION__, __LINE__, option);
            setReduceOMXMode(node, option);
            return OK;
        }
        //[ RTK end ]
