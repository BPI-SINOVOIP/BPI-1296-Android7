...
//#define RTK_MIN_FRAMECOUNT              (1024*6)
...
uint32_t AudioTrack::latency() const
{
    ...
    //uint32_t awLatency = pcm_get_aw_latency();
    uint32_t awLatency = 0;
    if (AudioSystem::getOutputFwLatency(&awLatency, AUDIO_STREAM_MUSIC) != NO_ERROR) {
        return NO_INIT;
    }

    //ALOGD("[func %s pid %d]mLatency %u (afLatency %u, mCblkLatency %u, awLatency %u)\n", __FUNCTION__, (int)gettid(), afLatency + mCblkLatency + awLatency, afLatency, mCblkLatency, awLatency);
    return afLatency + mCblkLatency + awLatency;
#else
    return mLatency;
#endif
}
...
status_t AudioTrack::createTrack_l()
{
...
#ifdef RTK_MIN_FRAMECOUNT
        //workaround for audioflinger underrun when frameCount is too small
        if (frameCount < RTK_MIN_FRAMECOUNT)
            frameCount = RTK_MIN_FRAMECOUNT;
#endif
    }
    ...
    /*
     * Rtk begin: Fix DHCHERC-469
     * Increase audio track buffer size to prevent from underrun
     */
    if (((flags & AUDIO_OUTPUT_FLAG_HW_AV_SYNC) != 0) && audio_is_linear_pcm(mFormat)){
        frameCount *= 5;
        ALOGW("PCM hardware sync: Increase frameCount to %zu", frameCount);
    }
    /* Rtk end */
...
//rtk begin
    mCblkLatency = (1000*frameCount) / mSampleRate;
//rtk end
...
#if 0
/*
 * DO NOT remove hw av sync header
 * Now we will separate header and audio data in linux kernel
 * Header and audio data will be written into differnt ring buffer
 */
//rtk begin
    // There is noise issue when using AUDIO_OUTPUT_FLAG_HW_AV_SYNC.
    // Because mAvSyncHeader is not real audio data. We need to drop it before writing audio data to audioflinger.
    // mAvSyncHeader sync word and size is defined in frameworks/base/media/java/android/media/AudioTrack.java
    if ((mFlags & AUDIO_OUTPUT_FLAG_HW_AV_SYNC) != 0) {
        size_t mAvSyncHeaderSize = 16;
        int headerSyncWord = 0x01005555;
        if (!memcmp(buffer, &headerSyncWord, sizeof(headerSyncWord))) {
            //ALOGI("drop mAvSyncHeader\n");
            buffer = ((const char *) buffer) + mAvSyncHeaderSize;
            userSize -= mAvSyncHeaderSize;
            written += mAvSyncHeaderSize;
        }
    }
//rtk end
#endif
...
#if 0 // rtk debug
        ALOGI("Dump buffer %p with size %d", buffer, toWrite);
        FILE *fp1;
        if((fp1 = fopen("/data/test-audio-track.pcm", "ab")) != NULL)
        {
            fwrite(buffer, toWrite, 1, fp1);
            fclose(fp1);
        }
#endif
