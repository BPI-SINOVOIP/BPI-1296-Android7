...
MediaScanner::MediaScanner()
    : mLocale(NULL), mSkipList(NULL), mSkipIndex(NULL), mForceStop(false) {
    loadSkipList();
}
...
MediaScanResult MediaScanner::processDirectory(
    ...
    if(mForceStop) return MEDIA_SCAN_RESULT_ERROR;
...
void MediaScanner::setNativeForceStopProperty(bool stop) {
    if(mForceStop != stop)
        mForceStop = stop;
}
...
bool MediaScanner::shouldSkipProcessingItsContentByResult(status_t status) {
    if (OK_MEDIA_AVCHD_FOLDER == status ||
        OK_MEDIA_BDROM_FOLDER == status ||
        OK_MEDIA_DVD_FOLDER == status) {
        return true;
    }
    return false;
}
...
MediaScanResult MediaScanner::doProcessDirectoryEntry(
    ...
    if (type == DT_DIR) {
        if(strcmp("$RECYCLE.BIN", entry->d_name) == 0) {
            ALOGD("Found windows recycle folder, ignore it ...");
            return MEDIA_SCAN_RESULT_SKIPPED;
        }
        ...
        if (stat(path, &statbuf) == 0) {
            ...
            if (shouldSkipProcessingItsContentByResult(status)) {
                ALOGD("%s is a media folder..", path);
                return MEDIA_SCAN_RESULT_SKIPPED;
            }
        }
