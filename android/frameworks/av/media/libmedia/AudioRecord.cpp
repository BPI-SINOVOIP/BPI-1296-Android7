        ...
        const audio_attributes_t* pAttributes)
{
    ...
    // rtk begin
    if (inputSource == AUDIO_SOURCE_HDMIRX) {
        //mLatency = (1000*(mFrameCount) / sampleRate + getInputLatency();
        mLatency = (1000*mFrameCount) / sampleRate + 28;     // HDMI RX latency avg is about 28.
                                                            //(getInputLatency is 0 when pcm_read is NOT running)
    } else {
...
//rtk begin
uint32_t AudioRecord::getInputLatency() const
{
    // no need to check mActive, because if inactive this will return 0, which is what we want
    return AudioSystem::getInputLatency(getInputPrivate());
}

int64_t AudioRecord::getInputTimeStamp() const
{
    // no need to check mActive, because if inactive this will return 0, which is what we want
    return AudioSystem::getInputTimeStamp(getInputPrivate());
}
//rtk end
