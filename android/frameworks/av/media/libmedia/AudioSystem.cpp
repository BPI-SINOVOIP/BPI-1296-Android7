...
//rtk begin
status_t AudioSystem::getOutputFwLatency(uint32_t* aflatency, audio_stream_type_t streamType)
{
    audio_io_handle_t output;
    output = getOutput(streamType);
    if (output == AUDIO_IO_HANDLE_NONE) {
...
//rtk begin
uint32_t AudioSystem::getInputLatency(audio_io_handle_t ioHandle)
{
    const sp<IAudioFlinger>& af = AudioSystem::get_audio_flinger();
    uint32_t result = 0;
    if (af == 0) return result;
    ...
    result = af->getInputLatency(ioHandle);
    return result;
}

int64_t AudioSystem::getInputTimeStamp(audio_io_handle_t ioHandle)
{
    const sp<IAudioFlinger>& af = AudioSystem::get_audio_flinger();
    if (af == 0) return 0;
    if (ioHandle == AUDIO_IO_HANDLE_NONE) return 0;

    return af->getInputTimeStamp(ioHandle);
}
//rtk end
...
//rtk begin
status_t AudioSystem::setRTKAudioActive(bool state)
{
    const sp<IAudioPolicyService>& aps = AudioSystem::get_audio_policy_service();
    if (aps == 0) return PERMISSION_DENIED;
    //ALOGD("%s %d state %d\n", __FUNCTION__, __LINE__, state);
    return aps->setRTKAudioActive(state);
}

bool AudioSystem::isRTKAudioActive()
{
    const sp<IAudioPolicyService>& aps = AudioSystem::get_audio_policy_service();
    if (aps == 0) return PERMISSION_DENIED;
    return aps->isRTKAudioActive();
}
//rtk end

status_t AudioSystem::setHDMIRXstate(bool state)
{
    const sp<IAudioPolicyService>& aps = AudioSystem::get_audio_policy_service();
    if (aps == 0) return PERMISSION_DENIED;
    return aps->setHDMIRXstate(state);
}

status_t AudioSystem::setHDMIRXvolume(int volume)
{
    const sp<IAudioPolicyService>& aps = AudioSystem::get_audio_policy_service();
    if (aps == 0) return PERMISSION_DENIED;
    return aps->setHDMIRXvolume(volume);
}
...
//rtk begin
    //Because the volume control is based on MUSIC isStreamActive to control volume type(MUSIC or NOTIFACTION),
    //we also need to consider dvdplayer or hdmi pass-through case with AUDIO_STREAM_MUSIC.
    if (stream == AUDIO_STREAM_MUSIC)
    {
        bool mRTKAudioActive = AudioSystem::isRTKAudioActive();
        *state |= mRTKAudioActive;
        //ALOGD("isStreamActive state %d mRTKAudioActive %d\n", *state, mRTKAudioActive);
    }
//rtk end
    return NO_ERROR;
}
