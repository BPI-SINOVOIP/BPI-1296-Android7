...
//rtk begin
    virtual status_t setRTKAudioActive(bool state)
    {
        Parcel data, reply;
        data.writeInterfaceToken(IAudioPolicyService::getInterfaceDescriptor());
        data.writeInt32(state);
        remote()->transact(SET_RTK_AUDIO_ACTIVE, data, &reply);
        return static_cast <status_t> (reply.readInt32());
    }

    virtual bool isRTKAudioActive()
    {
        Parcel data, reply;
        data.writeInterfaceToken(IAudioPolicyService::getInterfaceDescriptor());
        remote()->transact(IS_RTK_AUDIO_ACTIVE, data, &reply);
        return reply.readInt32();
    }
//rtk end

    virtual status_t setHDMIRXstate(bool state)
    {
        Parcel data, reply;
        data.writeInterfaceToken(IAudioPolicyService::getInterfaceDescriptor());
        data.writeInt32(state);
        remote()->transact(SET_HDMIRX_STATE, data, &reply);
        return static_cast <status_t> (reply.readInt32());
    }

    virtual status_t setHDMIRXvolume(int volume)
    {
        Parcel data, reply;
        data.writeInterfaceToken(IAudioPolicyService::getInterfaceDescriptor());
        data.writeInt32(volume);
        remote()->transact(SET_HDMIRX_VOLUME, data, &reply);
        return static_cast <status_t> (reply.readInt32());
    }
...
//rtk begin
        case SET_RTK_AUDIO_ACTIVE: {
            CHECK_INTERFACE(IAudioPolicyService, data, reply);
            bool state = static_cast <bool>(data.readInt32());
            reply->writeInt32(static_cast <int32_t>(setRTKAudioActive(state)));
            return NO_ERROR;
        } break;

        case IS_RTK_AUDIO_ACTIVE: {
            CHECK_INTERFACE(IAudioPolicyService, data, reply);
            reply->writeInt32(isRTKAudioActive());
            return NO_ERROR;
        } break;

        case SET_HDMIRX_STATE: {
            CHECK_INTERFACE(IAudioPolicyService, data, reply);
            bool state = static_cast <bool>(data.readInt32());
            reply->writeInt32(static_cast <int32_t>(setHDMIRXstate(state)));
            return NO_ERROR;
        } break;

        case SET_HDMIRX_VOLUME: {
            CHECK_INTERFACE(IAudioPolicyService, data, reply);
            int volume = data.readInt32();
            reply->writeInt32(static_cast <int32_t>(setHDMIRXvolume(volume)));
            return NO_ERROR;
        } break;
