...
#ifdef RTK_ENHANCE
status_t MediaRecorder::setOutputBuffer(const sp<IMemory> &bufferData, const sp<IMemory> &bufferInfo)
{
    ...
    if (!(mCurrentState & MEDIA_RECORDER_DATASOURCE_CONFIGURED)) {
        ALOGE("setOutputBuffer called in an invalid state: %d", mCurrentState);
        return INVALID_OPERATION;
    }

    status_t ret = mMediaRecorder->setOutputBuffer(bufferData, bufferInfo);
    if (OK != ret) {
        ALOGE("setOutputBuffer failed: %d", ret);
        mCurrentState = MEDIA_RECORDER_ERROR;
        return ret;
    }
    return ret;
