...
status_t convertMetaDataToMessage(
    ...
    /* Add for RtkExtractor */
    int32_t rtkExtractor = 0;
    if (meta->findInt32(kKeyRTKExtr, &rtkExtractor) && rtkExtractor != 0) {
        msg->setInt32("used-RTKExtr", 1);
    }
    ...
    if (!strncasecmp("video/", mime, 6)) {
        ...
        /* Fixed for SoftwareRender */
        int32_t strideWidth = 0;
        int32_t strideHight = 0;
        meta->findInt32(kKeyDisplayWidth, &strideWidth);
        meta->findInt32(kKeyDisplayHeight, &strideHight);
        if (strideWidth == 0)
            strideWidth = width;
        if (strideHight == 0)
            strideHight = height;
        msg->setInt32("stride", strideWidth);
        msg->setInt32("slice-height", strideHight);
        ...
        if (meta->findInt32(kKeyAudioBitsPerSample, &bitsPerSample)) {
            msg->setInt32("bits-sample", bitsPerSample);
        }
...
#ifdef RTK_PLATFORM
    float fFps;
    if (meta->findFloat(kKeyFrameRateFloat, &fFps) && fFps > 0) {
        msg->setFloat("frame-rate-float", fFps);
    }
    int32_t isMVC;
    if (meta->findInt32(kKeyIsMVC, &isMVC) && isMVC > 0) {
        msg->setInt32("cMVC", isMVC);
    }
    int32_t rows;
    if (meta->findInt32(kKeyRows,  &rows)) {
        msg->setInt32("rows", rows);
    }
    int32_t columns;
    if (meta->findInt32(kKeyColumns,  &columns)) {
        msg->setInt32("columns", columns);
    }
    int32_t outputWidth;
    if (meta->findInt32(kKeyOutputWidth,  &outputWidth)) {
        msg->setInt32("outputWidth", outputWidth);
    }
    int32_t outputHeight;
    if (meta->findInt32(kKeyOutputHeight,  &outputHeight)) {
        msg->setInt32("outputHeight", outputHeight);
    }
#endif /* end of RTK_PLATFORM */
    ...
    if (meta->findData(kKeyAVCC, &type, &data, &size)) {
...
#if 0 //FIXME: disable temporarily
        // Parse the HEVCDecoderConfigurationRecord
        unsigned profile, level;
        //status_t err;
        const uint8_t *ptr = (const uint8_t *)data;
        ALOGI("HVCC DATA : size= %d, 0x%x 0x%x 0x%x 0x%x 0x%x", (int)size, ptr[0], ptr[1], ptr[2], ptr[3], ptr[4]);

#ifdef RTK_PLATFORM
        // verify minimum size and configurationVersion == 1 or 0
        if (size < 22 || ptr[0] > 1) {
            ALOGE("size < 22 || ptr[0] != 1");
        ...
        profile = ptr[1] & 31;						// general_profile_idc	(5bits)
        level = ptr[12];							// general_level_idc	(8bits)

        //size_t lengthSize = 1 + (ptr[21] & 3);		// lenghtSizeMinusOne	(2bits)
        size_t numOfArrays = ptr[22];				// numOfArray			(8bits)

        ptr += 23;
        size -= 23;

        sp<ABuffer> buffer = new ABuffer(4096);
        buffer->setRange(0, 0);
        ...
        for (size_t i = 0; i < numOfArrays; i++) {
            ...
            ptr += 1;
            size -= 1;
            size_t numNalus = U16_AT(ptr);			// numNalus				(16bits)

            ptr += 2;
            size -= 2;
            for (size_t j = 0; j < numNalus; j++) {
                size_t nalUnitLength = U16_AT(ptr);	// nalUnitLength		(16bits)
                ptr += 2;
                size -= 2;
                if (size < nalUnitLength) {
                    ALOGE("size < nalUnitLength");
                    return ERROR_MALFORMED;
                }

                memcpy(buffer->data() + buffer->size(), "\x00\x00\x00\x01", 4);
                memcpy(buffer->data() + buffer->size() + 4, ptr, nalUnitLength);
                buffer->setRange(0, buffer->size() + 4 + nalUnitLength);

                ptr += nalUnitLength;
                size -= nalUnitLength;
            }
        }
        ALOGI("HEVC profile = %u , level = %u",profile, level);
#else
        const uint8_t *ptr = (const uint8_t *)data;

#ifdef RTK_PLATFORM
        if (size < 23 || ptr[0] > 1) {  // configurationVersion == 1 or 0
            ALOGE("b/23680780");
            return BAD_VALUE;
        }
#else
    ...
    } else if (meta->findData(kKeyPrivateInfo, &type, &data, &size)) {
...
#ifdef RTK_PLATFORM
    else if (meta->findData(kKeyWVC1, &type, &data, &size)) {
        sp<ABuffer> buffer = new ABuffer(size);
        memcpy(buffer->data(), data, size);
        buffer->meta()->setInt32("csd", true);
        buffer->meta()->setInt64("timeUs", 0);
    ...
    OMX_AUDIO_PARAM_ALACTYPE_COOKIE *pALACConfig;
    if (meta->findPointer(kKeyALAC_CAFFConfig, (void **)&pALACConfig)) {
        msg->setPointer("alac-config", pALACConfig);
    }
...
void convertMessageToMetaData(const sp<AMessage> &msg, sp<MetaData> &meta) {
    ...
    if (mime.startsWith("video/")) {
        ...
        /* Fixed android.media.cts.NativeDecoderTest#testMuxer display width is not correct */
        int32_t strideWidth = 0;
        int32_t strideHight = 0;
        if (msg->findInt32("stride", &strideWidth) && msg->findInt32("slice-height", &strideHight)) {
            meta->setInt32(kKeyDisplayWidth, strideWidth);
            meta->setInt32(kKeyDisplayHeight, strideHight);
        }
...
#ifdef RTK_PLATFORM
    float fFps;
    if (msg->findFloat("frame-rate-float", &fFps) && fFps > 0) {
        meta->setFloat(kKeyFrameRateFloat, fFps);
    }

    int32_t isMVC = 0;
    if (msg->findInt32("cMVC", &isMVC) && isMVC > 0) {
        meta->setInt32(kKeyIsMVC, isMVC);
    }
#endif /* end of RTK_PLATFORM */
    int32_t fps;
    float fpsFloat;
...
static const struct mime_conv_t mimeLookup[] = {
    ...
    { MEDIA_MIMETYPE_AUDIO_MLP,         AUDIO_FORMAT_DOLBY_TRUEHD},
    { MEDIA_MIMETYPE_AUDIO_DTS,         AUDIO_FORMAT_DTS},
    { MEDIA_MIMETYPE_AUDIO_DTS_HD,      AUDIO_FORMAT_DTS_HD},
