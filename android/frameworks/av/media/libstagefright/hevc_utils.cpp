...
void parseProfileTierLevel(ABitReader *br, unsigned subLayers) {
    ...
    ALOGI("parseProfileTierLevel general_level_idc %u", general_level_idc);
...
static sp<ABuffer> FindHEVCNALNew(
    ...
    size_t nalSize;
    if(stopOffset == NULL)
        ALOGI("FindHEVCNALNew stopOffset null");
...
sp<MetaData> MakeHEVCMetaData(const sp<ABuffer> &accessUnit) {
    ...
    ALOGD("[HEVC:SPS] MakeHEVCMetaData AU size:%zu, sps size:%zu, pps size:%zu, csd size:%zu",accessUnit->size(),seqParamSet->size(),picParamSet->size(),csd->size());
