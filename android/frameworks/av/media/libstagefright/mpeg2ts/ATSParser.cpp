...
#ifdef RTK_PLATFORM
static const int32_t kMaxHeight = 2208;
static const int32_t kMaxWidth = 4096;
static const int32_t kMinHeight = 16;
static const int32_t kMinWidth = 16;
#endif
...
#ifdef RTK_PLATFORM
    sp<MediaSource> getSource(SourceType type,unsigned mPMT_PID,size_t programSize);
    int64_t getPTS();
    bool getDequeueState();
    void setESSeeking();
    bool isVideoSizeSuitable();
    bool getVideoSeekStatus();
    bool isAVCvideo();
    int64_t getVideoSeekedTime();
    int64_t getBaseTime();
#else
...
#ifdef RTK_PLATFORM
    unsigned mCreateByMPEG2TSExt;
    bool mCheckedFirstFrame;
#endif
...
struct ATSParser::Stream : public RefBase {
    ...
    int64_t getBaseTime();
    int64_t getVideoStartFrameTimestamp();
    void setAudioSkipFrameTimestamp(int64_t timeUs);
    bool isAVC() const ;
#endif
    bool isAudio() const;
    bool isVideo() const;
...
#ifdef RTK_PLATFORM
    bool mTrackUseless;
    bool mIsVideoSizeSuitable;
    int64_t mMaxTimeUs;
    int64_t mFirstPTS;
    bool mUsed;
    int64_t mBaseTime;
    int64_t mVideoFirstFrameTimestamp;
    unsigned mCreateByMPEG2TSExt;
#endif
...
#ifdef RTK_PLATFORM
    bool checkVideoSize(int32_t width,int32_t height);
#endif
...
#ifdef RTK_PLATFORM
    ,mCreateByMPEG2TSExt(creatorType)
    ,mCheckedFirstFrame(false)
#endif
      {
...
bool ATSParser::Program::parsePID(
    ...
    *err = OK;
    ...
    if(mCheckedFirstFrame == false) {
        for (size_t i = 0; i < mStreams.size(); ++i) {
            if(mStreams.editValueAt(i)->isVideo())
            {
                videoStreamCount++;
                int64_t pts = mStreams.editValueAt(i)->getVideoStartFrameTimestamp();
                if(pts != -1)
                {
                    for (size_t j = 0; j < mStreams.size(); ++j)
                    {
                        if(mStreams.editValueAt(j)->isAudio())
                        {
                            if(mCreateByMPEG2TSExt != SOURCE_FROM_LOCAL)
                                pts = 0;
                            mStreams.editValueAt(j)->setAudioSkipFrameTimestamp(pts);
                        }
                    }
                    mCheckedFirstFrame = true;
                }
            }
        }
    }

    if(mCheckedFirstFrame == false && (videoStreamCount == 0 || mCreateByMPEG2TSExt != SOURCE_FROM_LOCAL)) {
        for (size_t j = 0; j < mStreams.size(); ++j)
        {
            if(mStreams.editValueAt(j)->isAudio())
                mStreams.editValueAt(j)->setAudioSkipFrameTimestamp(0);
        }
        mCheckedFirstFrame = true;
    }
#endif
...
void ATSParser::setESSeeking() {
    for (size_t i = 0; i < mPrograms.size(); ++i) {
        mPrograms.editItemAt(i)->setESSeeking();
    }
}


void ATSParser::setCreator(unsigned type) {
    mCreateByMPEG2TSExt = type;
}

int64_t ATSParser::getBaseTime() {
    int64_t maxBaseTime = 0;
    for (size_t i = 0; i < mPrograms.size(); ++i) {
        if(mPrograms.editItemAt(i)->programMapPID() == mPMT_PID){
            int64_t pts = mPrograms.editItemAt(i)->getBaseTime();
            maxBaseTime = pts;
        }
    }
    return maxBaseTime;
...
void ATSParser::eraseOtherPrograms() {
    //Ignore other program for performance
    for (int i = 0; i < (int)mPrograms.size(); ++i) {
        if(mPrograms.editItemAt(i)->programMapPID() != mPMT_PID)
        {
    ...
    ALOGD("%s select PID 0x%x remain %zu programs", __FUNCTION__, mPMT_PID, mPrograms.size());
}

void ATSParser::setEnableTimestamp(bool flag){
    enableTimestamp = flag;
}
#endif
...
#ifndef RTK_PLATFORM //Still try to playback
            return ERROR_MALFORMED;
#endif
        }
    }
...
#ifdef RTK_PLATFORM
sp<MediaSource> ATSParser::Program::getSource(SourceType type,unsigned mPMT_PID ,size_t programSize) {
#else
sp<MediaSource> ATSParser::Program::getSource(SourceType type) {
    ...
    for (size_t i = 0; i < mStreams.size(); ++i) {
        ...
        if (source != NULL
#ifdef RTK_PLATFORM
            && (mStreams.editValueAt(i)->isSourceUsed() == false || mCreateByMPEG2TSExt == false)
#endif
            ) {
#ifdef RTK_PLATFORM
                if(mStreams.editValueAt(i)->isAudio()&&(mPMT_PID!=0||programSize==1))
                    mStreams.editValueAt(i)->setSourceUsed();
#endif
            return source;
        }
    }
...
void ATSParser::Program::setESSeeking() {
    for (size_t i = 0; i < mStreams.size(); ++i) {
        mStreams.editValueAt(i)->setESSeeking();
    }
}

bool ATSParser::Program::isVideoSizeSuitable(){
    bool isHaveVideo = false;
    for (size_t i = 0; i < mStreams.size(); ++i) {
        if(mStreams.editValueAt(i)->isVideo()){
            isHaveVideo = true;
            return mStreams.editValueAt(i)->isVideoSizeSuitable();
        }
    }
    if(!isHaveVideo)
    {
        ALOGV("Program have no video ,in case of play error,set return true");
        return true;
    }
    return false;
}

bool ATSParser::Program::getVideoSeekStatus() {
    for (size_t i = 0; i < mStreams.size(); ++i) {
        if(mStreams.editValueAt(i)->isVideo()){
            return mStreams.editValueAt(i)->getSeekOverStatus();
        }
    }
    return false;
}

int64_t ATSParser::Program::getVideoSeekedTime() {
    for (size_t i = 0; i < mStreams.size(); ++i) {
        if(mStreams.editValueAt(i)->isVideo()){
            return mStreams.editValueAt(i)->getSeekOverTimestamp();
        }
    }
    return -1;
}

bool ATSParser::Program::isAVCvideo() {
    for (size_t i = 0; i < mStreams.size(); ++i) {
        if(mStreams.editValueAt(i)->isVideo()&&mStreams.editValueAt(i)->isAVC()){
            return true;
        }
    }
    return false;
...
int64_t ATSParser::Program::getBaseTime() {
    int64_t maxBaseTime = 0;
    for (size_t i = 0; i < mStreams.size(); ++i) {
        int64_t pts = mStreams.editValueAt(i)->getBaseTime();
        if (maxBaseTime < pts) {
            maxBaseTime = pts;
        }
    }
...
#ifdef RTK_PLATFORM
      ,mTrackUseless(false)
      ,mIsVideoSizeSuitable(true)
      ,mMaxTimeUs(0)
      ,mUsed(false)
      ,mBaseTime(-1)
      ,mVideoFirstFrameTimestamp(-1)
      ,mCreateByMPEG2TSExt(creatorType)
#endif
    {
    ALOGD("Create Stream Type 0x%x ", mStreamType);
    switch (mStreamType) {
        ...
        case STREAMTYPE_H265_CASE2:
        case STREAMTYPE_H265_CASE3:
            mQueue = new ElementaryStreamQueue(
                    ElementaryStreamQueue::H265);
            break;
        case STREAMTYPE_AVS:
            mQueue = new ElementaryStreamQueue(
                    ElementaryStreamQueue::AVS);
            break;
        case STREAMTYPE_VC1:
            mQueue = new ElementaryStreamQueue(
                    ElementaryStreamQueue::WVC1);
            break;
        case STREAMTYPE_MPEG2_AUDIO_DTS:
            mQueue = new ElementaryStreamQueue(
                    ElementaryStreamQueue::DTS);
            break;
        case STREAMTYPE_DTS_HD1:
        case STREAMTYPE_DTS_HD2:
            mQueue = new ElementaryStreamQueue(
                    ElementaryStreamQueue::DTS_HD);
            break;
#endif
        ...
        case STREAMTYPE_AC3:
#ifdef RTK_PLATFORM
        case STREAMTYPE_LPCM_DD_EAC3:
        case STREAMTYPE_LPCM_DD_EAC3_CASE2:
#endif
            mQueue = new ElementaryStreamQueue(
        ...
        case STREAMTYPE_LPCM_AC3:
            mQueue = new ElementaryStreamQueue(
                    ElementaryStreamQueue::TRUEHD);
            break;
        case STREAMTYPE_AAC_LATM:
            mQueue = new ElementaryStreamQueue(
                    ElementaryStreamQueue::AAC_LATM);
            break;
        case STREAMTYPE_PCM:
            mQueue = new ElementaryStreamQueue(
            ...
            break;
#endif
#ifndef RTK_PLATFORM //We got sample stream type 0x83 which is AC3 TrueHD case
        case STREAMTYPE_LPCM_AC3:
            mQueue = new ElementaryStreamQueue(
                    ElementaryStreamQueue::PCM_AUDIO);
            break;
...
status_t ATSParser::Stream::parse(
        ...
        SyncEvent *event) {
    if (mQueue == NULL || mTrackUseless == true) {
        return OK;
    }
    ...
    if (mExpectedContinuityCounter >= 0
        ...
        ALOGI("discontinuity on stream pid 0x%04x    mExpectedContinuityCounter=%d, continuity_counter=%d", mElementaryPID, (unsigned)mExpectedContinuityCounter, continuity_counter);

#if 0 // Just print warning message without reset buffer
        mPayloadStarted = false;
...
#ifdef RTK_PLATFORM
bool ATSParser::Stream::isAVC() const {
    if (mStreamType == STREAMTYPE_H264) {
        return true;
    }
    return false;
...
status_t ATSParser::Stream::parsePES(ABitReader *br, SyncEvent *event) {
    ...
    if (stream_id != 0xbc  // program_stream_map
        ...
        // ES data follows.
#ifdef RTK_PLATFORM
        if (PES_packet_length != 0 && PES_packet_length > (PES_header_data_length + 3)) {
#else
...
void ATSParser::Stream::onPayloadData(
    ...
    if (PTS_DTS_flags == 2 || PTS_DTS_flags == 3) {
        ...
        //ALOGD("PTS %" PRIu64 " timeUs %" PRId64, PTS, timeUs);
    }
#ifdef RTK_PLATFORM
    ...
    if(mBaseTime == -1 && timeUs > mBaseTime)
    {
        mBaseTime = timeUs;
    }
    ...
    //Deal with streamType error, only check 3 sec data at beginning
    if(timeUs < 3000000) {
        const uint8_t *ptr = data;
        unsigned prefix = (ptr[0] << 24) | (ptr[1]<<16) | (ptr[2]<<8) | (ptr[3]);
        //ALOGD("prefix 0x%x ", prefix);
        switch(mStreamType)
        {
            case STREAMTYPE_H265_CASE2:
                if(prefix == 0xff1f00e8 || prefix == 0x1fffe800 || prefix == 0xfe7f0180 || prefix == 0x7ffe8001)
                {
                    mStreamType = STREAMTYPE_MPEG2_AUDIO_DTS;
                    mQueue = new ElementaryStreamQueue(
                    ElementaryStreamQueue::DTS);
                    mQueue->setSkipFrameBeforeTimestamp(0);
                }
                else if((prefix & 0xFFFF0000) == 0x0B770000)
                {
                    mStreamType = STREAMTYPE_AC3;
                    mQueue = new ElementaryStreamQueue(
                    ElementaryStreamQueue::AC3);
                    mQueue->setSkipFrameBeforeTimestamp(0);
                }
    ...
    if(mCreateByMPEG2TSExt != SOURCE_FROM_M3U8 && isAudio() && mQueue->getSeekStatus()){
        if(mProgram->isAVCvideo()){
            if(mProgram->getVideoSeekStatus()){
                int64_t tempTime = mProgram->getVideoSeekedTime();
                if(tempTime!=-1){
                    mQueue->setSkipFrameAfterSeekTimestamp(tempTime);
    ...
    bool found = false;
#ifdef RTK_PLATFORM
    if(isAudio() && mQueue->getSkipFrmaeBeforeTimestamp() == -1) {
        return;
    }
#endif
    while ((accessUnit = mQueue->dequeueAccessUnit()) != NULL) {
        if (mSource == NULL) {
...
#ifndef RTK_PLATFORM//TS file may not contain IDR at beginning
                const char *mime;
...
#endif
#ifdef RTK_PLATFORM
            if(isAVC()&&mIsVideoSizeSuitable){
                sp<MetaData> meta = mQueue->getFormat();
                int32_t videoHeight = 0;
                int32_t videoWidth = 0;
                meta->findInt32(kKeyHeight,&videoHeight);
                meta->findInt32(kKeyWidth,&videoWidth);
                ALOGV("AVC kKeyHeight is :%d  kKeyWidth is %d",videoHeight,videoWidth);
                mIsVideoSizeSuitable =checkVideoSize(videoWidth,videoHeight);
                if(!mIsVideoSizeSuitable){
                    mTrackUseless = true;
                    return;
                }
            }
            ...
            size_t threshold = isVideo()?20971520 /*20MB*/: 11534336 /*11MB*/;
...
#ifdef RTK_PLATFORM
        else if(mTrackUseless == true)
        {
            accessUnit = NULL;
            return;
        }

    if(isVideo() && mVideoFirstFrameTimestamp == -1 && mQueue->getFirstFrameTimestamp() != -1)
    {
        ALOGD("getFirstFrameTimestamp %" PRId64, mQueue->getFirstFrameTimestamp());
        mVideoFirstFrameTimestamp = mQueue->getFirstFrameTimestamp();
    }
#endif
...
#ifdef RTK_PLATFORM
bool ATSParser::Stream::checkVideoSize(int32_t width,int32_t height){
    bool isSuitable = false;
    if(height<=kMaxHeight&&height>=kMinHeight&&width<=kMaxWidth&&width>=kMinWidth){
        isSuitable = true;
    }
    return isSuitable;
}

bool ATSParser::Stream::isVideoSizeSuitable(){
    return mIsVideoSizeSuitable;
}
#endif
...
#ifdef RTK_PLATFORM
bool ATSParser::Stream::isSourceUsed() {
    return mUsed;
}

void ATSParser::Stream::setSourceUsed() {
    mUsed = true;
}
...
void ATSParser::Stream::setESSeeking() {
    if(mQueue == NULL)
        return;
    mQueue->setSeeking();
}

bool ATSParser::Stream::getSeekOverStatus() {
    if(mQueue == NULL)
        return false;
    return mQueue->getSeekOverStatus();
}

int64_t ATSParser::Stream::getSeekOverTimestamp() {
    if(mQueue == NULL)
        return -1;
    return mQueue->getSeekOverTimestamp();
}


int64_t ATSParser::Stream::getBaseTime() {
    return mBaseTime;
}

int64_t ATSParser::Stream::getVideoStartFrameTimestamp() {
    return mVideoFirstFrameTimestamp;
}

void ATSParser::Stream::setAudioSkipFrameTimestamp(int64_t timeUs) {
    if(isAudio())
        mQueue->setSkipFrameBeforeTimestamp(timeUs);
}
...
ATSParser::ATSParser(uint32_t flags)
    : mFlags(flags),
      ...
      ,mPMT_PID(0)
      ,mCreateByMPEG2TSExt(SOURCE_FROM_M3U8)
...
#ifdef RTK_PLATFORM
status_t ATSParser::isVideoSizeSuitable() {
    for (size_t i = 0; i < mPrograms.size(); ++i) {
        if(!mPrograms.editItemAt(i)->isVideoSizeSuitable()){
...
void ATSParser::parseProgramAssociationTable(ABitReader *br) {
    ...
    for (size_t i = 0; i < numProgramBytes / 4; ++i) {
        ...
        if (program_number == 0) {
            ...
            if (!found
#ifdef RTK_PLATFORM
                && (mPMT_PID == 0 || mPMT_PID == programMapPID)
#endif
                ) {
                mPrograms.push(
...
#ifdef RTK_PLATFORM
        sp<MediaSource> source = program->getSource(type,mPMT_PID,mPrograms.size());
#else
...
#ifdef RTK_PLATFORM
        if (source != NULL) {
            if(mPrograms.size() <= 1) {//Case of normal TS file
                mPMT_PID = program->programMapPID();
                return source;
            }
            else if(mPMT_PID == 0 && type == VIDEO) {//Case of TRP file and prefer to find video first
                mPMT_PID = program->programMapPID();
                return source;
            }
            else if(mPMT_PID == program->programMapPID()) {
                return source;
            }
            else
                return NULL;
