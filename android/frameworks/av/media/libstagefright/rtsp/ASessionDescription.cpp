        ...
        switch (line.c_str()[0]) {
            ...
            case 'm':
            {
                ...
                if(line.size() > 15)
                {
                    ...
                    ssize_t spacePos = line.find(" ", 7 + 3);
                    if (spacePos < 0) {
                        return false;
                    }

                    value.setTo(line, 10, spacePos);
                    ALOGI("[%s][%d] value::%s\n",__FUNCTION__,__LINE__,value.c_str());
                }
                mTracks.push(Attribs());

                if(!strncasecmp("MP2T/", value.c_str(), 5))
                {
                    mTracks.editItemAt(mTracks.size() - 1).add("video", AString(line, 2, line.size() - 2).c_str());
                    mTracks.editItemAt(mTracks.size() - 1).add(key, value);
                }
#else
                mTracks.push(Attribs());
...
#ifdef RTK_PLATFORM
        ALOGI("[%s][%d] findAttribute fail flow check \n",__FUNCTION__,__LINE__);
        sprintf(key, "video");
        findAttribute(index, key, desc);
...
#ifdef RTK_PLATFORM
    if(!strncasecmp("MP2T/AVP", desc, 8))
    {
        ALOGI("[%s][%d] MP2T/AVP flow, end::%s",__FUNCTION__,__LINE__,end);
        *timescale = 90000;
        return;
    }
#endif
