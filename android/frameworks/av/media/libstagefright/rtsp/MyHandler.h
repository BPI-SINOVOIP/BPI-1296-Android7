...
struct MyHandler : public AHandler {
    ...
    MyHandler(
        ...
        mNetLooper->start(false /* runOnCallingThread */,
                          false /* canCallJava */,
                          PRIORITY_HIGHEST);
        mLastMediaTimeUs[0] =-1;
        mLastMediaTimeUs[1] =-1;
    ...
    virtual void onMessageReceived(const sp<AMessage> &msg) {
        switch (msg->what()) {
...
#ifdef RTK_PLATFORM
                if(msg->findInt32("reconnect", &reconnect) == false || (msg->findInt32("reconnect", &reconnect) && !reconnect))
                {
                    sp<AMessage> timeoutMsg = new AMessage('exit', this);
                    timeoutMsg->post(2*1E6);
                }
#endif
                break;
            }
            ...
            case 'exit':
            {
                ALOGI("[%s][%d]tear timeout 2 seconds need force TEARDOWN.\n",__FUNCTION__,__LINE__);
                sp<AMessage> reply = new AMessage('disc', this);
                mConn->disconnect(reply);
                break;
    ...
    void parsePlayResponse(const sp<ARTSPResponse> &response) {
        ...
        for (List<AString>::iterator it = streamInfos.begin();
            ...
            {
                trackIndex = 0;
                ALOGD("[%s][%d] val::%s can not match mTracks.editItemAt(trackIndex).mURL.c_str().\n",__FUNCTION__,__LINE__,val.c_str());
                while (trackIndex < mTracks.size())
                {
                    ALOGD("[%s][%d] mUrl ::%s \n",__FUNCTION__,__LINE__,mTracks.editItemAt(trackIndex).mURL.c_str());
                    if(strstr(mTracks.editItemAt(trackIndex).mURL.c_str(), val.c_str()))
                    {
                        break;
                    }
                    ++trackIndex;
...
#ifdef RTK_PLATFORM
    void setVideoIdx(int32_t index)
    {
        if(index >= 0)
            mVideoIdx = index;
    }
    void setAudioIdx(int32_t index)
    {
        if(index >= 0)
            mAudioIdx = index;
    }
...
private:
    ...
    void setupTrack(size_t index) {
        ...
        AString url;
#ifdef RTK_PLATFORM
        if(mSessionDesc->findAttribute(index, "a=control", &url) == false)
        {
            url = mOriginalSessionURL;
...
#ifdef RTK_PLATFORM
        if(!strncasecmp("MP2T/AVP", formatDesc.c_str(), 8))
        {
            ALOGI("[%s][%d] MP2T/AVP flow.",__FUNCTION__,__LINE__);
            IsMP2TFlag = true;
        }
#endif
    ...
    void onTimeUpdate(int32_t trackIndex, uint32_t rtpTime, uint64_t ntpTime) {
        ...
        if (mNTPAnchorUs < 0) {
            ...
            mMediaAnchorUs = mLastMediaTimeUs[trackIndex];
        }

#ifdef RTK_PLATFORM
        ALOGI("[%s][%d] Timestamp   A %f V %f\n",__FUNCTION__,__LINE__,(int64_t)mAudioPTS/1E6,(int64_t)mVideoPTS/1E6);

        ALOGW("[%s][%d] mTryFakeRTCP::%d mReceivedFirstRTCPPacket :%d\n",__FUNCTION__,__LINE__,mTryFakeRTCP,mReceivedFirstRTCPPacket);
        ALOGW("[%s][%d] id:%d mNTPAnchorUs %f ntpTimeUs::%f\n",__FUNCTION__,__LINE__,trackIndex,
                codec_show_pts(track->mNTPAnchorUs),codec_show_pts(ntpTimeUs));

        ALOGW("[%s][%d] mRTPAnchor %f rtpTimeUs::%f\n",__FUNCTION__,__LINE__,
                codec_show_pts(track->mRTPAnchor),codec_show_pts(rtpTime));
#endif
        ...
        if (!mAllTracksHaveTime) {
            ...
            if (allTracksHaveTime) {
                ...
                if(mTracks.size() == 1)
                {
                    ALOGI("[%s][%d] mTracks.size() :: %d drop all accessUnit in cache. \n",__FUNCTION__,__LINE__,(int32_t)mTracks.size());
                    TrackInfo *trackInfo = &mTracks.editItemAt(0);
...
#ifdef RTK_PLATFORM
        if(IsMP2TFlag == true && mAllTracksHaveTime == false && mTracks.size() == 1)
        {
            mAllTracksHaveTime = true;
            ALOGI("[%s][%d] mAllTracksHaveTime set true :: %d\n",__FUNCTION__,__LINE__,mAllTracksHaveTime);
        }
#endif
        if (!mAllTracksHaveTime) {
    ...
    bool addMediaTimestamp(
        ...
        if(IsMP2TFlag == true)
        {
            mediaTimeUs = mLastMediaTimeUs[trackIndex] + 33000ll;
            if (mediaTimeUs > mLastMediaTimeUs[trackIndex])
                mLastMediaTimeUs[trackIndex] = mediaTimeUs;
            accessUnit->meta()->setInt64("timeUs", mediaTimeUs);
            return true;
        }

#define MAX_GAP_PTS 100000000ll
        if(mLastMediaTimeUs[trackIndex]> 0 && diff(mediaTimeUs,mLastMediaTimeUs[trackIndex]) > MAX_GAP_PTS)
        {
#define DEBUG_LOG_FOR_RTSP_RTP_NTP_PTS
#ifdef DEBUG_LOG_FOR_RTSP_RTP_NTP_PTS

            ALOGD("[%s][%d] rtpTime::%f , mRTPAnchor::%f ,timeScale::%d \n",__FUNCTION__,__LINE__,codec_show_pts(rtpTime),codec_show_pts(track->mRTPAnchor),track->mTimeScale);
            ALOGD("[%s][%d] ntpTimeUs::%fs , mNTPAnchorUs::%fs rel:%fs \n",__FUNCTION__,__LINE__,codec_show_pts(ntpTimeUs),codec_show_pts(track->mNTPAnchorUs),codec_show_pts(relRtpTimeUs));
            ALOGD("[%s][%d] mediaTimeUs::%fs , mMediaAnchorUs::%fs \n",__FUNCTION__,__LINE__,codec_show_pts(mediaTimeUs),codec_show_pts(mMediaAnchorUs));

            ALOGD("[%s][%d] mediaTimeUs :: %f = %f + %f -%f \n",__FUNCTION__,__LINE__,codec_show_pts(mediaTimeUs),
                    codec_show_pts(mMediaAnchorUs),
                    codec_show_pts(ntpTimeUs),
                    codec_show_pts(mNTPAnchorUs)
                 );
#endif
            int64_t TimTick = 30000ll;
            if( mLastMediaTimeUs[trackIndex] <=0)
                TimTick =0;

            mNTPAnchorUs = mNTPAnchorUs + (mediaTimeUs - mLastMediaTimeUs[trackIndex]) - TimTick;
            ALOGW("[%s][%d] Rebase mNTPAnchorUs in PTS discontinuity.from :%f to %f .\n",__FUNCTION__,__LINE__,
                    codec_show_pts(mediaTimeUs), codec_show_pts(mLastMediaTimeUs[trackIndex]) + TimTick);
            mediaTimeUs = mMediaAnchorUs + ntpTimeUs - mNTPAnchorUs;
        }
#endif
        if (mediaTimeUs > mLastMediaTimeUs[trackIndex]) {
            mLastMediaTimeUs[trackIndex] = mediaTimeUs;
        ...
        if (mediaTimeUs < 0) {
#define NO_DROP_EARLY_ACCESSUNIT
#ifdef NO_DROP_EARLY_ACCESSUNIT
            if(mAudioPTS < 0 && mVideoPTS < 0 && diff(mediaTimeUs,0) > MAX_GAP_PTS)
            {
                ALOGD("[%s][%d] rebase mediaTimeUs :: %f = %f + %f -%f to 0\n",__FUNCTION__,__LINE__,codec_show_pts(mediaTimeUs),
                        codec_show_pts(mMediaAnchorUs),
                        codec_show_pts(ntpTimeUs),
                        codec_show_pts(mNTPAnchorUs)
                     );
                mNTPAnchorUs = mNTPAnchorUs + mediaTimeUs;
                mediaTimeUs = 0;
                ALOGW("[%s][%d] Keep early accessUnit\n",__FUNCTION__,__LINE__);
            }
            else
#endif
            {
...
#ifdef RTK_PLATFORM
        if(trackIndex == mAudioIdx)
            mAudioPTS = mediaTimeUs;
        if(trackIndex == mVideoIdx)
            mVideoPTS = mediaTimeUs;
#endif
