...
#if 1
    meta->setInt64(kKeyInitialReadTime, mInitialReadTimeUs);
#endif
...
status_t AudioSource::read(
    ...
    *out = NULL;
    ...
    buffer->add_ref();
#if 0
    static Statistics *st = new Statistics("aoutbps", 5);
...
status_t AudioSource::dataCallback(const AudioRecord::Buffer& audioBuffer) {
    ...
    if (mNumFramesReceived == 0 && mPrevSampleTimeUs == 0) {
        ...
        ALOGD("mStartTimeUs:%f mInitialReadTimeUs:%f", mStartTimeUs/1000000.0, mInitialReadTimeUs/1000000.0);
    ...
    //When got the first packet, to record the offset time. If "read fail", to set the offset time to 10 sec.
    if (mInputSource == AUDIO_SOURCE_HDMIRX && mFlagFirst == 0 && mNumFramesReceived > 0) {
        int64_t getACPUTimeUs = (mRecord->getInputTimeStamp() / 1000ll);
        mFlagFirst = 1;
        if (getACPUTimeUs > 0 && numLostBytes != 0) {
            mHDMIRXtimeoffset = numLostBytes * 1000000 / (mRecord->frameSize() * mSampleRate);
        } else if (getACPUTimeUs == 0) {
            mHDMIRXtimeoffset = 10000000;
        }

        if (mHDMIRXtimeoffset < 300000)    //skip the offset less than 0.3 sec. [the init time is about 0.24sec]
            mHDMIRXtimeoffset = 0;
        ALOGD("mHDMIRXtimeoffset %" PRId64 "", mHDMIRXtimeoffset);
    }

    //After got first "real audio data"(getACPUTimeUs != 0), do NOT fill the lost data.
    if (mInputSource == AUDIO_SOURCE_HDMIRX && mFlagCPU == 1) {
        numLostBytes = 0;
    }
...
#if 0
    static Statistics *st = new Statistics("ainbps", 5);
    st->rate(audioBuffer.size * 8);
#endif
...
//rtk begin
/**
 *      to get the audio timestamp of HDMI RX from ALSA.  
 */
    if (mInputSource == AUDIO_SOURCE_HDMIRX && mFlagLost != 1) {
        int64_t getACPUTimeUs = (mRecord->getInputTimeStamp() / 1000ll);
        if (getACPUTimeUs > 0) {
            mFlagCPU = 1;
            timestampUs = getACPUTimeUs - mInitialReadTimeUs + mHDMIRXtimeoffset;
        }
    }
// */
//rtk end
    ALOGV("prev:%f time:%f - init:%f = drift:%f", mPrevSampleTimeUs/1000000.0, timeUs/1000000.0, mInitialReadTimeUs/1000000.0, (timeUs - mInitialReadTimeUs)/1000000.0);
    ALOGV("time:%f - prev:%f = %f", timeUs/1000000.0, mPrevSampleTimeUs/1000000.0, (timeUs - mPrevSampleTimeUs)/1000000.0);
    ALOGV("qlen:%d", (int)mBuffersReceived.size());
