...
#define WORKAROUND_DHCKYLIN_2665    /* for DHCKYLIN-2665, interlaced file have green line */
...
status_t StagefrightMetadataRetriever::setDataSource(
    ...
    if (property_get("persist.rtk.ctstest", value, NULL) && (!strcmp(value, "1") || !strcasecmp(value, "true")))
        ...
        mSource->RTK_EXT = 0; //0x2379; //Android N will remove RtkExtractor
...
#ifdef WORKAROUND_DHCKYLIN_2665
    bool NeedMoreOne = false;
#endif /* end of WORKAROUND_DHCKYLIN_2665 */
...
#ifdef WORKAROUND_DHCKYLIN_2665
                    NeedMoreOne = TRUE;
                    ALOGV("\033[0;36m == for interlace thumbnail issue ==\033[m");
#else /* else of WORKAROUND_DHCKYLIN_2665 */
                    haveMoreInputs = false;
                    flags |= MediaCodec::BUFFER_FLAG_EOS;
#endif /* end of WORKAROUND_DHCKYLIN_2665 */
                }
...
void StagefrightMetadataRetriever::parseMetaData() {
    ...
    char tmp[32];
    snprintf(tmp, sizeof(tmp), "%zu", numTracks);
    ...
    if (meta->findFloat(kKeyCaptureFramerate, &captureFps)) {
        if(captureFps > 0) {
            snprintf(tmp, sizeof(tmp), "%f", captureFps);
    ...
    snprintf(tmp, sizeof(tmp), "%" PRId64, (maxDurationUs + 500) / 1000);
    ...
    if (hasVideo) {
        ...
        snprintf(tmp, sizeof(tmp), "%d", videoWidth);
        mMetaData.add(METADATA_KEY_VIDEO_WIDTH, String8(tmp));

        snprintf(tmp, sizeof(tmp), "%d", videoHeight);
        mMetaData.add(METADATA_KEY_VIDEO_HEIGHT, String8(tmp));

        snprintf(tmp, sizeof(tmp), "%d", rotationAngle);
        mMetaData.add(METADATA_KEY_VIDEO_ROTATION, String8(tmp));
    }

    if (numTracks == 1 && hasAudio && audioBitrate >= 0) {
        snprintf(tmp, sizeof(tmp), "%d", audioBitrate);
        ...
        if (mSource != NULL && mSource->getSize(&sourceSize) == OK) {
            ...
            snprintf(tmp, sizeof(tmp), "%" PRId64, avgBitRate);
            mMetaData.add(METADATA_KEY_BITRATE, String8(tmp));
        }
    }
