...
#ifdef RTK_PLATFORM
      MaxsamplesPerChunkSize(0),
      MLAWcompositionTime(0),
      MLAWSampleSize(1),
#endif
...
#ifdef RTK_PLATFORM
        if (mSampleToChunkEntries[i].samplesPerChunk > MaxsamplesPerChunkSize)
            MaxsamplesPerChunkSize = mSampleToChunkEntries[i].samplesPerChunk;
#endif
    }
...
#ifdef RTK_PLATFORM
uint32_t SampleTable::stsscount() const {
...
#ifdef RTK_PLATFORM
status_t SampleTable::getMaxsamplesPerChunkSize(size_t *max_size) {
    *max_size = MaxsamplesPerChunkSize;
    return OK;
}
status_t SampleTable::get_stszInfo(uint32_t *sample_size, uint32_t *sample_count) {
    *sample_size = mDefaultSampleSize;
    *sample_count = mNumSampleSizes;
    return OK;
}
status_t SampleTable::setAudioSampleSize(uint32_t sample_size) {
    MLAWSampleSize = sample_size;
    return OK;
}
#endif
            ...
            if (sampleIndex < mNumSampleSizes) {
                ...
                if ((compTimeDelta < 0 && sampleTime <
                    ...
                    ALOGE("%llu + %d would overflow, clamping",
                            (long long)sampleTime, compTimeDelta);
#else
...
#ifdef RTK_PLATFORM
    if (mNumSyncSamples <=1) {
        ALOGD("[%s][%d] mNumSyncSamples::%d,set start_sample_index to sample_index\n",__FUNCTION__,__LINE__,mNumSyncSamples);
        *sample_index = start_sample_index;
        return OK;
    }
#else
...
#ifdef RTK_PLATFORM
status_t SampleTable::findChunkSyncSampleNear(uint32_t start_sample_index, uint32_t *sample_index) {
    Mutex::Autolock autoLock(mLock);

    *sample_index = 0;

    if (start_sample_index >= mNumSampleSizes)
    {
        return ERROR_END_OF_STREAM;
    }

    uint32_t iStscIdx = 0, chunkRelativeSampleIndex = 0;

#if 0 // before seekTo debug message
    iStscIdx = mSampleIterator->getChunkIndex();
    off64_t sampleoffset  = mSampleIterator->getSampleOffset();
    size_t samplesize  = mSampleIterator->getSampleSize();
    uint32_t sampletime  = mSampleIterator->getSampleTime();

    ALOGD("\033[0;36m [%d] before seekTo  ===>  CurrIdx= %d, sampleoffset= %lld, samplesize= %d, sampletime= %d\033[m", __LINE__, iStscIdx, (long long)sampleoffset, (uint32_t)samplesize, sampletime);
#endif

    status_t err = OK;
    if ((err = mSampleIterator->seekTo(start_sample_index)) != OK)
    {
        return err;
    }
    ...
    for (uint32_t i = 0; i < mNumSampleToChunkOffsets; ++i)
    {
        if ((i == mNumSampleToChunkOffsets -1) && (iStscIdx >= mSampleToChunkEntries[i].startChunk))
        {
            for (uint32_t j = 0; j < i; j++)
                chunkRelativeSampleIndex += (mSampleToChunkEntries[j+1].startChunk - mSampleToChunkEntries[j].startChunk) * mSampleToChunkEntries[j].samplesPerChunk;
            isFound = true;
            break;
        }
        else
        {
            if ((iStscIdx >= mSampleToChunkEntries[i].startChunk) && (iStscIdx < mSampleToChunkEntries[i+1].startChunk))
            {
                ALOGD("\033[0;33m [%d] CurrIdx= %d, startChunk[%d]= %d, startChunk[%d]= %d\033[m", __LINE__, iStscIdx, i, mSampleToChunkEntries[i].startChunk, i+1, mSampleToChunkEntries[i+1].startChunk);
                for (uint32_t j = 0; j < (i+1); j++)
                {
                    if (j == i)
                    {
                        ALOGD("\033[0;33m [%d] chunkRelativeSampleIndex= %d\033[m", __LINE__, chunkRelativeSampleIndex);
                        chunkRelativeSampleIndex += (iStscIdx - mSampleToChunkEntries[j].startChunk) * mSampleToChunkEntries[j].samplesPerChunk;
                        isFound = true;
                        break;
                    }
                    else
    ...
    *sample_index = chunkRelativeSampleIndex;
    ALOGD("\033[0;33m [%d] CurrIdx= %d, sampleoffset= %lld, samplesize= %d, sampletime= %d\033[m", __LINE__, iStscIdx, (long long)sampleoffset, (uint32_t)samplesize, sampletime);
...
#ifdef RTK_PLATFORM
status_t SampleTable::getMetaDataForMLAWSample(
        uint32_t chunkIndex,
        off64_t *offset,
        size_t *size,
        uint64_t *compositionTime,
    ...
    status_t err;
    off64_t CurrentChunkOffset = 0;

    if (chunkIndex >= mNumChunkOffsets) {
        return ERROR_END_OF_STREAM;
    }

    if (offset) {
        if ((err = mSampleIterator->getChunkOffset(chunkIndex, &CurrentChunkOffset)) != OK) {
    ...
    if (size) {
        for (uint32_t i = 0; i < mNumSampleToChunkOffsets - 1; ++i) {
            if (chunkIndex >= mSampleToChunkEntries[i].startChunk && chunkIndex < mSampleToChunkEntries[i+1].startChunk)
            {
                *size = mSampleToChunkEntries[i].samplesPerChunk;
                break;
            }
        }
        MLAWcompositionTime += *size;
    }

    if (compositionTime) {
        *compositionTime = MLAWcompositionTime;
    }
    ...
    if (sampleDuration) {
        //*sampleDuration = mSampleIterator->getSampleDuration();
        *sampleDuration = (uint32_t)*size;
    }

    if (size && (MLAWSampleSize != 1))
        *size *= MLAWSampleSize;

    return OK;
}

status_t SampleTable::getSOWTsamplesPerChunkSize(uint32_t chunkIndex, size_t *size) {
    Mutex::Autolock autoLock(mLock);
    ...
    if (size) {
        if (chunkIndex == mSampleToChunkEntries[mNumSampleToChunkOffsets - 1].startChunk)
            *size = mSampleToChunkEntries[mNumSampleToChunkOffsets - 1].samplesPerChunk;
        else
        {
            for (uint32_t i = 0; i < mNumSampleToChunkOffsets - 1; ++i)
            {
