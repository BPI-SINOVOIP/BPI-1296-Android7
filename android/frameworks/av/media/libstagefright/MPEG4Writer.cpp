...
status_t MPEG4Writer::Track::makeAVCCodecSpecificData(
    ...
    if (parseAVCCodecSpecificData(data, size) != OK) {
        ALOGE("parseAVCCodecSpecificData not OK");
        return ERROR_MALFORMED;
    }
...
status_t MPEG4Writer::Track::threadEntry() {
    ...
    while (!mDone && (err = mSource->read(&buffer)) == OK) {
        //ALOGD("read %s buffer:%p", mIsAudio ? "audio" : "video", buffer);
...
void MPEG4Writer::Track::writeTkhdBox(uint32_t now) {
    ...
    if (mIsAudio) {
        ...
        success = success && mMeta->findInt32(kKeyHeight, &height);
        CHECK(success);
        mMeta->findInt32(kKeyDisplayWidth, &stride);

        /* Fixed android.media.cts.NativeDecoderTest#testMuxer display width is not correct */
        if (stride > 0)
            mOwner->writeInt32(stride << 16);  // 32-bit fixed-point value
        else
