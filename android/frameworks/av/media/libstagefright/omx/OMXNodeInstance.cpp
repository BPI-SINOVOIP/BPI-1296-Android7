...
struct BufferMeta {
    ...
    BufferMeta(size_t size, OMX_U32 portIndex, bool is_SideBand = false)
        : mSize(size),
          mIsRedeceOmxMode(is_SideBand),
          mCopyFromOmx(false),
          mCopyToOmx(false),
          mPortIndex(portIndex),
          mBackup(NULL) {
...
status_t OMXNodeInstance::getParameter(
    ...
    // For Realtek DRM Plugin
    if (index == OMX_IndexParamVideoInputPhysical)
    {
        OMX_BUFFERHEADERTYPE *checkHeader = NULL;
        OMX_PLUS_EXTRADATATYPE *inputHeader = (OMX_PLUS_EXTRADATATYPE *)params;
        checkHeader = findBufferHeader(inputHeader->nBufferId, inputHeader->nPortIndex);
        if (checkHeader != NULL )
        {
            RtkOMX *rtkOmx = (RtkOMX *) checkHeader->pPlatformPrivate;

            if (rtkOmx != NULL)
            {
                inputHeader->nAddress = (OMX_U64)rtkOmx->bus_address;  // put physical Address
                inputHeader->shareFd = rtkOmx->shareFd;
                ALOGV("\033[0;32m #### In %s:%d Get OMX_PLUS_EXTRADATATYPE, nPortIndex:%x, ionFd:%d, shareFd:%d nAddress:%llx ####\033[m", __FUNCTION__, __LINE__, inputHeader->nPortIndex, rtkOmx->ionFd, inputHeader->shareFd, inputHeader->nAddress);
            }
            else
            {
                inputHeader->nAddress = 0;
                inputHeader->shareFd = 0;
            }
        }
    }
...
status_t OMXNodeInstance::updateNativeHandleInMeta(
    ...
    if (header == NULL) {
        ALOGE("b/25884056 in %s:%d buffer:%x nativeHandle:%p", __FUNCTION__, __LINE__, buffer, nativeHandle->handle());
        return BAD_VALUE;
    }
...
status_t OMXNodeInstance::allocateSecureBuffer(
    ...
    BufferMeta *buffer_meta = new BufferMeta(size, portIndex, mIsRedeceOmxMode);
...
status_t OMXNodeInstance::emptyBuffer(
    ...
    if (header == NULL) {
        ALOGE("b/25884056 in %s:%d ", __FUNCTION__, __LINE__);
        return BAD_VALUE;
    }
    BufferMeta *buffer_meta =
...
//[ RTK begin ]
status_t OMXNodeInstance::setBSRingBuffer(
        OMX_INDEXTYPE index, const void *params, size_t /*size*/) {
    Mutex::Autolock autoLock(mLock);

    OMX_ERRORTYPE err = OMX_GetExtensionIndex(
            mHandle,
            const_cast<OMX_STRING>("OMX.RTK.index.SetBSRingMode"),
            &index);
    if (err == OMX_ErrorNone)
    {
          err = OMX_SetConfig(
...
status_t OMXNodeInstance::getIonFd(OMX_INDEXTYPE index, void *params, size_t /* size */, OMX_S32 *shareFd)
{
    Mutex::Autolock autoLock(mLock);
    ...
    *shareFd = 0;
    if (err != OMX_ErrorNone)
    {
        ALOGD("\033[0;32m $$$$ In %s:%d failed #$$$$4\033[m", __FUNCTION__, __LINE__);
        return err;
    }
    ...
    if (checkHeader != NULL )
    {
        ...
        if (rtkOmx != NULL)
        {
            inputHeader->nAddress = (OMX_U64)rtkOmx->bus_address;  // put physical Address
            inputHeader->shareFd = rtkOmx->shareFd;
            if (rtkOmx->shareFd > 0 && inputHeader->enableCrypto > 0)
                *shareFd = dup(rtkOmx->shareFd);
            ALOGD("\033[0;32m #### In %s:%d Get OMX_PLUS_EXTRADATATYPE, nPortIndex:%d nBufferId:%x, ionFd:%d, shareFd:%d nAddress:%llx newFd:%d enableCrypto:%d ####\033[m", __FUNCTION__, __LINE__, inputHeader->nPortIndex, inputHeader->nBufferId, rtkOmx->ionFd, inputHeader->shareFd, inputHeader->nAddress, *shareFd, inputHeader->enableCrypto);
        }
        else
        {
            inputHeader->nAddress = 0;
...
void OMXNodeInstance::setReduceOMXMode(bool enableReduceOmx)
{
    mIsRedeceOmxMode = enableReduceOmx;
}
