...
void OMX::binderDied(const wp<IBinder> &the_late_who) {
    ...
    {
        Mutex::Autolock autoLock(mLiveLock);
...
void OMX::setReduceOMXMode(node_id node, bool enableReduceOmx) {
    ALOGV("\033[0;32 @@@ In %s:%d enableReduceOmx:%d #### \033[m", __FUNCTION__, __LINE__, enableReduceOmx);
    return findInstance(node)->setReduceOMXMode(enableReduceOmx);
}

bool OMX::isReduceOMXMode(node_id node) {
    OMXNodeInstance *instance = findInstance(node);
    ALOGV("\033[0;32 @@@ In %s:%d isReduceOMXMode:%d #### \033[m", __FUNCTION__, __LINE__, (instance == NULL ? false : instance->isReduceOMXMode()));
    return (instance == NULL ? false : instance->isReduceOMXMode());
}
...
//[ RTK begin ]
status_t OMX::setBSRingBuffer(
        node_id node, OMX_INDEXTYPE index,
        const void *params, size_t size) {
    return findInstance(node)->setBSRingBuffer(
            index, params, size);
}

status_t OMX::getIonFd(node_id node, OMX_INDEXTYPE index, void *params, size_t size, OMX_S32 *shareFd) {
    return findInstance(node)->getIonFd(index, params, size, shareFd);
}
//[ RTK end ]
