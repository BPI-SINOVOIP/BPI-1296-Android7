...
#include <ion/ion.h>
#ifdef USE_HACK_SIDEBAND
#include <gui/SidebandClient.h>
#include <gui/SidebandControl.h>
#include <utils/NativeHandle.h>
#endif

#define ANDROID_N_DRM_SUPPORT
//#define DEBUG_N_DRM_SUPPORT

namespace android {

enum {
    kMaxIndicesToCheck = 64, //32, // used when enumerating supported formats and profiles
};
...
// For Realtek Camera
typedef struct RtkOMX { // reference osal/port.h
...
ACodec::ACodec()
    : mQuirks(0),
      ...
      mFirstAllocBuffer(true),
      mCameraSource(false),
      mSideBandDst(false),
#ifdef ENABLE_CTS_WORKAROUND_ATM_457
      mCtsFlags(false),
#endif /* end of ENABLE_CTS_WORKAROUND_ATM_457 */
        ...
        if (err == OK) {
...
#ifdef DEBUG_N_DRM_SUPPORT
            OMX_S32 shareFd = 0;
            int32_t ionFd = 0;
            unsigned char *mapDstStr = NULL;
            unsigned long physAddr = 0;
            uint32_t physSize = 0;
            ion_user_handle_t dstIonHandle = 0;
            int32_t mapDstFd = 0;
            int32_t ret = 0;
#endif /* end of DEBUG_N_DRM_SUPPORT */
            ...
            for (OMX_U32 i = 0; i < def.nBufferCountActual && err == OK; ++i) {
                ...
                if (portIndex == kPortIndexInput && (mFlags & kFlagIsSecure)) {
                    ...
                    {
                        /* TODO: using version and size to check overflow */ 
                        ALOGV("\033[0;32m --- in %s:%d sharedFd:%d size:%d bus_address:0x%.8x---\033[m", __FUNCTION__, __LINE__, native_handle_ptr->data[0], native_handle_ptr->data[1], native_handle_ptr->data[2]);
                        if (native_handle_ptr->data[2] != 0)
                        {
                            info.mData->meta()->setInt64("phys", (OMX_U64)native_handle_ptr->data[2]);
                            info.mPhyAddr = (OMX_U64)native_handle_ptr->data[2];
                        }

#ifdef DEBUG_N_DRM_SUPPORT
                        if (native_handle_ptr->data[0] != 0)
                            shareFd = native_handle_ptr->data[0];
                        if (shareFd > 0 && (!mCrypto))
                        {
                            if (!ionFd)
                                ionFd = open("/dev/ion", O_RDONLY);
                            ret = ion_import(ionFd, shareFd, &dstIonHandle);
                            if (ret < 0)
                                ALOGD("\033[0;32m 0------- %s:%d what ion_import failed ret:%x:%s \033[m", __FUNCTION__, __LINE__, ret, strerror(errno));
                            if (dstIonHandle > 0)
                            {
                                ret = ion_phys(ionFd, dstIonHandle, &physAddr, &physSize);
                                if (ret < 0)
                                    ALOGD("\033[0;34m ==== In %s:%d do ion_phys failed :%d - %s FROM_DESTINATION ==== \033[m", __FUNCTION__, __LINE__, ret, strerror(errno));
                                else
                                {
                                    ret = ion_map(ionFd, dstIonHandle, physSize, PROT_READ | PROT_WRITE, MAP_SHARED, 0, &mapDstStr, &mapDstFd);
                                    if (ret < 0)
                                        ALOGD("\033[0;34m ==== In %s:%d do ion_map failed :%d - %s ==== \033[m", __FUNCTION__, __LINE__, ret, strerror(errno));
                                    else
                                    {
                                        ALOGD("\033[0;32m ### In %s:%d phy:%lx virtual:%p size:%d ###\033[m", __FUNCTION__, __LINE__, physAddr, mapDstStr, (int)physSize);
                                        ptr = mapDstStr;
                                    }
                                    mapDstStr = NULL;
                                    if (mapDstFd > 0)
                    ...
                    {
                        OMX_PLUS_EXTRADATATYPE def;
                        InitOMXParams(&def);
                        def.nPortIndex = portIndex;
                        def.nBufferId = info.mBufferID;
                        status_t err = mOMX->getParameter(mNode, OMX_IndexParamVideoInputPhysical, &def, sizeof(def));
                        if (err == OK)
                        {
                            ALOGV("\033[0;32m --- in %s:%d native_handle is NULL get nAddress:%llx set mPhyAddr---\033[m", __FUNCTION__, __LINE__, def.nAddress);
                            info.mPhyAddr = def.nAddress;
                        }
                        else
                            ALOGD("\033[0;32m --- in %s:%d native_handle is NULL can not get Address---\033[m", __FUNCTION__, __LINE__);
                    }

#endif /* end of ANDROID_N_DRM_SUPPORT */
                ...
                if (mem != NULL) {
                    ...
                    if (mConverter[portIndex] != NULL) {
                        ...
                        if (mCameraSource)
                        {
                            ...
                            if (err == OK)
                            {
                                ALOGD("\033[0;32m --- in %s:%d get nAddress:%llx set mPhyAddr---\033[m", __FUNCTION__, __LINE__, def.nAddress);
                                info.mPhyAddr = def.nAddress;
                            }
                            sp<AMessage> myMeta = info.mData->meta();
                            myMeta->setInt32("RealPhy", info.mPhyAddr);
                        }
                    }
                }
...
status_t ACodec::setupNativeWindowSizeFormatAndUsage(
    ...
    /* Realtek reduce first time alloc memory for multi-instance */
    if (mFirstAllocBuffer)
    {
        def.format.video.nFrameWidth = 32;
        def.format.video.nFrameHeight = 32;
        def.nBufferSize = def.format.video.nFrameWidth * def.format.video.nFrameHeight * 3 / 2 ;
        ALOGV("\033[0;33m ### In %s:%d WxH: %ux%u fmt:%u size:%d first:%d\033[m", __FUNCTION__, __LINE__, def.format.video.nFrameWidth, def.format.video.nFrameHeight, def.format.video.eColorFormat, def.nBufferSize, mFirstAllocBuffer);
    }
    else
    {
        /* Keep decode information */

        //add for 2 alignment
        def.format.video.nFrameHeight = ((def.format.video.nFrameHeight + 1)& ~1);
        def.format.video.nFrameWidth = ((def.format.video.nFrameWidth + 1)& ~1);
    }
...
status_t ACodec::configureOutputBuffersFromNativeWindow(
    ...
    *minUndequeuedBuffers = 0;
    ...
    for (OMX_U32 extraBuffers = 1/*2 + 1*/; /* condition inside loop */; extraBuffers--) {
...
status_t ACodec::allocateOutputBuffersFromNativeWindow() {
    ...
    status_t err = configureOutputBuffersFromNativeWindow(
            ...
            &bufferCount, &bufferSize, &minUndequeuedBuffers, mLegacyAdaptiveExperiment);
    if (err != 0)
        return err;
...
status_t ACodec::allocateOutputMetadataBuffers() {
    ...
    for (OMX_U32 i = 0; i < bufferCount; i++) {
...
#ifdef CHANGE_ALLOC_BUFFER_LOCATION
        CHECK(mNativeWindow.get() != NULL);
        ANativeWindowBuffer *buf = NULL;
        // Dequeue buffers and send them to OMX
        err = native_window_dequeue_buffer_and_wait(mNativeWindow.get(), &buf);
        if (err != 0) {
            ALOGE("\033[0;35m In %s:%d dequeueBuffer failed: %s (%d) \033[m", __FUNCTION__, __LINE__, strerror(-err), -err);
            break;
        }
        info.mGraphicBuffer = new GraphicBuffer(buf, false);
        ALOGV("\033[0;32m Checkpoint In %s:%d GraphicBuffer :%p----\033[m", __FUNCTION__, __LINE__, info.mGraphicBuffer->handle);
        info.mStatus = BufferInfo::OWNED_BY_US;
#endif /* end of CHANGE_ALLOC_BUFFER_LOCATION */
        ...
        mOMX->updateGraphicBufferInMeta(
                mNode, kPortIndexOutput, info.mGraphicBuffer,
                info.mBufferID);
...
#if defined(__LP64__)
        ALOGV("replaced oldest buffer #%u with age %u (%u/%p stored in %p)",
                i,
                mDequeueCounter - info.mDequeuedAt,
                metaData->pHandle,
                info.mGraphicBuffer->handle, info.mData->base());
#else
...
#endif
        {
            //hw_module_t const * videoModule;
            const struct hw_module_t *psModule;
            mali_gralloc_private_module_t *psGrallocModule;
            status_t err = hw_get_module(GRALLOC_HARDWARE_MODULE_ID, &psModule);
            ALOGE_IF(err, "FATAL: can't find the %s module", GRALLOC_HARDWARE_MODULE_ID);
            if (!err)
            {
                psGrallocModule = (mali_gralloc_private_module_t *) psModule;
                uint32_t phys_info[2];
                err = psGrallocModule->GetPhyAddrs((struct private_module_t const *) psGrallocModule, info.mGraphicBuffer->handle, phys_info);
                if (!err)
                {
                    ALOGV("\033[0;34m #### In %s:%d index:%u :Got handle_t [0x%p] PhysicalAddr [0x%08x] size [%d] buf->handle:%p## \033[m", __FUNCTION__, __LINE__, i, info.mGraphicBuffer->handle, phys_info[0], phys_info[1], buf->handle);
                    info.mPhyAddr = phys_info[0];
                }
            }
        }
...
#ifdef CHANGE_ALLOC_BUFFER_LOCATION
    OMX_U32 totalAllocBufferCount = bufferCount;
    OMX_U32 ii = 0;
    if (err != 0)
        totalAllocBufferCount = mBuffers[kPortIndexOutput].size();
    /* We return all buffer in here */
    for (ii = 0; ii < totalAllocBufferCount; ii++) {
        BufferInfo *cancelInfo = &mBuffers[kPortIndexOutput].editItemAt(ii);
        cancelBufferToNativeWindow(cancelInfo);
    }
    ALOGD("--- In %s:%d send back buffer count : %u totalAllocBufferCount:%u ----", __FUNCTION__, __LINE__, ii, totalAllocBufferCount);

    if (err == OK)
    {
        OMX_INDEXTYPE index;
        err = mOMX->getExtensionIndex(mNode, "OMX.realtek.android.index.notifyBufferAllocDone", &index);
        if (err == OK)
        {
            OMX_U32 notify = index;
            index = (OMX_INDEXTYPE)OMX_IndexParamConsumerUsageBits;
            err = mOMX->setParameter(mNode, index, &notify, sizeof(notify));
            ALOGD("In %s:%d notifyBufferAllocDone index :0x%.8x and send notify done err:%x by using OMX_IndexParamConsumerUsageBits ", __FUNCTION__, __LINE__, index, err);
        }
        else
            ALOGW("\033[0;32m Decoder don't understand this command : OMX.realtek.android.index.notifyBufferAllocDone using OMX_IndexParamConsumerUsageBits,\033[m");
    }
#endif /* end of CHANGE_ALLOC_BUFFER_LOCATION */

    ALOGV("\033[0;35m In %s:%d bufferCount :%d minUndequeuedBuffers:%d mMetadataBuffersToSubmit:%d -----\033[m", __FUNCTION__, __LINE__, bufferCount, minUndequeuedBuffers, mMetadataBuffersToSubmit);
...
ACodec::BufferInfo *ACodec::dequeueBufferFromNativeWindow() {
...

    {
        ...
        if (!err)
        {
            psGrallocModule = (mali_gralloc_private_module_t *) psModule;
            uint32_t phys_info[2];
            err = psGrallocModule->GetPhyAddrs((struct private_module_t const *) psGrallocModule, oldest->mGraphicBuffer->handle, phys_info);
            if (!err)
            {
                ALOGD("\033[0;34m @@ Got handle_t [0x%p] PhysicalAddr [0x%08x] size [%d] buf->handle:%p## \033[m", oldest->mGraphicBuffer->handle, phys_info[0], phys_info[1], buf->handle);
                oldest->mPhyAddr = phys_info[0];
            }
        }
    }
...
const char *ACodec::getComponentRole(
    ...
    static const MimeToRole kMimeToRole[] = {
        ...
        { MEDIA_MIMETYPE_VIDEO_RV30,
            "video_decoder.rv30", "video_encoder.rv30" },
        { MEDIA_MIMETYPE_VIDEO_RV40,
            "video_decoder.rv40", "video_encoder.rv40" },
        ...
        { MEDIA_MIMETYPE_AUDIO_DTS_HD,
            "audio_decoder.dts-hd", NULL },
        { MEDIA_MIMETYPE_AUDIO_MLP,
            "audio_decoder.mlp", NULL },
        { MEDIA_MIMETYPE_AUDIO_WMA,
            "audio_decoder.wma", NULL },
        { MEDIA_MIMETYPE_AUDIO_WMAPRO,
            "audio_decoder.wmapro" , NULL },
        { MEDIA_MIMETYPE_AUDIO_ADPCM,
            "audio_decoder.adpcm" , NULL },
        { MEDIA_MIMETYPE_AUDIO_PCM,
            "audio_decoder.x-pcm" , NULL },
        { MEDIA_MIMETYPE_VIDEO_RAW,
            "video_decoder.raw" , "video_encoder.raw" },
...
status_t ACodec::configureCodec(
    ...
    int32_t hasCrypto = 0;
    msg->findInt32("mCrypto", &hasCrypto);
    if (hasCrypto > 0)
        mCrypto = true;
    else
    {
        ...
        {
            uint32_t newFlags = 0;
            newFlags |= (kFlagIsSecure | kFlagIsGrallocUsageProtected | kFlagPushBlankBuffersToNativeWindowOnShutdown);
            ALOGV("\033[0;32m === In %s:%d no Crypto original mFlags :%x newFlags:%x =====\033[m", __FUNCTION__, __LINE__, mFlags, mFlags);
            mFlags &= ~newFlags;
            ALOGV("\033[0;35m ######## In %s:%d Using normal playback mode mFlags:%x ############ \033[m", __FUNCTION__, __LINE__, mFlags);
        }
    }
    ...
    if (video && !encoder) {
        ...
        // For Realtek DRM Plugin
        if (usageProtected || ((mFlags & kFlagIsSecure) && mCrypto))
        {
            /* For FuChun modified */
            OMX_INDEXTYPE index;
            err = mOMX->getExtensionIndex(mNode, "OMX.realtek.android.index.notifyProtectedMode", &index);
            if (err == OK)
            {
                OMX_U32 notify = index;
                index = (OMX_INDEXTYPE)OMX_IndexParamConsumerUsageBits;
                err = mOMX->setParameter(mNode, index, &notify, sizeof(notify));
                ALOGD("\033[0;35m In %s:%d notifyProtectedMode index :0x%.8x and send notify done \033[m", __FUNCTION__, __LINE__, index);
            }
            else
                ALOGW("\033[0;32m Decoder don't understand this command : OMX.realtek.android.index.notifyProtectedMode\033[m");
        }
        // For Transcode , VE scaling
        int32_t encoder_width = 0;
        int32_t encoder_height = 0;
        int32_t encoder_fps = 0;
        msg->findInt32("eWid", &encoder_width);
        msg->findInt32("eHei", &encoder_height);
        msg->findInt32("eFps", &encoder_fps);
        if(encoder_fps == 0 || encoder_fps > 60 || encoder_fps < 0)
            encoder_fps = 0;
        if((encoder_width + encoder_height) > 0)
        {
            OMX_INDEXTYPE index;
            err = mOMX->getExtensionIndex(mNode, "OMX.realtek.android.index.notify_ve_scaling", &index);
            if (err == OK)
            {
                OMX_U32 encoder_size[4];
                encoder_size[0] = index;
                encoder_size[1] = encoder_width;
                encoder_size[2] = encoder_height;
                encoder_size[3] = encoder_fps;
                index = (OMX_INDEXTYPE)OMX_IndexParamConsumerUsageBits;
                err = mOMX->setParameter(mNode, index, encoder_size, sizeof(encoder_size));
                ALOGD("\033[0;35m In %s:%d write ve_scaling_size index:%x WxH:%dx%d scale:%d Consumer\033[m", __FUNCTION__, __LINE__, index, encoder_size[1], encoder_size[2], encoder_size[3]);
            }
            else
                ALOGW("\033[0;32m Decoder don't understand this command : OMX.realtek.android.index.notify_ve_scaling\033[m");
        }

        int32_t sideband_hacked = 0;
        msg->findInt32("hacked-sideband", &sideband_hacked);
        if(sideband_hacked != 0)
        {
            mHackedSideband = true;
            OMX_INDEXTYPE index;
            err = mOMX->getExtensionIndex(mNode, "OMX.realtek.android.index.set_hevc_performance", &index);
            if (err == OK)
            {
                err = mOMX->setParameter(mNode, index, &sideband_hacked, sizeof(sideband_hacked));
                ALOGD("\033[0;35m In %s:%d write sideband_hacked index:%x mode:%d\033[m", __FUNCTION__, __LINE__, index, sideband_hacked);
            }
            else
                ALOGW("\033[0;32m Decoder don't understand this command : OMX.realtek.android.index.notify_ve_scaling\033[m");
        }

        if ((mFlags & kFlagIsSecure) && mCrypto) { // Rtk add check mCrypto
        ...
        //For MVC video
        int32_t isMVC;
        msg->findInt32("cMVC", &isMVC);
        if(isMVC == 1)
        {
            OMX_INDEXTYPE index;
            err = mOMX->getExtensionIndex(mNode, "OMX.realtek.android.index.notify_mvc", &index);
            if (err == OK)
            {
                OMX_U32 values[2] = {index, (OMX_U32)isMVC};
                index = (OMX_INDEXTYPE)OMX_IndexParamConsumerUsageBits;
                err = mOMX->setParameter(mNode, index, values, sizeof(values));
                ALOGD("\033[0;35m In %s:%d enable OMX MVC feature \033[m", __FUNCTION__, __LINE__);
            }
            else
                ALOGW("\033[0;32m Decoder don't understand this command : OMX.realtek.android.index.notify_mvc\033[m");
        }
    }
    ...
    if (haveNativeWindow) {
        ...
        if (msg->findInt32("feature-tunneled-playback", &tunneled) &&
            ...
            //mSideBandDst = true; //20160826 fuchun disable for input port buffer be allocated by malloc()
            ALOGD("\033[0;34m ######## In %s:%d set mSideBandDst to true :%d maxWidth:%d maxHeight:%d ######## \033[m", __FUNCTION__, __LINE__, mSideBandDst, maxWidth, maxHeight);
            mOMX->setReduceOMXMode(mNode, mSideBandDst);
        } else {
            ...
            if (err != OK) {
                ...
                inputFormat->setInt32("adaptive-playback", true);
#ifdef SUPPORT_ADPATIVEPLAYBACK_MODE
                int32_t maxWidth = 0, maxHeight = 0;
                msg->findInt32("max-width", &maxWidth);
                msg->findInt32("max-height", &maxHeight);
                if ((mFlags & kFlagIsSecure) && mCrypto)
                {
                    /* Only support secure decoder like netflix, playmovie */
                    if (maxWidth > 0 && maxHeight > 0)
                    {
                        ALOGV("\033[0;32m @@@ %s:%d [%s] prepareForAdaptivePlayback(%dx%d) \033[m", __FUNCTION__, __LINE__, mComponentName.c_str(), maxWidth, maxHeight);
...
#endif /* end of SUPPORT_ADPATIVEPLAYBACK_MODE */
            }
    ...
    //rtk begin
    if (!video && !strncmp(mComponentName.c_str(), "OMX.realtek.audio.dec", 21)) {
/* ATM-1180 :the tunnel mode the ref. clock of audio is set in kernel.
        int32_t audioHwSync[2] = {0, 0};
        OMX_INDEXTYPE index;
        if (!msg->findInt32("audio-hw-sync", &audioHwSync[1])) {
            //ALOGW("No Audio HW Sync provided for video tunnel");
        } else {
            err = mOMX->getExtensionIndex(mNode, "OMX.realtek.android.index.configureAudioTunnelMode", &index);
            if (err == OK) {
                audioHwSync[0] = index;
                index = (OMX_INDEXTYPE)OMX_IndexParamConsumerUsageBits;
                err = mOMX->setParameter(mNode, index, audioHwSync, sizeof(audioHwSync));
                if (err == OMX_ErrorNone) {
                    ALOGD("setparameter audioTunnelMode");
                }
                else
                    ALOGD("\033[0;32m ---- in %s:%d failed :%x-----\033[m", __FUNCTION__, __LINE__, err);
            }
        }
*/
        int32_t bitsample = 0;
        if(msg->findInt32("bits-sample", &bitsample) && !strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_RAW)) {
            mBitPerSample = bitsample;
            ALOGD("Got audio BitPerSample = %d mine = %s", mBitPerSample, mime);
        }

        int32_t usageProtected = 0;
        if (msg->findInt32("protected", &usageProtected) && usageProtected) {
            ALOGD("usageProtected %d", usageProtected);
        }
        // For Realtek DRM Plugin
        if (usageProtected || ((mFlags & kFlagIsSecure) && mCrypto))
        {
            /* For Secure Path modified */
            OMX_INDEXTYPE index;
            err = mOMX->getExtensionIndex(mNode, "OMX.realtek.android.index.notifyProtectedMode", &index);
            if (err == OK)
            {
                OMX_U32 notify = 0x2379;
                err = mOMX->setParameter(mNode, index, &notify, sizeof(notify));
                ALOGD("\033[0;35m In %s:%d Audio notifyProtectedMode index :0x%.8x and send notify done \033[m", __FUNCTION__, __LINE__, index);
            }
            else
                ALOGW("\033[0;32m Audio Decoder don't understand this command : OMX.realtek.android.index.notifyProtectedMode\033[m");
        }

#if 0
        int32_t audioSampleRate[2] = {0, 0};
        int32_t audioChannelCount[2] = {0, 0};
        if (msg->findInt32("audio-samp-rate", &audioSampleRate[1]))
        {
            ALOGD("[%s %d] Audio get target SampleRate %d", __FUNCTION__, __LINE__, audioSampleRate[1]);
            err = mOMX->getExtensionIndex(mNode, "OMX.realtek.android.index.setAudioTransSampleRate", &index);
            if (err == OK) {
                audioSampleRate[0] = index;
                index = (OMX_INDEXTYPE)OMX_IndexParamConsumerUsageBits;
                err = mOMX->setParameter(mNode, index, audioSampleRate, sizeof(audioSampleRate));
                if (err == OMX_ErrorNone) {
                    ALOGD("setparameter audioSampleRate");
                }
            }
        }

        if (msg->findInt32("audio-chan-count", &audioChannelCount[1]))
        {
            ALOGD("[%s %d] Audio get target ChannelCount %d", __FUNCTION__, __LINE__, audioChannelCount[1]);
            err = mOMX->getExtensionIndex(mNode, "OMX.realtek.android.index.setAudioTransChannels", &index);
            if (err == OK) {
                audioChannelCount[0] = index;
                index = (OMX_INDEXTYPE)OMX_IndexParamConsumerUsageBits;
                err = mOMX->setParameter(mNode, index, audioChannelCount, sizeof(audioChannelCount));
                if (err == OMX_ErrorNone) {
                    ALOGD("setparameter audioChannelCount");
                }
            }
        }
#endif
    ...
    if (video) {
...
#ifdef SUPPORT_OMX_VIDEO_LOW_DELAY_MODE
            int32_t videoLowDelay = 0;
            bool enableLowDelayMode = false;
            char valueL[PROPERTY_VALUE_MAX];
            msg->findInt32("video-lowdelay-mode", &videoLowDelay);
            if (videoLowDelay)
                enableLowDelayMode = true;
            else
            {
                valueL[0] = 0;
                if (property_get("persist.rtk.LowDelay", valueL, NULL) && (!strcmp(valueL, "1") || !strcasecmp(valueL, "true")))
                    enableLowDelayMode = true;
            }
            ALOGV("\033[0;32m In %s:%d enableLowDelayMode :%d video lowdelaymode :%d value:%s \033[m", __FUNCTION__, __LINE__, enableLowDelayMode, videoLowDelay, valueL);
            if (enableLowDelayMode)
            {
                OMX_INDEXTYPE index;
                err = mOMX->getExtensionIndex(mNode, "OMX.realtek.android.index.videoLowDelayMode", &index);
                if (err == OK)
                {
                    OMX_U32 notify = index;
                    index = (OMX_INDEXTYPE)OMX_IndexParamConsumerUsageBits;
                    err = mOMX->setParameter(mNode, index, &notify, sizeof(notify));
                    ALOGD("\033[0;35m In %s:%d videoLowDelayMode index :0x%.8x and send notify done \033[m", __FUNCTION__, __LINE__, index);
                }
                else
                    ALOGW("\033[0;32m In %s:%d Decoder don't understand this command : OMX.realtek.android.index.videoLowDelayMode\033[m", __FUNCTION__, __LINE__);
            }
            else
            {
                ALOGV("\033[0;35m In %s:%d not detect video low delay mode \033[m", __FUNCTION__, __LINE__);
            }
#endif /* end of SUPPORT_OMX_VIDEO_LOW_DELAY_MODE */
#ifdef SUPPORT_OMX_VIDEO_SEARCH_I_FRAME
            int32_t searchIFrame = 0;
            bool enableSearchIFrame = false;
            char value[PROPERTY_VALUE_MAX];
            msg->findInt32("video-search-I", &searchIFrame);
            if (searchIFrame)
                enableSearchIFrame = true;
            else
            {
                value[0]=0;
                if (property_get("persist.rtk.searchI", value, NULL) && (!strcmp(value, "1") || !strcasecmp(value, "true")))
                    enableSearchIFrame = true;
            }
            ALOGV("\033[0;32m In %s:%d enableSearchIFrame:%d searchIFrame:%d value:%s \033[m", __FUNCTION__, __LINE__, enableSearchIFrame, searchIFrame, value);
            if (enableSearchIFrame)
            {
                OMX_INDEXTYPE index;
                err = mOMX->getExtensionIndex(mNode, "OMX.realtek.android.index.setSearchIFrm", &index);
                if (err == OK)
                {
                    OMX_U32 notify = index;
                    index = (OMX_INDEXTYPE)OMX_IndexParamConsumerUsageBits;
                    err = mOMX->setParameter(mNode, index, &notify, sizeof(notify));
                    ALOGD("\033[0;35m In %s:%d setSearchIFrm index :0x%.8x and send notify done \033[m", __FUNCTION__, __LINE__, index);
                }
                else
                    ALOGW("\033[0;32m In %s:%d Decoder don't understand this command : OMX.realtek.android.index.setSearchIFrm \033[m", __FUNCTION__, __LINE__);
            }
#endif /* end of SUPPORT_OMX_VIDEO_SEARCH_I_FRAME */
            //For decoder deinterlace option
            int32_t enableDeinterlace = -1;
            msg->findInt32("video-de-interlace", &enableDeinterlace);
            if(enableDeinterlace >= 0 && enableDeinterlace < 3)
            {
                OMX_INDEXTYPE index;
                err = mOMX->getExtensionIndex(mNode, "OMX.realtek.android.index.videoDeInterlaced", &index);
                if (err == OK)
                {
                    OMX_U32 notify[2] = {index, (OMX_U32)enableDeinterlace};
                    index = (OMX_INDEXTYPE)OMX_IndexParamConsumerUsageBits;
                    err = mOMX->setParameter(mNode, index, notify, sizeof(notify));
                    ALOGD("\033[0;35m In %s:%d enableDeinterlace index :0x%.8x and send notify done \033[m", __FUNCTION__, __LINE__, index);
                }
                else
                    ALOGW("\033[0;32m In %s:%d Decoder don't understand this command : OMX.realtek.android.index.videoDeInterlaced \033[m", __FUNCTION__, __LINE__);
            }
        }

        if (haveNativeWindow) {
#ifndef USE_HACK_SIDEBAND
            mNativeWindow = static_cast<Surface *>(obj.get());
#else
        char value[PROPERTY_VALUE_MAX]={0};
        if ((property_get("rtk.disable.sideband", value, NULL)
                && (!strcmp("1", value) || !strcasecmp("true", value)))
                || mCrypto == true)
        {
            ALOGV("\033[0;35m in %s:%d in haveNativeWindow && disable Sideband @@@@\033[m", __FUNCTION__, __LINE__);
            SB_control = NULL;
        }
        else if ((property_get("rtk.feature-tunneled-playback", value, NULL) && (!strcmp("1", value) || !strcasecmp("true", value)))
                  || (checkSpecialApp()))
            /* && (!strcasecmp(mime, "video/hevc"))*/
        {
            ALOGD("\033[0;35m in %s:%d %s in haveNativeWindow && bf use Sideband @@@@\033[m", __FUNCTION__, __LINE__, mComponentName.c_str());
            if (!strncmp(mComponentName.c_str(), "OMX.realtek.video.dec.hevc", 25))
            {
                OMX_INDEXTYPE index;
                status_t err = mOMX->getExtensionIndex(mNode, "OMX.realtek.android.index.set_hevc_performance", &index);
                if (err == OK)
                {
                    ALOGV("\033[0;36m --- In %s:%d check set_hevc_performance OK:%x ----\033[m", __FUNCTION__, __LINE__, index);
                    OMX_U32 values[2];
                    values[0] = index;
                    values[1] = 1;
                    index = (OMX_INDEXTYPE)OMX_IndexParamConsumerUsageBits;
                    err = mOMX->setParameter(mNode, index, values, sizeof(values));
#if 0 /* Marked debug message */
                    if(err == OK)
                        ALOGD("\033[0;36m --- In %s:%d set set_hevc_performance ok ----\033[m", __FUNCTION__, __LINE__);
                    else
                        ALOGD("\033[0;36m --- In %s:%d set set_hevc_performance failed  ----\033[m", __FUNCTION__, __LINE__);
#endif
                }
                else
                    ALOGD("\033[0;36m --- In %s:%d can't get set_hevc_performance ----\033[m", __FUNCTION__, __LINE__);
            }

            SB_client = new SidebandClient();
            CHECK(SB_client != NULL);
            ...
            {
                int retry_cnt = 10;
                ALOGD("\033[0;35m in %s:%d Sideband createSurface fail, retry ... @@@@\033[m", __FUNCTION__, __LINE__);

                while(SB_control == NULL && retry_cnt > 0)
                {
                    usleep(1000*10);
                    SB_control = SB_client->createSurface(String8("Sideband Surface"),100,100,PIXEL_FORMAT_RGB_888);
                    retry_cnt--;
                }
                ALOGD("\033[0;35m in %s:%d Sideband -->  retry cnts=%d ... @@@@\033[m", __FUNCTION__, __LINE__, 10 - retry_cnt);
            }
        }
        else
            SB_control = NULL;
        ...
        else
        {
            mNativeHandle = NULL;
            //mSideBandDst = true; //20160826 fuchun disable for input port buffer be allocated by malloc()
            ALOGD("\033[0;34m ######## In %s:%d set mSideBandDst to true :%d ######## \033[m", __FUNCTION__, __LINE__, mSideBandDst);
            mOMX->setReduceOMXMode(mNode, mSideBandDst);
            SB_control->setSBHint(true);
            CHECK(SB_control->getSidebandStream(&mNativeHandle) == 0);
            CHECK(mNativeHandle != NULL);
            mSourceHandle= NativeHandle::create(mNativeHandle, false);
            SF_surface = static_cast<Surface *>(obj.get());
            CHECK(SF_surface != NULL);
            SF_surface->setSidebandStream(mSourceHandle);
            mNativeWindow = SB_control->getSurface();
            ALOGV("%s %d native_window_api_connect\n", __FUNCTION__, __LINE__);
            native_window_api_connect(mNativeWindow.get(), NATIVE_WINDOW_API_RTK_SIDEBAND);
            native_window_set_sideband_stream(mNativeWindow.get(), mNativeHandle);
        }
#endif
        ...
        {
            int32_t cameraSource = 0;
            msg->findInt32("realtek-camera", &cameraSource);
            if (cameraSource != 0)
            {
                mCameraSource = true;
                ALOGD("\033[0;34m in %s:%d detect no NativeWindow :%s mCameraSource:%d  setReduceOMXMode@@@@\033[m", __FUNCTION__, __LINE__, msg->debugString().c_str(), mCameraSource);
                mOMX->setReduceOMXMode(mNode, true);
            }
            else
                mCameraSource = false;
        }
    ...
    } else if (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_AAC) || !strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_AAC_LATM) || !strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_AAC_MAIN)) {
        int32_t numChannels, sampleRate;
        if (!msg->findInt32("channel-count", &numChannels)
                || !msg->findInt32("sample-rate", &sampleRate)) {
            ...
            if (encoder) {
                if (msg->findInt32("audio-chan-count", &audioChannelCount)) {
                    ALOGI("[%s %d] Audio get target ChannelCount %d", __FUNCTION__, __LINE__, audioChannelCount);
                }

                if (msg->findInt32("audio-samp-rate", &audioSampleRate)) {
                    ALOGI("[%s %d] Audio get target SampleRate %d", __FUNCTION__, __LINE__, audioSampleRate);
                }

                ALOGI("input  channel %d sampleRate %d\n", numChannels, sampleRate);
                ALOGI("output channel %d sampleRate %d\n", audioChannelCount, audioSampleRate);
            }
            //rtk end
            err = setupAACCodec(
                    encoder, numChannels, sampleRate, bitRate, aacProfile,
                    isADTS != 0, sbrMode, maxOutputChannelCount, drc,
                    pcmLimiterEnable, audioChannelCount, audioSampleRate);
        }
        ...
        if (encoder &&
            ...
            if (encoder) {
                ...
                if (!msg->findInt32("channel-count", &numChannels)
                    ...
                    ALOGW("audio channel %d sample rate %d\n", numChannels, sampleRate);
                    err = setupFLACCodec(encoder, numChannels, sampleRate);
                }
            }
        }
    } else if (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_RAW) || !strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_PCM)) {
        int32_t numChannels, sampleRate;
        if (encoder
                || !msg->findInt32("channel-count", &numChannels)
                || !msg->findInt32("sample-rate", &sampleRate)) {
            ...
            if (sampleRate < 8000) {
                ALOGE("mime = %s sampleRate = %d < 8000 is NOT support", mime, sampleRate);
                err = INVALID_OPERATION;
            } else {
    ...
    } else if (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_DTS_HD)) {
        int32_t numChannels;
        int32_t sampleRate;
        if (!msg->findInt32("channel-count", &numChannels)
                || !msg->findInt32("sample-rate", &sampleRate)) {
            ...
            err = setupDTSHDCodec(encoder, numChannels, sampleRate);
        }
        ...
        if (!msg->findInt32("channel-count", &numChannels)
            ...
            err = setupMLPCodec(encoder, numChannels, sampleRate);
        }
    } else if (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_WMA) || !strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_WMAPRO)) {
        int32_t numChannels;
        int32_t sampleRate;
        bool isWMA = !strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_WMA);
        if (!msg->findInt32("channel-count", &numChannels)
                || !msg->findInt32("sample-rate", &sampleRate)) {
            err = INVALID_OPERATION;
        } else {
            err = setupWMACodec(encoder, numChannels, sampleRate, isWMA);
        }
    } else if (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_ADPCM)) {
        int32_t numChannels;
        int32_t sampleRate;
        if (!msg->findInt32("channel-count", &numChannels)
            || !msg->findInt32("sample-rate", &sampleRate)) {
            ...
            err = setupADPCMCodec(encoder, numChannels, sampleRate);
        }
    }
#ifdef RTK_AUDIO_EXTRACTOR_EN
...
//            ALOGD("DSD: ch %d sr %d @ %s %d\n", numChannels, sampleRate, __FUNCTION__, __LINE__);
            err = setupDSDCodec(encoder, numChannels, sampleRate);
        }
    }
#endif
    else if (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_ALAC))
    {
        err = setupALACCodec(msg, encoder);
        if(err != OK)
        {
            ALOGE("[%s:%d] setupALACCodec Error!\n", __FUNCTION__, __LINE__);
        }
    }
...
//            ALOGD("APE: ch %d sr %d @ %s %d\n", numChannels, sampleRate, __FUNCTION__, __LINE__);
            err = setupAPECodec(encoder, numChannels, sampleRate);
        }
    }
...
//[ RTK begin ]
    ...
    /* For DvdPlayer used ACodec */
    int32_t isBSRingfd;
    if(msg->findInt32("kKeySetBSRingfd", &isBSRingfd))
    {
        int32_t params[8];
        sp<ABuffer> buffer;
        msg->findBuffer("BSRingParams", &buffer);
        memcpy(params, buffer->data(), sizeof(params));

        OMX_INDEXTYPE index;
        status_t err =
            mOMX->getExtensionIndex(
                mNode,
                "OMX.RTK.index.SetBSRingMode",
                &index);

        if (err == OK)
        {
            err = mOMX->setBSRingBuffer(mNode, OMX_IndexComponentStartUnused, params, sizeof(params));
        }
    }
    /* For thumbnail used */
    int32_t isThumbnail = 0;
    if(!strncmp(mComponentName.c_str(), "OMX.realtek.video.dec", 21) && msg->findInt32("isThumbnail", &isThumbnail))
    {
        OMX_INDEXTYPE index;
        status_t err = mOMX->getExtensionIndex(mNode, "OMX.RTK.index.ThumbnailMode", &index);
        if (err != OK)
            return err;
        ALOGD("****** %d setConfig: index=0x%08X  OMX_IndexParamConsumerUsageBits:%x ********", __LINE__, (int)index, OMX_IndexParamConsumerUsageBits);
        OMX_U32 notify[2];
        notify[0] = index;
        notify[1] = isThumbnail == 1 ? OMX_TRUE : OMX_FALSE;
        index = (OMX_INDEXTYPE)OMX_IndexParamConsumerUsageBits;
        err = mOMX->setConfig(mNode, index, notify, sizeof(notify));

        if (err != OK)
        {
             ALOGE("setConfig('OMX.RTK.index.ThumbnailMode') returned error 0x%08x", err);
             return err;
        }
    }

    /* For RtkExtractor used */
    int32_t isRTKExtractor = 0;
    if(!strncmp(mComponentName.c_str(), "OMX.realtek.video.dec", 21) && msg->findInt32("used-RTKExtr", &isRTKExtractor))
    {
        OMX_INDEXTYPE index;
        status_t err =
            mOMX->getExtensionIndex(
                mNode,
                "OMX.RTK.index.ExtractorMode",
                &index);
        if (err != OK)
            return err;
        ALOGD("****** %d setConfig: index=0x%08X  OMX_IndexParamConsumerUsageBits:%x ********", __LINE__, (int)index, OMX_IndexParamConsumerUsageBits);
        OMX_U32 notify[2];
        notify[0] = index;
        notify[1] = (isRTKExtractor == 1 ? OMX_TRUE : OMX_FALSE);
        index = (OMX_INDEXTYPE)OMX_IndexParamConsumerUsageBits;
        err = mOMX->setConfig(mNode, index, notify, sizeof(notify));

        if (err != OK)
        {
             ALOGE("setConfig('OMX.RTK.index.ExtractorMode') returned error 0x%08x", err);
             return err;
        }
    }

    if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_WMV3, mime) || !strcasecmp(MEDIA_MIMETYPE_VIDEO_VC1,mime) || !strcasecmp(MEDIA_MIMETYPE_VIDEO_WMV, mime))
    {
        OMX_VIDEO_PARAM_WMVTYPE paramWMV;
        InitOMXParams(&paramWMV);
        paramWMV.nPortIndex = kPortIndexInput;

        ALOGD("Setting the OMX_VIDEO_PARAM_WMVTYPE params");
        status_t err = mOMX->getParameter(
    ...
    if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_RV40, mime))
    {
        OMX_VIDEO_PARAM_RVTYPE paramRV;
        InitOMXParams(&paramRV);
        paramRV.nPortIndex = kPortIndexInput;

        ALOGD("Setting the OMX_VIDEO_PARAM_RVTYPE params");
        status_t err = mOMX->getParameter(
    ...
    int32_t encodeToInterlace = 0;
    if (encoder
            && msg->findInt32("kKeyEncodeToInterlace", &encodeToInterlace)
            && encodeToInterlace != 0) {
        OMX_INDEXTYPE index;
        err = mOMX->getExtensionIndex(
                mNode,
                "OMX.RTK.index.EncodeToInterlace",
                &index);
...
status_t ACodec::setupAACCodec(
        ...
        int32_t pcmLimiterEnable, int32_t outChannels, int32_t outSampleRate) {
    if (encoder && isADTS) {
        return -EINVAL;
    }
...
//rtk begin
        if (outChannels)
            profile.nChannels = outChannels;
        if (outSampleRate)
            profile.nSampleRate = outSampleRate;
//rtk end
...
status_t ACodec::setupDTSHDCodec(
        bool encoder, int32_t numChannels, int32_t sampleRate) {
    status_t err = setupRawAudioFormat(
            encoder ? kPortIndexInput : kPortIndexOutput, sampleRate, numChannels);
    ...
    if (encoder) {
        ALOGW("DTS-HD encoding is not supported.");
        return INVALID_OPERATION;
    }
...
status_t ACodec::setupMLPCodec(
    ...
    if (encoder) {
        ALOGW("MLP encoding is not supported.");
        return INVALID_OPERATION;
    }

    OMX_AUDIO_PARAM_MLPTYPE def;
    InitOMXParams(&def);
    def.nPortIndex = kPortIndexInput;

    err = mOMX->getParameter(
            mNode,
            (OMX_INDEXTYPE)OMX_IndexParamAudioMlp,
            &def,
            sizeof(def));
...
status_t ACodec::setupWMACodec(
        bool encoder, int32_t numChannels, int32_t sampleRate, bool isWMA) {
    status_t err = setupRawAudioFormat(
            encoder ? kPortIndexInput : kPortIndexOutput, sampleRate, numChannels);
    ...
    if (encoder) {
        ALOGW("WMA/WMAPRO encoding is not supported.");
        return INVALID_OPERATION;
    }
    ...
    if (isWMA) {
        ...
        OMX_AUDIO_PARAM_WMAPROTYPE def;
        InitOMXParams(&def);
        def.nPortIndex = kPortIndexInput;

        err = mOMX->getParameter(
                mNode,
                (OMX_INDEXTYPE)OMX_IndexParamAudioWmaPro,
                &def,
                sizeof(def));
        ...
        bool encoder, int32_t numChannels, int32_t sampleRate)
{
    ...
    if (encoder) {
        ALOGW("DSD encoding is not supported.");
        err = INVALID_OPERATION;
        goto func_exit;
    }

    OMX_AUDIO_PARAM_DSDTYPE def;
    InitOMXParams(&def);
    def.nPortIndex = kPortIndexInput;

    err = mOMX->getParameter(
            mNode,
            (OMX_INDEXTYPE)OMX_IndexParamAudioDSD,
            &def,
            sizeof(def));
    if (err != OK) {
        ALOGE("err @ %s %d\n", __FUNCTION__, __LINE__);
...
status_t ACodec::setupALACCodec(const sp<AMessage> &msg, bool encoder)
{
    int32_t numChannels;
    int32_t sampleRate;
    int32_t bitRate;
    OMX_AUDIO_PARAM_ALACTYPE_COOKIE *pALACCookie;
    ...
    if (!msg->findPointer("alac-config", (void **)&pALACCookie)) {
        ALOGE("Missing ALAC Cookie!!\n");
        return INVALID_OPERATION;
    }
    status_t err = setupRawAudioFormat(encoder ? kPortIndexInput : kPortIndexOutput, sampleRate, numChannels);

    if (err != OK) {
        ALOGE("setupRawAudioFormat fail!\n");
        return err;
    }
    ...
    memcpy(&def.nConfig, pALACCookie, sizeof(*pALACCookie));
...
static const struct VideoCodingMapEntry {
    ...
    { MEDIA_MIMETYPE_VIDEO_AVS, OMX_VIDEO_CodingAVS },
    { MEDIA_MIMETYPE_VIDEO_HEVC, OMX_VIDEO_CodingHEVC },
    ...
    { MEDIA_MIMETYPE_VIDEO_RV30, OMX_VIDEO_CodingRV },
    { MEDIA_MIMETYPE_VIDEO_RV40, OMX_VIDEO_CodingRV },
    { MEDIA_MIMETYPE_VIDEO_MP43, OMX_VIDEO_CodingDIVX3 },
    { MEDIA_MIMETYPE_VIDEO_FLV, OMX_VIDEO_CodingSORENSON },
    { MEDIA_MIMETYPE_VIDEO_MJPEG, OMX_VIDEO_CodingMJPEG},
    { MEDIA_MIMETYPE_VIDEO_RAW, OMX_VIDEO_CodingAVC},//TODO: For CTS, we did not verify OMX decode in real case
...
status_t ACodec::setupAVCEncoderParameters(const sp<AMessage> &msg) {
    ...
    if (h264type.eProfile == OMX_VIDEO_AVCProfileBaseline) {
        ...
        h264type.nSliceHeaderSpacing = 0;
        h264type.bUseHadamard = OMX_TRUE;
        h264type.nRefFrames = 2; // Rtk always used 1
        h264type.nBFrames = 1; // Rtk always used 0
        h264type.nPFrames = setPFramesSpacing(iFrameInterval, frameRate, h264type.nBFrames);
        h264type.nAllowedPictureTypes =
...
//[ RTK begin ]
        fmt != (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_RTK_16_YCbCr_420_SP      &&
        fmt != (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_RTK_32_YCbCr_420_SP      &&
        fmt != (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_RTK_W16_H16_YCbCr_420_SP &&
        fmt != (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_RTK_W32_H16_YCbCr_420_SP &&
        fmt != (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_RTK_W16_H32_YCbCr_420_SP &&
        fmt != (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_RTK_W32_H32_YCbCr_420_SP &&
        fmt != (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_RTK_W16_H8_YCbCr_420_SP  &&
        fmt != (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_RTK_W16_H16_YCrCb_420_SP &&
        fmt != (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_RTK_W32_H16_YCrCb_420_SP &&
        fmt != (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_RTK_W16_H32_YCrCb_420_SP &&
        fmt != (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_RTK_W32_H32_YCrCb_420_SP &&
        fmt != (OMX_COLOR_FORMATTYPE)HAL_PIXEL_FORMAT_RTK_W16_H8_YCrCb_420_SP  &&
//[ RTK end ]
    ...
    switch (def.eDomain) {
        case OMX_PortDomainVideo:
        {
            ...
            switch ((int)videoDef->eCompressionFormat) {
                case OMX_VIDEO_CodingUnused:
                {
                    ...
                    OMX_INDEXTYPE index;
                    err = mOMX->getExtensionIndex(mNode, "OMX.realtek.android.index.get_frame_rate", &index);
                    if (err == OK)
                    {
                        double frameRate = 0;
                        status_t err = mOMX->getParameter(
                            mNode, index, &frameRate, sizeof(frameRate));

                        if(err == OK && frameRate > 0)
                            notify->setDouble("rtk-frame-rate", frameRate);
                    }
                    err = mOMX->getExtensionIndex(mNode, "OMX.realtek.android.index.get_is_prog", &index);
                    if (err == OK)
                    {
                        OMX_S32 isProg[2];
                        isProg[0] = index;
                        status_t err = mOMX->getParameter(
                            mNode, (OMX_INDEXTYPE)OMX_IndexParamConsumerUsageBits, isProg, sizeof(isProg));
                        if(err == OK)
                            notify->setInt32("rtk-is-progressive", isProg[1]);
                    }
                    err = mOMX->getExtensionIndex(mNode, "OMX.realtek.android.index.get_is_hdr", &index);
                    if (err == OK)
                    {
                        OMX_S32 isHdr[2];
                        isHdr[0] = index;
                        status_t err = mOMX->getParameter(
                            mNode, (OMX_INDEXTYPE)OMX_IndexParamConsumerUsageBits, isHdr, sizeof(isHdr));
                        if(err == OK)
                            notify->setInt32("rtk-is-hdr", isHdr[1]);
                    }
                    err = mOMX->getExtensionIndex(mNode, "OMX.realtek.android.index.notify_mvc", &index);
                    if (err == OK)
                    {
                        ...
                        index = (OMX_INDEXTYPE)OMX_IndexParamConsumerUsageBits;
                        status_t err = mOMX->getParameter(
                                mNode, index, values, sizeof(values));
                        if(err == OK)
                            notify->setInt32("rtk-is-mvc", values[1]);
                    }
                    ...
                    if (portIndex == kPortIndexOutput) {
                        ...
                        if (videoDef->nStride > 0)
                            videoDef->nFrameWidth = videoDef->nStride;
                        width = rect.nWidth;
                        height = rect.nHeight;
        ...
        case OMX_PortDomainAudio:
        {
            ...
            switch ((int)audioDef->eEncoding) {
                ...
                {
                    OMX_AUDIO_PARAM_DTSHDTYPE params;
                    InitOMXParams(&params);
                    params.nPortIndex = portIndex;

                    CHECK_EQ(mOMX->getParameter(
                                mNode, OMX_IndexParamAudioDtsHd,
                                &params, sizeof(params)),
                             (status_t)OK);

                    notify->setString("mime", MEDIA_MIMETYPE_AUDIO_DTS_HD);
                    notify->setInt32("channel-count", params.nChannels);
                    notify->setInt32("sample-rate", params.nSamplingRate);
                    break;
                }

                case OMX_AUDIO_CodingMLP:
                {
                    OMX_AUDIO_PARAM_MLPTYPE params;
                    InitOMXParams(&params);
                    params.nPortIndex = portIndex;

                    CHECK_EQ(mOMX->getParameter(
                                mNode, OMX_IndexParamAudioMlp,
                                &params, sizeof(params)),
                             (status_t)OK);

                    notify->setString("mime", MEDIA_MIMETYPE_AUDIO_MLP);
                    notify->setInt32("channel-count", params.nChannels);
                    notify->setInt32("sample-rate", params.nSamplingRate);
                    break;
                }
                ...
                {
                    OMX_AUDIO_PARAM_WMAPROTYPE params;
                    InitOMXParams(&params);
                    params.nPortIndex = portIndex;

                    CHECK_EQ(mOMX->getParameter(
                                mNode, OMX_IndexParamAudioWmaPro,
                                &params, sizeof(params)),
                             (status_t)OK);

                    notify->setString("mime", MEDIA_MIMETYPE_AUDIO_WMAPRO);
                    notify->setInt32("channel-count", params.nChannels);
                    notify->setInt32("sample-rate", params.nSamplingRate);
                    break;
                }
                ...
                case OMX_AUDIO_CodingADPCM:
                {
                    ...
                    CHECK_EQ(mOMX->getParameter(
                                mNode, OMX_IndexParamAudioAdpcm,
                                &params, sizeof(params)),
                             (status_t)OK);

                    notify->setString("mime", MEDIA_MIMETYPE_AUDIO_ADPCM);
                    notify->setInt32("channel-count", params.nChannels);
                    notify->setInt32("sample-rate", params.nSampleRate);
                    break;
                }
...
#ifdef RTK_AUDIO_EXTRACTOR_EN
                case OMX_AUDIO_CodingDSD:
                {
                    OMX_AUDIO_PARAM_DSDTYPE params;
                    InitOMXParams(&params);
                    params.nPortIndex = portIndex;
                    err = mOMX->getParameter(
                                mNode, OMX_IndexParamAudioDSD, &params, sizeof(params));
                    if (err != OK) {
                        return err;
                    }
                    ...
                    notify->setInt32("channel-count", params.nChannels);
                    notify->setInt32("sample-rate", params.nSampleRate);
//                    ALOGD("DSD: ch %d sr %d @ %s %d\n",  params.nChannels, params.nSampleRate, __FUNCTION__, __LINE__);
                    break;
                }
#endif
                case OMX_AUDIO_CodingALAC:
                {
                    ...
                    params.nPortIndex = portIndex;
                    err = mOMX->getParameter(
                                mNode, OMX_IndexParamAudioAlac, &params, sizeof(params));
                    if (err != OK) {
                        return err;
                    }
                    ...
                    notify->setInt32("channel-count", params.nChannels);
                    notify->setInt32("sample-rate", params.nSamplingRate);
                    ALOGD("[%s:%d] ALAC: ch %d sr %d\n", __FUNCTION__, __LINE__, params.nChannels, params.nSamplingRate);
                    break;
                }
...
void ACodec::signalError(OMX_ERRORTYPE error, status_t internalError) {
    ...
    if (mTunneled && mIsVideo && internalError == WOULD_BLOCK) {//Tunnel mode EOS case
        notify->setInt32("err", internalError);
        notify->setInt32("actionCode", ACTION_CODE_TRANSIENT);
        notify->post();
        return;
    }
...
bool ACodec::BaseState::onOMXEvent(
    ...
    status_t err = mCodec->freeBuffersOnPort(kPortIndexOutput);
    if (err != OK)
        ALOGD("\033[0;36m [%d]  omxError= 0x%X  err= 0x%X \033[m", __LINE__, omxError, err);
...
void ACodec::BaseState::postFillThisBuffer(BufferInfo *info) {
    ...
    // For RealTek DRM Plugin
    int64_t phyAddrs = 0;
    info->mData->meta()->findInt64("phys", &phyAddrs);
    info->mData->meta()->clear();

    if (phyAddrs != 0)
        info->mData->meta()->setInt64("phys", phyAddrs);
    ALOGV("\033[0;35m ----- In %s:%d buffer-id:%x -----\033[m", __FUNCTION__, __LINE__, info->mBufferID);
    ...
    switch (mode) {
        ...
        case RESUBMIT_BUFFERS:
        {
            if (rangeLength == 0 && (!(flags & OMX_BUFFERFLAG_EOS)
                ...
                ALOGV("%s:%d [%s] calling fillBuffer %u", __FUNCTION__, __LINE__, 
            ...
            /* For Transcode, only RtkTranscodePostProcessor used, remove (To Be Confirm) */
            notify->setInt64("phys", info->mPhyAddr);
            /* For Camera */
            notify->setInt64("RealPhysical", info->mPhyAddr);
            ...
            notify->post();
#ifdef ENABLE_CTS_WORKAROUND_ATM_457
            /* For CTS test workaround : android.media.cts.MediaCodecTest#testReleaseAfterFlush */
            if (mCodec->mCtsFlags)
            {
                if (flags & OMX_BUFFERFLAG_CODECCONFIG)
                {
                    sleep(2);
                    ALOGD("\033[0;34m ####### In %s:%d delay break ##### \033[m", __FUNCTION__, __LINE__);
                }
            }
#endif /* end of ENABLE_CTS_WORKAROUND_ATM_457 */
...
#ifdef USE_HACK_SIDEBAND
    if (mCodec->mNativeHandle != NULL && mCodec->mNativeWindow.get()) {
        ALOGV("%s %d native_window_api_disconnect\n", __FUNCTION__, __LINE__);
        native_window_api_disconnect(mCodec->mNativeWindow.get(), NATIVE_WINDOW_API_RTK_SIDEBAND);
    }
#endif
...
bool ACodec::LoadedState::onConfigureComponent(
        const sp<AMessage> &msg) {
    ALOGV("onConfigureComponent: %s", msg->debugString().c_str());
...
#ifdef ENABLE_CTS_WORKAROUND_ATM_457
    /* Realtek for CTS */
    if (mCodec->mComponentName.startsWith("OMX.realtek.video.enc.avc"))
    {
        char value[PROPERTY_VALUE_MAX];
        if (property_get("persist.rtk.ctstest", value, NULL) && (!strcmp(value, "1") || !strcasecmp(value, "true")))
        {
            mCodec->mCtsFlags = true;
            ALOGD("\033[0;33m ===== In %s:%d detect realtek video :%s mCtsFlags:%d ========\033[m", __FUNCTION__, __LINE__, mCodec->mComponentName.c_str(), mCodec->mCtsFlags);
        }
    }
#endif /* end of ENABLE_CTS_WORKAROUND_ATM_457 */
...
#else /* else of CHANGE_ALLOC_BUFFER_LOCATION */
...
status_t ACodec::setParameters(const sp<AMessage> &params) {
    ...
    int skipRefFrame;
    if (params->findInt32("skip-non-ref-frame", &skipRefFrame)) {
        OMX_INDEXTYPE index;
        status_t err = mOMX->getExtensionIndex(mNode, "OMX.realtek.android.index.skipNonRefFrame", &index);
        if (err == OK)
        {
            OMX_U32 notify[2] = {index, (OMX_U32)skipRefFrame};
            index = (OMX_INDEXTYPE)OMX_IndexParamConsumerUsageBits;
            err = mOMX->setParameter(mNode, index, notify, sizeof(notify));
            ALOGD("\033[0;35m In %s:%d write skipNonRefFrame index:%x enable:%d \033[m", __FUNCTION__, __LINE__, index, skipRefFrame);
        }
        else
            ALOGW("\033[0;32m Decoder don't understand this command : OMX.realtek.android.index.skipNonRefFrame\033[m");
    }
    ...
    int dropErrFramePercentage;
    if (params->findInt32("drop-err-frame-percentage", &dropErrFramePercentage)) {
        OMX_INDEXTYPE index;
        status_t err = mOMX->getExtensionIndex(mNode, "OMX.realtek.android.index.set_err_frame_drop_threshold", &index);
        if (err == OK)
        {
            err = mOMX->setParameter(mNode, index, &dropErrFramePercentage, sizeof(OMX_U32));
            ALOGD("\033[0;35m In %s:%d write dropErrFramePercentage index:%x percentage:%d \033[m", __FUNCTION__, __LINE__, index, dropErrFramePercentage);
        }
        else
            ALOGW("\033[0;32m Decoder don't understand this command : OMX.realtek.android.index.set_err_frame_drop_threshold\033[m");
    }
    return OK;
}
...
bool ACodec::ExecutingState::onOMXEvent(
    ...
    switch (event) {
        ...
        {
            if (mCodec->mTunneled && mCodec->mIsVideo) {//Inform MediaCodec that video decoder was finished in tunnel mode
                mCodec->signalError(OMX_ErrorNone, WOULD_BLOCK);
            }
            return true;
        }
    ...
    switch (event) {
        case OMX_EventCmdComplete:
        {
            if (data1 == (OMX_U32)OMX_CommandPortDisable) {
                ...
                /* Realtek enhance */
                mCodec->mFirstAllocBuffer = false;
...
#ifdef USE_HACK_SIDEBAND
/* Special App check need use HACK_SIDEBAND */
bool ACodec::checkSpecialApp()
{
#define APP_KODI    "org.xbmc.kodi"
#define APP_SPMC    "com.semperpax.spmc"
    ...
    sprintf(FileBuf, "/proc/%d/comm", CurrentPid);
    proc_fd = open(FileBuf, O_RDONLY);
    if (proc_fd <= 0)
        ALOGD("\033[0;36m##### open %s failed , errno: %d :%s \033[m", FileBuf, errno, strerror(errno));
    else
    {
        rLen = read(proc_fd, NameBuf, 1024);
        value[0]=0;
        if ((strncmp(NameBuf, APP_KODI, strlen(APP_KODI)) == 0))
        {
            ALOGV("\033[0;36m##### got special app :%s #####\033[m", NameBuf);
            ret = true;
        }
        else if ((strncmp(NameBuf, APP_SPMC, strlen(APP_SPMC)) == 0))
        {
            ALOGD("\033[0;36m##### got special app :%s #####\033[m", NameBuf);
            if (property_get("persist.rtk.spmc.surfaceview", value, NULL) && (!strcmp(value, "1") || !strcasecmp(value, "true")))
            {
                ALOGV("\033[0;36m ---- have surfaceview -----\033[m");
                ret = true;
            }
            else
            {
                ALOGV("\033[0;36m ---- no surfaceview -----\033[m");
            }
        }
        else
        {
            ALOGV("\033[0;36m##### NameBuf:%s #####\033[m", NameBuf);
        }
        close(proc_fd);
    }
    return ret;
}
#endif /* end of USE_HACK_SIDEBAND */
