...
struct WAVSource : public MediaSource {
    ...
    WAVSource(
            ...
            off64_t offset, size_t size, const sp<WAVExtractor> &extractor);
#else
    WAVSource(
            const sp<DataSource> &dataSource,
            const sp<MetaData> &meta,
...
#ifdef RTK_PLATFORM
    return new WAVSource(mDataSource, mTrackMeta, mWaveFormat, mBitsPerSample, mDataOffset, mDataSize, this);
#else
...
#ifdef RTK_PLATFORM
#define DTS_PARSE_SIZE (96*1024)
void WAVExtractor::MakeDTSCodecSpecificData(unsigned channel_configuration, unsigned sampling_freq_index) {
    ...
    if(sampling_freq_index < 16u)
        mSampleRate = (uint32_t)kSamplingFreq[sampling_freq_index];

    if(channel_configuration < 16u)
        mNumChannels = (uint16_t)kChannelArrage[channel_configuration];

    //ALOGD("MakeDTSCodecSpecificData get audio config %d channel count, %d sample rate", kChannelArrage[channel_configuration], kSamplingFreq[sampling_freq_index]);
}

bool WAVExtractor::checkDTSAudio() {
    // identify it is DTS-CD or not
    uint8_t checkDTSformat[DTS_PARSE_SIZE];
    ...
    if (mDataSource->readAt(44, checkDTSformat, DTS_PARSE_SIZE) < DTS_PARSE_SIZE) {
        return false;
    }
    for (i=0; i<DTS_PARSE_SIZE-4; i++)
    {
        msyncWord = checkDTSformat[i]<<24 | checkDTSformat[i+1]<<16 | checkDTSformat[i+2]<<8 | checkDTSformat[i+3];
        if ((msyncWord == 0xFF1F00E8) || (msyncWord == 0x1FFFE800) || (msyncWord == 0xFE7F0180) || (msyncWord == 0x72F81F4E) ||
          (msyncWord == 0x5A5A5A5A) || (msyncWord == 0x1D95F262) || (msyncWord == 0x64582025) || (msyncWord == 0x7FFE8001))
            break;
    }
    //ALOGD("\033[0;32m [%d]  find sync word pos: %d   msyncWord= 0x%X\033[m\n", __LINE__, i, msyncWord);

    switch(msyncWord){
        //14 bits LE
        case 0xFF1F00E8:
            //make sure frame type is DTS
            if ((checkDTSformat[i+4] & 0xF0) != 0xF0 || checkDTSformat[i+5] != 0x07){
            ...
            le_mode = 1;
            findDTSAudio = true;
            bLittleEndianDTS = true;
            break;
        //14 bit BE
        case 0x1FFFE800:
            //make sure frame type is DTS
            if (checkDTSformat[i+4] != 0x07 || (checkDTSformat[i+5] & 0xF0) != 0xF0){
                ALOGD("wrong DTS start code");
                return false;
            }
            word_mode =0;
    ...
    if (findDTSAudio)
    {
        ...
        {
            mDataOffset = (off64_t)(i + 44);
            mDataSize = FileSize - mDataOffset;
        }

        if(word_mode){
            frame_size = (checkDTSformat[i+5-le_mode] & 0x03) << 12 | checkDTSformat[i+6+le_mode] << 4| checkDTSformat[i+7-le_mode] >> 4;
            ++frame_size;//frame size
            amode = (checkDTSformat[i+7-le_mode] & 0x0F) << 2 | checkDTSformat[i+8+le_mode] >> 6;//amode
            sfreq = checkDTSformat[i+8+le_mode] >>2 & 0x0F;//sfreq
        }else{
            frame_size = (checkDTSformat[i+6+le_mode] & 0x03) << 12 | checkDTSformat[i+7-le_mode] << 4| (checkDTSformat[i+8+le_mode] & 0x3F) >> 2;//frame size
            ++frame_size;
            amode = (checkDTSformat[i+8+le_mode] & 0x03) << 4| (checkDTSformat[i+9-le_mode] & 0xF0) >> 4;//amode
            sfreq = checkDTSformat[i+9-le_mode] & 0x0F;//sfreq
        }
        //ALOGD("\033[0;32m [%d]  amode= %d, sfreq= %d,  frame_size= %d\033[m\n", __LINE__, amode, sfreq, (int)frame_size);
        MakeDTSCodecSpecificData(amode, sfreq);
        return true;
    }
    return false;
}
...
#ifdef RTK_PLATFORM
            mByteRate = U32_LE_AT(&formatSpec[8]);
            //ALOGD("\033[0;32m [%d]  mByteRate= %d\033[m\n", __LINE__, (int)mByteRate);
#endif
            ...
            if (mValidFormat) {
...
#ifdef RTK_PLATFORM
                    {
                        isDTSAudio = checkDTSAudio();
                        if (isDTSAudio)
                            mTrackMeta->setCString(kKeyMIMEType, MEDIA_MIMETYPE_AUDIO_DTS);
                        else
                ...
                if (mWaveFormat == WAVE_FORMAT_MSGSM) {
                    ...
                    if (isDTSAudio)
                    {
                        if (mByteRate > 0)
                        {
                            durationUs = 1000000LL * (chunkSize / mByteRate);
                            //ALOGD("\033[0;36m [%d]  durationUs= %lld   chunkSize= %d\033[m\n", __LINE__, (long long)durationUs, chunkSize);
                        }
                        else
                            ALOGD("\033[0;31m [%d]  Could not get duration value, due to mByteRate= %d\033[m\n", __LINE__, mByteRate);
                    }
                    else
                    {
...
#ifdef RTK_PLATFORM
            if (mExtractor->isDTSAudio)
            {
                pos = (int64_t)(seekTimeUs * mExtractor->mByteRate / 1000000LL);
                pos = (pos / 16384) * 16384;
                //ALOGD("\033[0;32m [%d]  seek pos= %lld   mSize= %d\033[m\n", __LINE__, (long long)pos, (int)mSize);
            }
            else
...
#ifdef RTK_PLATFORM
        if (mExtractor->isDTSAudio) {
            if (mExtractor->bLittleEndianDTS) {
...
#ifdef RTK_PLATFORM
        if (mExtractor->isDTSAudio)
            timeStampUs = 1000000LL * (mCurrentPos - mOffset) / mExtractor->mByteRate;
        else
        {
            size_t bytesPerSample = mBitsPerSample >> 3;
