...
#ifdef CTS_DEBUG_USED
        ALOGV("\033[0;32m --- In %s:%d mime:%s matchIndex:%d index:%d componentName:%s \033[m", __FUNCTION__, __LINE__, mime, (int)matchIndex, (int)index, componentName.c_str());
        char value[PROPERTY_VALUE_MAX];
        bool disableRtkDec = false;
        bool disableRtkEnc = false;
        if (property_get("persist.rtk.disable_dec", value, NULL) && (!strcmp(value, "1") || !strcasecmp(value, "true")))
        {
            disableRtkDec = true;
            value[0] = 0;
        }
        if (property_get("persist.rtk.disable_enc", value, NULL) && (!strcmp(value, "1") || !strcasecmp(value, "true")))
            disableRtkEnc = true;
        ALOGV("\033[0;34m ----%s:%d Cts module disableRtkDec:%d disableRtkEnc:%d check persist.rtk.disable_dec, persist.rtk.disable_enc \033[m", __FUNCTION__, __LINE__, disableRtkDec, disableRtkEnc);
        if ((!strncmp(componentName.c_str(), "OMX.realtek.video.dec", 21)) && disableRtkDec)
        {
            ALOGD("\033[0;34m ----%s:%d don't used hw decoder %s \033[m", __FUNCTION__, __LINE__, componentName.c_str());
            continue;
        }
        if ((!strncmp(componentName.c_str(), "OMX.realtek.video.enc", 21)) && disableRtkEnc)
        {
            ALOGD("\033[0;34m ----%s:%d don't used hw encoder %s \033[m", __FUNCTION__, __LINE__, componentName.c_str());
            continue;
        }
#endif
