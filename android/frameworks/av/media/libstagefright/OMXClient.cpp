...
//[ RTK begin ]
    ...
    virtual status_t getIonFd(node_id node, OMX_INDEXTYPE index, void *params, size_t size, OMX_S32 *shareFd);
...
//[ RTK begin ]
status_t MuxOMX::setBSRingBuffer(
        node_id node, OMX_INDEXTYPE index,
        const void *params, size_t size) {
    return getOMX(node)->setBSRingBuffer(node, index, params, size);
}

status_t MuxOMX::getIonFd(
        node_id node, OMX_INDEXTYPE index, void *params, size_t size, 
        OMX_S32 *shareFd) {
    return getOMX(node)->getIonFd(node, index, params, size, shareFd);
}
void MuxOMX::setReduceOMXMode(
        node_id node, bool enableReduceOmx) {
    ALOGV("\033[0;33m !!!!! In %s:%d enableReduceOmx:%d !!!!!\033[m", __FUNCTION__, __LINE__, enableReduceOmx);
    getOMX(node)->setReduceOMXMode(node, enableReduceOmx);
}
//[ RTK end ]
