...
//rtk begin
#define PAGE_OFFSET_THRESHOLD 30000
//rtk end
...
status_t MyOggExtractor::findPrevGranulePosition(
    ...
    *granulePos = 0;
    ...
    for (;;) {
        if (prevGuess >= PAGE_OFFSET_THRESHOLD /*5000*/) {
            prevGuess -= PAGE_OFFSET_THRESHOLD /*5000*/;
        } else {
            prevGuess = 0;
        }
...
status_t MyOggExtractor::init() {
    ...
    if (!(mSource->flags() & DataSource::kIsCachingDataSource)
        ...
        //ALOGD("\033[0;33;33m [%s %d] lastGranlePosition %lld, mVi rate %d duration %lld \033[m",
        //    __FUNCTION__, __LINE__, lastGranulePosition, mVi.rate, durationUs);
        buildTableOfContents();
    }
//rtk begin
    else if (!(mSource->flags() & DataSource::kIsCachingDataSource)
            && mSource->getSize(&size) == OK) {
        if (mVi.rate != 0 && lastGranulePosition != 0) {
            int64_t durationUs = lastGranulePosition * 1000000ll / mVi.rate;
            mMeta->setInt64(kKeyDuration, durationUs);
            //ALOGD("\033[0;33;33m [%s %d] lastGranlePosition %lld, mVi rate %d duration %lld \033[m",
            //    __FUNCTION__, __LINE__, lastGranulePosition, mVi.rate, durationUs);
        } else {
            //ALOGD("\033[0;33;33m [%s %d] lastGranlePosition %lld, mVi rate %d \033[m",
            //    __FUNCTION__, __LINE__, lastGranulePosition, mVi.rate);
        }
    }
//rtk end
    return OK;
    ...
    switch (type) {
        case 1:
        {
            ...
            if (mSource->getSize(&size) == OK) {
                uint64_t bps = approxBitrate();
                if (bps != 0) {
                    //ALOGD("Func %s line %d set kKeyDuration %lld", __FUNCTION__, __LINE__, size * 8000000ll / bps);
                    //ALOGD("size %lld, bps %lld", size, bps);
...
//rtk begin
            mMeta->setData(kKeyVorbisComment, 0, data, size);
//rtk end
            parseFileMetaData();
            break;
        }
