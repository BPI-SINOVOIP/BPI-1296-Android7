...
struct OMXNodeInstance {
    ...
    //[ RTK begin ]
    status_t setBSRingBuffer(OMX_INDEXTYPE index, const void *params, size_t size);

    status_t getIonFd(OMX_INDEXTYPE index, void *params, size_t size, OMX_S32 *shareFd);

    bool isReduceOMXMode() const {
        ALOGV("\033[0;32m ###### In %s:%d mIsRedeceOmxMode:%d #######\033[m", __FUNCTION__, __LINE__, mIsRedeceOmxMode);
        return mIsRedeceOmxMode;
    }
    void setReduceOMXMode(bool enableReduceOmx);
    //[ RTK end ]
