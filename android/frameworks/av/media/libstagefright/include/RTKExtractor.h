...
class RTKLoadableExtractor : public MediaExtractor {
public:
    RTKLoadableExtractor() {}
    virtual ~RTKLoadableExtractor() {}
    ...
    virtual status_t check_init();
    virtual void FlowStop(void) = 0;
};

class RTKExtractor : public MediaExtractor {
public:
    RTKExtractor(const sp<DataSource> &source);
...
protected:
    virtual ~RTKExtractor();

private:
    sp<DataSource> mDataSource;
    sp<RTKLoadableExtractor> mImpl;

    RTKExtractor(const RTKExtractor &);
    RTKExtractor &operator=(const RTKExtractor &);
};
