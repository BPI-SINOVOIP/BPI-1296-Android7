...
ssize_t MediaHTTP::readAt(off64_t offset, void *data, size_t size) {
    ...
    while (numBytesRead < size) {
        ...
        if(endTimeUs - startTimeUs > 6 * nOverTimeCount * 1E6)//Take 6 second to read.
        {
            ALOGW("[%s][%d] Read over time! need size:%d numBytesRead:%d nOverTimeCount:%d.\n",__FUNCTION__,__LINE__, (int)size, (int)numBytesRead, nOverTimeCount);
            nOverTimeCount++;
            if(nOverTimeCount > 10)
            {
                ALOGE("[%s][%d] Read over time! need size:%d numBytesRead:%d nOverTimeCount:%d.Break.\n",__FUNCTION__,__LINE__, (int)size, (int)numBytesRead, nOverTimeCount);
                break;
            }
        }
#endif
