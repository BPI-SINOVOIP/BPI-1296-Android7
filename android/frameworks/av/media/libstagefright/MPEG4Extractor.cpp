...
#ifdef RTK_PLATFORM
    status_t parseChunkAVS(uint8_t *data, size_t size);
    bool isKeyFrameAVS(const uint8_t *data, size_t size);
    uint32_t findSyncSampleNearAVS(uint32_t sample_index, uint32_t flags);
#endif
...
static const char *FourCC2MIME(uint32_t fourcc) {
    switch (fourcc) {
        ...
        case FOURCC('t', 'w', 'o', 's'): // big endian
            return MEDIA_MIMETYPE_AUDIO_PCM;

        case FOURCC('s', 'o', 'w', 't'): // little endian
        case FOURCC('r', 'a', 'w', ' '):
        case FOURCC('l', 'p', 'c', 'm'):
            return MEDIA_MIMETYPE_AUDIO_RAW;

        case FOURCC('i', 'm', 'a', '4'):
            return MEDIA_MIMETYPE_AUDIO_ADPCM;
...
static bool AdjustChannelsAndRate(uint32_t fourcc, uint32_t *channels, uint32_t *rate) {
    if (FourCC2MIME(fourcc) && !strcasecmp(MEDIA_MIMETYPE_AUDIO_AMR_NB, FourCC2MIME(fourcc))) {
        // AMR NB audio is always mono, 8kHz
        *channels = 1;
        *rate = 8000;
        return true;
    } else if (FourCC2MIME(fourcc) && !strcasecmp(MEDIA_MIMETYPE_AUDIO_AMR_WB, FourCC2MIME(fourcc))) {
...
MPEG4Extractor::MPEG4Extractor(const sp<DataSource> &source)
    : mMoofOffset(0),
      ...
      mFirstSINF(NULL),
#ifdef RTK_PLATFORM
      mMetaFoundAfterTrak(false),
#endif
      mIsDrm(false) {
#ifdef RTK_PLATFORM
    mTWOSChunkOffsets = NULL;
    mTWOSChunkSizes = NULL;
    mTWOSMaxPacketSize = 0;
    mTWOSsamplesPerChunk = 0;

    mSOWTChunkOffsets = NULL;
    mSOWTChunkSizes = NULL;
    mSOWTMaxPacketSize = 0;
    mSOWTsamplesPerChunk = 0;
    mCurrcts = 0;
    audio_ver1_header_present_flag = 0;
    audio_samples_per_packet = 0;
...
#ifdef RTK_PLATFORM
    if (mTWOSChunkOffsets)
        free(mTWOSChunkOffsets);
    if (mTWOSChunkSizes)
        free(mTWOSChunkSizes);

    if (mSOWTChunkOffsets)
        free(mSOWTChunkOffsets);
    if (mSOWTChunkSizes)
        free(mSOWTChunkSizes);
#endif
}
            ...
            if (mMoofOffset > 0) {
                ...
                if (track->timescale != 0 &&
                        ...
                        && track->sampleTable->getMetaDataForSample(
                            ...
                            &sampleTime) == OK) {
#ifdef RTK_PLATFORM
                    off64_t FileSize = 0, offset = 0;
                    if (mDataSource->getSize(&FileSize) == OK)
                        err = track->sampleTable->getMetaDataForSample(sampleIndex, &offset, NULL, NULL, NULL, NULL);

                    if ((err == OK) && (offset > FileSize)) // file truncated
                    {
                        track->meta->setInt64(kKeyThumbnailTime, 3000000);
                        ALOGD("\033[0;33m [%d] sampleIndex= %d,  FileSize= %lld,  offset= %lld,  sampleTime= %llu,  track->timescale= %d\033[m", __LINE__, sampleIndex, (long long)FileSize, (long long)offset, (long long)sampleTime, track->timescale);
                    }
                    else
#endif
                    track->meta->setInt64(
...
static bool underQTMetaPath(const Vector<uint32_t> &path, int32_t depth) {
    ...
    cond2 = path.size() >= 3
            && path[0] == FOURCC('m', 'o', 'o', 'v')
            && path[1] == FOURCC('t', 'r', 'a', 'k')
            && path[2] == FOURCC('m', 'e', 't', 'a')
            && (depth == 3
            || (depth == 4
                    && (path[3] == FOURCC('h', 'd', 'l', 'r')
                    ||  path[3] == FOURCC('i', 'l', 's', 't')
                    ||  path[3] == FOURCC('k', 'e', 'y', 's'))));

    //ALOGD("\033[0;33m [%d]  cond1= %d, cond2= %d    path.size()= %d \033[m", __LINE__, cond1, cond2, path.size());
    return (cond1 || cond2);
#else
    return path.size() >= 2
            && path[0] == FOURCC('m', 'o', 'o', 'v')
...
#ifdef RTK_PLATFORM // for debug used
    //char *type = (char *)&chunk_type;
    //ALOGD("\033[0;33m [%d] chunk_type= %.4s, chunk_size= %lld \033[m", __LINE__, type, (long long)chunk_size);
#endif
    switch(chunk_type) {
        case FOURCC('m', 'o', 'o', 'v'):
        case FOURCC('t', 'r', 'a', 'k'):
...
#ifdef RTK_PLATFORM
                track->mIsMLAWAudio = false;
                track->mIsTWOSAudio = false;
                track->mIsSOWTAudio = 0;
#endif
            }
...
#ifdef RTK_PLATFORM
            if((*offset != stop_offset) && (*offset > stop_offset-8)) {
                ALOGV("offset(%lld) > stop_offset(%lld) - 8", (long long)(*offset), (long long)stop_offset);
            }
            else
#endif
            if (*offset != stop_offset) {
...
#ifdef RTK_PLATFORM//For CTS testTunneledVideoPlayback
                int64_t tmpDuration = (duration * 1000000) / mLastTrack->timescale;
                tmpDuration = (tmpDuration*1000000)/1000000;
                mLastTrack->meta->setInt64(
                        kKeyDuration, tmpDuration);
#else
                mLastTrack->meta->setInt64(
...
#ifdef RTK_PLATFORM
            if(lang_code[0] == 0x60 && lang_code[1] == 0x60 && lang_code[2] == 0x60)    //invalid language code
            {
                break;
            }
#endif
...
#endif
        {
...
#ifdef RTK_PLATFORM
            if (chunk_type == FOURCC('l', 'p', 'c', 'm')) {
                if(sample_rate < 8000 && mLastTrack != NULL) {
                    ALOGD("\033[0;33m [%d] sample_rate changes from %d to %d \033[m", __LINE__, sample_rate, mLastTrack->timescale);
                    sample_rate = mLastTrack->timescale;
                }
            }

            ALOGD("\033[0;32m [%d] version= %d, num_channels= %d, sample_size= %d, sample_rate= %d \033[m", __LINE__, version, num_channels, sample_size, sample_rate);
            bool SampleRateUnder8k = false;
            if (sample_rate < 8000)
            {
                // only not support PCM under 8k sample rate, because other audio format maybe with wrong audio info at stsd chunk (ATM-960)
                if (FourCC2MIME(chunk_type) && (!strcasecmp(MEDIA_MIMETYPE_AUDIO_PCM, FourCC2MIME(chunk_type)) || !strcasecmp(MEDIA_MIMETYPE_AUDIO_RAW, FourCC2MIME(chunk_type))))
                {
                    ALOGD("\033[0;31m [%d]  audio_mime= %s   sample_rate= %d < 8000, not support!!\033[m", __LINE__, FourCC2MIME(chunk_type), sample_rate);
                    SampleRateUnder8k = true;
                }
            }
#endif
            ...
            *offset = data_offset + sizeof(buffer);
            ...
            if (SampleRateUnder8k == false)
            {
                if (FourCC2MIME(chunk_type) && !strcasecmp(MEDIA_MIMETYPE_AUDIO_RAW, FourCC2MIME(chunk_type))) // little endian pcm
                {
                    //ALOGD("\033[0;36m [%d] sample_size= %d \033[m", __LINE__, BitsPerSample);
                    if (sample_size == 8)
                        mLastTrack->meta->setInt32(kKeyPcmEncoding, kAudioEncodingPcm8bit);
                    else if (sample_size == 16)
                        mLastTrack->meta->setInt32(kKeyPcmEncoding, kAudioEncodingPcm16bit);
                    else if (sample_size == 32)
                        mLastTrack->meta->setInt32(kKeyPcmEncoding, kAudioEncodingPcmFloat);
                }
#endif
            mLastTrack->meta->setInt32(kKeyChannelCount, num_channels);
            mLastTrack->meta->setInt32(kKeySampleRate, sample_rate);
            mLastTrack->meta->setInt32(kKeyAudioBitsPerSample, sample_size);
#ifdef RTK_PLATFORM
            }
#endif

#ifdef RTK_PLATFORM
            if ((SampleRateUnder8k == false) && FourCC2MIME(chunk_type))
                ALOGD("\033[0;33m [%d] audio_mime= %s, num_channels= %d, sample_rate= %d, sample_size= %d \033[m", __LINE__, FourCC2MIME(chunk_type), num_channels, sample_rate, sample_size);
            uint32_t adjust = 0;
            if (chunk_type == FOURCC('u', 'l', 'a', 'w'))
            {
                *offset = stop_offset;
                mLastTrack->mIsMLAWAudio = true;
            }
            if (chunk_type == FOURCC('t', 'w', 'o', 's'))
                mLastTrack->mIsTWOSAudio = true;
            if (chunk_type == FOURCC('s', 'o', 'w', 't'))
                mLastTrack->mIsSOWTAudio = 1;
            if (chunk_type == FOURCC('r', 'a', 'w', ' '))
                mLastTrack->mIsSOWTAudio = 2;
            if (chunk_type == FOURCC('l', 'p', 'c', 'm'))
                mLastTrack->mIsSOWTAudio = 3;
#endif
            ...
            while (*offset < stop_offset) {
                ...
                //ALOGD("\033[0;31m [%d]  version= %d  adjust= %d\033[m", __LINE__, version, adjust);
                if(version == 0){
                    if (chunk_type == FOURCC('a', 'l', 'a', 'c')) {
                        ALOGD("alac audio found............................");
                        uint32_t atomhdr[2];
                        if (mDataSource->readAt(*offset, atomhdr, 8) < 8)
                            return ERROR_IO;
                        uint64_t header_size = ntohl(atomhdr[0]);
                        int32_t atom_type = ntohl(atomhdr[1]);
                        *offset += 8; //atomhdr

                        if (atom_type == FOURCC('a', 'l', 'a', 'c')) {
                            if(header_size-8 >= 28) {
                                *offset += 4;
                                uint8_t tmp_buf[24] = {0};
                                if (mDataSource->readAt(*offset, tmp_buf, sizeof(tmp_buf)) < (ssize_t)sizeof(tmp_buf))
                                    return ERROR_IO;

                                uint8_t * theActualCookie = (uint8_t *)tmp_buf;
                                mALACConfig.frameLength = Swap32BtoN(((ALACSpecificConfig *)theActualCookie)->frameLength);
                                mALACConfig.compatibleVersion = ((ALACSpecificConfig *)theActualCookie)->compatibleVersion;
                                mALACConfig.bitDepth = ((ALACSpecificConfig *)theActualCookie)->bitDepth;
                                mALACConfig.pb = ((ALACSpecificConfig *)theActualCookie)->pb;
                                mALACConfig.mb = ((ALACSpecificConfig *)theActualCookie)->mb;
                                mALACConfig.kb = ((ALACSpecificConfig *)theActualCookie)->kb;
                                mALACConfig.numChannels = ((ALACSpecificConfig *)theActualCookie)->numChannels;
                                mALACConfig.maxRun = Swap16BtoN(((ALACSpecificConfig *)theActualCookie)->maxRun);
                                mALACConfig.maxFrameBytes = Swap32BtoN(((ALACSpecificConfig *)theActualCookie)->maxFrameBytes);
                                mALACConfig.avgBitRate = Swap32BtoN(((ALACSpecificConfig *)theActualCookie)->avgBitRate);
                                mALACConfig.sampleRate = Swap32BtoN(((ALACSpecificConfig *)theActualCookie)->sampleRate);

                                mLastTrack->meta->setInt32(kKeyChannelCount, mALACConfig.numChannels);
                                mLastTrack->meta->setInt32(kKeySampleRate, mALACConfig.sampleRate);
                                mLastTrack->meta->setInt32(kKeyBitRate, mALACConfig.avgBitRate);
                                mLastTrack->meta->setPointer(kKeyALAC_CAFFConfig, &mALACConfig);
                                *offset += header_size-8-4;
                                break;
                            }
                        }
                    }
                ...
                {
                    if ((mLastTrack->mIsSOWTAudio > 0) && (version >= 1) && (chunk_data_size >= 44))
                    {
                        audio_ver1_header_present_flag = 1;

                        uint8_t tmp_buf[16] = {0};
                        if (mDataSource->readAt(*offset, tmp_buf, sizeof(tmp_buf)) < (ssize_t)sizeof(tmp_buf))
                            return ERROR_IO;
                        audio_samples_per_packet = ( (tmp_buf[0]<<24) | (tmp_buf[1]<<16) | (tmp_buf[2]<<8) | tmp_buf[3]);
                        audio_bytes_per_frame = ( (tmp_buf[8]<<24) | (tmp_buf[9]<<16) | (tmp_buf[10]<<8) | tmp_buf[11]);
                        //ALOGD("\033[0;33m [%d]  audio_samples_per_packet= %d, audio_bytes_per_frame= %d\033[m", __LINE__, audio_samples_per_packet, audio_bytes_per_frame);
                    }
                    ...
                    *offset += 16; // skip 16 bytes
                    ...
                    if (atom_type == FOURCC('c', 'h', 'a', 'n'))
                    {
                        *offset += header_size;
                        header_size = ntohl(atomhdr[0]);
                        atom_type = ntohl(atomhdr[1]);
                        if(*offset >= stop_offset)
                            break;
                    }

                    if (atom_type != FOURCC('e', 's', 'd', 's'))
                    {
                        *offset += 8; // header_size and atom_type, 8 bytes
                        *offset += (adjust - 16 - 8);
                        if (mDataSource->readAt(*offset, atomhdr, 8) < 8)
                            return ERROR_IO;
                        header_size = ntohl(atomhdr[0]);
                        atom_type = ntohl(atomhdr[1]);
                        //ALOGD("\033[0;31m [%d]  atom_type= %.4s  header_size= %lld   offset= %lld\033[m", __LINE__, (char *)&atom_type, (long long)header_size, (long long)*offset);

                        while (atom_type != FOURCC('e', 's', 'd', 's'))
                        {
                            if (chunk_type != FOURCC('m', 'p', '4', 'a'))
                                break;
                            if ((*offset + (off64_t)header_size) >= stop_offset)
                                break;
                            *offset += header_size;
                            if (mDataSource->readAt(*offset, atomhdr, 8) < 8)
                                return ERROR_IO;
        ...
        case FOURCC('D', 'X', '5', '0'):
        case FOURCC('d', 'x', '5', '0'):
            ALOGD("chunk= %s is not support!!", chunk);
...
#ifdef RTK_PLATFORM
            ALOGD("\033[0;33m [%d] video_mime= %s, width= %d, height= %d \033[m", __LINE__, FourCC2MIME(chunk_type), width, height);
#endif
...
#ifdef RTK_PLATFORM
            if (mLastTrack->mIsTWOSAudio)
            {
                uint8_t header[8] = {0};
                size_t max_size = 0;

                if (mDataSource->readAt(data_offset, header, sizeof(header)) < (ssize_t)sizeof(header))
                    ALOGD("\033[0;31m [%d]  read stco or co64 data fail...  \033[m", __LINE__);

                mTWOSNumChunk = U32_AT(&header[4]);
                if (!mTWOSChunkOffsets && !mTWOSChunkSizes)
                {
                    mTWOSChunkOffsets = (uint32_t *)malloc(mTWOSNumChunk * sizeof(uint32_t));
                    memset(&mTWOSChunkOffsets[0], 0, mTWOSNumChunk * sizeof(uint32_t));
                    mTWOSChunkSizes = (uint32_t *)malloc(mTWOSNumChunk * sizeof(uint32_t));
                    memset(&mTWOSChunkSizes[0], 0, mTWOSNumChunk * sizeof(uint32_t));
                }

                if ((chunk_type == FOURCC('s', 't', 'c', 'o')) && (chunk_data_size < 8 + mTWOSNumChunk * 4))
                    ALOGD("\033[0;31m [%d]  wrong stco size, chunk_data_size= %d, real_size= %d  \033[m", __LINE__, (uint32_t)chunk_data_size, 8 + mTWOSNumChunk * 4);
                else if ((chunk_type == FOURCC('c', 'o', '6', '4')) && (chunk_data_size < 8 + mTWOSNumChunk * 8))
                    ALOGD("\033[0;31m [%d]  wrong co64 size, chunk_data_size= %d, real_size= %d  \033[m", __LINE__, (uint32_t)chunk_data_size, 8 + mTWOSNumChunk * 8);

                mLastTrack->sampleTable->getMaxSampleSize(&max_size);

                for (unsigned int i = 0; i < mTWOSNumChunk; i++)
                {
                    if (chunk_type == FOURCC('s', 't', 'c', 'o'))
                    {
                        mDataSource->readAt(data_offset + 8 + i * 4, header, 4);
                        mTWOSChunkOffsets[i] = U32_AT(&header[0]);
                    }
                    else if (chunk_type == FOURCC('c', 'o', '6', '4'))
                    {
                        mDataSource->readAt(data_offset + 8 + i * 8, header, sizeof(header));
                        mTWOSChunkOffsets[i] = (uint32_t)U64_AT(header);
                    }

                    if (i == mTWOSNumChunk -1) // last audio packet
                    {
                        off64_t last_audio_offset = 0;
                        size_t audio_size = 0;
                        mLastTrack->sampleTable->getMetaDataForSample(mLastTrack->sampleTable->countSamples() - 1, &last_audio_offset, &audio_size, NULL);
                        mTWOSChunkSizes[i] = (uint32_t)(last_audio_offset + audio_size) - mTWOSChunkOffsets[i];
                        //ALOGD("\033[0;33m [%d]  mTWOSChunkOffsets[%d]= %d   mTWOSChunkSizes[%d]= %d   last_audio_offset= %d, audio_size= %d\033[m", __LINE__, mTWOSNumChunk - 1, mTWOSChunkOffsets[mTWOSNumChunk - 1], mTWOSNumChunk - 1, mTWOSChunkSizes[mTWOSNumChunk - 1], (int)last_audio_offset, (int)audio_size);
                    }
                    else
                    {
                        mTWOSChunkSizes[i] = (uint32_t)(max_size * mTWOSsamplesPerChunk);
                        //ALOGD("\033[0;32m [%d]  (idx: %d)  max_size= %d, mTWOSsamplesPerChunk= %d\033[m", __LINE__, i, (int)max_size, mTWOSsamplesPerChunk);
                    }
                    if (mTWOSChunkSizes[i] > mTWOSMaxPacketSize)
                        mTWOSMaxPacketSize = mTWOSChunkSizes[i];
                    //ALOGD("\033[0;33m [%d]  mTWOSChunkOffsets[%d]= %d   mTWOSChunkSizes[%d]= %d\033[m", __LINE__, i, mTWOSChunkOffsets[i], i, mTWOSChunkSizes[i]);
                }
                mLastTrack->meta->setInt32(kKeyMaxInputSize, mTWOSMaxPacketSize + 10 * 2);
            }
            else if (mLastTrack->mIsSOWTAudio > 0) // LPCM, little endian
            {
                uint8_t header[8] = {0};
                size_t max_size = 0, SamplesPerChunk = 0;
                int32_t a_chs = 0, a_bps = 0;

                if (mDataSource->readAt(data_offset, header, sizeof(header)) < (ssize_t)sizeof(header))
                    ALOGD("\033[0;31m [%d]  read stco or co64 data fail...  \033[m", __LINE__);

                mSOWTNumChunk = U32_AT(&header[4]);
                if (!mSOWTChunkOffsets && !mSOWTChunkSizes)
                {
                    mSOWTChunkOffsets = (uint32_t *)malloc(mSOWTNumChunk * sizeof(uint32_t));
                    memset(&mSOWTChunkOffsets[0], 0, mSOWTNumChunk * sizeof(uint32_t));
                    mSOWTChunkSizes = (uint32_t *)malloc(mSOWTNumChunk * sizeof(uint32_t));
                    memset(&mSOWTChunkSizes[0], 0, mSOWTNumChunk * sizeof(uint32_t));
                }

                if ((chunk_type == FOURCC('s', 't', 'c', 'o')) && (chunk_data_size < 8 + mSOWTNumChunk * 4))
                    ALOGD("\033[0;31m [%d]  wrong stco size, chunk_data_size= %d, real_size= %d  \033[m", __LINE__, (uint32_t)chunk_data_size, 8 + mSOWTNumChunk * 4);
                else if ((chunk_type == FOURCC('c', 'o', '6', '4')) && (chunk_data_size < 8 + mSOWTNumChunk * 8))
                    ALOGD("\033[0;31m [%d]  wrong co64 size, chunk_data_size= %d, real_size= %d  \033[m", __LINE__, (uint32_t)chunk_data_size, 8 + mSOWTNumChunk * 8);

                mLastTrack->meta->findInt32(kKeyChannelCount, &a_chs);
                mLastTrack->meta->findInt32(kKeyAudioBitsPerSample, &a_bps);

                if(mLastTrack->mIsSOWTAudio==3) {
                    uint32_t sumOfSamples = 0;
                    for (unsigned int i = 0; i < mSOWTNumChunk; i++) {
                        status_t err = mLastTrack->sampleTable->getSOWTsamplesPerChunkSize(i, &SamplesPerChunk);
                        if (err == OK) {
                            sumOfSamples += SamplesPerChunk;
                        }
                    }
                    int64_t durationUs;
                    int32_t sampleRate;
                    if (mLastTrack->meta->findInt64(kKeyDuration, &durationUs) && mLastTrack->meta->findInt32(kKeySampleRate, &sampleRate)) {
                        if (durationUs > 0 && sampleRate > 0) {
                            int32_t num_channels = (((int32_t)sumOfSamples) / ((int32_t)(durationUs/1000000))) / sampleRate;
                            mLastTrack->meta->setInt32(kKeyChannelCount, num_channels);
                            ALOGD("\033[0;33m [%d] num_channels changes from %d to %d \033[m", __LINE__, a_chs, num_channels);
                            a_chs = num_channels;
                            //ALOGD("\033[0;32m [%d] sumOfSamples= %d, durationUs= %lld, sampleRat= %d, num_channels= %d,  \033[m", __LINE__, sumOfSamples, durationUs, sampleRate, a_chs);
                        }
                    }
                }

                mLastTrack->sampleTable->getMaxSampleSize(&max_size);
                //ALOGD("\033[0;33m [%d]  mSOWTsamplesPerChunk= %d  max_size= %d, a_chs= %d, a_bps= %d\033[m", __LINE__, mSOWTsamplesPerChunk, (int)max_size, a_chs, a_bps);
                ...
                for (unsigned int i = 0; i < mSOWTNumChunk; i++)
                {
                    ...
                    {
                        mDataSource->readAt(data_offset + 8 + i * 4, header, 4);
                        mSOWTChunkOffsets[i] = U32_AT(&header[0]);
                    }
                    else if (chunk_type == FOURCC('c', 'o', '6', '4'))
                    {
                        mDataSource->readAt(data_offset + 8 + i * 8, header, sizeof(header));
                        mSOWTChunkOffsets[i] = (uint32_t)U64_AT(header);
                    }

                    if (mSOWTsamplesPerChunk != 0) {
                        if (max_size == 1)
                        {
                            mSOWTChunkSizes[i] = (uint32_t)(a_chs * (a_bps >> 3) * mSOWTsamplesPerChunk);
                        }
                        else
                        {
                            mSOWTChunkSizes[i] = (uint32_t)(max_size * mSOWTsamplesPerChunk);
                        }
                    }
                    else
                    {
                        SamplesPerChunk = 0;
                        err = mLastTrack->sampleTable->getSOWTsamplesPerChunkSize(i, &SamplesPerChunk);
                        if (err != OK)
                            ALOGD("\033[0;31m [%d]  Wrong getSOWTsamplesPerChunkSize value= %d  \033[m", __LINE__, (int)SamplesPerChunk);
                        else
                        {
                            if (audio_ver1_header_present_flag && audio_samples_per_packet && audio_bytes_per_frame)
                                mSOWTChunkSizes[i] = (uint32_t)((SamplesPerChunk / audio_samples_per_packet) * audio_bytes_per_frame);
                            else
                                mSOWTChunkSizes[i] = (uint32_t)(SamplesPerChunk * a_chs * (a_bps >> 3));
                        }
                    }

                    if (mSOWTChunkSizes[i] > mSOWTMaxPacketSize)
                        mSOWTMaxPacketSize = mSOWTChunkSizes[i];
                    //ALOGD("\033[0;33m [%d]  mSOWTChunkOffsets[%d]= %d   mSOWTChunkSizes[%d]= %d\033[m", __LINE__, i, mSOWTChunkOffsets[i], i, mSOWTChunkSizes[i]);
                }
                mLastTrack->meta->setInt32(kKeyMaxInputSize, mSOWTMaxPacketSize + 10 * 2);
                //ALOGD("\033[0;32m [%d]  kKeyMaxInputSize        mSOWTMaxPacketSize= %d  \033[m", __LINE__, mSOWTMaxPacketSize);
            }
#endif
...
#ifdef RTK_PLATFORM
            if (mLastTrack->mIsTWOSAudio || mLastTrack->mIsSOWTAudio > 0)
            {
                uint8_t header[4];
                mDataSource->readAt(data_offset + 8 + 4, header, sizeof(header));
                if (mLastTrack->mIsTWOSAudio)
                    mTWOSsamplesPerChunk = U32_AT(&header[0]);
                if (mLastTrack->mIsSOWTAudio > 0)
                {
                    if (chunk_size == 28) // only 1 entry
                        mSOWTsamplesPerChunk = U32_AT(&header[0]);
                    else
                        mSOWTsamplesPerChunk = 0; // not fix samplePerChunk value in stsc
                }
            }
#endif
            break;
        ...
        case FOURCC('s', 't', 'z', '2'):
        {
            ...
            *offset += chunk_size;
            ...
            size_t max_size;
#ifdef RTK_PLATFORM
            uint32_t stsz_sample_size = 0, stsz_sample_count = 0;
            int32_t chs = 0;
            if (mLastTrack->mIsMLAWAudio)
            {
                err = mLastTrack->sampleTable->get_stszInfo(&stsz_sample_size, &stsz_sample_count);
                mLastTrack->meta->findInt32(kKeyChannelCount, &chs);
                stsz_sample_size = chs;

                err = mLastTrack->sampleTable->getMaxsamplesPerChunkSize(&max_size);
                if (!(stsz_sample_size == 0 && stsz_sample_count > 0))
                {
                    max_size *= stsz_sample_size;
                    mLastTrack->sampleTable->setAudioSampleSize(stsz_sample_size);
                }
                else
                    ALOGE("\033[0;33m [%d]  SampleConstSize = 0, need to re-calc chunksize\033[m", __LINE__);
            }
            else
#endif
            ...
            if (!strncasecmp("video/", mime, 6)) {
                ...
                if (nSamples == 0) {
...
#ifdef RTK_PLATFORM
                                    float fFrameRate = (float)mLastTrack->timescale / (float)t->default_sample_duration;
                                    mLastTrack->meta->setFloat(kKeyFrameRate, fFrameRate);
#endif
                                    int32_t frameRate =
                    ...
                    if (mLastTrack->meta->findInt64(kKeyDuration, &durationUs)) {
                        if (durationUs > 0) {
#ifdef RTK_PLATFORM
                            float fFrameRate = (float)(nSamples * 1000000LL) / (float)durationUs;
                            mLastTrack->meta->setFloat(kKeyFrameRateFloat, fFrameRate);
#endif
...
#ifdef RTK_PLATFORM
                ALOGD("\033[0;33m [%d] location_length= %d >= sizeof(buffer)= %d, it should continue parse next chunk instead of return ERROR_MALFORMED...\033[m", __LINE__, (int)location_length, (int)sizeof(buffer));
                break;
#else
                return ERROR_MALFORMED;
#endif
...
#ifdef RTK_PLATFORM
                    if ((objectTypeIndication >= 0x60 && objectTypeIndication <= 0x65) || (objectTypeIndication == 0x6A)) {
#else
        ...
        case FOURCC('m', 'e', 't', 'a'):
        {
            ...
            *offset = data_offset;
#ifdef RTK_PLATFORM
            if (mPath.size() >= 3 && mPath[1] == FOURCC('t', 'r', 'a', 'k') && mPath[2] == FOURCC('m', 'e', 't', 'a'))
                mMetaFoundAfterTrak = true;

            bool isParsingMetaKeys = mMetaFoundAfterTrak ? underQTMetaPath(mPath, 3) : underQTMetaPath(mPath, 2);
#else
...
#ifdef RTK_PLATFORM
            if (mMetaFoundAfterTrak ? underQTMetaPath(mPath, 4) : underQTMetaPath(mPath, 3)) {
#else
        ...
        {
#ifdef RTK_PLATFORM
            //ALOGD("\033[0;32m [%d]  Unknown  chunk_type= 0x%X,   chunk_size= %lld    chunk_data_size= %lld  \033[m", __LINE__, chunk_type, (long long)chunk_size, (long long)chunk_data_size);
            if (mMetaFoundAfterTrak ? underQTMetaPath(mPath, 4) : underQTMetaPath(mPath, 3)) {
#else
...
MPEG4Source::MPEG4Source(
...
#ifdef RTK_PLATFORM
    mIsMLAW = !strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_G711_MLAW);
    mIsTWOS = !strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_PCM);
    mIsSOWT = !strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_RAW);
    mIsALAC = !strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_ALAC);
    mIsAVS = !strcasecmp(mime, MEDIA_MIMETYPE_VIDEO_AVS);
#endif
    ...
    if (mIsHEVC) {
        ...
        // The number of bytes used to encode the length of a NAL unit.
        mNALLengthSize = 1 + (ptr[21] & 3);	// lenghtSizeMinusOne + 1
    }
...
#ifdef RTK_PLATFORM
status_t MPEG4Source::parseChunkAVS(uint8_t *data, size_t size) {
    size_t offset = 0;
    size_t chunk_size = U32_AT(&data[offset]);
    while(chunk_size>0 && chunk_size<size && offset+4+chunk_size<=size) {
        //sending avs startcode
        data[offset++] = 0;
        data[offset++] = 0;
        data[offset++] = 0;
        data[offset++] = 0;
        data[offset] = 0x01;
        offset += chunk_size;
        chunk_size = U32_AT(&data[offset]);
    }
    ALOGV("[%d] parseChunkAVS: size=%zu", __LINE__, offset);
    return OK;
}

bool MPEG4Source::isKeyFrameAVS(const uint8_t *data, size_t size) {
    size_t offset = 0;
    size_t chunk_size = U32_AT(&data[offset]);
    bool ret = false;
    while(chunk_size>0 && chunk_size<size && offset+4+chunk_size<=size) {
        ...
        if(data[offset] == 0x05) {
            ALOGV("[%d] isKeyFrameAVS: offset=%zu", __LINE__, offset);
            ret = true;
            break;
        }
        offset += chunk_size;
...
uint32_t MPEG4Source::findSyncSampleNearAVS(uint32_t sample_index, uint32_t flags __unused) {
    int32_t count = sample_index;
    uint32_t syncSampleNear = 0;
    off64_t offset = 0;
    size_t size = 0;
    ...
    while(count-->0) {
        mSampleTable->getMetaDataForSample(count, &offset, &size, NULL, NULL, NULL);
        num_bytes_read = mDataSource->readAt(offset, data, min(dataSize, size));
        if(isKeyFrameAVS(data, num_bytes_read)) {
            syncSampleNear = count;
            break;
        }
    }
    ALOGD("[%d] findSyncSampleNearAVS: sample_index=%d, findSyncSampleNear=%d", __LINE__, sample_index, syncSampleNear);
    return syncSampleNear;
}
...
status_t MPEG4Source::read(
    ...
    *out = NULL;
    ...
    off64_t offset = 0;
    size_t size = 0;
    uint64_t cts = 0, stts = 0;
    bool isSyncSample = false;
    bool isHEVCSeek = false;
    status_t err;
    ...
    if (options && options->getSeekTo(&seekTimeUs, &mode)) {
...
#ifdef RTK_PLATFORM
        ALOGD("\033[0;31m [%d] seekTimeUs= %lld\033[m", __LINE__, (long long)seekTimeUs);
        uint32_t sampleIndex;
        err = mSampleTable->findSampleAtTime(seekTimeUs, 1000000, mTimescale, &sampleIndex, findFlags);
#else
        uint32_t sampleIndex;
        ...
        if (err == OK) {
#ifdef RTK_PLATFORM
            if (mIsHEVC && (mSampleTable->stsscount() == 1))
            {
                // find key frame index
                err = mSampleTable->findChunkSyncSampleNear(sampleIndex, &syncSampleIndex);
                ALOGD("\033[0;32m [%d] sampleIndex= %d   syncSampleIndex= %d\033[m", __LINE__, sampleIndex, syncSampleIndex);
            }
            else
#endif
...
#ifdef RTK_PLATFORM
            if(mIsAVS) {
                syncSampleIndex = findSyncSampleNearAVS(syncSampleIndex, findFlags);
            }
#endif
        }
        ...
        if (err == OK) {
            ...
            {
                int64_t tmp_ptsUs = 0;
                unsigned int mNumChunk = 0;

                sampleTime = 0;
                err = mSampleTable->getMetaDataForSample(sampleIndex, NULL, &size, NULL, NULL, &stts);

                if (mIsTWOS)
                {
                    stts = mOwner->mTWOSsamplesPerChunk;
                    mNumChunk = mOwner->mTWOSNumChunk;
                }
                else if (mIsSOWT)
                {
                    mNumChunk = mOwner->mSOWTNumChunk;
                    if (mOwner->mSOWTsamplesPerChunk != 0)
                        stts = mOwner->mSOWTsamplesPerChunk;
                    else
                        mOwner->mCurrcts = 0;
                }

                cts = mCurrentSampleIndex * stts;
                for (unsigned int i = 0; i < mNumChunk; i++)
                {
                    if (mIsSOWT && mOwner->mSOWTsamplesPerChunk == 0)
                        tmp_ptsUs = (int64_t)(mOwner->mCurrcts) * 1000000 / (int64_t)mTimescale;
                    else
                        tmp_ptsUs = (int64_t)(stts * i) * 1000000 / (int64_t)mTimescale;
                    //ALOGD("\033[0;33m [%d] tmp_ptsUs= %lld, mTimescale= %d  mTWOSChunkSizes[%d]= %d\033[m", __LINE__, (long long)tmp_ptsUs, mTimescale, i, mOwner->mTWOSChunkSizes[i]);
                    if (tmp_ptsUs > seekTimeUs)
                    {
                        if (mIsSOWT && mOwner->mSOWTsamplesPerChunk == 0)
                            mOwner->mCurrcts -= stts;
                        syncSampleIndex = i -1;
                        ALOGD("\033[0;33m [%d] syncSampleIndex= %d, tmp_ptsUs= %lld,  seekTimeUs= %lld\033[m", __LINE__, syncSampleIndex, (long long)tmp_ptsUs, (long long)seekTimeUs);
                        break;
                    }
                    ...
                    {
                        stts = 0;
                        err = mSampleTable->getSOWTsamplesPerChunkSize(i, (size_t *)&stts);
                        if (err != OK)
                        {
                            ALOGD("\033[0;31m [%d]  Wrong getSOWTsamplesPerChunkSize value= %llu   mCurrentSampleIndex= %d\033[m", __LINE__, (long long)stts, mCurrentSampleIndex);
                            return ERROR_END_OF_STREAM;
                        }
                        mOwner->mCurrcts += stts;
                    }
        ...
        if (mIsHEVC)
            isHEVCSeek = true;
        mOwner->mNextAudioOffset = 0;
        mOwner->mNextVideoOffset = 0;
        mOwner->mWouldBlockCount = 0;
#endif
        // fall through
    }
...
read_again:
        if (mIsMLAW) {
            err = mSampleTable->getMetaDataForMLAWSample(mCurrentSampleIndex, &offset, &size, &cts, &isSyncSample, &stts);
            mSampleTable->getMetaDataForMLAWSample(mCurrentSampleIndex+1, &nextOffset, NULL, NULL, NULL, NULL);
        }
        else if (mIsTWOS || mIsSOWT)
        {
            if (mIsTWOS && (mCurrentSampleIndex >= mOwner->mTWOSNumChunk))
                return ERROR_END_OF_STREAM;
            else if (mIsSOWT && (mCurrentSampleIndex >= mOwner->mSOWTNumChunk))
                return ERROR_END_OF_STREAM;

            err = mSampleTable->getMetaDataForSample(mCurrentSampleIndex, &offset, &size, &cts, &isSyncSample, &stts);
            mSampleTable->getMetaDataForSample(mCurrentSampleIndex+1, &nextOffset, NULL, NULL, NULL, NULL);
            if (err == OK)
            {
                if (mIsTWOS)
                {
                    offset = mOwner->mTWOSChunkOffsets[mCurrentSampleIndex];
                    stts = mOwner->mTWOSsamplesPerChunk;
                    size = mOwner->mTWOSChunkSizes[mCurrentSampleIndex];
                }
                else if (mIsSOWT)
                {
                    offset = mOwner->mSOWTChunkOffsets[mCurrentSampleIndex];
                    if (mOwner->mSOWTsamplesPerChunk != 0)
                        stts = mOwner->mSOWTsamplesPerChunk;
                    else
                    {
                        err = mSampleTable->getSOWTsamplesPerChunkSize(mCurrentSampleIndex, (size_t *)&stts);
                        if (err != OK)
                            ALOGD("\033[0;31m [%d]  Wrong getSOWTsamplesPerChunkSize value= %llu   mCurrentSampleIndex= %d\033[m", __LINE__, (long long)stts, mCurrentSampleIndex);
                    }
                    size = mOwner->mSOWTChunkSizes[mCurrentSampleIndex];
                }
                isSyncSample = true;
                if (mIsSOWT && (mOwner->mSOWTsamplesPerChunk == 0))
                {
                    ...
                    cts = mCurrentSampleIndex * stts;
                //ALOGD("\033[0;32m [%d] mCurrentSampleIndex= %d, offset= %lld,  size= %d, cts= %llu, isSyncSample= %d, stts= %llu  mTimescale= %d \033[m", __LINE__, mCurrentSampleIndex, (long long)offset, (int)size, (long long)cts, isSyncSample, (long long)stts, mTimescale);
            }
        }
        else {
            err = mSampleTable->getMetaDataForSample(mCurrentSampleIndex, &offset, &size, &cts, &isSyncSample, &stts);
        ...
        //ALOGD("\033[0;33m [%d] mCurrentSampleIndex= %d, offset= %lld,  size= %d, cts= %llu, isSyncSample= %d, stts= %llu  mTimescale= %d \033[m", __LINE__, mCurrentSampleIndex, (long long)offset, (int)size, (long long)cts, isSyncSample, (long long)stts, mTimescale);
#else
        status_t err =
    ...
    if (mDataSource->flags() & DataSource::kIsCachingDataSource)
    {
        ...
        if(!strncasecmp(mime, "audio/", 6)) {
            if(mOwner->mNextVideoOffset > 0 && offset > mOwner->mNextVideoOffset && (((int64_t)cts * 1000000) / mTimescale) > 1000000) {
                mBuffer->release();
                mBuffer = NULL;
                if((++mOwner->mWouldBlockCount%1000)==0 && mOwner->mWouldBlockCount < 10000) {
                    ALOGD("Audio returns WOULD_BLOCK more than %d times.", mOwner->mWouldBlockCount);
                }
                mOwner->mNextAudioOffset = offset;
                //ALOGD("\033[1;35moffset:%lld, mime %s, mCurrentSampleIndex %d, mNextVideoOffset:%lld, mNextAudioOffset:%lld \033[m\n", offset, mime, mCurrentSampleIndex , mOwner->mNextVideoOffset, mOwner->mNextAudioOffset);
                return WOULD_BLOCK;
            }
            mOwner->mNextAudioOffset = nextOffset;
            mOwner->mWouldBlockCount = 0;
        }
        else if(!strncasecmp(mime, "video/", 6)) {
            if(mOwner->mNextAudioOffset > 0 && offset > mOwner->mNextAudioOffset) {
                mBuffer->release();
                mBuffer = NULL;
                if((++mOwner->mWouldBlockCount%1000)==0 && mOwner->mWouldBlockCount < 10000) {
                    ALOGD("Video returns WOULD_BLOCK more than %d times.", mOwner->mWouldBlockCount);
                }
                mOwner->mNextVideoOffset = offset;
                //ALOGD("\033[1;35moffset:%lld, mime %s, mCurrentSampleIndex %d, mNextVideoOffset:%lld, mNextAudioOffset:%lld \033[m\n", offset, mime, mCurrentSampleIndex , mOwner->mNextVideoOffset, mOwner->mNextAudioOffset);
                return WOULD_BLOCK;
            }
            mOwner->mNextVideoOffset = nextOffset;
            mOwner->mWouldBlockCount = 0;
        }
    }
#endif
    ...
    if ((!mIsAVC && !mIsHEVC) || mWantsNALFragments) {
        if (newBuffer) {
            ...
            int header_size = 0;
            if(mIsALAC) {
                uint8_t* buffer = (uint8_t *)mBuffer->data();
                header_size = ALAC_HEADER_SIZE;
                // copy ALAC sync
                memcpy(buffer, ALAC_SYNCH_ASCII, 4);
                // copy packet length
                memcpy(buffer + 4, &size, 4);
            }
            ssize_t num_bytes_read =
                mDataSource->readAt(offset, (uint8_t *)mBuffer->data() + header_size, size);
#else
            ssize_t num_bytes_read =
            ...
            size += header_size;
            if(mIsAVS) {
                parseChunkAVS((uint8_t *)mBuffer->data(), size);
            }
#endif
        ...
        *out = clone;
        ...
        if (usesDRM) {
            ...
            while (srcOffset < size) {
                ...
                dstData[dstOffset++] = 1;
#ifdef RTK_PLATFORM
                if (mIsHEVC && isHEVCSeek)
                {
                    int sliceType = (mSrcBuffer[srcOffset] & 0x7E) >> 1;
                    if (sliceType >= 19 && sliceType <= 21) // NAL type (IDR,CRA,BLA)
                    {
                        ALOGD("\033[0;33m [%d] Find sliceType= %d \033[m", __LINE__, (int)sliceType);
                        isHEVCSeek = false;
                    }
                    else
                    {
                        //ALOGD("\033[0;32m [%d] sliceType= %d \033[m", __LINE__, (int)sliceType);
                        if (isHEVCSeek && ((srcOffset + nalLength) >= size))
                        {
                            mBuffer->release();
                            mBuffer = NULL;
                            mCurrentSampleIndex++;
        ...
        mBuffer->meta_data()->setInt64(
                kKeyDuration, ((int64_t)stts * 1000000) / mTimescale);
#ifdef RTK_PLATFORM
        //ALOGD("\033[0;32m [%d] pts= %d   frame_time= %d   targetSampleTimeUs= %d \033[m", __LINE__, (int)(((int64_t)cts * 1000000) / mTimescale), (int)(((int64_t)stts * 1000000) / mTimescale), (int)targetSampleTimeUs);
#endif
