...
void SoftwareRenderer::resetFormatIfChanged(const sp<AMessage> &format) {
    ...
    if (!runningInEmulator()) {
        switch (mColorFormat) {
            ...
            case OMX_TI_COLOR_FormatYUV420PackedSemiPlanar: //NV12
            {
                halFormat = HAL_PIXEL_FORMAT_YCbCr_420_SP; //NV12
                bufWidth = (mCropWidth + 1) & ~1;
                bufHeight = (mCropHeight + 1) & ~1;
                break;
            }
            case OMX_RTK_COLOR_FormatYVU420SemiPlanar: //NV21
            {
                halFormat = HAL_PIXEL_FORMAT_YCrCb_420_SP; //NV21
                bufWidth = (mCropWidth + 1) & ~1;
                bufHeight = (mCropHeight + 1) & ~1;
                break;
            }
    ...
    ALOGV("%s: mColorFormat:%x halFormat:%x", __FUNCTION__, mColorFormat, halFormat);
...
#else //RTK
    } else if (mColorFormat == OMX_COLOR_FormatYUV420SemiPlanar) { //NV12, copy directly
        size_t dst_y_size = buf->stride * buf->height;
        size_t dst_c_stride = ALIGN(buf->stride, 16);
        size_t dst_c_size = dst_c_stride * buf->height / 2;
        memcpy(dst, data, dst_y_size + dst_c_size);
    } else if (mColorFormat == OMX_RTK_COLOR_FormatYVU420SemiPlanar) { //NV21, copy directly
        size_t dst_y_size = buf->stride * buf->height;
        size_t dst_c_stride = ALIGN(buf->stride, 16);
        size_t dst_c_size = dst_c_stride * buf->height / 2;
        memcpy(dst, data, dst_y_size + dst_c_size);
