...
void SoftAACEncoder2::initPorts() {
    ...
    //revert to 8k for cts testConcurrentAudioVideoEncodings
    //If audio is multi-channl or high sample rate, it should set max-input-size and adjust nBufferSize in ACodec.
    def.nBufferSize = kNumSamplesPerFrame * sizeof(int16_t) * 8;
...
OMX_ERRORTYPE SoftAACEncoder2::internalSetParameter(
    ...
    switch (index) {
        ...
        case OMX_IndexParamAudioAac:
        {
            ...
            ALOGI("[%s %d OMX_IndexParamAudioAac] Ch %d Rate %d\n", __FUNCTION__, __LINE__, mNumChannels, mSampleRate);

            if (mNumChannels != mInputChannels) {
                ALOGI("mUseRemix\n");
                mUseRemix = true;
            }
            if (mSampleRate != mInputRate) {
                if (mResampler == NULL)
                    int ret = create_resampler(mInputRate,
                            mSampleRate,
                            mInputChannels,
        ...
        case OMX_IndexParamAudioPcm:
        {
            ...
            ALOGI("[%s %d OMX_IndexParamAudioPcm] Ch %d Rate %d\n", __FUNCTION__, __LINE__, mNumChannels, mSampleRate);
            mInputChannels = pcmParams->nChannels;
            mInputRate = pcmParams->nSamplingRate;
    ...
    for (;;) {
        ...
        while (mInputSize < numBytesPerInputFrame) {
            ...
            if (inHeader->nOffset == 0) {
                size_t frame_size = sizeof(int16_t) * mInputChannels;
                size_t in_frames = inHeader->nFilledLen / frame_size;
                size_t out_frames = 0;
                size_t out_frame_size = 0;
                void *out_buffer = NULL;
                void *out_buffer_remix = NULL;
                void *tmpBuffer = (unsigned char *)inHeader->pBuffer;

                if (mUseResample) {
                    out_frames = (in_frames * mSampleRate) / mInputRate + 1;
                    out_buffer = (unsigned char *)malloc(out_frames * frame_size);
                    memset(out_buffer, 0, out_frames * frame_size);

                    mResampler->resample_from_input(mResampler, (int16_t *)tmpBuffer, &in_frames, (int16_t *)out_buffer, &out_frames);
                    tmpBuffer = (unsigned char*)out_buffer;
                } else {
                    out_frames = in_frames;
                }

                inHeader->nFilledLen = out_frames * frame_size;

                if (mUseRemix) {
                    const size_t required_conversion_buffer_size = inHeader->nFilledLen * mNumChannels / mInputChannels;
                    out_buffer_remix = (unsigned char *) malloc(required_conversion_buffer_size);
                    size_t out_bytes = adjust_channels(tmpBuffer, mInputChannels,
                            out_buffer_remix, mNumChannels, sizeof(int16_t), inHeader->nFilledLen);
                    inHeader->nFilledLen = out_bytes;
                    tmpBuffer = (unsigned char*)out_buffer_remix;
                }

                if (mUseResample || mUseRemix) {
                    mPPBuffer = (unsigned char*) malloc(inHeader->nFilledLen);
                    memcpy(mPPBuffer, tmpBuffer, inHeader->nFilledLen);
                }
                ...
                if (out_buffer_remix != NULL) {
                    free(out_buffer_remix);
                    out_buffer_remix = NULL;
                }
            }
            const void *inData;
            if (mPPBuffer != NULL)
                inData = mPPBuffer + inHeader->nOffset;
            else
                inData = inHeader->pBuffer + inHeader->nOffset;
#else
...
//rtk modify start
            if (inHeader->nTimeStamp < mLastTimeUs) {
                inHeader->nTimeStamp = mLastTimeUs;
            } else {
                mLastTimeUs = inHeader->nTimeStamp;
            }
//rtk modify end
            if (mInputSize == 0) {
