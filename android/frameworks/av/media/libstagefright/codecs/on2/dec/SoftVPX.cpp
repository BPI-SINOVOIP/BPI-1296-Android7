...
#ifdef RTK_MONITOR_DECODE_TIME
    pdbg = (MoniorDecodeTimeGlobal *)malloc(sizeof(MoniorDecodeTimeGlobal));
    if(pdbg)
        memset(pdbg, 0, sizeof(MoniorDecodeTimeGlobal));
#endif
}
...
SoftVPX::~SoftVPX() {
    ...
    if(pdbg)
    {
        ...
        ALOGD("total %d frames decoded, %d frames (%d%%) decoded > %d(us)\n", pdbg->decode_count, pdbg->decode_too_long, pdbg->decode_too_long*100/pdbg->decode_count, DECODE_TIME_THRESHOLD);
        ALOGD("average decode time: %.3f (ms)\n",pdbg->avg_decode_time);
        ALOGD("############################################################################");
        free(pdbg);
    }
    pdbg = NULL;
...
//#define RTK_MONITOR_DECODE_TIME
...
#ifdef RTK_MONITOR_DECODE_TIME
        long long start_time = systemTime(SYSTEM_TIME_MONOTONIC)/1000;
#endif
...
#ifdef RTK_MONITOR_DECODE_TIME
        long long delta_time = (systemTime(SYSTEM_TIME_MONOTONIC)/1000) - start_time;//us

        if(pdbg)
        {
            pdbg->time_accumulate += (delta_time/1000);//ms
            pdbg->decode_count += 1;
            pdbg->avg_decode_time = (double)(pdbg->time_accumulate)/pdbg->decode_count;

            if(delta_time > DECODE_TIME_THRESHOLD)//50ms
            {
                pdbg->decode_too_long += 1;
                //ALOGD("[%s %d] %lld (us), %d, %d, %.2f (ms)\n",__FUNCTION__,__LINE__, delta_time, pdbg->decode_count, pdbg->decode_too_long, pdbg->avg_decode_time);
            }
        }
#endif
