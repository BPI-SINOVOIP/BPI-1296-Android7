...
ssize_t HTTPDownloader::fetchBlock(
    ...
    for (;;) {
        ...
        if(endTimeUs - startTimeUs > 8 * nOverTimeCount * 1E6)
        {
            ALOGW("[%s][%d] Read over time! need size:%d n:%d nOverTimeCount:%d.\n",__FUNCTION__,__LINE__, (int)size, (int)n, nOverTimeCount);
            nOverTimeCount++;
            if(nOverTimeCount > 9)
            {
                ALOGW("[%s][%d] Read over time! need size:%d n:%d nOverTimeCount:%d.\n",__FUNCTION__,__LINE__, (int)size, (int)n, nOverTimeCount);
