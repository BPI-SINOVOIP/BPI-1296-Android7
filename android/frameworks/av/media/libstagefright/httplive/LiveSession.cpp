...
#include <inttypes.h>
#ifdef RTK_PLATFORM
#define SUPPORT_BALANCE_DEQUEUE_SPEED
...
const int64_t LiveSession::kPrepareMarkUs = 500000ll;
const int64_t LiveSession::kUnderflowMarkUs = 100000ll;
const static int32_t kPlaylistStatusReTryNumber = 10;
const static int32_t kLiveSessionPTSCheckNumber = 1000;
#else
const int64_t LiveSession::kReadyMarkUs = 5000000ll;
...
#ifdef SUPPORT_BALANCE_DEQUEUE_SPEED
const static int64_t kForceWaitOtherSourceTimeGapUs = 3000000ll;
const static int32_t kBufferWaitTimeOut = 64;
#endif
...
#endif
#ifdef RTK_PLATFORM
    mInStartPlayBackFstChkBuffer = false;
    mBufInfoCnt.up = 0;
    mBufInfoCnt.down = 0;
    mBufInfoCnt.underflow = 0;
    mBufInfoCnt.bandwidthlow = 0;
    mOffsetTimeUs = -1;
    mPlaylist = NULL;
    mSwitchTimeoutCnt = 0;
    mInSwitchPhase = false;
    mInSwitchPreparationPhase = false;
...
#ifdef RTK_PLATFORM
    while(!mBufInfoQue.empty())
    {
        mBufInfoQue.erase(mBufInfoQue.begin());
    }
#endif
...
#ifdef RTK_PLATFORM
void LiveSession::onRTKCustomerHighWaterMBsChk()
{
    char value[PROPERTY_VALUE_MAX]={0};
    double ctrl = 0;
    if (property_get(RTK_PROPERTY_MEDIA_BUFFER, value, NULL))
    {
        ctrl = atof(value);
        if(ctrl > 0)
        {
            kHighWaterMarkBytes = ((ctrl*1000000) / (double(1280 * 720 * 1.5)/1000000));
            ALOGD("[%s][%d] set RTK_PROPERTY_MEDIA_BUFFER :: %f MBytes %zd Us\n",__FUNCTION__,__LINE__, ctrl, kHighWaterMarkBytes);
        }
        else
            kHighWaterMarkBytes = kReadyMarkUs;
    }
    else
        kHighWaterMarkBytes = kReadyMarkUs;
}
void LiveSession::reconnectUri(int32_t dLineInfo)
{
    mSwitchTimeoutCnt++;
    if(mInPreparationPhase == true)
        return;

    ALOGW("[%s][%d] need reconnect http, mSwitchTimeoutCnt::%d.\n",__FUNCTION__,dLineInfo, mSwitchTimeoutCnt);

#if defined (RTK_ENB_QUEUE_DEBUG_LOG)
    ssize_t streamIdx = typeToIndex(STREAMTYPE_VIDEO);
    ssize_t index = mFetcherInfos.indexOfKey(mStreams[streamIdx].mUri);
    LogSeqInfo();
    index = mFetcherInfos.indexOfKey(mStreams[streamIdx].mUri);
    if(index >= 0)
        mFetcherInfos.editValueAt(index).mFetcher->LogSeqInfo(1);
#endif
    //finishDisconnect();
    ...
    if(mInSwitchPhase == false)
        mInSwitchPreparationPhase = mInPreparationPhase;
    mInSwitchPhase = true;
    ...
    mFetcherInfos.clear();
    HLSTime curTime = latestMediaSegmentStartTime();
    mSwitchTime = curTime.mTimeUs;
    if(mInPreparationPhase == false)
    {
        mInPreparationPhase = true;
        mInStartPlayBackFstChkBuffer = false;
    }
    ...
    maudioretryCount = 0;
    mvideoretryCount = 0;
    connectAsync(mMasterURL.c_str(), mExtraHeaders.isEmpty() ? NULL : &mExtraHeaders);
}
#endif
...
status_t LiveSession::dequeueAccessUnit(
    ...
    if(maudioTime != -1 && mvideoTime != -1 && mOffloadAudio == false)
    {
        if(mLastSessionFlg == 1 && stream == STREAMTYPE_AUDIO && !mvideoEOS && maudioTime > mvideoTime && maudioTime - mvideoTime > kForceWaitOtherSourceTimeGapUs )
        {
            ALOGV("%s AUDIO FORCED WAIT HERE maudioTime %lld mvideoTime %lld !!!", __FUNCTION__, (long long)maudioTime, (long long)mvideoTime);
            maudioretryCount++;
            if(maudioretryCount < kBufferWaitTimeOut)
                return -EAGAIN;
            else
                ALOGD("%s AUDIO FORCE WAIT TIMEOUT maudioTime %lld mvideoTime %lld count %d !!!", __FUNCTION__, (long long)maudioTime, (long long)mvideoTime, maudioretryCount);
        }
        else if(mLastSessionFlg == 1 && stream == STREAMTYPE_VIDEO && !maudioEOS && mvideoTime > maudioTime && mvideoTime - maudioTime > kForceWaitOtherSourceTimeGapUs)
        {
            ALOGV("%s VIDEO FORCED WAIT HERE audioTime %lld mvideoTime %lld !!!", __FUNCTION__, (long long)maudioTime, (long long)mvideoTime);
            mvideoretryCount++;
            if(mvideoretryCount < kBufferWaitTimeOut)
                return -EAGAIN;
            else
                ALOGD("%s VIDEO FORCE WAIT TIMEOUT maudioTime %lld mvideoTime %lld count %d !!!", __FUNCTION__, (long long)maudioTime, (long long)mvideoTime, mvideoretryCount);
        }
    }
    ...
    if (err == INFO_DISCONTINUITY) {
...
#ifdef RTK_PLATFORM
        (*accessUnit)->meta()->setInt32("IsHLSflw", 1);
#endif
        ...
        if (stream == STREAMTYPE_AUDIO || stream == STREAMTYPE_VIDEO) {
...
#ifdef RTK_PLATFORM
                if(mCurSeqInfo[streamIdx].mIsNeedRebase == true)
                {
                    mCurSeqInfo[streamIdx].mIsNeedRebase = false;
                    offsetTimeUs = mCurSeqInfo[streamIdx].mTimeUs;
                    ALOGI("[%s][%d][DISC] mCurSeqInfo[%d].mIsNeedRebase set false\n",__FUNCTION__,__LINE__, (int)streamIdx);
                }

                size_t trackCount = 0;
                for (size_t i = 0; i < mPacketSources.size(); ++i) {
                    if (!(mStreamMask & mPacketSources.keyAt(i) & (STREAMTYPE_AUDIO | STREAMTYPE_VIDEO)))
                        trackCount++;
                }

                //synchronize AV time offset
                if(trackCount < 2)
                    mOffsetTimeUs = -1;
                else if(stream == STREAMTYPE_AUDIO || stream == STREAMTYPE_VIDEO)
                {
                    if(mOffsetTimeUs == -1)
                        mOffsetTimeUs = offsetTimeUs;
                    else
                    {
                        offsetTimeUs = mOffsetTimeUs;
                        mOffsetTimeUs = -1;
                    }
                }
#endif
            ...
            if(mCurSeqInfo[streamIdx].mIsNeedRebase == true)
            {
                int64_t tt = originalTimeUs + mCurSeqInfo[streamIdx].mOffsetUs;
                ALOGV("[%s][%d][DISC][%s] need ReBase, offset::%5.3f bef::%5.3f Aft::%5.3f Ori::%5.3f\n",__FUNCTION__,__LINE__,
                        (stream == STREAMTYPE_AUDIO)?"AUDIO":"VIDEO",
                        codec_show_pts(mCurSeqInfo[streamIdx].mOffsetUs),
                        codec_show_pts(timeUs),codec_show_pts(tt),codec_show_pts(originalTimeUs));
                timeUs = originalTimeUs + mCurSeqInfo[streamIdx].mOffsetUs;
            }
            if(streamIdx <= 1)
            {
                int32_t curSeqNumber;
                ssize_t dstreamIdx;
                int64_t slDiffUs = 5000000ll;
                int64_t tmpTimeUs = timeUs;
                int64_t slCurTime = (stream == STREAMTYPE_AUDIO)?maudioTime: mvideoTime;
                bool bIsdiscFlg = false;

                if(stream == STREAMTYPE_AUDIO)
                    dstreamIdx = typeToIndex(STREAMTYPE_VIDEO);
                else if(stream == STREAMTYPE_VIDEO)
                    dstreamIdx = typeToIndex(STREAMTYPE_AUDIO);

                mCurSeqInfo[streamIdx].mPreTimeUs = mCurSeqInfo[streamIdx].mTimeUs;
                mCurSeqInfo[streamIdx].mPreOriginalTimeUs = mCurSeqInfo[streamIdx].mOriginalTimeUs;
                mCurSeqInfo[streamIdx].mTimeUs = timeUs;
                mCurSeqInfo[streamIdx].mOriginalTimeUs = originalTimeUs;

                (*accessUnit)->meta()->findInt32("mSeqNumber", &curSeqNumber);

                ssize_t index = mFetcherInfos.indexOfKey(mStreams[streamIdx].mUri);
                int32_t dLineInfo = 0;
                if(slCurTime > 0 && (diff(tmpTimeUs, slCurTime) > slDiffUs))
                {
                    dLineInfo = -1;
                    ALOGI("[%s][%d][DISC] [%d]discontinuity:tmpTimeUs [%s]ori %f  = timeUs %f A:%f V:%f.\n",__FUNCTION__,__LINE__, (int)dstreamIdx,
                            (stream == STREAMTYPE_VIDEO)?"VIDEO":"AUDIO", codec_show_pts(originalTimeUs),
                            codec_show_pts(tmpTimeUs), codec_show_pts(maudioTime), codec_show_pts(mvideoTime));
                    bool IsSeqBoundary = false;
                    //chk PlaylistFetcher table
                    if(index >= 0 && mFetcherInfos.editValueAt(index).mFetcher->chkIsSeqDiscontinuity(streamIdx, dstreamIdx, curSeqNumber, originalTimeUs, &IsSeqBoundary))
                    {
                        bIsdiscFlg = true;
                        dLineInfo = __LINE__;
                    }
                    else if(IsSeqBoundary == true)
                    {
                        //check pre PTS and Pre originalTimeUs.
                        if((diff(tmpTimeUs, slCurTime) > slDiffUs) && diff(mCurSeqInfo[streamIdx].mPreOriginalTimeUs, mCurSeqInfo[streamIdx].mOriginalTimeUs) < slDiffUs)
                        {
                            bIsdiscFlg = true;
                            dLineInfo = __LINE__;
                        }
                        if(bIsdiscFlg == false && curSeqNumber > mCurSeqInfo[dstreamIdx].mCurSeqNumber)
                        {
                            bIsdiscFlg = true;
                            dLineInfo = __LINE__;
                        }
                    }
                    else if(mCurSeqInfo[streamIdx].mCurSeqNumber <= mCurSeqInfo[dstreamIdx].mCurSeqNumber)
                    {
                        if(getSeqdstRebaseOffset(streamIdx, dstreamIdx) == 1)
                        {
                            bIsdiscFlg = true;
                            dLineInfo = __LINE__;
                        }
                    ...
                    if(bIsdiscFlg == true)
                    {
                        ALOGI("[%s][%d][DISC] bIsdiscFlg is true ,Line::%d\n",__FUNCTION__,__LINE__, dLineInfo);
                        mCurSeqInfo[streamIdx].mPreSeqNumber = mCurSeqInfo[streamIdx].mCurSeqNumber;
                        mCurSeqInfo[streamIdx].mCurSeqNumber = curSeqNumber;
                        int32_t dRet = -1;
                        if(mCurSeqInfo[streamIdx].mCurSeqNumber <= mCurSeqInfo[dstreamIdx].mCurSeqNumber)
                        {
                            //match dstreamIdx rebase Offset.
                            ALOGI("[%s][%d][DISC] match dstreamIdx rebase Offset\n",__FUNCTION__,__LINE__);
                            dRet = getSeqdstRebaseOffset(streamIdx, dstreamIdx);
                        }

                        if(dRet == -1)
                        {
                            mCurSeqInfo[streamIdx].mOffsetUs = mCurSeqInfo[streamIdx].mPreTimeUs - originalTimeUs;
                            mCurSeqInfo[streamIdx].mIsNeedRebase = true;
                            ALOGI("[%s][%d][DISC] .mOffsetUs = .mPreTimeUs - .mOriginalTimeUs:: %f = %f - %f\n",__FUNCTION__,__LINE__,
                                    codec_show_pts( mCurSeqInfo[streamIdx].mOffsetUs) , codec_show_pts(mCurSeqInfo[streamIdx].mPreTimeUs) ,codec_show_pts( originalTimeUs));
                        }

                        if(originalTimeUs > 5 * 1E6)
                            packetSource->setDiscTimeUs(originalTimeUs - 5 * 1E6);
                        else
                            packetSource->setDiscTimeUs(0);

                    }
                    else
                    {
                        ALOGW("[%s][%d][DISC] Error Status not sync but chkIsSeqDiscontinuity is false. IsSeqBoundary::%d\n",__FUNCTION__,__LINE__, IsSeqBoundary);
                    }
                }

                if(bIsdiscFlg == false && curSeqNumber <= mCurSeqInfo[dstreamIdx].mCurSeqNumber
                        && mCurSeqInfo[streamIdx].mCurSeqNumber <= mCurSeqInfo[dstreamIdx].mCurSeqNumber
                        && curSeqNumber != mCurSeqInfo[streamIdx].mPreSeqNumber)
                {
                    int64_t tmpUs = mCurSeqInfo[streamIdx].mOffsetUs;
                    bool tmpflg = mCurSeqInfo[streamIdx].mIsNeedRebase;
                    getSeqdstRebaseOffset(streamIdx, dstreamIdx);
                    if(mCurSeqInfo[streamIdx].mIsNeedRebase == true)
                    {
                        timeUs = originalTimeUs + mCurSeqInfo[streamIdx].mOffsetUs;
                        //double check.
                        if(timeUs > 0 && (diff(timeUs, slCurTime) < slDiffUs))
                        {
                            ALOGD("[%s][%d] check getSeqdstRebaseOffset need rebase.\n",__FUNCTION__,__LINE__);
                            bIsdiscFlg = true;
                            dLineInfo = __LINE__;
                        }
                        else
                        {
                            mCurSeqInfo[streamIdx].mOffsetUs = tmpUs;
                            mCurSeqInfo[streamIdx].mIsNeedRebase = tmpflg;
                        }
                    }
                }
                if(mCurSeqInfo[streamIdx].mIsNeedRebase == true)
                ...
                if(bIsdiscFlg == true)
                {
                    ALOGD("[%s][%d][DISC] mCurSeqNumber:: %d bIsdiscFlg  %d\n",__FUNCTION__,__LINE__,mCurSeqInfo[streamIdx].mCurSeqNumber, bIsdiscFlg);
                    ALOGD("[%s][%d][DISC] [%d]discontinuity chk:tmpTimeUs [%s]ori %f  = timeUs %f A:%f V:%f.\n",__FUNCTION__,__LINE__, (int)dstreamIdx,
                            (stream == STREAMTYPE_VIDEO)?"VIDEO":"AUDIO", codec_show_pts(originalTimeUs),
                            codec_show_pts(tmpTimeUs), codec_show_pts(maudioTime), codec_show_pts(mvideoTime));
                    QueueSeqInfo(streamIdx);
                }

                if(mCurSeqInfo[streamIdx].mPreSeqNumber != mCurSeqInfo[streamIdx].mCurSeqNumber)
                {
                    int32_t minSeqNum = mCurSeqInfo[0].mCurSeqNumber;
                    if(minSeqNum > mCurSeqInfo[1].mCurSeqNumber)
                        minSeqNum = mCurSeqInfo[1].mCurSeqNumber;

                    index = mFetcherInfos.indexOfKey(mStreams[streamIdx].mUri);
                    if(index >= 0)
                        mFetcherInfos.editValueAt(index).mFetcher->updateMinSeq(minSeqNum);
                }

                if(mCurSeqInfo[streamIdx].mPreSeqNumber != mCurSeqInfo[streamIdx].mCurSeqNumber || bIsdiscFlg == true || dLineInfo == -1)
                {
                    mCurSeqInfo[streamIdx].mPreSeqNumber = mCurSeqInfo[streamIdx].mCurSeqNumber;
#if defined (RTK_ENB_QUEUE_DEBUG_LOG)
                    ALOGD("[%s][%d][DISC][%s] queueAccessUnit, timeUs=%f Ori::%f rebase::%d",__FUNCTION__,__LINE__,
                            (stream == STREAMTYPE_VIDEO)?"VIDEO":"AUDIO",
                            (double)timeUs/1E6,(double)originalTimeUs/1E6,(mCurSeqInfo[streamIdx].mIsNeedRebase == true));
                    LogSeqInfo();
                    if(bIsdiscFlg == true)
                    {
                        index = mFetcherInfos.indexOfKey(mStreams[streamIdx].mUri);
            ...
            if(stream == STREAMTYPE_AUDIO)
            {
//                if(timeUs > maudioTime)
                    maudioTime = timeUs;
            }
            else
            {
//                if(timeUs > mvideoTime)
                    mvideoTime = timeUs;
            }
#endif
            ...
            switch (what) {
                ...
                case PlaylistFetcher::kWhatStopped:
                {
                    ...
                    if (what == PlaylistFetcher::kWhatStopped) {
#ifdef RTK_PLATFORM
                        if(mInSwitchPhase == true)
                        {
                            ALOGI("[%s][%d] mInSwitchPhase is true break the stopped flow.\n",__FUNCTION__,__LINE__);
                            break;
                        }
#endif
                        mFetcherLooper->unregisterHandler(
                        ...
                        int32_t seekMode;
#ifdef RTK_PLATFORM
                        if(msg->findInt32("seekMode", &seekMode) == false)
                            seekMode = kSeekModeNextSegment;
#else
                    ...
                    if (mContinuation != NULL) {
                        ...
                        if (--mContinuationCounter == 0) {
                            ALOGI("[%d]mContinuation->post \n",__LINE__);
                            mContinuation->post();
                        }
                        else
                        {
                            ALOGW("[%d] not mContinuation->post \n",__LINE__);
                        }
                        ALOGI("%zu fetcher(s) left", mContinuationCounter);
                    }
                    break;
                }
...
#ifdef RTK_PLATFORM
                    if(mInPreparationPhase == false && mSwitchTimeoutCnt <= kPlaylistStatusReTryNumber && err != ERROR_END_OF_STREAM)
                    {
                        if(err == ERROR_CANNOT_CONNECT && mSwitchTimeoutCnt < kPlaylistStatusReTryNumber - 2)
                            mSwitchTimeoutCnt = kPlaylistStatusReTryNumber - 2;
                        reconnectUri(__LINE__);
                        break;
                    }
#endif
                ...
                case PlaylistFetcher::kWhatStartedAt:
                {
                    ...
                    if (checkSwitchProgress(stopParams, delayUs, &needResumeUntil)) {
                        ...
                        if (switchUp) {
                            ...
                            ALOGD("cancel up switch because we're too late");
                            cancelBandwidthSwitch(true /* resume */);
                        } else {
                            ALOGD("retry down switch at next sample");
...
void LiveSession::onConnect(const sp<AMessage> &msg) {
    ...
    ALOGI("onConnect %s", mMasterURL.c_str());
...
#ifdef RTK_PLATFORM
        ssize_t index = -1;
        size_t MaxBandwidthBps = 0;
        for( index = 0;(size_t)index < mBandwidthItems.size();index++)
        {
            const BandwidthItem &item = mBandwidthItems[index];
            if (item.mBandwidth >= MaxBandwidthBps ) {
                MaxBandwidthBps = item.mBandwidth;
                initialBandwidthIndex = index;
            }
        }
        // chose middle index
        initialBandwidthIndex = mBandwidthItems.size()/2;
#endif
    } else {
        // dummy item.
        BandwidthItem item;
...
sp<IMediaHTTPService> LiveSession::getHttphandle(void)
{
    return mHTTPService;
}
...
#ifdef RTK_PLATFORM
            size_t adjustedBandwidthBps = bandwidthBps * 9 / 10;
#else
...
void LiveSession::changeConfiguration(
    ...
    ALOGI("changeConfiguration: timeUs=%lld us, bwIndex=%zd, pickTrack=%d",
...
void LiveSession::onChangeConfiguration2(const sp<AMessage> &msg) {
    ...
    for (size_t i = 0; i < kMaxStreams; ++i) {
        if (streamMask & indexToType(i)) {
            ...
            ALOGI("%s = '%s'", uriKey.c_str(), URIs[i].c_str());
        }
    }
    ...
    for (size_t i = 0; i < kMaxStreams && i != kSubtitleIndex; ++i) {
        ...
        if ((mStreamMask & streamMask & indexToType(i))
            ...
            ALOGI("stream %zu changed: oldURI %s, newURI %s", i,
...
void LiveSession::onChangeConfiguration3(const sp<AMessage> &msg) {
    ...
    ALOGI("onChangeConfiguration3: timeUs=%lld, switching=%d, pickTrack=%d, "
    ...
    ALOGI("resuming fetchers for mask 0x%08x", resumeMask);
    for (size_t i = 0; i < mFetcherInfos.size(); ++i) {
        const AString &uri = mFetcherInfos.keyAt(i);
    ...
    if (streamMask != 0) {
        ALOGI("creating new fetchers for mask 0x%08x", streamMask);
    }
    ...
    for (size_t i = 0; i < kMaxStreams; i++) {
        ...
        sp<AnotherPacketSource> sources[kNumSources];
#ifdef RTK_PLATFORM
        if (i == kSubtitleIndex || (!pickTrack && !switching)||mSwitchTimeoutCnt > 0)
#else
...
#ifdef RTK_PLATFORM
                if (timeUs >= 0 && mSwitchTimeoutCnt == 0)
#else
                if (timeUs >= 0)
#endif
                {
                    ...
                    if (!switching) {
                        ...
                        ALOGI("stream[%zu]: queue format change", j);
        ...
        ALOGI("[fetcher-%d] startAsync: startTimeUs %lld mLastSeekTimeUs %lld "
        ...
        {
            bIsBandwidthSwitchFlw = true;
            ALOGI("[%s][%d] %d %d bIsBandwidthSwitchFlw set true \n",__FUNCTION__,__LINE__, (int)mOrigBandwidthIndex, (int)mCurBandwidthIndex);
        }
#endif
    ...
    ALOGI("onChangeConfiguration3: mSwitchInProgress %d, mStreamMask 0x%x",
            mSwitchInProgress, mStreamMask);

#ifdef RTK_PLATFORM
    if(mStreamMask == 0)
    {
        ALOGE("onChangeConfiguration3: mStreamMask 0x%x error return.\n", mStreamMask);
        finishDisconnect();
        postPrepared(ERROR_IO);
    }
#endif
...
void LiveSession::schedulePollBuffering() {
    ...
    if(mInPreparationPhase)
        msg->post(100000ll);
    else if(mInStartPlayBackFstChkBuffer == false)
    {
        msg->post(3000000ll);
        mInStartPlayBackFstChkBuffer = true;
    }
#endif
    else
        msg->post(1000000ll);
...
#ifdef RTK_PLATFORM
    int64_t bufferedDurationUsVideo = 0;
    int64_t bufferedDurationUsAudio = 0;
#endif
...
#ifdef RTK_PLATFORM
        onRTKCustomerHighWaterMBsChk();
        if(!mInPreparationPhase && kHighWaterMarkBytes != kReadyMarkUs)
        {
            readyMark = kHighWaterMarkBytes;
        }
#endif
...
#ifdef RTK_PLATFORM
        if(mPacketSources.keyAt(i) == STREAMTYPE_AUDIO)
        {
            bufferedDurationUsAudio = bufferedDurationUs;
        }else if(mPacketSources.keyAt(i) == STREAMTYPE_VIDEO)
        {
            bufferedDurationUsVideo = bufferedDurationUs;
        }
#endif
    }
...
#ifdef RTK_PLATFORM
    if(maudioretryCount + mvideoretryCount > kLiveSessionPTSCheckNumber)
        reconnectUri(__LINE__);

    ssize_t index = mFetcherInfos.indexOfKey(mStreams[typeToIndex(STREAMTYPE_AUDIO)].mUri);
    if(index >= 0 && mFetcherInfos.editValueAt(index).mFetcher->checkStatus() != PlaylistFetcher::kPlaylistStatus_OK)
    {
        reconnectUri(__LINE__);
    }

    if(mSwitchTimeoutCnt > kPlaylistStatusReTryNumber)
    {
        ALOGE("[%s][%d] mSwitchTimeoutCnt::%d send ERROR_IO to apk.\n",__FUNCTION__,__LINE__, mSwitchTimeoutCnt);
        sp<AMessage> msg = new AMessage(kWhatFetcherNotify,this);
        msg->setInt32("what", PlaylistFetcher::kWhatError);
        msg->setInt32("err", ERROR_IO);
        msg->post();
        return false;
    }
    if(mSwitchTimeoutCnt > 0)
    {
        HLSTime curTime = latestMediaSegmentStartTime();
        if(curTime.mTimeUs > mSwitchTime && diff(curTime.mTimeUs, mSwitchTime) > 60*1000000ll)
        {
            mSwitchTimeoutCnt = 0;
            mSwitchTime = curTime.mTimeUs;
            ALOGI("[%s][%d] set mSwitchTimeoutCnt to 0, normal mode.",__FUNCTION__,__LINE__);
        }
    }
#endif
...
#ifdef RTK_PLATFORM
        down      = (downCount == activeCount);
        underflow = (underflowCount == activeCount);
        if(!mInPreparationPhase && mBandwidthItems.size() > 2)
        {
            bool ini_underflow = underflow;
            QueueBufInfo(up, down, underflow);
            if(up == true || down == true|| underflow == true)
                QueueChkBufInfo(&up, &down, &underflow);
            mPreUnderflow = ini_underflow;
        }
#endif
        ALOGI("[%s][%d] up %d down %d ready %d underflow %d \n",__FUNCTION__,__LINE__,up,down,ready,underflow);
        if(mBandwidthItems.size() > 1)
            ALOGI("[%s][%d] mBandwidthItems.size() :: %d mCurBandwidthIndex:%d\n",__FUNCTION__,__LINE__, (int)mBandwidthItems.size(), (int)mCurBandwidthIndex);
...
#ifdef RTK_PLATFORM
    if(mSwitchTimeoutCnt > 0)
        ALOGI("startBufferingIfNecessary: mInPreparationPhase=%d, mBuffering=%d",
                mInPreparationPhase, mBuffering);
#endif
    if (!mBuffering) {
        mBuffering = true;
...
#ifdef RTK_PLATFORM
    if(mSwitchTimeoutCnt > 0)
        ALOGI("stopBufferingIfNecessary: mInPreparationPhase=%d, mBuffering=%d",
                mInPreparationPhase, mBuffering);
#endif
    if (mBuffering) {
        mBuffering = false;
...
void LiveSession::postPrepared(status_t err) {
    ...
    if(mSwitchTimeoutCnt > 0)
    {
        ALOGI("[%s][%d] mSwitchTimeoutCnt::%d, mInSwitchPreparationPhase::%d\n",__FUNCTION__,__LINE__, mSwitchTimeoutCnt, mInSwitchPreparationPhase);
        mInSwitchPhase = false;
        if(mInSwitchPreparationPhase == false)
        {
            if(err == OK)
            {
...
#ifdef RTK_PLATFORM
void LiveSession::setLastSessionFlg(int32_t dLastSessionFlg)
{
    mLastSessionFlg = dLastSessionFlg;
    ALOGV("[%s][%d] mLastSessionFlg set :: %d \n",__FUNCTION__,__LINE__,mLastSessionFlg);
}

void LiveSession::QueueBufInfo(bool up, bool down, bool underflow)
{

    BufInfoQueEntry entry;
    memset(&entry, 0, sizeof(BufInfoQueEntry));
    if(up == true)
        entry.up = 1;
    if(down == true)
        entry.down = 1;
    if(underflow == true)
        entry.underflow = 1;
    mBufInfoCnt.up += entry.up;
    mBufInfoCnt.down += entry.down;
    ...
    if (!isStable && (shortTermBps < bandwidthBps))
    {
        bandwidthBps = shortTermBps;
    }
    ...
    if(bandwidthBps < (int32_t)curBandwidth)
        entry.bandwidthlow = 1;
    mBufInfoCnt.bandwidthlow += entry.bandwidthlow;

    mBufInfoQue.push_back(entry);
    if(mBufInfoQue.size() > 20)
    {
        BufInfoQueEntry &entry = *mBufInfoQue.begin();
        mBufInfoCnt.up -= entry.up;
        mBufInfoCnt.down -= entry.down;
        mBufInfoCnt.bandwidthlow -= entry.bandwidthlow;
        mBufInfoQue.erase(mBufInfoQue.begin());
    }
    ALOGV("[%s][%d] (up,dn,under,bw)=(%2d,%2d,%2d,%2d) \n",__FUNCTION__,__LINE__, (int)mBufInfoCnt.up, (int)mBufInfoCnt.down, (int)mBufInfoCnt.underflow, (int)mBufInfoCnt.bandwidthlow);

}
void LiveSession::QueueChkBufInfo(bool *up, bool *down, bool *underflow)
{
    if(*up == true)
    {
        *up = false;
        if(mBufInfoCnt.up > 10)
        {
            *up = true;
        }
    }
    ...
    {
        *down = false;
        if((mBufInfoCnt.down > 15 && mBufInfoCnt.bandwidthlow > 10)|| mBufInfoCnt.underflow > 0)
        {
            *down = true;
        }
    }
    ...
    if(*underflow == true)
    {
        if(!(*underflow == true && mPreUnderflow == true))
        {
            *underflow = false;
        }
        else
        {
            mBufInfoCnt.underflow += 1;
        }
    }

}
void LiveSession::setOffloadAudio(bool offload)
{
    mOffloadAudio = offload;
}

int64_t LiveSession::diff(int64_t num1, int64_t num2)
{
    if(num1 > num2)
        return (num1 - num2);
    return (num2 - num1);
...
void LiveSession::initSeqQue()
{
    int32_t i =0;
    Vector<strSeqInfo> *strSeqQue;
    for(i=  0 ; i < 2; i++)
    {
        mCurSeqInfo[i].mTimeUs = 0;
        mCurSeqInfo[i].mOriginalTimeUs = 0;
        mCurSeqInfo[i].mCurSeqNumber = 0;
        mCurSeqInfo[i].mPreTimeUs = 0;
        mCurSeqInfo[i].mPreOriginalTimeUs = 0;
        mCurSeqInfo[i].mPreSeqNumber = 0;
        mCurSeqInfo[i].mIsNeedRebase = false;
    }
    strSeqQue = &mStrSeqQue[0];
    while(strSeqQue->size() > 0)
    {
        strSeqQue->erase(strSeqQue->begin());
    }
    strSeqQue = &mStrSeqQue[1];
    while(strSeqQue->size() > 0)
    {
        strSeqQue->erase(strSeqQue->begin());
    }
...
void LiveSession::QueueSeqInfo(ssize_t streamIdx)
{
    Vector<strSeqInfo> *strSeqQue;
...
void LiveSession::LogSeqInfo()
{
    ...
    for(Idx = 0; Idx <= 1; Idx++)
    {
        ...
        if(strSeqQue->size() > 0)
            ALOGI("[%s][%d][%s][DISC] Idx::%d size::%d\n",__FUNCTION__,__LINE__, (Idx == 0)?"AUDIO":"VIDEO", (int)Idx, (int)strSeqQue->size());
        for (index = 0; index < strSeqQue->size(); index++)
        {
            const strSeqInfo &item =(*strSeqQue)[index];
            ALOGD("[%s][%d][DISC] Seq[%2d] Num:[%4d],med:%05.5f \tOrd:%05.3f \t,Pre Num:[%4d],med:%05.3f \tOrd:%05.3f \t.%d. Off::%f.\n", __FUNCTION__,__LINE__, (int)index,
                    item.mCurSeqNumber, codec_show_pts(item.mTimeUs), codec_show_pts(item.mOriginalTimeUs),
                    item.mPreSeqNumber, codec_show_pts(item.mPreTimeUs), codec_show_pts(item.mPreOriginalTimeUs), item.mIsNeedRebase,
                    codec_show_pts(item.mOffsetUs));
        }
    }

}
int32_t LiveSession::getSeqdstRebaseOffset(ssize_t streamIdx, ssize_t dstreamIdx)
{
    size_t index = 0;
    Vector<strSeqInfo> *strSeqQue;
    strSeqQue = &mStrSeqQue[dstreamIdx];
    int32_t dIsSeqNumInTable = 0;
    //for list dstreamIdx
    for (index = 0; index < strSeqQue->size(); index++)
    {
        const strSeqInfo &item =(*strSeqQue)[index];
        if(mCurSeqInfo[streamIdx].mCurSeqNumber == item.mCurSeqNumber
                && item.mIsNeedRebase == true)
        {
            //VoffSet = AoffSet;
            mCurSeqInfo[streamIdx].mOffsetUs = item.mOffsetUs;
            mCurSeqInfo[streamIdx].mIsNeedRebase = true;
            ALOGD("[%s][%d][DISC] get mCurSeqInfo[streamIdx].mOffsetUs::%f\n",__FUNCTION__,__LINE__, codec_show_pts(mCurSeqInfo[streamIdx].mOffsetUs));
            return 1;
        }
        if(mCurSeqInfo[streamIdx].mCurSeqNumber == item.mCurSeqNumber)
        {
    ...
    ALOGW("[%s][%d] getSeqdstRebaseOffset fail\n",__FUNCTION__,__LINE__);
    if(dIsSeqNumInTable == 1)
    {
        ALOGW("[%s][%d][DISC] dstreamIdx not rebase, no rebase in streamIdx,Ret:: -2.\n",__FUNCTION__,__LINE__);
        return -2;
    }
