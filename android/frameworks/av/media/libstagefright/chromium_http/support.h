...
private:
    ...
    bool mAtEOS;
#ifdef ENABLE_DTCP_IP
    std::string mDtcpPInfo;
    DtcpSinkWrapper* mpDtcpSinkWrapper;
    bool mAkeOk;
    unsigned char mRemainingEncData[16384];//8192*2, PCP max is 128MB....
    size_t mRemainingEncDataSize;
    ...
    //[in/out] pDecBuf
    //[in/out] decSize
    bool DecryptDtcpipData(unsigned char* pEncBuf, size_t encSize, unsigned char* pDecBuf, size_t* decSize);
    //
    void ReleaseDtcpip();
    //
    void decodeUrl(std::string & url);
    //
    void parseDtcpipInfo(GURL &url);
    //
    void doAKE();
    //
    void resetDtcpip();
