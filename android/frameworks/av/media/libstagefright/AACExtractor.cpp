...
#if 0 //FIXME: aosp flow has changed, no need anymore?
                //rtk begin
                //Once the frame can not get frameSize, it will cause mInitCheck = NO_INIT.
                //Therefore, if AACExtractor can get numFrames, we keep going the playback flow (mInitCheck = OK).
                if (numFrames != 0)
                    break;
                //rtk end
#endif
...
//rtk add DHCKYLIN-659
        int64_t duration = 0;
        CHECK(mMeta->findInt64(kKeyDuration, &duration));
        if (mFrameDurationUs > 0 && seekTimeUs/1000000LL == duration/1000000LL ) {
            int64_t seekFrame = seekTimeUs / mFrameDurationUs;
            mCurrentTimeUs = (seekFrame - 3) * mFrameDurationUs;

            mOffset = mOffsetVector.itemAt(seekFrame - 3);
            ALOGD("Seek the end of stream, to get the EOS");
//rtk end
        } else if (mFrameDurationUs > 0) {
            int64_t seekFrame = seekTimeUs / mFrameDurationUs;
