...
void MediaCodec::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        ...
        case kWhatConfigure:
        {
            ...
            if (mCrypto != NULL)
            {
                format->setInt32("mCrypto", true);
                ALOGD("\033[0;32m IN %s:%d ---- set mCrypto to true -----\033[m", __FUNCTION__, __LINE__);
            }
...
status_t MediaCodec::amendOutputFormatWithCodecSpecificData(
    ...
    if (!strcasecmp(mime.c_str(), MEDIA_MIMETYPE_VIDEO_AVC)) {
        ...
        while (getNextNALUnit(&data, &size, &nalStart, &nalSize, true) == OK) {
            ...
            if(!memcmp(csd->data(), "\x00\x00\x00\x01\x09\x10", 6))
            {
                ALOGW("Find AUD data, skip it for the case of AVC");
                continue;
            }
