...
static sp<MediaMetadataRetrieverBase> createRetriever(player_type playerType)
{
    ...
    switch (playerType) {
        ...
        case NU_PLAYER:
            // For RTK players, we also use StagefrightMetadataRetriever for now
            // But, it doesn't fully support all media types.
            // If we expect client can retrieve more media types, we need to
            // enhance the media extractor in stagefright or we need to create
            // another retriever for RTK players.
        case REALTEK_PLAYER:
        case REALTEK_DTV_PLAYER:
        {
...
status_t MetadataRetrieverClient::setDataSource(int fd, int64_t offset, int64_t length)
{
    ...
    if (REALTEK_PLAYER == playerType) {
        ALOGW("Force to set player type to STAGEFRIGHT_PLAYER because we don't use REALTEK_PLAYER to extract metadata!!!");
