...
sp<IMediaPlayer> MediaPlayerService::getMediaPlayer(const pid_t pid)
{
    for (int i = 0; i < (int)mClients.size(); ++i) {
        sp<Client> c = mClients[i].promote();
        if (c->pid() == pid) {
            ALOGV("get client with pid %d", pid);
            return c;
        }
    }
    ALOGW("don't exist client with pid %d", pid);
    return NULL;
}
...
sp<MediaPlayerBase> MediaPlayerService::Client::createPlayer(player_type playerType)
{
    ...
    ALOGV("createPlayer: mForceUseRTMediaPlayer:%d", mForceUseRTMediaPlayer);
    mForceUseRTMediaPlayer = FORCE_NONE;
...
#ifdef USE_RT_MEDIA_PLAYER
status_t MediaPlayerService::Client::useRTMediaPlayer(int forceType)
{
    ALOGV("Client::useRTMediaPlayer...: %d", forceType);
    mForceUseRTMediaPlayer = forceType;
    MediaPlayerFactory::useRTMediaPlayer(mForceUseRTMediaPlayer);
    return OK;
}
#endif
        ...
        const KeyedVector<String8, String8> *headers)
{
    ...
    if (strncmp(url, "content://", 10) == 0) {
...
#ifdef USE_RT_MEDIA_PLAYER
        // Because DvdPlayer can't be created 2 instances to handle media playback simultaneoutly,
        // we just set its content path and don't do real setDataSouce if there exists a RTMediaPlayer
        // already.
        // @20140422: Because music playback is handled by Stagefright, not by RTMediaPlayer,
        // we don't need to care of music repeat playback case. So if a RTMediaPlayer has been running,
        // we need to stop and release it before setting a new RTMediaPlayer.
        bool RTMediaPlayerRunning = false;
        if (REALTEK_PLAYER == playerType || REALTEK_DTV_PLAYER == playerType) {
            SortedVector< wp<Client> > clients = mService->getClients();
            for (int i = 0, n = clients.size(); i < n; ++i) {
                sp<Client> c = clients[i].promote();
                if (c != 0) {
                    sp<MediaPlayerBase> p = c->getPlayer();
                    if (p != NULL && (p->playerType() == REALTEK_PLAYER || p->playerType() == REALTEK_DTV_PLAYER)) {
                        ALOGV("RTMediaPlayer's state = %d", p->getState());
                        RTMediaPlayerRunning = p->getState() != 0;
                        //int len = strlen(url);
                        //if (len >= 5 && !strcasecmp(".m3u8", &url[len - 5])) {
                            ALOGW("disconnect the running player");
                            c->disconnect();
                            RTMediaPlayerRunning = false;
                        //}
                        break;
                    }
                }
        ...
        if (true == RTMediaPlayerRunning)
            setDataSource_post(p, p->setDataPathNotLoad(url, headers));
        else
...
status_t MediaPlayerService::Client::setDataSource(int fd, int64_t offset, int64_t length)
{
    ...
    if (playerType == REALTEK_PLAYER)
    {
        ...
        char path_buff[4096] = "";
        link_buff[len] = '\0';
        //sprintf(path_buff, "file://%s", link_buff);
        sprintf(path_buff, "%s?offset=%lld&length=%lld", link_buff, (int64_t)offset, (int64_t)length);

        setDataSource_post(p, p->setDataSource(NULL, path_buff, NULL));
    }
#endif
    else
    {
...
#ifdef USE_RT_MEDIA_PLAYER
        err = native_window_api_disconnect(mConnectedWindow.get(),
        ...
        const sp<IGraphicBufferProducer>& bufferProducer)
{
    ...
    if (bufferProducer != NULL) {
        ...
        for (int k=0;k<10;k++)
        {
            ...
            Mutex::Autolock l(mLock);
#ifdef USE_RT_MEDIA_PLAYER
        if (p->playerType() == REALTEK_PLAYER ||
            p->playerType() == REALTEK_DTV_PLAYER)
            ...
            if (err == OK)
                ...
                ALOGW("[%s - %d] native_window_api_connect != OK: retry counter=%d\n", __FILE__, __LINE__, k);
            }
...
status_t MediaPlayerService::Client::getVideoInfo(int *width, int *height)
{
    ALOGV("getVideoInfo ");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    status_t ret = p->getVideoInfo(width, height);
    if (ret == NO_ERROR) {
//        ALOGV("[%d] getVideoInfo = %d", mConnId, *msec);
    } else {
        ALOGE("getVideoInfo returned %d", ret);
    }
    return ret;
}
...
#ifdef USE_RT_MEDIA_PLAYER
status_t MediaPlayerService::Client::fastforward(int speed)
{
    ALOGV("[%d] fastforward(%d)", mConnId, speed);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->fastforward(speed);
}

status_t MediaPlayerService::Client::fastrewind(int speed)
{
    ALOGV("[%d] fastrewind(%d)", mConnId, speed);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->fastrewind(speed);
}
int      *MediaPlayerService::Client::getSubtitleInfo(int streamNum)
{
    ALOGV("[%d] getSubtitleInfo(%d)", mConnId,streamNum);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return NULL;
    int *tmp=p->getSubtitleInfo(streamNum);
    ALOGI("[%s,%s,%d] m_subtitleInfo 0x%x = %d,%d,%d,%d,%d\n",__FILE__,__func__,__LINE__,(int)tmp,tmp[0],tmp[1],tmp[2],tmp[3],tmp[4]);
    return tmp;
}

status_t MediaPlayerService::Client::setSubtitleInfo(int streamNum,int enable,int textEncoding,int textColor,int fontSize, int syncTime, int offset)
{
    ALOGI("[%s,%s,%d,%d] streamNum= %d,enable= %d,textEncoding= %d,textColor= %d,fontSize= %d\n",__FILE__,__func__,__LINE__,mConnId,streamNum,enable,textEncoding,textColor,fontSize);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->setSubtitleInfo(streamNum,enable,textEncoding,textColor,fontSize,syncTime,offset);
}
status_t MediaPlayerService::Client::setAutoScanExtSubtitle(int autoScan)
{
    ALOGV("[%d] setAutoScanExtSubtitle(%d)", mConnId, autoScan);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->setAutoScanExtSubtitle(autoScan);
}

status_t MediaPlayerService::Client::setExtSubtitlePath(const char *path)
{
    ALOGV("[%d] setExtSubtitlePath(%s)", mConnId, path);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->setExtSubtitlePath(path);
}

int      *MediaPlayerService::Client::getAudioTrackInfo(int streamNum)
{
    ALOGV("[%d] getAudioTrackInfo(%d)", mConnId,streamNum);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return NULL;
    int *tmp=p->getAudioTrackInfo(streamNum);
...
status_t MediaPlayerService::Client::setAudioTrackInfo(int streamNum)
{
    ALOGV("[%d] setAudioTrackInfo(%d)", mConnId,streamNum);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->setAudioTrackInfo(streamNum);
}

status_t MediaPlayerService::Client::execSetGetNavProperty(int propertyID,int inDataSize,int outDataOffset,unsigned long *phyAddr)
{
    ALOGV("[%d] execSetGetNavProperty(propertyID = %d)", mConnId,propertyID);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->execSetGetNavProperty(propertyID,inDataSize,outDataOffset,phyAddr);
}

status_t MediaPlayerService::Client::deliverNaviControlCmd(int action, int status)
{
    ALOGV("[%d] deliverNaviControlCmd(%d, %d)", mConnId, action, status);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->deliverNaviControlCmd(action, status);
}

status_t MediaPlayerService::Client::setStartTime(int msec)
{
    ALOGV("[%d] setStartTime(%d)", mConnId, msec);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->setStartTime(msec);
}

// for transcode
status_t MediaPlayerService::Client::setTranscodeMode(bool bEnable)
{
    ALOGV("[%d] %s(%d)", mConnId, __func__, bEnable);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->setTranscodeMode(bEnable);
}

status_t MediaPlayerService::Client::startTranscode()
{
    ALOGV("[%d] %s", mConnId, __func__);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->startTranscode();
}

status_t MediaPlayerService::Client::pauseTranscode()
{
    ALOGV("[%d] %s", mConnId, __func__);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->pauseTranscode();
}

status_t MediaPlayerService::Client::stopTranscode()
{
    ALOGV("[%d] %s", mConnId, __func__);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->stopTranscode();
}

status_t MediaPlayerService::Client::setOutputFormat(int outputType,
                                        int outputFormat,
                                        int fileDesc,
                                        const char *fileName,
                                        ...
                                        bool rtpHeader)
{
    ...
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->setOutputFormat(outputType,
                            outputFormat,
                            fileDesc,
                            fileName,
                            ashmemData,
...
status_t MediaPlayerService::Client::configureTargetFormat(
                                            int width,
                                            int height,
                                            int fps,
                                            int v_bitrate,
                                            ...
                                            int a_bitrate)
{
    ...
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->configureTargetFormat(width,
                                    height,
                                    fps,
                                    v_bitrate,
                                    iframeInterval,
...
status_t MediaPlayerService::Client::registerStatusHandler(sp<AHandler> handlerId)
{
    ALOGV("[%d] %s(%d)", mConnId, __func__, handlerId->id());
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    //FIXME: need to pass AHandler instead of handler id
    CHECK(0);
    return p->registerStatusHandler(handlerId);
}

// for RVSD
status_t MediaPlayerService::Client::setTranscodeVOWriteback(const int32_t flag)
{
    ALOGV("[%d] %s", mConnId, __func__);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->setTranscodeVOWriteback(flag);
}

status_t MediaPlayerService::Client::setTranscodeOSDStatus(const int32_t flag)
{
    ALOGV("[%d] %s", mConnId, __func__);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->setTranscodeOSDStatus(flag);
}

status_t MediaPlayerService::Client::setLayerOrderValue(const uint32_t value)
{
    ALOGV("[%d] %s", mConnId, __func__);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->setLayerOrderValue(value);
}

status_t MediaPlayerService::Client::setPlaySpeedValue(const int32_t nSpeed)
{
    ALOGV("[%d] %s", mConnId, __func__);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->setPlaySpeedValue(nSpeed);
}

status_t MediaPlayerService::Client::initCheck()
{
    ALOGV("[%d] %s", mConnId, __func__);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
...
void MediaPlayerService::Client::SetContentMode(/*RTK_PLAYER_CONTENTMODE_e*/ int contentMode)
{
    ALOGV("[%d] %s", mConnId, __func__);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return;
...
int MediaPlayerService::Client::checkLoadMedia()
{
    ALOGV("[%d] %s", mConnId, __func__);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->checkLoadMedia();
}

int MediaPlayerService::Client::getState()
{
    ALOGV("[%d] %s", mConnId, __func__);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
...
int MediaPlayerService::Client::SetVideoWindow(int x, int y, int width, int height)
{
    ALOGV("[%d] %s", mConnId, __func__);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->SetVideoWindow(x,y,width,height);
}

int MediaPlayerService::Client::VideoShow()
{
    ALOGV("[%d] %s", mConnId, __func__);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
...
int MediaPlayerService::Client::VideoHide()
{
    ALOGV("[%d] %s", mConnId, __func__);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
...
void MediaPlayerService::Client::setVideoDisapper(bool disapper)
{
    ALOGV("[%d] %s", mConnId, __func__);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return;
    p->setVideoDisapper(disapper);
}
...
#ifdef RTK_PLATFORM
int64_t MediaPlayerService::AudioOutput::getPlayedOutDurationUsInt64(int64_t nowUs)
{
    Mutex::Autolock lock(mLock);
    if (mTrack == 0 || mSampleRateHz == 0) {
        return 0;
    ...
    int64_t numFramesPlayed;
    int64_t numFramesPlayedAt;
    ExtendedTimestamp ts;
    static const int64_t kStaleTimestamp100ms = 100000;

    status_t res = mTrack->getTimestamp(&ts);
    ExtendedTimestamp::Location loca_enum;

    if (res == OK) {                 // case 1: mixing audio tracks and offloaded tracks.
        ts.getBestTimestamp(&numFramesPlayed, &numFramesPlayedAt ,ExtendedTimestamp::TIMEBASE_MONOTONIC, &loca_enum);
        numFramesPlayedAt = numFramesPlayedAt / 1000; // nsec / 1000.
        if(mFlushFlag == true)
        {
            mFlushFlag = false;
            mExtTimestamp_FlushmPos = numFramesPlayed;
            ALOGI("[%s][%d]mExtTimestamp_FlushmPos set ::%lld \n",__FUNCTION__,__LINE__,mExtTimestamp_FlushmPos);
        }
        numFramesPlayed = numFramesPlayed - mExtTimestamp_FlushmPos;
        ...
        //ALOGD("getTimestamp: OK %lld %lld", (long long)numFramesPlayed, (long long)numFramesPlayedAt);
    } else if (res == WOULD_BLOCK) { // case 2: transitory state on start of a new track
        numFramesPlayed = 0;
        numFramesPlayedAt = nowUs;
        mExtTimestamp_FlushmPos = 0;
        ALOGD("getTimestamp: WOULD_BLOCK %lld %lld",
                (long long)numFramesPlayed, (long long)numFramesPlayedAt);
    } else {                         // case 3: transitory at new track or audio fast tracks.
        uint32_t numFramesPlayed_32;
        res = mTrack->getPosition(&numFramesPlayed_32);
        numFramesPlayed = (int64_t)numFramesPlayed_32;

        CHECK_EQ(res, (status_t)OK);
        numFramesPlayedAt = nowUs;
        numFramesPlayedAt += 1000LL * mTrack->latency() / 2; /* XXX */
        ALOGD("getPosition: %llu %lld", (long long)numFramesPlayed, (long long)numFramesPlayedAt);
    }

    mNumFramesPlayed = numFramesPlayed;
    int64_t durationUs = (int64_t)((int64_t)mNumFramesPlayed * 1000000LL / mSampleRateHz)
            + nowUs - numFramesPlayedAt;
    ...
    if (durationUs < 0) {
        ...
        //
        // Both of these are transitory conditions.
        ALOGD("getPlayedOutDurationUsInt64: negative duration %lld set to zero", (long long)durationUs);
        durationUs = 0;
    }
    ALOGV("getPlayedOutDurationUsInt64(%lld) nowUs(%lld) frames(%llu) framesAt(%lld)",
            (long long)durationUs, (long long)nowUs, (long long)numFramesPlayed, (long long)numFramesPlayedAt);
    return durationUs;
}

status_t MediaPlayerService::AudioOutput::getFramesWritten64(uint64_t *frameswritten)
{
    Mutex::Autolock lock(mLock);
    if (mTrack == 0) return NO_INIT;
    ...
    if (status == OK || status == WOULD_BLOCK) {
        *frameswritten = ets.mPosition[ExtendedTimestamp::LOCATION_CLIENT];
    }
    return status;
}
#endif
