...
#ifdef RTK_PLATFORM
                    int32_t dIsHLSflw = 0 ;
                    accessUnit->meta()->findInt32("IsHLSflw", &dIsHLSflw);
                    if(dIsHLSflw == 1)
                        seamlessFormatChange  = true;
#endif
...
#ifdef RTK_PLATFORM
    if(mCodec != NULL && !mIsAudio) {
        sp<AMessage> params = new AMessage();
        if(mRenderer->getVideoLateByUs() > 50000 /*50ms*/) {
            params->setInt32("skip-non-ref-frame", 1);
            if(mSkipFrameControl == false) {
                mCodec->setParameters(params);
                mSkipFrameControl = true;
            }
        } else {
            params->setInt32("skip-non-ref-frame", 0);
            if(mSkipFrameControl == true) {
                mCodec->setParameters(params);
                mSkipFrameControl = false;
            }
        }
