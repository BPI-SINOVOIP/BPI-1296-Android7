...
bool NuPlayer::RTSPSource::haveSufficientDataOnAllTracks() {
    ...
    static const int64_t kMinDurationUs = 10000ll;
    ...
    if (!source->hasBufferAvailable(&finalResult)) {
        if (finalResult == OK) {
            ...
            if (!sourceNearEOS(!audio)) {
                ...
                if(!audio)
                {
                    mVideoSourceNullCount++;
                    ALOGV("[%s][%d] Video mVideoSourceNullCount %d set  \n",__FUNCTION__,__LINE__,mVideoSourceNullCount);
                }
                if((!audio && mVideoSourceNullCount > 1000) || (!audio && mAudioSourceNullCount > 0 && mVideoSourceNullCount > 500))
                {
                    ALOGV("[%s][%d] startBufferingIfNecessary A %11lld V %11lld \n",__FUNCTION__,__LINE__,
                            (long long)mAudioPTS,(long long)mVideoPTS);
                    startBufferingIfNecessary();
                }

                if(audio)
                    mAudioSourceNullCount++;
#else
                startBufferingIfNecessary();
#endif
            }
...
#ifdef RTK_PLATFORM
    if(!audio)
        mVideoSourceNullCount = 0;
    else
        mAudioSourceNullCount = 0;
#endif
...
#ifdef RTK_PLATFORM
                int64_t nptUs2;
                accessUnit->meta()->findInt64("timeUs", (int64_t *)&nptUs2);

                if(trackIndex == (size_t)mAudioTrackIdx)
                    mAudioPTS = nptUs2;
                if(trackIndex == (size_t)mVideoTrackIdx)
                    mVideoPTS = nptUs2;
#endif
...
#ifdef RTK_PLATFORM
                mAudioTrackIdx = i;
                mHandler->setAudioIdx(mAudioTrackIdx);
                mAudioPTS = -1;
                ALOGV("[%s][%d] mAudioTrackIdx set ::%d \n" ,__FUNCTION__,__LINE__, mAudioTrackIdx);
#endif
            } else {
                mVideoTrack = source;
#ifdef RTK_PLATFORM
                mVideoTrackIdx = i;
                mHandler->setVideoIdx(mVideoTrackIdx);
                mVideoPTS = -1;
                ALOGV("[%s][%d] mVideoTrackIdx  set ::%d \n" ,__FUNCTION__,__LINE__, mVideoTrackIdx);
#endif
            }
