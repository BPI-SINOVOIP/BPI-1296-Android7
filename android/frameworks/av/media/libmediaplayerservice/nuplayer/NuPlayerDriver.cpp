...
status_t NuPlayerDriver::setDataSource(
    ...
    mPlayer->setResetFlgForAsyncFlw(false);
    mPlayer->setmIsNetAsyncFlw(true);
#endif
...
#if defined (RTK_PLATFORM) && defined (RTK_NET_ASYNC_PLAYBACK_FLOW)
    if(mPlayer->chkmIsNetAsyncFlw() == true)
        return OK;
#endif
...
#if defined (RTK_PLATFORM) && defined (RTK_NET_ASYNC_PLAYBACK_FLOW)
        if(mPlayer->chkmIsNetAsyncFlw() == true)
            ALOGE("[%s][%d] mState:: set %d mSetSurfaceInProgress %d return INVALID_OPERATION. \n",__FUNCTION__,__LINE__, mState, mSetSurfaceInProgress);
#endif
        return INVALID_OPERATION;
    }
...
#if defined (RTK_PLATFORM) && defined (RTK_NET_ASYNC_PLAYBACK_FLOW)
            if(mPlayer->chkmIsNetAsyncFlw() == true && mState == STATE_SET_DATASOURCE_PENDING)
                break;
#endif
            return INVALID_OPERATION;
...
#if defined (RTK_PLATFORM) && defined (RTK_NET_ASYNC_PLAYBACK_FLOW)
    if(mPlayer->chkmIsNetAsyncFlw() == true)
        mPlayer->setResetFlgForAsyncFlw(true);
#endif
...
status_t NuPlayerDriver::invoke(const Parcel &request, Parcel *reply) {
    ...
    switch (methodId) {
        ...
        case INVOKE_ID_SET_TV_SYSTEM_TYPE:
        case INVOKE_ID_SET_LANGUAGES:
        case INVOKE_ID_SET_GET_NAV_PROPERTY:
        case INVOKE_ID_EXEC_BOOKMARK_OPERATION:
        case INVOKE_ID_EXEC_SUBTITLE_OPERATION:
        case INVOKE_ID_EXEC_AUDIO_OPERATION:
        {
            return OK;
        }
...
status_t NuPlayerDriver::setParameter(
    ...
    ALOGV("NuPlayerDriver::setParameter(%d)", key);
    switch (key) {
        case KEY_PARAMETER_VIDEO_BUFFER_SIZE:
        {
            int bufferSize = request.readInt32();
            ALOGD("NuPlayerDriver::setParameter(%d, %d)", key, bufferSize);
            return OK;
        }
        default:
        {
...
void NuPlayerDriver::notifyListener_l(
    ...
    switch (msg) {
        case MEDIA_PLAYBACK_COMPLETE:
        {
            ...
            // fall through
#ifdef RTK_PLATFORM
            if(mPlayer->chkIsLiveFlw() == true && mAtEOS == true)
            {
                ALOGE("[%s][%d] Error notifyListener MEDIA_PLAYBACK_COMPLETE after sending MEDIA_ERROR event. return.\n",__FUNCTION__,__LINE__);
                return;
            }
#endif
        }
...
void NuPlayerDriver::notifySetDataSourceCompleted(status_t err) {
...
#if defined (RTK_PLATFORM) && defined (RTK_NET_ASYNC_PLAYBACK_FLOW)
    if(mPlayer->chkmIsNetAsyncFlw() == false)
#endif
    ...
    if(mPlayer->chkmIsNetAsyncFlw() == true)
    {
        if(err == OK)
        {
            if(mState != STATE_PREPARING && mState!= STATE_RESET_IN_PROGRESS)
