...
bool NuPlayer::IsHTTPLiveData(const sp<IMediaHTTPService> &httpService,
    ...
    mUri.clear();
    mUri = url;
#ifndef RTK_NET_ASYNC_PLAYBACK_FLOW
    mHeaders = headers;
#else
    (void)headers;
#endif
...
void NuPlayer::setDataSourceAsync(
    ...
    //clone headers, because original headers will be freed.
    if(headers != NULL && headers->size() > 0)
    {
    ALOGI("[%s][%d] copy headers, size :: %d \n",__FUNCTION__,__LINE__,headers->size());
    KeyedVector<String8, String8>* cloneHeaders = new KeyedVector<String8, String8>();
    for(size_t i=0;i<headers->size();i++)
        cloneHeaders->add(headers->keyAt(i), headers->valueAt(i));
    mHeaders = cloneHeaders;
    }
    msg->setString("url", url);
    msg->setPointer("headers", (void *) mHeaders);
    mHttpService = httpService;
    msg->post();
    return;
#else
    ...
    switch (msg->what()) {
        ...
        case kWhatSetVideoSurface:
        {
            ...
            if (obj != NULL || mAudioDecoder != NULL) {
                if (mStarted) {
                    ...
                    if (getCurrentPosition(&currentPositionUs) == OK) {
#ifdef RTK_PLATFORM
                        if(mIsHttpFlw == true && mIsLiveFlw == true && mPaused == true)
                        {
                            ALOGI("[%s][%d] already seek and paused.\n",__FUNCTION__,__LINE__);
                        }
                        else
                        {
                            mDeferredActions.push_back(
        ...
        case kWhatAudioNotify:
        {
            ...
            if (what == DecoderBase::kWhatInputDiscontinuity) {
                ...
                if(mIsLiveFlw == true)
                {
                    int32_t OMX_ErrorInsufficientResources  = (int32_t)0x80001000;
                    int32_t OMX_ErrorMAX  = (int32_t)0x80001100;

                    mDecErrorCnt++;
                    if((err < OMX_ErrorMAX) && (err > OMX_ErrorInsufficientResources) && (mDecErrorCnt < 10))
                    {
...
#if 0
            const KeyedVector<String8, String8> extHeaders=*headers;
            ALOGW("kWhatHttpCheck extHeaders.size()=%d, headers=%x", extHeaders.size(), (unsigned int)headers);
            for(size_t i=0;i<extHeaders.size();i++)
                ALOGW("extHeaders, k=%s, v=%s", extHeaders.keyAt(i).string(), extHeaders.valueAt(i).string());
#endif

            setDataSourceAsyncHttpCheck(mHttpService, url.c_str(), mHeaders);
            if(headers!=NULL){
...
#ifdef RTK_PLATFORM
        if(displayWidth > 0 && displayHeight > 0 && ((displayWidth * displayHeight) > (4128 * 2192)))
        {
            ALOGE("[%s][%d] displayWidth::%d displayHeight::%d over spec!\n",__FUNCTION__,__LINE__, displayWidth, displayHeight);
            notifyListener(MEDIA_ERROR, MEDIA_ERROR_VIDEO_SIZE_EXCEED_LIMITATION, 1);
            return;
        }
#endif
...
#ifdef RTK_PLATFORM
    if(mIsHttpFlw == true && mPaused == true)
    {
...
#ifdef RTK_NET_ASYNC_PLAYBACK_FLOW
    mAsyncIsResetFlg = false;
    ALOGI("[%s][%d] mAsyncIsResetFlg set false\n",__FUNCTION__,__LINE__);
#endif
}
...
void NuPlayer::setResetFlgForAsyncFlw(bool flags)
{
    ...
    ALOGI("[%s][%d] mAsyncIsResetFlg set %d \n",__FUNCTION__,__LINE__,flags);
}
