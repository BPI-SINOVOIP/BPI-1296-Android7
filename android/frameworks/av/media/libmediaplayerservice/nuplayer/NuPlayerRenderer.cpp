...
//#define DISABLE_RTKTUNING 1
#ifdef RTK_PLATFORM
//#define ENABLE_TEST_FREE_RUN   true
#define codec_show_pts(pts) (((double)pts/1E6))
#define MAXTIME_WAIT_FOR_AUDIO_SINK 600000
#endif
...
#ifdef RTK_PLATFORM
    mPreVideoLateByUs = 0;
    mSelectTrackAnchorTimeUs = 0;
    mSelectTrackEnb = false;
    mVideoDropFlg = false;
    mAudioDropFlg = false;
#endif
}
...
#if defined (RTK_PLATFORM) && defined (RTK_ENB_QUEUE_DEBUG_LOG)
    while (!mDBGInfoQueue.empty())
    {
        mDBGInfoQueue.erase(mDBGInfoQueue.begin());
    }
#endif
...
#ifdef RTK_PLATFORM
        int64_t nowMediaUs = mAudioSink->getPlayedOutDurationUsInt64(nowUs)
        ...
        case kWhatDrainAudioQueue:
        {
            ...
            if (onDrainAudioQueue()) {
                ...
                CHECK_EQ(mAudioSink->getPosition(&numFramesPlayed),
                         (status_t)OK);
#ifdef RTK_PLATFORM
                uint32_t mNumFramesWritten_under_32bit = (uint32_t)(mNumFramesWritten & 0XFFFFFFFF);
                uint32_t numFramesPendingPlayout =
                    mNumFramesWritten_under_32bit - numFramesPlayed;
#else
                uint32_t numFramesPendingPlayout =
                ...
                if(delayUs > 5000000)
                {
                    ALOGE("[%s][%d] delayUs %f too large \n",__FUNCTION__,__LINE__,codec_show_pts(delayUs));
                    POPQueueDBGInfo();
                }
#endif
...
#ifdef RTK_PLATFORM
        int64_t nowMediaUs =
            mAudioFirstAnchorTimeMediaUs + mAudioSink->getPlayedOutDurationUsInt64(nowUs);
#else
        int64_t nowMediaUs =
    ...
    if (hasEOS) {
        ...
        if (!offloadingAudio()) {
            ...
            ALOGV("fillAudioBuffer: notifyEOS "
                    "mNumFramesWritten:%llu  finalResult:%d  postEOSDelay:%lld",
...
#ifdef RTK_PLATFORM
    if(mSelectTrackEnb == true)
    {
        while (!mAudioQueue.empty() && mAudioDropFlg == true)
        {
            ...
            if (mAudioDropFlg == true)
            {
                int64_t mediaTimeUs;
                entry->mBuffer->meta()->findInt64("timeUs", &mediaTimeUs);
                if(mSelectTrackAnchorTimeUs > mediaTimeUs)
                {
                    ALOGV("[%s][%d] mSelectTrackEnb mSelectTrackAnchorTimeUs::%f > %f \n",__FUNCTION__,__LINE__,
                            codec_show_pts(mSelectTrackAnchorTimeUs), codec_show_pts(mediaTimeUs));
                    entry->mNotifyConsumed->post();
                    entry = NULL;
                    mAudioQueue.erase(mAudioQueue.begin());
                    continue;
                ...
                {
                    ALOGI("[%s][%d] mAudioDropFlg set false\n",__FUNCTION__,__LINE__);
                    mAudioDropFlg = false;
                    break;
                }
            }
        }
        if(mAudioDropFlg == false && mVideoDropFlg == false)
            mSelectTrackEnb = false;

        if(mSelectTrackEnb == true)
        {
            postDrainAudioQueue_l(10);
            bool reschedule = !mAudioQueue.empty() && (!mPaused); // permit pause to fill buffers
            return reschedule;
        }
...
#ifdef RTK_PLATFORM
    uint64_t prevFramesWritten = mNumFramesWritten;
#else
...
#ifdef RTK_PLATFORM
    return writtenAudioDurationUs - mAudioSink->getPlayedOutDurationUsInt64(nowUs);
#else
...
int64_t NuPlayer::Renderer::Diff(int64_t a , int64_t b)
{
    if(a>b)
        return (a-b);
    return (b-a);
...
#if 1
            /* RTK : When the updateAnchor frequency is reduced,
             * the maxTimeMedia should include kMinimumAudioClockUpdatePeriodUs */
            mMediaClock->updateAnchor(nowMediaUs, nowUs, mediaTimeUs + kMinimumAudioClockUpdatePeriodUs);
#else
...
#if defined (RTK_PLATFORM) && defined (RTK_ENB_QUEUE_DEBUG_LOG)
            // store the mDBGInfoQueue
            mAudioTimeUs = mediaTimeUs + kMinimumAudioClockUpdatePeriodUs;
            QueueDBGInfo(true, false, false, mAudioFirstAnchorTimeMediaUs, nowUs, getPendingAudioPlayoutDurationUs(nowUs));
#endif
        }
    } else {
        int64_t unused;
...
#if defined (RTK_PLATFORM) && defined (RTK_ENB_QUEUE_DEBUG_LOG)
            // store the mDBGInfoQueue
            mAudioTimeUs = mediaTimeUs;
            QueueDBGInfo(true, false, false, mAudioFirstAnchorTimeMediaUs, nowUs, -1);
#endif
        }
    }
#if defined (RTK_PLATFORM) && defined (RTK_ENB_QUEUE_DEBUG_LOG)
    if(mAnchorTimeMediaUs != -1 && Diff(mAnchorTimeMediaUs,mediaTimeUs) > 100000000ll)
    {
        ALOGW("[%s][%d] ERROR Status %f %f %f %f %lld\n",__FUNCTION__,__LINE__,
                codec_show_pts(mAnchorTimeMediaUs),
                codec_show_pts(nowUs),codec_show_pts(mediaTimeUs),
                codec_show_pts(getPendingAudioPlayoutDurationUs(nowUs)),mNumFramesWritten);
        POPQueueDBGInfo();
    }
#endif
...
void NuPlayer::Renderer::postDrainVideoQueue() {
    ...
    if(mSelectTrackEnb == true)
    {
        while(!mVideoQueue.empty() && mVideoDropFlg == true)
        {
    ...
    if (mFlags & FLAG_REAL_TIME) {
        ...
        } else if (needRepostDrainVideoQueue && !mAudioEOS) {
...
#ifdef RTK_PLATFORM
        else if (mWaitAudioSink && mNextAudioClockUpdateTimeUs < 0) {
            int64_t delayTimeUs = getRealTimeUs(mediaTimeUs + MAXTIME_WAIT_FOR_AUDIO_SINK, nowUs);     //wait maximum time for audio sink to start
            if(nowUs < delayTimeUs) {
                postDelayUs = 10000;
            }
            else {
                ALOGW("AudioSink getTimestamp is not ready. nowUs %lldms exceeds delayTime %dms", (long long)(nowUs/1000), (int)(MAXTIME_WAIT_FOR_AUDIO_SINK/1000));
                mWaitAudioSink = false;
                if(!mHasAudio) {
                    mAnchorTimeMediaUs=-1;
                    postDelayUs = 1000;
                }
                // MediaClock has not started yet. Try to start it if possible.
                {
                    ...
                    AudioTimestamp ts;
                    status_t res = mAudioSink->getTimestamp(ts);
                    if (res == OK && mAudioFirstAnchorTimeMediaUs != -1) {
                        ALOGI("[%s][%d] MediaClock has not started yet. Try to start it if possible.\n",__FUNCTION__,__LINE__);
                        // AudioSink has rendered some frames.
                        int64_t nowUs = ALooper::GetNowUs();
                        int64_t nowMediaUs = mAudioSink->getPlayedOutDurationUsInt64(nowUs)
                            + mAudioFirstAnchorTimeMediaUs;
...
#ifdef RTK_PLATFORM
            if(postDelayUs > 100000)
            ALOGI("[jump] real:: %fs,medUs:: %fs, diff(r-m):: %fs, nowUs::%fs ,mAudioAnchorUs::%fs.",
                    codec_show_pts(realTimeUs), codec_show_pts(mediaTimeUs), codec_show_pts(double(realTimeUs - mediaTimeUs)),
                    codec_show_pts(nowUs), codec_show_pts(mAnchorTimeMediaUs));
#endif
            mDrainVideoQueuePending = true;

#if defined (RTK_PLATFORM) && defined (RTK_ENB_QUEUE_DEBUG_LOG)
            // store the mDBGInfoQueue
            mVideoTimeUs = mediaTimeUs;
            QueueDBGInfo(false, false, true, realTimeUs, nowUs, 0);
#endif
            return;
        }
#endif
...
#ifdef RTK_PLATFORM
    if(delayUs > twoVsyncsUs)
    {
        if(delayUs > 1000000)
        {
            ALOGE("[%s][%d] delayUs %fs too large error \n",__FUNCTION__,__LINE__,codec_show_pts(delayUs));
            delayUs = 1000000;
...
#ifdef RTK_PLATFORM
#ifdef RTK_ENB_QUEUE_DEBUG_LOG
void NuPlayer::Renderer::POPQueueDBGInfo()
{
    ALOGI("[%s][%d][DBG] POP ++ \n",__FUNCTION__,__LINE__);
    ALOGI("bIsStoreByAudio bIsVideoLate bIsVideoJump sAnchorTimeMediaUs sAudmediaTimeUs sVidmediaTimeUs sVidrealTimeUs snowUs sPendingAudioPlayoutDurationUs sClkAnchorTimeMediaUs sClkAnchorTimeRealUs sClkMaxTimeMediaUs sClkStartingTimeMediaUs\n");
    int32_t dCount=0;
    while(!mDBGInfoQueue.empty())
    {
        DeBugQueueEntry &entry = *mDBGInfoQueue.begin();
        ALOGD("[DBG1][%3d]Aud:%d Late:%d Jump:%d Anchor:%10.3f Aud:%10.3f Vid:%10.3f VidReal:%10.3f now:%10.3f\n",dCount, entry.bIsStoreByAudio, entry.bIsVideoLate, entry.bIsVideoJump,
              codec_show_pts(entry.sAnchorTimeMediaUs), codec_show_pts(entry.sAudmediaTimeUs), codec_show_pts(entry.sVidmediaTimeUs),
              codec_show_pts(entry.sVidrealTimeUs), codec_show_pts(entry.snowUs));
        ALOGI("[DBG2]Pend:%10.3f ClkAnchor:%10.3f ClkReal:%10.3f ClkMax:%10.3f ClkStart:%10.3f \n", codec_show_pts(entry.sPendingAudioPlayoutDurationUs),
                codec_show_pts(entry.sClkAnchorTimeMediaUs), codec_show_pts(entry.sClkAnchorTimeRealUs),
                codec_show_pts(entry.sClkMaxTimeMediaUs), codec_show_pts(entry.sClkStartingTimeMediaUs));
        mDBGInfoQueue.erase(mDBGInfoQueue.begin());
        if(dCount++ % 10==0)
            ALOGI("[DBG]-----------------------------------------------------------------------\n");
    }
}
void NuPlayer::Renderer::QueueDBGInfo(bool bIsStoreByAudio, bool bIsVideoLate, bool bIsVideoJump, int64_t realTimeUs, int64_t nowUs, int64_t PendingAudDuraUs)
{

    DeBugQueueEntry entry;
    entry.bIsStoreByAudio = bIsStoreByAudio;
    entry.bIsVideoLate = bIsVideoLate;
    entry.bIsVideoJump = bIsVideoJump;
    entry.sAnchorTimeMediaUs = mAnchorTimeMediaUs;
    entry.sAudmediaTimeUs = mAudioTimeUs;
    entry.sVidmediaTimeUs = mVideoTimeUs;
    entry.sVidrealTimeUs = realTimeUs;
    entry.snowUs = nowUs;
    entry.sPendingAudioPlayoutDurationUs = PendingAudDuraUs;

    mMediaClock->getClkInfo(&entry.sClkAnchorTimeMediaUs, &entry.sClkAnchorTimeRealUs,
            &entry.sClkMaxTimeMediaUs, &entry.sClkStartingTimeMediaUs);

    mDBGInfoQueue.push_back(entry);
    if(mDBGInfoQueue.size() > 100)
    {
        mDBGInfoQueue.erase(mDBGInfoQueue.begin());
    }
...
int32_t NuPlayer::Renderer::setSelectTrackFlg(bool Flag)
{
    mSelectTrackEnb = Flag;

    if(mSelectTrackEnb == true)
    {
        int64_t nowUs = ALooper::GetNowUs();
        int64_t mgetPendingAudioPlayoutDurationUs = getPendingAudioPlayoutDurationUs(nowUs);
        int64_t nowMediaUs = mAnchorTimeMediaUs - mgetPendingAudioPlayoutDurationUs;
        mSelectTrackAnchorTimeUs = nowMediaUs;
        mVideoDropFlg = true;
        mAudioDropFlg = true;
        ALOGI("[%s][%d]  mSelectTrackEnb mSelectTrackAnchorTimeUs:: %f\n",__FUNCTION__,__LINE__,codec_show_pts(mSelectTrackAnchorTimeUs));
    }
...
void NuPlayer::Renderer::onDrainVideoQueue() {
    ...
    if (!mPaused) {
...
#ifdef RTK_PLATFORM
        if(tooLate && mPreVideoLateByUs < realTimeUs
                && (realTimeUs - mPreVideoLateByUs) <30000
                && (mVideoLateByUs < 500000))
        {
            tooLate = 0;
            ALOGW("[%s][%d] Set tooLate 0 diff(realTimeUs, mPreVideoLateByUs):%f < 30ms \n",
                    __FUNCTION__,__LINE__,(double)(realTimeUs - mPreVideoLateByUs)/1E3);
        }
#ifdef RTK_ENB_QUEUE_DEBUG_LOG
        if(Diff(nowUs,realTimeUs) > 100000000ll)
        {
            int64_t mediaUs = 0;
            mMediaClock->getMediaTime(realTimeUs, &mediaUs);
            ALOGW("[%s][%d] ERROR Status : nowUs::%f realTimeUs::%f mediaUs::%f mediaTimeUs::%f\n",__FUNCTION__,__LINE__,
                    codec_show_pts(nowUs),codec_show_pts(realTimeUs),codec_show_pts(mediaUs),codec_show_pts(mediaTimeUs));
            POPQueueDBGInfo();
        }
#endif
#endif
        ...
        if (tooLate) {
            mVideoLateCount++;
            ALOGI("\033[0;31;31m video late by %lld us (%.2f secs) count %d\033[m",
                 (long long)mVideoLateByUs, mVideoLateByUs / 1E6, mVideoLateCount);
#ifdef RTK_PLATFORM
            mPreVideoLateByUs = realTimeUs;
            ALOGI("[late] real:: %fs,medUs:: %fs, diff(r-m):: %fs, nowUs::%fs ,mAudioAnchorUs::%fs.",
                    codec_show_pts(realTimeUs), codec_show_pts(mediaTimeUs), codec_show_pts(double(realTimeUs - mediaTimeUs)),
                    codec_show_pts(nowUs), codec_show_pts(mAnchorTimeMediaUs));
#endif
        } else {
    ...
    // store the mDBGInfoQueue
    mVideoTimeUs = mediaTimeUs;
    QueueDBGInfo(false, tooLate, false, realTimeUs, nowUs, 0);
#endif
...
#ifdef RTK_PLATFORM
    uint64_t written;
    if (mAudioSink->getFramesWritten64(&written) == OK) {
