...
namespace android {
#ifdef RTK_PLATFORM
static int64_t kLowWaterMarkUs = 100000ll;  // 0.1secs
// 10secs, 4K streaming will forever pause play since NuCachedSource2 only buffering 40MB
static int64_t kHighWaterMarkRebufferUs = 5000000ll;
static int64_t kHighWaterMarkUs = 1000000ll;  // 1secs
#else
static int64_t kLowWaterMarkUs = 2000000ll;  // 2secs
static int64_t kHighWaterMarkRebufferUs = 15000000ll;  // 15secs
static int64_t kHighWaterMarkUs = 50000000ll;  // 5secs
#endif
...
status_t NuPlayer::GenericSource::setDataSource(
    ...
    mUri = url;
#ifdef RTK_PLATFORM
    ALOGI("[%s][%d] mUri::%s\n",__FUNCTION__,__LINE__, mUri.c_str());
#endif
...
void NuPlayer::GenericSource::start() {
    ...
    property_set("rtk.media.USE_GenericSource", "true");
}
...
void NuPlayer::GenericSource::stop() {
    ...
    property_set("rtk.media.USE_GenericSource", "false");
}
...
sp<AMessage> NuPlayer::GenericSource::getTrackInfo(size_t trackIndex) const {
    ...
    {
        int32_t v_width = 0, v_height = 0, v_FrameRate =0;

        meta->findInt32(kKeyWidth, &v_width);
        if (v_width > 0)
            format->setInt32("width", v_width);
        meta->findInt32(kKeyHeight, &v_height);
        if (v_height > 0)
            format->setInt32("height", v_height);
#ifdef RTK_PLATFORM
        float v_FrameRateFloat = v_FrameRate;
        meta->findFloat(kKeyFrameRateFloat, &v_FrameRateFloat);
        if (v_FrameRateFloat > 0)
            format->setFloat("frame-rate-float", v_FrameRateFloat);
#endif
        meta->findInt32(kKeyFrameRate, &v_FrameRate);
        if (v_FrameRate > 0)
            format->setInt32("frame-rate", v_FrameRate);
    }

    if (trackType == MEDIA_TRACK_TYPE_AUDIO)
    {
        int32_t a_sample_rate = 0, a_channel = 0;

        meta->findInt32(kKeySampleRate, &a_sample_rate);
        if (a_sample_rate > 0)
            format->setInt32("sample-rate", a_sample_rate);
        meta->findInt32(kKeyChannelCount, &a_channel);
        if (a_channel > 0)
            format->setInt32("channel-count", a_channel);
    }
    ...
    switch (trackType) {
        case MEDIA_TRACK_TYPE_VIDEO:
            ...
            if (mIsWidevine) {
                ...
                maxBuffers = 2; //8;  // too large of a number may influence seeks
            }
            break;
        case MEDIA_TRACK_TYPE_AUDIO:
            track = &mAudioTrack;
...
#ifndef RTK_PLATFORM
#if 0//Workaround for MPEG2 live stream not ready
        if (finalStatus == OK) {
#else
        if (finalStatus == OK && mIsStreaming == false) {
#endif
#else
        if (finalStatus == OK) {
#endif
...
#ifdef RTK_PLATFORM
            if (cachedDurationUs < kLowWaterMarkUs&&mInStartPlayBackFstChkBuffer == true)
#else
            if (cachedDurationUs < kLowWaterMarkUs)
#endif
            {
