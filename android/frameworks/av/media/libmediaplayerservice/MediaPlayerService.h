...
#ifdef RTK_PLATFORM
        virtual int64_t         getPlayedOutDurationUsInt64(int64_t nowUs);
        int64_t                 mNumFramesPlayed;
        bool                    mFlushFlag;
        int64_t                 mExtTimestamp_FlushmPos;
        virtual status_t        getFramesWritten64(uint64_t *frameswritten);
#endif
...
public:
    ...
    virtual sp<IMediaPlayer>    getMediaPlayer(const pid_t pid);
    ...
    // RTK
    SortedVector< wp<Client> > getClients() { Mutex::Autolock lock(mLock); return mClients; };
    // end RTK
private:
    ...
    class Client : public BnMediaPlayer {
        ...
        virtual status_t        useRTMediaPlayer(int forceType);
        ...
        virtual status_t        deliverNaviControlCmd(int action, int status);
