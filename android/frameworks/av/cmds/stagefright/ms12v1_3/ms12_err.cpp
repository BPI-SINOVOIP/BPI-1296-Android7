...
const MS_ERR ms_errmsgtab[] =
{
    ...
    {MS_ERR_INVALID_INPUT_FILE,               "Invalid input file (-im / -ia / -im2 / -is / -ias)"},
    {MS_ERR_INVALID_INPUT_TYPE,               "Invalid input type (-it)"},
    {MS_ERR_INCOMPATIBLE_INPUTS,              "Input files must have identical extensions (-im / -ia / -im2)"},
    ...
    {MS_ERR_MISSING_LANG_SELECTION,           "AC4 preferred language selection is missing an argument (-lang[2] <string>)"},
    {MS_ERR_INVALID_ASSOCIATED_TYPE,          "AC4 preferred associated content type argument is invalid (-at: 1-3)"},
    {MS_ERR_OUTOFRANGE_ASSOCIATED_TYPE,       "AC4 preferred associated content type argument is invalid (-at: 1-3)"},
    {MS_ERR_INVALID_PREFER_ASSOCIATED_TYPE,   "AC4 prefer associated content type over language argument is invalid (-pat: 0-1)"},
    {MS_ERR_OUTOFRANGE_PREFER_ASSOCIATED_TYPE,"AC4 prefer associated content type over language argument is out of range (-pat: 0-1)"},
    {MS_ERR_INVALID_AC4_MODE,                 "AC4 mode is invalid (-ac4_mode: 0-3), 3 is mandatory if two input streams are present"},
    {MS_ERR_OUTOFRANGE_AC4_MODE,              "AC4 mode argument is invalid (-at: 0-3)"},
    {MS_ERR_INVALID_AC4_PROG_ID_TYPE,         "AC4 program id type is invalid (-ac4_prog_id_type: 0-2)"},
    {MS_ERR_OUTOFRANGE_AC4_PROG_ID_TYPE,      "AC4 program id type is out of range (-at: 0-2)"},
    {MS_ERR_INVALID_AC4_SHORT_PROG_ID,        "AC4 short program id is invalid (-ac4_prog_id: 16 bit value)"},
    {MS_ERR_OUTOFRANGE_AC4_SHORT_PROG_ID,     "AC4 short program id is out of range (-ac4_prog_id: 16 bit value)"},
    {MS_ERR_INVALID_UUID_PROG_ID,             "AC4 program uuid is invalid for the given id type(-ac4_prog_id: <32 hexadecimal characters 0..F without leading 0x>"},
    {MS_ERR_INVALID_AC4_PRES_IDX,             "AC4 direct presentation selection is invalid (-ac4_main_pres_idx, -ac4_assoc_pres_idx: 0..63)"},
    {MS_ERR_OUTOFRANGE_AC4_PRES_IDX,          "AC4 direct presentation selection is out of range (-ac4_main_pres_idx, -ac4_assoc_pres_idx: 0..63)"},
    {MS_ERR_INVALID_AC4_DE_GAIN,              "AC4 dialogue enhancement gain is invalid (-ac4_de: 0..12)"},
    {MS_ERR_OUTOFRANGE_AC4_DE_GAIN,           "AC4 dialogue enhancement gain is out of range (-ac4_de: 0..12)"},
#endif
    {MS_ERR_INVALID_MIXER_FADE_VALUES,        "Mixer gain/fade/ramp values are invalid"},
    {MS_ERR_OUTOFRANGE_MIXER_FADE,            "Mixer gain/fade/ramp values are out of range (attenuation: -96..0, duration: 0..60000, type: 0..2)"},
    {MS_ERR_OUTOFRANGE_ENC_CHMOD_LOCK,        "Encoder channel mode locking out of range (-chmod_locking: 0 <auto> or 1 <5.1>)"},
    {MS_ERR_INVALID_MAX_NUM_CHANNEL_VALUE,    "Encoder channel mode locking parameter invalid (-chmod_locking: 0 <auto> or 1 <5.1>)"},
    {MS_ERR_EOT,                              ""}
};
