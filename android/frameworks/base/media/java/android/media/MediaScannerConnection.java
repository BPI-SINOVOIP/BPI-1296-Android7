...
public class MediaScannerConnection implements ServiceConnection {
     ...
     */
     ...
     /**
      * Requests the media scanner to rescan specify volume.
...
/*
     public void rescanVolume(String volumeRootPath){
         synchronized (this) {
             ...
             String[] _ExternalStoragePaths = storageManager.getVolumePaths();
             boolean bRootPathValid=false;
             Log.d(TAG, "rescanVolume volumeRootPath:"+volumeRootPath);
             
             for (int i = 0; i < _ExternalStoragePaths.length; i++) {
                 Log.d(TAG, "rescanVolume _ExternalStoragePaths[i]:"+_ExternalStoragePaths[i]);
                 if(volumeRootPath.equalsIgnoreCase(_ExternalStoragePaths[i])){
                     bRootPathValid=true;
                     break;
                 }
             }
             if(bRootPathValid==false){
                 Log.d(TAG, "Volume root path is invalid.");
                 return;
             }
             ...
             try {
                 if (false) {
                     Log.v(TAG, "rescan volume --> volumeRootPath: " + volumeRootPath);
                 }
                 mService.rescanVolume(volumeRootPath);
             } catch (RemoteException e) {
                 if (false) {
                     Log.d(TAG, "Failed to rescan volume  --> volumeRootPath:" + volumeRootPath);
                 }
             }
         }
     }
