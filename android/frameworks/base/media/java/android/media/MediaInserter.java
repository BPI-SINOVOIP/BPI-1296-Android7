...
public class MediaInserter {
    ...
    private final int mAutoIncBufferSize;
    private final int mMaxBufferSizePerUri;

    public MediaInserter(ContentProviderClient provider, int bufferSizePerUri) {
        mProvider = provider;
        mBufferSizePerUri = bufferSizePerUri;
        mAutoIncBufferSize = 0;
        mMaxBufferSizePerUri = bufferSizePerUri;
    }

    public MediaInserter(ContentProviderClient provider, String packageName, int bufferSizePerUri, int maxbufferSizePerUri, int autoInc) {
        mProvider = provider;
        mBufferSizePerUri = bufferSizePerUri;
        mAutoIncBufferSize = 1;
        mMaxBufferSizePerUri = maxbufferSizePerUri;
    }
    ...
    private void insert(Uri tableUri, ContentValues values, boolean priority) throws RemoteException {
        ...
        if (list.size() >= mBufferSizePerUri) {
            flushAllPriority();
            flush(tableUri, list);
            if (mAutoIncBufferSize == 1 && (mBufferSizePerUri < mMaxBufferSizePerUri))
            {
                mBufferSizePerUri += (int)(mMaxBufferSizePerUri * 0.2f);
                if (mBufferSizePerUri > mMaxBufferSizePerUri)
                {
                    mBufferSizePerUri = mMaxBufferSizePerUri;
                }
            }
        }
    }
