...
import android.net.OpenWrtNetUtils;
...
public class WifiManager {
    ...
    /**
     * Wi-Fi AP is need to start now
     * enabling or disabling
     *
     * @see #WIFI_AP_STATE_NEED_TO_START
     * @see #getWifiApState()
     *
     * @hide
     */
    public static final int WIFI_AP_STATE_NEED_START = 15;
    ...
    public WifiInfo getConnectionInfo() {
        if(OpenWrtNetUtils.isEnabled())
            return OpenWrtNetUtils.getConnectionInfo();
        try {
    ...
    */
    ...
    public DhcpInfo getDhcpInfo() {
        if(OpenWrtNetUtils.isEnabled())
            return OpenWrtNetUtils.getDhcpInfo();
        try {
    ...
    public boolean setWifiEnabled(boolean enabled) {
        if(OpenWrtNetUtils.isEnabled())
	        return OpenWrtNetUtils.setWifiEnabled(enabled);
        try {
    ...
    public int getWifiState() {
        if(OpenWrtNetUtils.isEnabled())
            return OpenWrtNetUtils.getWifiState();
        try {
    ...
    public boolean isWifiEnabled() {
        if(OpenWrtNetUtils.isEnabled())
            return OpenWrtNetUtils.isWifiEnabled();
     ...
     *         {@link #WIFI_AP_STATE_ENABLING}, {@link #WIFI_AP_STATE_FAILED},  {@link #WIFI_AP_STATE_NEED_START}
     * @see #isWifiApEnabled()
     *
     * @hide
     */
   ...
   /**
    * Sets the Wi-Fi SoftAP mode.
    * @return {@code true} if the operation succeeded, {@code false} otherwise
    *
    * @hide Dont open yet
    */
   public boolean setSoftApMode(boolean enabled) {
       return true;
   }
