...
import com.realtek.server.RtkVoutUtilService;
import com.realtek.server.RtkAoutUtilService;
import com.realtek.server.RtkHDMIRxService;
import com.realtek.server.RtkHDMIService;
import com.realtek.server.SettingsService;
import com.android.server.RtkGPIOService;
import com.realtek.server.MediaQueryService;
import com.realtek.server.AirFunService;
import com.realtek.server.RtkDPTxService;
...
// [ RTK Begin]
        ToGoProxyService togoProxyService = null;
        TvManagerService tv = null;
        RtkVoutUtilService rtkVoutUtilService = null;
        RtkAoutUtilService rtkAoutUtilService = null;
        RtkHDMIService rtkHDMIService = null;
        RtkGPIOService rtkGPIOService = null;
        SettingsService settingsService = null;
        MediaQueryService mediaQueryService = null;
        RtkHDMIRxService rtkHDMIRxService = null;
        AirFunService airfun = null;
        RtkDPTxService rtkDPTxService = null;

        //Check if OpenWRT specific parameter exist then enable persist.rtk.net.openwrt property. 
        try {
            String result;
            result = getStringFromFile("/proc/cmdline");
            Slog.i(TAG, result);
            if(result.contains("init=/etc/init")) {
                Slog.i(TAG, "In OpenWRT");
                SystemProperties.set("persist.rtk.net.openwrt", "true");

		//Check if ottwifi exist then enable persist.rtk.net.openwrt.ottwifi
                File ottWifi = new File("/.ottwifi");
                if(ottWifi.exists()){
                    Slog.i(TAG, "Has .ottwifi flag");
                    SystemProperties.set("persist.rtk.net.openwrt.ottwifi", "true");
                } else {
                    SystemProperties.set("persist.rtk.net.openwrt.ottwifi", "false");
                    // SystemProperties.set("wifi.interface", "eth9");
                }
            }
...
// [ RTK Begin]
            traceBeginAndSlog("TvManager");
            tv = new TvManagerService(context);
            ServiceManager.addService(Context.TV_SERVICE, tv);
            //ActivityManagerService.self().setTvManager(tv);
            mActivityManagerService.setTvManager(tv);
            Trace.traceEnd(Trace.TRACE_TAG_SYSTEM_SERVER);

            if (SystemProperties.get("ro.config.with_airfun").equals("false")) {
                Slog.i(TAG, "No AirFun Service Registry");
            } else {
                traceBeginAndSlog("AirFunService");
                try{
                    airfun = new AirFunService(context);
                    ServiceManager.addService("airfun", airfun);
                } catch  (Throwable e) {
                    reportWtf("starting AirFun service", e);
                }
                Trace.traceEnd(Trace.TRACE_TAG_SYSTEM_SERVER);
            }

            if (SystemProperties.get("ro.with_togoproxy").equals("false")) {
                //Slog.i(TAG, "No ToGoProxy Service Registry");
                Slog.i(TAG, "\033[0;32m No ToGoProxy Service Registry \033[m");
            } else {
                traceBeginAndSlog("ToGoProxyService");
                togoProxyService = new ToGoProxyService(context);
                ServiceManager.addService("ToGoProxy", togoProxyService);
                Trace.traceEnd(Trace.TRACE_TAG_SYSTEM_SERVER);
            }
// [ RTK End]
...
// [ RTK Begin ]
        if ("true".equals(SystemProperties.get("ro.config.enablemediaquery"))) {
            traceBeginAndSlog("MediaQueryService");
            try {
                mediaQueryService = new MediaQueryService(context);
                ServiceManager.addService("MediaQueryService", mediaQueryService);
            } catch (Throwable e) {
                reportWtf("starting MediaQueryService", e);
            }
            Trace.traceEnd(Trace.TRACE_TAG_SYSTEM_SERVER);
        }

        if ("true".equals(SystemProperties.get("ro.config.enablehdmirxservice"))) {
            traceBeginAndSlog("RtkHDMIRxService");
            try {
                rtkHDMIRxService = new RtkHDMIRxService(context);
                ServiceManager.addService("RtkHDMIRxService", rtkHDMIRxService);
            } catch (Throwable e) {
                reportWtf("starting RtkHDMIRxService", e);
            }
            Trace.traceEnd(Trace.TRACE_TAG_SYSTEM_SERVER);
        }

        if ("true".equals(SystemProperties.get("ro.config.enablevoututilservice"))) {
            traceBeginAndSlog("RtkVoutUtilService");
            try {
                rtkVoutUtilService = new RtkVoutUtilService(context);
                ServiceManager.addService("RtkVoutUtilService", rtkVoutUtilService);
            } catch (Throwable e) {
                reportWtf("starting RtkVoutUtilService", e);
            }
            Trace.traceEnd(Trace.TRACE_TAG_SYSTEM_SERVER);
        }
        else
        {
            Slog.i(TAG, "\033[0;32m Not start RtkVoutUtilService \033[m");
        }

        if ("true".equals(SystemProperties.get("ro.config.enableaoututilservice"))) {
            traceBeginAndSlog("RtkAoutUtilService");
            try {
                rtkAoutUtilService = new RtkAoutUtilService(context);
                ServiceManager.addService("RtkAoutUtilService", rtkAoutUtilService);
            } catch (Throwable e) {
                reportWtf("starting RtkAoutUtilService", e);
            }
            Trace.traceEnd(Trace.TRACE_TAG_SYSTEM_SERVER);
        }
        else
        {
            Slog.i(TAG, "\033[0;32m Not Start RtkAoutUtilService \033[m");
        }

        if ("true".equals(SystemProperties.get("ro.config.enablegpioservice"))) {
            traceBeginAndSlog("RtkGPIOService");
            try {
                rtkGPIOService = new RtkGPIOService(context);
                ServiceManager.addService("RtkGPIOService", rtkGPIOService);
            } catch (Throwable e) {
                reportWtf("starting RtkGPIOService", e);
            }
            Trace.traceEnd(Trace.TRACE_TAG_SYSTEM_SERVER);
        }
// [ RTK End]
...
// [ RTK Begin ]
				if ("true".equals(SystemProperties.get("ro.config.use_settingsservice"))) {
					try {
                        traceBeginAndSlog("SettingsService");
						settingsService = new SettingsService(context);
						ServiceManager.addService("SettingsService", settingsService);
                        Trace.traceEnd(Trace.TRACE_TAG_SYSTEM_SERVER);
					} catch (Throwable e) {
						reportWtf("starting SettingsService", e);
					}
				}

                if ("true".equals(SystemProperties.get("ro.config.enablehdmiservice"))) {
                    try {
                        traceBeginAndSlog("RtkHDMIService");
                        rtkHDMIService = new RtkHDMIService(context);
                        ServiceManager.addService("RtkHDMIService", rtkHDMIService);
                        Trace.traceEnd(Trace.TRACE_TAG_SYSTEM_SERVER);
                    } catch (Throwable e) {
                        reportWtf("starting RtkHDMIService", e);
                    }
                }
                else
                {
                    Slog.i(TAG, "\033[0;32m Not Start RtkHDMIService \033[m");
                }

				if ("true".equals(SystemProperties.get("ro.config.enabledptxservice"))) {
					try {
                        traceBeginAndSlog("RtkDPTxService");
						rtkDPTxService = new RtkDPTxService(context);
						ServiceManager.addService("RtkDPTxService", rtkDPTxService);
                        Trace.traceEnd(Trace.TRACE_TAG_SYSTEM_SERVER);
					} catch (Throwable e) {
						reportWtf("starting RtkDPTxService", e);
					}
				}
...
// [ RTK End ]
            ...
            if (mPackageManager.hasSystemFeature(PackageManager.FEATURE_HDMI_CEC)) {
                Slog.d(TAG, "Start HdmiControlService ");
