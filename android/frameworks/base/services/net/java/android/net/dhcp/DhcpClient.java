...
import com.rtk.net.openwrt.UbusUtil;
...
public class DhcpClient extends StateMachine {
    ...
    public static DhcpClient makeDhcpClient(
        ...
        if(isOpenWrtWifi() && "wlan0".equals(intf))
            intf = "eth9";
    ...
    private static boolean isOpenWrtWifi(){
        ...
        protected void receivePacket(DhcpPacket packet) {
            ...
            if (mOffer != null) {
                ...
                if(!UbusRpc.isXen() && UbusRpc.isOpenWrt() && !UbusRpc.isOttWifi()){
                    UbusUtil uu = UbusUtil.getInstance(mContext);
                    if(DBG) Log.d(TAG, "OpenWrt disabled dhcp server ="+uu.isDhcpLanIgnoreHasValueOne());
                    if(!uu.isDhcpLanIgnoreHasValueOne()){
                        String openWrtDhcpServerIp=uu.getOpenWrtDhcpServerIp();
                        if(DBG) Log.d(TAG, "getOpenWrtDhcpServerIp="+openWrtDhcpServerIp);
                        if(DBG) Log.d(TAG, "mOffer.serverAddress.toString()="+mOffer.serverAddress.toString());
                        if(openWrtDhcpServerIp!=null && openWrtDhcpServerIp.length()>0){
                            if(!mOffer.serverAddress.toString().endsWith(openWrtDhcpServerIp)){
                                Log.i(TAG, "get dhcp offer other than OpenWrt, ignore it");
                                mOffer = null;
                                return;
                            } else {
                                Log.i(TAG, "get dhcp offer from OpenWrt");
                            }
                        } else {
                            Log.w(TAG, "get dhcp info from OpenWrt failed");
                        }
                    }
                }
    ...
    class DhcpBoundState extends LoggingState {
        ...
        public void enter() {
            super.enter();
            if (mDhcpLease != null && mDhcpLease.serverAddress != null && !connectUdpSock(mDhcpLease.serverAddress)) {
