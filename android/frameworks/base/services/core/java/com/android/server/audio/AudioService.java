...
*/
    ...
    public AudioService(Context context) {
        ...
        intentFilter.addAction("rtk.intent.action.POWER_SUSPEND");
    ...
    private void setStreamVolume(int streamType, int index, int flags, String callingPackage,
        ...
        /* rtk begin: [ATM-964] RTK tv platform has more than one audio output device */
        /* We should use streamTypeAlias instead of streamType to find the exact device when setting stream volume */
        final int device = getDeviceForStream(streamTypeAlias);
        /* rtk end */
    ...
    private int checkForRingerModeChange(int oldIndex, int direction, int step, boolean isMuted,
        ...
        case RINGER_MODE_NORMAL:
            if (direction == AudioManager.ADJUST_LOWER) {
                ...
                // rtk TV version mute issue. TV can NOT setRingerMode so have to ADJUST_VOLUME
    ...
    private boolean handleDeviceConnection(boolean connect, int device, String address,
        ...
        synchronized (mConnectedDevices) {
            ...
            if (connect && !isConnected) {
                ...
                return true;
            } else if ((isConnected && connect) && device == AudioSystem.DEVICE_OUT_ANLG_DOCK_HEADSET) {
                // rtk add:
                // for usb_audio support multi cards case
                // card is Connected and the other card plug-in
                final int res = AudioSystem.setDeviceConnectionState(device,
                        AudioSystem.DEVICE_STATE_AVAILABLE, address, deviceName);
                if (res != AudioSystem.AUDIO_STATUS_OK) {
                    Slog.e(TAG, "not connecting device 0x" + Integer.toHexString(device) +
    ...
    private void sendDeviceConnectionIntent(int device, int state, String address,
        ...
        synchronized (mCurAudioRoutes) {
            if (connType != 0) {
                ...
                // for usb_audio support multi cards case
                if (newConn != mCurAudioRoutes.mainType || (device == AudioSystem.DEVICE_OUT_ANLG_DOCK_HEADSET && state == 1)) {
    ...
    private class AudioServiceBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            } else if (action.equals(Intent.ACTION_SCREEN_OFF) || action.equals("rtk.intent.action.POWER_SUSPEND")) {
                if (mMonitorRotation) {
