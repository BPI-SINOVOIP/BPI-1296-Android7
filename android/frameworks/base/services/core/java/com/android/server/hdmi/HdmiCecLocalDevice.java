...
abstract class HdmiCecLocalDevice {
    ...
    protected final boolean onMessage(HdmiCecMessage message) {
        ...
        if (dispatchMessageToAction(message)) {
            Slog.d(TAG,"dispatchMessageToAction: " + message.toString());
            return true;
        }
        Slog.d(TAG,"onMessage: " + message.toString());
        switch (message.getOpcode()) {
            ...
            case Constants.MESSAGE_SET_MENU_LANGUAGE:
                return handleSetLanguage(message); //FIXME: use rtk version temporarily
    ...
    protected boolean handleStandby(HdmiCecMessage message) {
        assertRunOnServiceThread();
        if (SystemProperties.get("persist.rtk.cec.standby","true").equals("false")){
            Slog.d(TAG, "Standby is disabled by SystemProperties");
            return true;
        }
        // Seq #12
    ...
    protected boolean handleSetLanguage(HdmiCecMessage message) {
        Slog.d(TAG,"setDeviceLanguage: " + message.toString());
        if (SystemProperties.get("persist.rtk.cec.setlanguage","true").equals("false")){
            Slog.d(TAG, "Set Language is disabled by SystemProperties");
            return true;
        }
        ...
        //With regard to ISO 639-2, Chinese is noted as chi and English is noted as eng.
        String cec_lang = new String(message.getParams()); 
        Locale locale;
        if(cec_lang.equals("chi"))
            locale = Locale.TAIWAN;
        else if(cec_lang.equals("zho"))
            locale = Locale.SIMPLIFIED_CHINESE;
        else
            locale = Locale.US;

        Slog.d(TAG,"cec_lang is " + cec_lang + " , to locale: " + locale.getLanguage() + " , Default: " + Locale.getDefault());

        //request the same language, do nothing.
