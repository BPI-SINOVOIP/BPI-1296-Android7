    ...
    private final IBinder mService = new IAlarmManager.Stub() {
        ...
        public boolean setTime(long millis) {
            ...
            if((netinfo != null) && (netinfo.getType() == ConnectivityManager.TYPE_WIFI)){
                ...
                int i=0;
                final int TIMEOUT_MS = 6000;
                final int DELAY_MS = 2000;
                final int SLEEP_INTERVAL = 100;
                while(!SupplicantState.COMPLETED.equals(supState) && i<TIMEOUT_MS){
                    try{
                ...
                if(i>=TIMEOUT_MS) {
                    Slog.w(TAG, "wifi SupplicantState checking timeout");
                } else {
                    Slog.i(TAG, "wifi SupplicantState.COMPLETED, wait "+DELAY_MS+" ms");
                    i=0;
                    while(i<DELAY_MS){
                        try{
                            Thread.sleep(SLEEP_INTERVAL);
                ...
                Slog.i(TAG, "setTime in nonwifi network");
                synchronized (mLock) {
                    return setKernelTime(mNativeData, millis) == 0;
                }
            }
