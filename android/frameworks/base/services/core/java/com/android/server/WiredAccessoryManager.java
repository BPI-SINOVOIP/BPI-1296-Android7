...
final class WiredAccessoryManager implements WiredAccessoryCallbacks {
    ...
    private boolean flag_usb_change;
    ...
    private void updateLocked(String newName, int newState) {
        ...
        Slog.v(TAG, "flag_usb_change = " + flag_usb_change);
        ...
        //rtk for support multi usb_audio card case
        //to info there is a new card plug
        if (flag_usb_change) {
            mHeadsetState &= ~BIT_USB_HEADSET_ANLG;
        ...
        private void updateStateLocked(String devPath, String name, int state) {
            for (int i = 0; i < mUEventInfo.size(); ++i) {
                ...
                if (devPath.equals(uei.getDevPath())) {
                    if ( name.compareTo(NAME_USB_AUDIO) == 0 && state == 1)
