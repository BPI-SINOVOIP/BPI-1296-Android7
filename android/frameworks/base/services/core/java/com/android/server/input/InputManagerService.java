...
public class InputManagerService extends IInputManager.Stub
    ...
    private static native void nativeSetShowMouseCursor(long ptr, boolean enabled);
    ...
    public void start() {
        ...
        registerShowMouseCursorSettingObserver();
        ...
        mContext.registerReceiver(new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                updateShowMouseCursorFromSettings();
            }
    ...
    public void updateShowMouseCursorFromSettings() {
    	int setting = getShowMouseCursorSetting(1);//1->true,0->false
    	Slog.d(TAG, "updateShowMouseCursorFromSettings,setting="+setting);
        nativeSetShowMouseCursor(mPtr, setting != 0);
    }
    ...
    private void registerShowMouseCursorSettingObserver() {
    	//initial set to 1
    	Slog.d(TAG, "registerShowMouseCursorSettingObserver 0");
    	Settings.System.putInt( mContext.getContentResolver(), Settings.System.SHOW_MOUSECURSOR, 1);
    	Slog.d(TAG, "registerShowMouseCursorSettingObserver 1");
        mContext.getContentResolver().registerContentObserver(
                Settings.System.getUriFor(Settings.System.SHOW_MOUSECURSOR), true,
                new ContentObserver(mHandler) {
                    @Override
                    public void onChange(boolean selfChange) {
                    	Slog.d(TAG, "registerShowMouseCursorSettingObserver,onChange");
                        updateShowMouseCursorFromSettings();
                    }
                }, UserHandle.USER_ALL);
    }
    ...
    private int getShowMouseCursorSetting(int defaultValue) {
        int result = defaultValue;
        try {
            result = Settings.System.getIntForUser(mContext.getContentResolver(),
                    Settings.System.SHOW_MOUSECURSOR, UserHandle.USER_CURRENT);
			Slog.d(TAG, "getShowMouseCursorSetting 0,result="+result);
        } catch (SettingNotFoundException snfe) {
        	Slog.d(TAG, "getShowMouseCursorSetting 1,result="+result);
        }
        return result;
    }
