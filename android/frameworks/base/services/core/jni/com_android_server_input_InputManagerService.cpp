...
NativeInputManager::NativeInputManager(jobject contextObj,
...

    {
        ...
        mLocked.showMouseCursor = true;
        ALOGI("NativeInputManager, mLocked.showMouseCursor=%d", mLocked.showMouseCursor);
    }
    mInteractive = true;
...
void NativeInputManager::getReaderConfiguration(InputReaderConfiguration* outConfig) {
    ...
    { // acquire lock
        ...
        outConfig->showMouseCursor = mLocked.showMouseCursor;
...
void NativeInputManager::setShowMouseCursor(bool enabled) {
    { // acquire lock
        AutoMutex _l(mLock);
		ALOGI("Setting show mousecursor showMouseCursor=%d,enabled=%d", mLocked.showMouseCursor,enabled);
        if (mLocked.showMouseCursor == enabled) {
            return;
        }

        ALOGI("Setting show mousecursor feature to %s.", enabled ? "enabled" : "disabled");
        mLocked.showMouseCursor = enabled;
    } // release lock

    mInputManager->getReader()->requestRefreshConfiguration(
            InputReaderConfiguration::CHANGE_SHOW_CURSOR);
}
...
static void nativeSetShowMouseCursor(JNIEnv* env,
        jclass clazz, jlong ptr, jboolean enabled) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);

    im->setShowMouseCursor(enabled);
}
...
static const JNINativeMethod gInputManagerMethods[] = {
    ...
    { "nativeSetShowMouseCursor", "(JZ)V",
            (void*) nativeSetShowMouseCursor },
