...
public class Input {
        ...
        try {
            if (command.equals("text")) {
                ...
                if (length >= 2) {
                    ...
                    int mode3D = 0;
                    if("--sbs".equals(args[index + 1])) {
                        mode3D=1;
                    }
                    else if("--tb".equals(args[index + 1])) {
                        mode3D=2;
                    }
                    final int start = (longpress || mode3D>0) ? index + 2 : index + 1;
                    inputSource = getSource(inputSource, InputDevice.SOURCE_KEYBOARD);
                    if (length > start) {
                        for (int i = start; i < length; i++) {
                            ...
                            sendKeyEvent(inputSource, keyCode, longpress, mode3D);
                        }
                        return;
                    }
                }
    ...
    private void sendKeyEvent(int inputSource, int keyCode, boolean longpress, int mode3D) {
        long now = SystemClock.uptimeMillis();
        int metaState = 0;
        if(longpress==false) {
            if(mode3D==1) {
                metaState |= KeyEvent.META_3DMODE_SBS;
            }
            else if(mode3D==2) {
                metaState |= KeyEvent.META_3DMODE_TB;
            }
        }
        injectKeyEvent(new KeyEvent(now, now, KeyEvent.ACTION_DOWN, keyCode, 0, metaState,
                KeyCharacterMap.VIRTUAL_KEYBOARD, 0, 0, inputSource));
        if (longpress) {
            injectKeyEvent(new KeyEvent(now, now, KeyEvent.ACTION_DOWN, keyCode, 1, metaState,
                    KeyCharacterMap.VIRTUAL_KEYBOARD, 0, KeyEvent.FLAG_LONG_PRESS,
                    inputSource));
        }
        injectKeyEvent(new KeyEvent(now, now, KeyEvent.ACTION_UP, keyCode, 0, metaState,
                KeyCharacterMap.VIRTUAL_KEYBOARD, 0, 0, inputSource));
    }
