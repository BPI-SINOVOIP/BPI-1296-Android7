...
jstring encodedFormatToString(JNIEnv* env, SkEncodedFormat format) {
    ...
    switch (format) {
        ...
        case SkEncodedFormat::kPNG_SkEncodedFormat:
            mimeType = "image/png";
            break;
        case SkEncodedFormat::kTIFF_SkEncodedFormat:
            mimeType = "image/tiff";
            break;
...
static jobject nativeDecodeStream(JNIEnv* env, jobject clazz, jobject is, jbyteArray storage,
    ...
    if (stream.get()) {
        ...
        options = env->NewGlobalRef(options);
        bitmap = doDecode(env, bufferedStream.release(), padding, options);
        env->DeleteGlobalRef(options);
    }
    return bitmap;
}
...
static jobject nativeDecodeFileDescriptor(JNIEnv* env, jobject clazz, jobject fileDescriptor,
	...
	*/
    ...
    bitmapFactoryOptions = env->NewGlobalRef(bitmapFactoryOptions);
    jobject retObj = doDecode(env, fileStream.release(), padding, bitmapFactoryOptions);
    env->DeleteGlobalRef(bitmapFactoryOptions);
    return retObj;
}
...
static jobject nativeDecodeAsset(JNIEnv* env, jobject clazz, jlong native_asset,
    ...
    options = env->NewGlobalRef(options);
    jobject retObj = doDecode(env, stream.release(), padding, options);
    env->DeleteGlobalRef(options);
    return retObj;
}
...
static jobject nativeDecodeByteArray(JNIEnv* env, jobject, jbyteArray byteArray,
    ...
    options = env->NewGlobalRef(options);
    jobject retObj = doDecode(env, stream.release(), NULL, options);
    env->DeleteGlobalRef(options);
    return retObj;
}

#if 0
static void nativeRequestCancel(JNIEnv* env, jobject joptions) {
    (void)AutoDecoderCancel::RequestCancel(env, joptions);
}
#endif
