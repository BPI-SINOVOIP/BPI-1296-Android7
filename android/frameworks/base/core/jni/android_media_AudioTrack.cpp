...
android_media_AudioTrack_setup(JNIEnv *env, jobject thiz, jobject weak_this, jobject jaa,
    ...
    if (nativeAudioTrack == 0) {
        ...
        size_t frameCount;
//rtk modify
//to match android_media_AudioTrack_get_min_buff_size and  AudioFlinger::ThreadBase::TrackBase::TrackBase
...
/*
 * rtk begin
 * Workaround for ATM-968 CTS AudioTrackSurroundTest
 * Increase frame count of IEC61937 and AC3 format to prevent from getting underrun
 */
        if (format == AUDIO_FORMAT_IEC61937 && channelCount <= 2){
            size_t rtkFrameCount = frameCount * 12;
            ALOGD("IEC61937 format: Increase frameCount from %zu to %zu", frameCount, rtkFrameCount);
            return rtkFrameCount * channelCount * bytesPerSample;
        }
        else
            return frameCount * channelCount * bytesPerSample;
    } else if (format == AUDIO_FORMAT_AC3){
        size_t rtkFrameCount = frameCount * 3;
        ALOGD("AC3 format: Increase frameCount from %zu to %zu", frameCount, rtkFrameCount);
        return rtkFrameCount;
/* rtk end */
    }else {
        return frameCount;
    }
