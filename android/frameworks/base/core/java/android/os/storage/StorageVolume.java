...
public final class StorageVolume implements Parcelable {
     ...
     */
    // By hamer@20140110
    // Using Long.parseLong to replace Integer.parseInt
    // to avoid issuing NumberFormatException if number is
    // larger than 0x7FFFFFFF
    public int getFatVolumeId() {
        if (mFsUuid == null /*|| mFsUuid.length() != 9*/) { // remove the length restriction for non-fat file system
            return -1;
        }
        try {
            return (int) Long.parseLong(mFsUuid.replace("-", ""), 16);
        } catch (NumberFormatException e) {
            return (int) getShortVolumeId(); // if converting to Long causes overflow exception,
                                       // try to covert short uuid string.
        }
    }

    private long getShortVolumeId() {
        String id = mFsUuid.replace("-","");
        try {
            return Long.parseLong(id.substring(0, id.length()<14?id.length():14), 16);
        } catch (NumberFormatException e) {
            return -1;
        }
    }
