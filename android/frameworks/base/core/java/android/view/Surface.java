...
public class Surface implements Parcelable {
    ...
    private static native int nativeSetUsage(long nativeObject, int usage);
    ...
    /**
     * Set the usage to be used for this surfaces buffers
     * @hide
     */
    void setUsage(int usage) {
        synchronized (mLock) {
            checkNotReleasedLocked();
            int err = nativeSetUsage(mNativeObject, usage);
            if (err != 0) {
