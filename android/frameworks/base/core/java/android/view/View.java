...
public class View implements Drawable.Callback, KeyEvent.Callback,
    ...
    */
    ...
    /**
     * Send broadcast to control panel during a touch down event.
     *
     ...
     * @hide
     */
	protected boolean sendBroadcastControlPanelOnTouchDown(MotionEvent event)
	{
		if (event.getAction()==MotionEvent.ACTION_DOWN && (event.getButtonState() & MotionEvent.BUTTON_SECONDARY) != 0) {
			Intent intent = new Intent();
			intent.setAction("com.rtk.ControlPanel.ControlPanel_Action");
    ...
    */
    ...
    public final boolean dispatchPointerEvent(MotionEvent event) {
        if (event.isTouchEvent()) {
			if( sendBroadcastControlPanelOnTouchDown(event) )
				return true;
			else
