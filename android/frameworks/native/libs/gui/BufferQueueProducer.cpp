...
/*
 *  RTK_ATM_789_WORKAROUND
 *
 *  The new architecture supports dynamically adjusting the number of
 *  buffers, but the architecture does not match the RTK platform because
 *  the RTK platform needs to know all the buffers at the time of begin
 *  play.
 */
#define RTK_ATM_789_WORKAROUND
...
status_t BufferQueueProducer::setMaxDequeuedBufferCount(
    ...
    { // Autolock scope
        ...
        if (listener != NULL)
            listener->onProducerNotify(BUFFER_RELEASING, 0);
        ...
        if (listener != NULL)
            listener->onProducerNotify(BUFFER_COUNT, mCore->getMaxBufferCountLocked());
    } // Autolock scope
...
status_t BufferQueueProducer::connect(const sp<IProducerListener>& listener,
    ...
    ATRACE_CALL();
    Mutex::Autolock lock(mCore->mMutex);
    sp<IConsumerListener> consumerListener = NULL;
    mConsumerName = mCore->mConsumerName;
...
#ifdef USE_RT_MEDIA_PLAYER
    {
        switch (api) {
            ...
            case NATIVE_WINDOW_API_RTK_SIDEBAND:
    ...
    if (consumerListener != NULL) {
        consumerListener->onProducerNotify(CONNECT_API_START, api);
    }

    {
        char value [PROPERTY_VALUE_MAX];
        int count = 3;  /* for interlaced case */
        if (property_get("persist.rtk.video.maxAcqCnt", value, "3"))
            count = atoi(value);
        ...
        switch (api) {
            ...
            case NATIVE_WINDOW_API_CAMERA:
                if (count > 0 && count < 6) {
                    int delta = count - mCore->mMaxAcquiredBufferCount;
                    if (delta > 0) {
                        if (!mCore->adjustAvailableSlotsLocked(delta))
                            BQ_LOGE("connect(P): error maxAcquiredCount (%d) adjustAvailableSlotsLocked return fail!", count);
                        else
                            mCore->mMaxAcquiredBufferCount = count;
                    }
                } else
                    BQ_LOGE("connect(P): error maxAcquiredCount (%d)", count);
        }
    }
...
/*
#ifdef MAX_ACQUIRED_BUFFER_COUNT
    if (mCore->mDequeueBufferCannotBlock
            && (mCore->mMaxAcquiredBufferCount > 1)
            && (mCore->mMaxAcquiredBufferCount == MAX_ACQUIRED_BUFFER_COUNT))
        mCore->mMaxAcquiredBufferCount--;
#endif
*/
    if (consumerListener != NULL)
        consumerListener->onProducerNotify(CONNECT_READY, mCore->mConnectedApi);
...
status_t BufferQueueProducer::disconnect(int api, DisconnectMode mode) {
    ...
    if (status == NO_ERROR && consumerListener != NULL)
        consumerListener->onProducerNotify(DISCONNECT, mCore->mConnectedApi);
