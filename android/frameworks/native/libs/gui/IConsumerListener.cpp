...
public:
    ...
    virtual void onProducerNotify(const int notify, const int ext) {
        Parcel data, reply;
        data.writeInterfaceToken(IConsumerListener::getInterfaceDescriptor());
        data.write(&notify, sizeof(notify));
        data.write(&ext, sizeof(ext));
        remote()->transact(ON_PRODUCER_NOTIFY, data, &reply, IBinder::FLAG_ONEWAY);
    }
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch(code) {
        case ON_FRAME_AVAILABLE: {
            ...
            return NO_ERROR; }
        case ON_PRODUCER_NOTIFY: {
            CHECK_INTERFACE(IConsumerListener, data, reply);
            int notify;
            int ext;
            data.read(&notify, sizeof(notify));
            data.read(&ext, sizeof(ext));
            onProducerNotify(notify, ext);
            return NO_ERROR; }
