...
#define REMAPP_MOUSE_EVENT 1
#if REMAPP_MOUSE_EVENT
static bool bCursorEnable = false;
static bool bGesturePress = false;
static int offset=15;
static int iRepeatCount = 0;
static RawEvent backupRawEvent[2];
InputReader* mThisReader;
...
void timer_sig_handler(int arg)
{
	...
	backupRawEvent[0].when = now;
	backupRawEvent[1].when = now;
	
	mThisReader->processEventsLocked(backupRawEvent, 2);

	/*
	ALOGD("timer_sig_handler event: device=%d type=0x%04x code=0x%04x value=0x%08x when=%lld iRepeatCount =%d",
			backupRawEvent[0].deviceId, backupRawEvent[0].type, backupRawEvent[0].code, backupRawEvent[0].value,
			backupRawEvent[0].when, iRepeatCount);
	*/			
	iRepeatCount ++;
	if (iRepeatCount == 2)
	{
		backupRawEvent[0].value = backupRawEvent[0].value * 2;		
		stop_timer();
		start_timer(100);
	}
	else if (iRepeatCount == 5)
	{
		backupRawEvent[0].value = backupRawEvent[0].value * 2;
	}
}

size_t InputReader::RemapKey2Mouse(RawEvent* rawEvents, size_t count)
{
	...
	for (RawEvent* rawEvent = rawEvents; count;)
	{
		...
		{			
			if ((rawEvent->code == KEY_TOUCHPAD_OFF)&&(rawEvent->value!=0))
				bCursorEnable = !bCursorEnable;
			

			if (bCursorEnable)
			{
				ALOGD("OutPut event: device=%d type=0x%04x code=0x%04x value=0x%08x when=%lld first=%d",
						rawEvent->deviceId, rawEvent->type, rawEvent->code, rawEvent->value,
						rawEvent->when);
													
				if(rawEvent->code == KEY_RIGHT)
				{	
					...
					if (rawEvent->value!=0)
					{
						mThisReader = this;
						start_timer(350);
						rawEvent->value = offset;
						memcpy((void*)&(backupRawEvent[0]),rawEvent,sizeof(RawEvent));
						backupRawEvent[1].deviceId = backupRawEvent[0].deviceId;
						iRepeatCount = 0;
					}
					else
					{
				...
				else if(rawEvent->code == KEY_LEFT)
				{
					rawEvent->type = EV_REL;
					rawEvent->code = REL_X;
					if (rawEvent->value!=0)
				...
				else if(rawEvent->code == KEY_DOWN)
				{
					rawEvent->type = EV_REL;
					rawEvent->code = REL_Y;
					if (rawEvent->value!=0)
				...
				else if(rawEvent->code == KEY_OK)
				{
					//rawEvent->type = EV_KEY;
					rawEvent->code = BTN_LEFT;
					//rawEvent->value = 1;					
				}		
				else if (rawEvent->code == KEY_TOUCHPAD_ON)
				{				
					if (rawEvent->value!=0)
					{
						bGesturePress = !bGesturePress;
						rawEvent->code = BTN_LEFT;
						if (bGesturePress)
							rawEvent->value = 1;					
						else
...
#if REMAPP_MOUSE_EVENT
		count = RemapKey2Mouse(mEventBuffer, count);
#endif
...
void InputReader::processEventsForDeviceLocked(int32_t deviceId,
    ...
    if (mWFD) {
	 /***
        the values of volume control are defined in device/realtek/proprietary/libs/Include/Platform_Lib/HDMIControl/mhl_rcp_priv.h
        	KEYCODE_VOLUME_UP   = 115,
        	KEYCODE_VOLUME_DOWN = 114,
         	KEYCODE_VOLUME_MUTE = 113,
         ***/
         if((rawEvents->code >= 113) && (rawEvents->code <= 115))
         	ALOGI("Miracast connected, pass the volume setting from input devices.");
         else if (rawEvents->code == 158)	//KEYCODE_BACK
                ALOGI("Miracast connected, pass back key event from input devices.");
         else
         {
                ALOGI("Miracast connected, ignore input devices.");
                return;
         }
    }
...
void InputReader::refreshConfigurationLocked(uint32_t changes) {
    ...
    if (changes) {
		...
		if (changes & (InputReaderConfiguration::CHANGE_SHOW_CURSOR)) {
			ALOGI("Reconfiguring input devices.  mWFD=%d", mWFD);
			mWFD = !mConfig.showMouseCursor;//showMouseCursor true: wfd false,
		}
    }
}
...
void CursorInputMapper::configure(nsecs_t when,
    ...
    //test
    ALOGI("[%s][%s][%d],changes=0x%x\n", __FILE__, __func__, __LINE__,changes);
    if (changes & (InputReaderConfiguration::CHANGE_SHOW_CURSOR)) {
		ALOGI("[%s][%s][%d],bVisible=%d\n", __FILE__, __func__, __LINE__,config->showMouseCursor);
		mPointerController->SetVisible(config->showMouseCursor);
	}
