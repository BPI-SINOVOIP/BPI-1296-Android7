        ...
        for (size_t dpy=0 ; dpy<mDisplays.size() ; dpy++) {
            ...
            if (id >= 0) {
                ...
                for (size_t i=0 ; cur!=end && i<count ; ++i, ++cur) {
                    ...
                    if (Layer::eRTKVideoFlagsMark & layer->mRTKVideoFlags) {
                        // commented by hamer@20130522
                        // For the RTKVideo layer, it is handled directly by VO
                        // so SurfaceFlinger doesn't know whether it is dirty or not
                        // So, always set mHwWorkListDirty to true if there exists
                        // RTKVideo layer to prevent the layer from being covered with
                        // black surface.
                        unsigned int hint = HWC_HINT_RTK_SURFACEFLINGER2HAL_VIDEO_LAYER;
                        mHwWorkListDirty = true;
                        if (Layer::eRTKVideoFlagsExt & layer->mRTKVideoFlags)
                            hint |= HWC_HINT_RTK_SURFACEFLINGER2HAL_VIDEO_LAYER_EXT;
                                (*cur).setHints(hint);
                    } else if (Layer::eRTKMediaFlagsMark & layer->mRTKVideoFlags) {
                        mHwWorkListDirty = true;
                        (*cur).setHints(HWC_HINT_RTK_SURFACEFLINGER2HAL_MEDIA_LAYER);
                    }
