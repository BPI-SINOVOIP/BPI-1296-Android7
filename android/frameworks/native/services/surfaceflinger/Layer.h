...
private:
    ...
    virtual void onProducerNotify(const int notify, const int ext) override;
...
public:
    ...
    enum {
        eRTKVideoFlagsMark      = 0x1U << 1,
        eRTKMediaFlagsMark      = 0x1U << 2,
        eRTKVideoFrameReady     = 0x1U << 3,
        eRTKMediaSetDefaultSize = 0x1U << 4,
        eRTKHwcCleanupReady     = 0x1U << 5,
        eRTKVideoFlagsExt       = 0x1U << 6,
    };

    void modifyRTKFlags(FlagMode mode, unsigned int flags);
    /* End RTK Mark for RTK Video Player */
    uint32_t mRTKVideoFlags;
