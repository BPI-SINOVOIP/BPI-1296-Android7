    ...
    for (size_t displayId = 0; displayId < mDisplays.size(); ++displayId) {
		...
		for (auto& layer : displayDevice->getVisibleLayersSortedByZ()) {
			...
			if (Layer::eRTKVideoFlagsMark & layer->mRTKVideoFlags) {
				layer->setCompositionType(hwcId, HWC2::Composition::Scale);
				if (Layer::eRTKVideoFlagsExt & layer->mRTKVideoFlags)
					layer->setCompositionType(hwcId, HWC2::Composition::Scale_ext);

			} else if (Layer::eRTKMediaFlagsMark & layer->mRTKVideoFlags) {
				auto type = layer->getCompositionType(hwcId);
				if(type != HWC2::Composition::Sideband){
					layer->setCompositionType(hwcId, HWC2::Composition::Overlay);
				}
			}
        ...
        for (auto& layer : displayDevice->getVisibleLayersSortedByZ()) {
            ...
            if (!clip.isEmpty()) {
                switch (layer->getCompositionType(hwcId)) {
                    ...
                    case HWC2::Composition::Overlay:
                    case HWC2::Composition::Scale:
                    case HWC2::Composition::Scale_ext:
