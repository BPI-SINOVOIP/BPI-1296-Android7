...
public class WifiP2pServiceImpl extends IWifiP2pManager.Stub {
    ...
    public static final int WFD_SOURCE_ROLE   = 0;
    public static final int WFD_SINK_ROLE            = 1;
    ...
    private static final int DISCOVER_TIMEOUT_S = 120;
     ...
     /* Set a 3 seconds discover timeout to avoid STA scans from being blocked */
    private static final int DISCOVER_TIMEOUT_AUTO_S = 3;
    ...
    public static final int SET_MIRACAST_MODE_AP               =   BASE + 17;

    // WifiStateMachine notify P2P that AP is reconnected successfully
    public static final int CMD_NETWORK_CONNECTION_EVENT         = BASE + 18;

    // WifiStateMachine notify P2P that AP is reconnected failure
    public static final int CMD_ASSOCIATION_REJECTION_EVENT         = BASE + 19;
    public static final int CMD_AUTHENTICATION_FAILURE_EVENT         = BASE + 20;
    ...
    // Realtek: Need to fine tune the p2p_command due to the role
    // EX: If current action is triggered by sink, WifiP2pService do not need to 
    //       send the P2P_FIND (for source role) to wpa_supplicant
    private int mWfdRole = WFD_SINK_ROLE;
    ...
    private class ClientHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
              ...
              case WifiP2pManager.LISTEN_PEERS:
              case WifiP2pManager.LISTEN_PEERS_AUTO:
     ...
     * when acting as a source or sink to minimize impact on miracast.
     */
    public void setMiracastMode_AP(int mode) {
        enforceConnectivityInternalPermission();
        mP2pStateMachine.sendMessage(SET_MIRACAST_MODE_AP, mode);
    }
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiP2pManager.DISCOVER_PEERS:
                    ...
                    break;
                case WifiP2pManager.LISTEN_PEERS:
                	logd(getName() + " WifiP2pManager.LISTEN_PEERS");
                		if (mWifiNative.p2pListen(DISCOVER_TIMEOUT_S)) {
                		logd("P2p LISTEN_PEERS successful");
                	}
                	else {
                		logd("P2p LISTEN_PEERS NG");
                	}
                	break;
                 case WifiP2pManager.LISTEN_PEERS_AUTO:
                	logd("LISTEN_PEERS"+DISCOVER_TIMEOUT_AUTO_S);
                	if (mWifiNative.p2pListen(DISCOVER_TIMEOUT_AUTO_S)) {
                		logd("P2p LISTEN_PEERS_AUTO successful");
                	}
                	else {
                		logd("P2p LISTEN_PEERS_AUTO NG");
                	}
                	break;
                case WifiMonitor.P2P_FIND_STOPPED_EVENT:
                ...
                case SET_MIRACAST_MODE:
                    ...
                    break;
                 case SET_MIRACAST_MODE_AP:
                    if (DBG) logd(getName() + "  SET_MIRACAST_MODE_AP: " + message.arg1);
                    //mWifiNative.setMiracastMode(message.arg1);
					boolean bWifiConnected = false;
                    ConnectivityManager mConnectionMgr = (ConnectivityManager) mContext.getSystemService(Context.CONNECTIVITY_SERVICE);
                    NetworkInfo wifiInfo = mConnectionMgr.getNetworkInfo(ConnectivityManager.TYPE_WIFI);
					
                    if(message.arg1 == WifiP2pManager.MIRACAST_SOURCE)
                        {
                            mWfdRole = WFD_SOURCE_ROLE;
                        }
                    else if(message.arg1 == WifiP2pManager.MIRACAST_SINK)
                        {
                            mWfdRole = WFD_SINK_ROLE;
							//if (wifiInfo != null && wifiInfo.isConnected()) { // Check wifi is connected to AP
							if (wifiInfo != null) { //Check if wifi exists whether it connected to AP or not
								//if (DBG)  logd("Wifi is CONNECTED to AP, try to disconnect wifi first" );
								if (DBG)  logd("Wifi exists, try to disconnect wifi first" );
								bWifiConnected = true;                                    
							} else {
								//if (DBG)  logd("Wifi is NOT connected to AP" );
								if (DBG)  logd("Wifi not exist, not do anything" );
							}
							//drop wifi
							if( bWifiConnected == true )
							{
							//	if (DBG)  logd("Try disconnecting to AP when receiving " + message.arg1);
								if (DBG)  logd("Try disconnecting wlan when receiving " + message.arg1);
								mWifiChannel.sendMessage(WifiP2pServiceImpl.DISCONNECT_WIFI_REQUEST, 1);
								mTemporarilyDisconnectedWifi = true;
								//Sleep for a while, make sure WIFI DISCONNECT happened before P2P_CONNECT
								try {
									Thread.sleep(50);
    ...
    class InactiveState extends State {
		...
		/* Add by Collin
		We try to read the box generated PIN to the config.
		So that WifiNative can read it to send out P2P_CONNECT with PIN code.
		*/
		 String strPin = Settings.System.getString(mContext.getContentResolver(), Settings.System.REALTEK_SETUP_MIRACAST_SINK_PIN_CODE);
		 config.wps.pin = strPin;
		 if (DBG) logd(getName() + "Put the MIRACAST_SINK_PIN_CODE here:" + config.wps.pin);
		 
                    mSavedPeerConfig = config;
                    mSavedPeerConfig.groupOwnerIntent = 6;
                    mAutonomousGroup = false;
    ...
    class UserAuthorizingNegotiationRequestState extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                case PEER_CONNECTION_USER_ACCEPT:                
                    //mWifiNative.p2pExtListen(false, 0, 0);
                    if (DBG) logd("UserAuthorizingNegotiationRequestState, stop listen");
                    //
                    mWifiNative.p2pStopFind();
    ...
    class UserAuthorizingInviteRequestState extends State {
        ...
        public boolean processMessage(Message message) {
            if (DBG) logd(getName() + message.toString());
            boolean ret = HANDLED;
            if (date0!=null) {//timer started
            	date1 = new Date();
            	long ldate1 = date1.getTime();
            	date1 = null;
            	if (ldate1-date0.getTime() > 10000) {
            		if (DBG) logd("UserAuthorizingInviteRequestState, timeout 1.");
            		mWifiNative.p2pStopFind();
                	transitionTo(mInactiveState);
                	date0 = null;
					return ret;
            ...
            switch (message.what) {
            	case WifiMonitor.P2P_DEVICE_FOUND_EVENT:
            		WifiP2pDevice device = (WifiP2pDevice) message.obj;
                    if (DBG) logd("P2P_DEVICE_FOUND_EVENT2, devaddr="+device.deviceAddress);
                    if (mThisDevice.deviceAddress.equals(device.deviceAddress)) break;
                    if (date0!=null) {
                    	if (mSavedPeerConfig.deviceAddress.equals(device.deviceAddress)) {
                    		if (DBG) logd("UserAuthorizingInviteRequestState, found peer.");
                    		mWifiNative.p2pStopFind();
                    ...
                    break;
                case WifiMonitor.P2P_DEVICE_LOST_EVENT:
                	if (DBG) logd("P2P_DEVICE_LOST_EVENT2");
					device = (WifiP2pDevice) message.obj;
                    	// Gets current details for the one removed
                    device = mPeers.remove(device.deviceAddress);
                    if (device != null) {
                ...
                case PEER_CONNECTION_USER_ACCEPT:
                    //mWifiNative.p2pExtListen(false, 0, 0);
                    if (DBG) logd("UserAuthorizingInviteRequestState, stop listen");
                    //find peer for 3 secs
                    //mPeers.clear();
                    mWifiNative.p2pStopFind();
                    date0 = new Date();
                    mWifiNative.p2pFind(DISCOVER_TIMEOUT_S);
                    if (DBG) logd("UserAuthorizingInviteRequestState, find again.");
                    //
                    /*
                    mWifiNative.p2pStopFind();
                    if (!reinvokePersistentGroup(mSavedPeerConfig)) {
    ...
    class GroupNegotiationState extends State {
        ...
        public boolean processMessage(Message message) {
			...
			if (DBG) logd(getName() + "i am GC, GO address: "+groupOwner.deviceAddress);
                        if (peer == null) {//bug47273, workaround.
                            peer = mPeers.get(mSavedPeerConfig.deviceAddress);
                            mGroup.setOwner(peer);
                            groupOwner = mGroup.getOwner();
                            logw("Unknown group owner, huess peer is: "+mSavedPeerConfig.deviceAddress);
                        }
                        if (peer != null) {
                ...
                case WifiMonitor.P2P_GROUP_REMOVED_EVENT:
                    ...
                    if (DBG) logd(getName() + " mTemporarilyDisconnectedWifi="+mTemporarilyDisconnectedWifi);
                    if (mTemporarilyDisconnectedWifi) {
                        mWifiChannel.sendMessage(WifiP2pServiceImpl.DISCONNECT_WIFI_REQUEST, 0);
                        mTemporarilyDisconnectedWifi = false;
                    }
                    ...
                    break;
                    /* Wait RECONNECT operation is done */                    
                    case CMD_NETWORK_CONNECTION_EVENT:
                    case CMD_ASSOCIATION_REJECTION_EVENT:
                    case CMD_AUTHENTICATION_FAILURE_EVENT:
                        if (DBG) logd(getName() + " Receive reconnect response ");                    
                    handleGroupCreationFailure();
                    transitionTo(mInactiveState);
                    break;
                // A group formation failure is always followed by
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case IPM_POST_DHCP_ACTION:
                    ...
                    // To fast p2p disconnect from 10 seconds to 1 second
	                mWifiNative.setP2pGroupIdle(mGroup.getInterface(), 1 );
                    break;
                case IPM_DHCP_RESULTS:
                ...
                case WifiMonitor.P2P_GROUP_REMOVED_EVENT:
                    ...
                    if (DBG) logd(getName() + " mTemporarilyDisconnectedWifi="+mTemporarilyDisconnectedWifi);
                    if (mTemporarilyDisconnectedWifi) {                        
                        // Need to wait AP connected first

                        //workaround, to avoid RVSD stopping procedure affect wifi authenticate
                       try {
                        Thread.sleep(250);
                       } catch (InterruptedException e) {
                        e.printStackTrace();
                        ...
                        if (DBG) logd(getName() + " Send out DISCONNECT_WIFI_REQUEST 0...");
                        mWifiChannel.sendMessage(WifiP2pServiceImpl.DISCONNECT_WIFI_REQUEST, 0);
                        mTemporarilyDisconnectedWifi = false;

                        if (DBG) logd(getName() + " stay in created State");
							
						File f_sta_disconnected = new File("/tmp/ap_sta_disconnected");
						if(f_sta_disconnected.exists()){
							if (DBG) logd(getName() + "send message to wifi rtsp thread");
							String text = "Receive AP_STA_DISCONNECTED_EVENT";
							try{
								FileWriter fw = new FileWriter(f_sta_disconnected.getAbsoluteFile());
                    ...
                    {
                        // Do not need to reconnect AP, jump to InactiveState directly
                        handleGroupRemoved();
                        transitionTo(mInactiveState);
                    }
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case PEER_CONNECTION_USER_ACCEPT:
                    ...
                    //mWifiNative.p2pExtListen(false, 0, 0);
                    //if (DBG) logd("UserAuthorizingJoinState, stop listen");
                    mWifiNative.p2pStopFind();
                    if (mSavedPeerConfig.wps.setup == WpsInfo.PBC) {
    ...
    private void handleGroupCreationFailure() {
        if (DBG) logd(getName() + " handleGroupCreationFailure()" );
        if (DBG) logd(getName() + " mTemporarilyDisconnectedWifi="+mTemporarilyDisconnectedWifi);
        if (mTemporarilyDisconnectedWifi) {
            mWifiChannel.sendMessage(WifiP2pServiceImpl.DISCONNECT_WIFI_REQUEST, 0);
            mTemporarilyDisconnectedWifi = false;
        ...
        if(mWfdRole == WFD_SOURCE_ROLE) {            
        sendMessage(WifiP2pManager.DISCOVER_PEERS);
    }
        else {
            if (DBG) logd("Skip trigger  DISCOVER_PEERS (P2P_FIND) because the DUT is Sink role ");
        }        
    }

    private void handleGroupRemoved() {
    	if (DBG) logd(getName() + " handleGroupRemoved");
        if (mGroup.isGroupOwner()) {
    ...
    private void judgeToShowInvitationDialog() {		
	int reminder = 0;
	try {			
		reminder = Settings.System.getInt(mContext.getContentResolver(),
					Settings.System.REALTEK_SETUP_MIRACAST_SINK_CONNECTION_REMINDER_ON);
	}catch (SettingNotFoundException snfe) {
		if (DBG) logd(getName() + "Cannot find reminder setting, use true instead" );
		reminder = 1;
	}

	//Use the Miracast Sink setting to decide to show the AlertDialog or not
	if(reminder == 1)
	{
	...
	{
		if (DBG) logd(getName() + "Connection reminder is off,  accept invitation directly " );
		sendMessage(PEER_CONNECTION_USER_ACCEPT);
	}
    }
