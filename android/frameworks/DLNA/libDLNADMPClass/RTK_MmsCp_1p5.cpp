...
//#include <Utility/upnp/MSCP_ControlPoint.h>
#include "ILibParsers.h"
#include "MediaServerCP_ControlPoint.h"
#include "RTK_MmsCp.h"
//#include <Utility/upnp3/MediaServerControlPoint.h>
#include "MyString.h"
#include <assert.h>
...
#define UNSUPPORTED_BY_CP ULOGD("Action is not supported by this implementation."); ASSERT(1);
...
struct MMSCP_MediaObject* MMSCP_CreateMediaObject(struct ILibXMLNode *node, struct ILibXMLAttribute *attribs, int isItem, const char *rangeStart, const char *rangeEnd)
{
	...
	if (node->StartTag == 0)
	{
		ULOGE("MMSCP_CreateMediaObject requires node->StartTag!=0.");
		ASSERT(0);
	}
	...
	{
		ULOGE("MMSCP_CreateMediaObject requires item or container node.");
		ASSERT(0);
	}
	#endif
...
////		node->Name[node->NameLength] = '\0';
				...
				while (att != NULL)
				{
                    ...
                    */
					...
					else if (stricmp(att->Name, MMSCP_ATTRIB_DURATION) == 0)
					{
						...
						{
							/* parse out number of seconds from hh:mm:ss string */
							att->Value[2] = '\0';
							att->Value[5] = '\0';
							c = att->Value[8];
							att->Value[8] = '\0';

							/* 1 hr = 3600 seconds, 1 min = 60 seconds*/
							res->Duration =
								(atoi(att->Value) * 3600) +
								(atoi(att->Value+3) * 60) +
								(atoi(att->Value+6)		);
						...
						{
							/* parse out number of seconds from mm:ss string */
							att->Value[2] = '\0';
							c = att->Value[5];
							att->Value[5] = '\0';
		...
		if (node == NULL)
		{
			ULOGE("MMSCP_CreateMediaObject: Unexpected null node.");
			ASSERT(0);
		}
		#endif
		/* free attribute mapping */
...
{
	ULOGD("MSCP Invoke Response: ConnectionManager/GetCurrentConnectionInfo(%d,%d,%s,%s,%d,%s,%s)",RcsID,AVTransportID,ProtocolInfo,PeerConnectionManager,PeerConnectionID,Direction,Status);
	UNSUPPORTED_BY_CP;
}

void MMSCP_ProcessResponse_GetProtocolInfo(struct UPnPService* Service,int ErrorCode,void *User,char* Source,char* Sink)
{
	ULOGD("MSCP Invoke Response: ConnectionManager/GetProtocolInfo(%s,%s)",Source,Sink);
	UNSUPPORTED_BY_CP;
}

void MMSCP_ProcessResponse_GetCurrentConnectionIDs(struct UPnPService* Service,int ErrorCode,void *User,char* ConnectionIDs)
{
	ULOGD("MSCP Invoke Response: ConnectionManager/GetCurrentConnectionIDs(%s)",ConnectionIDs);
	UNSUPPORTED_BY_CP;
}
...
{
	ULOGD("MSCP Invoke Response: ContentDirectory/GetSystemUpdateID(%u)",Id);
	UNSUPPORTED_BY_CP;
}
...
{
	ULOGD("MSCP Invoke Response: ContentDirectory/GetTransferProgress(%s,%s,%s)",TransferStatus,TransferLength,TransferTotal);
	UNSUPPORTED_BY_CP;
}
...
{
	ULOGD("MSCP Invoke Response: ContentDirectory/GetSortCapabilities(%s)",SortCaps);
	UNSUPPORTED_BY_CP;
}
...
{
	ULOGD("MSCP Invoke Response: ContentDirectory/GetSearchCapabilities(%s)",SearchCaps);
	UNSUPPORTED_BY_CP;
}
...
{
	ULOGD("MSCP Invoke Response: ContentDirectory/CreateObject(%s,%s)",ObjectID,Result);
	UNSUPPORTED_BY_CP;
}
...
{
	ULOGD("MSCP Invoke Response: ContentDirectory/Search(%s,%u,%u,%u)",Result,NumberReturned,TotalMatches,UpdateID);
	UNSUPPORTED_BY_CP;
}
...
void MSCPResponseSink_ContentDirectory_Browse(struct UPnPService* Service,int ErrorCode,void *User,char* Result,unsigned int NumberReturned,unsigned int TotalMatches,unsigned int UpdateID)
{
	...
	struct BrowseInfo* pBrowseInfo = (BrowseInfo*)User;
	struct MMSCP_BrowseArgs *pBrowseArgs = NULL;

	if (pBrowseInfo)
	{
		pBrowseArgs = pBrowseInfo->args;
	}
	...
	TEMPDEBUGONLY(ULOGD("MSCP Invoke Response: ContentDirectory/Browse(%s,%u,%u,%u)",Result,NumberReturned,TotalMatches,UpdateID);)
	...
	if ((ErrorCode == 0) && (Result != NULL))
	{
		...
		{
			MMSCP_Callback_Browse(Service, pBrowseArgs, (int)MMSC_Error_XmlNotWellFormed, NULL);
		}
		else if (resultLen == 0)
		{
			MMSCP_Callback_Browse(Service, pBrowseArgs, ErrorCode, NULL);
		}
		else
		{
			node = nodeList;
		...
		{
			ULOGD("MSCPResponseSink_ContentDirectory_Browse: Detected mismatch with number of objects returned=%u and parsed=%d.", resultsList->NumberReturned, resultsList->NumberParsed);
		}
		...
		MMSCP_Callback_Browse(Service, pBrowseArgs, ErrorCode, resultsList);
	}
	else
	{
		MMSCP_Callback_Browse(Service, pBrowseArgs, ErrorCode, NULL);
...
{
	ULOGD("MSCP Event from %s/ContentDirectory/ContainerUpdateIDs: %s",Service->Parent->FriendlyName,ContainerUpdateIDs);
	UNSUPPORTED_BY_CP;
}

void MMSCPEventSink_ContentDirectory_SystemUpdateID(struct UPnPService* Service,unsigned int SystemUpdateID)
{
	ULOGD("MSCP Event from %s/ContentDirectory/SystemUpdateID: %u",Service->Parent->FriendlyName,SystemUpdateID);
	UNSUPPORTED_BY_CP;
}
#endif // 0
...
{
	ULOGD("MSCP Device Added: %s", device->FriendlyName);
...
{
	ULOGD("MSCP Device Removed: %s", device->FriendlyName);
...
void MMSCP_Invoke_Browse_Stop(void *serviceObj)
{
	MSCP_Invoke_ContentDirectory_Browse_Stop((struct UPnPService *)serviceObj);
}
