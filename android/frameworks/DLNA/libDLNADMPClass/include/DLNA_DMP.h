...
public:
	...
	bool queryItemObjInfoByIndex(int index, struct UPnPObjInfo *pInfo);
		...
		// Just prepare first
	////TODO, modified later
        unsigned int queryFileMediaType(char *filename, char *ID, char **ProtocolInfo);
	...
	bool UPnPServiceBrowse(char *id, int startingIndex, int requestedCount, int *returnedCount, int *totalCount, bool bMetaData = false);
...
private:
		static int m_state;	// 0:stop, 1:run
		static int m_bIsBrowsingState;
		static int m_bIsStopBrowsing;

		// internal variables
		static unsigned long m_curBrowserTicketID;
		static int m_nBrowserErrorCode;
