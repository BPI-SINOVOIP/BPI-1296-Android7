...
static int ne_uri_urlencode(const unsigned char ori, char * des) 
{
        // urlencode an unsafe character
    snprintf(des, 4, "%%%.2x", (unsigned char)ori);
    return strlen(des);
}
...
int ne_uri_parse(const char *uri, ne_uri *parsed)
{
  ...
  path:
    ...
    while (uri_lookup(*p) & URI_SEGCHAR)
        p++;
    if (*p != '?' && *p != '#' && *p != '\0') {
        // not ? (query) or # (fragment), well it's a bad char in URL, but
        // it happens
        p++;
        goto path;
    }
    ...
    if (*p != '\0') {
        char * reform = ne_calloc(strlen(s) * 3 + 1);
        int reformindex = 0;
        s = p++;

        while (1) {
            if (uri_lookup(*p) & URI_QUERY) {
                reform[reformindex] = *p;
                p++; reformindex ++;
            }
            else {
                if (*p =='\0' || *p == '#' || *p < 0)
                    break;
                    /*
                     * okay, we have an unsafe character here. this is
                     * strictly against HTTP protocol, but many websites
                     * put unsafe chars without urlencode. so we have
                     * to deal with it.
                     */
                int l = ne_uri_urlencode(*p, reform+reformindex);
                reformindex += l;
                p++;
            }
        }        
