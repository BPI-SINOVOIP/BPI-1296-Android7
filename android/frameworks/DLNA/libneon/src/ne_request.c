...
void ne_add_request_headers(ne_request *req, const char *valString)
{
  ne_buffer_concat(req->headers, valString, EOL, NULL);
}
	...
	if (resp->body.chunk.remain == 0) {
	   ...
	   if (resp->body.chunk.total == 0) {
		...
		break;
            ...
            chunk_len = strtoull(req->respbuf, &ptr, 16);
	    ...
	    resp->body.chunk.remain = resp->body.chunk.total = chunk_len;
	}
...
static int read_message_header(ne_request *req, char *buf, size_t buflen)
{
    ...
    if(sock==NULL || buf ==NULL || buflen ==0)
...
//	   && ++count < MAX_HEADER_FIELDS) {//remark by yuyu, for DLNA-1.5 verification
	char *pnt;
	unsigned int hash = 0;
...
#if 0	//remark by yuyu, for DLNA-1.5 verification
...
ne_off_t ne_request_remain(ne_request *req) {
    struct ne_response *const resp = &req->resp;
    ...
    case R_CHUNKED:
         ...
         * number of bytes left to read in the current chunk. */
	if (resp->body.chunk.remain == 0) {
	    return resp->body.chunk.total;
	}
	return resp->body.chunk.remain;
    case R_CLENGTH:
	return resp->body.clen.remain;
    case R_TILLEOF:
      return -1;
...
static int do_connect(ne_session *sess, struct host_info *host, const char *err)
{
    ...
    if (ne_get_session_flag(sess, NE_SESSFLAG_NOBUFFER)) {
	ne_sock_set_nonbuffer(sess->socket);
    }
