...
static int ILibLocalIgnoreIPAddress = 0;
	...
	*pp_int = (int*)malloc(sizeof(int)*num);
	...
	for (i = 0;(i < ifConf.ifc_len);)
	{
		...
		if (pifReq -> ifr_addr.sa_family == AF_INET)
		{
			...
			if (LocalAddr.sin_addr.s_addr != htonl (INADDR_LOOPBACK) && LocalAddr.sin_addr.s_addr != ILibLocalIgnoreIPAddress)
			{
...
#define CONVERT_TO_TICKTIME(tv) ((tv.tv_sec*1000ULL) + (tv.tv_usec/1000ULL))

struct LifeTimeMonitorData
{
	TickTime ExpirationTick;
	int ms;
	void *data;
...
void ILibStartChain(void *Chain)
{
	...
	while(((struct ILibBaseChain*)Chain)->TerminateFlag==0)
	{
		...
		if(slct==-1)
		{
			...
			if (errno != EBADF)
			{
				ULOGE("UPnP Select sleep errno = %d, This means %s", errno, strerror(errno));
				usleep(100000);
			}
		}
...
/*! \fn ILibGetLongLong(char *TestValue, int TestValueLength, long* NumericValue)
       \brief Reads a long value from a string, in a validating fashion
       \param TestValue The string to read from
       \param TestValueLength The length of the string
       \param NumericValue The long value extracted from the string
       \returns 0 if succesful, nonzero if there was an error
*/
int ILibGetLongLong(char *TestValue, int TestValueLength, long long* NumericValue)
{
       char* StopString;
       char* TempBuffer = (char*)malloc(1+sizeof(char)*TestValueLength);
       memcpy(TempBuffer,TestValue,TestValueLength);
       TempBuffer[TestValueLength] = '\0';
       *NumericValue = strtoll(TempBuffer,&StopString,10);
       if(*StopString!='\0')
       {
	       //
	       // If strtol stopped somewhere other than the end, there was an error
	...
	while(HeaderLine!=NULL)
	{
		...
		else
		{
			...
			if(node->Field==NULL)
			{
				...
				//
				free(node);
				// re-initialize the pointer to make sure not to access to invaild memory
				// 20090520, yuyu
				node = NULL;
				// 20090520, yuyu
...
void ILibLifeTime_AddEx(void *LifetimeMonitorObject,void *data, int ms, ILibLifeTime_OnCallback Callback, ILibLifeTime_OnCallback Destroy)
{
	...
	//
	ltms->data = data;
	ltms->ExpirationTick = CONVERT_TO_TICKTIME(tv) + ms;
	ltms->ms = ms;
...
void ILibLifeTime_Check(void *LifeTimeMonitorObject,void *readset, void *writeset, void *errorset, int* blocktime)
{
	...
	//
	gettimeofday(&tv,NULL);
	CurrentTick = CONVERT_TO_TICKTIME(tv);
	...
	while(node!=NULL)
	{
		...
		if(Temp->ExpirationTick<CurrentTick ||
		   ((Temp->ExpirationTick - CurrentTick) > Temp->ms))
		{
			//ULOGV("Do ExpirationTick(%llu),CurrentTick(%llu), ms(%d), diff(%lld)", Temp->ExpirationTick, CurrentTick, Temp->ms, CurrentTick - Temp->ExpirationTick);
				...
				if(pr->FirstResult->NextResult->datalength==2 && pr->LastResult->datalength==2)
				{
					...
					if(atoi(startTime)<=12 && atoi(startTime)>0)
					{
						...
						if(atoi(startTime)<=31 && atoi(startTime)>0)
						{
							...
							errCode = 0;
							RetVal = (char*)malloc((int)strlen(timeString)+10);
							sprintf(RetVal,"%sT00:00:00",timeString);
						}
					}
					free(startTime);
				}
...
int ILibTime_ParseEx(char *timeString, time_t *val)
{
	...
	if(pr->NumResults==2)
	{
		...
		if(pr2->NumResults==3)
		{
			...
			//ILibDestructParserResults(pr2); // move to appropriate location, 20090518, yuyu
		}
		ILibDestructParserResults(pr2);//make up memory release, 20090518, yuyu
	}
...
void ILibSetLocalIgnoreIPAddress(int intIP)
{
	ILibLocalIgnoreIPAddress = intIP;
}
