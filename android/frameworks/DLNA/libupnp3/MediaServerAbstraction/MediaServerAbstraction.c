...
void MSA_Helper_UpdateImmediate_ContainerUpdateID(MSA msa_obj)
{
	...
	/* clear the ContainerUpdateID list as everything as been evented */
	state->ContainerUpdateID = NULL;

	ASSERT(sv <= var+size);
	#ifdef _DEBUG
	printf("UPnP ContainerUpdateID Event Fired: %s\r\n", var);
	#endif
	
	MediaServer_SetState_ContentDirectory_ContainerUpdateIDs(msa_obj->DeviceMicroStack, var);
	free(var);	
}
...
void MSA_Helper_UpdateModerated_ContainerUpdateID(MSA msa_obj)
{
	...
	if (state->ModerationFlag == 0)
	{
		...
		ILibLifeTime_Add(msa_obj->LifeTimeMonitor,(void*)msa_obj,1,(void*)MSA_Helper_ModerationSink_ContainerUpdateID, NULL);
	}
}
...
void MediaServer_ContentDirectory_Browse(void* upnptoken,char* ObjectID,char* BrowseFlag,char* Filter,unsigned int StartingIndex,unsigned int RequestedCount,char* SortCriteria)
{
	...
	#ifdef _DEBUG
	printf("UPnP Invoke: MediaServer_ContentDirectory_Browse() - ID %s;\r\n", ObjectID);
...
void MediaServer_ConnectionManager_GetCurrentConnectionInfo(void* upnptoken,int ConnectionID)
{
	...
	_AddMethodParameter(method, (METHOD_PARAM)(unsigned long) ConnectionID);
	if(_ExecuteCallbackThroughThreadPool(msa, method) != MSA_ERROR_NONE)
	{
		MediaServer_Response_Error(upnptoken, CDS_EC_INTERNAL_ERROR, CDS_EM_INTERNAL_ERROR);
	}
...
void MediaServer_ContentDirectory_CreateObject(void* upnptoken, char* id, char* Elements)
{
	...
	#ifdef _DEBUG
	printf("UPnP Invoke: MediaServer_ContentDirectory_CreateObject() - ID %s;\r\n", id);
	#endif
		...
		else
		{
			...
			while (node != NULL)
			{
				if (node->StartTag != 0)
				{
					...
					{
						newObj = CDS_DeserializeDidlToObjectEx(node, attribs, 0, Elements, lastResultPos, 1);
					...
					{
						newObj = CDS_DeserializeDidlToObjectEx(node, attribs, 1, Elements, lastResultPos, 1);
...
void MediaServer_ContentDirectory_DestroyObject(void* upnptoken, char* ObjectID)
{
	...
	#ifdef _DEBUG
	printf("UPnP Invoke: MediaServer_ContentDirectory_DestroyObject() - ID %s;\r\n", ObjectID);
	#endif
...
int MSA_ForCreateObjectResponse_Accept(MSA msa_obj, void* upnp_session, struct MSA_CdsCreateObj* create_obj_arg, struct CdsObject* cds_obj)
{
	...
	if(strlen(didl) == 0)
	{
		...
		*/
		MediaServer_Response_Error(upnp_session, 712, "Error - Deserializing response to DIDL-Lite format.");
	}
	else
	{
...
int MSA_ForQueryResponse_Start(MSA msa_obj, void* upnp_session, struct MSA_CdsQuery* cds_query, int send_didl_header_flag)
{
	...
	{
		unsigned int filter = CDS_ConvertCsvStringToBitString(cds_query->Filter);
		if(CDS_FilterContainsDlnaElements(filter))
		{
		...
		{
			status = MediaServer_AsyncResponse_OUT(upnp_session, CDS_STRING_RESULT, CDS_DIDL_HEADER_ESCAPED_NO_DLNA_NAMESPACES, CDS_DIDL_HEADER_ESCAPED_NO_DLNA_NAMESPACES_LEN, ILibAsyncSocket_MemoryOwnership_STATIC, 1, 0);
		}
	}
	else
	{
...
void MSA_IncrementSystemUpdateID(MSA msa_obj)
{
	...
	#ifdef _DEBUG
	printf("UPnP SystemUpdate Event Fired: %d\r\n", state->SystemUpdateID);
	#endif
...
void MSA_UpdateContainerID(MSA msa_obj, const char *container_id, unsigned int container_update_id)
{
	...
	if(msa_obj == NULL || container_id == NULL) return;
	...
	if (fcu == NULL)
	{
		...
		fcu->ContainerID = ILibString_Copy(container_id, -1);
		fcu->UpdateID = 0;
		fcu->Next = NULL;
	}
	...
	/* Call MSA_Helper_UpdateModerated_ContainerUpdateID(msa_obj) if you need to switch to aggregate events at once */
	MSA_Helper_UpdateImmediate_ContainerUpdateID(msa_obj);
		...
		case MSA_CMS_GETCURRENTCONNECTIONINFO:
            {
                ...
                {
					msa->OnGetCurrentConnectionInfo(msa, contextSwitch->UPnPSession, (int)(unsigned long)contextSwitch->Parameters[0]);
                }
