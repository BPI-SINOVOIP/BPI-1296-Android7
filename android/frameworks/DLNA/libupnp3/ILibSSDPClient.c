...
#define MAX_SSDP_LISTEN_SOCKET_COUNT	5
...
struct SSDPClientModule
{
	...
	void* SSDPListenSocket[MAX_SSDP_LISTEN_SOCKET_COUNT];
...
{
    struct SSDPClientModule* pModule = (struct SSDPClientModule*)user;
    if (pModule)
    {
        int i = 0;
        for(i=0; i<pModule->NumIPAddress; i++)
        {
            if (remoteInterface == pModule->IPAddress[i]) {
                ULOGV("remoteInterface(0x%x) is ignored.", remoteInterface);
                return;
            }
        }
    }
...
void ILibSSDP_IPAddressListChanged(void *SSDPToken)
{
	...
	{
		ILibAsyncUDPSocket_JoinMulticastGroup(RetVal->SSDPListenSocket[i], RetVal->IPAddress[i], inet_addr(UPNP_GROUP));
	}
...
void* ILibCreateSSDPClientModule(void *chain, char* DeviceURN, int DeviceURNLength, void (*CallbackPtr)(void *sender, char* UDN, int Alive, char* LocationURL, int Timeout, UPnPSSDP_MESSAGE m,void *user),void *user)
{
	...
	if (RetVal->NumIPAddress > MAX_SSDP_LISTEN_SOCKET_COUNT)
	{
		ALOGE("NumIPAddress(%d) is over flow, reduce it.", RetVal->NumIPAddress);
		RetVal->NumIPAddress = MAX_SSDP_LISTEN_SOCKET_COUNT;
	}
	for(i=0;i<RetVal->NumIPAddress;++i)
	{
		RetVal->SSDPListenSocket[i] = ILibAsyncUDPSocket_Create(chain, 4096, 0, 1900, ILibAsyncUDPSocket_Reuse_SHARED, ILibSSDPClient_OnData , NULL, RetVal);
	}
