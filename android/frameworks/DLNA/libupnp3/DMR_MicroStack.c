...
#ifdef DLNADMRCTT
#define DMR_IGNORE_INVALID_ARGUMENT
#endif
...
void DMR_EnableP2P(int bEnable)
{
#ifdef _ANDROID
    static int nStopCount = 0;
    //ULOGD("DMR_EnableP2P(%d) --- StopCount(%d)", bEnable, nStopCount);
    ...
    if (bEnable)
    {
        ...
        if (nStopCount <= 0)
        {
            property_set("rtk.miracastsink.STATUS", "WFD_CH_AUTO_SWITCH_OFF");
            //ULOGD("DMR_EnableP2P --- WFD_CH_AUTO_SWITCH_OFF");
            nStopCount = 0;
        }
    }
    else
    {
        if (nStopCount == 0)
        {
            property_set("rtk.miracastsink.STATUS", "WFD_CH_AUTO_SWITCH_ON");
            //ULOGD("DMR_EnableP2P --- WFD_CH_AUTO_SWITCH_ON");
        }
        ++nStopCount;
    }
#endif
...
void DMR_PostMX_Destroy(void *object)
{
   ...
   //ULOGD("DMR_EnableP2P(T) is called in DMR_PostMX_Destroy (%p)", mss);
   DMR_EnableP2P(1);
}
...
//^^^ modified by yuyu, fixed socket not close leakage
   ...
   //ULOGD("DMR_EnableP2P() is called in DMR_OnPostMX_MSEARCH_SendOK (%p)", mss);
//   DMR_EnableP2P(1);
}
...
//^^^ modified by yuyu, fixed socket not close leakage
   ...
   //ULOGD("DMR_EnableP2P(T) is called in DMR_PostMX_MSEARCH (%p)", mss);
   DMR_EnableP2P(1);
}
void DMR_ProcessMSEARCH(struct DMR_DataObject *upnp, struct packetheader *packet)
{
   ...
   if(memcmp(packet->DirectiveObj,"*",1)==0)
   {
      if(memcmp(packet->Version,"1.1",3)==0)
      {
         ...
         if(MANOK!=0 && MXOK!=0)
         {
            ...
            //
            //ULOGD("DMR_EnableP2P(F) is called in DMR_ProcessMSEARCH (%p)", mss);
            DMR_EnableP2P(0);
					...
					while(xnode!=NULL)
					{
						if(xnode->StartTag!=0 && xnode->NameLength==26 && memcmp(xnode->Name,"GetCurrentTransportActions",26)==0)
						{
							...
							while(xnode!=NULL)
							{
								...
								#ifndef DMR_IGNORE_INVALID_ARGUMENT
								else if(xnode->NameLength != 0 && xnode->Name!= NULL)
								{
									ULOGE("invalid Argument %s",__FUNCTION__);
									OK  |= 0x8000;
								}
								#endif
								if(xnode->Peer==NULL)
...
/* Type Checking */
   OK = ILibGetULong(p_InstanceID,p_InstanceIDLength, &TempULong);
	...
	if(DMR_FP_AVTransport_GetCurrentTransportActions == NULL)
		DMR_Response_Error(ReaderObject,401,"Invalid Action");
	else
...
/* Type Checking */
   OK = ILibGetULong(p_InstanceID,p_InstanceIDLength, &TempULong);
	...
	{
	OK = 0;
        if (DMR__Device_MediaRenderer_Impl.RenderingControl->StateVar_Brightness)
        {
...
/* Type Checking */
   OK = ILibGetULong(p_InstanceID,p_InstanceIDLength, &TempULong);
	...
	{
	OK = 0;
        if (DMR__Device_MediaRenderer_Impl.RenderingControl->StateVar_Contrast)
        {
...
int DMR_ProcessPOST(struct ILibWebServer_Session *session, struct packetheader* header, char *bodyBuffer, int offset, int bodyBufferLength)
{
   ...
   if(header->DirectiveObjLength==20 && memcmp((header->DirectiveObj)+1,"AVTransport/control",19)==0)
	...
	else if(header->DirectiveObjLength==25 && memcmp((header->DirectiveObj)+1,"RenderingControl/control",24)==0)
	{
		 ...
		 if(SOAPACTIONLength==13 && memcmp(SOAPACTION,"GetBrightness",13)==0)
		{
   ...
   if(header->DirectiveObjLength==18 && memcmp(header->DirectiveObj + 1,"AVTransport/event",17)==0)
	{
		...
		if(Info!=NULL)
		{
			...
			if(NotificationType || URL)
      		{
         		ULOGE("NotificationType or URL != NULL");
...
void DMR_TryToSubscribe(char* ServiceName, long Timeout, char* URL, int URLLength,struct ILibWebServer_Session *session)
{
   ...
   if(*HeadPtr!=NULL)
   {
      ...
      while(NewSubscriber!=NULL)
      {
         if(DMR_SubscriptionExpired(NewSubscriber)!=0 ||
         	ILibString_StartsWith(NewSubscriber->URL, NewSubscriber->URLLength, URL, URLLength)!=0)
         {
            ULOGD("The Subscriber is removed: URL(%s)", NewSubscriber->URL);
   ...
   if(*TotalSubscribers<10)
   {
      ...
      for (indexURL=0; indexURL<URLLength; ++indexURL)
      {
         if (URL[indexURL] == '<')
         {
            pURI = &URL[indexURL];
         ...
         // It is begin to parse the URI

         p = ILibParseString(pURI,0,URILength,"://",3);
         ...
         int address_value = inet_addr(TempString2);
         if (address_value == -1)
         {
            ULOGE("The Subscribe IP(%s) is ignored.", TempString2);
            ILibDestructParserResults(p);
            ILibDestructParserResults(p2);
            free(TempString2);
            continue;
         }
         NewSubscriber->Address = address_value;

         struct parser_result *ref_result = NULL;
         if(p2->NumResults==1)
         {
         ...
         if(TempStringLength==ref_result->FirstResult->datalength)
         {
            path = (char*)malloc(2);
            memcpy(path,"/",1);
            path[1] = '\0';
         ...
         {
            path = (char*)malloc(1+TempStringLength - ref_result->FirstResult->datalength -1);
            memcpy(path,TempString + ref_result->FirstResult->datalength,TempStringLength - ref_result->FirstResult->datalength -1);
            path[TempStringLength - ref_result->FirstResult->datalength -1] = '\0';
         }
      ...
      NewSubscriber->SEQ = 0;
      NewSubscriber->URL = ILibString_Copy(URL, URLLength);
      NewSubscriber->URLLength = URLLength;
...
void DMR_RenewEvents(char* path,int pathlength,char *_SID,int SIDLength, char* Timeout, int TimeoutLength, struct ILibWebServer_Session *ReaderObject)
{
   ...
   LVL3DEBUG(ULOGV("SUBSCRIBER [%s] attempting to Renew Events for %s Duration [",SID,Timeout);)
   ...
   if(info!=NULL)
   {
	  ...
	  ULOGV("tvsec=%d RenewbyTime=%d",tv.tv_sec, (info->RenewByTime).tv_sec);
	  if(tv.tv_sec > (info->RenewByTime).tv_sec )
	  {
		ULOGE("DMR_RenewEvents FAILED");
...
void DMR_ProcessHTTPPacket(struct ILibWebServer_Session *session, struct packetheader* header, char *bodyBuffer, int offset, int bodyBufferLength)
{
   ...
   else if(header->DirectiveLength==3 && memcmp(header->Directive,"GET",3)==0)
   {
      ...
      while(f!=NULL)
      {
        ...
        if(f->FieldLength==10 && strncasecmp(f->Field,"User-Agent",10)==0)
        {
        	if (strstr(f->FieldData, "CyberGarage-HTTP/1.0")) // Fixbug39172
        	{
				if (header && header->VersionLength==3 && memcmp(header->Version,"1.1",3)==0) {
					header->Version[2] = '0';
				}
        	}
		}
      ...
      else
      {
        ...
        if((RequiredLang != NULL) && (RequiredLangLen != 0))
        {
			...
			{
			#ifndef DLNADMRCTT
				sprintf(packet,"%s\r\nCONTENT-LENGTH: %d\r\nDATE: %s",responseHeader2,13096+489,date);
			#else
				sprintf(packet,"%s\r\nCONTENT-LENGTH: %d\r\nDATE: %s",responseHeader2,13096,date);
			#endif
			}
			else if(header->DirectiveObjLength==27 && memcmp((header->DirectiveObj)+1,"ConnectionManager/scpd.xml",26)==0)
        ...
        else
        {
			...
			{
			#ifndef DLNADMRCTT
				sprintf(packet,"%s\r\nCONTENT-LENGTH: %d\r\nDATE: %s",responseHeader,13096+489,date);
			#else
				sprintf(packet,"%s\r\nCONTENT-LENGTH: %d\r\nDATE: %s",responseHeader,13096,date);
			#endif
			}
			else if(header->DirectiveObjLength==27 && memcmp((header->DirectiveObj)+1,"ConnectionManager/scpd.xml",26)==0)
...
void DMR_Response_ConnectionManager_GetProtocolInfo(const DMR_SessionToken DMR_Token, const char* unescaped_Source, const char* unescaped_Sink)
{
  char* body;
	...
	ULOGV("DMR_MicroStack.c Invoke: DMR_Response_ConnectionManager_GetProtocolInfo();");
	ULOGV("DMR_MicroStack.c unescaped_Sink:%s", unescaped_Sink);
...
int *PAUSE)
{
   ...
   if (bIsDestroied)
   {
      ULOGD("DMR_SendEventSink is called after destroied for %p", subscriber);
   }
   else
#endif
   ...
   else if( ((struct SubscriberInfo*)subscriber)->Disposing == 1 )
   {
	   sem_wait(&(((struct DMR_DataObject*)upnp)->EventLock));
	   --((struct SubscriberInfo*)subscriber)->RefCount;
	   if(((struct SubscriberInfo*)subscriber)->RefCount==0)
...

{
   ...
   DMR__Device_MediaRenderer_Impl.FriendlyName = FriendlyName;
	...
	if(DMR__Device_MediaRenderer_Impl.Manufacturer == NULL) {DMR__Device_MediaRenderer_Impl.Manufacturer = "Intel Corporation";}
   if(DMR__Device_MediaRenderer_Impl.ManufacturerURL == NULL) {DMR__Device_MediaRenderer_Impl.ManufacturerURL = "http://www.intel.com";}
   if(DMR__Device_MediaRenderer_Impl.ModelDescription == NULL) {DMR__Device_MediaRenderer_Impl.ModelDescription = "Intel DLNA DMR Using Auto-Generated UPnP Stack";}
   if(DMR__Device_MediaRenderer_Impl.ModelName == NULL) {DMR__Device_MediaRenderer_Impl.ModelName = "Intel DLNA DMR";}
   if(DMR__Device_MediaRenderer_Impl.ModelNumber == NULL) {DMR__Device_MediaRenderer_Impl.ModelNumber = "Intel-X1";}
   if(DMR__Device_MediaRenderer_Impl.ModelURL == NULL) {DMR__Device_MediaRenderer_Impl.ModelURL = "http://www.intel.com";}
   if(DMR__Device_MediaRenderer_Impl.ProductCode == NULL) {DMR__Device_MediaRenderer_Impl.ProductCode = "DMR-Intel-X1";}
