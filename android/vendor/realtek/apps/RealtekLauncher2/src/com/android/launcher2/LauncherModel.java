...
public class LauncherModel extends BroadcastReceiver {
    ...
    public interface Callbacks {
        ...
        // RTKCOMMENT: callback when app is removed.
        ...
        // RTKCOMMENT: declare callback function for realtek launcher bindCategoryPageUpdated
        public void bindCategoryPageUpdated();
        // RTKCOMMENT: callback for Launcher widget launching app customized page
        public void switchToAppCustomizedPage(ContentType type);
        // RTKCOMMENT: callback for onBindWorkspaceItemComplete
    ...
    static void addItemToDatabase(Context context, final ItemInfo item, final long container,
    	...
    	DebugHelper.dump("[LauncherModel] addItemToDatabase item:"+item.getClass().getName());
    ...
    /**
     * RTKCOMMENT: Important, the entry point of deleting item on workspace
    ...
    static void deleteItemFromDatabase(Context context, final ItemInfo item) {
    	...
    	DebugHelper.dump("-------------> LauncherModel deleteItemFromDatabase");
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        //DebugHelper.dump("LauncherModel onReceive!!");
        
        // RTKCOMMENT: handle LAUNCH_APP_CUSTOMIZED_PAGE_INTENT passed from widget
        if(ConstantsDef.LAUNCH_APP_CUSTOMIZED_PAGE_INTENT.equals(action)){
        	if (mCallbacks != null) {
        		Callbacks callbacks = mCallbacks.get();
        		if (callbacks != null) {
        			callbacks.switchToAppCustomizedPage(ContentType.Applications);
        		}
        	}
        	return;
        }
        
         // RTKCOMMENT: handle LAUNCH_FAVAPP_CUSTOMIZED_PAGE_INTENT passed from widget
        if(ConstantsDef.LAUNCH_FAVAPP_CUSTOMIZED_PAGE_INTENT.equals(action)){
        	if(mCallbacks!=null){
        		Callbacks callbacks = mCallbacks.get();
        		if(callbacks!=null){
        			callbacks.switchToAppCustomizedPage(ContentType.Favorite);
        		}
        	}
        	return;
        }
        
        if(ConstantsDef.LAUNCH_VIDEOAPP_CUSTOMIZED_PAGE_INTENT.equals(action)){
        	if(mCallbacks!=null){
        		Callbacks callbacks = mCallbacks.get();
        		if(callbacks!=null){
        			callbacks.switchToAppCustomizedPage(ContentType.Video);
        		}
        	}
        	return;
        }
        
        if(ConstantsDef.CATEGORIED_APP_ADDED_INTENT.equals(action) || 
        	ConstantsDef.CATEGORIED_APP_REMOVED_INTENT.equals(action)){
        	DebugHelper.dump("LauncherModel received Realetk type intent: "+action);
        	
        	int op = PackageUpdatedTask.OP_CATEGORIED_PAGE_UPDATED;
        	// RTKCOMMENT: feed in an empty array when category page is updated
    ...
    private class LoaderTask implements Runnable {
                    ...
                    while (!mStopped && c.moveToNext()) {
                        try {
                            ...
                            case LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET:
                                ...
                                DebugHelper.dump("[Launcher.Model] isSafeMode:"+isSafeMode);
                                DebugHelper.dump("[Launcher.Model] provider:"+provider);
                                ...
                                if (!isSafeMode && (provider == null || provider.provider == null ||
                                    ...
                                    appWidgetInfo = new LauncherAppWidgetInfo(appWidgetId,
                                            provider.provider,
                                            false);		// load workspace, do nothing
                                    appWidgetInfo.id = id;
        ...
        private void bindWorkspaceItems(final Callbacks oldCallbacks,
            ...
            // Finally, insert a runnable at the end of the queue as complete runnable
            final Runnable bindCompleteRunnable = new Runnable() {
                
                @Override
                public void run() {
                    Callbacks callbacks = tryGetCallbacks(oldCallbacks);
                    if (callbacks != null) {
                        callbacks.setFocusOnBindWorkspaceComplete();
                    }
                }
            };
            ...
            if (postOnMainThread) {
                deferredBindRunnables.add(bindCompleteRunnable);
            } else {
                runOnMainThread(bindCompleteRunnable, MAIN_THREAD_BINDING_RUNNABLE);
            }
        }
        ...
        private void bindWorkspace(int synchronizeBindPage) {
        	...
        	DebugHelper.dump("[RTKDEBUG]           bindWorkspace");
    ...
    private class PackageUpdatedTask implements Runnable {
        ...
        // RTKCOMMENT: add OP code for Realtek launcher
        public static final int OP_CATEGORIED_PAGE_UPDATED=5;
        ...
        public void run() {
            ...
            switch (mOp) {
                ...
                case OP_UNAVAILABLE:
                    ...
                    break;
                // RTKCOMMENT: OP code used in realtek launcher
                case OP_CATEGORIED_PAGE_UPDATED:
                	DebugHelper.dump("OP_CATEGORIED_PAGE_UPDATED");
                	break;
            }
            ...
            if(mOp==OP_CATEGORIED_PAGE_UPDATED){
            	mHandler.post(new Runnable() {
                    public void run() {
                        Callbacks cb = mCallbacks != null ? mCallbacks.get() : null;
                        if (callbacks == cb && cb != null) {
                            callbacks.bindCategoryPageUpdated();
                        }
                    }
                });
            }
