...
public class CellLayout extends ViewGroup {
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        // RTKCOMMENT: let shortcut and widget container aware key event.
        mShortcutsAndWidgets.setOnKeyListener(new ShortcutAndWidgetContainerKeyEventListener());
    ...
    void setPressedOrFocusedIcon(BubbleTextView icon) {
    	...
    	//DebugHelper.dump("CellLayout setPressedOrFocusedIcon icon:"+icon);
    ...
    protected void onDraw(Canvas canvas) {
    	...
    	DebugHelper.dump("CellLayout onDraw.");
    	
    	/** Remove BG holo blue rectangle
    ...
    public void removeView(View view) {
    	DebugHelper.dump("CellLayout, remove View:"+view);
    ...
    /**
     * Add for fake drag command
     * let view append {@link CellInfo} as Tag
     * @param v child view
     */
    public void setTagChildSelfCellInfo(View child, int currentScreen){
    ...
    public void setTagToCellInfoForPoint(int touchX, int touchY) {
    	DebugHelper.dump("----------- setTagToCellInfoForPoint x="+touchX+" y="+touchY);
        final CellInfo cellInfo = mCellInfo;
        Rect frame = mRect;
        ...
        for (int i = count - 1; i >= 0; i--) {
            ...
            DebugHelper.dump("----------- setTagToCellInfoForPoint child="+child);
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        DebugHelper.dump("CellLayout onInterceptTouchEvent motion event:"+ev);
        ...
        if (mInterceptTouchListener != null && mInterceptTouchListener.onTouch(this, ev)) {
        	DebugHelper.dump("steal touch event");
            return true;
        }
    ...
    // RTKBOOKMARK: might be where re-arrangement happened.
    ...
    // RTKBOOKMARK: might be where swap happened.
    ...
    // RTKCOMMENT: the animator to shake floating child view in workspace.
    ...
    public void checkOccupiedArrayValidation(){
        boolean[][] checkOccupied = new boolean[mCountX][mCountY];
        for(int x=0;x<mCountX;x++){
            for(int y=0;y<mCountY;y++){
                checkOccupied[x][y]=false;
            }
        }
        
        for(int x=0;x<mCountX;x++){
            for(int y=0;y<mCountY;y++){
                if(mShortcutsAndWidgets.getChildAt(x, y)!=null){
                    checkOccupied[x][y]=true;
                }
            }
        }
        ...
        for(int x=0;x<mCountX;x++){
            for(int y=0;y<mCountY;y++){
                if(mOccupied[x][y]!=checkOccupied[x][y]){
                    Log.e(TAG,"mOccupied error at x:"+x+" y:"+y);
                }else{
                    //Log.i(TAG,"mOccupied correct at x:"+x+" y:"+y);
                }
            }
        }
    ...
    */
    ...
    int[] createArea(int pixelX, int pixelY, int minSpanX, int minSpanY, int spanX, int spanY,
        ...
        // fix mOccupied here is better.
        fixOccupiedArray2(dragView);
...
out:            for (int i = x; i < x + spanX - 1 && x < xCount; i++) {
    ...
    public View isOccupiedAdv(int x, int y, int spanX, int spanY){
        // dump all occupied child info under test mode.
//        int childCount = mShortcutsAndWidgets.getChildCount();
//        for(int i=0;i<childCount;i++){
//            View v = mShortcutsAndWidgets.getChildAt(i);
//            CellLayout.LayoutParams lp = (CellLayout.LayoutParams) v.getLayoutParams();
//            Log.i("CellLayout","--> Child "+i+":"+v+" x:"+lp.cellX+" y:"+lp.cellY+" spanH:"+lp.cellHSpan+" spanV:"+lp.cellVSpan);
//
//        }
    ...
    public void fixOccupiedArray2(View dragview){
        
        // copy original mOccupied to a temp buffer
        boolean[][] mOccupiedOrg= new boolean[mCountX][mCountY];
        for(int i=0;i<mCountX;i++){
            for(int j=0;j<mCountY;j++){
                mOccupiedOrg[i][j]=mOccupied[i][j];
            }
        }
        
        clearOccupiedCells();
        
        int childsNum = mShortcutsAndWidgets.getChildCount();
        for(int i=0;i<childsNum;i++){
            ...
            if(child!=dragview){
                ...
                Log.e(TAG,"exclude dragView:"+dragview);
            }
        }
        
        for(int i=0;i<mCountX;i++){
            for(int j=0;j<mCountY;j++){
                if(mOccupiedOrg[i][j]!=mOccupied[i][j]){
                    Log.e(TAG,"mOccupied error!!");
                }
            }
        }
    }
    ...
    public void fixOccupiedArray(){
        //Log.e("CellLayout", "Found mCooupied array error, fix it");
        checkOccupiedArrayValidation();
        clearOccupiedCells();
        // then update mOccupied array
        for(int x=0;x<mCountX;x++){
            for(int y=0;y<mCountY;y++){
    ...
    static final class CellInfo {
        ...
        @Override
        public String toString() {
            return "CellInfo[view=" + (cell == null ? "null" : cell.getClass().getName())
                    + ", x=" + cellX + ", y=" + cellY + "]";
        }
    }
