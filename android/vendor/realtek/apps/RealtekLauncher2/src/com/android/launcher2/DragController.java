...
public class DragController {
     ...
     *          Makes dragging feel more precise, e.g. you can clip out a transparent border
     */
    public void startDrag(Bitmap b, 
    		int dragLayerX,
    		int dragLayerY,
            DragSource source,
            Object dragInfo,
    	...
    	DebugHelper.dump("startDrag (A)........mMotionDownX:"+mMotionDownX+" mMotionDownY:"+mMotionDownY);
        ...
        // RTKCOMMENT: according to the definition of registrationX and registrationY,
        // dragLayer should be the start coordinate of selected child?
        ...
        // RTKCOMMENT: create drag view
        final DragView dragView = mDragObject.dragView = 
        		new DragView(mLauncher,
        				b,
        				registrationX,	// X coordinate of touch point inside child view area
        				registrationY,	// Y coordinate of touch point inside child view area
        				0,
        				0,
        				b.getWidth(),
        				b.getHeight(),
        ...
        DebugHelper.dump("startDrag: x:"+mMotionDownX+" y:"+mMotionDownY);
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
    	...
    	DebugHelper.dump("DragController -----------> onInterceptTouchEvent");
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP:
            	DebugHelper.dump("--------------- Confirm moving by drag&drop..");
    ...
    // RTKCOMMENT: x and y is absolute position of screen
    public void handleMoveEvent(int x, int y) {
    	// RTKCOMMENT: Important!! this is where drag&drop move animation handled
    	if(x!=0&&y!=0)
    		DebugHelper.dump("DragController handleMoveEvent x:"+x+" y:"+y);
    ...
    public boolean onTouchEvent(MotionEvent ev) {
    	...
    	if(mLauncher.bIsInFakeDragState){
    		DebugHelper.dump(" bypass mouce onTouchEvent DragController during FakeDragState");
    		return true;
    	}
        ...
        case MotionEvent.ACTION_DOWN:
            ...
            break;
        case MotionEvent.ACTION_MOVE:
        	DebugHelper.dump("DragController onTouchEvent MotionEvent.ACTION_MOVE");
            handleMoveEvent(dragLayerX, dragLayerY);
            break;
        case MotionEvent.ACTION_UP:
        	DebugHelper.dump("DragController onTouchEvent MotionEvent.ACTION_UP x:"+dragLayerX+" y:"+dragLayerY);
            ...
            break;
        case MotionEvent.ACTION_CANCEL:
        	DebugHelper.dump("DragController onTouchEvent MotionEvent.ACTION_CANCEL");
            mHandler.removeCallbacks(mScrollRunnable);
            cancelDrag();
            break;
        }
    ...
    /**
     * API added for Realtek launcher
     * @param dragLayerX
     * @param dragLayerY
     */
    public void confirmFakeDrag(int dragLayerX, int dragLayerY){
    	handleMoveEvent(dragLayerX, dragLayerY);
        mHandler.removeCallbacks(mScrollRunnable);
        ...
        if (mDragging) {
        	...
        	// RTKCOMMENT: do not consider delete app case
            //PointF vec = isFlingingToDelete(mDragObject.dragSource);
            //if (vec != null) {
            //    dropOnFlingToDeleteTarget(dragLayerX, dragLayerY, vec);
            //} else {
    ...
    public void cancelFakeDrag(){
    	mHandler.removeCallbacks(mScrollRunnable);
        cancelDrag();
        // BUG_FIX: 42045 [cancel, via remote]
        View v = mLauncher.mWorkspaceFocusChild;
        if(v==null){
            DebugHelper.dumpSQA("CancelFakeDrag, can not find old view, query a new one");
            v=mLauncher.mWorkspace.getCurrentShortcutAndWidgetContainer().getChildAt(0);
        }
        // BUG_FIX: 42335
        v.requestFocus();
        DebugHelper.dumpSQA("CancelFakeDrag "+v);
        
        // DEV_FIX: delay a little bit to redraw focus frame
        //mLauncher.redrawWorkspaceFocusViewFrame(v);
        mLauncher.redrawFocusFrameDelay(500);
    }
     ...
     * manually set {@link #mMotionDownX} and {@link #mMotionDownY}
     * @param x
     * @param y
     */
    public void setMotionDownXYToPos(int x, int y){
    	
    	DebugHelper.dump("---> DragController setMotionDownXYToPos x="+x+" y="+y);
