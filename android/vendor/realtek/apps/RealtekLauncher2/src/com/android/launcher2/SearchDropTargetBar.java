...
public class SearchDropTargetBar extends FrameLayout implements DragController.DragListener {
    ...
    private android.view.ViewGroup.LayoutParams mDeleteDropTargetLP=null;
    ...
    private boolean bHasProcessDragStart;
    ...
    public void setup(Launcher launcher, DragController dragController) {
    	mLauncher=launcher;
    	bHasProcessDragStart=false;
        dragController.addDragListener(this);
        dragController.addDragListener(mInfoDropTarget);
        dragController.addDragListener(mDeleteDropTarget);
        dragController.addDropTarget(mInfoDropTarget);
        dragController.addDropTarget(mDeleteDropTarget);
        // disable flingToDeleteDropTarget in RTKLauncher
    ...
    public void onDragStart(DragSource source, Object info, int dragAction) {
    	...
    	//DebugHelper.dump("SearchDropTargetBar source:"+source);
    	//DebugHelper.dump("onLongClickView:"+mLauncher.onLongClickSelectedView);
    	
        boolean bHideRemoveText = mLauncher.bIsInFakeDragState || !mLauncher.isRemovableView(mLauncher.onLongClickSelectedView);
        if(bHideRemoveText){
            Log.i("SearchDrop","do not show DeleteDropTarget");
            return;
        }

        // This block of code is useless
    	if(!mLauncher.isRemovableView(mLauncher.onLongClickSelectedView)){
    	    // set DeleteDropTarget size to 0x0 - BUG_FIX: 42228
    	    if(mDeleteDropTargetLP==null)
    	        mDeleteDropTargetLP=mDeleteDropTarget.getLayoutParams();
    	    mDeleteDropTarget.setLayoutParams(new FrameLayout.LayoutParams(0, 0));
    		return;
    	}
    	// restore delete button layout
    	if(mDeleteDropTarget.getLayoutParams().width==0 && mDeleteDropTarget.getLayoutParams().height==0 && mDeleteDropTargetLP!=null)
    	    mDeleteDropTarget.setLayoutParams(mDeleteDropTargetLP);
    	
    	bHasProcessDragStart=true;
    ...
    //RTKCOMMENT: disable any deferOnDragEnd request, but might have some side effect. still need to be clarified
    public void deferOnDragEnd() {
    	DebugHelper.dump("----> deferOnDragEnd");
        //mDeferOnDragEnd = true;
    }

    @Override
    public void onDragEnd() {
    	DebugHelper.dump("----------> onDragEnd entry");
    	// do nothing during fake drag mode
    	if(!bHasProcessDragStart){
    		return;
    	}
    	
    	DebugHelper.dump("----------> onDragEnd");
