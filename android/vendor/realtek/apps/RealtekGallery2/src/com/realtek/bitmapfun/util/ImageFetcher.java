...
public class ImageFetcher extends ImageResizer {
    private static final String TAG = "ImageFetcher_Gallery";
    private static final int HTTP_CACHE_SIZE = 10 * 1024 * 1024; // 10MB
    public static final String HTTP_CACHE_DIR = "http_gallery_download";
...
//    		System.out.println("tmpStringis" +tmpStr);
...
*/
                // Return a sampled down version
                if(data.toLowerCase().endsWith("heic") || data.toLowerCase().endsWith("heic")) {
...
//    	Log.d(TAG, "downloadBitmap - download to http cache - " + urlString);
    	if (bCancel== true) 
    		return null;
...
//      	Log.d(TAG, "downloadBitmap - found in http cache - " + urlString);
        ...
        try {
            ...
            while ((b = in.read()) != -1) 
            {
                if(bCancel == true)
                {
                    Log.d(TAG, "Cancel in downloadBitmap");
                    out.close();
