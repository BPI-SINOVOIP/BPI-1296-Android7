...
public class GalleryUtils {
    ...
    private static final int PHOTO_NOTIFICATION_ID = 0x1000;

    public static final int MEDIAITEM_LIST_TASK_THRESHOLD = 25;

    public static final int EFFECT_OFF = 0;
    public static final int EFFECT_KEN_BURNS = 1;

    private static float sPixelDensity = -1f;
    private static boolean sCameraAvailableInitialized = false;
    private static boolean sCameraAvailable;
    private static boolean sUseReal4KPhoto = false;
    ...
    private static void initializeThumbnailSizes(DisplayMetrics metrics, Resources r) {
        ...
        MediaItem.setThumbnailSizes(maxPixels * 2, maxPixels, maxPixels / 2);
    ...
    public static Uri getImageContentUri(GalleryApp application, File imageFile) {
        if(imageFile==null)
            return null;
    ...
    public static String getItemPathString(GalleryApp application, String itemName, String mimeType) {
        String path;
        if(itemName.indexOf("file://")==0)
        {
            itemName = itemName.substring(7);
        }
        if(itemName.indexOf("://")<0 || itemName.startsWith("content://"))
        {
            Path localPath=null;
            if( application!=null)
            {
                if(itemName.indexOf("content://")>=0) {
                    localPath = application.getDataManager().findPathByUri(Uri.parse(itemName), mimeType);
                }
                else {
                    localPath = application.getDataManager().findPathByUri( getImageContentUri(application, new File(itemName)), mimeType);
                }
            }
            ...
            {
                path = "/uri/" + URLEncoder.encode("file://"+itemName) + "/" + URLEncoder.encode(mimeType);
            }
        }
        else
        {
            path = "/uri/" + URLEncoder.encode(itemName) + "/" + URLEncoder.encode(mimeType);
        }
        if(DBG) Log.d(TAG,"getItemPathString :"+path);
        return path;
    }

    public static String getItemPathString(GalleryApp application, String itemName) {
        String path;
        if(itemName.indexOf("file://")==0)
        {
            itemName = itemName.substring(7);
        }
        return getItemPathString(application, itemName, getMimeType(itemName));
    }

    public static boolean setMediaItemArray(GalleryApp application, String[] playlist, int getStartIdx, int getEndIdx, ArrayList<MediaItem> data)
    {
        if(data.size()<=0)
            return false;
        if(getStartIdx<0)
            getStartIdx=0;
        if(getEndIdx>=data.size())
            getEndIdx=data.size()-1;
        if(getStartIdx>getEndIdx)
            return false;

        if(DBG) Log.d(TAG, "getMediaItemArray from "+getStartIdx+" to "+getEndIdx);

        for(int count=getStartIdx;count<=getEndIdx;count++) {
            String path = GalleryUtils.getItemPathString(application, playlist[count]);
            data.set(count, (MediaItem)application.getDataManager().getMediaObject(path));
            if(DBG) Log.d("getMediaItemArray", count + ") mData.getFilePath() =" + data.get(count).getFilePath());
        }
        return true;
    }

    public static class MediaItemListTask extends AsyncTask<Void, Void, Void> {
        ArrayList<MediaItem> mData;
        String[] mPlaylist;
        ...
        public MediaItemListTask(String tag, GalleryApp application, ArrayList<MediaItem> data, String[] playlist)
        {
            this.tag = tag;
            this.mData = data;
        ...
        public MediaItemListTask(String tag, GalleryApp application, ArrayList<MediaItem> data, String[] playlist, boolean delay)
        {
            this.tag = tag;
            this.mData = data;
            this.mPlaylist = playlist;
        ...
        {
            Log.d(tag, "MediaItemListTask doInBackground [Start]");
            if (!isCancelled())
            {
                for (int count = 0; count < mPlaylist.length; count++) {
                    if (!isCancelled()) {
                        if(mData.get(count)==null)
                            setMediaItemArray(mApplication, mPlaylist, count, count, mData);
                        currentCount = count;
                        if( count%(mPlaylist.length/10)==0 )
                            Log.d(tag,"setMediaItemArray status: "+count+"/"+mPlaylist.length);
                    }
                    if (delay && !isCancelled() && count%(mPlaylist.length/10)==0) {
                        try {
                            Thread.sleep(1000);
                        }
            ...
            Log.d(tag, "MediaItemListTask doInBackground [End] : Data size is "+mData.size());
            return null;
        }
        ...
        {
            super.onCancelled();
            Log.d(tag,"MediaItemListTask onCancelled ");
        }
        @Override
        protected void finalize ()throws Throwable
        {
            super.finalize();
            Log.d(tag,"MediaItemListTask finalize()");
        }
    }
    ...
    public static String getMimeType(String filename) {
        ...
        if(DBG) Log.d(TAG,"getMimeType mimetype:"+ mimetype);
        return mimetype;
    }

    public static String[] getPlayListFromFile(String playListAddress) {
        ArrayList<String> playlist = new ArrayList<String>();
        try {
            BufferedReader br = new BufferedReader(new FileReader(playListAddress) );
            String line;
            ...
            Log.e(TAG, "getPlayListFromFile Exception: "+e);
        }
        return playlist.toArray(new String[playlist.size()]);
    }

    public static boolean hasSubtitlePlayListFromFile(String playListAddress) {
        boolean ret = false;
        Log.d(TAG, "Check subtitle playlist:"+playListAddress);
        ...
        try {
            ...
            Log.e(TAG, "hasSubtitlePlayListFromFile Exception: "+e);
        }
        return ret;
    }

    public static void updatePhotoNotification(String fileName, Activity activity)
    {
//        if(filePath.indexOf("file://")>=0)
//            filePath = filePath.substring(filePath.indexOf("file://")+7);
/*
        // Creates an explicit intent for an Activity in your app
        Intent resultIntent = new Intent(activity, GalleryActivity.class);
        resultIntent.setAction(Intent.ACTION_VIEW);
        resultIntent.setDataAndType(Uri.parse(filePath), "image/*");
        resultIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
        resultIntent.putExtra("index-hint", currentIndex);
        if(playList!=null)
            resultIntent.putExtra("playlist", playList);
        resultIntent.putExtra("SourceFrom", sourceFrom);
...
*/
        ...
        if(mBuilder==null) {
            mBuilder = new Notification.Builder(activity)
                .setSmallIcon(R.drawable.infobar_icon)
                .setAutoCancel(false)
                .setOngoing(true)
//                .setContentIntent(resultPendingIntent)
        ...
        notificationManager.notify(PHOTO_NOTIFICATION_ID, mBuilder.build());
    }

    public static void cancelPhotoNotification(Activity activity)
    {
        NotificationManager notificationManager =
            (NotificationManager) activity.getSystemService(Context.NOTIFICATION_SERVICE);
        // mId allows you to update the notification later on.
    ...
    public static String removePathPrefix(String path, boolean decode) {
...
//        Log.d(TAG, "removePathPrefix: "+path);
        return path;
    }
    ...
    public static String[] removePathPrefix(String path[], boolean decode) {
        ...
        for(int i=0;i<path.length;i++) {
            path[i] = removePathPrefix(path[i], decode);
        }
        return path;
    }

    public static void checkReal4KPhoto() {
        RtkHDMIManager rtkHDMIManager = new RtkHDMIManager();
        if(rtkHDMIManager == null) {
            Log.d(TAG, "rtkHDMIManager is null!");
            sUseReal4KPhoto = false;
        }
        else {
            int tvSystem =  rtkHDMIManager.getTVSystem();
            if(tvSystem==RtkTVSystem.TV_SYS_2160P_24HZ || tvSystem==RtkTVSystem.TV_SYS_2160P_25HZ ||
                tvSystem==RtkTVSystem.TV_SYS_2160P_30HZ || tvSystem==RtkTVSystem.TV_SYS_4096_2160P_24HZ ||
                tvSystem>=RtkTVSystem.TV_SYS_2160P_23HZ) {
                Log.d(TAG, "use_Real_4K_Photo");
                sUseReal4KPhoto = true;
            }
            else {
                Log.d(TAG, "does not use_Real_4K_Photo");
                sUseReal4KPhoto = false;
            }
        }
    }

    public static boolean useReal4KPhoto() {
        return sUseReal4KPhoto;
    }
}
