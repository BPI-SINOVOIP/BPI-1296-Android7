...
public class GLRootView extends GLSurfaceView
    ...
    private void layoutContentPane() {
        ...
        if(GalleryUtils.useReal4KPhoto() == true) {
            if(w==1920 && h==1080) {
                w*=2;
                h*=2;
                Log.i(TAG, "layoutContentPane: change to " + w + "x" + h + " form " + getWidth() + "x" + getHeight());
            }
        }
    ...
    public void onSurfaceCreated(GL10 gl1, EGLConfig config) {
        GL11 gl = (GL11) gl1;
        if(GalleryUtils.useReal4KPhoto() == true) {
            setUsage(0x00000003); //GRALLOC_USAGE_SW_READ_OFTEN
            Log.d(TAG, "setUsage to GRALLOC_USAGE_SW_READ_OFTEN");
        }
        if (mGL != null) {
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int w, int h) {
        if(GalleryUtils.useReal4KPhoto() == true) {
            if(w==1920 && h==1080) {
                getHolder().setFixedSize(w*2, h*2);
                requestLayout();
                Log.i(TAG, "surfaceChanged: change to " + w + "x" + h);
            }
        }
        unfreeze();
