...
public class DownloadCache {
    ...
    public Entry download(JobContext jc, Uri url) {
        if (!mInitialized) initialize();

        String stringUrl = url.toString();
        String scheme = url.getScheme();
        boolean bUseLastAccess = true;

        if(scheme.equals("directconnect")) {
            bUseLastAccess = false;
        }
        ...
        synchronized (mEntryMap) {
            Entry entry = mEntryMap.get(stringUrl);
            if (entry != null) {
                if (bUseLastAccess == true) {
                    updateLastAccess(entry.mId);
                    return entry;
                } else {
                    // Remove old one from entry map
                    boolean containsKey = mEntryMap.containsKey(stringUrl);
                    long hash = Utils.crc64Long(stringUrl);
                    String whereArgs[] = {String.valueOf(hash), stringUrl};
                    if (containsKey) {
                        mEntryMap.remove(stringUrl);
                        mDatabase.delete(TABLE_NAME,
                                    WHERE_HASH_AND_URL, whereArgs);
                    }
                }
            }
        }
    ...
    private class DownloadTask implements Job<File>, FutureListener<File> {
        ...
        public File run(JobContext jc) {
            ...
            try {
				boolean downloaded;
				if( mUrl.contains("directconnect") ) {
					tempFile = File.createTempFile("cache", ".tmp", mRoot);
					jc.setMode(ThreadPool.MODE_NETWORK);
					downloaded = DownloadUtils.requestDownload(jc, mUrl, tempFile);
				} else {
    ...
    public static class TaskProxy {
        ...
        synchronized void setResult(Entry entry) {
            ...
            if(entry==null) mIsCancelled=true;
            mEntry = entry;
            notifyAll();
        }
