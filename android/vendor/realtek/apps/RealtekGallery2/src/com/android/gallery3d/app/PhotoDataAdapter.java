...
public class PhotoDataAdapter implements PhotoPage.Model {
    ...
    private static final int DATA_CACHE_SIZE = 32;	// 256
    ...
    private ArrayList<MediaItem> mDataSet = new ArrayList<MediaItem>();
    ...
    public PhotoDataAdapter(AbstractGalleryActivity activity, PhotoView view,
            String[] playlist, Path itemPath, int indexHint, int cameraIndex,
            boolean isPanorama, boolean isStaticCamera) {
        mSource = null;
        mPhotoView = Utils.checkNotNull(view);
        mItemPath = Utils.checkNotNull(itemPath);
        ...
        mThreadPool = activity.getThreadPool();
        mNeedFullImage = true;
        setMediaDataSet(activity, playlist);
    ...
    @Override
    public void resume() {
        if(mMediaItemListTask != null && mMediaItemListTask.getStatus()==AsyncTask.Status.PENDING) {
    ...
    public void setMediaDataSet(AbstractGalleryActivity activity, String[] playlist) {
        ...
        if(playlist.length>GalleryUtils.MEDIAITEM_LIST_TASK_THRESHOLD) {
            if(mMediaItemListTask==null || (mMediaItemListTask != null && mMediaItemListTask.getStatus()==AsyncTask.Status.FINISHED)) {
                mDataSet = new ArrayList(Arrays.asList(new MediaItem[playlist.length]));
                GalleryUtils.setMediaItemArray((GalleryApp)activity.getApplication(), playlist, mCurrentIndex-GalleryUtils.MEDIAITEM_LIST_TASK_THRESHOLD/2,
                mCurrentIndex+GalleryUtils.MEDIAITEM_LIST_TASK_THRESHOLD/2, mDataSet);
                Log.d(TAG, "Create new mMediaItemListTask.");
                mMediaItemListTask = new GalleryUtils.MediaItemListTask(TAG, (GalleryApp)activity.getApplication(), mDataSet, playlist, false);
            }
        }
        else {
            mDataSet = new ArrayList(Arrays.asList(new MediaItem[playlist.length]));
            GalleryUtils.setMediaItemArray((GalleryApp)activity.getApplication(), playlist, 0, playlist.length, mDataSet);
        }
    }
        ...
        public void run() {
            while (mActive) {
                ...
                else
                {
                    ...
                    for(int count=info.contentStart;count<info.contentEnd;count++)
                    {
                        info.items.add(mDataSet.get(count));
                    }
                }
