    ...
    private FutureListener<BitmapRegionDecoder> mLargeListener =
        ...
        public void onFutureDone(Future<BitmapRegionDecoder> future) {
            ...
            if(GalleryUtils.useReal4KPhoto() == true) {
                if(mItem.getMimeType().toLowerCase().indexOf("jpeg")>=0 || mItem.getMimeType().toLowerCase().indexOf("jpg")>=0)
                    targetSize = MediaItem.getTargetSize(MediaItem.TYPE_LARGETHUMBNAIL);
            }

            options.inSampleSize = BitmapUtils.computeSampleSize(
                    (float) targetSize / Math.max(width, height));
