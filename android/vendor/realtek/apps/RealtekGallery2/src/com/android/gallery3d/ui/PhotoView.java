...
public class PhotoView extends GLView {
    ...
    public static final int SCREEN_NAIL_MAX = 1;	// 3
    ...
    public PhotoView(AbstractGalleryActivity activity) {
        ...
        mErrorIcon = new ResourceTexture(mContext, R.drawable.icon_default_preview);
    ...
    public void notifyImageChangeWithoutAnim(int index) {
        if (index == 0) {
            mListener.onCurrentImageUpdated();
        }
        mPictures.get(index).reload();
    ...
    public boolean toggleZoomIn() {
        PositionController controller = mPositionController;
        float scale = controller.getImageScale();

        if(scale * 1.4f < controller.getMaximalScale()-1) {
            controller.zoomIn(Math.max(1.0f, scale * 1.4f));
        } else if(scale == controller.getMaximalScale()) {
            controller.resetToFullView();
        } else {
            controller.zoomIn(Math.max(1.0f, controller.getMaximalScale()));
        }
        return true;
    }

    public boolean scrollPage(int x, int y) {
        PositionController controller = mPositionController;
        controller.scrollPage(x, y);
        return true;
    }

    public boolean isAtMinimalScale() {
        PositionController controller = mPositionController;
        return controller.isAtMinimalScale();
    }
...
//                drawLoadingFailMessage(canvas);
    ...
    // Draw the error icon (in the place where the spinner was)
    private void drawErrorIcon(GLCanvas canvas, int side) {
        int s = side / 3;
        // Draw the error icon at the center
        mErrorIcon.draw(canvas, (int)(-s*1.5 / 2), -s / 2, (int)(s*1.5), s);
    }
