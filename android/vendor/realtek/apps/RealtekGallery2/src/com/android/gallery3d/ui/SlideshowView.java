...
public class SlideshowView extends GLView {
    ...
    private static final float SCALE_SPEED = 0.10f ;
    ...
    public void next(Bitmap bitmap, int rotation) {
        ...
        if (mPrevTexture != null) {
            if (mPrevTexture.getBitmap() != bitmap) {
        ...
        if(mSlideshowEffect==GalleryUtils.EFFECT_KEN_BURNS) {
    ...
    public void setSlideshowDuration(int slideshowDuration) {
        mSlideshowDuration = slideshowDuration;
        if(GalleryUtils.useReal4KPhoto() == true) {
            mTransitionAnimation = new FloatAnimation(0, 1, 100);
        }
    }

    public void setSlideshowEffect(int slideshowEffect) {
        mSlideshowEffect = slideshowEffect;
    }
