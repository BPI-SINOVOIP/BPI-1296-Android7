...
public class DeviceInfoSettings extends SettingsPreferenceFragment implements Indexable {
    ...
    private static final String KEY_VERSION_INFORMATION = "version_information";

    static final int TAPS_TO_BE_A_DEVELOPER = 7;

    long[] mHits = new long[3];
    int mDevHitCountdown;
    int mDevVerHitCountdown = 3;
    Toast mDevHitToast;
    ...
    public void onCreate(Bundle icicle) {
        ...
        if (UbusRpc.isXen()) {
            setStringSummary(KEY_DEVICE_MODEL, Build.MODEL + "_NAS_ROUTER_XEN");
        } else if (UbusRpc.isOpenWrt()) {
            if (UbusRpc.isOttWifi()) { //NAS
                setStringSummary(KEY_DEVICE_MODEL, Build.MODEL + "_NAS");
            } else { //Router
                setStringSummary(KEY_DEVICE_MODEL, Build.MODEL + "_NAS_ROUTER");
            }
        }
        setStringSummary(KEY_BUILD_NUMBER, Build.DISPLAY);
        setStringSummary(KEY_VERSION_INFORMATION, parseRealtekVersionInformationAndroid());
    ...
    public boolean onPreferenceTreeClick(Preference preference) {
        ...
        } else if (preference.getKey().equals(KEY_VERSION_INFORMATION)) {
            Log.d(LOG_TAG, "Click rtk_version");
            //Toast.makeText(getActivity(), "Click rtk_version", Toast.LENGTH_LONG).show();
            mDevVerHitCountdown--;  // hit 3 times to display the version information dialog.
            if(mDevVerHitCountdown == 0) {
                findPreference(KEY_VERSION_INFORMATION).setEnabled(true);
                mDevVerHitCountdown = 3;
            } else {
                return true;
            }
        }
    ...
    public static final SearchIndexProvider SEARCH_INDEX_DATA_PROVIDER =
     ...
     /* Here we only display first Android version information */
    private String parseRealtekVersionInformationAndroid() {
        ...
        try {
            ...
            resStr = sb.toString();
            Log.d(LOG_TAG, "rtk_version: " + resStr);
            fis.close();

        } catch (Exception e) {
            Log.d(LOG_TAG, "Open /system/vendor/resource/rtk_version.txt failed!");
            resStr = "Realtek\nOpen /system/vendor/resource/rtk_version.txt failed!";  // To get 2 lines string.
        } finally {
            if (fis != null) {
                fis = null;
            }
        ...
        return res[1];  // Be careful, we return second line.
    }
