    ...
    public static class PartitionTask extends AsyncTask<Void, Integer, Exception> {
        ...
        protected Exception doInBackground(Void... params) {
            ...
            if ((UbusRpc.isOpenWrt())) {
                publishProgress(40);
                // Log.d(TAG, "activity.mDisk.sysPath="+activity.mDisk.sysPath);
                String targetDisk = activity.mDisk.sysPath.substring(activity.mDisk.sysPath.lastIndexOf("/")+1);
                boolean success = UbusUtil.getInstance(mActivity).format(targetDisk, "fat32");
                if (!success) {
                    return new Exception("OpenWrt format failed");
                } else {
                    return null;
                }
            } else {
