...
*/
        ...
        mHttpTimeout = config.getHttpConnectTimeout();
        mSuggestUri = null;
    }
...
*/
            JSONArray results = getHttpResults(suggestUri);
            if(results!=null) {
...
*/
    ...
    private boolean isNetworkConnected() {
        ...
        if (networkInfo == null)
        {
            Log.w(LOG_TAG, "networkInfo == null");
        }
        else
        {
            Log.w(LOG_TAG, "networkInfo.isConnected = " + networkInfo.isConnected());
        }
        return true;
    }
    ...
    private JSONArray getHttpResults(String queryUrl){
        InputStream input = null;
        HttpURLConnection connection = null;
        try {
            URL url = new URL(queryUrl);
            connection = (HttpURLConnection) url.openConnection();
            connection.setConnectTimeout(mHttpTimeout);  
            connection.setReadTimeout(mHttpTimeout); 
            connection.connect();
            ...
            if ((count = input.read(data)) != -1) {
                String res = new String(data, 0, count);
                if (DBG) Log.d(LOG_TAG, "GetInputStream: " + res);
