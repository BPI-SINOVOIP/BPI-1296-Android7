...
public class SuggestionsListAdapter extends SuggestionsAdapterBase<ListAdapter> {
    ...
    class Adapter extends BaseAdapter {
        ...
        public boolean isEnabled (int position) {
            if (DBG) Log.d("Adapter", "isEnabled position = " + position);
            SuggestionPosition item = getPromotedSuggestion(position);
            if (item.getSuggestionIntentAction() == null)
            {
                if (DBG) Log.d("Adapter", "return false ");
                return false;
            }
            else
            {
                if (DBG) Log.d("Adapter", "return true " );
                return true;
            }
        }
