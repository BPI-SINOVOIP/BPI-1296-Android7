...
public class DefaultSuggestionView extends BaseSuggestionView {
    ...
    protected void onFinishInflate() {
        ...
        mAsyncIcon1 = new AsyncIcon(mIcon1) {
            ...
            protected String getFallbackIconId(Source source) {
                if (source != null)
                {
                    if (DBG) Log.d(TAG, "source.getSourceIconUri().toString() = " + source.getSourceIconUri().toString());
    ...
    public void bindAsSuggestion(Suggestion suggestion, String userQuery) {
        ...
        setText1(text1);
        setText2(text2);
        if (DBG) Log.d(TAG, "text1 = " + text1);
        if (DBG) Log.d(TAG, "text2 = " + text2);
        if (DBG) Log.d(TAG, "Icon1 = " + suggestion.getSuggestionIcon1());
        if (DBG) Log.d(TAG, "Icon2 = " + suggestion.getSuggestionIcon2());
    ...
    private static void setViewDrawable(ImageView v, Drawable drawable) {
        ...
        if (drawable == null) {
            if (DBG) Log.d("DefaultSuggestionView", "drawable == null" );
    ...
    private class AsyncIcon {
        ...
        private void handleNewDrawable(Drawable icon, String id, Source source) {
            if (icon == null) {
                ...
                if (mWantedId != null && TextUtils.equals(mWantedId, mCurrentId)) {
                    return;
                }
