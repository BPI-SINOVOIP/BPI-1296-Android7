...
public class RankAwarePromoter extends AbstractPromoter {
    ...
    public void doPickPromoted(Suggestions suggestions,
        ...
        Log.d(TAG, "maxPromoted = " + maxPromoted );
    ...
    void promoteSuggestions(Iterable<CorpusResult> suggestions, int maxPromoted,
        ...
        if (DBG) 
            Log.d(TAG, "Available results: " + suggestions);

        for (CorpusResult result : suggestions) {
            if (result.getCount() > 0) {                
                result.moveTo(0);
                Corpus corpus = result.getCorpus();
                Log.d(TAG, "Add CorpusResult : " + corpus.getName());
                ...
                if (bIsGallery)
                {
                    ...
                    if (videoFirst)
                        promoted.add(new SuggestionPosition("video"));
                    else
                        promoted.add(new SuggestionPosition("photo"));

                }
                else
                {
                    if (corpus.getName().contains("music"))
                        promoted.add(new SuggestionPosition("music"));
                    else
                        promoted.add(new SuggestionPosition(corpus.getName()));
                }
                ...
                do {
                    if (accept(result)) {
                        ...
                        promoted.add(new SuggestionPosition(result));
                    }
    ...
    private void partitionSuggestionsByRank(Iterable<CorpusResult> suggestions,
        ...
        for (CorpusResult result : suggestions) {
            if (result.getCount() > 0) {
                result.moveTo(0);
                Log.d(TAG, result.getSuggestionText1());
                Corpus corpus = result.getCorpus();
                Log.d(TAG, corpus.getName() + " getCount() = " + result.getCount());
               
                if (isCorpusHighlyRanked(corpus) && !corpus.getName().equals("web")) {
