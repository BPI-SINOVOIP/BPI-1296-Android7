...
public abstract class SearchActivityView extends RelativeLayout {
    ...
    protected void onFinishInflate() {
        ...
        mSearchCloseButton.setOnFocusChangeListener(new SearchCloseFocusListener());
    ...
    protected void updateUi(boolean queryEmpty) {
		updateSearchClosButton(queryEmpty);
//        updateQueryTextView(queryEmpty);
        updateSearchGoButton(queryEmpty);
        updateVoiceSearchButton(queryEmpty);
    }

    protected void updateSearchClosButton(boolean queryEmpty) {
        if (queryEmpty) {
            mSearchCloseButton.setImageResource(R.drawable.us_inputbar_remove_none);
        } else {
            mSearchCloseButton.setImageResource(R.drawable.us_inputbar_remove);
        }
    }
    ...
    private class SuggestListFocusListener implements OnFocusChangeListener {
        public void onFocusChange(View v, boolean focused) {
			...
			if (focused) {
				Icon2.setImageResource(R.drawable.us_google_suggestion);
                        if(!(v.getParent() instanceof SuggestionsView))
                            return;
				SuggestionsView parent = (SuggestionsView)v.getParent();
				int position = parent.getPositionForView(findFocus());
				parent.setSelection(position);
			}
			else
			{
				Icon2.setImageResource(R.drawable.us_google_suggestion_none);
			}
        }
    }
    ...
    private class QueryTextViewFocusListener implements OnFocusChangeListener {
        public void onFocusChange(View v, boolean focused) {
			...
			LinearLayout Search_edit_frame = (LinearLayout)findViewById(R.id.search_edit_frame);
			ImageView Search_app_icon = (ImageView)findViewById(R.id.search_app_icon);
			ImageButton Search_close_btn = (ImageButton)findViewById(R.id.search_close_btn);
			if(focused)
			{
				Search_edit_frame.setBackgroundColor(getResources().getColor(R.color.colorFocusBG));
				((QueryTextView)v).setTextColor(getResources().getColor(R.color.colorFocusText));
				Search_app_icon.setImageResource(R.drawable.us_search_inputbar);
				Search_close_btn.setImageResource(R.drawable.us_inputbar_remove);
			}
			else
			{
				Search_edit_frame.setBackgroundColor(0xFFB4C2C9);
				((QueryTextView)v).setTextColor(0xFF49595D);
				Search_app_icon.setImageResource(R.drawable.us_search_inputbar_none);
				Search_close_btn.setImageResource(R.drawable.us_inputbar_remove_none);
			}
            if (focused) {
    ...
    private class SearchCloseFocusListener implements OnFocusChangeListener {
        public void onFocusChange(View v, boolean focused) {
            if (DBG) Log.d(TAG, "SearchClose focus change, now: " + focused);

			if(focused)
			{
				((ImageButton)v).setImageResource(R.drawable.us_inputbar_remove_focus);
			}
			else
			{
				((ImageButton)v).setImageResource(R.drawable.us_inputbar_remove_none);
			}
        }
    }
