...
// Initialize the gyroStillDet_t structure.
void gyroStillDetInit(struct gyroStillDet_t* gyro_still_det,
                      float var_threshold,
                      float confidence_delta) {

  // Clear all data structure variables to 0.
  memset(gyro_still_det, 0, sizeof(struct gyroStillDet_t));
...
void gyroStillDetUpdate(struct gyroStillDet_t* gyro_still_det,
...
float gyroStillDetCompute(struct gyroStillDet_t* gyro_still_det) {
  ...
  gyro_still_det->win_var_x = (gyro_still_det->acc_var_x * tmp_denom) -
                         tmp_mean * tmp_mean;
  tmp_mean = gyro_still_det->win_mean_y * tmp_denom;
  gyro_still_det->win_var_y = (gyro_still_det->acc_var_y * tmp_denom) -
                         tmp_mean * tmp_mean;
  tmp_mean = gyro_still_det->win_mean_z * tmp_denom;
  gyro_still_det->win_var_z = (gyro_still_det->acc_var_z * tmp_denom) -
                         tmp_mean * tmp_mean;

  // Define the variance thresholds.
  float upper_var_thresh = (gyro_still_det->var_threshold +
                            gyro_still_det->confidence_delta);

  float lower_var_thresh = (gyro_still_det->var_threshold -
                            gyro_still_det->confidence_delta);
  ...
  if ((gyro_still_det->win_var_x > upper_var_thresh) ||
    ...
    if ((gyro_still_det->win_var_x <= lower_var_thresh) &&
      ...
      // Sensor variance is below the lower threshold (i.e., stillness detected).
...
void gyroStillDetReset(struct gyroStillDet_t* gyro_still_det,
                       bool reset_stats) {
