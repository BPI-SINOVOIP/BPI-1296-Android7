...
static CGI_varlist *
read_multipart(CGI_varlist *v, const char *ptemplate) {
    ...
    for (;;) {
        ...
        if (filename != 0) {
            ...
            out = 0;
            if (ptemplate != 0 && *filename != 0) {
                if (localname == 0) {
                    localname = (char *) mymalloc(strlen(ptemplate) + 1);
                }
                strcpy(localname, ptemplate);
...
CGI_varlist *
CGI_get_post(CGI_varlist *v, const char *ptemplate) {
    const char *env;
    char *buf;
    int len;
...
CGI_varlist *
CGI_get_all(const char *ptemplate) {
    CGI_varlist *v = 0;

    v = CGI_get_cookie(v);
    v = CGI_get_query(v);
    v = CGI_get_post(v, ptemplate);
    return v;
}
