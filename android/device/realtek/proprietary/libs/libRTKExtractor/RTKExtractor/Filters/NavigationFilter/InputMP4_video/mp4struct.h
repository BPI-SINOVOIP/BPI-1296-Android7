...
#define	CMD_MP4_AUDIO_MEDIA_PROPERTY	(1L << 5)
#define	CMD_MP4_PRESENTATION_TIME		(1L << 6)
...
#define	CMD_MP4_SEND_STSZ_TABLE			(1L << 11)
#define CMD_MP4_SEND_FRAME_IDX			(1L << 12)
...
typedef struct _mp4NavInfo
{
    ...
    //playback info
    index_table_t	*ChunkIndex;
    index_table_t	*SampleIndex;
    ...
    unsigned long	DataLeft;// the len of the remaining payload not sent in the previous run
    unsigned long	CurChunkOffset;
...
---------------------------------------------------------------*/
	long long			reservedPayloadSize;// the len of the remaining payload not sent in the previous run											
	long long			dataObjectStart;	// start address of the Data Object
	long long			dataObjectEnd;		// end address of the Data Object
	int64_t				presentationTime;
	unsigned long		audioContext;		//for display time // Ivan*
	unsigned long		videoContext;		//for display time // neco+
	//long long			currentReadPosition;// for PlayAtTime
	//long long			streamTotalBytes;   // for PLayAtTime
	unsigned long		prevPTS;
	int					timeOutCounter;		// new
    int64_t				lastPresentedPTS;	// new
	NAV_REPEAT_MODE		repeatMode;
	mp4ff_t				mp4ff;              // mp4 file information
	//long long			SendFrame;          // num of frame data that has been send
    //mp4NavInfo_t		NavInfo[2];			//0:audio   1:video	// Ivan*
...
---------------------------------------------------------------*/
	...
	//unsigned char		buffer[MP4_BUFSIZE]; 
	NAVIOINFO			ioInfo;//ABRAY
	unsigned int 				streamTotalBytes;//ABRAY
	...
	bool				bIsSendingVideoData;
	bool				bIsSendingAudioData;
	bool        bIsSeekAudioBySample;
        bool                            bIsSendExtra4BHeader;
	int 				last_payloadSize;
  ...
  unsigned char bIsSendingStartcode;//RAY ++
  unsigned char bIsSkipingheader;//RAY ++
  unsigned long naluleftlen;//RAY ++
  unsigned long nalulenread;//RAY ++
  unsigned long bytenalulenread;//RAY ++
  unsigned char* pdatapool;//RAY ++
  unsigned long Pooldataleft;//RAY ++
  unsigned char* startcode;//RAY
#endif
  FILE* dumphandler;//RAY ++
  int total_subtracks;//RAYsubtitle
  int firstsubtrack;
  int subtrackidx;
  bool bSubtitleSync;
  int m_spicOnOff;
