...
static TP_INFO_FORMAT TP_GetInfoFormat(unsigned char *p)
{
    return (TP_INFO_FORMAT)(p[0] >> 6);
}

static unsigned int TP_GetInfoPointer(unsigned char *p)
{
...
static inline unsigned int TP_GetTPInfo_Error(unsigned char *p)
{
    return p[7];
}

static inline unsigned int TP_GetAFInfo_Timestamp(unsigned char *p)
{
...
static inline unsigned int TP_GetSecInfo_Status(unsigned char *p)
{
...
static inline unsigned int TP_GetSecInfo_MatchedSecIndex(TP_CONTEXT* ctx, unsigned char *p)
{
...
void TP_SetMemToMemMode      (TP_CONTEXT* ctx, int bEnable); // true: memory-to-memory TP;  false: data from external interface
void TP_SetTsDescramble      (TP_CONTEXT* ctx, int bEnable); // true: enable transport packet descrambling;  false: disable
void TP_SetExtractTimestamp  (TP_CONTEXT* ctx, int bEnable); // true: extract timestamp from 1394 packet;  false: use internal clock
void TP_SetTimestampFreq     (TP_CONTEXT* ctx, char freq);    // 0: 27MHz;  1: 90KHz  (used only when ExtractTimestamp is false)
void TP_SetChangeTscToZero   (TP_CONTEXT* ctx, int bEnable); // true: change transport-scrambling-control to '00' after descrambling
void TP_SetPIDFilter         (TP_CONTEXT* ctx, int bEnable); // true: enable PID filtering;  false: disable
void TP_SetResetMachine      (TP_CONTEXT* ctx, int bEnable); // true: reset state machine;  false: normal operating state

void TP_SetDiscardDuplicated (TP_CONTEXT* ctx, int bEnable); // true: discard duplicated packet;  false: keep
void TP_SetDiscardNull       (TP_CONTEXT* ctx, int bEnable); // true: discard null packet;  false: keep
void TP_SetDiscardTsError    (TP_CONTEXT* ctx, int bEnable); // true: discard transport error packet;  false: keep
void TP_SetDiscardSyncError  (TP_CONTEXT* ctx, int bEnable); // true: discard sync byte error packet;  false: keep
...
void TP_ConfigFramer(TP_CONTEXT* ctx, int bEnable,     // true: enable framer;  false: disable
...
void TP_SetInterruptState_Overflow (TP_CONTEXT* ctx, int bSet); // true: set;  false: reset
void TP_SetInterruptState_Drop     (TP_CONTEXT* ctx, int bSet); // true: set;  false: reset
void TP_SetInterruptState_Sync     (TP_CONTEXT* ctx, int bSet); // true: set;  false: reset
int TP_GetInterruptState_Overflow (TP_CONTEXT* ctx);
int TP_GetInterruptState_Drop     (TP_CONTEXT* ctx);
int TP_GetInterruptState_Sync     (TP_CONTEXT* ctx);
void TP_EnableInterrupt_Overflow   (TP_CONTEXT* ctx, int bEnable); // true: enable;  false: disable
void TP_EnableInterrupt_Drop       (TP_CONTEXT* ctx, int bEnable); // true: enable;  false: disable
void TP_EnableInterrupt_Sync       (TP_CONTEXT* ctx, int bEnable); // true: enable;  false: disable
unsigned int TP_GetBusyState(TP_CONTEXT* ctx);
// ********* DES settings ********* //
...
void TP_AddPIDFilter(TP_CONTEXT* ctx, char pid_index,   // PID table index relative to pid_partition base (0 ~ pid_partition_size-1)
                                      ...
                                      int bCheckCC,    // true: check continuity counter;  false: ignore
                                      int bUseTPInfo,  // true: enable TP info;  false: disable
                                      int bUseSecInfo, // true: enable section info;  false: disable
                                      int bTrackAF);   // true: enable AF tracking;  false: disable
...
void TP_SetSecFilter(TP_CONTEXT* ctx, char sec_index,                // section table index relative to sec_partition base (0 ~ sec_partition_size-1)
                                      ...
                                      int bCheckCRC,                // true: enable CRC32 check;  false: disable
                                      int bMatchWillPass,           // true: match will pass;  false: match will drop
                                      int bDefaultPass);            // true: default pass;  false: default drop (for last section filter only)
...
void TP_UpdateM2MRingBufferRWP(TP_CONTEXT* ctx, unsigned int RP, unsigned int WP);
...
int TP_GetInterruptState_RingFull  (TP_CONTEXT* ctx, char buf_index); // ring buffer index relative to buf_partition base (0 ~ buf_partition_size-1)
int TP_GetInterruptState_RingAvail (TP_CONTEXT* ctx, char buf_index); // ring buffer index relative to buf_partition base (0 ~ buf_partition_size-1)
...
void TP_EnableInterrupt_RingFull  (TP_CONTEXT* ctx, char buf_index, // ring buffer index relative to buf_partition base (0 ~ buf_partition_size-1)
                                                    int bEnable);  // true: enable;  false: disable
...
void TP_StopM2M(TP_CONTEXT* ctx);
...
void TP_StartStopM2MTransfer(TP_CONTEXT* ctx, int bStart); // true: start;  false: stop

void TP_SetInterruptState_M2M_Empty (TP_CONTEXT* ctx, int bSet);    // true: set;  false: reset
int TP_GetInterruptState_M2M_Empty (TP_CONTEXT* ctx);
void TP_EnableInterrupt_M2M_Empty   (TP_CONTEXT* ctx, int bEnable); // true: enable;  false: disable
