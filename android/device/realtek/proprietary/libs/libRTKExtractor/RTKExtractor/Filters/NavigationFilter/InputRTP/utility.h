#ifndef RTP_UTILITY_FUNCTION
#define RTP_UTILITY_FUNCTION
#include <stdlib.h>
#include <string.h>
...
static int detect_sbr_present_flag(unsigned char *buffer,
  ...
  char forceUpSampling= 0;  
  char dontUpSampleImplicitSBR = 1;
...
/*		if(syncExtensionType == 0x2b7) {
    ...
    if(syncExtensionType == 0x2b7){
      unsigned char tmp_OTi = getbits_get(5, buffer);//extensionAudioObjectType
      ...
      if(objectTypeIndex == 5){
        ...
        if(sbr_present_flag){
          unsigned char tmp;
			    ...
			    objectTypeIndex = tmp_OTi;
          tmp = getbits_get(4, buffer);//extensionSamplingFrequencyIndex
          /* check for downsampled SBR */
          if(tmp == *samplingFrequencyIndex){
            downSampledSBR = 1;           
          }
  ...
  // (1)no SBR signalled, this could mean either implicit signalling or no SBR in this file 
  //    MPEG specification states: assume SBR on files with samplerate <= 24000 Hz 
  // (2)we decide not to do upsampling for implicit sbr for now temporarily,
  //    but still keep 'forceUpSampling' here to remind me of future support.    
  if(sbr_present_flag == 0){
    if((*samplingFrequencyIndex >= 6)&&(!dontUpSampleImplicitSBR)){
    	//(dec->samplingFrequency <= 24000)
      *samplingFrequencyIndex -= 3; 
      forceUpSampling = 1;
    }else if(*samplingFrequencyIndex < 6){/* > 24000*/
      downSampledSBR = 1;  
		}
  }		
	
	*privateData1 = (downSampledSBR | 0x1000000);//objectTypeIndex;
