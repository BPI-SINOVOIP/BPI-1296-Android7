...
DRM_VOID DRM_CALL PrintCopyrightInfo( __in_z DRM_CHAR *f_pszAppDesc );

DRM_VOID DRM_CALL PrintCopyrightAndVersionInfo( __in_z DRM_CHAR  *f_pszAppDesc, 
                                                __in   DRM_DWORD  f_dwMajorVersion, 
                                                __in   DRM_DWORD  f_dwMinorVersion );

DRM_RESULT DRM_CALL DRMTOOLS_PrintLicenseStateData(
    __in_z    DRM_CHAR                  *pszOutputPrefix, 
    __in      DRM_LICENSE_STATE_DATA    *pDRMLicenseStateData );

DRM_RESULT DRM_CALL DRMTOOLS_TestKeyPair( 
    __in                       DRM_CRYPTO_CONTEXT                *f_pContextCrypto,
    __in                       DRM_PKCRYPTO_SUPPORTED_ALGORITHMS  f_eAlgorithm,
    __in_bcount( f_cbPubKey )  DRM_BYTE                          *f_pbPubKey,
    __in                       DRM_DWORD                          f_cbPubKey,
    __in_bcount( f_cbPrivKey ) DRM_BYTE                          *f_pbPrivKey,
    __in                       DRM_DWORD                          f_cbPrivKey );

DRM_RESULT DRM_CALL DRMTOOLS_CreateKeyPair(
...
DRM_RESULT  DRM_CALL DRMTOOLS_ReallocXmlContext(
    __inout    _XMBContext        **f_ppbXmlContext);

DRM_RESULT DRM_CALL DRMTOOLS_Base64EncodeOfByteArray(
    __in_bcount( f_cbArray ) const DRM_BYTE   *f_pbArray,
    __in                     const DRM_DWORD   f_cbArray,
    __inout                        DRM_STRING *f_pdstrB64 );

DRM_RESULT DRM_CALL DRMTOOLS_OpenOutputFile(
    __in  const   DRM_CONST_STRING    *f_pdstrFileName,
    __out         OEM_FILEHDL         *f_pFileHandle );

DRM_RESULT DRM_CALL DRMTOOLS_ReadFileData(
    __in  const DRM_CONST_STRING    *f_pdstrFileName,
    __out       DRM_STRING          *f_pdstrFileData);

DRM_RESULT DRM_CALL DRMTOOLS_ReadBinaryFileData(
    __in  const DRM_CONST_STRING    *f_pdstrFileName,
    __out       DRM_BYTE            **f_ppbData,
    __out       DRM_DWORD           *f_cbData );

DRM_RESULT DRM_CALL DRMTOOLS_WriteBufferToFile( 
    __in const DRM_CONST_STRING     *f_pdstrOutputFile,
    __in const DRM_BYTE             *pbBuffer,
    __in const DRM_DWORD            cbBuffer );

DRM_VOID DRM_CALL DRMTOOLS_PrintErrorCode( 
    __in DRM_RESULT dr );

DRM_RESULT DRM_CALL DRMTOOLS_PrintOPLOutputIDs(  __in DRM_OPL_OUTPUT_IDS *f_pOPLs);

DRM_RESULT DRM_CALL DRMTOOLS_PrintVideoOutputProtectionIDs( __in DRM_VIDEO_OUTPUT_PROTECTION_IDS_EX *f_pOPLs);

DRM_RESULT DRM_CALL DRMTOOLS_PrintInclusionListGUIDs( __in DRM_INCLUSION_LIST_CALLBACK_STRUCT *f_pInclusionList);

DRM_RESULT DRM_CALL DRMTOOLS_PrintPolicyCallback(
