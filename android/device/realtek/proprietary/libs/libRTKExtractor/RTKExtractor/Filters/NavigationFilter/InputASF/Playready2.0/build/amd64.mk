...
#*
#* Description:
#*    Defines options specific to amd64 required for compiling/linking
#*    DRM PK source code
#*
...
_DRM_OBJ_TARGETS=$(_DRM_OBJ_TARGETS:PLACEHOLDER_OBJ_PATH\=objchk\amd64\)
_DRM_OBJ_DIR=objchk\amd64
_DRM_BIN_DIR=objchk\amd64
#-------------------------------
!elseif "$(DRM_BUILD_TYPE)" == "FREE"
_DRM_OBJ_TARGETS=$(_DRM_OBJ_TARGETS:PLACEHOLDER_OBJ_PATH\=objfre\amd64\)
_DRM_OBJ_DIR=objfre\amd64
_DRM_BIN_DIR=objfre\amd64
#-------------------------------
!elseif "$(DRM_BUILD_TYPE)" == "PERF"
_DRM_OBJ_TARGETS=$(_DRM_OBJ_TARGETS:PLACEHOLDER_OBJ_PATH\=objperf\amd64\)
_DRM_OBJ_DIR=objperf\amd64
_DRM_BIN_DIR=objperf\amd64
!endif
#-------------------------------

#---------------------------------------------------
# Replace *'s in path's with the architecture
#
# This allows user to perform magic like LINKLIBS = lib\*\foo.lib and
# #and if they ensure that there exists lib\amd64\foo.lib for AMD64 platform
# the library the include are automatically platform specific without
# any ugly #ifdef in sources file.
#---------------------------------------------------
LINKLIBS=$(LINKLIBS:*=amd64)
TARGETLIBS=$(TARGETLIBS:*=amd64)
DRM_COPY_ROOT=$(DRM_COPY_ROOT:*=amd64)
...
SDK_LIB_PATH=$(PUBLIC_ROOT)\sdk\lib\amd64

#
# Visual Studio's setup.exe will add x86 lib paths (not x64 lib paths!) to all
# CMD.EXE's if the user consents, so add a real working x64 path in front of
# the existing ones.
#
LIBPATH= $(SDK_LIB_PATH);$(LIBPATH)
...
_DRM_C_DEFINES = $(_DRM_C_DEFINES) _CRT_SECURE_NO_DEPRECATE _CRT_OBSOLETE_NO_DEPRECATE
!endif

#---------------------------------------
# Add architecture-specific defines (used by DRM and SDKs)
#   _AMD64_, _M_AMD64     defined only for amd64 compilation
#---------------------------------------
_DRM_C_DEFINES = $(_DRM_C_DEFINES) _AMD64_ _M_AMD64
...
#---------------------------------------
!if "$(DRM_BUILD_TYPE)" == "CHECKED"
_C_COMPILER_SWITCHES = $(_C_COMPILER_SWITCHES) -Odib1y-
!else
...
#----------------------------------------------
# Set switch for C++ link time code generation
#----------------------------------------------
!if "$(DRM_BUILD_PROFILE)" == "PC" && "$(DRM_BUILD_TYPE)" != "CHECKED" && "$(DRM_STUBLIB_BUILD)" != "1"
_C_COMPILER_SWITCHES = $(_C_COMPILER_SWITCHES) -GL
!endif
...
_DRM_VERSIONING_TOOL_SWITCHES = -r \
	...
	-DVER_INTERNALNAME_STR=\"$(TARGETNAME).exe\"
...
# Note: SAFESEH is an implicit feature of Windows binaries on X64. No need to
#       ask for it by name (and the switch doesn't exist anyway)
#---------------------------------------
_C_LINKER_NOLOGO_SWITCH = -nologo
_C_LINKER_SWITCHES = -machine:X64 -DEBUG -NXCOMPAT -DYNAMICBASE
...
_C_LIBRARIAN_SWITCHES = -machine:X64 -IGNORE:$(_C_LIBRARIAN_IGNORE_LIST)
