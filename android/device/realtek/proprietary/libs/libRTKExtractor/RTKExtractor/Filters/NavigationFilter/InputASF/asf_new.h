...
#define STATUS_ASF_PLAYWITHNEWHEADER             (1L << 18)
...
#define CMD_ASF_VIDEO_USABILITY_INFO                 (1L << 17)
...
typedef enum {
  ...
  ASF_READ_AGAIN = -5,
  ASF_READ_ERROR_EIO = IOPLUGIN_ERROR_EIO,      //-8
...
typedef enum {
   ...
   ASF_CONTENT_ALBUM,
   ASF_CONTENT_COUNT
} ENUM_ASF_CONTENT_DES;
typedef struct _asf_content_descript {
    short		valueSize;
    unsigned char*	pValue;
}asf_content_descript_t;
//////////////////////////////////
...
//sync by baili_feng
//added by helen_he for mantis bug #29311 atlantic
...
typedef struct _tagasf_input_plugin {
   ...
   // long long			  curReadAddr;			// current read position
    ...
    //for content description object
    unsigned char*	content_description;
    int			content_description_size;
    asf_content_descript_t content_des[ASF_CONTENT_COUNT];

    long m_lastAudioPTS;
    int m_lastAudioPayloadSize;
    int m_lastVideoPayloadSize;

#if defined(ASF_SUPPORT_DRM)
	DRM_DWORD                     extendedContentEncryptionDataCount;
	DRM_BYTE                      *pExtendedContentEncryptionData;
	ASF_DRM_MANAGER_T	          drmManager;
	ASF_DRM_DECRYPT_CONTEXT_T     drmDecryptContext;
	...
	char*                         pLicenseURL;
	// Bruce add for Maxdome:
	int                           bHasURLArg;
	char*                         pChallengePrefix;

  #if defined(SUPPORT_PLAYREADY) || defined(SUPPORT_PLAYREADY20)
	DRM_AES_COUNTER_MODE_CONTEXT  drmAesCounterModeContext;
	bool			      bAESCTR;
  #endif
#endif    
    //Special Data needed for the AceTrex Playback
    char AceTrexDeviceID[128];
    char AceTrexPurchaseID[128];    
    char AceTrexRetailerID[32];   
...
long long doing_seek(asf_input_plugin_t* this, long long offset, int origin);
asf_count_jump_res_t asf_query_index_from_time(asf_input_plugin_t* this, int jumpSecond, int accordingToNowTime);
asf_count_jump_res_t asf_query_index_from_time_smooth(asf_input_plugin_t* this, int jumpSecond, int accordingToNowTime);
HRESULT asf_playback_atTime(asf_input_plugin_t* this, unsigned int eltmSeconds);
int read_to_buffer(asf_input_plugin_t* this);
asf_read_status_t read_payloads(asf_input_plugin_t* this, NAVBUF* pBuffer);
int query_demux_target(asf_input_plugin_t* this);
#endif //__ASF_PARSE__
