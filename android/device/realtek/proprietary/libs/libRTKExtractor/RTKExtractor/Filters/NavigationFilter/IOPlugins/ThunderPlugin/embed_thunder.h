...
#define ET_INVALID_CMD_PROXY_ID    0     //无效的命令代理id

/* 命令属性 */
#define ET_CMD_PROXY_HTTP_HEAD      0x01 //是否收发包命令有http头
#define ET_CMD_PROXY_AES            0x02 //是否收发包命令使用AES加密
#define ET_CMD_PROXY_LONGCONNECT    0x04 //是否和远程机器建立长连接
#define ET_CMD_PROXY_ACTIVECONNECT  0x08 //是否主动连接远程机器


/*
 * 创建命令代理接口(通过ip地址创建)
 * ip,port分别为远程机器的ip地址和端口(均为主机字节序)
 * attribute为命令代理属性,可以通过上面列出的属性组合指定
 * p_cmd_proxy_id返回创建的命令代理id
 * 返回值: 0    成功
 */
...
/*
 * 创建命令代理接口(通过域名创建)
 * host,port分别为远程机器的域名和端口(为主机字节序)
 * attribute为命令代理属性,可以通过上面列出的属性组合指定
 * p_cmd_proxy_id返回创建的命令代理id
 * 返回值: 0    成功
           21505 host长度过长(大于128字节)
 */
 ...
 * 得到命令代理信息
 * cmd_proxy_id  命令代理id
 * p_errcode     得到错误码,非0表示错误,需要调用et_cmd_proxy_close关闭命令代理,
                 为0时p_recv_cmd_id和p_data_len有效,需要外界调用et_cmd_proxy_get_recv_info取走命令
 * p_recv_cmd_id 得到下一个需要获得的命令id
 * p_data_len    得到下一条命令需要的buffer长度
 * 返回值: 0    成功
           21505 host长度过长(大于128字节)
           21506 无效的命令代理id
 */
 ...
 * 通过命令代理接收命令
 * cmd_proxy_id    命令代理id
 * recv_cmd_id     待接收的命令id(通过et_cmd_proxy_get_info的p_recv_cmd_id得到)
 * data_buffer     接收命令的buffer,由外层申请和释放
 * data_buffer_len 接收命令的buffer长度,大小必须大于et_cmd_proxy_get_info的p_data_len
 * 返回值: 0     成功
           21506 无效的命令代理id
           21507 无效的命令id(不是et_cmd_proxy_get_info的p_recv_cmd_id给出的)
           21508 接收命令的buffer过短
 */
...
ETDLL_API int32 et_get_free_disk( const char *path, u32 *free_size );
 ...
 */
ETDLL_API int32 et_open_drm_file( const char *p_file_full_path, u32 *p_drm_id );
