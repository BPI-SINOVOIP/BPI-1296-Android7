 ...
 */
int _x_io_select (int fd, int state, int timeout_msec);
 ...
 */
int _x_io_tcp_connect( const char *host, int port);
 ...
 */
int _x_io_tcp_connect_finish( int fd, int timeout_msec);
 ...
 */
off_t _x_io_tcp_read ( int s, char *buf, off_t todo);
 ...
 * aborts with zero if no data is available and *abort is set
 */
off_t _x_io_tcp_write ( int s, char *buf, off_t todo);
 ...
 * aborts with zero if no data is available and *abort is set
 */
off_t _x_io_file_read ( int fd, char *buf, off_t todo);
 ...
 */
off_t _x_io_file_write (int fd, char *buf, off_t todo);
 ...
 */
int _x_io_tcp_read_line( int sock, char *str, int size);

int _x_io_tcp_connect_ipv4(const char *host, int port);
