...
static inline void put16_be(void *put,uint16_t x)
{
    *((uint8_t *)put    ) = x >>8   & 0xff;
    *((uint8_t *)put + 1) = x       & 0xff;
}



static inline void put8(void *put,uint8_t x)
{
    *(uint8_t *)put       = x       & 0xff;
}
...
static inline uint32_t get32_be(void *put)
{
    return *(uint8_t *)put << 24 | *((uint8_t *)put+ 1) << 16 |
	*((uint8_t *)put + 2) << 8 | *((uint8_t *)put + 3);
}



static inline void put32_be(void *put,uint32_t x)
{
    *((uint8_t *)put    ) = x >> 24 & 0xff;
    *((uint8_t *)put + 1) = x >> 16 & 0xff;
    *((uint8_t *)put + 2) = x >>  8 & 0xff;
    *((uint8_t *)put + 3) = x       & 0xff;
}
...
static inline uint64_t get64_be(void *put)
{
    return (uint64_t)get32_be((uint8_t *)put) << 32 | (uint64_t)get32_be((uint8_t *)put + 4);
}

static inline uint64_t get64_le(void *put)
{
    return (uint64_t)get32_le((uint8_t *)put + 4) << 32 | (uint64_t)get32_le((uint8_t *)put);
}

static inline uint64_t getnb_le(void *put,int n)
{
    uint64_t val = 0;
    for(; n > 0 ; n--) {
	val <<= 8;
...
static inline uint16_t get16_be(void *put)
{
    return *(uint8_t *)put << 8 | *((uint8_t *)put + 1);
}

static inline uint16_t get16_le(void *put)
{
    return *(uint8_t *)put | *((uint8_t *)put + 1) << 8;
}

static inline uint8_t get8(void *put)
{
    return *(uint8_t *)put;
}
