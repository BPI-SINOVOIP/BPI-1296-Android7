...
/* send wma DRC settings (WMAPRODRC) 46 */
    ...
    /* notify video in DPB by pass mode 64 */
    PRIVATEINFO_VIDEO_DPBBYPASS_MODE,
    
    /* pass ts descramble info to software descrambler 65 (DESCRAMBLE_CONFIG)*/
    PRIVATEINFO_DEMUX_TS_DESCRAMBLE_INFO,
   
    /* In Widi MPEG2 need ignore aspect ratio 66 */
    PRIVATEINFO_VIDEO_DEC_IGNORE_BS_ASPECT_RATIO,
    
    /* set key offset in CW 67 (NAVDEMUXCWOFFSET)*/
    PRIVATEINFO_DEMUX_TRANSPORT_KEYS_OFFSET,

    /* set iv offset in CW 68 (NAVDEMUXCWOFFSET)*/
    PRIVATEINFO_DEMUX_TRANSPORT_IV_OFFSET,

    /* Use this infoId to deliver skip info 69*/
    PRIVATEINFO_AUDIO_APE_SKIPBITS,
	
	/* notify video fw the marker bit in RTP header  70*/
	PRIVATEINFO_RTP_HEADER_MARKER_BIT,	

    PRIVATEINFO_INC_PKTSIZE, /* 71 */
...
typedef struct _tagVIDEORMFORMAT{
    unsigned int  Length;                   /* BitstreamHeader length (for CnM) */
...
typedef struct _tagNAVPktInfo
{
	unsigned char *pPktData;
	unsigned int PktSize;
	int64_t pts64;
	bool isEOS;
	bool isAUDIONewFormat;
	unsigned int PinPktCnts;
    int PinPktBSByte;
} NAVPktInfo;
