...
typedef unsigned char           BYTE ;
    ...
    typedef struct _tagNAVDURATIONINFO {
...
//==== new for Stagefright
	int		totalTracks;
        ...
        int		video_width;
        int		video_height;
        unsigned int video_rotationDegrees;
        NAVDURATIONINFO	media_duration;
#define RTKEXTRACTOR_MAX_TRACK_COUNT     8
        NAVDURATIONINFO track_duration[RTKEXTRACTOR_MAX_TRACK_COUNT];
        int		bitRate;
//        int		Duration;
//        int		time_scale;
	int		avc_mCodecPrivateLen;
	char *		avc_mCodecPrivate;
	int		audio_mCodecPrivateLen;
	char *		audio_mCodecPrivate;
        int 		config_version;
        int 		profile;
        int 		level;
	...
	int		audio_NumberOfChannels;
        int		esds_decoderConfigLen;
        unsigned char*	esds_decoderConfig;
        int		esds_raw_len;
        unsigned char*	esds_raw_buffer;
...
//        int64_t         totalBytes;               /* -1 means unknown */
    	...
    	long            offsetToPhyAddr;          /* (non)cached address + offsetToPhyAddr = physical address */			
    ...
    #define NAV_MAX_AUDIO_STREAMS 24
    #define NAV_MAX_SPIC_STREAMS  48
	...
	typedef struct _tagNAVUSERDROP
	{
		...
		long bNotDropUntilEventComing;	// not continue drop packet until handle SOCKET_INFO_FLUSH.
	} NAVUSERDROP;
	
	typedef struct _tagNAVBDTITLELIST
	{
		long mplsId;
		long duration;
		long mvcSubpathIndex;	//-1 means it is 2D content
	} NAVBDTITLELIST;
