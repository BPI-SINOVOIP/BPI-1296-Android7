...
#endif
#elif 0
  static inline void pli_IPCWriteULONG(uint8_t *des, uint32_t value)
  {
      *(volatile uint32_t *)des = htonl(value);
  }

  static inline void pli_IPCWriteULONGLONG(uint8_t *des, uint64_t value)
  {
      *(volatile uint64_t *)des = htonq(value);
  }

  static inline uint32_t pli_IPCReadULONG(uint8_t *src)
  {
      return ntohl(*(volatile uint32_t *)src);
  }

  static inline uint64_t pli_IPCReadULONGLONG(uint8_t *src)
  {
      return ntohq(*(volatile uint64_t *)src);
  }
#endif
