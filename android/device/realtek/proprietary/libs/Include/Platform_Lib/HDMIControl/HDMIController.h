...
public:
...
#if defined(__LINARO_SDK__)
	static HDMIAdapter* sHDMIDevice;
#else
	static struct hdmi_device_t* sHDMIDevice;
#endif
	static HRESULT initHDMIController(void);
	static HRESULT deInitHDMIController(void);
	...
	static int checkRxSense(void);

	// The following functions are added by Kevin. They may call the HAL function.
	static int isEDIDReady(void);
	static int isHDMIDevice(void);
	static int getTVMaxLuminace(void);
	static int getTVSupportHDR(void);
    ...
    // New added API
    static int getEotfValue(void);
    static int setEotfMode(int mode);
...
private:
	static HRESULT initHDMIDevice(void);
	static HRESULT deInitHDMIDevice(void);
...
public:
	...
	static unsigned char ucArrSpec3D[18][2];
