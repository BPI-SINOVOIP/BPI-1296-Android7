...
#define	USE_MEMCPY_MEMSET
//#define	USE_SP_MACRO

#define	SP_MULTIPLY(p,x,y)	\
		{	uint64_t tt = (uint64_t)x * (uint64_t)y;	\
			p[1] = (uint32_t)(tt >> 32);	\
			p[0] = (uint32_t)(tt & 0xFFFFFFFF);	\
		}
#define	SP_DIVIDE(ret,pq,pr,u,v)	\
		{	uint64_t _uu, _q;	\
			_uu = (uint64_t)u[1] << 32 | (uint64_t)u[0];	\
			_q = _uu / (uint64_t)(v);	\
			*(pr) = (uint32_t)(_uu - _q * v);	\
			*(pq) = (uint32_t)(_q & 0xFFFFFFFF);	\
			ret = (uint32_t)(_q >> 32);	\
		}
