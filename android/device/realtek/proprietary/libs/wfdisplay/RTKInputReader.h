...
typedef void (*T_UIBCCallback)(WFDWindowEvent &evt);
void SetUIBCCallback(T_UIBCCallback lUIBC);
//void SetUIBCCallback1(void);
 ...
 */
struct RTKInputReaderConfiguration {
 ...
 */
#if 0 
class RTKInputReaderPolicyInterface : public virtual RefBase {
protected:
    RTKInputReaderPolicyInterface() { }
    virtual ~RTKInputReaderPolicyInterface() { }

public:
    /* Gets the input reader configuration. */
    virtual void getReaderConfiguration(RTKInputReaderConfiguration* outConfig) = 0;
...
class RTKInputReaderInterface : public virtual RefBase {
protected:
    RTKInputReaderInterface() { }
    virtual ~RTKInputReaderInterface() { }
 ...
 */
class RTKInputReaderContext {
public:
    RTKInputReaderContext() { }
    virtual ~RTKInputReaderContext() { }
    ...
    //virtual RTKInputReaderPolicyInterface* getPolicy() = 0;
 ...
 *
 * The RTKInputReader owns a collection of InputMappers.  Most of the work it does happens
 * on the input reader thread but the RTKInputReader can receive queries from other system
 * components running on arbitrary threads.  To keep things manageable, the RTKInputReader
 * uses a single Mutex to guard its state.  The Mutex may be held while calling into the
 * EventHub or the RTKInputReaderPolicy but it is never held while calling into the
 * InputListener.
 */

 
class RTKInputReader : public RTKInputReaderInterface {
public:
    RTKInputReader(const sp<EventHubInterface>& eventHub);
    //RTKInputReader(const sp<EventHubInterface>& eventHub,RTKWifiDisplaySink *pSink);
    virtual ~RTKInputReader();
	...
	//void SetUIBCCallback(T_UIBCCallback lCallback); //{mUIBCCallback = lCallback;};	
protected:
    // These members are protected so they can be instrumented by test cases.
    virtual InputDevice* createDeviceLocked(int32_t deviceId, int32_t controllerNumber,
            const InputDeviceIdentifier& identifier, uint32_t classes);

    class ContextImpl : public RTKInputReaderContext {
        RTKInputReader* mReader;

    public:
        ContextImpl(RTKInputReader* reader);
        ...
        //virtual RTKInputReaderPolicyInterface* getPolicy();
...
private:	
    ...
    //sp<RTKInputReaderPolicyInterface> mPolicy;
    //sp<QueuedInputListener> mQueuedListener;

    RTKInputReaderConfiguration mConfig;
...
class RTKInputReaderThread : public Thread {
public:
    RTKInputReaderThread(const sp<RTKInputReaderInterface>& reader);
    virtual ~RTKInputReaderThread();

private:
    sp<RTKInputReaderInterface> mReader;
...
class InputDevice {
public:
    InputDevice(RTKInputReaderContext* context, int32_t id, int32_t generation, int32_t
            controllerNumber, const InputDeviceIdentifier& identifier, uint32_t classes);
    ~InputDevice();

    inline RTKInputReaderContext* getContext() { return mContext; }
    ...
    void configure(nsecs_t when, const RTKInputReaderConfiguration* config, uint32_t changes);
...
private:
    RTKInputReaderContext* mContext;
    int32_t mId;
...
public:
    ...
    inline RTKInputReaderContext* getContext() { return mContext; }
    //inline RTKInputReaderPolicyInterface* getPolicy() { return mContext->getPolicy(); }
    ...
    virtual void dump(String8& dump);
    virtual void configure(nsecs_t when, const RTKInputReaderConfiguration* config, uint32_t changes);
...
public:
	...
	void move(int32_t relX,int32_t relY);
