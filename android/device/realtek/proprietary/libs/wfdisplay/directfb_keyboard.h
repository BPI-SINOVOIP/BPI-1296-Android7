...
//uibc
typedef enum {
     DSCROLL_V           = 0x00000001,  /* left mouse button */
     DSCROLL_H          = 0x00000002,  /* right mouse button */
     //DIBM_MIDDLE         = 0x00000004,  /* middle mouse button */
     //DIBM_TOUCH          = 0x00000008   /* touchpad touch */
} WFDInputDeviceScrollMask;
...
typedef enum {
     ...
     DWET_MT_PRESS       = 0x01000000,  /* multi-touch finger landing */
     DWET_MT_RELEASE     = 0x02000000,  /* multi-touch finger leaving */
     DWET_MT_MOTION      = 0x04000000,  /* multi-touch */

     DWET_POSITION_SIZE  = DWET_POSITION | DWET_SIZE,/* initially sent to
                                                        window when it's
                                                        created */

     DWET_ALL            = 0x073F033F   /* all event types */
} WFDWindowEventType;

typedef struct {
     WFDWindowEventType              type;       /* type of event */
	...
	DFBInputDeviceKeyIdentifier     key_id;
     ...
     int                             y;          /* y position of window
                                                    or coordinate within
                                                    window */
     int                             point_id;   /* point id for this event. for multi-touch event */
     int                             point_num;  /* total points pressed. for multi-touch motion event */
     ...
     /* used by DWET_MOTION, DWET_BUTTONDOWN, DWET_BUTTONUP */
     unsigned char        buttons;    /* mask of currently
                                                    pressed buttons */     
} WFDWindowEvent;
