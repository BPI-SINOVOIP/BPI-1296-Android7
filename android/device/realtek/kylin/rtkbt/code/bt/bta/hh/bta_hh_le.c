...
static void bta_hh_process_cache_rpt (tBTA_HH_DEV_CB *p_cb, tBTA_HH_RPT_CACHE_ENTRY *p_rpt_cache, UINT8 num_rpt);
#endif
...
void bta_hh_security_cmpl(tBTA_HH_DEV_CB *p_cb, tBTA_HH_DATA *p_buf)
{
    ...
    else
    {
        ...
        if (!(p_cb->status == BTA_HH_ERR_SEC && p_cb->reason == BTM_ERR_PROCESSING)) {
            ...
            bta_hh_le_api_disc_only_act(p_cb);
        }
        p_cb->status = BTA_HH_OK;
        p_cb->reason = BTM_SUCCESS;
#else
        if (!(p_cb->status == BTA_HH_ERR_SEC && p_cb->reason == BTM_ERR_PROCESSING))
            bta_hh_le_api_disc_act(p_cb);
#endif
...
static void bta_hh_le_search_hid_chars(tBTA_HH_DEV_CB *p_dev_cb, tBTA_GATTC_SERVICE *service)
{
    ...
    for (list_node_t *cn = list_begin(service->characteristics);
        ...
        LOG_DEBUG(LOG_TAG, "%s: %s 0x%04x", __func__, bta_hh_uuid_to_str(p_char->uuid.uu.uuid16),
...
#ifdef BLUETOOTH_RTK
            p_dev_cb->scan_int_win_char_handle = 0;
#endif
            for (list_node_t *cn = list_begin(service->characteristics);
                 cn != list_end(service->characteristics); cn = list_next(cn)) {
                tBTA_GATTC_CHARACTERISTIC *p_char = list_node(cn);
                ...
                if((p_char->uuid.len == LEN_UUID_16 &&
                    ...
                    p_dev_cb->scan_int_win_char_handle = p_char->handle;
                    p_dev_cb->scps_supported = TRUE;
                }
                if(p_dev_cb->scan_int_win_char_handle & p_dev_cb->scan_refresh_char_handle)
                    break;
#endif
            }
...
#ifdef BLUETOOTH_RTK_API
    if(rtkbt_api_BleHHNotify(p_dev_cb, p_rpt, p_data))  // stolen by app
    {
         APPL_TRACE_DEBUG("Notification Stolen by RTKBT\n");
         return;
    }
#endif
...
/*******************************************************************************
**
** Function         bta_hh_le_api_disc_only_act
**
** Description      initaite a Close API to a remote HID device,but we don't
**                  remove device from white list
**
** Returns          void
**
*******************************************************************************/
void bta_hh_le_api_disc_only_act(tBTA_HH_DEV_CB *p_cb)
{
    APPL_TRACE_DEBUG("bta_hh_le_api_disc_only_act p_cb->conn_id = %d", p_cb->conn_id);
    if (p_cb->conn_id != BTA_GATT_INVALID_CONN_ID)
    {
        BTA_GATTC_Close(p_cb->conn_id);
    }
...
#ifdef BLUETOOTH_RTK
    gatt_queue_write_op(GATT_WRITE_CHAR, p_dev_cb->conn_id, p_dev_cb->scan_int_win_char_handle, 4,
    ...
    if (num_rpt != 0)  /* no cache is found */
    {
        ...
        for (; i <num_rpt; i ++, p_rpt_cache ++)
        {
            ...
            else
            {
                ...
                {
                    p_rpt->client_cfg_value = BTA_GATT_CLT_CONFIG_NOTIFICATION;
                    //add by RTK, only one instance of 0x180A
                    if(p_cb->hid_srvc.srvc_inst_id != p_rpt_cache->srvc_inst_id)
                    {
                        APPL_TRACE_DEBUG("bta_hh_process_cache_rpt: set srvc_inst_id to %04x", p_rpt_cache->srvc_inst_id);
                        p_cb->hid_srvc.srvc_inst_id = p_rpt_cache->srvc_inst_id;
                    }
                    //end by RTK
                }
            }
...
**
** Description      Deregister for all notifications for the report applicable
**                  for the protocol mode.
**
** Parameters:
**
*******************************************************************************/
void bta_hh_le_deregister_input_notif(tBTA_HH_DEV_CB *p_dev_cb, UINT8 proto_mode, BOOLEAN register_ba)
{
    tBTA_HH_LE_RPT  *p_rpt = &p_dev_cb->hid_srvc.report[0];

#if BTA_HH_DEBUG == TRUE
    APPL_TRACE_DEBUG("%s: bta_hh_le_deregister_input_notif mode: %d", __func__, proto_mode);
#endif
