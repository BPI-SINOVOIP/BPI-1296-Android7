...
#ifdef BLUETOOTH_RTK
tBTA_DM_SP_CFM_REQ tmp_cfm_req;
#endif
...
void bta_dm_remove_device(tBTA_DM_MSG *p_data)
{
    ...
    bt_bdaddr_t bda;
    bdstr_t bd_addr_str;
    bdcpy(bda.address, p_dev->bd_addr);

    bdaddr_to_string(&bda, bd_addr_str, sizeof(bd_addr_str));

    btif_config_get_int(bd_addr_str, "DevType", &dev_type);
    if(dev_type == BT_DEVICE_TYPE_BLE)
    {
        APPL_TRACE_DEBUG("%s: remove from whitelist before link down", __func__, bta_dm_cb.device_list.count);
        btm_update_dev_to_white_list(FALSE, p_dev->bd_addr);
    }
#endif
...
void bta_dm_disc_rmt_name (tBTA_DM_MSG *p_data)
{
    ...
    if( p_data->rem_name.result.disc_res.bd_name[0] ) {
        bdcpy (result.disc_ble_res.bd_addr, p_data->rem_name.result.disc_res.bd_addr);
		strlcpy((char*)result.disc_ble_res.bd_name,
                (char*)bta_dm_search_cb.peer_name, BD_NAME_LEN);
        if(!bdcmp(result.disc_ble_res.bd_addr, bta_dm_search_cb.peer_bdaddr))
            bta_dm_search_cb.p_search_cback(BTA_DM_DISC_NAME_EVT, &result);
    }
...
void bta_dm_disc_result (tBTA_DM_MSG *p_data)
{
    ...
    p_msg->hdr.layer_specific = BTA_DM_API_DISCOVER_EVT;
#ifdef BLUETOOTH_RTK
    memcpy(&p_msg->disc_result.result, &p_data->disc_result.result, sizeof(tBTA_DM_SEARCH));
#endif
    bta_sys_sendmsg(p_msg);
}
...
static void bta_dm_discover_device(BD_ADDR remote_bd_addr)
{
            ...
            ||(bta_dm_search_cb.p_btm_inq_info && (!bta_dm_search_cb.p_btm_inq_info->appl_knows_rem_name))))
    {
         ...
         {
             APPL_TRACE_DEBUG("RSSI: %d", bta_dm_search_cb.p_btm_inq_info->results.rssi);
         }
         if(bta_dm_search_cb.p_btm_inq_info != NULL &&
            bta_dm_search_cb.p_btm_inq_info->results.rssi < gRssiThresholdForRemoteName &&
            (bta_dm_search_cb.state == BTA_DM_SEARCH_ACTIVE))
         {
             /* not req remote name */
             APPL_TRACE_DEBUG("RSSI is too low, device is far away, not ask remote name");
             bta_dm_search_cb.name_discover_done = TRUE;
         }
         else
         {
...
#ifdef BLUETOOTH_RTK
    memset(&sec_event, 0x0, sizeof(tBTA_DM_SEC));
#endif
...
#ifdef BLUETOOTH_RTK
    if(tmp_cfm_req.loc_auth_req || tmp_cfm_req.rmt_auth_req || tmp_cfm_req.loc_io_caps || tmp_cfm_req.rmt_io_caps)
    {
        sec_event.cfm_req.loc_auth_req = tmp_cfm_req.loc_auth_req;
        sec_event.cfm_req.rmt_auth_req = tmp_cfm_req.rmt_auth_req;
        sec_event.cfm_req.loc_io_caps = tmp_cfm_req.loc_io_caps;
        sec_event.cfm_req.rmt_io_caps = tmp_cfm_req.rmt_io_caps;
    }
#endif
...
#ifdef BLUETOOTH_RTK
    tmp_cfm_req.loc_auth_req = 0;
    tmp_cfm_req.rmt_auth_req = 0;
    tmp_cfm_req.loc_io_caps = 0;
    tmp_cfm_req.rmt_io_caps = 0;
#endif
}
...
#ifdef BLUETOOTH_RTK
        tmp_cfm_req.loc_auth_req = p_data->cfm_req.loc_auth_req;
        tmp_cfm_req.rmt_auth_req = p_data->cfm_req.rmt_auth_req;
        tmp_cfm_req.loc_io_caps = p_data->cfm_req.loc_io_caps;
        tmp_cfm_req.rmt_io_caps = p_data->cfm_req.rmt_io_caps;
#endif
...
#ifdef BLUETOOTH_RTK
                    /* unbond, remove all cached GATT information */
...
{
    APPL_TRACE_DEBUG("bta_dm_add_ble_device_extra_info");
    ...
    {
        APPL_TRACE_ERROR ("BTA_DM: Error adding BLE Device Extra Info for device %08x%04x",
...
#ifdef BLUETOOTH_RTK
        else if(conn_id == BTA_GATT_INVALID_CONN_ID && status == (tBTA_GATT_STATUS) BTA_GATT_ERROR)
            bta_dm_search_cb.conn_id = BTA_GATT_INVALID_CONN_ID;
#endif
