...
#ifdef BLUETOOTH_RTK
    int attr_type = *(int *)context;
    if(attr_type == 0) {
        tBTA_GATTC_INCLUDED_SVC *p_inc_svc = (tBTA_GATTC_INCLUDED_SVC *)data;
        APPL_TRACE_ERROR("\t Attr handle[%d] uuid[0x%04x] type[%s] prop[0x%1x]",
                        p_inc_svc->handle, p_inc_svc->uuid.uu.uuid16,
                        bta_gattc_attr_type[0], 0);
    }
    else if(attr_type == 1) {
        tBTA_GATTC_CHARACTERISTIC *p_charac = (tBTA_GATTC_CHARACTERISTIC *)data;
        APPL_TRACE_ERROR("\t Attr handle[%d] uuid[0x%04x] type[%s] prop[0x%1x]",
                        p_charac->handle, p_charac->uuid.uu.uuid16,
                        bta_gattc_attr_type[1], p_charac->properties);
        if (p_charac->descriptors != NULL) {
            attr_type = 2;
            list_foreach(p_charac->descriptors, display_cache_attribute, &attr_type);
        }
    }
    else if(attr_type == 2) {
        tBTA_GATTC_DESCRIPTOR *p_decri = (tBTA_GATTC_DESCRIPTOR *)data;
        APPL_TRACE_ERROR("\t Attr handle[%d] uuid[0x%04x] type[%s] prop[0x%1x]",
                        p_decri->handle, p_decri->uuid.uu.uuid16,
                        bta_gattc_attr_type[2], 0);
...
#ifdef BLUETOOTH_RTK
    int attr_type = 0;
    if (p_cur_srvc->included_svc != NULL) {
        list_foreach(p_cur_srvc->included_svc, display_cache_attribute, &attr_type);
    }
    if (p_cur_srvc->characteristics != NULL) {
        attr_type = 1;
        list_foreach(p_cur_srvc->characteristics, display_cache_attribute, &attr_type);
    }
