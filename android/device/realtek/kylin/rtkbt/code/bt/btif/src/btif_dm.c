...
#ifdef BLUETOOTH_RTK
//Device name length should be long enough
#define DEFAULT_LOCAL_NAME_MAX  240
#else
...
#ifdef BLUETOOTH_RTK
        if(!bdcmp(bd_addr->address, pairing_cb.bd_addr)) {
...
static void btif_dm_search_devices_evt (UINT16 event, char *p_param)
{
    ...
    switch (event)
    {
        ...
        case BTA_DM_INQ_RES_EVT:
        {
            ...
            {
#ifdef BLUETOOTH_RTK
                UINT8 manufact_length = 0;
                UINT8* p_manufacter = NULL;
                bt_remote_version_t info;
                memset(&info, 0 , sizeof(bt_remote_version_t));
                if (p_search_data->inq_res.p_eir && (p_search_data->inq_res.device_type != BT_DEVICE_TYPE_BREDR))
                {
                    p_manufacter =  BTM_CheckEirData(p_search_data->inq_res.p_eir, BTM_EIR_MANUFACTURER_SPECIFIC_TYPE,&manufact_length);
                    if(p_manufacter) {
                        info.manufacturer = p_manufacter[0] + (p_manufacter[1] << 8);
                        BTIF_TRACE_DEBUG("%s manufacturer = 0x%04x", __FUNCTION__, info.manufacturer);
                    }
                }
...
static void btif_dm_search_services_evt(UINT16 event, char *p_param)
{
    ...
    switch (event)
    {
...
#ifdef BLUETOOTH_RTK
        {
            ...
            BOOLEAN is_hid = check_cod_hid(&bd_addr);
            BTIF_TRACE_ERROR("%s: device_type: 0x%x", __FUNCTION__,p_data->disc_res.device_type);
            if(is_hid && (device_type & BT_DEVICE_TYPE_BLE) && BTM_UseLeLink(p_data->disc_res.bd_addr))
            {
                BTIF_TRACE_DEBUG("%s  in BTA_DM_DISC_CMPL_EVT the device is ble hid",__FUNCTION__);
                if((pairing_cb.state == BT_BOND_STATE_BONDING) && (bdcmp(p_data->disc_res.bd_addr, pairing_cb.bd_addr) == 0))
                {
                    if(p_data->disc_res.result == BTA_SUCCESS)
                    {
                         bond_state_changed(BT_STATUS_SUCCESS, &bd_addr, BT_BOND_STATE_BONDED);
                         BTIF_TRACE_DEBUG("%s  in BTA_DM_DISC_CMPL_EVT bond state change bonded",__FUNCTION__);
                    }
                    else
                    {
...
#ifdef BLUETOOTH_RTK
        case BTA_DM_DISC_NAME_EVT:
        {
            bt_property_t prop;
            bt_bdaddr_t bd_addr;
            bt_status_t ret;

            BTIF_TRACE_DEBUG("%s: Found remote name, update it",__FUNCTION__);
            bdcpy(bd_addr.address, p_data->disc_res.bd_addr);
            if(p_data->disc_res.bd_name[0]) {
                prop.type = BT_PROPERTY_BDNAME;
                prop.len = strlen((char *)p_data->disc_res.bd_name);
                prop.val = p_data->disc_res.bd_name;
                ret = btif_storage_set_remote_device_property(&bd_addr, &prop);
                ASSERTC(ret == BT_STATUS_SUCCESS, "storing remote name failed", ret);
                HAL_CBACK(bt_hal_cbacks, remote_device_properties_cb,
                                 BT_STATUS_SUCCESS, &bd_addr, 1, &prop);
            }
        }
...
#ifdef BLUETOOTH_RTK
             if ((p_data->disc_res.result == BTA_SUCCESS) && (p_data->disc_res.raw_data_size > 0 && p_data->disc_res.p_raw_data != NULL)) {
                 param_len += p_data->disc_res.raw_data_size;
             }
#endif
         } break;
...
bt_status_t btif_dm_start_discovery(void)
{
    ...
    {
        BTIF_TRACE_EVENT("%s, bonding is on the way, inquiry return", __FUNCTION__);
        return BT_STATUS_BUSY;
    }
    BTIF_TRACE_EVENT("%s, bonding is not on the way, inquiry continue", __FUNCTION__);
#endif
...
{
    BTIF_TRACE_EVENT("%s, transport = %d", __func__, transport);
...
#ifdef BLUETOOTH_RTK
       if((!bdcmp(pairing_cb.bd_addr,bd_addr.address)) && check_cod_hid(&bd_addr))
        {
            BTIF_TRACE_DEBUG("%s device is hid,bond state change bonded lator",__FUNCTION__ );
            state = BT_BOND_STATE_BONDING;
        }
#endif
