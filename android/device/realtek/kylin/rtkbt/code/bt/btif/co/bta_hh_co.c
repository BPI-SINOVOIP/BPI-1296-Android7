...
#ifdef BLUETOOTH_RTK
        if (ret < (ssize_t)sizeof(ev) && ret < UHID_EVENT_SIZE )
#else
...
void bta_hh_co_close(UINT8 dev_handle, UINT8 app_id)
{
    ...
    for (i = 0; i < BTIF_HH_MAX_HID; i++) {
        ...
        if (p_dev->dev_status != BTHH_CONN_STATE_UNKNOWN && p_dev->dev_handle == dev_handle) {
            ...
            bdstr_t bdstr = {0};
            if(&(p_dev->bd_addr))
                 bdaddr_to_string(&(p_dev->bd_addr), bdstr,18);
            btif_config_get_int((char const *)&bdstr,"DevType", &device_type);
            if(device_type == BT_DEVICE_TYPE_BLE)
                 rtkbt_api_NotifyHIDStatus(0, &(p_dev->bd_addr));
#endif
            break;
        }
     }
