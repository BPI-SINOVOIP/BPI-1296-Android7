...
static void event_command_ready(fixed_queue_t *queue, UNUSED_ATTR void *context) {
  if (command_credits > 0) {
    ...
    // Send it off
#ifdef BLUETOOTH_RTK
    //For 0xFC94, do not hold wake_lock
    if(wait_entry->opcode != 0xFC94) {
...
#ifdef BLUETOOTH_RTK
    STREAM_TO_UINT8(temp_command_credits, stream);
#else
