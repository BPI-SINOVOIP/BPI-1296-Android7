...
#define RTKBT_RELEASE_NAME "20191111_TV_ANDROID_7.x"
...
{
    LogMsg("syncword: %02x", hdr->syncword);
    LogMsg("freq %skHz", sample_freqs[hdr->sampling_frequency]);
    LogMsg("blocks %u", sbc_blocks[hdr->blocks]);
    LogMsg("channel mode %s", chan_modes[hdr->channel_mode]);
    LogMsg("allocation method %s", alloc_methods[hdr->allocation_method]);
    LogMsg("subbands %u", subbands[hdr->subbands]);
}
    ...
    if(0 == hci_conn->type)//l2cap
    {
        ...
        if((prof_info->profile_index == profile_a2dp) && (length > 100))//avdtp media data
        {
            if(!is_profile_busy(profile_a2dp)){
                ...
                LogMsg("rtp: v %u, cc %u, pt %u", rtph->v, rtph->cc, rtph->pt);
                /* move forward */
        ...
        if(rtk_prof.a2dp_packet_count == 0)
        {
            if(is_profile_busy(profile_a2dp))
            {
                ...
                if (is_profile_busy(profile_sink)) {
                    LogMsg("timeout_handler: sink busy->idle!");
