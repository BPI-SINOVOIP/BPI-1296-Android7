...
#ifndef BLUETOOTH_RTK_UPD_PARAMS_RETRY
    //Do not use preferred parameters at init, the latency may not be used at this point
...
void l2cble_process_conn_update_evt (UINT16 handle, UINT8 status)
{
    ...
    {
#ifdef BLUETOOTH_RTK_UPD_PARAMS_RETRY
        if(p_lcb->conn_update_mask & L2C_BLE_CONN_UPDATE_DISABLE)
        {
            L2CAP_TRACE_WARNING("l2cble_process_conn_update_evt: Reset after failure");
            p_lcb->conn_update_mask &= ~L2C_BLE_NEW_CONN_PARAM;
        }
        else
        {
            L2CAP_TRACE_WARNING("l2cble_process_conn_update_evt: Retry after failure");
            p_lcb->conn_update_mask |= L2C_BLE_NEW_CONN_PARAM;
        }
#endif
...
#ifdef BLUETOOTH_RTK
        //shall not use preferred paramter at init, update when requested
        (UINT16)(BTM_BLE_CONN_INT_MIN_DEF),
        (UINT16)(BTM_BLE_CONN_INT_MAX_DEF),
        (UINT16)(BTM_BLE_CONN_SLAVE_LATENCY_DEF),
        (UINT16)(BTM_BLE_CONN_TIMEOUT_DEF),
#else
