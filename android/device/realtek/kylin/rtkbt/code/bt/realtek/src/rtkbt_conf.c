...
static int rtkbt_conf_parse_config(struct config_entry **items, char *content)
{
    ...
    BTIF_TRACE_DEBUG("rtkbt_conf_parse_config\n");
    while((line = rtkbt_conf_getline(content, &content)) != NULL)
    {
        BTIF_TRACE_DEBUG("rtkbt_conf_parse_config line:%s\n", line);
        switch(line[0])
        {
            case '[':
            {
...
int rtkbt_loadconfig(char * path)
{
    ...
    BTIF_TRACE_EVENT("rtkbt_loadconfig(%s) load!\n", path);
    fd = open(path, O_RDONLY);
    if(fd < 0)
    {
        BTIF_TRACE_ERROR("rtkbt_loadconfig(%s) open fail!\n", path);
        return -1;
    }
    res = fstat(fd, &st);
    if(res)
    {
        BTIF_TRACE_ERROR("rtkbt_loadconfig(%s) stat fail! reason=%d\n", path, res);
        return -1;
    }
    if(st.st_size <= 0)
    {
        BTIF_TRACE_ERROR("rtkbt_loadconfig(%s) size fail! size=%d\n", path, (int)st.st_size);
        return -1;
    }
    BTIF_TRACE_DEBUG("rtkbt_loadconfig(%s) size=%d!\n", path, (int)st.st_size);
    content = malloc(st.st_size+1);
    if(content == NULL)
    {
        BTIF_TRACE_ERROR("rtkbt_loadconfig(%s) malloc fail!\n", path);
        return -1;
    }
    res = read(fd, content, st.st_size);
    if(res <= 0)
    {
        BTIF_TRACE_ERROR("rtkbt_loadconfig(%s) read fail! res=%d\n", path, res);
        return -1;
    }
    content[res] = 0;
    ...
    {
        BTIF_TRACE_ERROR("rtkbt_loadconfig(%s) rtkbt_conf_parse_config fail! res=%d\n", path, res);
        return -1;
    }
    configs[res] = config;
