...
void VolumeManager::createSmbDirs()
{
    const char *RAMFS_PATH = "/tmp/ramfs";
    const char *MOUNT_ROOT = "/tmp/ramfs/mnt";
    std::lock_guard<std::mutex> lock(mLock);
    int ret = -1;
    struct stat s;
    stat(RAMFS_PATH, &s);
    if((s.st_mode&0775)!=0775)
    {
        SLOGI("Create Folder /tmp/ramfs");
        ret = mkdir(RAMFS_PATH, 0775); // create FILE_PATH
        if (ret ==-1)
            SLOGI("mkdir err = %s", strerror(errno));
        ret = chmod(RAMFS_PATH, 0775);
        if (ret ==-1)
            SLOGE("chmod err = %s", strerror(errno));
        ret = chown(RAMFS_PATH, AID_SYSTEM, AID_SYSTEM);
        if (ret ==-1)
            SLOGE("chown err = %s", strerror(errno));
    }
    memset(&s, 0, sizeof(struct stat));
    stat(MOUNT_ROOT, &s);
    if((s.st_mode&0775)!=0775)
    {
        SLOGI("Create Folder /tmp/ramfs/mnt");
        int ret = mkdir(MOUNT_ROOT, 0775); // create MOUNT_ROOT
        if (ret ==-1)
            SLOGI("mkdir err = %s", strerror(errno));
        ret = chmod(MOUNT_ROOT, 0775);
        if (ret ==-1)
            SLOGE("chmod err = %s", strerror(errno));
        ret = chown(MOUNT_ROOT, AID_SYSTEM, AID_SYSTEM);
        if (ret ==-1)
            SLOGE("chown err = %s", strerror(errno));
    }
}

int VolumeManager::SmbMountShare(const char* smbPath, const char* mountPath, const char* user, const char* passwd, const char* domain)
{
    int mount_ret = 0;
    char option[256];
    sprintf(option, "vers=2.0,username=%s,password=%s,domain=%s,iocharset=utf8,mapchars,soft,nounix,noserverino,uid=%u,gid=%u,file_mode=0777,dir_mode=0777", user, passwd, domain,AID_MEDIA_RW,AID_MEDIA_RW);
    char dumpOption[256];
    sprintf(dumpOption, "vers=2.0,username=%s,domain=%s,iocharset=utf8,mapchars,soft,nounix,noserverino,uid=%u,gid=%u,file_mode=0777,dir_mode=0777", user, domain,AID_MEDIA_RW,AID_MEDIA_RW);
    SLOGI("dumpOption = %s", option);
    SLOGI("Smb Path = %s", smbPath);
    SLOGI("mount Path = %s", mountPath);

    if((mount_ret = mount(smbPath, mountPath, "cifs", 0x8000, option))!=0)
    {
        SLOGI("[%s,%s,%d] ret = %d , %d , %s\n",__FILE__,__func__,__LINE__,mount_ret,errno,strerror(errno));
        getBroadcaster()->sendBroadcast(ResponseCode::SmbMountFailed, strerror(errno), false);
    }
    else
    {
        getBroadcaster()->sendBroadcast(ResponseCode::SmbMountOkay, mountPath, false);
    }
    return mount_ret;
}
int VolumeManager::SmbUmountShare()
{
    int umount_ret = 0;
    FILE* pf=NULL;
    char line[512];
    ...
    SLOGI("SmbUmountShare():%d",__LINE__);
    if (NULL != pf)
    {
        while(!feof(pf))
        {
            ...
            fgets(line,512,pf);
            // //ip/xxx /tmp/ramfs/mnt/xxxxxxxx cifs xxxxxxxxxxxxxxx
            if (NULL != (indexS = strstr(line, "/tmp/ramfs/mnt/")))
            {
                indexE = strstr(indexS, " cifs ");
                memset(mountPath, 0x00, 256);
                snprintf(mountPath, (1+(int)(indexE-indexS)),"%s",indexS);
                if ( (umount_ret = umount(mountPath) ) != 0 )
                {
                    SLOGI("[%s,%s,%d] umount [%s] fail = %d , %d , %s\n",__FILE__,__func__,__LINE__,mountPath,umount_ret,errno,strerror(errno));
                    getBroadcaster()->sendBroadcast(ResponseCode::SmbUmountFailed, strerror(errno), false);
                }
                else
                {
                    if (0 != rmdir(mountPath))
                    {
                        SLOGI("[%s,%s,%d] rmdir [%s] error , %d , %s\n",__FILE__,__func__,__LINE__,mountPath,errno,strerror(errno));
                    }
                    getBroadcaster()->sendBroadcast(ResponseCode::SmbUmountOkay, mountPath, false);
                }
            }
        }
        fclose(pf);
...
int VolumeManager::NFSMountShare(const char* nfsPath, const char* mountPath, const char* addr)
{
    int mount_ret = 0;
    char option[256];
    //sprintf(option, "nolock,addr=%s,iocharset=utf8,mapchars,soft,nounix,noserverino,sec=ntlm,uid=%u,gid=%u,file_mode=0777,dir_mode=0777,cache=none", addr,AID_MEDIA_RW,AID_MEDIA_RW);
    sprintf(option, "nolock,addr=%s", addr);
    SLOGI("option = %s", option);
    SLOGI("NFS Path = %s", nfsPath);
    SLOGI("mount Path = %s", mountPath);
    //if(mount(":/tmp","/tmp/test-nfs","nfs",0,"nolock,addr=192.168.1.100") == -1)
    if((mount_ret = mount(nfsPath, mountPath, "nfs", 0, option))!=0)
    {
        SLOGI("[%s,%s,%d] ret = %d , %d , %s\n",__FILE__,__func__,__LINE__,mount_ret,errno,strerror(errno));
        getBroadcaster()->sendBroadcast(ResponseCode::NFSMountFailed, strerror(errno), false);
    }
    else
    {
        getBroadcaster()->sendBroadcast(ResponseCode::NFSMountOkay, mountPath, false);
    }
    return mount_ret;
}

int VolumeManager::NFSUmountShare()
{
    int umount_ret = 0;
    FILE* pf=NULL;
    char line[512];
    ...
    char* indexE;
    pf = fopen("/proc/mounts","r");
    SLOGI("NFSUmountShare():%d",__LINE__);
    if (NULL != pf)
    {
        while(!feof(pf))
        {
            ...
            if (NULL != (indexS = strstr(line, "/tmp/ramfs/mnt/")))
            {
                indexE = strstr(indexS, " nfs ");
                memset(mountPath, 0x00, 256);
                snprintf(mountPath, (1+(int)(indexE-indexS)),"%s",indexS);
                if ( (umount_ret = umount(mountPath) ) != 0 )
                {
                    SLOGI("[%s,%s,%d] umount [%s] fail = %d , %d , %s\n",__FILE__,__func__,__LINE__,mountPath,umount_ret,errno,strerror(errno));
                    getBroadcaster()->sendBroadcast(ResponseCode::NFSUmountFailed, strerror(errno), false);
                }
                else
                {
                    if (0 != rmdir(mountPath))
                    ...
                    getBroadcaster()->sendBroadcast(ResponseCode::NFSUmountOkay, mountPath, false);
                }
            }
        }
        fclose(pf);
