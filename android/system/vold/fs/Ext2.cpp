...
status_t Check(const std::string& source, const std::string& target) {
     ...
     * fix the filesystem.
     */
    ret = mount(c_source, c_target, "ext2", tmpmnt_flags, tmpmnt_opts);
    if (!ret) {
        int i;
        for (i = 0; i < 5; i++) {
...
status_t Mount(const std::string& source, const std::string& target, bool ro,
    ...
    rc = mount(c_source, c_target, "ext2", flags, NULL);
...
status_t RepairMount(const std::string& source, const std::string& target,
    ...
    status_t ret = 0;
    if(bTryMount){
        ret = ext2::Mount(source, target, ro, remount, executable);
        if(ret == 0){
            SLOGD("Ext2 try mount success.");
            return ret;
        }
    }
    if(bCheckVolume){
       if(ext2::Check(source, target)){
           SLOGE("Ext2 check volume fail.");
           if(bFormatWhenError){
               if(ext2::Format(source)){
                   SLOGE("Ext2 format volume fail.");
               }
           }
       }
    }
    return ext2::Mount(source, target, ro, remount, executable);
}
