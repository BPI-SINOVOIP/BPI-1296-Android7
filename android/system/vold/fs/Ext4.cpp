                            ...
                            bool bCheckVolume, bool bFormatWhenError, bool bTryMount)
{
    status_t ret = 0;
    if(bTryMount){
        ret = ext4::Mount(source, target, ro, remount, executable);
        if(ret == 0){
            SLOGD("Ext4 try mount success.");
            return ret;
        }
    }
    if(bCheckVolume){
        if(ext4::Check(source, target)){
            SLOGE("Ext4 check volume fail.");
            if(bFormatWhenError){
                if(ext4::Format(source, 0, "")){
                    SLOGE("Ext4 format volume fail.");
                }
            }
        }
    }
    return ext4::Mount(source, target, ro, remount, executable);
}
