                        ...
                        bool bCheckVolume, bool bFormatWhenError, bool bTryMount)
{
    status_t ret = 0;
    if(bTryMount){
        ret = vfat::Mount(source, target, ro, remount, executable, ownerUid, ownerGid, permMask, createLost);
        if(ret == 0){
            SLOGD("vfat try mount success.");
            return ret;
        }
    }
    if(bCheckVolume){
        if(vfat::Check(source)){
            SLOGE("fat check volume fail.");
            if(bFormatWhenError){
                if(vfat::Format(source, 0)){
                    SLOGE("fat format volume fail.");
                }
            }
        }
    }
    return vfat::Mount(source, target, ro, remount, executable, ownerUid, ownerGid, permMask, createLost);
}
}  // namespace vfat
}  // namespace vold
}  // namespace android
