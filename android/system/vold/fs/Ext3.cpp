...
status_t Check(const std::string& source, const std::string& target) {
     ...
     * fix the filesystem.
     */
    ret = mount(c_source, c_target, "ext3", tmpmnt_flags, tmpmnt_opts);
    if (!ret) {
        int i;
        for (i = 0; i < 5; i++) {
...
status_t Mount(const std::string& source, const std::string& target, bool ro,
    ...
    rc = mount(c_source, c_target, "ext3", flags, NULL);
...
status_t RepairMount(const std::string& source, const std::string& target,
    ...
    status_t ret = 0;
    if(bTryMount){
        ret = ext3::Mount(source, target, ro, remount, executable);
        if(ret == 0){
            SLOGD("Ext3 try mount success.");
            return ret;
        }
    }
    if(bCheckVolume){
       if(ext3::Check(source, target)){
           SLOGE("Ext3 check volume fail.");
           if(bFormatWhenError){
               if(ext3::Format(source)){
                   SLOGE("Ext3 format volume fail.");
               }
           }
       }
    }
    return ext3::Mount(source, target, ro, remount, executable);
}
