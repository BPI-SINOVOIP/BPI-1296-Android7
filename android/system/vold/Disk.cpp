...
static const char* kGptLinuxData = "0FC63DAF-8483-4772-8E79-3D69D8477DE4";
...
status_t Disk::readMetadata() {
    ...
    notifyEvent(ResponseCode::DiskNickname, mNickname);
    return OK;
}
...
status_t Disk::readPartitions() {
        ...
        if (!strcmp(token, "DISK")) {
            ...
            if (table == Table::kMbr) {
                ...
                if (!strcasecmp(typeGuid, kGptBasicData)) {
                    createPublicVolume(partDevice);
                } else if (!strcasecmp(typeGuid, kGptLinuxData)) {
                    createPublicVolume(partDevice);
                } else if (!strcasecmp(typeGuid, kGptAndroidExpand)) {
                    createPrivateVolume(partDevice, partGuid);
                } else {
                    //LOG(DEBUG) << "unsupport filesystem";
                    //Mac OS uses Partition type GUID randomly. 
                    LOG(DEBUG) << "unmatched GUIG, mount as a PublicVolume";
                    createPublicVolume(partDevice);
                }
            }
        }
    ...
    if( table == Table::kApm ){
        //disk major and minor number
        int comma_index = mId.find(",");
        int colon_index = mId.find(":");
        std::string major = mId.substr(colon_index+1, comma_index-(colon_index+1));
        std::string minor = mId.substr(comma_index+1);

        //find disk partitions.
        std::string line;
        std::vector<std::string> tmp_vect;
        std::vector<std::string> part_vect;
        std::vector<std::string> dev_vect;
        std::string dev_node;
        std::ifstream file("/proc/partitions");
        if (file.is_open()){
            while ( getline (file,line) ){
                tmp_vect.clear();
                dev_vect.push_back(line);
                std::istringstream iss(line);
                for(std::string s; iss >> s; ){
                    tmp_vect.push_back(s);
                }

                if(!strcasecmp(tmp_vect.at(0).c_str(), major.c_str())){
                    if(!strcasecmp(tmp_vect.at(1).c_str(), minor.c_str())){
                        LOG(DEBUG) << "disk: " << line;
                       dev_node = tmp_vect.at(3);
                    }
                }
            }
            file.close();
        ...
        for(unsigned i=0; i<dev_vect.size(); i++){
            std::size_t found = dev_vect.at(i).find(dev_node);
            std::vector<std::string> disk_info;
            std::istringstream iss(dev_vect.at(i));
            for(std::string s; iss >> s; )
                disk_info.push_back(s);

            if(found!=std::string::npos){
                int partNum_index = found + dev_node.size();
                std::string partID = dev_vect.at(i).substr(found);
                if(partID.size() > 1){
                    part_vect.push_back(partID);
                    std::vector<std::string> blkid_cmd;
                    std::vector<std::string> blkid_out;
                    blkid_cmd.push_back("/system/bin/blkid");
                    blkid_cmd.push_back("/dev/block/" + partID);
                    status_t res = ForkExecvp(blkid_cmd, blkid_out);
                    for (auto line : blkid_out) {
                        char* cline = (char*) line.c_str();
                        if (cline == nullptr) break;
                        LOG(DEBUG) << "found partition: " << disk_info.at(0) << " , " << disk_info.at(1);
                        dev_t partDevice = makedev(std::stoi(disk_info.at(0)), std::stoi(disk_info.at(1)));
                        createPublicVolume(partDevice);
                    }
                }
            }
        ...
        LOG(WARNING) << mId << " if has unknown partition table; trying entire device";
        std::string fsType;
        std::string unused;
