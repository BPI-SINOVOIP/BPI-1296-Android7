...
int _ion_alloc_test(int fd, ion_user_handle_t *handle)
{
	...
	if (tiler_test)
		ret = ion_alloc_tiler(fd, width, height, fmt, len, alloc_flags,
					  handle, &stride);
	else
		ret = ion_alloc(fd, len, align, ION_HEAP_SYSTEM_MASK| ION_HEAP_SYSTEM_CONTIG_MASK, alloc_flags, handle);
...
int ion_alloc_test(int count)
{
	...
	handle = (ion_user_handle_t *)malloc(count * sizeof(ion_user_handle_t));
	if(handle == NULL) {
...
int ion_alloc_fail_alloc_test()
{
	...
	handle = (ion_user_handle_t *)malloc(COUNT_ALLOC_MAX * sizeof(ion_user_handle_t));
	if(handle == NULL) {
		printf("%s(): FAILED to allocate memory for ion_handles\n", __func__);
		return -ENOMEM;
	}
