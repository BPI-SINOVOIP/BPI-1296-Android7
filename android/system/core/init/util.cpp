...
int ubi_attach_mtd(const char *name)
{
	...
	//already attached, find out corresponding ubi_num
	for(ubi_num = 0; ubi_num < 256; ubi_num++){
		if(ubi_dev_read_int(ubi_num, "mtd_num", -1) == mtd_num){
			ERROR("%s ubi%d already attached to mtd%d\n", __func__, ubi_num, mtd_num);
			return ubi_num;
		}
	}
    }
