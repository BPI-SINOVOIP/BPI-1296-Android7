...
public final class MainKeyboardView extends KeyboardView implements DrawingProxy,
    ...
    // draw focus on virtual keyboard (soft keyboard, on screen keyboard)
    // stanely, 20140407.
    ...
    private Rect rect;
    private List<Key> sequenceKeys = new ArrayList<Key>();  // 4 rows, top-left element is start point.
    private List<Key> rowKeys1 = new ArrayList<Key>();  // row1
    private List<Key> rowKeys2 = new ArrayList<Key>();  // row2
    private List<Key> rowKeys3 = new ArrayList<Key>();  // row3
    private List<Key> rowKeys4 = new ArrayList<Key>();  // row4
    ...
    // since the keys are not sorted according to row layout, we do the sorting here to get sequence list.
    public List<Key> getSequenceKeys() {
        sequenceKeys.clear();
        sequenceKeys.addAll(getKeyboard().getSortedKeys());
        return sequenceKeys;
    }
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        // stanely 20140407, draw focus for IR input.
        Paint p = new Paint();
        p.setColor(Color.CYAN);
        p.setStyle(Style.STROKE);
        p.setStrokeWidth(4.25f);

        getSequenceKeys(); 
        if(lastKeyIndex >= sequenceKeys.size() - 1) {
            lastKeyIndex = sequenceKeys.size() - 2;
        }
        focusedKey = sequenceKeys.get(lastKeyIndex);  // get the candidate key object.
        rect = new Rect(focusedKey.getX(), focusedKey.getY(), focusedKey.getX() + focusedKey.getWidth(), focusedKey.getY() + focusedKey.getHeight());
