...
public class WebViewBrowserActivity extends Activity implements PopupMenu.OnMenuItemClickListener {
    ...
    private static final int SYSTEM_UI_HIDE_DELAY = 3000;
...
public void onResume(){
    ...
    private void registerMediaButtonEvent(){
        Context ctx = this;
        mSession = new MediaSession(ctx,ctx.getPackageName());
        //     the associated intent will be handled by the component being registered
        Intent intent = new Intent(ctx, MediaButtonReceiver.class);
        PendingIntent pi = PendingIntent.getBroadcast(ctx,
                0/*requestCode, ignored*/, intent, 0/*flags*/);
        ...
        this.getWindow().getDecorView().setOnSystemUiVisibilityChangeListener(new View.OnSystemUiVisibilityChangeListener() {
            ...
            public void onSystemUiVisibilityChange(int visibility) {
                if ((visibility & View.SYSTEM_UI_FLAG_FULLSCREEN) == 0) {
                    mHandler.postDelayed(checkSystemUiRunnable, SYSTEM_UI_HIDE_DELAY);
                    if(isFullscreen()) isSystemUiShown = true;
                } else {
    ...
    private void createAndInitializeWebView() {
        ...
        webview.setWebChromeClient(new WebChromeClient() {
            ...
            public void onShowCustomView(View view, WebChromeClient.CustomViewCallback callback) {
                ...
                mFullscreenView.setBackgroundColor(Color.BLACK);
...
//                getWindow().clearFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN);
            ...
            private void switchToImmersiveMode() {
                final Activity activity = WebViewBrowserActivity.this;
                if (activity == null) {
                    return;
                }
        ...
        webview.setOnFocusChangeListener(new View.OnFocusChangeListener() {
            ...
            public void onFocusChange(View view, boolean b) {
                log("onFocusChange");
                if(isFullscreen())
                    hideKeyboard(WebViewBrowserActivity.this.getWindow().getDecorView());
            }
        });
    ...
    public boolean onMenuItemClick(MenuItem item) {
        switch(item.getItemId()) {
            case R.id.menu_reset_webview:
                ...
                createAndInitializeWebView();
                return true;
            case R.id.menu_fullscreen_webview:
                showFullscreen(true);
                return true;
    ...
    private boolean isFullscreen() {
        return mNavigationBar != null && mNavigationBar.getVisibility() == View.GONE;
    }

    private void showFullscreen(boolean show) {
        ActionBar actionBar = getActionBar();
        if(show){
            if(actionBar!=null) actionBar.hide(); // slides out
            if(mNavigationBar!=null) mNavigationBar.setVisibility(View.GONE);
            hideSystemUI(this.getWindow().getDecorView());
        } else {
            if(actionBar!=null) actionBar.show(); // slides in
            if(mNavigationBar!=null) mNavigationBar.setVisibility(View.VISIBLE);
            showSystemUI(this.getWindow().getDecorView());
        }
    }
