...
#ifdef RTK_AUTO_AC
	return CW_MSG_ELEMENT_DISCOVERY_TYPE_BROADCAST;
#else
...
#ifdef RTK_CAPWAP
	(valPtr->vendorInfos)[0].vendorIdentifier = cwRtkWtpGetVendorId();
#else
...
#ifdef RTK_CAPWAP
	*(int *)(((valPtr->vendorInfos)[0]).valuePtr) = cwRtkWtpGetModelNo(); // MODEL NUMBER
#else
	*(int *)(((valPtr->vendorInfos)[0]).valuePtr) = 123456; // MODEL NUMBER
#endif
	// my vendor identifier (IANA assigned "SMI Network Management Private Enterprise Code")
#ifdef RTK_CAPWAP
	(valPtr->vendorInfos)[1].vendorIdentifier = cwRtkWtpGetVendorId();
#else
...
#ifdef RTK_CAPWAP
	*(int *)(((valPtr->vendorInfos)[1]).valuePtr) = cwRtkWtpGetSerialNo(); // SERIAL NUMBER
#else
...
CWBool CWWTPGetWtpDesc(CWWTPVendorInfos *valPtr) 
{
	...
	CW_CREATE_OBJECT_SIZE_ERR(( ( (valPtr->vendorInfos)[0] ).valuePtr), (valPtr->vendorInfos)[0].length, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
#ifdef RTK_CAPWAP
	*(int *)(((valPtr->vendorInfos)[0]).valuePtr) = cwRtkWtpGetHwVersion(); // HW version
#else
	*(int *)(((valPtr->vendorInfos)[0]).valuePtr) = 123456; // HW version
	...
	CW_CREATE_OBJECT_SIZE_ERR(( ( (valPtr->vendorInfos)[1] ).valuePtr), (valPtr->vendorInfos)[1].length, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
#ifdef RTK_CAPWAP
	//*(int *)(((valPtr->vendorInfos)[1]).valuePtr) = cwRtkWtpGetSwVersion(); // SW version
	*(int *)(((valPtr->vendorInfos)[1]).valuePtr) = cwRtkWtpGetCurrentFwId();
#else
	*(int *)(((valPtr->vendorInfos)[1]).valuePtr) = 12347; // SW version
#endif	
	// my vendor identifier (IANA assigned "SMI Network Management Private Enterprise Code")
#ifdef RTK_CAPWAP
	(valPtr->vendorInfos)[2].vendorIdentifier = cwRtkWtpGetVendorId();
#else
...
#ifdef RTK_CAPWAP
	*(int *)(((valPtr->vendorInfos)[2]).valuePtr) = cwRtkWtpGetBootVersion(); // Boot version
#else
...
/* 2013 Babylon: we use the flash mib
