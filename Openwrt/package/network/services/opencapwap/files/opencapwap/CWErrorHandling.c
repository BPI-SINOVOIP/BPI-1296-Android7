...
CWErrorHandlingInfo capwap_error_info;

CWBool _CWErrorRaise(CWErrorCode code, const char *msg, const char *fileName, int line) {
	CWErrorHandlingInfo *infoPtr=NULL;		
	
	#ifndef CW_SINGLE_THREAD
		infoPtr = CWThreadGetSpecific(&gLastError);
		if(infoPtr==NULL){
			//printf("%s:%d############\n",__FUNCTION__,__LINE__);
			memset(&capwap_error_info, 0, sizeof(CWErrorHandlingInfo));
			infoPtr=&capwap_error_info;
...
CWBool _CWErrorHandleLast(const char *fileName, int line) {
	...
	switch(infoPtr->code) {
		...
		case CW_ERROR_NONE:
			...
			__CW_ERROR_PRINT("babylon test _CWErrorHandleLast (when success)");
