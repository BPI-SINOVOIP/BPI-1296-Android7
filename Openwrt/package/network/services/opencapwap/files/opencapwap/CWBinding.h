...
// Babylon
static inline void CWBindingTransportHeaderValuesFree(CWBindingTransportHeaderValues *p)
{
	free(p);
}
static inline void CWBindingDataListElementFree(CWBindingDataListElement *p)
{
	if (p->frame) {
		CWProtocolMessageFree(p->frame);
	}
	if (p->bindingValues) {
		CWBindingTransportHeaderValuesFree(p->bindingValues	);
	}
}
