...
#define SMART_ROAMING_WTP_LOG_FILE_NAME		"/var/log/smart_roaming.wtp.log.txt"
#endif
#ifdef RTK_AUTO_AC
#define AAC_WTP_LOG_FILE_NAME		"/var/log/aac.wtp.log.txt"
CWBool breakACInfo;
#endif
...
extern int gMesh_portal;
extern int gHostapd_port;
extern char*  gHostapd_unix_path;
extern unsigned char gRadioBaseMac[CW_MAX_RADIOS_PER_WTP][6];
...
extern CWThread gThreadReceiveFrame;
...
#ifdef RTK_AUTO_AC
extern CWACInfoValues *gminACInfoPtr;
#endif
#ifdef RTK_SMART_ROAMING
extern unsigned int gPingSpeed;
#endif
...
extern CWThreadMutex gPendingRequestMutex;
extern CWPendingRequestMessage gPendingRequestMsgs[MAX_PENDING_REQUEST_MSGS];
extern unsigned char gRcvSeqNum[SEQUENCE_ARRAY_SIZE];

extern CWSafeList gPacketReceiveList;
extern CWThreadCondition gInterfaceWait;
extern CWThreadMutex gInterfaceMutex;

extern CWBool gGotoReset; 
extern CWBool gImageDataStarted;
extern CWBool gImageDataEof;
extern int gLastSeqNum;
		// handle in-sequence problem
   		// TODO: Babylon: Currently it is only used for image data request from AC. Should be general used in the future.
extern CWBool gWillReboot;
		
extern CWProtocolJoinResponseValues gJoinResponseVal;
...
#ifdef RTK_SMART_ROAMING
CW_THREAD_RETURN_TYPE CWWTPRtkReceiveFrame(void *);
#ifdef NEW_CHECKINTERNET
void CWWTPRtkCheckInternet();
#else
CW_THREAD_RETURN_TYPE CWWTPRtkCheckInternet(void *); 
#endif
...
CWBool CWSendPendingRequestMessage(CWProtocolMessage *messages, int fragmentsNum);

int CWFindPendingRequestMsgsBox(CWPendingRequestMessage *pendingRequestMsgs,
				const int length,
				const int msgType,
				const int seqNum);
void CWInitPendingMsgBox(CWPendingRequestMessage *pendingRequestMsgs);
