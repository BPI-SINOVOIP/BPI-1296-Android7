...
//	printf("~~~~~~ %s:%d before recvfrom\n",__FUNCTION__,__LINE__);	
	while((*readBytesPtr = recvfrom(sock, buf, len, flags, (struct sockaddr*)addrPtr, &addrLen)) < 0) {
		if (gBreakRcvLoop) {			
//			printf("~~~~~~ %s:%d because of gBreakRcvLoop, errno=%d\n",__FUNCTION__,__LINE__, errno);
			CWNetworkRaiseSystemError(CW_ERROR_CANCEL);
		} else if(errno == EAGAIN) {	
//			printf("~~~~~~ %s:%d errno=%d continue\n",__FUNCTION__,__LINE__, errno);
			continue;
		} else {	
//			printf("~~~~~~ %s:%d errno=%d Error and break loop\n",__FUNCTION__,__LINE__, errno);
			CWNetworkRaiseSystemError(CW_ERROR_RECEIVING); // this returns CW_FALSE
		}
	}
//	printf("~~~~~~ %s:%d leave recvfrom loop\n",__FUNCTION__,__LINE__);
	return CW_TRUE;
}
...
CWBool CWNetworkInitSocketClient(CWSocket *sockPtr, CWNetworkLev4Address *addrPtr) {
	...
	CWNetworkSetSocketNonblockRead(*sockPtr, 5);
	gBreakRcvLoop = CW_FALSE;
