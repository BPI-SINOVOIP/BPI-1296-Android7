...
CWBool CWAssembleMsgElemACDescriptor(CWProtocolMessage *msgPtr) {
	...
	CWProtocolStore16(msgPtr, CWACGetMaxStations()); // Maximum number of mobile stations supported	
	...
	for(i=0; i<infos.vendorInfosCount; i++) {
		...
		CWProtocolStoreRawBytes(msgPtr, (uint8_t *)infos.vendorInfos[i].valuePtr, (infos.vendorInfos)[i].length);
	}
...
CWBool CWAssembleMsgElemAddWLAN(CWProtocolMessage *msgPtr, const CWProtocolAddWLANValues *addWlan)
{
	...
	// create message
	CW_CREATE_PROTOCOL_MESSAGE(*msgPtr, sizeof(CWProtocolAddWLANValues), return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	
	CWProtocolStore8(msgPtr, addWlan->radio_id);
	CWProtocolStore8(msgPtr, addWlan->wlan_id);
	CWProtocolStore16(msgPtr, addWlan->capability.u16);
	CWProtocolStore8(msgPtr, addWlan->key_index);
	CWProtocolStore8(msgPtr, addWlan->key_status);
	CWProtocolStore16(msgPtr, addWlan->key_len);
	CWProtocolStoreRawBytes(msgPtr, addWlan->key, addWlan->key_len);
	CWProtocolStoreRawBytes(msgPtr, addWlan->group_TSC, CW_GRP_TSC_SIZE);
	CWProtocolStore8(msgPtr, addWlan->non_WMM_sta_qos);
	CWProtocolStore8(msgPtr, addWlan->auth_type);
	CWProtocolStore8(msgPtr, addWlan->mac_mode);
	CWProtocolStore8(msgPtr, addWlan->tunnel_mode);
	CWProtocolStore8(msgPtr, (addWlan->suppress_ssid==0)?0:1);
	CWProtocolStoreRawBytes(msgPtr, addWlan->ssid, strlen(addWlan->ssid)+1);
...
CWBool CWAssembleMsgElemImageInformation(CWProtocolMessage *msgPtr, uint32_t fileSize, const uint8_t *hash) 
{
	// create message
	CW_CREATE_PROTOCOL_MESSAGE(*msgPtr, 20, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	CWProtocolStore32(msgPtr, fileSize);
	CWProtocolStoreRawBytes(msgPtr, hash, 16);
	return CWAssembleMsgElem(msgPtr, CW_MSG_ELEMENT_IMAGE_INFO_CW_TYPE);
}

CWBool CWAssembleMsgElemImageData(CWProtocolMessage *msgPtr, void *data_start, int data_len, CWBool data_type_eof)
{
	CW_CREATE_PROTOCOL_MESSAGE(*msgPtr, data_len+1, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	if (data_type_eof) {
		CWProtocolStore8(msgPtr, CW_IMAGE_DATA_TYPE_EOF);
	} else {
		CWProtocolStore8(msgPtr, CW_IMAGE_DATA_TYPE_DATA);
	}
	CWProtocolStoreRawBytes(msgPtr, data_start, data_len);
	return CWAssembleMsgElem(msgPtr, CW_MSG_ELEMENT_IMAGE_DATA_CW_TYPE);
}

 /*_________________________________________________________________________*/
/*  *****************************___PARSE___*****************************  */
CWBool CWParseMsgElemACNameWithIndex (CWProtocolMessage *msgPtr, int len, CWACNameWithIndexValues *valPtr)
{
...
CWBool CWParseMsgElemDiscoveryType(CWProtocolMessage *msgPtr, int len, CWDiscoveryRequestValues *valPtr) {	
	CWParseMessageElementStart();
...
CWBool CWParseMsgElemLocationData(CWProtocolMessage *msgPtr, int len, char **valPtr) {	
	CWParseMessageElementStart();
...
CWBool CWParseMsgElemWTPStatisticsTimer (CWProtocolMessage *msgPtr, int len, int *valPtr)
{
	CWParseMessageElementStart();
...
CWBool CWParseMsgElemWTPBoardData (CWProtocolMessage *msgPtr, int len, CWWTPVendorInfos *valPtr)
{
...
CWBool CWParseMsgElemWTPDescriptor(CWProtocolMessage *msgPtr, int len, CWWTPDescriptor *valPtr) {
	int theOffset, i;
...
CWBool CWParseMsgElemWTPFrameTunnelMode(CWProtocolMessage *msgPtr, int len, CWframeTunnelMode *valPtr) {	
	CWParseMessageElementStart();
...
CWBool CWParseMsgElemWTPIPv4Address(CWProtocolMessage *msgPtr, int len, CWProtocolJoinRequestValues *valPtr) {
	CWParseMessageElementStart();
...
CWBool CWParseMsgElemWTPMACType(CWProtocolMessage *msgPtr, int len, CWMACType *valPtr) {	
	CWParseMessageElementStart();
...
CWBool CWParseMsgElemWTPSupportedRates(CWProtocolMessage *msgPtr, int len, unsigned char *valPtr) {	
...
CWBool CWParseMsgElemWTPMultiDomainCapability(CWProtocolMessage *msgPtr, int len, unsigned char *valPtr) {	
...
CWBool CWParseMsgElemWTPName(CWProtocolMessage *msgPtr, int len, char **valPtr) {	
	CWParseMessageElementStart();
...
CWBool CWParseMsgElemWTPRebootStatistics (CWProtocolMessage *msgPtr, int len, WTPRebootStatisticsInfo *valPtr)
{
	CWParseMessageElementStart();
...
CWBool CWParseMsgElemWTPRadioStatistics(CWProtocolMessage *msgPtr, int len, WTPRadioStatisticsValues *valPtr) 
{
	CWParseMessageElementStart();
...
CWBool CWParseMsgElemWTPOperationalStatistics(CWProtocolMessage *msgPtr, int len, WTPOperationalStatisticsValues *valPtr) 
{
	CWParseMessageElementStart();
...
CWBool CWParseMsgElemRtkSiteSurvey(CWProtocolMessage *msgPtr, int len, CWRtkSiteSurveyValues* valPtr) 
{
	...
	valPtr->radioID = CWProtocolRetrieve8(msgPtr);
	valPtr->numBss = CWProtocolRetrieve8(msgPtr);
	for (i=0; i<valPtr->numBss; i++) {
		CWProtocolCopyRawBytes(valPtr->bssInfo[i].bssid, msgPtr, 6);
		CWProtocolCopyRawBytes(valPtr->bssInfo[i].ssid, msgPtr, CW_MAX_SSID_SIZE);
		valPtr->bssInfo[i].bss_type = CWProtocolRetrieve8(msgPtr);
		valPtr->bssInfo[i].channel = CWProtocolRetrieve8(msgPtr);
		valPtr->bssInfo[i].signal_strength = CWProtocolRetrieve8(msgPtr);
		valPtr->bssInfo[i].radio_type = CWProtocolRetrieve8(msgPtr);
	}
	CWParseMessageElementEnd();
}
