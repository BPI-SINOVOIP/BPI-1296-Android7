...
// Babylon: should use init instead of reset when WTP entering run state.
...
CWBool CWUpdatePendingMsgBox(CWPendingRequestMessage *pendingRequestMsgs, 
	...
	if(pendingRequestMsgs->msgType != UNUSED_MSG_TYPE) return CW_FALSE;
	...
	pendingRequestMsgs->timer_usec = timer_usec;
	pendingRequestMsgs->timer_hdl = timer_hdl;
	pendingRequestMsgs->timer_arg = timer_arg;

	pendingRequestMsgs->timer = timer_add(0,
								pendingRequestMsgs->timer_usec,
								pendingRequestMsgs->timer_hdl,
								pendingRequestMsgs->timer_arg);
	if(pendingRequestMsgs->timer == -1) {
		CWResetPendingMsgBox(pendingRequestMsgs);
		return CW_FALSE;
	}
	//CWDebugLog("Pending Msg Box Updated");	
...
CWBool CWSendPendingRequestMessage(CWProtocolMessage *messages, int fragmentsNum) {
	...
	int i;
	if(messages == NULL || fragmentsNum <0)		return CW_FALSE;
