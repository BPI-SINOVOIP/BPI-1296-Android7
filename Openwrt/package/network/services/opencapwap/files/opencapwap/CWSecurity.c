...
#ifndef CW_DEBUGGING
			SSL_free(*sessionPtr); // Babylon add 20140528
...
CWBool CWSecuritySend(CWSecuritySession session, const unsigned char *buf, int len) {
...
CWBool CWSecurityInitContext(CWSecurityContext *ctxPtr,
	...
	if(caList != NULL) { 
		...
		/* pre-shared keys */
		CWLog("OpenSSL PrivateSharedKey not ready");
		exit(0);
		/*
...
CWBool CWSecurityVerifyCertEKU(X509 *x509, const char * const expected_oid) {
	...
	if(x509) // The X509 object must be explicitly freed
		X509_free(x509);
