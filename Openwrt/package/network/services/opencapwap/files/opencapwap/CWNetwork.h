...
#ifdef RTK_AUTO_AC
#define AACCWUseSockNtop(sa, block) 		{ 						\
							char __str[128];			\
							char *str; str = AACsock_ntop_r(((struct sockaddr*)(sa)), __str);\
							{block}					\
						}
#endif
...
#define		CWNetworkSetSocketNonblockRead(sock,sec) do \
	{\
			struct timeval timeout;   \
			timeout.tv_sec = sec;   \
			timeout.tv_usec = 0;   \
			setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, &timeout, (socklen_t)sizeof(timeout));  \
	} while(0)
	

#define		CWNetworkCloseSocket(x)	do{ if (x!=-1) { shutdown(SHUT_RDWR, x); close(x); x=-1; }  } while(0)

int CWNetworkGetAddressSize(CWNetworkLev4Address *addrPtr);
CWBool CWNetworkSendUnsafeConnected(CWSocket sock, const uint8_t *buf, int len);
CWBool CWNetworkSendUnsafeUnconnected(CWSocket sock, CWNetworkLev4Address *addrPtr, const uint8_t *buf, int len);
CWBool CWNetworkReceiveUnsafe(CWSocket sock, unsigned char *buf, int len, int flags, CWNetworkLev4Address *addrPtr, int *readBytesPtr);
