...
fs-types-$(CONFIG_TARGET_NAS_IMG) += nas
fs-types-$(CONFIG_TARGET_ROOTFS_INITRD) += initrd
...
# ref: scripts/gen_initramfs_list.sh
define Image/mkfs/initrd
	( cd $(TARGET_DIR); find . | cpio -o -H newc >$(KDIR)/root.cpio )
ifeq ($(CONFIG_TARGET_INITRAMFS_COMPRESSION_GZIP),y)
	gzip -n -9 -f $(KDIR)/root.cpio
endif
ifeq ($(CONFIG_TARGET_INITRAMFS_COMPRESSION_BZIP2),y)
	bzip2 -9 -f $(KDIR)/root.cpio
endif
ifeq ($(CONFIG_TARGET_INITRAMFS_COMPRESSION_LZMA),y)
	lzma e -d20 -lc1 -lp2 -pb2 -eos $(KDIR)/root.cpio $(KDIR)/root.cpio.lzma
endif
ifeq ($(CONFIG_TARGET_INITRAMFS_COMPRESSION_LZO),y)
	lzop -9 -f $(KDIR)/root.cpio
endif
ifeq ($(CONFIG_TARGET_INITRAMFS_COMPRESSION_LZ4),y)
	lz4 -l -9 -f $(KDIR)/root.cpio
endif
ifeq ($(CONFIG_TARGET_INITRAMFS_COMPRESSION_XZ),y)
	xz --check=crc32 --lzma2=dict=1MiB -f $(KDIR)/root.cpio
endif
...
define Image/Checksum
	( cd ${BIN_DIR} ; \
		$(FIND) -maxdepth 1 -type f \! -name 'md5sums' \! -name 'sha256sums' -printf "%P\n" | sort | xargs $1 > $2 \
	)
endef
...
define BuildImage/mkfs
  ...
  .PHONY: mkfs-$(1)
  mkfs-$(1): mkfs_prepare
	/sbin/depmod -a $(LINUX_UNAME_VERSION) -b $(TARGET_DIR)
	$(Image/mkfs/$(1))
