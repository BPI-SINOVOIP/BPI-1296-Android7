...
//memset(data_buf, 0x5a,page_size);
		...
		if ((REG_READ_U32(REG_BLANK_CHK+map_base) & 0x8)==0){
			...
			printk("[DBG][%s] page(0x%x) ecc error,  REG_BLANK_CHK reg: 0x%x\n", __FUNCTION__, page, REG_READ_U32(REG_BLANK_CHK+map_base));
			//up (&sem_NF_CARDREADER);
			if (g_enReadRetrial) {
				if (read_retry_cnt !=0) {
					switch(g_id_chain)
...
static int rtk_nand_profile (void)
{
	...
	if(g_sw_WP_level == -1)
	{
		printk(KERN_INFO "Disable software write protect!\n");
	}
	else
		printk(KERN_INFO "Enable software write protect!\n");

	if (rtk_nand_scan (rtk_mtd, maxchips) < 0 || rtk_mtd->size == 0){
		printk(KERN_ERR "%s: Error, cannot do nand_scan(on-board)\n", __FUNCTION__);
		return -ENODEV;
	}
	...
	if ( !(rtk_mtd->writesize&(0x200-1)) )
		...
		printk(KERN_ERR "Error: pagesize is not 512B Multiple");
		return -1;
	}
	...
	printk(KERN_ERR "[%s] mtd_device_parse_register rc = 0x%x\n", __FUNCTION__, rc);
#endif
...
static int rtk_nand_probe(struct platform_device *pdev)
{
    ...
    //printk(KERN_ERR "muxpad0_base:0x%08x\n", muxpad0_base);
    //REG_WRITE_U32(muxpad0_base, 0x55555555);
	...
	WAIT_DONE(REG_ND_CTL+map_base,0x80,0);
	printk(KERN_INFO "trigger xfer.\n");
	WAIT_DONE(REG_ND_CTL+map_base,0x40,0x40);
	printk(KERN_INFO "nand ready. \n");
	...
	printk(KERN_INFO "(X)Enable interrupt mechanism of NAND driver.\n");
	...
	if(g_BootcodeSize == -1)
	{
		if(is_darwin_cpu()||is_saturn_cpu()||is_macarthur_cpu()||is_nike_cpu()||is_macarthur2_cpu())
	...
	//nRun = nInc = 0;
	printk(KERN_INFO "Get parameter from register...\n");
	printk(KERN_INFO "[%s]Bootcode size 0x%x\n",__FUNCTION__,g_BootcodeSize);
	printk(KERN_INFO "[%s]Factory parameter size 0x%x\n",__FUNCTION__,g_Factory_param_size);

	{
		printk(KERN_INFO "[%s]Disable NFCP...\n", __FUNCTION__);
	...
	if ( !rtk_mtd ){
		printk(KERN_ERR "%s: Error, no enough memory for rtk_mtd\n",__FUNCTION__);
		rc = -ENOMEM;
		goto EXIT;
	}
...
printk("[%s]line: %d\n",__FUNCTION__,__LINE__);
	...
	WAIT_DONE(REG_ND_CTL+map_base,0x80,0);
	WAIT_DONE(REG_ND_CTL+map_base,0x40,0x40);
	REG_WRITE_U32( REG_MULTI_CHNL_MODE+map_base,0x00);
	REG_WRITE_U32( REG_TIME_PARA1+map_base,NF_TIME_PARA1_T1(0x1));
	REG_WRITE_U32( REG_TIME_PARA2+map_base,NF_TIME_PARA2_T2(0x1));
	REG_WRITE_U32( REG_TIME_PARA3+map_base,NF_TIME_PARA3_T3(0x1));
	if(!is_jupiter_cpu()||!is_saturn_cpu()||!is_darwin_cpu()||!is_macarthur_cpu()||is_nike_cpu()||is_macarthur2_cpu())
		REG_WRITE_U32( REG_DELAY_CTL+map_base,NF_DELAY_CTL_T_WHR_ADL(0x09) );
printk("[%s]line: %d\n",__FUNCTION__,__LINE__);
	REG_WRITE_U32( REG_MULTI_CHNL_MODE+map_base,NF_MULTI_CHNL_MODE_edo(0x00));
	REG_WRITE_U32( REG_ECC_STOP+map_base,NF_ECC_STOP_ecc_n_stop(0x01));
	...
	printk(KERN_INFO "[%s]Ecc bit select %u\n",__FUNCTION__,this->ecc_select);
