...
#define __regulator_set_voltage(_regulator, _vdd_min, _vdd_max) \
	if (_regulator) { \
...
static unsigned int cpu_regulator_latency = 40000;
	...
	unsigned int idx)
{
	...
	unsigned int delay = cpu_regulator_latency / 1000;


	if (freq_new > freq_old) {
		__regulator_set_voltage(cpu_regulator, volt_table[volt_idx].vdd_min,
			volt_table[volt_idx].vdd_max);
		__regulator_set_voltage(l2_regulator, l2_volt_table[volt_idx].vdd_min,
			l2_volt_table[volt_idx].vdd_max);
...
static int realtek_129x_cpufreq_driver_init(struct cpufreq_policy *policy)
{
	...
	/* enlarge scpu pll freq slop
	 *   default: 30 ~ 200 us
	 *   max:  10 ~ 25 us
	 */
	do {
		void __iomem * reg = ioremap(0x98000508, 0x4);
		writel(readl(reg) | (0x3ff << 17), reg);
		iounmap(reg);

		cpu_transition_latency -= 100000;
	} while(0);

	/* NOTE: transition_latency is used as the sampling rate in the ondemand
	 * governor, which defined as following
	 *
	 *  sampling_rate(us) = transition_latency(ns) / 1000 * LATENCY_MULTIPLIER
	 */
...
static int rtk_dvfs_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "use regulator(s)\n");
	...
	l2_regulator  = devm_regulator_get_optional(&pdev->dev, "l2");
	if (IS_ERR(l2_regulator)) {
		ret = PTR_ERR(l2_regulator);
		dev_info(&pdev->dev, "no L2 regulator: %d\n", ret);
		l2_regulator = NULL;
	}


#else /* !defined(CONFIG_REGULATOR) */
	dev_info(&pdev->dev, "no regulator\n");
	cpu_regulator = NULL;
	l2_regulator  = NULL;
#endif
	...
	prop_name = "regulator_latency";
	ret = of_property_read_u32(np, prop_name, &cpu_regulator_latency);
	if (ret)
		cpu_regulator_latency = 40000;

	/* only A00 use 'frequency-table' */
	if (rev != 0)
		rev = 1;
