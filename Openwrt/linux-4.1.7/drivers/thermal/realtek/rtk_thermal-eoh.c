...
{
	struct thermal_zone_device *tz = tdev->tz_dev;
...
static int rtk_thermal_eoh_cpufreq_cb(struct notifier_block *nb,
					 unsigned long event, void *data)
{
	struct cpufreq_policy *policy = data;
	struct rtk_thermal_priv *tdev = container_of(nb,
		struct rtk_thermal_priv, eoh_cpufreq_nb);
...
int rtk_thermal_add_eoh(struct rtk_thermal_priv *tdev)
{
	...
	tdev->eoh_cpufreq_nb.notifier_call = rtk_thermal_eoh_cpufreq_cb;
