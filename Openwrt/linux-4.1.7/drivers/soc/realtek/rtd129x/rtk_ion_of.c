...
#include "../../drivers/staging/android/ion/ion_rtk_carveout_heap.h"
...
enum ion_heap_ids {
    ...
    ION_HEAP_ID_RESERVED = 31	/** bit reserved for secure flag **/
};
...
#define COLUMN 3
            ...
            if (heap_data->id == RTK_PHOENIX_ION_HEAP_TYPE_MEDIA_ID) {
                ...
                dbg_info("%s %s, ION using CMA not supported on XEN\n", __FILE__, __func__);
#else
                if( cma_area_count == 1 ) {
                    struct ion_rtk_priv_pool * pool = kzalloc(sizeof(struct ion_rtk_priv_pool), GFP_KERNEL);
