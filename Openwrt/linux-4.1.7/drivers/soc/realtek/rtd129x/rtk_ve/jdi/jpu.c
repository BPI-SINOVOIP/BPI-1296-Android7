...
#endif
    printk("[JPUDRV] base:0x%08x, phy_addr:0x%08x, size:%d\n", (unsigned int)jb->base, (unsigned int)jb->phys_addr, jb->size);
    return 0;
}
...
static int jpu_probe(struct platform_device *pdev)
{
    ...
    printk("[JPUDRV] : jpu base address get from DTB physical base addr=0x%lx, virtual base=0x%lx, size=0x%x\n", (unsigned long)s_jpu_reg_phy_base, (unsigned long)s_jpu_reg_virt_base, (unsigned int)(res.end - res.start + 1));
    ...
    s_jpu_irq = irq;
    printk("s_jpu_irq:%d want to register jpu_irq_handler\n", s_jpu_irq);
    ...
    if (err != 0)
    {
        if(err == -EINVAL)
        {
            printk(KERN_ERR "[JPUDRV] : Bad s_jpu_irq number or handler\n");
        }
        else if(err == -EBUSY)
        {
            printk(KERN_ERR "[JPUDRV] : s_jpu_irq <%d> busy, change your config\n", s_jpu_irq);
        }
        goto ERROR_PROVE_DEVICE;
    }
#endif
...
static int jpu_suspend(struct platform_device *pdev, pm_message_t state)
{
    ...
    printk(KERN_INFO "[RTK_JPEG] Exit %s\n", __func__);
