    ...
    vpudrv_buffer_t __user *data)
{
    ...
    err |= get_user(m, &data32->mem_type);
    err |= put_user(m, &data->mem_type);
    ...
    vpudrv_buffer_t __user *data)
{
    ...
    err |= get_user(m, &data->mem_type);
    err |= put_user(m, &data32->mem_type);
...
long compat_vpu_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_VDI_IOCTL_SET_RTK_CLK_SELECT:
	{
		...
		return filp->f_op->unlocked_ioctl(filp, VDI_IOCTL_SET_RTK_CLK_SELECT, (unsigned long)data);
	}
	...
	case COMPAT_VDI_IOCTL_GET_RTK_CLK_SELECT:
	{
		...
		if (err)
			return err;
		ret = filp->f_op->unlocked_ioctl(filp, VDI_IOCTL_GET_RTK_CLK_SELECT, (unsigned long)data);
		err = compat_put_ve1_clock_info(data32, data);
		return ret ? ret : err;
	}
