...
static unsigned long vpu_clk_get_rate(struct clk *clk);
...
static long vpu_ioctl(struct file *filp, u_int cmd, u_long arg)
{
    ...
    case VDI_IOCTL_GET_RTK_CLK_PLL:
    {
        ...
        if (ret != 0)
            break;
        DPRINTK("[VPUDRV][-]VDI_IOCTL_GET_RTK_CLK_PLL clockInfo.core_idx:%d, clockInfo.value:%d\n", clockInfo.core_idx, clockInfo.value);
    }
    break;
	case VDI_IOCTL_SET_RTK_CLK_SELECT:
	{
		...
		DPRINTK("[VPUDRV][+]VDI_IOCTL_SET_RTK_CLK_SELECT\n");
		ret = copy_from_user(&clockInfo, (vpu_clock_info_t *)arg, sizeof(vpu_clock_info_t));
		if (ret != 0)
			break;
	...
	case VDI_IOCTL_GET_RTK_CLK_SELECT:
	{
		...
		struct clk *s_vpu_clk;
		struct clk *s_pclk;
		DPRINTK("[VPUDRV][+]VDI_IOCTL_GET_RTK_CLK_SELECT\n");
		ret = copy_from_user(&clockInfo, (vpu_clock_info_t *)arg, sizeof(vpu_clock_info_t));
		if (ret != 0)
			break;
		...
		if (ret != 0)
			break;
		DPRINTK("[VPUDRV][-]VDI_IOCTL_GET_RTK_CLK_SELECT clockInfo.core_idx:%d, clockInfo.value:%d\n", clockInfo.core_idx, clockInfo.value);
	}
	break;
...
struct clk *vpu_clk_get(struct device *dev, u32 coreIdx)
{
	...
	if (IS_ERR_OR_NULL(clk)) {
		dev_warn(dev, "[VPUDRV] failed to get %s\n", clk_name);
		clk = NULL;
	}
	return clk;
}
...
static void vpu_clk_disable(struct clk *clk)
{
	int i = __clk_get_enable_count(clk);
