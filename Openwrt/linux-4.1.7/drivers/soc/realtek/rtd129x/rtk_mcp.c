...
static int mcp_probe(struct platform_device *pdev)
{
	...
	reset_control_assert(reset_cp);

	clk_disable_unprepare(clk_cp);

	reset_control_deassert(reset_cp);

	clk_prepare_enable(clk_cp);

	printk(KERN_INFO "[RTK MCP] MCP driver initial done. cp clk prepare for resume!!\n");
...
static int mcp_resume(struct platform_device *dev)
{
	...
	printk(KERN_INFO "[RTK MCP] Resume Enter. Reset CP Module\n");
