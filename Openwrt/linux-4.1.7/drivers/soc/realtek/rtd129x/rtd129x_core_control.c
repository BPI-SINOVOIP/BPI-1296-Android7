...
static int do_hotplug(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (n_on > n_off) {
			...
			pr_debug("cpus_to_online: %*pbl\n", cpumask_pr_args(&cpus_to_online));
			pr_debug("cpu = %d\n", cpu);
			...
			cpu = cpumask_last(&cpus_to_offline);
			pr_debug("cpus_to_offline: %*pbl\n", cpumask_pr_args(&cpus_to_offline));
			pr_debug("cpu = %d\n", cpu);
		...
		sysfs_notify(&pdev->dev.kobj, NULL, "online_cpus");
	}
...
static int rtk_core_control_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "CPU List = [ %*pbl ]\n", cpumask_pr_args(&available_cpus));
	if (ret)
		goto error;

	init_completion(&hotplug_notify_complete);
	hotplug_task = kthread_create(do_hotplug, pdev, "hotplug");
	if (IS_ERR(hotplug_task)) {
		dev_err(&pdev->dev, "task hotplug not init\n");
	...
	if (of_property_read_u32(pdev->dev.of_node, "enable", &core_control_enabled))
		core_control_enabled = 0;

	/* add sysfs */
	ret = sysfs_create_group(&pdev->dev.kobj, &core_control_attr_group);
	if (ret) {
		dev_err(&pdev->dev, "cannot create sysfs group. err:%d\n", ret);
		goto error;
	}
