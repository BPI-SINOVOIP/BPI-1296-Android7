...
//#define rtk_rpc_wmb(start, size)           do { smp_mb(); __flush_dcache_area(start, size); writel(0x1234, rpc_rbussync_base); smp_mb(); } while (0)
...
#define AVCPU_SCPU_OFFSET   (AVCPU_NOCACHE + RPC_RINGBUF_PHYS - (unsigned long)rpc_ringbuf_base)
...
typedef struct RPC_DEV {
    ...
    volatile uint32_t        ringIn;    /* pointer to where next data will be inserted  in   the ring buffer */
    volatile uint32_t        ringOut;   /* pointer to where next data will be extracted from the ring buffer */
...
static inline int __get_ring_data_size(uint32_t start, uint32_t end, uint32_t in, uint32_t out)
{
...
static inline uint32_t get_ring_data(const char *func, RPC_DEV *dev, uint32_t out, char *buf, int datasize)
{
...
#ifdef CONFIG_REALTEK_RPC_PROGRAM_REGISTER
static inline RPC_PROCESS *pick_supported_proc(RPC_DEV_EXTRA *extra, uint32_t programID)
{
    RPC_PROCESS *proc;
    RPC_HANDLER *handler;
...
static inline RPC_PROCESS *lookup_by_taskID(RPC_DEV_EXTRA *extra, uint32_t taskID)
{
    pid_t pid;
    RPC_PROCESS *proc = NULL;
...
static inline int update_thread_list(RPC_DEV_EXTRA *extra, pid_t pid)
{
    ...
    if(proc == NULL) //no found, should not happen
        return 0;
