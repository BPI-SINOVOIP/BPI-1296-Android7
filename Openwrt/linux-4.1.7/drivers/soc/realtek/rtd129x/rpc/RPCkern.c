    ...
    for (i = 0; i < num; i++) {
        ...
        if (i%RPC_NR_PAIR == 1) {
            if (rpc_kthread[i/RPC_NR_PAIR] == NULL)
                rpc_kthread[i/RPC_NR_PAIR] = kthread_run(rpc_kernel_thread, (void *)i, "rpc-%lu", i);
        }
    }
    if (!is_init) {
...
//      pr_info(" #@# wait %s %x %x \n", current->comm, dev, dev->waitQueue);
        if (wait_event_interruptible(dev->ptrSync->waitQueue, dev->ringIn != dev->ringOut || kthread_should_stop())) {
...
//      pr_info(" #@# wakeup %s \n", current->comm);
        ...
        // read the reply data...
        if (rpc_kern_read(opt, readbuf, sizeof(RPC_STRUCT)) != sizeof(RPC_STRUCT)) {
        ...
        if (rpc->taskID) {
            ...
            // read the payload...
            if (rpc_kern_read(opt, readbuf + sizeof(RPC_STRUCT), 3*sizeof(uint32_t))
                    != 3*sizeof(uint32_t)) {
            ...
            *(tmp+1) = htonl(ret);
            ...
            if (rpc_kern_write(opt, replybuf, sizeof(replybuf)) != sizeof(replybuf)) {
                pr_err("ERROR in send kernel RPC...\n");
                return RPC_FAIL;
            }
        } else {
            // read the payload...
            if (rpc_kern_read(opt, readbuf+sizeof(RPC_STRUCT), 2*sizeof(uint32_t))
                    != 2*sizeof(uint32_t)) {
                pr_err("ERROR in read payload...\n");
                continue;
            }
...
#if 0
            *((uint32_t *)ntohl(rpc->mycontext)) = ntohl(*(tmp+1)); //FIXME: mycontext should be 64bit
#else
...
#if 0
            //wake_up((wait_queue_head_t *)ntohl(*tmp)); // ack the sync... //FIXME: wait_queue_head_t * should be 64bit
#else
...
int send_rpc_command(int opt, uint32_t command, uint32_t param1, uint32_t param2, uint32_t *retvalue)
{
    ...
    rpc->procedureID = 0;
#if 0
    rpc->taskID = htonl((uint32_t)&rpc_wq[opt]); //using taskID to store wait queue address //FIXME: should be 64bit
#else
//    rpc->taskID = 0;
...
#if 0
    rpc->mycontext = htonl((uint32_t)retvalue); //FIXME: mycontext should be 64bit
#else
    rpc->mycontext = 0;
