...
/*2007-12-19*/
	...
	entry.enHWRoute   = intfp->enableRoute;
#else //defined(CONFIG_RTD_1295_HWNAT)
...
#if defined(CONFIG_RTD_1295_HWNAT)
// Joey_added, 03-09-2017
int32 rtl8651_clearAsicNaptTable(void)
{
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
	REG32(MEMCR) &= ~(1<<1);
	REG32(MEMCR) |= (1<<1); // L4

	// wait for L4 table clear done.
	while ( (REG32(MEMCR) & (1<<9)) != (1<<9) ) {} ;
#else
...
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
#if !defined(CONFIG_RTD_1295_HWNAT)
	/* for RTD1295, we move full reset to rtd129x_hwnat_clk_init() */
	REG32(SIRR) |= FULL_RST;
	mdelay(300);
