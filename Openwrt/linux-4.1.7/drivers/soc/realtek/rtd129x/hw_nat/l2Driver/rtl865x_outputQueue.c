...
#if defined(CONFIG_RTD_1295_HWNAT)
extern rtl865x_netif_local_t *_rtl865x_getNetifByIdx(unsigned int i);
extern int rtl_getPortlistByDevName(char *name, uint32 *portlist);

static int rtl865x_setIgressBandwidthByName(uint8 *netIfName,unsigned int kbps)
{
	rtl865x_netif_local_t	*netIf;
	uint32	memberPort;
	rtl865x_vlan_entry_t	*vlan;
	...
	if (rtl_getPortlistByDevName(netIfName, &memberPort) == FAILED) {
		netIf = _rtl865x_getNetifByName(netIfName);
		if(netIf == NULL)
			return FAILED;
		vlan = _rtl8651_getVlanTableEntry(netIf->vid);
	...
	for(port=0; port < CPU; port++)
	{
		if(((1 << port) & memberPort) == 0)
			continue;
		rtl8651_setAsicPortIngressBandwidth(port, kbps);
	}

	return SUCCESS;
}

static int rtl865x_setIgressBandwidthByPort(unsigned int port,unsigned int kbps)
{
	if(port > CPU)
		return FAILED;
...
static int rtl865x_resetRemark(void)
{
	int i, j;
...
static int32 rtl_hw_qos_config_write( struct file *filp, const char *buff,unsigned long len, void *data )
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		...
		#if defined(CONFIG_RTD_1295_HWNAT)
		/*echo "police netif eth0 50000000" > /proc/rtl_hw_qos_config*/
		/*echo "police port 5 50000000" > /proc/rtl_hw_qos_config*/
		/*echo "police reset" > /proc/rtl_hw_qos_config*/
		else if (strncmp(cmdptr, "police",6) == 0) {
			int port = 0;
			int kbps = 0;
			...
			if (strncmp(cmdptr, "netif",5) == 0)
			{
				...
				if (cmdptr == NULL)
					goto errout;
				kbps = simple_strtol(cmdptr, NULL, 0);
				rtl865x_setIgressBandwidthByName(name, kbps);
			}
			else if (strncmp(cmdptr, "port",4) == 0)
			{
				...
				if (cmdptr == NULL)
					goto errout;
				kbps = simple_strtol(cmdptr, NULL, 0);
				rtl865x_setIgressBandwidthByPort(port, kbps);
			}
			else if (strncmp(cmdptr, "reset",5) == 0)
			{
				for (port = 0; port < CPU; port++)
