...
void rtl865x_dumpAllEventLayerListInfo(void)
{
	...
	CTAILQ_FOREACH(eventLayerList, &eventsTables.inuseList.eventLayerListHead, next)
	{
		...
		CTAILQ_FOREACH(event, &eventLayerList->eventHead, next)
		{
			...
			rtl865x_debug_printf(" \tNo.%d event:eventId:%d,priority:%d,action_fn is 0x%lx\n",eventCnt,event->eventId, event->eventPriority,(uintptr_t)(event->event_action_fn));
...
int32 rtl865x_event_proc_read(struct seq_file *s, void *v)
{
	...
	CTAILQ_FOREACH(eventLayerList, &eventsTables.inuseList.eventLayerListHead, next)
	{
		...
		CTAILQ_FOREACH(event, &eventLayerList->eventHead, next)
		{
			eventCnt++;
			#if defined(CONFIG_RTD_1295_HWNAT)
			seq_printf(s, " \tNo.%d event:eventId:0x%x,priority:%d,action_fn is 0x%lx\n",eventCnt,event->eventId, event->eventPriority,(uintptr_t)(event->event_action_fn));
			#else /* defined(CONFIG_RTD_1295_HWNAT) */
