...
extern uint8 hwnat_rgmii_tx_delay;	/* 0 to 7: 0.5ns per step */
extern uint8 hwnat_rgmii_rx_delay;	/* 0 to 7: 0.5ns per step */
extern uint8 hwnat_led_mode;		/* 0~3: SCAN_0~3, 4~6: DIRECT 0~2 */
static uint8 hwnat_gpio_link_led_enable;	/* 0:disable, 1:enable */
...
static unsigned int pvid_per_port[RTL8651_PORT_NUMBER + 3] =
	{[0 ... RTL8651_PORT_NUMBER + 2] = RTL_LANVLANID };
static struct lan_dev_bind_mask_drv
	lan_dev_bind_mask_mapping_drv[LAN_DEV_NUM_MAX] = {
...
static void rtmsg_ifinfo_voip(int type, struct net_device *dev,
	unsigned change);
#endif
...
#include <net/netlink.h>
#include <linux/rtnetlink.h>
static int ethernet_rtnl_fill_ifinfo(struct sk_buff *skb,
	struct net_device *dev, int type, u32 pid, u32 seq, u32 change,
	unsigned int flags, unsigned int status);
static void ethernet_rtmsg_ifinfo(int type, struct net_device *dev,
	unsigned change, unsigned int status);
...
static int32 rtl_check_swCore_tx_hang_interval = 10;
...
__MIPS16 __IRAM_FWD static struct sk_buff *dev_alloc_skb_priv_eth(unsigned int
	size);
...
extern int get_dev_ip_mask(const char *name, unsigned int *ip,
	unsigned int *mask);
static inline int rtl865x_decideUcDevice(unsigned char *data, int pid,
	rtl_nicRx_info *info);
static inline int rtl865x_decideMcDevice(int pid, rtl_nicRx_info *info);
static inline int rtl865x_decideBcDevice(struct sk_buff *skb, int pid,
	rtl_nicRx_info *info);
...
extern int rtk_port_phyStatus_get(uint32 port, uint32 *pLinkStatus,
	uint32 *pSpeed, uint32 *pDuplex);
#endif
...
#endif
static void rtk_fill_vlan_txinfo(struct sk_buff *skb, uint16 *vid,
	uint16 *member);
...
static char *multicast_filter_limit =
...
#else
static int tr181_eth_link_read(char *page, char **start, off_t off, int count,
	int *eof, void *data);
static int tr181_eth_if_read(char *page, char **start, off_t off, int count,
	int *eof, void *data);
static int tr181_eth_stats_read(char *page, char **start, off_t off, int count,
	int *eof, void *data);
static int tr181_eth_set_read(char *page, char **start, off_t off, int count,
	int *eof, void *data);
#endif
static int tr181_eth_link_write(struct file *file, const char *buffer,
	unsigned long count, void *data);
static int tr181_eth_if_write(struct file *file, const char *buffer,
	unsigned long count, void *data);
static int tr181_eth_stats_write(struct file *file, const char *buffer,
	unsigned long count, void *data);
static int tr181_eth_set_write(struct file *file, const char *buffer,
	unsigned long count, void *data);
#endif
...
#else
static int inband_ctl_acl_read(char *page, char **start, off_t off, int count,
	int *eof, void *data);
#endif
static int inband_ctl_acl_write(struct file *file, const char *buffer,
	unsigned long count, void *data);
...
__IRAM_GEN static inline void rtl_rx_interrupt_process(unsigned int status,
	struct dev_priv *cp);
__IRAM_GEN static inline void rtl_tx_interrupt_process(unsigned int status,
	struct dev_priv *cp);
...
__IRAM_GEN static inline void rtl_link_change_interrupt_process(unsigned int
	status, struct dev_priv *cp);
#endif
...
int get_P2P_local_ip(unsigned char *ifname, unsigned int *ipAddr)
{
	...
	if (dslite_dev != NULL) {
		for (ifap = dslite_dev->ifa_list; ifap != NULL;
			ifap = ifap->ifa_next) {
...
#else
static int32 rtk_link_event_read(char *page, char **start, off_t off, int count,
	int *eof, void *data);
static int32 rtk_link_status_read(char *page, char **start, off_t off,
	int count, int *eof, void *data);
#endif
static int32 rtk_link_event_write(struct file *filp, const char *buff,
	unsigned long len, void *data);
static int32 rtk_link_status_write(struct file *filp, const char *buff,
	unsigned long len, void *data);
#endif
...
static inline int rtk_isHwlookup_vlan(uint16 vid, struct sk_buff *skb,
	uint32 *portlist);
...
#else
static int rtk_process_11q_rx(int port, struct sk_buff *skb,
	struct sk_buff **pskb1, rtl_nicRx_info *info);
#endif
static inline int32 rtl_isWanPort(struct dev_priv *cp, uint16 vid,
	struct sk_buff *skb, int pid, int vid_type);
#if defined(CONFIG_RTL_HARDWARE_MULTICAST)
...
extern int rtl_vlan_RTL8367R_set(unsigned short vid, unsigned int tagmask,
	unsigned int mask, unsigned int fid);
#endif
...
static inline void rtk_insert_vlan_tag(uint16 vid, struct sk_buff *skb)
{
	if (skb_headroom(skb) < VLAN_HLEN && skb_cow(skb, VLAN_HLEN) != 0) {
		panic_printk("%s-%d: error! (skb_headroom(skb) == %d < 4). Enlarge it!\n",
			__func__, __LINE__, skb_headroom(skb));
		while (1);
	}

	memmove(skb->data - VLAN_HLEN, skb->data, ETH_ALEN << 1);
	skb_push(skb, VLAN_HLEN);
	*((uint16 *)(skb->data + (ETH_ALEN << 1))) =
		__constant_htons(ETH_P_8021Q);
	*((uint16 *)(skb->data + (ETH_ALEN << 1) + 2)) = htons(vid);
}
...
#else
static int32 rtk_hwL3v6_read(char *page, char **start, off_t off, int count,
	int *eof, void *data);
#endif
static int32 rtk_hwL3v6_write(struct file *filp, const char *buff,
	unsigned long len, void *data);
#endif
...
static int32 jumbo_frame_support_write(struct file *filp, const char *buff,
	unsigned long len, void *data);
...
static ssize_t jumbo_frame_single_write(struct file *file,
...
#else
static int32 jumbo_frame_support_read(char *page, char **start, off_t off,
	int count, int *eof, void *data);
static int32 jumbo_frame_support_write(struct file *filp, const char *buff,
	unsigned long len, void *data);
#endif
...
#else
static int read_proc_vlan(char *page, char **start, off_t off, int count,
	int *eof, void *data);
static int32 rtk_vlan_support_read(char *page, char **start, off_t off,
	int count, int *eof, void *data);
#endif
static int write_proc_vlan(struct file *file, const char *buffer,
	unsigned long count, void *data);
static int32 rtk_vlan_support_write(struct file *filp, const char *buff,
	unsigned long len, void *data);
#if defined(CONFIG_RTK_BRIDGE_VLAN_SUPPORT)
#ifdef CONFIG_RTL_PROC_NEW
...
#else
static int32 rtk_vlan_wan_tag_support_read(char *page, char **start, off_t off,
	int count, int *eof, void *data);
#endif
static int32 rtk_vlan_wan_tag_support_write(struct file *filp, const char *buff,
	unsigned long len, void *data);
#endif
...
static int32 link_aggregation_write(struct file *filp, const char *buff,
	unsigned long len, void *data);
...
static ssize_t link_aggregation_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
#else
static int32 link_aggregation_read(char *page, char **start, off_t off,
	int count, int *eof, void *data);
static int32 link_aggregation_write(struct file *filp, const char *buff,
	unsigned long len, void *data);
#endif
...
static int32 rtl_phy_status_read(char *page, char **start, off_t off, int count,
	int *eof, void *data);
static int32 rtl_phy_status_write(struct file *filp, const char *buff,
	unsigned long len, void *data);
static int32 port_mibStats_read_proc(char *page, char **start, off_t off,
	int count, int *eof, void *data);
static int32 port_mibStats_write_proc(struct file *filp, const char *buff,
	unsigned long len, void *data);
...
#else
static int32 rtl_hw_vlan_support_read(char *page, char **start, off_t off,
	int count, int *eof, void *data);
#endif
static int32 rtl_hw_vlan_support_write(struct file *filp, const char *buff,
	unsigned long len, void *data);
#if defined(CONFIG_RTL_PROC_NEW)
static int rtl_hw_vlan_tagged_bridge_multicast_read(struct seq_file *s,
	void *v);
#else
static int32 rtl_hw_vlan_tagged_bridge_multicast_read(char *page, char **start,
	off_t off, int count, int *eof, void *data);
#endif
static int32 rtl_hw_vlan_tagged_bridge_multicast_write(struct file *filp,
...
extern int rtk_vlan_get(uint32 vid, uint32 *pMbrmsk, uint32 *pUntagmsk,
	uint32 *pFid);
extern int rtl865x_enableRtl8367ToCpuAcl(void);
#endif
#endif
...
#else
int32 rtl_8367r_vlan_read(char *page, char **start, off_t off, int count,
	int *eof, void *data);
#endif
#endif
...
static void rtl_update_lan_cp_vinfo(struct dev_priv *cp, int enable, int vid,
	int fw, int wan_tag)
{
...
static void rtl_update_wan_cp_vinfo(struct dev_priv *cp, int enable, int vid,
	int wan_tag)
{
	cp->vlan_setting.global_vlan = 1;
	cp->vlan_setting.vlan = enable;
...
static void rtl_update_manage_vlan(int enable, int vid, int wan_tag)
{
	...
	if (manage_vlan_dev != NULL) {
		...
		rtl_update_wan_cp_vinfo((struct dev_priv *)
			NETDRV_PRIV(manage_vlan_dev), enable, vid, wan_tag);
		/* update cp vid */
...
static inline int is_ARP_packet(struct sk_buff *skb)
{
	uint8 *ptr;
...
static inline int is_DHCP_packet(struct sk_buff *skb)
{
	struct iphdr *iph;
	struct udphdr *udph;
...
#endif
			return ((struct dev_priv *)
				NETDRV_PRIV(manage_vlan_dev));
		} else
#endif
#if defined(CONFIG_RTD_1295_HWNAT)
...
{
	int32 totalVlans =
		((sizeof(vlanconfig)) / (sizeof(struct rtl865x_vlanConfig))) -
		1;
	int i, vlan_num = 0;
...
#else
static int32 rtk_multicast_scream_vid_read(char *page, char **start, off_t off,
	int count, int *eof, void *data)
{
	int len;
...
#endif
static int rtk_query_for_bridge_port_write(struct file *file,
...
static int32 rtk_query_for_bridge_port_read(struct seq_file *s, void *v)
{
	...
	seq_printf(s, "%s %d\n", "query for bridge port:",
		RTK_QUERYFORBRIDGEPORT);

	return 0;
}
#else
static int32 rtk_query_for_bridge_port_read(char *page, char **start, off_t off,
	int count, int *eof, void *data)
{
	int len;
	len = sprintf(page, "%s %d\n", "query for bridge port:",
...
#endif
int checkMulticatTxInfo(struct sk_buff *skb, struct net_device *dev,
...
void rtk_8367_vlan_mapping(void)
{
	...
	for (i = 0; i < totalVlans; i++) {
		...
		if (dp->id != 0) {
			...
			if ((vlan.memberPortMask & vlan.
					untagPortMask & RTL_WANPORT_MASK) ==
				RTL_WANPORT_MASK)
				vlan.untagPortMask =
					(vlan.
					untagPortMask & 0x3f) |
					BIT(r8367_cpu_port);
			else if ((vlan.memberPortMask & vlan.
					untagPortMask & RTL_CPUPORT_MASK) ==
				RTL_CPUPORT_MASK)
				vlan.untagPortMask =
					(vlan.
					untagPortMask & 0x3f) |
				...
				vlan.untagPortMask =
					(vlan.untagPortMask & 0x3f);
			/* exclude 819x cpu port , and always add 8367 cpu port */
			vlan.memberPortMask =
				(vlan.
				memberPortMask & 0x3f) | BIT(r8367_cpu_port);
			/* mark_mcdebug */
			...
			retVal = rtk_vlan_set(vlan.vid, vlan.memberPortMask,
				vlan.untagPortMask, vlan.fid);
		}
...
#endif
#if defined(CONFIG_RTL_LOCAL_PUBLIC)
static int32 rtl865x_getPortlistByMac(const unsigned char *mac,
	uint32 *portlist);
#endif
	...
	if (delta == 0) {
		if (cb->LED_Interval == GPIO_LED_NOBLINK_TIME)
			...
			if (cb->blink_once_done == 1) {
				mod_timer(&(cb->LED_Timer),
					jiffies + cb->LED_Interval);
				cb->blink_once_done = 0;
			}
		}
	} else {
...
void update_mib_counter(int port)
{
	...
	if ((regVal & PortStatusLinkUp) != 0) {
		...
		rtl819x_getAsicMibCounter(port, MIB_TX_PKT_CNT,
			(uint32 *)&cb->LED_tx_cnt);
		rtl819x_getAsicMibCounter(port, MIB_RX_PKT_CNT,
			(uint32 *)&cb->LED_rx_cnt);
	} else {
		/* link down */
		if (cb->link_status & GPIO_LINK_STATUS) {
			cb->link_status = GPIO_LINK_STATE_CHANGE;
...
void rtl_setPortMask(uint32 reg_data)
{
	if ((reg_data != 0) && (reg_data != 0xFFFF)) {
		...
		REG32(MACCR) =
			(REG32(MACCR) & ~CF_FCDSC_MASK) | (0x20 <<
			CF_FCDSC_OFFSET);
	} else {
		/*8211ds is not found */
		lanPortMask = 0x10f;
...
void rtl_ps_drv_netif_mapping_show(void)
{
	...
	for (i = 0; i < NETIF_NUMBER; i++) {
		DEBUG_ERR
			...
			ps_drv_netif_mapping[i].
			ps_netif ? ps_drv_netif_mapping[i].ps_netif->
			name : NULL, ps_drv_netif_mapping[i].drvName);
	}
}
#endif
...
{
	memset(ps_drv_netif_mapping, 0,
		NETIF_NUMBER * sizeof(ps_drv_netif_mapping_t));
	return SUCCESS;
}

int rtl_get_ps_drv_netif_mapping_by_psdev_name(const char *psname,
	char *netifName)
{
	int i;
	...
	for (i = 0; i < NETIF_NUMBER; i++) {
		if (ps_drv_netif_mapping[i].valid == 1
			&& memcmp(ps_drv_netif_mapping[i].ps_netif->name,
				psname, strlen(psname)) == 0) {
			memcpy(netifName, ps_drv_netif_mapping[i].drvName,
				MAX_IFNAMESIZE);
			return SUCCESS;
		}
	}
	...
	if (memcmp(psname, RTL_PS_BR0_DEV_NAME,
			strlen(RTL_PS_BR0_DEV_NAME)) == 0) {
		for (i = 0; i < NETIF_NUMBER; i++) {
			if (ps_drv_netif_mapping[i].valid == 1 &&
				memcmp(ps_drv_netif_mapping[i].drvName,
					RTL_DRV_LAN_NETIF_NAME,
					strlen(RTL_DRV_LAN_NETIF_NAME)) == 0) {
				memcpy(netifName,
					ps_drv_netif_mapping[i].drvName,
					MAX_IFNAMESIZE);
				return SUCCESS;
			}
		}
...
ps_drv_netif_mapping_t *rtl_get_ps_drv_netif_mapping_by_psdev(struct net_device
	*dev)
{
	int i;
	for (i = 0; i < NETIF_NUMBER; i++) {
	...
	/* back compatible,user use br0 to get lan netif */
	if (memcmp(dev->name, RTL_PS_BR0_DEV_NAME,
			strlen(RTL_PS_BR0_DEV_NAME)) == 0) {
		for (i = 0; i < NETIF_NUMBER; i++) {
			if (ps_drv_netif_mapping[i].valid == 1 &&
				memcmp(ps_drv_netif_mapping[i].drvName,
...
int rtl_del_ps_drv_netif_mapping_by_name(const char *name)
{
	...
	for (i = 0; i < NETIF_NUMBER; i++) {
		if ((ps_drv_netif_mapping[i].valid == 1)
			&& (strncmp(ps_drv_netif_mapping[i].drvName, name,
					MAX_IFNAMESIZE) == 0)) {
...
#if defined(CONFIG_RTD_1295_HWNAT)
EXPORT_SYMBOL(rtl_del_ps_drv_netif_mapping_by_name);
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
#endif
...
#if defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_8370_SUPPORT) || defined(CONFIG_RTL_8198C_8367RB)
rtk_stat_port_cntr_t port_cntrs;
#endif

#if defined(CONFIG_RTL_INBAND_CTL_API)
struct port_stats port_stats[6];
int rtl_get_portRate(unsigned int port, unsigned long *rxRate,
...
void re865x_accumulate_port_stats(uint32 portnum,
	struct net_device_stats *net_stats)
{
	...
	net_stats->rx_packets +=
		rtl8651_returnAsicCounter(OFFSET_IFINUCASTPKTS_P0 +
		addrOffset_fromP0);
	net_stats->rx_packets +=
		rtl8651_returnAsicCounter(OFFSET_ETHERSTATSMULTICASTPKTS_P0 +
		addrOffset_fromP0);
	net_stats->rx_packets +=
		rtl8651_returnAsicCounter(OFFSET_ETHERSTATSBROADCASTPKTS_P0 +
		addrOffset_fromP0);
	/* tx_pkt = tx_unicast +tx_multicast + tx_broadcast */
	net_stats->tx_packets +=
		rtl8651_returnAsicCounter(OFFSET_IFOUTUCASTPKTS_P0 +
		addrOffset_fromP0);
	net_stats->tx_packets +=
		rtl8651_returnAsicCounter(OFFSET_IFOUTMULTICASTPKTS_P0 +
		addrOffset_fromP0);
	net_stats->tx_packets +=
		rtl8651_returnAsicCounter(OFFSET_IFOUTBROADCASTPKTS_P0 +
		addrOffset_fromP0);

	net_stats->rx_bytes +=
		rtl8651_returnAsicCounter(OFFSET_IFINOCTETS_P0 +
		addrOffset_fromP0);
	net_stats->rx_bytes +=
		(rtl8651_returnAsicCounter(OFFSET_IFINOCTETS_P0 + 4 +
			addrOffset_fromP0) << 22);
	net_stats->tx_bytes +=
		rtl8651_returnAsicCounter(OFFSET_IFOUTOCTETS_P0 +
		addrOffset_fromP0);
	net_stats->tx_bytes +=
		(rtl8651_returnAsicCounter(OFFSET_IFOUTOCTETS_P0 + 4 +
			addrOffset_fromP0) << 22);
#if defined(CONFIG_RTL_INBAND_CTL_API)
	port_stats[portnum].rx_bytes_current =
		rtl8651_returnAsicCounter(OFFSET_IFINOCTETS_P0 +
		addrOffset_fromP0);
	port_stats[portnum].rx_bytes_current +=
		(rtl8651_returnAsicCounter(OFFSET_IFINOCTETS_P0 + 4 +
			addrOffset_fromP0) << 22);
	port_stats[portnum].tx_bytes_current =
		rtl8651_returnAsicCounter(OFFSET_IFOUTOCTETS_P0 +
		addrOffset_fromP0);
	port_stats[portnum].tx_bytes_current +=
		(rtl8651_returnAsicCounter(OFFSET_IFOUTOCTETS_P0 + 4 +
	...
	/*rx_errors = CRC error + Jabber error  + Fragment error */
	net_stats->rx_errors +=
		rtl8651_returnAsicCounter(OFFSET_DOT3STATSFCSERRORS_P0 +
		addrOffset_fromP0);
	net_stats->rx_errors +=
		rtl8651_returnAsicCounter(OFFSET_ETHERSTATSJABBERS_P0 +
		addrOffset_fromP0);
	net_stats->rx_errors +=
	...
	net_stats->tx_dropped +=
		rtl8651_returnAsicCounter(OFFSET_IFOUTDISCARDS +
		addrOffset_fromP0);
	net_stats->multicast +=
		rtl8651_returnAsicCounter(OFFSET_ETHERSTATSMULTICASTPKTS_P0 +
		addrOffset_fromP0);
	...
	net_stats->collisions +=
		rtl8651_returnAsicCounter(OFFSET_ETHERSTATSCOLLISIONS_P0 +
		addrOffset_fromP0);
...
#if defined(CONFIG_RTL_INBAND_CTL_API)
			port_stats[port].rx_bytes_last =
				port_stats[port].rx_bytes_current;
			port_stats[port].tx_bytes_last =
				port_stats[port].tx_bytes_current;
#endif
			re865x_accumulate_port_stats(port, &priv->net_stats);
#if defined(CONFIG_RTL_INBAND_CTL_API)
			port_stats[port].rx_rate =
				port_stats[port].rx_bytes_current -
				port_stats[port].rx_bytes_last;
			port_stats[port].tx_rate =
				port_stats[port].tx_bytes_current -
				port_stats[port].tx_bytes_last;
#endif
		}
...
#if defined(CONFIG_RTL_INBAND_CTL_API)
		port_stats[RTL83XX_WAN].rx_bytes_last =
			port_stats[RTL83XX_WAN].rx_bytes_current;
		port_stats[RTL83XX_WAN].tx_bytes_last =
			port_stats[RTL83XX_WAN].tx_bytes_current;
#endif
		re865x_accumulate_port_stats(RTL83XX_WAN, &priv->net_stats);
#if defined(CONFIG_RTL_INBAND_CTL_API)
		port_stats[RTL83XX_WAN].rx_rate =
			port_stats[RTL83XX_WAN].rx_bytes_current -
			port_stats[RTL83XX_WAN].rx_bytes_last;
		port_stats[RTL83XX_WAN].tx_rate =
			port_stats[RTL83XX_WAN].tx_bytes_current -
			port_stats[RTL83XX_WAN].tx_bytes_last;
#endif
...
static int32 re865x_packVlanConfig(struct rtl865x_vlanConfig vlanConfig1[],
	struct rtl865x_vlanConfig vlanConfig2[])
{
	int i, j;
	uint32 vlanCnt = 0;
	uint32 found = FALSE;
	...
	memset(vlanConfig2, 0,
		(vlanCnt + 1) * sizeof(struct rtl865x_vlanConfig));
		...
		for (j = 0; j < vlanCnt; j++) {
			if (vlanConfig1[i].if_type != vlanConfig2[j].if_type) {
				...
				if (vlanConfig1[i].if_type == IF_ETHER) {
					...
					/*PPP interface */
					if (strcmp(vlanConfig1[i].ifname,
							vlanConfig2[j].
							ifname)) {
						continue;
					} else {
		...
		if (found == TRUE) {
			...
			for (j = 0; j < vlanCnt; j++) {
				if (vlanConfig2[j].vid == 0) {
					memcpy(&vlanConfig2[j], &vlanConfig1[i],
						sizeof(struct
							rtl865x_vlanConfig));
...
static void rtl865x_disableDevPortForward(struct net_device *dev,
	struct dev_priv *cp)
{
	int port;
	...
	for (port = 0; port < RTL8651_AGGREGATOR_NUMBER; port++) {
		if ((1 << port) & cp->portmask) {
			...
			if (port <= 4) {
				...
				extern int rtk_port_adminEnable_set(uint32 port,
					uint32 enable);
				rtk_port_adminEnable_set(port, 0);
			}
#else
			REG32(PCRP0 + (port << 2)) =
				((REG32(PCRP0 + (port << 2))) & (~ForceLink));
			REG32(PCRP0 + (port << 2)) =
				((REG32(PCRP0 + (port << 2))) & (~EnablePHYIf));
			TOGGLE_BIT_IN_REG_TWICE(PCRP0 + (port << 2),
				EnablePHYIf);
...
static void rtl865x_restartDevPHYNway(struct net_device *dev,
	struct dev_priv *cp)
{
	int port;
	for (port = 0; port < RTL8651_AGGREGATOR_NUMBER; port++) {
...
static void rtl865x_setPortForward(int port_num, int forward)
{
	...
	if (forward == FALSE) {
		REG32(PCRP0 + (port_num << 2)) =
			((REG32(PCRP0 + (port_num << 2))) & (~EnablePHYIf));
#ifdef CONFIG_RTL_8196C_ESD
...
static void rtl865x_enableDevPortForward(struct net_device *dev,
	struct dev_priv *cp)
{
	int port;
...
static void rtk_queue_init(struct ring_que *que)
{
	...
	que->ring = (struct sk_buff **)kmalloc(
		...
		, GFP_ATOMIC);
	memset(que->ring, 0,
		(sizeof(struct sk_buff *)) * (rtl865x_maxPreAllocRxSkb + 1));
...
__MIPS16 __IRAM_FWD static struct sk_buff *rtk_dequeue(struct ring_que *que)
{
	struct sk_buff *skb;
...
/* __MIPS16 */
__IRAM_FWD static void refill_rx_skb(void)
{
	struct sk_buff *skb;
	unsigned long flags = 0;
	int idx;
	...
	while (rx_skb_queue.qlen < rtl865x_maxPreAllocRxSkb || ((idx >= 0)
			&& (SUCCESS == check_rx_pkthdr_ring(idx, &idx))))
#else
...
#endif
	{
		...
		if (idx >= 0) {
			if (SUCCESS == check_and_return_to_rx_pkthdr_ring(skb,
					idx)) {
				continue;
			}
		}
...
/* --------------------------------------------------------------------------- */
__IRAM_FWD unsigned char *alloc_rx_buf(void **skb, int buflen)
{
...
__IRAM_FWD unsigned char *alloc_rx_buf_init(void **skb, int buflen)
{
	struct sk_buff *new_skb;
	unsigned long flags = 0;
#ifdef CONFIG_RTL_JUMBO_FRAME
...
#define RTL_PRIV_DATA_SIZE	128
	...
	if (((struct sk_buff *)skb)->fast_br_forwarding_flags == 1
		&& is_rtl865x_eth_priv_buf(((struct sk_buff *)skb)->head)) {
		if (!(skb_cloned(skb))) {
			/* disable irq */
			SMP_LOCK_ETH_BUF(flags);
			if (rtk_queue_tail(&rx_skb_queue,
					(struct sk_buff *)skb)) {
				unsigned char *data;
				...
				memset(skb, 0, offsetof(struct sk_buff,
						truesize));
...
static inline int rtl_isMcastPkt(struct sk_buff *skb)
{
	...
	if ((skb->data[0] & 0x1) && (skb->data[0] != 0xff)) {
		...
		} else if ((skb->data[0] == 0x33) && (skb->data[1] == 0x33)
			&& (skb->data[2] != 0xff)) {
	...
	if (iph) {
		if (iph->protocol == IPPROTO_IPV6) {
			...
			if (ipv6h && (ipv6h->version == IP_VERSION6)
				&& (IN6_IS_ADDR_MULTICAST(ipv6h->daddr.
						s6_addr32))) {
	...
	if (ipv6h) {
		if (ipv6h->nexthdr == IPPROTO_IPIP)
		{
			...
			if (iph && (iph->version == IP_VERSION4)
...
int re865x_getIpv6TransportProtocol(struct ipv6hdr *ipv6h)
{
	...
	while (ptr < lastPtr) {
		...
		case ICMP_PROTOCOL:
			...
			if ((ptr[0] == MLD_QUERY) || (ptr[0] == MLDV1_REPORT)
				|| (ptr[0] == MLDV1_DONE)
				|| (ptr[0] == MLDV2_REPORT)) {
		...
		case MOSPF_PROTOCOL:
			nextHeader = NO_NEXT_HEADER;
			if (IS_IPV6_MOSPF_ADDR1(ipv6addr)
				|| IS_IPV6_MOSPF_ADDR2(ipv6addr)) {
	...
	unsigned int keepOrigSkb)
{
	...
	if (skb2 != NULL) {
...
#ifdef CONFIG_RTK_VLAN_WAN_TAG_SUPPORT
		if (vlan_bridge_tag && vlan_bridge_multicast_tag
			&& (vid == vlan_bridge_tag)
			...
			if (skb_wan != NULL) {
				...
				RTL_CACHE_WBACK((unsigned long)skb_wan->data,
					skb_wan->len);
				if (RTL_swNic_send((void *)skb_wan,
						skb_wan->data, skb_wan->len,
						&nicTx_wan) < 0) {
		...
		if (((struct sk_buff *)skb)->srcPhyPort ==
			QOS_PATCH_RX_FROM_LOCAL) {
		...
		nicTx.portlist =
			mcastFwdPortMask & ((struct dev_priv
				*)(NETDRV_PRIV(skb->dev)))->portmask;
#else
...
#ifdef CONFIG_RTK_VLAN_WAN_TAG_SUPPORT
		if ((vlan_tag && vid == vlan_tag) || (vlan_bridge_tag
				&& (vid == vlan_bridge_tag))) {
		...
		if (RTL_swNic_send((void *)skb2, skb2->data, skb2->len,
				&nicTx) < 0) {
			dev_kfree_skb_any(skb2);
		}
	}
...
unsigned int (*IgmpRxFilter_Hook)(struct sk_buff *skb,
	...
	const struct net_device *out, struct xt_table *table);
...
static bool rtl_MulticastRxFilterOff(struct sk_buff *skb, int ipversion)
{
	...
	if (ipversion == 4) {
		...
		if (type == PPPOE_IGMP) {
			...
			skb_set_network_header(skb,
				(int)((uint8 *)rtl_get_pppoe_ipv4_hdr(skb->data,
						1, 0) - skb->data));
		} else
#endif
			skb_set_network_header(skb,
				(int)((uint8 *)re865x_getIpv4Header(skb->data) -
					skb->data));
	}
#if defined(CONFIG_RTL_MLD_SNOOPING)
	else if (ipversion == 6)
		skb_set_network_header(skb,
			(int)((uint8 *)re865x_getIpv6Header(skb->data) -
				skb->data));
#endif
	else
	...
	if (ipversion == 4) {
		...
		if (type == PPPOE_IGMP) {
			/* only filer igmp packets */
			skb->network_header =
				(sk_buff_data_t) rtl_get_pppoe_ipv4_hdr(skb->
				data, 1, 0);
		} else
#endif
			skb->network_header =
				(sk_buff_data_t) re865x_getIpv4Header(skb->
				data);
	}
#if defined(CONFIG_RTL_MLD_SNOOPING)
	else if (ipversion == 6)
		skb->network_header =
			(sk_buff_data_t) re865x_getIpv6Header(skb->data);
#endif
	else
		return ret;	/* error shouldn't happen */
	...
	if (type == PPPOE_IGMP)
		skb->data =
			(unsigned char *)rtl_get_pppoe_ipv4_hdr(skb->data, 1,
			0);
	else
#endif
...
/* #ifndef CONFIG_RTL_8198C // modified by lynn_pu, 2014-10-22 */
		...
		if ((skb->dev->br_port != NULL)) {
			skb->dev =
				__dev_get_by_name(dev_net(skb->dev),
				RTL_PS_BR0_DEV_NAME);
		}
#else
...
#endif
		ret = ((IgmpRxFilter_Hook(skb, NF_INET_LOCAL_IN, skb->dev, NULL,
					dev_net(skb->dev)->ipv4.
					iptable_filter)) != NF_ACCEPT);
		skb->dev = origDev;
	} else if (ipversion == 6)
...
#endif
__MIPS16 int rtl_MulticastRxCheck(struct sk_buff *skb, rtl_nicRx_info *info,
	int mCastFlag)
{
...
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
	if ((info->vid == 0)
		&& (rtl819x_getSwEthPvid(pid, &(info->vid)) == SUCCESS))
		vid = info->vid;
#endif
	...
	if (linux_vlan_enable) {
		...
		vid = skb_vid;
		if (type != VID_FROM_PACKET &&
			*(uint16 *)skb->linux_vlan_src_tag ==
			__constant_htons(ETH_P_8021Q))
			vid = __constant_ntohs(*(unsigned short *)(skb->
					linux_vlan_src_tag + 2)) & 0xfff;
	}
#endif
	...
	vlanRelayPortMask =
		rtl865x_getVlanPortMask(pvid) & (~(1 << pid)) & ((1 <<
			RTL8651_MAC_NUMBER) - 1);
#else
	vlanRelayPortMask =
		rtl865x_getVlanPortMask(vid) & (~(1 << pid)) & ((1 <<
			RTL8651_MAC_NUMBER) - 1);
#endif
#if defined(CONFIG_RTL_LINK_AGGREGATION)
...
#else
#if defined(CONFIG_RTL_PROCESS_PPPOE_IGMP_FOR_BRIDGE_FORWARD)
	if ((mCastFlag == IPV4_MCAST_PACKET)
		|| (mCastFlag == DSLITE_MCAST_PACKET)
		|| (pppoe_type == PPPOE_IGMP))
#else
	if ((mCastFlag == IPV4_MCAST_PACKET)
		|| mCastFlag == DSLITE_MCAST_PACKET)
#endif
#endif
	{
...
#if defined(CONFIG_RTL_HARDWARE_MULTICAST)
#ifdef CONFIG_RTL_VLAN_PASSTHROUGH_SUPPORT
		if (*(uint16 *)skb->vlan_passthrough_saved_tag ==
			__constant_htons(ETH_P_8021Q)) {
			vid = *((unsigned short *)(skb->
					vlan_passthrough_saved_tag + 2));
...
#if defined(CONFIG_RTL_PROCESS_PPPOE_IGMP_FOR_BRIDGE_FORWARD)
		if (pppoe_type != 0)
			iph = rtl_get_pppoe_ipv4_hdr(skb->data, 1,
				skb->protocol);
		else
#endif
			iph = re865x_getIpv4Header(skb->data);
		...
		if (iph != NULL) {
			...
			if ((l4Protocol == IPPROTO_UDP)
				...
				|| (l4Protocol == Any_0_hop_protocl)) {
				/*relay packets which are trapped by hardware multicast table */
#if defined(CONFIG_RTL_HARDWARE_MULTICAST)
#ifdef CONFIG_RTK_VLAN_WAN_TAG_SUPPORT
				if (vlan_bridge_tag
					&& !strcmp(cp_this->dev->name,
						RTL_PS_ETH_NAME_ETH2)) {
					if (igmpsnoopenabled
						&& (nicIgmpModuleIndex_2 !=
							0xFFFFFFFF)) {
						multicastDataInfo.ipVersion = 4;
						multicastDataInfo.sourceIp[0] =
							(uint32)(iph->saddr);
						multicastDataInfo.groupAddr[0] =
							(uint32)(iph->daddr);

						multicastDataInfo.sourceIp[0] =
							ntohl(multicastDataInfo.
							sourceIp[0]);
						multicastDataInfo.groupAddr[0] =
							ntohl(multicastDataInfo.
							groupAddr[0]);
				...
				if (igmpsnoopenabled
					...
					memcpy(&multicastDataInfo.sourceIp,
						&iph->saddr, 4);
					memcpy(&multicastDataInfo.groupAddr,
						&iph->daddr, 4);
					multicastDataInfo.sourceIp[0] =
						ntohl(multicastDataInfo.
						sourceIp[0]);
...
#if definedCONFIG_RTL_VLAN_8021Q
							if (linux_vlan_enable)
								goto relay_out;
							else
								return 0;
					...
					ret = rtl_getMulticastDataFwdInfo
						(nicIgmpModuleIndex,
						&multicastDataInfo,
						&nicMCastFwdInfo);
#ifndef CONFIG_RTL_VLAN_8021Q
#ifdef CONFIG_RTL_VLAN_PASSTHROUGH_SUPPORT
					vlanRelayPortMask =
						rtl865x_getVlanPortMask(pvid) &
						(~(1 << pid)) & nicMCastFwdInfo.
						fwdPortMask & cp_this->
						portmask & ((1 <<
							RTL8651_MAC_NUMBER) -
						1);
					...
					if (ret == SUCCESS) {
						...
						if (linux_vlan_enable) {
							if (vlanRelayPortMask ==
								...
								igmpModuleIndex
									=
									rtl_get_brIgmpModuleIndexbyVid
									(vid);
						...
						if (linux_vlan_enable) {
							...
							igmpModuleIndex =
								rtl_get_brIgmpModuleIndexbyName
								(RTL_BR_NAME,
								&br_index);
						...
						if (toCpuFlag == 0
							...
							mapPortMask =
								...
								(vid) & ((1 <<
									RTL8651_MAC_NUMBER)
								- 1);
							rtl865x_nic_MulticastHardwareAccelerate
								(nicMCastFwdInfo.
								fwdPortMask,
								pid, vid,
								multicastDataInfo.
				...
				vlanportmask =
					(~(1 << pid)) & nicMCastFwdInfo.
					fwdPortMask & cp_this->
					portmask & ((1 << RTL8651_MAC_NUMBER) -
					1);
				if (linux_vlan_enable)
					re865x_relayTrappedMCast(skb, vid,
						vlanportmask, TRUE);
				else
#endif
#endif
					re865x_relayTrappedMCast(skb, vid,
						vlanRelayPortMask, TRUE);
		...
		&& (skb->data[2] != 0xff))
#else
	else if ((mCastFlag == IPV6_MCAST_PACKET)
		|| mCastFlag == SIXRD_MCAST_PACKET)
#endif
	{
			...
			if ((l4Protocol == IPPROTO_UDP)
				...
				if (nicIgmpModuleIndex != 0xFFFFFFFF) {
					...
					multicastDataInfo.sourceIp[0] =
						ntohl(multicastDataInfo.
						sourceIp[0]);
					multicastDataInfo.sourceIp[1] =
						ntohl(multicastDataInfo.
						sourceIp[1]);
					multicastDataInfo.sourceIp[2] =
						ntohl(multicastDataInfo.
						sourceIp[2]);
					multicastDataInfo.sourceIp[3] =
						ntohl(multicastDataInfo.
						sourceIp[3]);
					multicastDataInfo.groupAddr[0] =
						ntohl(multicastDataInfo.
						groupAddr[0]);
					multicastDataInfo.groupAddr[1] =
						ntohl(multicastDataInfo.
						groupAddr[1]);
					multicastDataInfo.groupAddr[2] =
						ntohl(multicastDataInfo.
						groupAddr[2]);
					multicastDataInfo.groupAddr[3] =
						ntohl(multicastDataInfo.
						groupAddr[3]);
				...
				/*icmp packet */
				if (nicIgmpModuleIndex != 0xFFFFFFFF) {
					rtl_igmpMldProcess(nicIgmpModuleIndex,
						skb->data, pid,
						&vlanRelayPortMask);
#ifdef CONFIG_RTL_VLAN_PASSTHROUGH_SUPPORT
					vlanRelayPortMask =
...
#endif
		vlanRelayPortMask =
			rtl865x_getVlanPortMask(vid) & (~(1 << pid)) & cp_this->
			portmask & ((1 << RTL8651_MAC_NUMBER) - 1);
#if defined(CONFIG_RTL_LINK_AGGREGATION)
		if (ret_flag) {
			/* if port is link aggregation port group, remove other link aggregation ports too. */
			vlanRelayPortMask &= ~(res_mask);
...
static inline int32 rtl_isPassthruFrame(uint8 *data)
{
	...
	if (oldStatus) {
		if (oldStatus & IP6_PASSTHRU_MASK) {
			if ((*((uint16 *)(data + (ETH_ALEN << 1))) ==
					__constant_htons(ETH_P_IPV6))
				|| ((*((uint16 *)(data + (ETH_ALEN << 1))) ==
						__constant_htons(ETH_P_8021Q))
					&& (*((uint16 *)(data + (ETH_ALEN << 1)
								+ VLAN_HLEN)) ==
						__constant_htons(ETH_P_IPV6))))
			...
			if (((*((uint16 *)(data + (ETH_ALEN << 1))) ==
				...
				|| ((*((uint16 *)(data + (ETH_ALEN << 1))) ==
					...
					&& ((*((uint16 *)(data + (ETH_ALEN << 1)
							...
							__constant_htons
							(ETH_P_PPP_SES))
						|| (*((uint16 *)(data + (ETH_ALEN << 1) + VLAN_HLEN)) == __constant_htons(ETH_P_PPP_DISC))))) {
				ret = SUCCESS;
			}
		}
#endif
...
static rtl_queue_entry pktQueueByPri[RTL865X_SWNIC_RXRING_MAX_PKTDESC] =
	{ {0} };

__MIPS16 __IRAM_FWD int rtl_enqueueSkb(rtl_nicRx_info *info)
{
	...
	entry->cnt++;
	entry->end =
		(entry->end == (RTL_NIC_QUEUE_LEN - 1)) ? 0 : (entry->end + 1);
...
__MIPS16 __IRAM_FWD int rtl_dequeueSkb(rtl_nicRx_info *info)
{
	rtl_queue_entry *entry;
	...
	if (entry->cnt == 0) {
		...
		memcpy(info, &entry->entry[entry->start],
			sizeof(rtl_nicRx_info));
		entry->cnt--;
		entry->start =
			(entry->start ==
			(RTL_NIC_QUEUE_LEN - 1)) ? 0 : (entry->start + 1);
		return SUCCESS;
	}
}
#endif
...
#if defined(RTL_CPU_QOS_ENABLED)
__MIPS16 __IRAM_FWD
	static inline int32 rtl_processReceivedInfo(rtl_nicRx_info *info,
	int nicRxRet)
{
	int ret;
	...
	case RTL_NICRX_OK:
		{
			...
			if (info->priority ==
				(RTL865X_SWNIC_RXRING_MAX_PKTDESC - 1)) {
...
__MIPS16 __IRAM_FWD static inline int32 rtl_decideRxDevice(rtl_nicRx_info *info)
{
	struct dev_priv *cp;
	int32 pid, i, ret;
	struct sk_buff *skb;
...
#endif
	{
		...
		if (rtl865x_curOpMode == GATEWAY_MODE) {
			if (0 == (skb->data[0] & 0x01)) {
				if (rtl865x_decideUcDevice(skb->data, pid,
						info) == SUCCESS) {
					ret = SUCCESS;
					goto out;
				}
				...
				if (skb->data[0] != 0xff) {
					if (rtl865x_decideMcDevice(pid,
							info) == SUCCESS) {
						ret = SUCCESS;
						goto out;
					}
				} else {
					if (rtl865x_decideBcDevice(skb, pid,
							info) == SUCCESS) {
						ret = SUCCESS;
						goto out;
					}
		...
		for (i = 0; i < ETH_INTF_NUM; i++) {
			cp = ((struct dev_priv *)NETDRV_PRIV(_rtl86xx_dev.
					dev[i]));
			/* printk("=========%s(%d),cp(%s),i(%d)\n",__FUNCTION__,__LINE__,cp->dev->name,i); */
#ifdef CONFIG_RTK_VLAN_WAN_TAG_SUPPORT
			if (*((unsigned short *)(skb->data +
						(ETH_ALEN << 1))) ==
				__constant_htons(ETH_P_8021Q)) {
				vid = *((unsigned short *)(skb->data +
						(ETH_ALEN << 1) + 2)) & 0x0fff;

				if (vlan_tag && vid == vlan_tag) {
					if (rtl865x_curOpMode == BRIDGE_MODE)
						vid = RTL_LANVLANID;
				} else if (vlan_bridge_tag
					&& vid == vlan_bridge_tag) {
					vid = vlan_bridge_tag;
				} else if (vlan_bridge_multicast_tag
					&& vid == vlan_bridge_multicast_tag) {
					vid = vlan_bridge_tag;
				}
...
#if defined(CONFIG_RTL_CUSTOM_PASSTHRU)
#if defined(CONFIG_RTD_1295_HWNAT)
		else if (SUCCESS == rtl_isPassthruFrame(data)
			&& (rtl_isWanDev(cp) == TRUE)
			&& (!ether_addr_equal((char *)cp->dev->dev_addr,
					(char *)dest_mac)))
#else /* defined(CONFIG_RTD_1295_HWNAT) */
		else if (SUCCESS == rtl_isPassthruFrame(data)
			&& (rtl_isWanDev(cp) == TRUE)
			&& (compare_ether_addr((char *)cp->dev->dev_addr,
					(char *)dest_mac)))
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
		{
#if defined(CONFIG_IPV6)
...
int rtl_add_Acl_for_dns_trap(struct rtl865x_vlanConfig *vlanConfig)
{
	...
	for (i = 0; pVlanConfig[i].vid != 0; i++) {
		...
		if (pVlanConfig[i].isWan == 0) {	/*lan config */
			...
			rtl865x_regist_aclChain(pVlanConfig[i].ifname,
				RTL865X_ACL_DNSTRAP_USED, RTL865X_ACL_INGRESS);
			...
			ret = rtl865x_add_acl(&rule, pVlanConfig[i].ifname,
				RTL865X_ACL_DNSTRAP_USED);
...
int rtl_remove_Acl_for_dns_trap(struct rtl865x_vlanConfig *vlanConfig)
{
	...
	for (i = 0; pVlanConfig[i].vid != 0; i++) {
		...
		if (pVlanConfig[i].isWan == 0) {	/*lan config */
			rtl865x_unRegist_aclChain(pVlanConfig[i].ifname,
				RTL865X_ACL_DNSTRAP_USED, RTL865X_ACL_INGRESS);
		} else {	/*wan config */
	...
	if (rtk_vlan_support_enable == 0) {
		rtl865x_setDefACLForAllNetif(RTL865X_ACLTBL_PERMIT_ALL,
			RTL865X_ACLTBL_PERMIT_ALL, RTL865X_ACLTBL_PERMIT_ALL,
			RTL865X_ACLTBL_PERMIT_ALL);
	} else {
		rtl865x_setDefACLForAllNetif(RTL865X_ACLTBL_ALL_TO_CPU,
			RTL865X_ACLTBL_ALL_TO_CPU, RTL865X_ACLTBL_PERMIT_ALL,
			RTL865X_ACLTBL_PERMIT_ALL);
	}
#else
	rtl865x_setDefACLForAllNetif(RTL865X_ACLTBL_PERMIT_ALL,
...
#if defined(BR_SHORTCUT)
__MIPS16 __IRAM_FWD
	static inline int32 rtl_processBridgeShortCut(struct sk_buff *skb,
	struct dev_priv *cp_this, rtl_nicRx_info *info)
{
	struct net_device *dev;
	...
	if (
...
#else
#if defined(CONFIG_RTL_ULINKER_BRSC)
		(((dev = brsc_get_cached_dev(0, skb->data)) != NULL)
		...
		if (cached_usb.dev && dev == cached_usb.dev) {
			...
			if (skb->dev)
				brsc_cache_dev(2, skb->dev,
					skb->data + ETH_ALEN);
		} else
#endif /* #if defined(CONFIG_RTL_ULINKER_BRSC) */
		{
#if defined(CONFIG_RTL_HARDWARE_NAT)
			if (memcmp(&skb->data[ETH_ALEN], cp_this->dev->dev_addr,
					ETH_ALEN))
#endif
			{
				if (memcmp(cached_eth_addr,
						&skb->data[ETH_ALEN],
						ETH_ALEN) == 0) {
					/* memcpy(cached_eth_addr, &skb->data[ETH_ALEN], ETH_ALEN); */
					cached_dev = cp_this->dev;
					last_used = 0;
				} else if (memcmp(cached_eth_addr2,
						&skb->data[ETH_ALEN],
						ETH_ALEN) == 0) {
					/* memcpy(cached_eth_addr2, &skb->data[ETH_ALEN], ETH_ALEN); */
					cached_dev2 = cp_this->dev;
					last_used = 1;
				} else if (memcmp(cached_eth_addr3,
						&skb->data[ETH_ALEN],
						ETH_ALEN) == 0) {
					cached_dev3 = cp_this->dev;
					last_used = 2;
				} else if (memcmp(cached_eth_addr4,
						&skb->data[ETH_ALEN],
						ETH_ALEN) == 0) {
					cached_dev4 = cp_this->dev;
					last_used = 3;
				} else if (last_used == 3) {
					memcpy(cached_eth_addr3,
						&skb->data[ETH_ALEN], ETH_ALEN);
					cached_dev3 = cp_this->dev;
					last_used = 2;
				} else if (last_used == 2) {
					memcpy(cached_eth_addr2,
						&skb->data[ETH_ALEN], ETH_ALEN);
					cached_dev2 = cp_this->dev;
					last_used = 1;
				} else if (last_used == 1) {
					memcpy(cached_eth_addr,
						&skb->data[ETH_ALEN], ETH_ALEN);
					cached_dev = cp_this->dev;
					last_used = 0;
				} else {
					memcpy(cached_eth_addr4,
						&skb->data[ETH_ALEN], ETH_ALEN);
					cached_dev4 = cp_this->dev;
					last_used = 3;
				}
		...
		if (linux_vlan_enable) {
			if (*((unsigned short *)(skb->data +
						(ETH_ALEN << 1))) ==
				__constant_htons(ETH_P_8021Q)) {
				vid = ntohs(*(uint16 *)(skb->data +
						(ETH_ALEN << 1) + 2)) & 0x0fff;
				if (!(vlan_ctl_p->group[vid].
						tagMemberMask & (1 << dev->
							vlan_member_map))) {
					if (skb_cloned(skb)) {
						newskb = skb_copy(skb,
...
#if 0
__MIPS16 __IRAM_FWD
	static inline int32 rtl_processExtensionPortShortCut(struct sk_buff
	*skb, struct dev_priv *cp_this, rtl_nicRx_info *info)
{
	struct net_device *dev;

	if ((PKTHDR_EXTPORT_MAGIC != info->vid)
		|| (info->pid != PKTHDR_EXTPORT_P3))
		return FAILED;
...
__MIPS16 __IRAM_FWD
	static inline void rtl_processRxToProtcolStack(struct sk_buff *skb,
	struct dev_priv *cp_this)
{
	...
	skb_queue_tail(&cp_this->rx_napi_skb_queue, skb);
...
#ifdef CONFIG_RTL_8197F
#ifdef CONFIG_RTL_ETH_NAPI_GRO_SUPPORT
...
extern int rx_vlan_process(struct net_device *dev, struct vlan_info *info_ori,
	struct sk_buff *skb, struct sk_buff **new_skb);
extern int tx_vlan_process(struct net_device *dev, struct vlan_info *info_ori,
	struct sk_buff *skb, int wlan_pri);
#else
extern int rx_vlan_process(struct net_device *dev, struct vlan_info *info,
	struct sk_buff *skb);
extern int tx_vlan_process(struct net_device *dev, struct vlan_info *info,
	struct sk_buff *skb, int wlan_pri);
#endif
#endif
...
static inline int32 rtl_fwdToExtPort(struct sk_buff *skb,
	struct dev_priv *cp_this, rtl_nicRx_info *info)
{
	struct net_device *dev = NULL;
	int ret = FAILED;
	...
	if (dev != NULL) {
		...
		if (net_ratelimit()) {
			printk("dev:%s,%x,%x,[%s]:[%d].\n", dev->name,
				info->vid, info->pid, __FUNCTION__, __LINE__);
...
#endif
		panic_printk
			("eth ext Tx:can not find dst  ext  port device!\n");
		dev_kfree_skb_any(skb);
		return SUCCESS;
	}
	return FAILED;
...
static int32 rtl_fwdFromExtPort(struct sk_buff *skb, struct dev_priv *cp_this,
	rtl_nicRx_info *info)
{
	int ret = FAILED;
	...
	if (ret == SUCCESS) {
		if (1)		/* (info->pid==PKTHDR_EXTPORT_P3) */
		{
			...
			ori_wlan_dev =
				rtl_get_wlan_dev_by_mac(skb->data + ETH_ALEN);
			if (ori_wlan_dev) {
				...
				panic_printk
					("eth ext Rx:can not find src ext port device!\n");
				dev_kfree_skb_any(skb);
				return SUCCESS;
			}
		}
...
__MIPS16 __IRAM_FWD static inline void rtl_processRxFrame(rtl_nicRx_info *info)
{
	struct dev_priv *cp_this;
	struct sk_buff *skb;
	uint32 vid, pid, len;
...
#ifdef CONFIG_RTK_VLAN_WAN_TAG_SUPPORT
		if ((PKTHDR_EXTPORT_MAGIC != vid)
			|| (info->pid != PKTHDR_EXTPORT_P3
				&& info->pid != PKTHDR_EXTPORT_P2))
#else
		if ((PKTHDR_EXTPORT_MAGIC != vid)
			|| (info->pid != PKTHDR_EXTPORT_P3))
#endif
	...
	if (*(uint16 *)(&(skb->data[12])) == htons(0x8899)
		&& voip_dsp_L2_pkt_rx_trap) {
	...
	if (*((uint16 *)(skb->data + (ETH_ALEN << 1))) !=
		...
		printk("skb + 6=  %2x %2x %2x %2x %2x %2x | %2x %2x %2x %2x \n", skb->data[0 + 6], skb->data[1 + 6], skb->data[2 + 6], skb->data[3 + 6], skb->data[4 + 6], skb->data[5 + 6], skb->data[6 + 6], skb->data[7 + 6], skb->data[8 + 6], skb->data[9 + 6]);	/* mark_sw */
#endif
	}
...
#endif
	{
		...
		int vlan_check;
		vlan_check =
			rx_vlan_process(cp_this->dev, &cp_this->vlan_setting,
			skb, &new_skb);
		...
		if (rtk_vlan_support_enable == 2) {
			DEBUG_ERR
				("====%s(%d),cp(%s),cp->vlan.id(%d),skb->tag.vid(%d)\n",
				__FUNCTION__, __LINE__, cp_this->dev->name,
				cp_this->vlan_setting.id,
				skb->tag.f.pci & 0xfff);
			if (cp_this->vlan_setting.vlan
	...
	if (*((uint16 *)(skb->data + (ETH_ALEN << 1))) ==
		__constant_htons(ETH_P_8021Q)) {
		vid = ntohs(*(uint16 *)(skb->data + (ETH_ALEN << 1) +
				2)) & 0x0fff;
#if	defined(CONFIG_RTL_QOS_8021P_SUPPORT)
		skb->srcVlanPriority =
			ntohs(*(uint16 *)(data + (ETH_ALEN << 1) + 2)) >> 13;
#endif

#if defined(CONFIG_RTL_VLANPRI_IPTABLE_CHECK)
		skb->original_vlanpri =
			ntohs(*(uint16 *)(data + (ETH_ALEN << 1) + 2)) >> 13;
#endif
	}
#ifdef CONFIG_RTL_VLAN_8021Q
...
#endif
				{
					...
					info1.vid =
						ntohs(*(uint16 *)(skb1->
							...
							2));
					info1.priv =
						NETDRV_PRIV(_rtl86xx_dev.
						dev[1]);
					info1.isPdev = FALSE;

					/*      multicast process       */
#if defined(CONFIG_RTL_IGMP_SNOOPING) || defined(CONFIG_BRIDGE_IGMP_SNOOPING)
					rtl_MulticastRxCheck(skb1, &info1,
						mCastFlag);
#endif
				}
#endif
	...
	if (linux_vlan_enable && (!linux_vlan_hw_support_enable)) {
		...
		if (*((uint16 *)(skb->data + (ETH_ALEN << 1))) !=
			__constant_htons(ETH_P_8021Q)) {
#if 0
			if ((info->srcvid != RTL_LANVLANID)
				&& (info->srcvid != RTL_WANVLANID))
#endif
				if ((vlan_ctl_p->pvid[info->pid] !=
						RTL_LANVLANID)
					&& (vlan_ctl_p->pvid[info->pid] !=
						RTL_WANVLANID)) {
					memmove(skb->data - VLAN_HLEN,
						skb->data, ETH_ALEN << 1);
					skb_push(skb, VLAN_HLEN);
					*((uint16 *)(skb->data +
					...
					if (info->srcvid) {
						...
						*((uint16 *)(skb->data +
								(ETH_ALEN << 1)
								+ 2)) =
							vlan_ctl_p->pvid[info->
							pid];
					}
#else
					*((uint16 *)(skb->data +
							(ETH_ALEN << 1) + 2)) =
						__constant_htons(vlan_ctl_p->
						pvid[info->pid]);
#endif
					data = skb->data;
				}
	...
	if (*((uint16 *)(skb->data + (ETH_ALEN << 1))) ==
		__constant_htons(ETH_P_8021Q)) {
#if defined(CONFIG_RTL_VLAN_PASSTHROUGH_SUPPORT)
		memcpy(skb->vlan_passthrough_saved_tag,
			skb->data + ETH_ALEN * 2, 4);
#endif
#if defined(CONFIG_RTL_HW_VLAN_SUPPORT)
		if ((skb->data[0] & 1) || ((skb->data[0] == 0x33)
				&& (skb->data[1] == 0x33)
				&& (skb->data[2] != 0xFF))) {
			memcpy(&skb->tag, skb->data + ETH_ALEN * 2,
				sizeof(struct vlan_tag));
		}
#endif
...
reserve_vlan_tag:
	...
	if (isDHCPpkt(skb)) {
		...
		if (port_id < RTL8651_PHY_NUMBER) {
			...
			memcpy(port_mac_table[port_id].client_mac, client_mac,
				ETH_ALEN);
			for (i = 0; i < RTL8651_PHY_NUMBER; i++) {
				if ((i != port_id)
					&& (memcmp(port_mac_table[i].client_mac,
							client_mac,
							ETH_ALEN) == 0))
					memset(port_mac_table[i].client_mac, 0,
						ETH_ALEN);
			}
		} else
...
updated_end:
...
#endif /*      unicast pkt only        */
	{
		...
		if (vpn_port > 0 && vpn_port < 65536) {
			...
			if (skb->len > 43 && *((uint8 *)(skb->data + 23)) == 17
				&& *((uint16 *)(skb->data + 36)) == vpn_port
				&& (*((uint16 *)(skb->data +
							20)) & ntohs(0x3fff)) ==
				0)
				dec_ret = rtk_openvpn_hw_decrypto(skb, 1);	/* static/dhcp */
			else if (skb->len > 51
				&& *((uint8 *)(skb->data + 31)) == 17
				&& *((uint16 *)(skb->data + 44)) == vpn_port
				&& (*((uint16 *)(skb->data +
							28)) & ntohs(0x3fff)) ==
				0)
		...
		if (!linux_vlan_enable) {
			...
			/*      shortcut process        */
#if defined(BR_SHORTCUT)
			if (SUCCESS == rtl_processBridgeShortCut(skb, cp_this,
					info)) {
				return;
			}
#elif defined(CONFIG_RTL_HARDWARE_NAT) && (defined(CONFIG_RTL8192SE) || defined(CONFIG_RTL8192CD))
...
#if defined(CONFIG_RTL_FASTBRIDGE)
			if (RTL_FB_RETURN_SUCCESS == rtl_fb_process_in_nic(skb,
					cp_this->dev))
				return;
#endif
...
__IRAM_FWD
	static inline int32 interrupt_dsr_rx_per_packe_check(rtlInterruptRxData
	*rxData, unsigned long task_priv)
{
	if (((rxData->voip_rx_cnt++ > 100)
			|| ((jiffies - rxData->voip_rx_start_time) >= 1))
		&& (check_voip_channel_loading
			&& (check_voip_channel_loading() > 0))) {
#ifdef RX_TASKLET
		/* avoid cp  change! */
		tasklet_schedule(&((struct dev_priv *)task_priv)->
			rx_dsr_tasklet);
#endif
...
#if defined(CONFIG_RTK_VOIP_QOS) && !defined(CONFIG_RTK_VOIP_ETHERNET_DSP_IS_HOST)
		if (RTL_RX_PROCESS_RETURN_BREAK ==
			interrupt_dsr_rx_per_packe_check(&RxIntData, task_priv))
			break;
#endif
...
__IRAM_FWD static void interrupt_dsr_tx(unsigned long task_priv)
{
	int32 idx;
#ifdef TX_TASKLET
	unsigned long flags = 0;
	...
	for (i = 0; i < 5; i++) {
		...
		if ((curLinkPortMask & (1 << i)) != (newLinkPortMask & (1 << i))) {	/* port i link changed */
			...
			if (newLinkPortMask & (1 << i)) {	/* link up */
				...
				rtl8651_setAsicEthernetPHYReg(i, 13, 0x4007);
				rtl8651_getAsicEthernetPHYReg(i, 14,
					&Linkpartner_eee);

				if ((((DUT_eee & 0x2)) != 0)
					&& (((Linkpartner_eee & 0x2)) != 0)) {
					rtl8651_getAsicEthernetPHYReg(i, 21,
						&snr);
					snr = snr | 0x4000;
					rtl8651_setAsicEthernetPHYReg(i, 21,
						snr);
				}
				...
				snr = 0;
				for (val = 0; val < 3; val++) {
					rtl8651_getAsicEthernetPHYReg(i, 29,
						&agc);
					snr += agc;
				}
				snr = snr / 3;
	...
	for (port = 0; port < RTL8651_PHY_NUMBER; port++) {
		if ((1 << port) & (newLinkPortMask & (~curLinkPortMask))) {
			...
			if (snr > SNR_THRESHOLD) {
				...
				if (re865x_restartNWayCtrl[port].
					restartNWayTime == 0) {
					/*last time snr is good */
					re865x_restartNWayCtrl[port].
						restartNWayTime = jiffies;
					re865x_restartNWayCtrl[port].
						restartNWayCnt = 1;

					rtl8651_restartAsicEthernetPHYNway
						(port);
				...
				} else if (re865x_restartNWayCtrl[port].
					restartNWayCnt > MAX_RESTART_NWAY_CNT) {
				...
				re865x_restartNWayCtrl[port].restartNWayTime =
					0;
...
unsigned int rtl865x_getPhysicalPortLinkStatus(void)
{
	...
	for (port_num = 0; port_num <= RTL8651_PHY_NUMBER; port_num++) {
		if ((READ_MEM32(PSRP0 + (port_num << 2)) & PortStatusLinkUp) !=
			0) {
...
#if defined(CONFIG_RTD_1295_HWNAT)
static void rtd129x_send_uevent_work(struct work_struct *work)
{
	struct dev_priv *cp =
		container_of((work), struct dev_priv, link_event_work);

	if (cp->link_event == KOBJ_LINKUP) {
		netif_carrier_on(cp->dev);
		if (hwnat_gpio_link_led_enable &&
			(strcmp(cp->dev->name, "eth0") == 0))
			gpio_set_value(hwnat_gpio_link_led, 1);
		pr_info("%s link up\n", cp->dev->name);
	} else {
		netif_carrier_off(cp->dev);
		if (hwnat_gpio_link_led_enable &&
			(strcmp(cp->dev->name, "eth0") == 0))
			gpio_set_value(hwnat_gpio_link_led, 0);
		pr_info("%s link down\n", cp->dev->name);
	}
	kobject_uevent(&cp->dev->dev.kobj, cp->link_event);
}

void rtd129x_send_uevent(unsigned int curLinkPortMask, unsigned int newportmask)
{
	int i;
	struct dev_priv *cp = NULL;
	...
	for (i = 0; i < ETH_INTF_NUM; i++) {
		...
		if (_rtl86xx_dev.dev[i] && cp && cp->opened) {
			if (((cp->portmask & curLinkPortMask) == 0)
				&& (cp->portmask & newportmask)) {
				cp->link_event = KOBJ_LINKUP;
				schedule_work(&cp->link_event_work);
			} else if ((cp->portmask & curLinkPortMask)
				&& ((cp->portmask & newportmask) == 0)) {
				cp->link_event = KOBJ_LINKDOWN;
				schedule_work(&cp->link_event_work);
			}
		}
	}
...
void rtk_eventd_netlink_send(int pid, struct sock *nl_sk, int eventID,
	char *ifname, char *data, int data_len);

int rtl_event_indicate_api(unsigned int curLinkPortMask,
	unsigned int newportmask)
{
...
#define WIRE_PLUG_ON 3
	...
	if (offPortMask != 0) {
		...
		if (portId != -1)
			rtk_eventd_netlink_send(rtk_eventd_pid, nl_eventd_sk,
				event, NULL, data, data_len);
	}
	portId = -1;
	/* plug on */
	if (onPortMask != 0) {
...
#endif
unsigned int rtl865x_setLinkEventFlag(unsigned int curLinkPortMask,
	unsigned int newportmask)
{
	int i;
	struct dev_priv *cp = NULL;
	...
	for (i = 0; i < RTL8651_PHY_NUMBER; i++) {
		...
		if ((curLinkPortMask ^ newLinkPortMask) & (1 << i)) {
			...
			if (rtl_get_net_device_by_port(&dev, i) == SUCCESS) {
				...
				ethernet_rtmsg_ifinfo(RTM_NEWLINK, dev, ~0U,
					status);
			}
...
/* #if LINUX_VERSION_CODE < KERNEL_VERSION(3,10,0) */
	...
	if (rtl865x_curOpMode == GATEWAY_MODE) {
		for (i = 0; i < ETH_INTF_NUM; i++) {
			cp = ((struct dev_priv *)NETDRV_PRIV(_rtl86xx_dev.
					dev[i]));
#if defined(CONFIG_RTL_ISP_MULTI_WAN_SUPPORT) || defined(CONFIG_RTD_1295_HWNAT)
			if (_rtl86xx_dev.dev[i] && cp && cp->opened
				&& rtl_isWanDev(cp))
#else
...
static inline int diff_more_than_1(uint32 a, uint32 b)
{
	uint32 c;
...
static int esd_recovery(void)
{
	...
	for (i = 0; i < RTL8651_PHY_NUMBER; i++) {
		...
		/************ Link up  ************/
			...
			rtl8651_setAsicEthernetPHYReg(i, 22,
				((val & (0xff00)) | 0x17));
...
void rtl819x_port0_force_refined(void)
{
	...
	if (newLinkPortMask & 0x01) {
		/* if AN =0(reg6[0]) */
		rtl8651_getAsicEthernetPHYReg(rtl8651_tblAsicDrvPara.
			externalPHYId[0], 6, &regData);
		if ((regData & 0x01) == 0) {
			rtl8651_getAsicEthernetPHYReg(rtl8651_tblAsicDrvPara.
				externalPHYId[0], 5, &regData);
			if (regData & 0x080) {	/* if spd = 100M(reg5[7]) */
				...
				regData |=
					...
					ForceDuplex | PauseFlowControlEtxErx);
...
static int rtl819xD_check_phy_cb_snr(void)
{
	...
	for (port = CHECK_SNR_START_PORT; port < RTL8651_PHY_NUMBER; port++) {
		...
		if ((REG32(PSRP0 + (port * 4)) & PortStatusLinkSpeed_MASK) ==
			PortStatusLinkSpeed10M)
			link_speed_10M = 1;
		else
			link_speed_10M = 0;
		...
		if (AutoDownSpeed_10M[port] == 0x12345678) {
			...
			if ((!curr_sts) && (ReverSpeed_flag[port] == 1)) {
				REG32(PCRP0 + (port * 4)) |=
					(NwayAbility1000MF | NwayAbility100MF |
					NwayAbility100MH);
...
uint64 rtl819xD_get_trx_bytes(uint32 port, int direction)
{
	...
	if (0 == direction)
		return rtl865xC_returnAsicCounter64(OFFSET_IFINOCTETS_P0 +
			addrOffset_fromP0);

	if (1 == direction)
		return rtl865xC_returnAsicCounter64(OFFSET_IFOUTOCTETS_P0 +
			addrOffset_fromP0);
	...
	for (i = 0; i < 5; i++) {
		if ((old_sts & BIT(i)) != (new_sts & BIT(i))) {	/* port i link status changed */
			...
			if (new_sts & BIT(i)) {	/* link up */
				...
				rtl8651_setAsicEthernetPHYReg(phyid, 16,
					(data | BIT(3)));
				rtl8651_getAsicEthernetPHYReg(phyid, 18, &data);
				if ((REG32(PSRP0 + (i * 4)) & 0x3) == 0) {	/* 10M */
					rtl8651_setAsicEthernetPHYReg(phyid, 18,
						((data & 0x00ff) | 0xcc00));
				} else {	/* 100M or Giga */
					rtl8651_setAsicEthernetPHYReg(phyid, 18,
						(data & 0x00ff));
...
#if defined(CONFIG_RTD_1295_HWNAT)
	rtd129x_send_uevent(curLinkPortMask, newLinkPortMask);
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
	...
	struct dev_priv *cp)
{
...
#endif
	{
		...
		if ((rtl_rx_tasklet_running == 0) && (cp->napi.poll)) {
			...
			REG32(CPUIIMR) &=
				~(RX_DONE_IE_ALL | PKTHDR_DESC_RUNOUT_IE_ALL);
...
void refine_phy_setting(void)
{
	...
	if ((val == BOND_8196ES) || (val == BOND_8196ES1)
		|| (val == BOND_8196ES2) || (val == BOND_8196ES3))
		return;

	if ((val == BOND_8196EU) || (val == BOND_8196EU1)
		|| (val == BOND_8196EU2) || (val == BOND_8196EU3))
		start_port = 4;
...
int flush_l2(void)
{
...
#if defined(CONFIG_RTL_8367R_SUPPORT)
		uint32 phyData;
		extern int rtl8367b_getAsicPHYReg(unsigned int phyNo,
			unsigned int phyAddr, unsigned int *pRegData);
		/*Get PHY status register */
		if (0 != rtl8367b_getAsicPHYReg(port, 1 /* PHY_STATUS_REG */,
				&phyData))
			return (-1);
	...
	if (port_link_sts2 != cur_link_sts) {
		...
		{
			extern int32 _rtl865x_ClearFDBEntryByPort(int32
				port_num);
			_rtl865x_ClearFDBEntryByPort(RTL83XX_WAN);
		}
#else
#if defined(CONFIG_RTD_1295_HWNAT)
		if ((cur_link_sts & (vlanconfig[RTL_WAN_IDX].memPort))
			&& (cur_link_sts & (0x1f
			& ~(vlanconfig[RTL_WAN_IDX].memPort))))	/* wan is up and "one of LANs" is link up */
#else /* defined(CONFIG_RTD_1295_HWNAT) */
		if ((cur_link_sts & (vlanconfig[1].memPort))
			&& (cur_link_sts & (0x1f
			& ~(vlanconfig[1].memPort))))	/* wan is up and "one of LANs" is link up */
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
		{
			extern int32 _rtl865x_ClearFDBEntryByPort(int32
				port_num);
			...
			for (port = 0; port < 5; port++)
#if defined(CONFIG_RTD_1295_HWNAT)
				if ((vlanconfig[RTL_WAN_IDX].memPort
					& (1 << port)))
					break;
#else /* defined(CONFIG_RTD_1295_HWNAT) */
				if ((vlanconfig[1].memPort & (1 << port)))
					break;
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
...
uint32 rtl_get_8367r_port_link_status(void)
{
	...
	for (port = PHY0; port <= PHY4; port++) {
		extern int rtk_port_macStatus_get(int port,
...
#if (defined(CONFIG_RTL_819XD) && !defined(CONFIG_RTL_8367R_SUPPORT)) || defined(CONFIG_RTL_8196E)
...
static void suspend_check_timer_fn(void)
{
	...
	count = (wlan1_int_count[index] - wlan1_int_count[j]) + (wlan_int_count[index] - wlan_int_count[j]);	/* unit: number of interrupt occurred */

	if (tso_can_stop == 1) {
		if (count > suspend_check_high_water_mark) {
			_rtl86xx_dev.dev[0]->features &=
				~(NETIF_F_TSO | NETIF_F_TSO_ECN |
				NETIF_F_HW_CSUM | NETIF_F_SG | NETIF_F_GSO);
			tso_can_stop = 0;
		}
	} else {
		if (count < suspend_check_low_water_mark) {
			_rtl86xx_dev.dev[0]->features |=
				(NETIF_F_TSO | NETIF_F_TSO_ECN | NETIF_F_HW_CSUM
...
extern int rtl_mdio_read(unsigned int mdio_phyaddr, unsigned int reg,
	unsigned int *pdata);
int extphyid = 0x5;
#define BIT10F 	(1<<6)
#define BIT100H 	(1<<7)
	...
	if (regData & (1 << 2) && !(prelink & (1 << 2))) {
		...
		if (((regData1 >> 8) & (regData2 >> 10)) & (1 << 1)) {	/* force Giga */
			REG32(PCRP0) = (REG32(PCRP0) & ~AutoNegoSts_MASK)
				| (EnForceMode | ForceLink | ForceSpeed1000M |
				ForceDuplex);
		} else {
			for (rp = 0; rp < 2; rp++)
...
#endif
	{
		...
		if (first_time_read_reg6) {
			...
			for (phy = 0; phy < 5; phy++) {
				...
				rtl8651_setAsicEthernetPHYReg(phy, 1,
					val | 0x4);
				...
				if (phy_reg30[phy] != 0) {
					rtl8651_getAsicEthernetPHYReg(phy, 22,
						&val);
					rtl8651_setAsicEthernetPHYReg(phy, 22,
						((val & (0xff00)) | 0x17));
					rtl8651_getAsicEthernetPHYReg(phy, 30,
						&val);
		...
		if (++one_second_counter >= 10) {
			for (phy = 0; phy < 5; phy++) {
				if (phy_reg30[phy] != 0) {
					rtl8651_setAsicEthernetPHYReg(phy, 31,
						5);
					rtl8651_setAsicEthernetPHYReg(phy, 5,
						0);

					rtl8651_getAsicEthernetPHYReg(phy, 6,
						&val);
					rtl8651_setAsicEthernetPHYReg(phy, 31,
						0);
					...
					if (need_to_check_esd2) {
						rtl8651_setAsicEthernetPHYReg
							(phy, 31, 5);
						rtl8651_setAsicEthernetPHYReg
							(phy, 5, 0xfff6);

						rtl8651_getAsicEthernetPHYReg
							(phy, 6, &val);
						rtl8651_setAsicEthernetPHYReg
							(phy, 31, 0);
					...
					if (esd3_skip_one == 1) {
						phy_reg30[phy] =
							BIT(31) | (val & 0xfff);
					} else if ((phy_reg30[phy] & 0xfff) !=
						(val & 0xfff)) {
						if (diff_more_than_1((phy_reg30
									[phy] &
									0xf),
								(val & 0xf))
							||
...
#define panic_printk         printk
	 ...
	 */
	{
		 ...
		 * attribute definedin 30.5.1.1.4, aMediaAvailable.
		 */
		/* so we read it twice */
		...
		if ((rtl8211eLinkStsSpeed & BIT(2)) != (rData & BIT(2))) {	/* status changed */
			if (rData & BIT(2)) {	/* link up */
				rtl8651_getAsicEthernetPHYReg(P0_EXT_PHY_ID, 17,
					&rData);
	...
	if (((_br0_mask != 0) && (_br0_mask < 0xffffff00))
		&& (oldStatus & PPPOE_PASSTHRU_MASK)) {
		if (_rtl8651_getVlanTableEntry(PASSTHRU_VLAN_ID) == NULL) {
			...
			rtl865x_addVlanPortMember(PASSTHRU_VLAN_ID,
...
int32 rtd129x_set_phy_power_down(uint32 port, uint32 pwr_down)
{
	...
	case 4:
		break;
	case 5:
		if ((hwnat_mac5_conn_to != 0) || (hwnat_rgmii_enable == 0))
			goto out;
		break;
	default:
		pr_err("set PHY power down to unknown port %d\n", port);
		goto out;
	}

	rtl8651_setAsicEthernetPHYPowerDown(port, pwr_down);
out:
	return 0;
}
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
...
static int re865x_open(struct net_device *dev)
{
...
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
	{
...
#ifdef LINK_TASKLET
		tasklet_init(&cp->link_dsr_tasklet, interrupt_dsr_link,
			(unsigned long)cp);
		...
		memset(re865x_restartNWayCtrl, 0,
			sizeof(re865x_restartNWayCtrl));
#endif

#if defined(CONFIG_RTD_1295_HWNAT)
		rc = request_irq(dev->irq, interrupt_isr, IRQF_SHARED,
			dev->name, dev);
#else /* defined(CONFIG_RTD_1295_HWNAT) */
		rc = request_irq(dev->irq, interrupt_isr, IRQF_DISABLED,
			dev->name, dev);
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
		if (rc) {
...
#if defined(CONFIG_RTL_PUBLIC_SSID)
		rtl8198c_addIpv6Route(ipAddr, 0, nexthop,
			RTL_GW_WAN_DEVICE_NAME);
#else
		rtl8198c_addIpv6Route(ipAddr, 0, nexthop,
			RTL_DRV_WAN0_NETIF_NAME);
#endif
	}
#endif
	...
	if (rtl_get_ps_drv_netif_mapping_by_psdev_name(dev->name,
			drvNetif_name) == SUCCESS)
		rtl_enable_advRt_by_netifName(drvNetif_name);
#endif
	rtl865x_enableDevPortForward(dev, cp);

#if defined(CONFIG_RTD_1295_HWNAT)
	/* set PHY power up to enable traffic */
	for (port = 0; port <= RTL8651_PHY_NUMBER; port++) {
		if ((1 << port) & cp->portmask) {
			rtd129x_set_phy_power_down(port, false);
			break;
		}
	}

	if (cp->link_event == KOBJ_MAX) {
		newLinkPortMask = rtl865x_getPhysicalPortLinkStatus();
		if (cp->portmask & newLinkPortMask) {
			curLinkPortMask |= cp->portmask;
			cp->link_event = KOBJ_LINKUP;
			schedule_work(&cp->link_event_work);
		} else {
			cp->link_event = KOBJ_LINKDOWN;
...
#if defined(CONFIG_RTD_1295_HWNAT)
	/* set PHY power down to stop traffic */
	for (port = 0; port <= RTL8651_PHY_NUMBER; port++) {
		if ((1 << port) & cp->portmask) {
			rtd129x_set_phy_power_down(port, true);
			break;
		}
	}
	msleep(100);
	flush_work(&cp->link_event_work);
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
	...
	uint32 *portlist)
{
	...
	found = rtl865x_Lookup_fdb_entry(0, macAddr, FDB_DYNAMIC, &column,
		&fdbEntry);
	if (found == SUCCESS) {
		if (portlist)
...
#if defined(CONFIG_RTL_IGMP_SNOOPING)
int re865x_setMCastTxInfo(struct sk_buff *skb, struct net_device *dev,
	rtl_nicTx_info *nicTx)
{
	int32 ret;
	struct dev_priv *cp;
	...
	rtl_nicTx_info nicTx_wan;
#endif
#if definedCONFIG_RTL_PROCESS_PPPOE_IGMP_FOR_BRIDGE_FORWARD
	int type;
#endif
		...
		if (iph) {
			...
			if ((l4Protocol == IPPROTO_UDP)
				...
				memcpy(multicastDataInfo.groupAddr,
					&(iph->daddr), 4);
				multicastDataInfo.sourceIp[0] =
					ntohl(multicastDataInfo.sourceIp[0]);
				multicastDataInfo.groupAddr[0] =
					ntohl(multicastDataInfo.groupAddr[0]);
				/*
				 * multicastDataInfo.sourceIp[0]=  (uint32)(iph->saddr);
				 * multicastDataInfo.groupAddr[0]=  (uint32)(iph->daddr);
				 */
				ret = rtl_getMulticastDataFwdInfo
					(nicIgmpModuleIndex, &multicastDataInfo,
					&multicastFwdInfo);
#ifdef CONFIG_RTL_VLAN_8021Q
				if (linux_vlan_enable) {
					nicTx->portlist &=
...
#else
				nicTx->portlist =
					multicastFwdInfo.fwdPortMask & cp->
					portmask & ((1 << RTL8651_MAC_NUMBER) -
					1);
#endif
			...
			/* fix tim; upnp */
			if (vlan_bridge_multicast_tag) {
				if ((cp->id == vlan_bridge_tag)
					&& (nicTx->
						portlist & RTL_WANPORT_MASK)) {
					nicTx->portlist &= (~RTL_WANPORT_MASK);
					skb_wan = skb_copy(skb, GFP_ATOMIC);
					if (skb_wan != NULL) {
						nicTx_wan.txIdx = 0;
						nicTx_wan.vid =
							vlan_bridge_multicast_tag;
						nicTx_wan.portlist =
							RTL_WANPORT_MASK;
						nicTx_wan.srcExtPort = 0;
						nicTx_wan.flags =
						...
						nicTx_wan.tagport =
							RTL_WANPORT_MASK;
						RTL_CACHE_WBACK((unsigned long)
							skb_wan->data,
							skb_wan->len);
						if (RTL_swNic_send((void *)
								skb_wan,
								skb_wan->data,
								skb_wan->len,
								&nicTx_wan) <
		...
		if (ipv6h != NULL) {
			...
			if ((l4Protocol == IPPROTO_UDP)
				...
				multicastDataInfo.sourceIp[0] =
					ntohl(multicastDataInfo.sourceIp[0]);
				multicastDataInfo.sourceIp[1] =
					ntohl(multicastDataInfo.sourceIp[1]);
				multicastDataInfo.sourceIp[2] =
					ntohl(multicastDataInfo.sourceIp[2]);
				multicastDataInfo.sourceIp[3] =
					ntohl(multicastDataInfo.sourceIp[3]);
				multicastDataInfo.groupAddr[0] =
					ntohl(multicastDataInfo.groupAddr[0]);
				multicastDataInfo.groupAddr[1] =
					ntohl(multicastDataInfo.groupAddr[1]);
				multicastDataInfo.groupAddr[2] =
					ntohl(multicastDataInfo.groupAddr[2]);
				multicastDataInfo.groupAddr[3] =
					ntohl(multicastDataInfo.groupAddr[3]);
				ret = rtl_getMulticastDataFwdInfo
					(nicIgmpModuleIndex, &multicastDataInfo,
					&multicastFwdInfo);
				...
				if (linux_vlan_enable) {
					...
					if ((skb->passThruFlag &
						...
						&& (skb->
							...
							FLOOD_IN_BRIDGE))
						nicTx->portlist &=
							((1 << RTL8651_MAC_NUMBER) - 1) & (~(1 << src_port));
					else
						nicTx->portlist &=
							multicastFwdInfo.
							fwdPortMask & ((1 <<
...
struct net_device *re865x_get_netdev_by_name(const char *name)
{
	...
	for (i = 0; i < ETH_INTF_NUM; i++) {
		if (_rtl86xx_dev.dev[i]
			&& (strcmp(_rtl86xx_dev.dev[i]->name, name) == 0))
...
static inline int rtl_process_stp_tx(rtl_nicTx_info *txInfo)
{
	...
	if (!dev->irq) {
		...
		if (memcmp((void *)(dev->name), "port", 4) == 0) {
			if ((skb->data[0] & 0x01)
				&& !memcmp(&(skb->data[0]), STPmac, 5)
				&& !(skb->data[5] & 0xF0)) {
				stpPortNum =
					dev->name[strlen(dev->name) - 1] - '0';
				if (STP_PortDev_Mapping[stpPortNum] !=
					NO_MAPPING) {
...
#if defined(CONFIG_RTD_1295_HWNAT)
		if ((rtl865x_curOpMode == GATEWAY_MODE
			&& cp->id == vlanconfig[RTL_WAN_IDX].vid)
			|| (rtl865x_curOpMode == BRIDGE_MODE))
#else /* defined(CONFIG_RTD_1295_HWNAT) */
		if ((rtl865x_curOpMode == GATEWAY_MODE
			&& cp->id == vlanconfig[1].vid)
			|| (rtl865x_curOpMode == BRIDGE_MODE))
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
		{
...
static inline int rtl_process_rtk_vlan_tx(rtl_nicTx_info *txInfo)
{
	...
	struct sk_buff *newskb;
	struct dev_priv *cp;
#if	definedCONFIG_PPPOE_VLANTAG
...
int rtl_process_vlan_passthrough_tx(struct sk_buff **ppskb)
{
	...
	if (*(uint16 *)skb->vlan_passthrough_saved_tag ==
		...
		memcpy(skb->data + ETH_ALEN * 2,
			skb->vlan_passthrough_saved_tag, VLAN_HLEN);
	}
...
uint16 rtl_get_linux_vlan_src_tag_vid(struct sk_buff *skb)
{
	...
	if (*(uint16 *)skb->linux_vlan_src_tag == __constant_htons(ETH_P_8021Q)) {
		return ntohs(*(short *)(skb->linux_vlan_src_tag + 2)) & 0x0fff;
...
int rtl_process_linux_vlan_tx_check(rtl_nicTx_info *txInfo)
{
	...
	if (cp->id != rtk_vlan_wan_vid)	/* LAN tx. */
	{
		...
		if (*(unsigned short *)(skb->data + (ETH_ALEN << 1)) ==
			__constant_htons(ETH_P_8021Q)) {
			vid = ntohs(*(unsigned short *)(skb->data +
					(ETH_ALEN << 1) + 2));
			if ((vid & 0x0fff) == rtk_vlan_wan_vid) {
				vid = (vid & 0xf000) | RTL_LANVLANID;
				*(unsigned short *)(skb->data +
					(ETH_ALEN << 1) + 2) = htons(vid);
			}
		}
	}
	...
	if (cp->id == rtk_vlan_wan_vid)	/* WAN tx. */
	{
		...
		if (rtk_vlan_wan_vid != RTL_WANVLANID &&
			*(unsigned short *)(skb->linux_vlan_src_tag) ==
			__constant_htons(ETH_P_8021Q)) {
			vid = ntohs(*(unsigned short *)(skb->
					linux_vlan_src_tag + 2));
...
static inline int rtl_pstProcess_xmit(struct dev_priv *cp,
...
#endif
#if defined(CONFIG_RTL_VLAN_PASSTHROUGH_SUPPORT)
	retval = rtl_process_vlan_passthrough_tx((struct sk_buff **)&txInfo->
		out_skb);
	if (FAILED == retval)
		return retval;
#endif
...
#endif
{
	...
	txInfo->flags =
		(PKTHDR_USED | PKTHDR_HWLOOKUP | PKTHDR_BRIDGING |
		PKT_OUTGOING);
}
...
static inline int rtl_ip_option_check(struct sk_buff *skb)
{
	...
	if (((*((unsigned short *)(skb->data + ETH_ALEN * 2)) ==
			...
			&& ((*((unsigned char *)(skb->data + ETH_ALEN * 2 +
							2)) != 0x45)))
...
#ifndef CONFIG_RTL_VLAN_8021Q
static inline int rtl_isHwlookup(struct sk_buff *skb, struct dev_priv *cp,
	uint32 *portlist)
#else
static inline int rtl_isHwlookup(uint16 vid, struct sk_buff *skb,
	struct dev_priv *cp, uint32 *portlist)
#endif
{
	int flag = FALSE;
...
/* #if defined(CONFIG_RTL_MULTI_LAN_DEV) ||defined(CONFIG_POCKET_ROUTER_SUPPORT) */
	...
	*portlist = cp->portmask;
...
#endif
	{
		...
		int portNum = -1;
		if (*((uint16 *)(skb->data + (ETH_ALEN << 1))) ==
			__constant_htons(inband_ctl_acl_ethernet_type))
	...
	if ((rtk_vlan_support_enable == 0)
		&& ((!memcmp(cp->dev->name, "eth2", 5))
			|| (!memcmp(cp->dev->name, "eth3", 5))
			|| (!memcmp(cp->dev->name, "eth4", 5))))
		flag = FALSE;
#endif
#endif
...
static inline int rtl_fill_txInfo(rtl_nicTx_info *txInfo)
{
	...
	if (vlan_ctl_p->flag) {
		...
		if (skb_vid != 0) {
			...
			if (rtl865x_curOpMode == GATEWAY_MODE
				&& cp->id == rtk_vlan_wan_vid) {
				txInfo->vid =
					rtl_get_linux_vlan_src_tag_vid(skb);
...
#endif
	{
		...
		if (((skb->data[0] == 0x01) && (skb->data[1] == 0x00)
				&& (skb->data[2] == 0x5e))
#if defined(CONFIG_RTL_MLD_SNOOPING)
			|| ((skb->data[0] == 0x33) && (skb->data[1] == 0x33)
				&& (skb->data[2] != 0xFF))
#endif
#if defined(CONFIG_RTL_PROCESS_PPPOE_IGMP_FOR_BRIDGE_FORWARD)
			|| (type != 0)
	...
	/* for WAN Tag */
#ifdef CONFIG_RTK_VLAN_WAN_TAG_SUPPORT
	if ((vlan_bridge_tag && cp->id == vlan_bridge_tag) || (vlan_tag
			&& cp->id == vlan_tag))
...
#if defined(CONFIG_RTK_BRIDGE_VLAN_SUPPORT)
static int rtl_bridge_wan_start_xmit(struct sk_buff *skb,
	struct net_device *dev)
{
...
#if ((defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_8370_SUPPORT)) && !defined(CONFIG_RTL_CPU_TAG)) || defined(CONFIG_RTL_LINK_AGGREGATION)
...
#endif
#if defined(CONFIG_RTL_ISP_MULTI_WAN_SUPPORT)
static int rtl_insert_vlan_tag(struct sk_buff **skb, unsigned int vid,
	unsigned int priority)
{
	...
	if ((*skb)->mark & (1 << 16)) {	/* marked by IPQoS */
		insert[0] =
			(((*skb)->
				mark & 0x7) << 5) | (priority << 5) | (vid >>
			8);
	} else if (((*skb)->mark & 0x7) >= 1) {
		insert[0] =
			((((*skb)->mark & 0x7) -
				1) << 5) | (priority << 5) | (vid >> 8);
	} else
#endif
	...
	struct net_device *dev)
{
	...
	if (vpn_port > 0 && vpn_port < 65536) {
		if (skb->len > 43 && *((uint8 *)(skb->data + 23)) == 17
			&& *((uint16 *)(skb->data + 34)) == vpn_port
			&& (*((uint16 *)(skb->data + 20)) & ntohs(0x3fff)) == 0)
		...
		else if (skb->len > 51 && *((uint8 *)(skb->data + 31)) == 17
			&& *((uint16 *)(skb->data + 42)) == vpn_port
			&& (*((uint16 *)(skb->data + 28)) & ntohs(0x3fff)) ==
			0) {
			/* printk("\n%s calling rtk_openvpn_hw_encrypto()!\n",__FUNCTION__); */
	...
	if (((struct sk_buff *)tx_skb)->srcPhyPort ==
		QOS_PATCH_RX_FROM_WIRELESS) {
		if (strncmp(tx_skb->dev->name, RTL_DRV_WAN0_NETIF_NAME,
				IFNAMSIZ) == 0) {
			nicTx.priority =
				rtl_qosGetPriorityByMark(tx_skb->dev->name,
				skb->mark);
			nicTx.txIdx = RTL865X_SWNIC_TXRING_MAX_PKTDESC - 1;	/* use the highest tx ring index, note: not RTL865X_SWNIC_TXRING_HW_PKTDESC-1 */
		}
	}
...
#if defined(CONFIG_RTL_ETH_802DOT1X_SUPPORT)
	if ((dot1x_config.enable) && (rtl_is802dot1xFrame(skb->data))
		&& (!dot1x_config.enable_unicastresp)) {
	...
	if (enable_port_mapping) {
		...
		if ((tx_skb->from_dev && (tx_skb->from_dev->priv_flags & (IFF_DOMAIN_ELAN | IFF_DOMAIN_WLAN))) && (tx_skb->dev->priv_flags & IFF_DOMAIN_WAN)) {	/* upstream */
#if 0
			if (tx_skb->from_dev->priv_flags & IFF_DOMAIN_ELAN) {
				TOKEN_NUM(tx_skb->from_dev->name, &port);
				if (port > 1)
					port -= 1;
			} else if (tx_skb->from_dev->
				priv_flags & IFF_DOMAIN_WLAN) {
				TOKEN_NUM(tx_skb->from_dev->name, &port);
			}
#else
			port_mask =
				rtl_get_bind_port_mask_by_dev_name(tx_skb->
				from_dev->name);
#endif
			...
			&& (tx_skb->from_dev->priv_flags & IFF_DOMAIN_WAN)) {
	...
	if ((RTL_WANVLANID != nicTx.vid) && (RTL_LANVLANID != nicTx.vid)
		&& (RTL_BRIDGE_WANVLANID != nicTx.vid)) {
		if (!rtl_insert_vlan_tag(&tx_skb, nicTx.vid,
				tx_skb->vlan_tci >> 13)) {
#if defined(CONFIG_SMP)
			SMP_UNLOCK_ETH_XMIT(flags);
#endif
		...
		if (!rtl_insert_vlan_tag(&tx_skb, tx_skb->mark >> 20,
				tx_skb->mark & 0x7)) {
#if defined(CONFIG_SMP)
			SMP_UNLOCK_ETH_XMIT(flags);
#endif
	...
	{
		if (skb_shinfo(tx_skb)->nr_frags > 0)
			RTL_CACHE_WBACK((unsigned long)tx_skb->data,
				(tx_skb->len - tx_skb->data_len));
		else
			RTL_CACHE_WBACK((unsigned long)tx_skb->data,
				tx_skb->len);
	}
	...
	while (1) {
...
#else
#if defined(CONFIG_SMP)
		retval = _swNic_send((void *)tx_skb, (void *)mapping,
			tx_skb->len, &nicTx);
#else
		retval = swNic_send((void *)tx_skb, (void *)mapping,
			tx_skb->len, &nicTx);
#endif
#endif
#else /* defined(CONFIG_RTD_1295_HWNAT) */
		...
		if ((skb_shinfo(tx_skb)->nr_frags > 0)
			|| (skb_shinfo(tx_skb)->gso_size > 0)) {
			retval = New_swNic_send_tso_sg(tx_skb, tx_skb->data,
				tx_skb->len, &nicTx);
		} else {
			retval = New_swNic_send((void *)tx_skb, tx_skb->data,
				tx_skb->len, &nicTx);
		}
#else
		retval = New_swNic_send((void *)tx_skb, tx_skb->data,
...
#else
#if defined(CONFIG_SMP)
		retval = _swNic_send((void *)tx_skb, tx_skb->data, tx_skb->len,
			&nicTx);
#else
		retval = swNic_send((void *)tx_skb, tx_skb->data, tx_skb->len,
			&nicTx);
#endif
#endif
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
...
int rtl819x_get_port_status(int portnum, struct lan_port_status *port_status)
{
	...
	data0 = (regData & PortStatusLinkSpeed_MASK) >>
		PortStatusLinkSpeed_OFFSET;
...
int rtl819x_get_port_stats(int portnum, struct port_statistics *port_stats)
{
	...
	port_stats->rx_discard =
		rtl8651_returnAsicCounter(OFFSET_DOT1DTPPORTINDISCARDS_P0 +
		addrOffset_fromP0);
	port_stats->rx_error =
		(rtl8651_returnAsicCounter(OFFSET_DOT3STATSFCSERRORS_P0 +
			addrOffset_fromP0) +
	...
	port_stats->tx_error =
		...
		(OFFSET_DOT3STATSDEFERREDTRANSMISSIONS_P0 + addrOffset_fromP0));
...
int re865x_priv_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	case RTL819X_IOCTL_READ_PORT_STATUS:
		...
		if (copy_to_user((void *)rq->ifr_data, (void *)&port_status,
	...
	case RTL819X_IOCTL_READ_PORT_STATS:
		...
		if (rc != 0)
			return -EFAULT;
		if (copy_to_user((void *)rq->ifr_data, (void *)&port_stats,
				sizeof(struct port_statistics)))
			return -EFAULT;
		break;
		break;
	default:
...
void rtl_signal_linkdetect(void)
{
		...
		if (link_status_pid[i].valid) {
			...
			if (pid) {
				...
				if (task) {
					...
					panic_printk
						("Path selection daemon pid: %d does not exist\n",
						pid);
				}
				cnt++;
			}
	...
	if (cnt == 0)
		panic_printk
			("Path selection daemon pid: %d does not register\n",
			pid);
...
#else
static int32 rtk_linkstatus_pid_read(char *page, char **start, off_t off,
	int count, int *eof, void *data)
{
	int len = 0;
	int i = 0;
...
#endif
static int32 rtk_linkstatus_pid_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	char tmpbuf[64];
	char *tokptr, *strptr;
	...
	case RTL8651_IOCTL_GETWANLINKSTATUS:
		{
			...
			*pRet = FAILED;
			...
			rc = rtk_port_phyStatus_get(RTL83XX_WAN, &linkStatus,
				&speed, &duplex);
	...
	case RTL8651_IOCTL_GETWANLINKSTATUS:
		{
			...
			*pRet = FAILED;
			...
			totalVlans =
				((sizeof(vlanconfig)) /
				(sizeof(struct rtl865x_vlanConfig))) - 1;
			for (i = 0; i < totalVlans; i++) {
	...
	case RTL8651_IOCTL_GETWANLINKSPEED:
		{
			...
			*pRet = FAILED;
			...
			switch (READ_MEM32(PSRP0 +
					(i << 2)) & PortStatusLinkSpeed_MASK) {
	...
	case RTL8651_IOCTL_GETPORTIDBYCLIENTMAC:
		{
			...
			for (i = 0; i < RTL8651_PHY_NUMBER; i++) {
				if (memcmp(port_mac_table[i].client_mac,
						client_mac, ETH_ALEN) == 0) {
					port_id = port_mac_table[i].port_id;
					break;
				}
			}
			if (port_id > 0)
				copy_to_user(rq->ifr_data, &port_id,
					sizeof(port_id));
			break;
		}
#endif
...
static int rtl865x_set_hwaddr(struct net_device *dev, void *addr)
{
	...
	if (vlancfg_entry->vid != 0) {
		...
		memcpy(netif.macAddr.octet, vlancfg_entry->mac.octet,
			ETHER_ADDR_LEN);
...
static int rtl865x_set_link(struct net_device *dev, int enable)
{
	...
	if (portmask) {
		if (enable) {
			for (i = 0; i < RTL8651_PHY_NUMBER; i++) {
				if (portmask & (1 << i)) {
					rtl865xC_setAsicEthernetForceModeRegs(i,
						FALSE, TRUE, 1, TRUE);
					rtl8651_restartAsicEthernetPHYNway(i);
				}
			}
...
static int rtl865x_addAclForIPV6ReadyLogo(struct rtl865x_vlanConfig *vlanConfig)
{
	...
	for (i = 0; pVlanConfig[i].vid != 0; i++) {
		...
		if (pVlanConfig[i].isWan == 0) {	/*lan config */
			rtl865x_regist_aclChain(pVlanConfig[i].ifname,
				RTL865X_ACL_IPV6_USED, RTL865X_ACL_INGRESS);
			bzero((void *)&rule, sizeof(rtl865x_AclRule_t));
			...
			ntohl_array((u32 *)&rule.un_ty.L3V6._dstIpV6Addr.
				v6_addr32[0],
				(u32 *)&rule.un_ty.L3V6._dstIpV6Addr.
				v6_addr32[0], 4);

			rule.un_ty.L3V6._dstIpV6AddrMask.inv6_u.u6_addr32[0] =
				0xffffffff;
			rule.un_ty.L3V6._dstIpV6AddrMask.inv6_u.u6_addr32[1] =
				0xffffffff;
			rule.un_ty.L3V6._dstIpV6AddrMask.inv6_u.u6_addr32[2] =
				0xffffffff;
			rule.un_ty.L3V6._dstIpV6AddrMask.inv6_u.u6_addr32[3] =
				0xffffffff;

			ret = rtl865x_add_acl(&rule, pVlanConfig[i].ifname,
				RTL865X_ACL_IPV6_USED);
			...
			rule.ipv6EntryType_ = 1;
			/*fe80::/64 */
			rule.un_ty.L3V6._dstIpV6Addr.v6_addr16[0] =
				ntohs(0xfe80);
			rule.un_ty.L3V6._dstIpV6Addr.v6_addr16[1] = 0x00;
			rule.un_ty.L3V6._dstIpV6Addr.v6_addr16[2] = 0x00;
			rule.un_ty.L3V6._dstIpV6Addr.v6_addr16[3] = 0x00;
...
static int rtl865x_addAclForIgmpSnooping(struct rtl865x_vlanConfig *vlanConfig)
{
	...
	for (i = 0; pVlanConfig[i].vid != 0; i++) {
		...
		if (pVlanConfig[i].isWan == 0) {	/*lan config */
			...
			rtl865x_regist_aclChain(pVlanConfig[i].ifname,
				RTL865X_ACL_IGMP_USED, RTL865X_ACL_INGRESS);
			bzero((void *)&rule, sizeof(rtl865x_AclRule_t));
...
static int rtl865x_removeAclForIgmpSnooping(struct rtl865x_vlanConfig
	*vlanConfig)
{
#if defined(CONFIG_RTL_HARDWARE_MULTICAST)
	int i;
...
static int rtl865x_addAclForInbandCtl(struct rtl865x_vlanConfig *vlanConfig)
{
	...
	for (i = 0; pVlanConfig[i].vid != 0; i++) {
		...
		if (pVlanConfig[i].isWan == 0) {	/*lan config */
			rtl865x_regist_aclChain(pVlanConfig[i].ifname,
				RTL865X_ACL_TEST_USED, RTL865X_ACL_INGRESS);
			/*drop all packet */
			bzero((void *)&rule, sizeof(rtl865x_AclRule_t));
...
static int rtl865x_addAclForHttpRedirect(struct rtl865x_vlanConfig *vlanConfig)
{
	...
	for (i = 0; pVlanConfig[i].vid != 0; i++) {
		...
		if (pVlanConfig[i].isWan == 0) {	/*lan config */
			rtl865x_regist_aclChain(pVlanConfig[i].ifname,
				RTL865X_ACL_HTTP_REDIRECT_USED,
				RTL865X_ACL_INGRESS);
			/*to cpu */
			bzero((void *)&rule, sizeof(rtl865x_AclRule_t));
...
static int rtl865x_removeAclForHttpRedirect(struct rtl865x_vlanConfig
	*vlanConfig)
{
	int i;
#if defined(CONFIG_RTL_MULTI_LAN_DEV) || defined(CONFIG_RTK_VLAN_SUPPORT) || defined(CONFIG_RTL_VLAN_8021Q)
...
static int rtl865x_removeAclForMldSnooping(struct rtl865x_vlanConfig
	*vlanConfig)
{
#if (defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)) && defined(CONFIG_RTL_HARDWARE_MULTICAST)
...
static ssize_t mib_vlan_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t up_event_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
{
	return (single_open(file, rtk_link_status_read,
			(void *)PDE_DATA(inode)));
}

static ssize_t link_status_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t auto_dhcp_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t hwL3v6_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t rtk_vlan_support_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t rtk_vlan_management_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t rtk_vlan_wan_tag_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t rtk_inband_ctl_acl_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static int dslite_hw_fw_write(struct file *file, const char *buffer,
	unsigned long count, void *data)
{
	uint32 tmpBuf[32];
	uint32 uintVal;
...
static ssize_t rtk_dslite_hw_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static int sixrd_hw_fw_write(struct file *file, const char *buffer,
	unsigned long count, void *data)
{
	uint32 tmpBuf[32];
	uint32 uintVal;
...
static ssize_t rtk_6rd_hw_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t rtl_hw_vlan_support_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
{
	return (single_open(file, rtl_hw_vlan_tagged_bridge_multicast_read,
			NULL));
}

static ssize_t rtl_hw_vlan_tagged_mc_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
	return rtl_hw_vlan_tagged_bridge_multicast_write(file, userbuf, count,
		off);
}
...
static ssize_t rtk_multicast_scream_vid_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
int rtk_query_for_bridge_port_single_open(struct inode *inode,
	struct file *file)
{
	return (single_open(file, rtk_query_for_bridge_port_read, NULL));
}

static ssize_t rtk_query_for_bridge_port_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t rtl_8367r_vlan_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
	return 0;
}
...
static ssize_t rtl_phy_status_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t port_mibEntry_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t tr181_eth_link_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t tr181_eth_if_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t tr181_eth_stats_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t tr181_eth_set_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t custom_Passthru_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t StormCtrl_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static int rtl865x_extendStormCtrlWriteProc(struct file *file,
...
static ssize_t extendStormCtrl_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t wan_port_setting_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static int32 proc_phyTest_write(struct file *filp, const char *buff,
	unsigned long len1, void *data);
/*phyRegTest*/
...
static ssize_t phyRegTest_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t br_mldsnoop_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t phyPower_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static int32 port_state_write_proc(struct file *filp, const char *buff,
	unsigned long len, void *data);
/*portState*/
...
static ssize_t portState_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static int rtl865x_reInitSwitchCoreWriteProc(struct file *file,
	const char *buffer, unsigned long count, void *data);
/*reInitSwitchCore*/
...
static ssize_t reInitSwitchCore_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static int rtl_check_swCore_tx_hang_write_proc(struct file *file,
	const char *buffer, unsigned long count, void *data);
...
static ssize_t rtl_check_swCore_tx_hang_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t sockDebug_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static int jate_write_proc(struct file *file, const char *buffer,
	unsigned long count, void *data);
/*jate*/
...
#endif
#else
static int eee_read_proc(char *page, char **start, off_t off, int count,
	int *eof, void *data);
static int eee_write_proc(struct file *file, const char *buffer,
	unsigned long count, void *data);
#ifdef CONFIG_RTL_JATE_TEST
static int jate_read_proc(char *page, char **start, off_t off, int count,
	int *eof, void *data);
static int jate_write_proc(struct file *file, const char *buffer,
	unsigned long count, void *data);
#endif
...
/*2007-12-19*/
...
#if defined(CONFIG_TR181_ETH)
	struct proc_dir_entry *tr181_eth_link, *tr181_eth_if, *tr181_eth_stats,
		*tr181_eth_set;
#endif
#endif
...
#endif
	memset(hw_vlan_info, 0,
		PORT_NUMBER * sizeof(struct hw_vlan_port_setting));
#endif
#endif
	...
	if (hwnat_mac0_enable == 0) {
		vlanconfig[2].if_type = IF_NONE;
		vlanconfig[2].memPort = 0;
		RTL_LANPORT_MASK &= ~RTL_LANPORT_MASK_1;
	}
	...
	if (hwnat_gpio_link_led_enable) {
		hwnat_gpio_link_led = of_get_gpio(pdev->dev.of_node, 0);
	}
	 ...
	 * pkt count = 32 */
	WRITE_MEM32(CPUIMTTR0, 0x320C8320);
	WRITE_MEM32(CPUIMTTR1, 0x320C8320);
	WRITE_MEM32(CPUIMTTR2, 0x320C8320);
	WRITE_MEM32(CPUIMTTR3, 0x00000320);
...
#if defined(CONFIG_FPGA_V6_HWNAT)
	{
		...
		phy_db_addr =
			((uint32)rtl_hwnat_mmio) + HWNAT_CPUIR_OFFSET + 0x9100;
...
#endif /* !defined(CONFIG_RTD_1295_HWNAT) */
	{
		...
		if (RTL865X_PORTMASK_UNASIGNED == rtl865x_wanPortMask) {
			...
			assert(RTL865X_PORTMASK_UNASIGNED ==
				rtl865x_lanPortMask);
			...
			if (REG32(BOND_OPTION) == 0x0a) {	/* Check is RTL8881AM */
				...
				if ((REG32(PEFGH_DAT) & 0x80) == 0x80) {	/* Pull high */
					...
					vlanconfig[0].memPort =
						vlanconfig[0].untagSet = 0x110;
					vlanconfig[1].memPort =
						vlanconfig[1].untagSet = 0;
					if (totalVlans >= 3)
						vlanconfig[2].memPort =
							vlanconfig[2].untagSet =
							0;
				}
			}
		 ...
		 */
		{
			if (RTL865X_PREALLOC_SKB_UNASIGNED ==
				rtl865x_maxPreAllocRxSkb) {
				...
				assert(rtl865x_rxSkbPktHdrDescNum !=
					RTL865X_PREALLOC_SKB_UNASIGNED);
				assert(rtl865x_txSkbPktHdrDescNum !=
					RTL865X_PREALLOC_SKB_UNASIGNED);
...
#if defined(CONFIG_RTD_1295_HWNAT)
	if (hwnat_gpio_link_led_enable) {
		if (gpio_request(hwnat_gpio_link_led, "NAT link LED") < 0)
			pr_err("NAT: fail to request GPIO pin %d\n", hwnat_gpio_link_led);
	}
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
...
#if defined(CONFIG_RTL_SWITCH_NEW_DESCRIPTOR)
	REG32(CPUICR1) =
		(REG32(CPUICR1) & ~CF_PKT_HDR_TYPE_MASK) |
		TX_PKTHDR_SHORTCUT_LSO;
#if defined(CONFIG_RTL_JUMBO_FRAME)
	REG32(CPUICR1) |= BIT(CF_RX_GATHER_OFFSET);
#endif
#endif
...
/* rtlglue_printf("        creating eth0~eth%d...\n",totalVlans-1 ); */
...
#endif
	memset(&mCastSnoopingGlobalConfig, 0,
		sizeof(struct rtl_mCastSnoopingGlobalConfig));
	...
	for (i = 0; i < totalVlans; i++) {
		...
		if (vlanconfig[i].isWan
			&& (strncmp((vlanconfig[i].ifname),
					RTL_DRV_WAN0_NETIF_NAME, IFNAMSIZ)))
			continue;
#endif
		...
		dp->plat_dev = pdev;
		INIT_WORK(&dp->link_event_work, rtd129x_send_uevent_work);
		dp->link_event = KOBJ_MAX;
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
		...
		memcpy((char *)dev->dev_addr, (char *)(&(vlanconfig[i].mac)),
			ETHER_ADDR_LEN);
#if defined(CONFIG_RTL_ISP_MULTI_WAN_SUPPORT)
		if (vlanconfig[i].isWan == 0) {
#if defined(CONFIG_RTL_MULTI_LAN_DEV)
			if (strncmp((vlanconfig[i].ifname),
					RTL_DRV_LAN_P0_NETIF_NAME,
					IFNAMSIZ) == 0)
				strncpy(dev->name, RTL_DRV_ETHLAN_P0_NETIF_NAME,
					IFNAMSIZ);
			else
				strncpy(dev->name, vlanconfig[i].ifname,
					IFNAMSIZ);
#endif
...
#if defined(CONFIG_RTK_BRIDGE_VLAN_SUPPORT)
		if (memcmp((vlanconfig[i].ifname), RTL_DRV_LAN_P7_NETIF_NAME,
				4) == 0) {
			memcpy((char *)dev->name,
				(char *)(&(vlanconfig[i].ifname)), 5);
		...
		if (!rc) {
			...
			/*2007-12-19 */
			rtlglue_printf
				("eth%d added. vid=%d Member port 0x%x...\n", i,
...
#if defined(CONFIG_RTK_VLAN_SUPPORT)
#ifdef CONFIG_RTL_PROC_NEW
		proc_create_data("mib_vlan", 0644, res_stats_root,
			&mib_vlan_proc_fops, (void *)dev);
#else
		if ((res_stats = create_proc_read_entry("mib_vlan", 0644,
					res_stats_root, read_proc_vlan,
					(void *)dev)) == NULL) {
		...
		rtk_linkEvent = 0;
#ifdef CONFIG_RTL_PROC_NEW
		proc_create_data("up_event", 0, res_stats_root,
			&up_event_proc_fops, NULL);
		proc_create_data("link_status", 0644, res_stats_root,
			&link_status_proc_fops, (void *)dev);
#else /*CONFIG_RTL_PROC_NEW */
		rtk_link_event_entry =
		...
		if ((rtk_link_status_entry =
				create_proc_read_entry("link_status", 0644,
					res_stats_root, rtk_link_status_read,
					(void *)dev)) == NULL) {
			printk("create_proc_read_entry failed!\n");
		}
	...
	if (retVal == SUCCESS) {
		...
		devInfo.swPortMask =
			devInfo.portMask & (~((1 << RTL8651_MAC_NUMBER) - 1));
	...
	for (i = 0; i < MAX_RE865X_ETH_STP_PORT; i++) {
		...
		memcpy((char *)dev->dev_addr, (char *)(&(vlanconfig[0].mac)),
			ETHER_ADDR_LEN);
		dp = NETDRV_PRIV(dev);
		memset(dp, 0, sizeof(*dp));
		dp->dev = dev;
	...
	((struct dev_priv *)(NETDRV_PRIV(_rtl86xx_dev.dev[0])))->dev_next =
		_rtl86xx_dev.dev[1];
	((struct dev_priv *)(NETDRV_PRIV(_rtl86xx_dev.dev[1])))->dev_prev =
		_rtl86xx_dev.dev[0];
	...
	proc_create_data("rtk_linkstatus_pid", 0, &proc_root,
		&rtk_auto_dhcp_proc_fops, NULL);
#else
	rtl_link_status_pid_init();
...
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("rtk_vlan_support", 0, &proc_root,
		&rtk_vlan_support_proc_fops, NULL);
#else
...
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("rtk_vlan_management_entry", 0, &proc_root,
		&rtk_vlan_management_proc_fops, NULL);
#else
	rtk_vlan_management_entry =
		create_proc_entry("rtk_vlan_management_entry", 0, NULL);
	if (rtk_vlan_management_entry) {
		rtk_vlan_management_entry->read_proc = rtk_vlan_management_read;
		rtk_vlan_management_entry->write_proc =
			rtk_vlan_management_write;
	}
#endif
#endif
#endif
...
#ifdef CONFIG_RTK_VLAN_WAN_TAG_SUPPORT
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("rtk_vlan_wan_tag", 0, &proc_root,
		&rtk_vlan_wan_tag_proc_fops, NULL);
#else
	rtk_vlan_wan_tag_support_entry =
		create_proc_entry("rtk_vlan_wan_tag", 0, NULL);
	if (rtk_vlan_wan_tag_support_entry) {
		rtk_vlan_wan_tag_support_entry->read_proc =
			rtk_vlan_wan_tag_support_read;
		rtk_vlan_wan_tag_support_entry->write_proc =
			rtk_vlan_wan_tag_support_write;
	}
#endif
#endif
...
#if defined(CONFIG_RTL_JUMBO_FRAME)
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("jumbo_frame_support", 0, &proc_root,
		&jumbo_frame_proc_fops, NULL);
#else
	jumbo_frame_support_entry =
		create_proc_entry("jumbo_frame_support", 0, NULL);
	if (jumbo_frame_support_entry) {
		jumbo_frame_support_entry->read_proc = jumbo_frame_support_read;
		jumbo_frame_support_entry->write_proc =
			jumbo_frame_support_write;
	}
#endif
#endif
...
#if defined(CONFIG_RTL_LINK_AGGREGATION)
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("link_aggregation", 0, &proc_root,
		&link_aggregation_proc_fops, NULL);
#else
...
#if defined(CONFIG_RTL_HW_VLAN_SUPPORT)
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("rtl_hw_vlan_support", 0, &proc_root,
		&rtl_hw_vlan_support_proc_fops, NULL);
	proc_create_data("rtl_hw_vlan_tagged_mc", 0, &proc_root,
		&rtl_hw_vlan_tagged_mc_proc_fops, NULL);
#ifdef CONFIG_RTL_HW_VLAN_SUPPORT_HW_NAT
	proc_create_data("rtk_multicast_scream_vid", 0, &proc_root,
		&rtk_multicast_scream_vid_proc_fops, NULL);
	proc_create_data("rtk_query_for_bridge_port", 0, &proc_root,
		&rtk_query_for_bridge_port_proc_fops, NULL);
#endif
#else
	rtl_hw_vlan_support_entry =
		create_proc_entry("rtl_hw_vlan_support", 0, NULL);
	if (rtl_hw_vlan_support_entry) {
		rtl_hw_vlan_support_entry->read_proc = rtl_hw_vlan_support_read;
		rtl_hw_vlan_support_entry->write_proc =
			rtl_hw_vlan_support_write;
	}
	rtl_hw_vlan_tagged_bridge_multicast_entry =
		create_proc_entry("rtl_hw_vlan_tagged_mc", 0, NULL);
	if (rtl_hw_vlan_tagged_bridge_multicast_entry) {
		rtl_hw_vlan_tagged_bridge_multicast_entry->read_proc =
			rtl_hw_vlan_tagged_bridge_multicast_read;
		rtl_hw_vlan_tagged_bridge_multicast_entry->write_proc =
			rtl_hw_vlan_tagged_bridge_multicast_write;
	}
#ifdef CONFIG_RTL_HW_VLAN_SUPPORT_HW_NAT
	rtk_multicast_scream_vid_entry =
		create_proc_entry("rtk_multicast_scream_vid", 0, NULL);
	if (rtk_multicast_scream_vid_entry) {
		rtk_multicast_scream_vid_entry->read_proc =
			rtk_multicast_scream_vid_read;
		rtk_multicast_scream_vid_entry->write_proc =
			rtk_multicast_scream_vid_write;
	}

	rtk_query_for_bridge_port_entry =
		create_proc_entry("rtk_query_for_bridge_port", 0, NULL);
	if (rtk_query_for_bridge_port_entry) {
		rtk_query_for_bridge_port_entry->read_proc =
			rtk_query_for_bridge_port_read;
		rtk_query_for_bridge_port_entry->write_proc =
			rtk_query_for_bridge_port_write;
	}
#endif
#endif
#endif
...
#if defined(CONFIG_RTL_8367R_SUPPORT)
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("rtl_8367r_vlan", 0, &proc_root,
		&rtl_8367r_vlan_proc_fops, NULL);
#else
...
/* #if defined(CONFIG_RTK_VLAN_FOR_CABLE_MODEM) */
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("rtl_phy_status", 0, &proc_root,
		&rtl_phy_status_proc_fops, NULL);
#else
...
#ifdef CONFIG_RTL_PROC_NEW
		proc_create_data(port_mibEntry_name, 0, port_mibStats_root,
			&port_mibEntry_proc_fops, (void *)portnum);
#else
		port_mibStats_entry =
			create_proc_entry(port_mibEntry_name, 0,
			port_mibStats_root);
		port_mibStats_entry->data = (void *)portnum;
		if (port_mibStats_entry) {
			port_mibStats_entry->read_proc =
				port_mibStats_read_proc;
			port_mibStats_entry->write_proc =
				port_mibStats_write_proc;
		}
#endif
	}
#endif /* #if defined(CONFIG_819X_PHY_RW) */
...
#if defined(CONFIG_TR181_ETH)
	{
...
#ifdef CONFIG_RTL_PROC_NEW
		proc_create_data("tr181_eth_link", 0, rtl865x_proc_dir,
			&tr181_eth_link_proc_fops, NULL);
		proc_create_data("tr181_eth_if", 0, rtl865x_proc_dir,
			&tr181_eth_if_proc_fops, NULL);
		proc_create_data("tr181_eth_stats", 0, rtl865x_proc_dir,
			&tr181_eth_stats_proc_fops, NULL);
		proc_create_data("tr181_eth_set", 0, rtl865x_proc_dir,
			&tr181_eth_set_proc_fops, NULL);
#else
		tr181_eth_link =
			create_proc_entry("tr181_eth_link", 0,
			rtl865x_proc_dir);
		if (tr181_eth_link) {
		...
		tr181_eth_if =
			create_proc_entry("tr181_eth_if", 0, rtl865x_proc_dir);
		if (tr181_eth_if) {
			tr181_eth_if->read_proc = tr181_eth_if_read;
			tr181_eth_if->write_proc = tr181_eth_if_write;
		}
		tr181_eth_stats =
			create_proc_entry("tr181_eth_stats", 0,
			rtl865x_proc_dir);
		if (tr181_eth_stats) {
		...
		tr181_eth_set =
			create_proc_entry("tr181_eth_set", 0, rtl865x_proc_dir);
		if (tr181_eth_set) {
...
#ifdef CONFIG_RTL_INBAND_CTL_ACL
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("inband_ctl_acl", 0, &proc_root,
		&rtk_inband_ctl_acl_proc_fops, NULL);
#else
...
#if defined(CONFIG_RTL_HW_DSLITE_SUPPORT)
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("dslite_hw_fw", 0, &proc_root,
		&rtk_dslite_hw_forward_proc_fops, NULL);
#endif
#endif
#if defined(CONFIG_RTL_HW_6RD_SUPPORT)
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("6rd_hw_fw", 0, &proc_root,
		&rtk_6rd_hw_forward_proc_fops, NULL);
#endif
#endif
#endif
	...
	cpuQosHoldLow = 0;
	totalLowQueueCnt = 0;
	memset(pktQueueByPri, 0,
		sizeof(rtl_queue_entry) * (RTL865X_SWNIC_RXRING_MAX_PKTDESC));
#endif

	rtl865x_config_callback_for_get_drv_netifName
		(rtl_get_ps_drv_netif_mapping_by_psdev_name);
#if defined(REINIT_SWITCH_CORE)
...
static inline unsigned char *get_buf_from_poll(struct list_head *phead,
	unsigned int *count)
{
	unsigned long flags = 0;
	unsigned char *buf;
...
static inline void release_buf_to_poll(unsigned char *pbuf,
	struct list_head *phead, unsigned int *count)
{
	unsigned long flags = 0;
	struct list_head *plist;
	...
	*count = *count + 1;
	plist = (struct list_head *)((unsigned int)pbuf -
...
#if defined(DELAY_REFILL_ETH_RX_BUF) || defined(ALLOW_RX_RING_PARTIAL_EMPTY)
	if (((atomic_read(&rtl_devOpened) > 0)
			&& (FAILED == return_to_rx_pkthdr_ring(head)))
		|| (atomic_read(&rtl_devOpened) == 0))
#endif
	{
		release_buf_to_poll(head, &eth_skbbuf_list,
...
__MIPS16 __IRAM_FWD int is_rtl865x_eth_priv_buf(unsigned char *head)
{
	unsigned long offset =
		(unsigned long)(&((struct priv_skb_buf2 *)0)->buf);
	struct priv_skb_buf2 *priv_buf =
		(struct priv_skb_buf2 *)(((unsigned long)head) - offset);
...
#endif				/* defined(CONFIG_RTD_1295_HWNAT) */
{
	...
	for (i = 0; i < ETH_INTF_NUM; i++) {
		...
		if (skb_queue_len(&cp->rx_napi_skb_queue)) {
			printk(KERN_WARNING "rx_napi_skb_queue not empty\n");
...
static bool rtl_hwnat_enable_orig;
...
static int rtd129x_nat_resume(struct device *dev)
{
	...
	rtl_hwnat_enable = rtl_hwnat_enable_orig;
...
/*layer4*/
	...
	/*queue id & rx ring descriptor mapping */
	REG32(CPUQDM0) =
		QUEUEID1_RXRING_MAPPING | (QUEUEID0_RXRING_MAPPING << 16);
	REG32(CPUQDM2) =
		QUEUEID3_RXRING_MAPPING | (QUEUEID2_RXRING_MAPPING << 16);
	REG32(CPUQDM4) =
		QUEUEID5_RXRING_MAPPING | (QUEUEID4_RXRING_MAPPING << 16);
...
@comm
	struct rtl865x_vlanConfig is definedas follows:
...
int32 rtl865x_config(struct rtl865x_vlanConfig vlanconfig[])
{
	...
	for (i = 0; vlanconfig[i].vid != 0; i++) {
		...
		if (vlanconfig[i].if_type == IF_ETHER) {
			...
			if (retval == SUCCESS) {
				rtl865x_addVlanPortMember(vlanconfig[i].vid,
					vlanconfig[i].
					memPort & valid_port_mask);

#if (defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_8370_SUPPORT)) && !defined(CONFIG_RTL_CPU_TAG)
				/* remove untagPortMask in 8197D hardware VLAN table */
				rtl865x_setVlanPortTag(vlanconfig[i].vid,
					vlanconfig[i].memPort, TRUE);
#endif
				rtl865x_setVlanFilterDatabase(vlanconfig[i].vid,
					vlanconfig[i].fid);
			}
#if defined(CONFIG_RTL_ISP_MULTI_WAN_SUPPORT)
		}
		...
		memcpy(netif.macAddr.octet, vlanconfig[i].mac.octet,
			ETHER_ADDR_LEN);
		...
		if (netif.is_slave == 1)
#if defined(CONFIG_RTL_PUBLIC_SSID)
			rtl865x_attachMasterNetif(netif.name,
				RTL_GW_WAN_DEVICE_NAME);
#else
			rtl865x_attachMasterNetif(netif.name,
				RTL_DRV_WAN0_NETIF_NAME);
...
#endif
#if defined(CONFIG_PROC_FS) && defined(CONFIG_NET_SCHED) && defined(CONFIG_RTL_LAYERED_DRIVER)
		memcpy(&netIfName[i][0], vlanconfig[i].ifname,
			sizeof(vlanconfig[i].ifname));
#endif

#if defined(CONFIG_RTL_UNKOWN_UNICAST_CONTROL)
		if (vlanconfig[i].isWan == 0)
			memcpy(lanIfName, vlanconfig[i].ifname,
				sizeof(vlanconfig[i].ifname));
#endif
	...
	if ((++__865X_Config) == 1) {
		for (i = 0; i < RTL8651_PORT_NUMBER + 3; i++) {
			...
			if (pvid != 0) {
				...
				/*2007-12-19 */
				rtlglue_printf
					("%s:%d:lrconfig[j].vid is %d,pvid is %d, j is %d,i is %d\n",
					__FUNCTION__, __LINE__,
					vlanconfig[j].vid, pvid, j, i);
#endif
...
static void rtl_unkownUnicastTimer(unsigned long data)
{
	...
	ret = rtl865x_del_acl(&rule, RTL_DRV_LAN_NETIF_NAME,
		RTL865X_ACL_IPV6_USED);
...
static void rtl_clearUnknownUnicastCnt(void)
{
	...
	for (idx = 0; idx < RTL_MAC_RECORD_NUM; idx++) {
		if ((macRecord[idx].enable == 1)
			&& (macRecord[idx].acl_config == 0))
			macRecord[idx].cnt = 0;
	}
	spin_unlock_bh(&unknown_unicast_lock);
...
static void rtl_unkownUnicastUpdate(struct sk_buff *skb)
{
...
#if defined(CONFIG_RTD_1295_HWNAT)
	if (ether_addr_equal(mac, skb->dev->dev_addr))
		return;
#else /* defined(CONFIG_RTD_1295_HWNAT) */
	...
	for (idx = 0; idx < RTL_MAC_RECORD_NUM; idx++) {
		...
		found = 1;
		if ((macRecord[idx].cnt == RTL_MAC_THRESHOLD)
			|| (++macRecord[idx].cnt < RTL_MAC_THRESHOLD)) {
			spin_unlock_bh(&unknown_unicast_lock);
			return;
		}
	...
	if (found == 0) {
		...
		ret = rtl865x_add_acl(&rule, RTL_DRV_LAN_NETIF_NAME,
			RTL865X_ACL_IPV6_USED);
		if (ret == RTL_EENTRYNOTFOUND) {
			rtl865x_regist_aclChain(RTL_DRV_LAN_NETIF_NAME,
				RTL865X_ACL_IPV6_USED, RTL865X_ACL_INGRESS);
			ret = rtl865x_add_acl(&rule, RTL_DRV_LAN_NETIF_NAME,
				RTL865X_ACL_IPV6_USED);
		}
...
#if defined(CONFIG_RTL_IGMP_SNOOPING)
extern int32 rtl_setIgmpSnoopingModuleStaticRouterPortMask(uint32 moduleIndex,
...
static int re865x_reInitIgmpSetting(int mode)
{
	...
	for (i = 0; i < totalVlans; i++) {
		...
		if (mode == GATEWAY_MODE) {
			rtl_setIgmpSnoopingModuleStaticRouterPortMask
				(nicIgmpModuleIndex, 0);
		} else {
			rtl_setIgmpSnoopingModuleStaticRouterPortMask
				(nicIgmpModuleIndex, 0x01);
		}
...
static int rtl_config_perport_perdev_vlanconfig(int mode)
{
	...
	if (hwnat_mac0_enable > 0) {
		...
		vlanconfig[2].memPort = RTL_LANPORT_MASK_1;
		vlanconfig[2].untagSet = RTL_LANPORT_MASK_1;
	...
	if ((mode == BRIDGE_MODE) || (mode == WISP_MODE)) {
		...
		((struct dev_priv *)NETDRV_PRIV(_rtl86xx_dev.dev[1]))->id =
			RTL_WANVLANID;
#else
		vlanconfig[1].vid = RTL_LANVLANID;
		((struct dev_priv *)NETDRV_PRIV(_rtl86xx_dev.dev[1]))->id =
...
static int rtl_config_lanwan_dev_vlanconfig(int mode)
{
	...
	if (mode == GATEWAY_MODE) {
		...
		((struct dev_priv *)NETDRV_PRIV(_rtl86xx_dev.dev[0]))->id =
			RTL_LANVLANID;
		vlanconfig[1].vid = RTL_WANVLANID;
		vlanconfig[1].memPort = RTL_WANPORT_MASK;
		vlanconfig[1].untagSet = RTL_WANPORT_MASK;
...
int rtl_is_lan_dev(struct net_device *dev)
{
	...
	if (rtl865x_curOpMode == GATEWAY_MODE) {
		if ((strcmp(dev->name, "eth0") == 0)
			|| (strcmp(dev->name, RTL_DRV_LAN_P1_NETIF_NAME) == 0)
...
static int32 reinit_vlan_configure(struct rtl865x_vlanConfig new_vlanconfig[])
{
	...
	for (i = 0; i < totalVlans; i++) {
		...
		if (pvlanconfig[i].if_type == IF_ETHER) {
			...
			if (pvlanconfig[i].isWan) {
				rtl865x_addVlanPortMember(pvlanconfig[i].vid,
					(pvlanconfig[i].
						memPort & valid_port_mask) |
					0x100);
			} else
...
#if (defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_8370_SUPPORT)) && !defined(CONFIG_RTL_CPU_TAG)
			/* remove untagPortMask in 8197D hardware VLAN table */
			rtl865x_setVlanPortTag(pvlanconfig[i].vid,
				pvlanconfig[i].memPort, TRUE);
#endif
			rtl865x_setVlanFilterDatabase(pvlanconfig[i].vid,
				pvlanconfig[i].fid);
		}
		...
		memcpy(netif.macAddr.octet, pvlanconfig[i].mac.octet,
			ETHER_ADDR_LEN);
	...
	if (rtl_hw_vlan_enable) {
		...
		if ((hw_vlan_info[0].vlan_port_enabled)
			&& (hw_vlan_info[0].vlan_port_bridge == 1)) {
#ifdef CONFIG_RTL_HW_VLAN_SUPPORT_HW_NAT
			if (hw_vlan_info[0].vlan_port_tag == 0) {
				if (RTK_NAT_WAN_TAGGED) {
					rtl8651_setAsicPvid(RTK_WAN_PORT_IDX,
						hw_vlan_info[0].vlan_port_vid);
			...
			rtl865x_addVlanPortMember(hw_vlan_info[0].vlan_port_vid,
				RTL_LANPORT_MASK_4 | RTL_WANPORT_MASK);
			...
			rtl865x_setVlanFilterDatabase(hw_vlan_info[0].
				vlan_port_vid, 0);
		...
		if ((hw_vlan_info[2].vlan_port_enabled)
			&& (hw_vlan_info[2].vlan_port_bridge == 1)) {
#ifdef CONFIG_RTL_HW_VLAN_SUPPORT_HW_NAT
			if (hw_vlan_info[2].vlan_port_tag == 0) {
				if (RTK_NAT_WAN_TAGGED) {
					rtl8651_setAsicPvid(RTK_WAN_PORT_IDX,
						hw_vlan_info[2].vlan_port_vid);
			...
			rtl865x_addVlanPortMember(hw_vlan_info[2].vlan_port_vid,
				RTL_LANPORT_MASK_3 | RTL_WANPORT_MASK);
			...
			rtl865x_setVlanFilterDatabase(hw_vlan_info[2].
				vlan_port_vid, 0);
		...
		if ((hw_vlan_info[3].vlan_port_enabled)
			&& (hw_vlan_info[3].vlan_port_bridge == 1)) {
#ifdef CONFIG_RTL_HW_VLAN_SUPPORT_HW_NAT
			if (hw_vlan_info[3].vlan_port_tag == 0) {
				if (RTK_NAT_WAN_TAGGED) {
					rtl8651_setAsicPvid(RTK_WAN_PORT_IDX,
						hw_vlan_info[3].vlan_port_vid);
			...
			rtl865x_addVlanPortMember(hw_vlan_info[3].vlan_port_vid,
				RTL_LANPORT_MASK_2 | RTL_WANPORT_MASK);
			...
			rtl865x_setVlanFilterDatabase(hw_vlan_info[3].
				vlan_port_vid, 0);
		...
		if ((hw_vlan_info[4].vlan_port_enabled)
			&& (hw_vlan_info[4].vlan_port_bridge == 1)) {
#ifdef CONFIG_RTL_HW_VLAN_SUPPORT_HW_NAT
			if (hw_vlan_info[4].vlan_port_tag == 0) {
				if (RTK_NAT_WAN_TAGGED) {
					rtl8651_setAsicPvid(RTK_WAN_PORT_IDX,
						hw_vlan_info[4].vlan_port_vid);
			...
			rtl865x_addVlanPortMember(hw_vlan_info[4].vlan_port_vid,
				RTL_LANPORT_MASK_1 | RTL_WANPORT_MASK);
			...
			rtl865x_setVlanFilterDatabase(hw_vlan_info[4].
				vlan_port_vid, 0);
...
/* panic_printk("####%s:%d curLinkPortMask=%u\n",__FUNCTION__,__LINE__,curLinkPortMask); */
	...
	if (cp &&
		((cp->portmask & curLinkPortMask & ((1 << RTL8651_MAC_NUMBER) -
					1)) != 0)) {
#if defined(CONFIG_AUTO_DHCP_CHECK) || defined(CONFIG_RTK_WLAN_EVENT_INDICATE)
	...
	len = sprintf(page, "%d\n", rtk_linkStatus);
#if defined(CONFIG_AUTO_DHCP_CHECK) || defined(CONFIG_RTK_WLAN_EVENT_INDICATE)
	len += sprintf(page + len, "%d %x", eth0_portLinkStatus,
		curLinkPortMask);
	...
	unsigned long len, void *data)
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		if (jumboFrameFlag) {
			...
			rtl865x_set_jumbo_frame_size(JUMBO_FRAME_SIZE_NORMAL,
				rtl865x_curOpMode);
		}
	}
	...
	unsigned long len, void *data)
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		if (cmd_addr
			&& !memcmp(cmd_addr, "by_ipv6_route",
				strlen("by_ipv6_route"))) {
			...
			if (enable) {
				...
				if (rtl8651_getAsicOperationLayer() > 2) {
					...
					rtl8198c_addIpv6Route(ipAddr, 0,
						nexthop,
						RTL_DRV_WAN0_NETIF_NAME);
				}
#endif
...
#if 0
		else if (!memcmp(cmd_addr, "by_ipv6_netif",
				strlen("by_ipv6_netif"))) {
			sscanf(strptr, "%x", &enable);
			/* panic_printk("%s %d enable=%d \n", __FUNCTION__, __LINE__, enable); */
			if (enable) {
				rtk_hwL3v6_enable = 1;
	...
	unsigned long len, void *data)
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		num = sscanf(tmpbuf, "%d %d %d %d %d", &vlan_tag,
			&vlan_host_pri, &vlan_bridge_tag, &vlan_bridge_port,
			&vlan_bridge_multicast_tag);
		if (num != 5) {
		...
		if (vlan_bridge_tag) {
			lan_portmask =
				rtk_vlan_wan_tag_getportmask(vlan_bridge_port);
			...
			/* need verify */
			vlanconfig[2].fid = 1;
			((struct dev_priv *)NETDRV_PRIV(_rtl86xx_dev.dev[2]))->
				portmask = lan_portmask | RTL_WANPORT_MASK;
			((struct dev_priv *)NETDRV_PRIV(_rtl86xx_dev.dev[2]))->id = vlan_bridge_tag;	/* eth2 */
			/* vlanconfig[0].vid= RTL_LANVLANID; */
			vlanconfig[0].memPort =
				RTL_LANPORT_MASK & (~lan_portmask);
			vlanconfig[0].untagSet =
				RTL_LANPORT_MASK & (~lan_portmask);
		...
		if (rtl865x_curOpMode == BRIDGE_MODE && vlan_tag) {
			...
			rtl865x_addVlanPortMember(vlan_tag,
				RTL_LANPORT_MASK | RTL_WANPORT_MASK);
			rtl865x_setVlanPortTag(vlan_tag,
				(RTL_LANPORT_MASK | RTL_WANPORT_MASK) &
				(~0x100), 1);
		...
		if (vlan_bridge_tag) {
			...
			if (nicIgmpModuleIndex_2 == 0xFFFFFFFF) {
				ret = rtl_registerIgmpSnoopingModule
					(&nicIgmpModuleIndex_2);
#if defined(CONFIG_RTL_HARDWARE_MULTICAST)
				memset(&devInfo, 0,
			...
			if (vlan_bridge_multicast_tag) {
				if (vlan_bridge_multicast_tag ==
					...
					rtl865x_addVlan
						(vlan_bridge_multicast_tag);
					rtl865x_addVlanPortMember
						(vlan_bridge_multicast_tag,
						RTL_WANPORT_MASK);
					rtl865x_setVlanPortTag(vlan_bridge_multicast_tag, RTL_WANPORT_MASK, 1);	/* wan port tag */
					rtl865x_setVlanFilterDatabase
						(vlan_bridge_multicast_tag, 1);
				}
			}
		} else {
			if (nicIgmpModuleIndex_2 != 0xFFFFFFFF)
				rtl_unregisterIgmpSnoopingModule
					(nicIgmpModuleIndex_2);
...
{
	seq_printf(s,
		"vlan_tag: %d vlan_host_pri: %d \nvlan_bridge_tag: %d, vlan_bridge_port: 0x%x vlan_bridge_stream_tag: %d\n",
		vlan_tag, vlan_host_pri, vlan_bridge_tag, vlan_bridge_port,
		vlan_bridge_multicast_tag);
	return 0;
}
#else
...
#endif
{
	...
	for (port = PHY0; port < CPU; port++) {
		...
		seq_printf(s, "Speed %s\n\n",
			data0 == PortStatusLinkSpeed100M ? "100M" : (data0 ==
				PortStatusLinkSpeed1000M ? "1G" : (data0 ==
					PortStatusLinkSpeed10M ? "10M" :
					"Unkown")));
#else
...
#define SPEED1000M 	2
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		if (!memcmp(cmd_addr, "port", 4)) {
			...
			for (port = 0; port < CPU; port++) {
				if ((1 << port) & port_mask) {
					rtl865xC_setAsicEthernetForceModeRegs
						(port, forceMode, forceLink,
						forceLinkSpeed, forceDuplex);

					/*Set PHY Register */
					rtl8651_setAsicEthernetPHYSpeed(port,
						forceLinkSpeed);
					rtl8651_setAsicEthernetPHYDuplex(port,
						forceDuplex);
					rtl8651_setAsicEthernetPHYAutoNeg(port,
						forceMode ? FALSE : TRUE);
					rtl8651_setAsicEthernetPHYAdvCapality
						(port, advCapability);
					rtl8651_restartAsicEthernetPHYNway
						(port);
					rtl865x_setPortForward(port,
						forwardEnable);
				}
			}
...
int rtl819x_get_port_mibStats(int port, struct port_mibStatistics *port_stats)
{
	...
	port_stats->ifInErrors =
		(rtl8651_returnAsicCounter(OFFSET_DOT3STATSFCSERRORS_P0 +
			addrOffset_fromP0) +
		rtl8651_returnAsicCounter(OFFSET_ETHERSTATSJABBERS_P0 +
			addrOffset_fromP0));
	port_stats->ifInMulticastPkts =
		rtl8651_returnAsicCounter(OFFSET_ETHERSTATSMULTICASTPKTS_P0 +
		addrOffset_fromP0);
	...
	port_stats->etherStatsUndersizePkts =
		rtl8651_returnAsicCounter(OFFSET_ETHERSTATSUNDERSIZEPKTS_P0 +
		addrOffset_fromP0);
	port_stats->etherStatsFraments =
		rtl8651_returnAsicCounter(OFFSET_ETHERSTATSFRAGMEMTS_P0 +
		addrOffset_fromP0);
	port_stats->etherStatsPkts64Octets =
		rtl8651_returnAsicCounter(OFFSET_ETHERSTATSPKTS64OCTETS_P0 +
		addrOffset_fromP0);
	port_stats->etherStatsPkts65to127Octets =
		rtl8651_returnAsicCounter(OFFSET_ETHERSTATSPKTS65TO127OCTETS_P0
		+ addrOffset_fromP0);
	port_stats->etherStatsPkts128to255Octets =
		rtl8651_returnAsicCounter(OFFSET_ETHERSTATSPKTS128TO255OCTETS_P0
		+ addrOffset_fromP0);
	port_stats->etherStatsPkts256to511Octets =
		rtl8651_returnAsicCounter(OFFSET_ETHERSTATSPKTS256TO511OCTETS_P0
		+ addrOffset_fromP0);
	port_stats->etherStatsPkts512to1023Octets =
		rtl8651_returnAsicCounter
		(OFFSET_ETHERSTATSPKTS512TO1023OCTETS_P0 + addrOffset_fromP0);
	port_stats->etherStatsPkts1024to1518Octets =
		rtl8651_returnAsicCounter
		(OFFSET_ETHERSTATSPKTS1024TO1518OCTETS_P0 + addrOffset_fromP0);
	port_stats->etherStatsOversizePkts =
		rtl8651_returnAsicCounter(OFFSET_ETHERSTATSOVERSIZEPKTS_P0 +
		addrOffset_fromP0);
	port_stats->etherStatsJabbers =
		rtl8651_returnAsicCounter(OFFSET_ETHERSTATSJABBERS_P0 +
		addrOffset_fromP0);
	port_stats->etherStatusDropEvents =
		rtl8651_returnAsicCounter(OFFSET_ETHERSTATSDROPEVENTS_P0 +
		addrOffset_fromP0);
	port_stats->dot3FCSErrors =
		rtl8651_returnAsicCounter(OFFSET_DOT3STATSFCSERRORS_P0 +
		addrOffset_fromP0);
	port_stats->dot3StatsSymbolErrors =
		rtl8651_returnAsicCounter(OFFSET_DOT3STATSSYMBOLERRORS_P0 +
		addrOffset_fromP0);
	port_stats->dot3ControlInUnknownOpcodes =
		rtl8651_returnAsicCounter(OFFSET_DOT3CONTROLINUNKNOWNOPCODES_P0
		+ addrOffset_fromP0);
	port_stats->dot3InPauseFrames =
		rtl8651_returnAsicCounter(OFFSET_DOT3INPAUSEFRAMES_P0 +
		addrOffset_fromP0);

	port_stats->ifOutOctets =
		rtl8651_returnAsicCounter(OFFSET_IFOUTOCTETS_P0 +
		addrOffset_fromP0);
	...
	port_stats->ifOutDiscards =
		rtl8651_returnAsicCounter(OFFSET_IFOUTDISCARDS +
		addrOffset_fromP0);
	port_stats->ifOutErrors =
		(rtl8651_returnAsicCounter(OFFSET_ETHERSTATSCOLLISIONS_P0 +
			addrOffset_fromP0) +
		rtl8651_returnAsicCounter
		(OFFSET_DOT3STATSDEFERREDTRANSMISSIONS_P0 + addrOffset_fromP0));
	port_stats->ifOutMulticastPkts =
		rtl8651_returnAsicCounter(OFFSET_IFOUTMULTICASTPKTS_P0 +
		addrOffset_fromP0);
	port_stats->ifOutBroadcastPkts =
		rtl8651_returnAsicCounter(OFFSET_IFOUTBROADCASTPKTS_P0 +
		addrOffset_fromP0);
	...
	port_stats->ifOutDiscards =
		rtl8651_returnAsicCounter(OFFSET_IFOUTDISCARDS +
		addrOffset_fromP0);
	port_stats->dot3StatsSingleCollisionFrames =
		rtl8651_returnAsicCounter
		(OFFSET_DOT3STATSSINGLECOLLISIONFRAMES_P0 + addrOffset_fromP0);
	port_stats->dot3StatsMultipleCollisionFrames =
		rtl8651_returnAsicCounter
		(OFFSET_DOT3STATSMULTIPLECOLLISIONFRAMES_P0 +
		addrOffset_fromP0);
	port_stats->dot3StatsDefferedTransmissions =
		rtl8651_returnAsicCounter
		(OFFSET_DOT3STATSDEFERREDTRANSMISSIONS_P0 + addrOffset_fromP0);
	port_stats->dot3StatsLateCollisions =
		rtl8651_returnAsicCounter(OFFSET_DOT3STATSLATECOLLISIONS_P0 +
		addrOffset_fromP0);
	port_stats->dot3StatsExcessiveCollisions =
		rtl8651_returnAsicCounter(OFFSET_DOT3STATSEXCESSIVECOLLISIONS_P0
		+ addrOffset_fromP0);
	port_stats->dot3OutPauseFrames =
		rtl8651_returnAsicCounter(OFFSET_DOT3OUTPAUSEFRAMES_P0 +
		addrOffset_fromP0);
	port_stats->dot1dBasePortDelayExceededDiscards =
		rtl8651_returnAsicCounter
		(OFFSET_DOT1DBASEPORTDELAYEXCEEDEDDISCARDS_P0 +
		addrOffset_fromP0);
	port_stats->etherStatsCollisions =
		rtl8651_returnAsicCounter(OFFSET_ETHERSTATSCOLLISIONS_P0 +
		addrOffset_fromP0);

	/* port_stats->ifInUnknownProtos = ethPortInUnknownProtos[port]; //mark_rm */
	port_stats->ifInUnknownProtos = 0;
	port_stats->dot1dTpLearnedEntryDiscards =
		rtl8651_returnAsicCounter(MIB_ADDROFFSETBYPORT);
	port_stats->etherStatsCpuEventPkts =
		rtl8651_returnAsicCounter(MIB_ADDROFFSETBYPORT);
...
#ifdef CONFIG_RTL_PROC_NEW
static int show_port_mibStats(struct seq_file *s,
	struct port_mibStatistics *port_stats, int port)
{
	...
	seq_printf(s, "%s%llu\n", "ifHCInUcastPkts=",
		port_stats->ifHCInUcastPkts);
	seq_printf(s, "%s%u\n", "ifInMulticastPkts=",
		port_stats->ifInMulticastPkts);
	seq_printf(s, "%s%llu\n", "ifHCInMulticastPkts=",
		port_stats->ifHCInMulticastPkts);
	seq_printf(s, "%s%u\n", "ifInBroadcastPkts=",
		port_stats->ifInBroadcastPkts);
	seq_printf(s, "%s%llu\n", "ifHCInBroadcastPkts=",
		port_stats->ifHCInBroadcastPkts);
	seq_printf(s, "%s%u\n", "ifInDiscards=", port_stats->ifInDiscards);
	seq_printf(s, "%s%u\n", "ifInErrors=", port_stats->ifInErrors);
	seq_printf(s, "%s%llu\n", "etherStatsOctets=",
		port_stats->etherStatsOctets);
	...
	seq_printf(s, "%s%u\n", "dot1dTpPortInDiscards=",
		port_stats->dot1dTpPortInDiscards);
	seq_printf(s, "%s%u\n", "etherStatusDropEvents=",
		port_stats->etherStatusDropEvents);
	...
	seq_printf(s, "%s%llu\n", "ifHCOutUcastPkts=",
		port_stats->ifHCOutUcastPkts);
	seq_printf(s, "%s%u\n", "ifOutMulticastPkts=",
		port_stats->ifOutMulticastPkts);
	seq_printf(s, "%s%llu\n", "ifHCOutMulticastPkts=",
		port_stats->ifHCOutMulticastPkts);
	seq_printf(s, "%s%u\n", "ifOutBroadcastPkts=",
		port_stats->ifOutBroadcastPkts);
	seq_printf(s, "%s%llu\n", "ifHCOutBroadcastPkts=",
		port_stats->ifHCOutBroadcastPkts);
...
#else
static int show_port_mibStats(struct port_mibStatistics *port_stats, int port,
	char *page, int off)
{
	int len;
	/* struct lan_port_status tmp_port_status; */
...
static int32 read_proc_vlan(struct seq_file *s, void *v)
{
	...
	seq_printf(s,
		"gvlan=%d, lan=%d, vlan=%d, tag=%d, vid=%d, priority=%d, cfi=%d, forwarding_rule=%d\n",
#else
	seq_printf(s,
		"gvlan=%d, lan=%d, vlan=%d, tag=%d, vid=%d, priority=%d, cfi=%d\n",
#endif
...
#endif
#if 0
		printk("===%s(%d), cp->name(%s),global_vlan(%d),is_lan(%d),vlan(%d),tag(%d),id(%d),pri(%d),cfi(%d)", __FUNCTION__, __LINE__, cp->dev->name, cp->vlan_setting.global_vlan, cp->vlan_setting.is_lan, cp->vlan_setting.vlan, cp->vlan_setting.tag, cp->vlan_setting.id, cp->vlan_setting.pri, cp->vlan_setting.cfi);
		printk("-------------%s(%d),cp->portmask(%d)\n", __FUNCTION__,
			__LINE__, cp->portmask);
#endif
	}
out:
	if (tmp)
...
uint32 rtl_hw_vlan_get_tagged_portmask(void)
{
	...
	for (i = 0; i < PORT_NUMBER; i++) {
		if ((hw_vlan_info[i].vlan_port_enabled)
			&& (hw_vlan_info[i].vlan_port_tag)) {
			temp = (i == 0) ? 0 : (i - 1);
			portmask |= 1 << temp;
		}
	}
...
int rtl_process_hw_vlan_tx(rtl_nicTx_info *txInfo)
{
	...
	newskb = NULL;
	if (skb_cloned(skb)) {
		if (cp->vlan_setting.is_lan
			&& (cp->vlan_setting.forwarding_rule == 2)) {
	...
	/* if((skb->data[0]&1) ||((skb->data[0]==0x33) &&(skb->data[1]==0x33) && (skb->data[2]!=0xFF))) */
	{
		if (skb->tag.f.tpid == htons(ETH_P_8021Q)) {
			...
			for (i = 0; i < PORT_NUMBER; i++) {
				temp = (i == 0) ? i : (i - 1);
				if ((((struct dev_priv *)NETDRV_PRIV(skb->
								dev))->
						portmask & (0x1 << temp))
					&& (vid ==
						hw_vlan_info[i].vlan_port_vid)
					&& (hw_vlan_info[i].vlan_port_bridge ==
						1)
					&& (hw_vlan_info[i].vlan_port_tag)
					&& (hw_vlan_info[i].
						vlan_port_enabled)) {
			...
			for (i = 0; i < PORT_NUMBER; i++) {
				if (((struct dev_priv *)NETDRV_PRIV(skb->dev))->
					portmask & (0x1 << i)) {
					flag = 1;
					break;
				}
			...
			temp = (i == 0) ? i : (i + 1);
			if ((flag) && (hw_vlan_info[temp].vlan_port_bridge == 1)
				&& (hw_vlan_info[temp].vlan_port_tag)
				&& (hw_vlan_info[temp].vlan_port_enabled)) {
				port_tag.f.tpid = htons(ETH_P_8021Q);
				port_tag.f.pci =
					(unsigned short)((0x3 << 13) | (0x1 <<
						12) | ((unsigned
							short)(hw_vlan_info
							[temp].
							vlan_port_vid &
...
static int rtl_hw_vlan_support_read(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < PORT_NUMBER; i++) {
		...
		seq_printf(s,
			"lan_%d_bridge_enabled: %d, lan_%d_bridge: %d, lan_%d_tag: %d, lan_%d_vid: %d\n",
			((i == 0) ? i : i - 1),
			hw_vlan_info[i].vlan_port_enabled,
			((i == 0) ? i : i - 1),
			hw_vlan_info[i].vlan_port_bridge,
			((i == 0) ? i : i - 1), hw_vlan_info[i].vlan_port_tag,
			((i == 0) ? i : i - 1), hw_vlan_info[i].vlan_port_vid);
	}

	seq_printf(s,
		"wan_port_enabled: %d, wan_port_tag: %d, wan_port_vid: %d\n",
		hw_vlan_info[1].vlan_port_enabled,
		hw_vlan_info[1].vlan_port_tag, hw_vlan_info[1].vlan_port_vid);
...
static int32 rtl_hw_vlan_config(void)
{
	...
	if (rtl_hw_vlan_enable && (rtl865x_curOpMode == GATEWAY_MODE)) {
		...
		for (i = 0; i < PORT_NUMBER; i++) {
			...
			if (hw_vlan_info[i].vlan_port_enabled == 1
				&& hw_vlan_info[i].vlan_port_bridge ==
				FW_NAT_VLAN)
				rtl_set_nat_vlan_info(hw_vlan_info[i].
					vlan_port_vid,
					hw_vlan_info[i].vlan_port_tag);
			...
			if (hw_vlan_info[i].vlan_port_enabled == 1
				...
				if ((RTK_MC_VLANID != 0)
					&& (hw_vlan_info[i].vlan_port_vid ==
						RTK_MC_VLANID)
					&& (hw_vlan_info[i].vlan_port_tag == 1))
					mc_tag = 1;
#endif
		}
		rtl_update_lan_cp_vinfo((struct dev_priv *)
			NETDRV_PRIV(_rtl86xx_dev.dev[0]), 1, RTK_NAT_VLANID,
			FW_NAT_VLAN, RTK_NAT_WAN_TAGGED);
#endif
#ifdef CONFIG_RTL_HW_VLAN_SUPPORT_HW_NAT
		...
		rtl_update_wan_cp_vinfo((struct dev_priv *)
			NETDRV_PRIV(_rtl86xx_dev.dev[1]), 1, RTK_NAT_VLANID,
			RTK_NAT_WAN_TAGGED);
		/* p4 (or CPU port) setting is chaged to used by Management vlan */
		if (hw_vlan_info[1].vlan_port_enabled)
			rtl_update_manage_vlan(hw_vlan_info[1].
				vlan_port_enabled,
				hw_vlan_info[1].vlan_port_vid,
				hw_vlan_info[1].vlan_port_tag);
		else
			rtl_update_manage_vlan(hw_vlan_info[1].
				vlan_port_enabled, RTK_NAT_VLANID,
				RTK_NAT_WAN_TAGGED);
#endif
		for (i = 0; i < PORT_NUMBER; i++) {
			if (i == 1)	/*bypass eth1 */
...
#endif
			if ((hw_vlan_info[i].vlan_port_enabled == 1)
				&& (hw_vlan_info[i].vlan_port_bridge == 1)) {
				if (i == 0) {
#if 0
					vlanconfig[2 + bridge_num].memPort =
						1 << i;
#else
					vlanconfig[2 + bridge_num].memPort =
						RTL_LANPORT_MASK_4;
#endif
					vlanconfig[2 + bridge_num].vid =
						hw_vlan_info[i].vlan_port_vid;
#if 0
					vlanconfig[2 + bridge_num].untagSet =
						1 << i;
#else
					vlanconfig[2 + bridge_num].untagSet =
						RTL_LANPORT_MASK_4;
					...
					rtl_update_lan_cp_vinfo(((struct
								dev_priv *)
							NETDRV_PRIV
							(_rtl86xx_dev.dev[2 +
									bridge_num])),
...
#endif
					((struct dev_priv *)
						NETDRV_PRIV(_rtl86xx_dev.dev[2 +
								bridge_num]))->
						portmask = 1 << i;
					((struct dev_priv *)
						NETDRV_PRIV(_rtl86xx_dev.dev[2 +
...
#else
					vlanconfig[2 + bridge_num].memPort =
						((i == 2) ? RTL_LANPORT_MASK_3
						: ((i == 3) ? RTL_LANPORT_MASK_2
							: ((i == 4) ?
								RTL_LANPORT_MASK_1
								: 0)));
#endif
	...
	if (rtl_hw_vlan_enable && (rtl865x_curOpMode == GATEWAY_MODE)) {
		...
		int32 totalVlans =
			((sizeof(vlanconfig)) /
			(sizeof(struct rtl865x_vlanConfig))) - 2;
	...
	unsigned long len, void *data)
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		num = sscanf(tmpbuf,
			...
			&rtl_hw_vlan_enable, &hw_vlan_info[1].vlan_port_enabled,
			&hw_vlan_info[1].vlan_port_tag,
			&hw_vlan_info[1].vlan_port_vid,
			&hw_vlan_info[0].vlan_port_enabled,
			&hw_vlan_info[0].vlan_port_bridge,
			&hw_vlan_info[0].vlan_port_tag,
			&hw_vlan_info[0].vlan_port_vid,
			&hw_vlan_info[2].vlan_port_enabled,
			&hw_vlan_info[2].vlan_port_bridge,
			&hw_vlan_info[2].vlan_port_tag,
			&hw_vlan_info[2].vlan_port_vid,
			&hw_vlan_info[3].vlan_port_enabled,
			&hw_vlan_info[3].vlan_port_bridge,
			&hw_vlan_info[3].vlan_port_tag,
			&hw_vlan_info[3].vlan_port_vid,
			&hw_vlan_info[4].vlan_port_enabled,
			&hw_vlan_info[4].vlan_port_bridge,
			&hw_vlan_info[4].vlan_port_tag,
			&hw_vlan_info[4].vlan_port_vid);
...
void rtk_add_manage_vlan(void)
{
	...
	if (cp->vlan_setting.vlan) {
		...
		rtl865x_addVlanPortMember(cp->vlan_setting.id,
			(1 << RTK_WAN_PORT_IDX) | RTL_CPUPORT_MASK);
		if (cp->vlan_setting.tag)
			rtl865x_setVlanPortTag(cp->vlan_setting.id,
				(1 << RTK_WAN_PORT_IDX) | RTL_CPUPORT_MASK, 1);
		else
...
static void rtk_add_manage_vlan_dev(uint32 vid)
{
	...
	manage_vlan_dev =
		(struct net_device *)rtk_add_vlan_if(vid, portmask,
		RTK_MANAGE_IFNAME);
}
...
#if defined(CONFIG_RTK_VLAN_SUPPORT)
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("mib_vlan", 0644, res_stats_root, &mib_vlan_proc_fops,
		(void *)dev);
#else
	if ((res_stats = create_proc_read_entry("mib_vlan", 0644,
				res_stats_root, read_proc_vlan,
...
extern int rtl8367_setProtocolBasedVLAN(uint32 proto_type, uint32 cvid,
	int cmdFlag);
#endif
	...
	unsigned long count, void *data)
{
...
#if	!defined(CONFIG_RTL_LAYERED_DRIVER)
	struct rtl865x_vlanConfig passThruVlanConf =
		{ "passThru", 0, IF_ETHER, PASSTHRU_VLAN_ID, 0,
		rtl865x_lanPortMask | rtl865x_wanPortMask,
		rtl865x_lanPortMask | rtl865x_wanPortMask,
	...
	if (buffer && !copy_from_user(&passThru_flag, buffer, count)) {
		...
		if (flag ^ oldStatus) {
			/* IPv6 PassThru */
			if ((flag & IP6_PASSTHRU_MASK) ^ (oldStatus &
					IP6_PASSTHRU_MASK)) {
				if (flag & IP6_PASSTHRU_MASK) {	/* add */
					for (i = 0; i < RTL865XC_PORT_NUMBER;
						i++) {
						rtl8651_setProtocolBasedVLAN
							(IP6_PASSTHRU_RULEID, i,
							TRUE, PASSTHRU_VLAN_ID);
					}
#ifdef CONFIG_RTL_8367R_SUPPORT
					cmdFlag = TRUE;
#endif
...
#ifdef CONFIG_RTL_8367R_SUPPORT
				rtl8367_setProtocolBasedVLAN(ETH_P_IPV6,
					PASSTHRU_VLAN_ID, cmdFlag);
#endif
			}
#if defined(CONFIG_RTL_CUSTOM_PASSTHRU_PPPOE)
			/* PPPoE PassThru */
			if ((flag & PPPOE_PASSTHRU_MASK) ^ (oldStatus &
					PPPOE_PASSTHRU_MASK)) {
				if (flag & PPPOE_PASSTHRU_MASK) {	/* add */
					for (i = 0; i < RTL865XC_PORT_NUMBER;
						i++) {
						rtl8651_setProtocolBasedVLAN
							(RTL8651_PBV_RULE_PPPOE_CONTROL,
							i, TRUE,
							PASSTHRU_VLAN_ID);
						rtl8651_setProtocolBasedVLAN
							(RTL8651_PBV_RULE_PPPOE_SESSION,
							i, TRUE,
							PASSTHRU_VLAN_ID);
					}
#ifdef CONFIG_RTL_8367R_SUPPORT
...
#ifdef CONFIG_RTL_8367R_SUPPORT
				rtl8367_setProtocolBasedVLAN(ETH_P_PPP_SES,
					PASSTHRU_VLAN_ID, cmdFlag);
				rtl8367_setProtocolBasedVLAN(ETH_P_PPP_DISC,
					PASSTHRU_VLAN_ID, cmdFlag);
#endif
			}
#endif
...
static int32 rtl8651_customPassthru_init(void)
{
	oldStatus = 0;
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("custom_Passthru", 0, &proc_root,
		&custom_Passthru_proc_fops, NULL);
#else
...
#if defined(CONFIG_RTL_CUSTOM_PASSTHRU_PPPOE)
	rtl8651_defineProtocolBasedVLAN(PPPOE_PASSTHRU_RULEID1, 0x0,
		__constant_htons(ETH_P_PPP_SES));
	rtl8651_defineProtocolBasedVLAN(PPPOE_PASSTHRU_RULEID2, 0x0,
		__constant_htons(ETH_P_PPP_DISC));
#endif
	...
	unsigned long count, void *data)
{
	...
	if (buffer && !copy_from_user(tmpBuf, buffer, count)) {
		...
		rtl865x_setStormControl(stormCtrlType, enableStormCtrl,
			percentage);
		return count;
	}
	return -EFAULT;
...
{
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("StormCtrl", 0, &proc_root, &StormCtrl_proc_fops,
		NULL);
#else
...
static int rtl865x_extendStormCtrlReadProc(struct seq_file *s, void *v)
{
	...
	seq_printf(s, "counter 0, %s drop, timing unit %s, rate(%d)\n",
		regDataCounter & ENDROPSC0 ? "enable" : "disable",
		regDataCounter & ENDROPSC0 ? str : "0",
		((regData << 16) >> 16) * 100 * 4 / 30360);

#if 0
	regData = READ_MEM32(EXTSTMCR4);
	seq_printf(s, "counter 1, %s drop, timing unit %s, rate(%d)\n",
		regDataCounter & ENDROPSC1 ? "enable" : "disable",
		regDataCounter & ENDROPSC1 ? str : "0",
		(regData >> 16) * 100 * 4 / 30360);

	regData = READ_MEM32(EXTSTMCR5);
	seq_printf(s, "counter 2, %s drop, timing unit %s, rate(%d)\n",
		regDataCounter & ENDROPSC2 ? "enable" : "disable",
		regDataCounter & ENDROPSC2 ? str : "0",
		((regData << 16) >> 16) * 100 * 4 / 30360);

	regData = READ_MEM32(EXTSTMCR5);
	seq_printf(s, "counter 3, %s drop, timing unit %s, rate(%d)\n\n",
		regDataCounter & ENDROPSC3 ? "enable" : "disable",
		regDataCounter & ENDROPSC3 ? str : "0",
		(regData >> 16) * 100 * 4 / 30360);
#endif
	...
	for (i = 0; i < RTL8651_PORT_NUMBER; i++) {
		...
		if (i & 0x1) {
			seq_printf(s,
				"port%d, %s unknown unicast, counter %d\n", i,
				regData & OUUSCS ? "enable" : "disable",
				(regData & OPUUSC) >> OPUUSC_OFFSET);
#if 0
			seq_printf(s,
				"port%d, %s unknown multicast, counter %d\n", i,
				regData & OUMSCS ? "enable" : "disable",
				(regData & OPUMSC) >> OPUMSC_OFFSET);
			seq_printf(s, "port%d, %s ARP, counter %d\n", i,
				regData & OASCS ? "enable" : "disable",
				(regData & OPASC) >> OPASC_OFFSET);
			seq_printf(s, "port%d, %s DHCP, counter %d\n", i,
				regData & ODSCS ? "enable" : "disable",
				(regData & OPDSC) >> OPDSC_OFFSET);
			seq_printf(s, "port%d, %s IGMP/MLD, counter %d\n\n", i,
				regData & OIMSCS ? "enable" : "disable",
				(regData & OPIMSC) >> OPIMSC_OFFSET);
#endif
		} else {
			seq_printf(s,
				"port%d, %s unknown unicast, counter %d\n", i,
				regData & EUUSCS ? "enable" : "disable",
				(regData & EPUUSC) >> EPUUSC_OFFSET);
#if 0
			seq_printf(s,
				"port%d, %s unknown multicast, counter %d\n", i,
				regData & EUMSCS ? "enable" : "disable",
				(regData & EPUMSC) >> EPUMSC_OFFSET);
			seq_printf(s, "port%d, %s ARP, counter %d\n", i,
				regData & EASCS ? "enable" : "disable",
				(regData & EPASC) >> EPASC_OFFSET);
			seq_printf(s, "port%d, %s DHCP, counter %d\n", i,
				regData & EDSCS ? "enable" : "disable",
				(regData & EPDSC) >> EPDSC_OFFSET);
			seq_printf(s, "port%d, %s IGMP/MLD, counter %d\n\n", i,
				regData & EIMSCS ? "enable" : "disable",
				(regData & EPIMSC) >> EPIMSC_OFFSET);
#endif
		}
	}
	...
	const char *buffer, unsigned long count, void *data)
{
	...
	if (buffer && !copy_from_user(tmpBuf, buffer, count)) {
		...
		rtl865x_setExtendStormControl(stormCtrlType, enableStormCtrl,
			percentage);
...
{
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("ExtendStormCtrl", 0, &proc_root,
		&extendStormCtrl_proc_fops, NULL);
#else
	extendStormCtrlProc = create_proc_entry("ExtendStormCtrl", 0, NULL);
	if (extendStormCtrlProc) {
		extendStormCtrlProc->read_proc =
			rtl865x_extendStormCtrlReadProc;
		extendStormCtrlProc->write_proc =
			rtl865x_extendStormCtrlWriteProc;
	}
#endif
	...
	unsigned long count, void *data)
{
	...
	if (buffer && !copy_from_user(tmpbuf, buffer, count)) {
		...
		if (!memcmp(cmd_addr, "portmask", 8)) {
			...
			/* add CPU port */
			vlanconfig[0].memPort =
				vlanconfig[0].memPort | (1 << 8);
			vlanconfig[0].untagSet =
				vlanconfig[0].untagSet | (1 << 8);

			memset((void *)&vlanconfig[2], 0,
				sizeof(struct rtl865x_vlanConfig));

			((struct dev_priv *)NETDRV_PRIV(_rtl86xx_dev.dev[0]))->portmask = vlanconfig[0].memPort;	/* eth0 */
			((struct dev_priv *)NETDRV_PRIV(_rtl86xx_dev.dev[0]))->
				id = vlanconfig[0].vid;
			((struct dev_priv *)NETDRV_PRIV(_rtl86xx_dev.dev[1]))->id = vlanconfig[1].vid;	/* eth1 */
...
{
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("wan_port_setting", 0, &proc_root,
		&wan_port_setting_proc_fops, NULL);
#else
...
#else
static int32 proc_phyTest_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	return 0;
}
	...
	unsigned long len1, void *data)
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len1)) {
		...
		if (!memcmp(cmd_addr, "read", 4)) {
			...
			ret = rtl8651_getAsicEthernetPHYReg(phyId, regId,
				&regData);
			if (ret == SUCCESS) {
...
/* goto errout; */
		...
		{
			extern void Set_GPHYWB(uint32 phyid, uint32 page,
				uint32 reg, uint32 mask, uint32 val);
			tokptr = strsep(&strptr, " ");
			if (tokptr == NULL)
				goto errout;
			...
			iNo = IOL_100_finish;
			if (!memcmp(tokptr, iol_test_str[IOL_100_MDIX],
					strlen(iol_test_str[IOL_100_MDIX])))
				iNo = IOL_100_MDIX;
			else if (!memcmp(tokptr, iol_test_str[IOL_100_MDI],
					strlen(iol_test_str[IOL_100_MDI])))
				iNo = IOL_100_MDI;
			else if (!memcmp(tokptr, iol_test_str[IOL_100_finish],
					strlen(iol_test_str[IOL_100_finish])))
				iNo = IOL_100_finish;
			else if (!memcmp(tokptr, iol_test_str[IOL_10_MDIX],
					strlen(iol_test_str[IOL_10_MDIX])))
				iNo = IOL_10_MDIX;
			else if (!memcmp(tokptr, iol_test_str[IOL_10_MDI],
					strlen(iol_test_str[IOL_10_MDI])))
				iNo = IOL_10_MDI;
			else if (!memcmp(tokptr, iol_test_str[IOL_10_finish],
					strlen(iol_test_str[IOL_10_finish])))
				iNo = IOL_10_finish;
			else if (!memcmp(tokptr, iol_test_str[IOL_pktGen_allFF],
					strlen(iol_test_str[IOL_pktGen_allFF])))
				iNo = IOL_pktGen_allFF;
			else if (!memcmp(tokptr,
					iol_test_str[IOL_pktGen_random],
					strlen(iol_test_str
						[IOL_pktGen_random])))
				iNo = IOL_pktGen_random;
			else if (!memcmp(tokptr,
					iol_test_str[IOL_pktGen_disable],
					strlen(iol_test_str
						[IOL_pktGen_disable])))
			...
			case IOL_10_MDI:
				...
				rtl8651_setAsicEthernetPHYReg(phyId, 28, 0x40C2);	/* force MDI mode */
				rtl8651_getAsicEthernetPHYReg(phyId, 23,
					&regData);
			...
			case IOL_pktGen_allFF:
				...
				REG32(regAddr) =
					(REG32(regAddr) & ~(0x001B07FF))
					| ((1 << 16) | 1518);
#else /* defined(CONFIG_RTD_1295_HWNAT) */
				regData = PGFPCR + (0x80 * iPort);
				REG32(regData) =
					(REG32(regData) & ~(0x001B07FF))
					| ((1 << 16) | 1518);
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
				...
				REG32(PGGCR1) =
					(REG32(PGGCR1)
					& ~(0x3 << (iPort * 4))) |
					(StopTXcommandPulse << (iPort * 4));
				mdelay(50);
				REG32(PGGCR1) =
					(REG32(PGGCR1)
					& ~(0x3 << (iPort * 4))) |
					(StartTXcommandPulse << (iPort * 4));
			...
			if (iNo == 1)	/* test mode 1 */
			{
				...
				for (i = 0; i < 5; i++) {
					port = rtl8651AsicEthernetTable[i].
						phyId;
					rtl8651_setAsicEthernetPHYReg(port,
						0x09, 0x2E00);
					mdelay(50);
				}
#else
				...
				for (i = 0; i < 5; i++) {
					rtl8651_setAsicEthernetPHYReg(i, 0x1f,
						0x0000);
					rtl8651_setAsicEthernetPHYReg(i, 0x09,
						0x2E00);
				}
#endif
				for (i = 0; i < 5; i++)
			...
			} else if (iNo == 3)	/* test mode 3 */
			{
				...
				case 'A':
					rtl8651_setAsicEthernetPHYReg(1, 0x10,
						0x1100);
					rtl8651_setAsicEthernetPHYReg(1, 0x1f,
						0x0000);
					printk("A channel\n");
				...
				rtl8651_setAsicEthernetPHYReg(iPort, 0x1f,
					0x0000);
				rtl8651_setAsicEthernetPHYReg(iPort, 0x09,
					0x6e00);
				for (i = 0; i < 5; i++)
					REG32(PCRP0 + i * 4) &= ~(EnForceMode);
				printk("Please reset the target after leaving Test Mode\n");
			...
			else if (iNo == 4)	/* test mode 4 */
			{
				...
				if (tokptr != NULL) {
					...
					if (iPort == 0)
						iPort = 8;
					rtl8651_setAsicEthernetPHYReg(iPort,
						0x09, 0x8E00);
				} else {
					printk("all ports\n");
					for (i = 0; i < 5; i++) {
						phyId = rtl8651AsicEthernetTable
							[i].phyId;
						rtl8651_setAsicEthernetPHYReg
							(phyId, 0x09, 0x8E00);
						mdelay(50);
				...
				for (i = 0; i < 5; i++) {
					phyId = rtl8651AsicEthernetTable[i].
						phyId;
					rtl8651_setAsicEthernetPHYReg(phyId,
						0x09, 0xE00);
					mdelay(50);
				}
				for (i = 0; i < 5; i++)
...
{
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("phyRegTest", 0, &proc_root, &phyRegTest_proc_fops,
		NULL);
#else
...
{
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("br_mldsnoop", 0, &proc_root, &br_mldsnoop_proc_fops,
		NULL);
#else
...
#ifdef CONFIG_RTL_8367R_SUPPORT
		{
			extern int rtk_port_phyReg_get(int port, int reg,
				uint32 *pData);
...
#if defined(CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_8370_SUPPORT)
				{
					extern int rtk_port_phyReg_get(int port,
						int reg, uint32 *pData);
					extern int rtk_port_phyReg_set(int port,
						int reg, uint32 regData);
					uint32 regData;
...
#else
static int32 port_state_read_proc(char *page, char **start, off_t off,
	int count, int *eof, void *data)
{
	int len;
	uint32 regData;
	...
	for (port = PHY0; port < PHY5; port++) {
		...
		data0 = regData & PortStatusRXPAUSE;
		len += sprintf(page + len, "RXPause:%s|",
...
{
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("portState", 0, &proc_root, &portState_proc_fops,
		NULL);
#else
...
static int rtl_port_used_by_device(uint32 portMask)
{
	...
	if (rtl_multiWan_net_dev && cp && cp->opened
		&& (cp->portmask & portMask))
		retval = SUCCESS;
...
static int rtl_regist_multipleWan_dev(void)
{
	...
	memcpy((char *)dev->dev_addr, (char *)(&(rtl_multiWan_config.mac)),
		ETHER_ADDR_LEN);
#if defined(CONFIG_COMPAT_NET_DEV_OPS)
	dev->open = re865x_open;
	dev->stop = re865x_close;
	...
	if (!rc) {
		...
		printk("==%s(%d) %s added. vid=%d Member port 0x%x...\n",
			...
			rtl_multiWan_config.vid, rtl_multiWan_config.memPort);
	} else
...
static int rtl_config_multipleWan_netif(int32 cmd)
{
	...
	case RTL_MULTIWAN_ADD:
		{
			...
			if (retval == SUCCESS) {
				rtl865x_addVlanPortMember(rtl_multiWan_config.
					vid, rtl_multiWan_config.memPort);
				rtl865x_setVlanFilterDatabase
					(rtl_multiWan_config.vid,
					rtl_multiWan_config.fid);
			...
			/*add network interface */
			memset(&netif, 0, sizeof(rtl865x_netif_t));
			memcpy(netif.name, rtl_multiWan_config.ifname,
				MAX_IFNAMESIZE);
			memcpy(netif.macAddr.octet,
				rtl_multiWan_config.mac.octet, ETHER_ADDR_LEN);
	...
	case RTL_MULTIWAN_DEL:
		...
		rtl865x_delVlanPortMember(rtl_multiWan_config.vid,
			rtl_multiWan_config.memPort);
		break;
	default:
		retval = FAILED;
	}
...
static int rtl865x_isArpFrame(struct sk_buff *skb, int *ah_offset)
{
	...
	if (*((uint16 *)(skb->data + (ETH_ALEN << 1))) ==
		...
		*ah_offset = ETH_HLEN;
		ret = TRUE;
	} else if ((*((uint16 *)(skb->data + (ETH_ALEN << 1))) ==
			__constant_htons(ETH_P_8021Q))
		&& (*((uint16 *)(skb->data + (ETH_ALEN << 1) + VLAN_HLEN)) ==
			__constant_htons(ETH_P_ARP))) {
	...
	rtl_nicRx_info *info)
{
	...
	for (i = 0; i < ETH_INTF_NUM; i++) {
		cp = (struct dev_priv *)NETDRV_PRIV(_rtl86xx_dev.dev[i]);
		if (_rtl86xx_dev.dev[i] && cp && cp->opened
			&& (memcmp(data, cp->dev->dev_addr, 6) == 0)) {
			info->priv = cp;
			return SUCCESS;
		}
	}
...
static inline int rtl865x_decideMcDevice(int pid, rtl_nicRx_info *info)
{
	...
	for (i = 0; i < ETH_INTF_NUM; i++) {
		...
		if (_rtl86xx_dev.dev[i] && cp && cp->opened
			&& (cp->portmask & (1 << pid))) {
			if (strncmp(cp->dev->name, igmp_proxy_wan_dev,
					MAX_IFNAMESIZE) == 0) {
				info->priv = cp;
				return SUCCESS;
			}
	...
	rtl_nicRx_info *info)
{
	...
	if (rtl865x_isArpFrame(skb, &ah_offset) == TRUE) {
		...
		if ((arp->ar_hrd == htons(ARPHRD_ETHER))
			...
			for (i = 0; i < ETH_INTF_NUM; i++) {
				cp = ((struct dev_priv *)
					NETDRV_PRIV(_rtl86xx_dev.dev[i]));
				if (_rtl86xx_dev.dev[i] && cp && cp->opened
					&& (cp->portmask & (1 << pid))) {
					if ((get_dev_ip_mask(cp->dev->name,
								&dev_ip,
								&dev_mask) == 0)
						&& (ntohl(tip) == dev_ip)) {
						info->priv = cp;
			...
			if (rtl_multiWan_net_dev) {
				cp = (struct dev_priv *)
					NETDRV_PRIV(rtl_multiWan_net_dev);
				if (rtl_multiWan_net_dev && cp && cp->opened
					&& (cp->portmask & (1 << pid))) {
					if ((get_dev_ip_mask(cp->dev->name,
								&dev_ip,
...
static int rtl865x_delMultiCastNetif(void)
{
...
#if defined(CONFIG_RTK_VLAN_SUPPORT)
	{
		...
		rule.pktOpApp_ = RTL865X_ACL_ALL_LAYER;
		rule.actionType_ = RTL865X_ACL_PERMIT;
		ret = rtl865x_del_acl(&rule, multiCastNetIf,
			RTL865X_ACL_SYSTEM_USED);
	}
#endif
...
static int rtl865x_addMultiCastNetif(void)
{
...
#if defined(CONFIG_RTL_MAC_BASED_NETIF)
	ret = rtl865x_addVlanPortMember(MULTICAST_NETIF_VLAN_ID,
		RTL_WANPORT_MASK);
	ret = rtl865x_setVlanFilterDatabase(MULTICAST_NETIF_VLAN_ID,
		RTL_WAN_FID);
#else
	ret = rtl865x_addVlanPortMember(MULTICAST_NETIF_VLAN_ID,
		rtl_multiWan_config.memPort);
...
#if 1				/* defined(CONFIG_RTK_VLAN_SUPPORT) */
	{
		...
		rule.pktOpApp_ = RTL865X_ACL_ALL_LAYER;
		rule.actionType_ = RTL865X_ACL_PERMIT;
		ret = rtl865x_add_acl(&rule, multiCastNetIf,
			RTL865X_ACL_SYSTEM_USED);
	}
#endif
...
static ssize_t igmp_proxy_wan_dev_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
{
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("igmp_proxy_wan_dev", 0, &proc_root,
		&igmp_proxy_wan_dev_proc_fops, NULL);
#else
	res_igmp_proxy_wan_dev =
		create_proc_entry("igmp_proxy_wan_dev", 0, NULL);
	if (res_igmp_proxy_wan_dev) {
		res_igmp_proxy_wan_dev->read_proc =
			igmp_proxy_wan_dev_read_proc;
		res_igmp_proxy_wan_dev->write_proc =
			igmp_proxy_wan_dev_write_proc;
	}
#endif
	return 0;
}
...
/*2007-12-19*/
	...
	phy_reg30[0] = phy_reg30[1] = phy_reg30[2] = phy_reg30[3] =
		phy_reg30[4] = 0;
#endif
...
int rtl865x_reinitSwitchCore(void)
{
...
#if defined(CONFIG_RTL_8197F)
	REG32(DMA_CR0) =
		(REG32(DMA_CR0) & ~(LowFifoMark_MASK | HiFifoMark_MASK)) |
		((0x30 << LowFifoMark_OFFSET) | 0xD7);
#if defined(CONFIG_RTD_1295_HWNAT)
	{
	...
	REG32(CPUIISR) = REG32(CPUIISR);
	REG32(CPUIIMR) =
		RX_DONE_IE_ALL | TX_ALL_DONE_IE_ALL | LINK_CHANGE_IE |
...
{
	seq_printf(s,
		"check_tx_hang_interval=%d, reinit_swCore_threshold=%d, rtl_reinit_swCore_counter=%d\n",
		rtl_check_swCore_tx_hang_interval, rtl_reinit_swCore_threshold,
...
static int rtl_check_swCore_tx_hang_read_proc(char *page, char **start,
	off_t off, int count, int *eof, void *data)
{
	int len = 0;
	...
	const char *buffer, unsigned long count, void *data)
{
	...
	if (buffer && !copy_from_user(tmpBuf, buffer, count)) {
		int num =
			sscanf(tmpBuf, "%d %d",
			&rtl_check_swCore_tx_hang_interval,
			&rtl_reinit_swCore_threshold);
...
{
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("reInitSwitchCore", 0, &proc_root,
		&reInitSwitchCore_proc_fops, NULL);
	proc_create_data("check_swCore_tx_hang", 0, &proc_root,
		&rtl_check_swCore_tx_hang_proc_fops, NULL);
#else
	reInitSwitchCoreProc = create_proc_entry("reInitSwitchCore", 0, NULL);
	if (reInitSwitchCoreProc) {
		reInitSwitchCoreProc->read_proc =
			rtl865x_reInitSwitchCoreReadProc;
		reInitSwitchCoreProc->write_proc =
			rtl865x_reInitSwitchCoreWriteProc;
	}
	rtl_check_swCore_tx_hang_proc =
		create_proc_entry("check_swCore_tx_hang", 0, NULL);
	if (rtl_check_swCore_tx_hang_proc) {
		rtl_check_swCore_tx_hang_proc->read_proc =
			rtl_check_swCore_tx_hang_read_proc;
		rtl_check_swCore_tx_hang_proc->write_proc =
			rtl_check_swCore_tx_hang_write_proc;
	}
#endif
...
void __exit rtl865x_destroyReInitResetSwitchCore(void)
{
	...
	if (rtl_check_swCore_tx_hang_proc) {
		remove_proc_entry("check_swCore_tx_hang",
			rtl_check_swCore_tx_hang_proc);
...
int rtl_check_swCore_tx_hang(void)
{
	...
	if (((rtl_check_swCore_timer++) % rtl_check_swCore_tx_hang_interval) ==
		0) {
	...
	unsigned change, unsigned int status)
{
	...
	err = ethernet_rtnl_fill_ifinfo(skb, dev, type, 0, 0, change, 0,
		status);
	if (err < 0) {
...
{
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("sockDebug", 0, &proc_root, &sockDebug_proc_fops,
		NULL);
#else
...
static int eee_read_proc(struct seq_file *s, void *v)
{
...
#ifdef CONFIG_RTL_8198C_8367RB
	{
		...
		for (j = 0; j < 5; j++) {
			...
			if (ret != 0)
				seq_printf(s,
					"  port %d: read 8367RB EEE status error.\n",
					j);
			else
				seq_printf(s,
					"  port %d: 8367RB EEE %sabled.\n", j,
					((enable == 1) ? "en" : "dis"));
		}
	}
#endif
	...
	unsigned long count, void *data)
{
	...
	if (buffer && !copy_from_user(tmpBuf, buffer, count)) {
		...
		if ((tmpBuf[0] == '0') || (tmpBuf[0] == '1')) {
			...
			{
				int i;
				extern int rtk_eee_portEnable_set(uint32 port,
					uint32 enable);
				for (i = 0; i < 5; i++)
...
static int jate_write_proc(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	char tmpbuf[64];
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		if (!memcmp(cmd_addr, "port", 4)) {
			...
			if (!memcmp(tokptr, "10H", 3)) {
				...
				mode = NwayAbility10MH | NwayAbility10MF |
					NwayAbility100MH | NwayAbility100MF |
					NwayAbility1000MF;
			...
			if (port == 5) {
				for (i = 0; i < 5; i++) {
					...
					REG32(PCRP0 + (i * 4)) =
						(REG32(PCRP0 +
							(i * 4)) &
						~(0x03fc0000)) | mode;
					rtl8651_setAsicEthernetPHYReg(i, 0,
						(phy_mode | RESTART_AUTONEGO));
					/* rtl8651_restartAsicEthernetPHYNway(i); */
				}
			} else {
				...
				REG32(PCRP0 + (port * 4)) =
					(REG32(PCRP0 +
						(port * 4)) & ~(0x03fc0000)) |
					mode;
				rtl8651_setAsicEthernetPHYReg(port, 0,
					(phy_mode | RESTART_AUTONEGO));
				/* rtl8651_restartAsicEthernetPHYNway(port); */
			}
#endif

			panic_printk("successfully set port %d to %s", port,
				tokptr);
		}
				...
				for (j = 0; j < tx_cnt; j++)	/* for sending multiple packets */
				{
					...
					*(unsigned short *)(skb->data + 12) =
					...
					if (mode == 0) {
						...
						memset(skb->data + 14, 0,
							PKT_LEN);
					else if (mode == 2)
						memset(skb->data + 14, 0xff,
							PKT_LEN);
...
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
					RTL_CACHE_WBACK((unsigned long)skb->
						data, skb->len);
#else
/* RTL_CACHE_WBACK((unsigned long) skb->data, skb->len); */
#endif
					RTL_swNic_send((void *)skb,
					...
					rtl_periodic_watchdog_kick(j,
						WATCHDOG_NUM_OF_TIMES);
#endif
				}
...
static int tr181_eth_link_read(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < NUMOFLINKENTRY; i++) {
		...
		if (((struct dev_priv *)NETDRV_PRIV(_rtl86xx_dev.dev[i]))->
			opened)
			seq_printf(s, "eth%d: %s %s ", i, "true", "Up");
		else
			seq_printf(s, "eth%d: %s %s ", i, "false", "Down");

		seq_printf(s, "%s %d ", _rtl86xx_dev.dev[i]->name,
			get_uptime_by_sec() - if_change_time[i]);

		for (j = 0; j < 5; j++)
			seq_printf(s, "%02x:",
				_rtl86xx_dev.dev[i]->dev_addr[j]);
	...
	int *eof, void *data)
{
	...
	for (i = 0; i < NUMOFLINKENTRY; i++) {
		...
		if (((struct dev_priv *)NETDRV_PRIV(_rtl86xx_dev.dev[i]))->
			opened)
			len += sprintf(page + len, "eth%d: %s %s ", i, "true",
				"Up");
		else
			len += sprintf(page + len, "eth%d: %s %s ", i, "false",
				"Down");

		len += sprintf(page + len, "%s %d ", _rtl86xx_dev.dev[i]->name,
			get_uptime_by_sec() - if_change_time[i]);
...
static int tr181_eth_if_read(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < (NUMOFIFENTRY + 3); i++) {
		extern int rtk_port_phyStatus_get(uint32 port,
			uint32 *pLinkStatus, uint32 *pSpeed, uint32 *pDuplex);
		...
		if (i == 7) {
			if (port_change_time[6] == 0)
				seq_printf(s, "port%d: %s %s ", i, "false",
					"Down");
			else
				seq_printf(s, "port%d: %s %s ", i, "true",
					"Up");

			seq_printf(s, "%d ",
				get_uptime_by_sec() - port_change_time[i]);
			seq_printf(s, "%s %d %s\n", "false", -1, "Auto");
			continue;
		}
		...
		seq_printf(s, "%d ",
			(speed == PortStatusLinkSpeed1000M) ? 1000 : (speed ==
				PortStatusLinkSpeed100M) ? 100 : 10);
		seq_printf(s, "%s\n", (duplex) ? "Full" : "Half");
	}
#else
#endif
	...
	int *eof, void *data)
{
	...
	for (i = 0; i < (NUMOFIFENTRY + 3); i++) {
		...
		if (i == 7) {
			if (port_change_time[6] == 0)
				len += sprintf(page + len, "port%d: %s %s ", i,
					"false", "Down");
			else
				len += sprintf(page + len, "port%d: %s %s ", i,
					"true", "Up");
		...
		if (status == 0)
			len += sprintf(page + len, "port%d: %s %s ", i, "false",
				"Down");
		else
			len += sprintf(page + len, "port%d: %s %s ", i, "true",
				"Up");

		len += sprintf(page + len, "%d ",
			get_uptime_by_sec() - port_change_time[i]);
		len += sprintf(page + len, "%s ",
			(i == RTL83XX_WAN) ? "true" : "false");
		len += sprintf(page + len, "%d ",
			(speed == PortStatusLinkSpeed1000M) ? 1000 : (speed ==
				PortStatusLinkSpeed100M) ? 100 : 10);
		len += sprintf(page + len, "%s\n", (duplex) ? "Full" : "Half");
...
static int tr181_eth_stats_read(struct seq_file *s, void *v)
{
	...
	seq_printf(s, "%s%s", "Inter-|   Receive                            ",
		"                    |  Transmit\n");
	...
	if ((_rtl86xx_dev.dev[1]) && NETDRV_PRIV(_rtl86xx_dev.dev[1]))
		eth1_portmask =
			((struct dev_priv *)NETDRV_PRIV(_rtl86xx_dev.dev[1]))->
			portmask;
...
#endif
	{
		...
		if (eth0_portmask & (1 << i)) {
			...
			eth0_stats.ErrorsSent +=
				port_cntrs.etherStatsCollisions;
			eth0_stats.ErrorsReceived +=
				port_cntrs.dot3StatsFCSErrors +
				port_cntrs.dot3StatsSymbolErrors;
			eth0_stats.UnicastPacketsSent +=
				port_cntrs.ifOutUcastPkts;
			eth0_stats.UnicastPacketsReceived +=
				port_cntrs.ifInUcastPkts;
			eth0_stats.DiscardPacketsSent +=
				port_cntrs.dot1dBasePortDelayExceededDiscards;
			eth0_stats.DiscardPacketsReceived +=
				port_cntrs.dot1dTpPortInDiscards;
			eth0_stats.MulticastPacketsSent +=
				port_cntrs.ifOutMulticastPkts;
			eth0_stats.MulticastPacketsReceived +=
				port_cntrs.etherStatsMcastPkts;
			eth0_stats.BroadcastPacketsSent +=
				port_cntrs.ifOutBrocastPkts;
			eth0_stats.BroadcastPacketsReceived +=
				port_cntrs.etherStatsBcastPkts;
			eth0_stats.PacketsSent +=
				port_cntrs.ifOutUcastPkts +
				port_cntrs.ifOutMulticastPkts +
				port_cntrs.ifOutBrocastPkts;
			eth0_stats.PacketsReceived +=
				port_cntrs.ifInUcastPkts +
				port_cntrs.etherStatsMcastPkts +
				port_cntrs.etherStatsBcastPkts;
		}
		if (eth1_portmask & (1 << i)) {
			...
			eth1_stats.ErrorsSent +=
				port_cntrs.etherStatsCollisions;
			eth1_stats.ErrorsReceived +=
				port_cntrs.dot3StatsFCSErrors +
				port_cntrs.dot3StatsSymbolErrors;
			eth1_stats.UnicastPacketsSent +=
				port_cntrs.ifOutUcastPkts;
			eth1_stats.UnicastPacketsReceived +=
				port_cntrs.ifInUcastPkts;
			eth1_stats.DiscardPacketsSent +=
				port_cntrs.dot1dBasePortDelayExceededDiscards;
			eth1_stats.DiscardPacketsReceived +=
				port_cntrs.dot1dTpPortInDiscards;
			eth1_stats.MulticastPacketsSent +=
				port_cntrs.ifOutMulticastPkts;
			eth1_stats.MulticastPacketsReceived +=
				port_cntrs.etherStatsMcastPkts;
			eth1_stats.BroadcastPacketsSent +=
				port_cntrs.ifOutBrocastPkts;
			eth1_stats.BroadcastPacketsReceived +=
				port_cntrs.etherStatsBcastPkts;
			eth1_stats.PacketsSent +=
				port_cntrs.ifOutUcastPkts +
				port_cntrs.ifOutMulticastPkts +
				port_cntrs.ifOutBrocastPkts;
			eth1_stats.PacketsReceived +=
				port_cntrs.ifInUcastPkts +
				port_cntrs.etherStatsMcastPkts +
				port_cntrs.etherStatsBcastPkts;
		}
	...
	seq_printf(s, "%llu %llu %llu %llu  %llu\n",
		eth0_stats.UnicastPacketsReceived,
		eth0_stats.BroadcastPacketsReceived,
		eth0_stats.UnicastPacketsSent, eth0_stats.MulticastPacketsSent,
		eth0_stats.BroadcastPacketsSent);
	...
	seq_printf(s, "%llu %llu %llu %llu  %llu\n",
		eth1_stats.UnicastPacketsReceived,
		eth1_stats.BroadcastPacketsReceived,
		eth1_stats.UnicastPacketsSent, eth1_stats.MulticastPacketsSent,
		eth1_stats.BroadcastPacketsSent);
...
int tr181_get_eth_if(int if_link_num, tr181_eth_interface *pIf)
{
...
#if defined(CONFIG_RTL_8367R_SUPPORT)
	{
		...
		if ((rtk_port_phyStatus_get(if_link_num, &status, &speed,
					&duplex)) == 0) {
		...
		pIf->LastChange =
			get_uptime_by_sec() - port_change_time[if_link_num];
	}
#else
#endif
...
int tr181_get_eth_if_stats(int if_link_num, tr181_eth_if_stats *pStats)
{
	...
	pStats->PacketsSent =
		pStats->UnicastPacketsSent + pStats->MulticastPacketsSent +
		pStats->BroadcastPacketsSent;
	pStats->PacketsReceived =
		pStats->UnicastPacketsReceived +
		pStats->MulticastPacketsReceived +
		pStats->BroadcastPacketsReceived;
...
#else
	{
		...
		pStats->BytesSent =
			rtl865xC_returnAsicCounter64(OFFSET_IFOUTOCTETS_P0 +
			addrOffset_fromP0);
		pStats->BytesReceived =
			rtl865xC_returnAsicCounter64(OFFSET_IFINOCTETS_P0 +
			addrOffset_fromP0);
		pStats->ErrorsSent =
			rtl8651_returnAsicCounter(OFFSET_ETHERSTATSCOLLISIONS_P0
			+ addrOffset_fromP0);
		...
		pStats->DiscardPacketsSent =
			rtl8651_returnAsicCounter(OFFSET_IFOUTDISCARDS +
			addrOffset_fromP0);
		pStats->DiscardPacketsReceived =
			rtl8651_returnAsicCounter
			(OFFSET_DOT1DTPPORTINDISCARDS_P0 + addrOffset_fromP0);
		pStats->MulticastPacketsSent =
			rtl8651_returnAsicCounter(OFFSET_IFOUTMULTICASTPKTS_P0 +
			addrOffset_fromP0);
		pStats->MulticastPacketsReceived =
			rtl8651_returnAsicCounter
			(OFFSET_ETHERSTATSMULTICASTPKTS_P0 + addrOffset_fromP0);
		pStats->BroadcastPacketsSent =
			rtl8651_returnAsicCounter(OFFSET_IFOUTBROADCASTPKTS_P0 +
			addrOffset_fromP0);
		pStats->BroadcastPacketsReceived =
			rtl8651_returnAsicCounter
			(OFFSET_ETHERSTATSBROADCASTPKTS_P0 + addrOffset_fromP0);
		pStats->PacketsSent =
			pStats->UnicastPacketsSent +
			pStats->MulticastPacketsSent +
			pStats->BroadcastPacketsSent;
...
int tr181_get_eth_link(int if_link_num, tr181_eth_link *pLink)
{
	...
	if (!(_rtl86xx_dev.dev[if_link_num])
		|| !NETDRV_PRIV(_rtl86xx_dev.dev[if_link_num]))
		return (-1);

	pLink->Enable =
		((struct dev_priv *)NETDRV_PRIV(_rtl86xx_dev.
			dev[if_link_num]))->opened;
	...
	for (i = 0; i < 6; i++)
		sprintf(pLink->MACAddress + (i * 2), "%02x",
			_rtl86xx_dev.dev[if_link_num]->dev_addr[i]);
	...
	for (i = 0; i < 5; i++) {
		...
		if (portmask & (1 << i)) {
			pStats->BytesSent +=
				...
				(OFFSET_IFOUTOCTETS_P0 + addrOffset_fromP0);
			pStats->BytesReceived +=
				rtl865xC_returnAsicCounter64
			...
			pStats->ErrorsReceived +=
				rtl8651_returnAsicCounter
				(OFFSET_DOT3STATSFCSERRORS_P0 +
				addrOffset_fromP0);
			pStats->UnicastPacketsSent +=
				rtl8651_returnAsicCounter
				(OFFSET_IFOUTUCASTPKTS_P0 + addrOffset_fromP0);
			pStats->UnicastPacketsReceived +=
				rtl8651_returnAsicCounter
				(OFFSET_IFINUCASTPKTS_P0 + addrOffset_fromP0);
			pStats->DiscardPacketsSent +=
				rtl8651_returnAsicCounter(OFFSET_IFOUTDISCARDS +
				addrOffset_fromP0);
			pStats->DiscardPacketsReceived +=
			...
			pStats->MulticastPacketsReceived +=
				rtl8651_returnAsicCounter
				(OFFSET_ETHERSTATSMULTICASTPKTS_P0 +
				addrOffset_fromP0);
			pStats->BroadcastPacketsSent +=
				rtl8651_returnAsicCounter
				(OFFSET_IFOUTBROADCASTPKTS_P0 +
				addrOffset_fromP0);
			pStats->BroadcastPacketsReceived +=
				rtl8651_returnAsicCounter
...
#ifdef CONFIG_RTL_8367R_SUPPORT
	{
		extern int rtk_port_phyAutoNegoAbility_get(uint32 port,
			rtk_port_phy_ability_t *pAbility);
		extern int rtk_port_phyAutoNegoAbility_set(uint32 port,
			rtk_port_phy_ability_t *pAbility);
		...
		if (!memcmp(tokptr, "stats", 5)) {
			...
			if (!memcmp(tokptr, "true", 4)) {
				if (if_link == 1)
					...
					tr181_set_eth_link_enable(if_link_num,
						1);
			...
			if (!memcmp(tokptr, "Half", 4)) {
				tr181_set_eth_if_DuplexMode(if_link_num,
					TR181_ETH_IF_DUPLEX_HALF);
			} else if (!memcmp(tokptr, "Full", 4)) {
				tr181_set_eth_if_DuplexMode(if_link_num,
					TR181_ETH_IF_DUPLEX_FULL);
			} else if (!memcmp(tokptr, "Auto", 4)) {
				tr181_set_eth_if_DuplexMode(if_link_num,
					TR181_ETH_IF_DUPLEX_AUTO);
			} else
				goto errout;
		} else
			goto errout;
...
int rtk_dev_name_netif_mapping(char *br_name, char *br_port_name, int flag)
{
	...
	case 1:
		for (i = 0; i < NETIF_NUMBER; i++) {
			...
			if (netif_name_mapping[i].netif_Name[0] == '\0') {
				memcpy(netif_name_mapping[i].dev_name, br_name,
					MAX_IFNAMESIZE);
				memcpy(netif_name_mapping[i].netif_Name,
					br_port_name, MAX_IFNAMESIZE);
				break;
			}
	...
	case 2:
		for (i = 0; i < NETIF_NUMBER; i++) {
			/* find it, clear */
			if (!memcmp(netif_name_mapping[i].netif_Name,
					br_port_name, MAX_IFNAMESIZE)) {
				memset((void *)&netif_name_mapping[i], '\0',
					sizeof(netif_name_mapping_t));
				break;
			}
...
int rtk_get_devname_by_netifname(char *dev_name, char *netif_name)
{
	...
	for (i = 0; i < NETIF_NUMBER; i++) {
		if ((!memcmp(netif_name_mapping[i].netif_Name, netif_name,
					MAX_IFNAMESIZE))) {
			memcpy(dev_name, netif_name_mapping[i].dev_name,
				MAX_IFNAMESIZE);
			return 1;
		}
	}
...
int rtk_get_netif_name_by_devname(char *dev_name, char *netif_name)
{
	...
	for (i = 0; i < NETIF_NUMBER; i++) {
		if ((!memcmp(netif_name_mapping[i].dev_name, dev_name,
					MAX_IFNAMESIZE))) {
			...
			if ((netif_name_mapping[i].valid)) {
				...
				memcpy(netif_name,
					netif_name_mapping[i].netif_Name,
					MAX_IFNAMESIZE);
				return SUCCESS;
			} else {
				...
				rtk_add_acl_for_sw_forward(netif_name_mapping
					[i].netif_Name, dev_name,
					netif_name_mapping[i].valid);
				return FAILED;
			}
		}
	}
...
int rtk_get_devname_by_vid(char *dev_name, int vid)
{
	...
	for (i = 0; i < NETIF_NUMBER; i++) {
		...
		if (strvlanid != NULL) {
			strvlanid++;
			if ((strvlanid != NULL)
				&& (vid == simple_strtol(strvlanid, NULL, 0))) {
				memcpy(dev_name, netif_name_mapping[i].dev_name,
					MAX_IFNAMESIZE);
				/* panic_printk("%s %d dev_name=%s netif_name=%s \n", __FUNCTION__, __LINE__, dev_name, netif_Name); */
				return SUCCESS;
...
unsigned int rtl_get_brIgmpModuleIndexbyVid(unsigned int vid)
{
	...
	if (ret == SUCCESS)
		igmpModuleIndex =
			rtl_get_brIgmpModuleIndexbyName(dev_name, &index);
...
struct dev_priv *rtk_get_multi_lan_dev_cp(uint16 vid, int pid)
{
	...
	for (j = 0; j < ETH_INTF_NUM; j++) {
		...
		if (_rtl86xx_dev.dev[j] && dp && dp->opened
			&& (dp->portmask & (1 << pid))) {
			break;
		}
	}
...
inline uint32 rtk_get_vlan_tagmask(uint16 vid)
{
	uint32 mask;
	...
	mask = (vlan_ctl_p->group[vid].memberMask) & (vlan_ctl_p->group[vid].
		tagMemberMask);
	return mask;
}

inline uint32 rtk_get_vlan_portmask(uint16 vid)
{
	uint32 mask;
...
inline uint8 rtk_get_vlan_type(uint16 vid)
{
	uint32 vlan_type;
	...
	struct sk_buff **pskb1, rtl_nicRx_info *info)
{
	...
	*pskb1 = NULL;
	...
	if (rtl865x_curOpMode == GATEWAY_MODE) {
		...
		if (!((1 << port) & RTL_WANPORT_MASK)) {
			if (rtk_get_vlan_type(rtk_get_vid_by_port(port)) ==
				VLAN_TYPE_BRIDGE) {
#if defined(CONFIG_RTD_1295_HWNAT)
				skb->dev = _rtl86xx_dev.dev[RTL_WAN_IDX];
#else /* defined(CONFIG_RTD_1295_HWNAT) */
...
#if defined(CONFIG_RTL_MULTI_LAN_DEV)
static void rtk_update_netif_vlan_multi_lan_dev(int netif_idx, uint16 vid,
	uint32 portmask)
{

	/* update hw netif table vid , depend on vlanconfig idx */
#if defined(CONFIG_RTD_1295_HWNAT)
	if (portmask & RTL_LANPORT_MASK_5) {
	...
	uint32 vlantype)
{
...
#if defined(CONFIG_RTD_1295_HWNAT)
				rtk_update_netif_vlan(RTL_WAN_IDX, vid,
					rtk_hw_port_map(vid, portmask));
#else /* defined(CONFIG_RTD_1295_HWNAT) */
				rtk_update_netif_vlan(1, vid,
					rtk_hw_port_map(vid, portmask));
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
			}
		} else {
...
#else /* defined(CONFIG_RTD_1295_HWNAT) */
				rtk_update_netif_vlan(1, vid,
					rtk_hw_port_map(vid, portmask));	/* wan 1 */
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
				if (portmask & (~(RTK_WAN_PORT_MASK |
							RTK_CPU_PORT_MASK))) {
					/* bridge lan, bridge vid equal nat vid case */
#if defined(CONFIG_RTL_MULTI_LAN_DEV)
					rtk_update_vlan_multi_lan_dev(vid,
						portmask);
#endif
				}
			} else if (portmask & (~(RTK_WAN_PORT_MASK |
						RTK_CPU_PORT_MASK))) {
...
#if defined(CONFIG_RTL_MULTI_LAN_DEV)
#if defined(CONFIG_RTD_1295_HWNAT)
			rtk_update_netif_vlan_multi_lan_dev(RTL_LAN_IDX, vid,
				rtk_hw_port_map(vid, portmask));
#else /* defined(CONFIG_RTD_1295_HWNAT) */
			rtk_update_netif_vlan_multi_lan_dev(0, vid,
				rtk_hw_port_map(vid, portmask));
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
#endif
...
#if defined(CONFIG_RTD_1295_HWNAT)
				rtk_update_netif_vlan(RTL_LAN_IDX, vid,
					rtk_hw_port_map(vid, portmask));
#else /* defined(CONFIG_RTD_1295_HWNAT) */
				rtk_update_netif_vlan(0, vid,
					rtk_hw_port_map(vid, portmask));	/* lan 0 */
#endif /* defined(CONFIG_RTD_1295_HWNAT) */
		}
#endif
	...
	if (rtl865x_curOpMode == GATEWAY_MODE) {
		if ((portmask & RTK_WAN_PORT_MASK) == RTK_WAN_PORT_MASK) {
			...
			if (vlantype == VLAN_TYPE_BRIDGE
				&& !(tagmask & RTK_WAN_PORT_MASK))
				portmask |= 0x0f;
...
#if defined(CONFIG_RTL_MLD_SNOOPING)
				else if ((skb->data[0] == 0x33)
					&& (skb->data[1] == 0x33)
					&& (skb->data[2] != 0xff)) {
					ipv6h = re865x_getIpv6Header(skb->data);
					if (ipv6h != NULL) {
						l4Protocol =
							re865x_getIpv6TransportProtocol
							(ipv6h);
						if (l4Protocol ==
							IPPROTO_ICMPV6) {
							return 1;
...
static int32 rtk_vlan_ctl_read(char *page, char **start, off_t off, int count,
	int *eof, void *data)
{
	int len = 0;
...
static int32 rtk_vlan_tbl_write(struct file *filp, const char *buff,
	unsigned long len, void *data)
{
	char tmpbuf[100];
	int num;
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		num = sscanf(tmpbuf, "%d %d %x %x", &index, &vid, &mask,
			&tagMask);
		...
		rtk_add_eth_vlan(vlan_group_p->vid, vlan_group_p->memberMask,
			vlan_group_p->tagMemberMask);
...
#endif
{
	...
	existMulticastEntry =
		rtl865x_findMCastEntry(destIpAddr, srcIpAddr,
		(unsigned short)srcVlanId, (unsigned short)srcPort);
	if (existMulticastEntry != NULL) {
		/*it's already in cache */
		return 0;
	...
	if (brFwdPortMask == 0) {
#if defined(CONFIG_RTL_MULTICAST_PORT_MAPPING)
		rtl865x_blockMulticastFlow(srcVlanId, srcPort, srcIpAddr,
			destIpAddr, mapPortMask);
#else
		rtl865x_blockMulticastFlow(srcVlanId, srcPort, srcIpAddr,
			destIpAddr);
#endif
	...
	ret = rtl_getMulticastDataFwdInfo(nicIgmpModuleIndex,
	...
	if (fwdDescriptor.fwdPortMask != 0) {
		...
		/* fwdDescriptor.fwdPortMask); */
#if defined(CONFIG_RTL_MULTICAST_PORT_MAPPING)
		ret = rtl865x_addMulticastEntry(destIpAddr, srcIpAddr,
			(unsigned short)srcVlanId, (unsigned short)srcPort,
...
#if defined(CONFIG_RTL_ETH_NAPI_SUPPORT)
static int napi_recv(struct dev_priv *cp, int budget)
{
	struct sk_buff *pskb;
	int work_done = 0;

	while ((work_done < budget) &&
	       (!skb_queue_empty(&cp->rx_napi_skb_queue))) {
		pskb = skb_dequeue(&cp->rx_napi_skb_queue);
		if (!pskb)
			break;

#ifdef CONFIG_RTL_ETH_NAPI_GRO_SUPPORT
		if (napi_gro_receive(&cp->napi, pskb) != GRO_DROP)
			work_done++;
#else
...
struct rtl819x_mib_counter {
	...
	unsigned offset;
	unsigned length;
	unsigned memidx;	/* const char *name; */
};
/* 8367 mib stats */
...
static struct rtl819x_mib_counter rtl819x_mib_counters[] = {
	...
	{0, OFFSET_ETHERSTATSUNDERSIZEPKTS_P0, 4, STAT_EtherStatsUnderSizePkts,
			"EtherStatsUnderSizePkts"},
	{0, OFFSET_ETHERSTATSFRAGMEMTS_P0, 4, STAT_EtherStatsFragments,
			"EtherFragments"},
	{0, OFFSET_ETHERSTATSPKTS64OCTETS_P0, 4, STAT_EtherStatsPkts64Octets,
			"EtherStatsPkts64Octets"},
	{0, OFFSET_ETHERSTATSPKTS65TO127OCTETS_P0, 4,
			STAT_EtherStatsPkts65to127Octets,
			"EtherStatsPkts65to127Octets"},
	{0, OFFSET_ETHERSTATSPKTS128TO255OCTETS_P0, 4,
			STAT_EtherStatsPkts128to255Octets,
			"EtherStatsPkts128to255Octets"},
	{0, OFFSET_ETHERSTATSPKTS256TO511OCTETS_P0, 4,
			STAT_EtherStatsPkts256to511Octets,
			"EtherStatsPkts256to511Octets"},
	{0, OFFSET_ETHERSTATSPKTS512TO1023OCTETS_P0, 4,
			STAT_EtherStatsPkts512to1023Octets,
			"EtherStatsPkts512to1023Octets"},
	{0, OFFSET_ETHERSTATSPKTS1024TO1518OCTETS_P0, 4,
			STAT_EtherStatsPkts1024to1518Octets,
			"EtherStatsPkts1024to1518Octets"},
	{0, OFFSET_ETHERSTATSOVERSIZEPKTS_P0, 4, STAT_EtherOversizeStats,
			"EtherOversizeStats"},
	{0, OFFSET_ETHERSTATSJABBERS_P0, 4, STAT_EtherStatsJabbers,
			"EtherStatsJabbers"},
	{0, OFFSET_IFINUCASTPKTS_P0, 4, STAT_IfInUcastPkts, "IfInUcastPkts"},
	{0, OFFSET_ETHERSTATSMULTICASTPKTS_P0, 4, STAT_EtherStatsMulticastPkts,
			"EtherStatsMulticastPkts"},
	{0, OFFSET_ETHERSTATSBROADCASTPKTS_P0, 4, STAT_EtherStatsBroadcastPkts,
			"EtherStatsBroadcastPkts"},
	{0, OFFSET_DOT1DTPPORTINDISCARDS_P0, 4, STAT_EtherStatsDropEvents,
			"EtherStatsDropEvents"},
	{0, OFFSET_DOT3STATSFCSERRORS_P0, 4, STAT_Dot3StatsFCSErrors,
			"Dot3StatsFCSErrors"},
	{0, OFFSET_DOT3INPAUSEFRAMES_P0, 4, STAT_Dot3InPauseFrames,
			"Dot3InPauseFrames"},
	{0, OFFSET_DOT3CONTROLINUNKNOWNOPCODES_P0, 4,
			STAT_Dot3ControlInUnknownOpcodes,
			"Dot3ControlInUnknownOpcodes"},
	/* out */
	{0, OFFSET_IFOUTOCTETS_P0, 8, STAT_IfOutOctets, "IfOutOctets"},
	{0, OFFSET_DOT3STATSSINGLECOLLISIONFRAMES_P0, 4,
			STAT_Dot3StatsSingleCollisionFrames,
			"Dot3StatsSingleCollisionFrames"},
	{0, OFFSET_DOT3STATSMULTIPLECOLLISIONFRAMES_P0, 4,
			STAT_Dot3StatsMultipleCollisionFrames,
			"Dot3StatMultipleCollisionFrames"},
	{0, OFFSET_DOT3STATSDEFERREDTRANSMISSIONS_P0, 4,
			STAT_Dot3StatsDeferredTransmissions,
			"Dot3sDeferredTransmissions"},
	{0, OFFSET_DOT3STATSLATECOLLISIONS_P0, 4, STAT_Dot3StatsLateCollisions,
			"Dot3StatsLateCollisions"},
	{0, OFFSET_ETHERSTATSCOLLISIONS_P0, 4, STAT_EtherStatsCollisions,
			"EtherStatsCollisions"},
	{0, OFFSET_DOT3OUTPAUSEFRAMES_P0, 4, STAT_Dot3OutPauseFrames,
			"Dot3OutPauseFrames"},
	{0, OFFSET_IFOUTUCASTPKTS_P0, 4, STAT_IfOutUcastPkts, "IfOutUcastPkts"},
	{0, OFFSET_IFOUTMULTICASTPKTS_P0, 4, STAT_IfOutMulticastPkts,
			"IfOutMulticastPkts"},
	{0, OFFSET_IFOUTBROADCASTPKTS_P0, 4, STAT_IfOutBroadcastPkts,
			"IfOutBroadcastPkts"},
};
...
static inline unsigned int to_swconfig_portmask(unsigned int portmask)
{
	unsigned int mask;
	mask = portmask;
	if ((portmask & BIT(RTL819X_HW_PORT_NUM_CPU)) ==
		BIT(RTL819X_HW_PORT_NUM_CPU))
		mask = ((mask) & (~(BIT(RTL819X_HW_PORT_NUM_CPU)))) |
			BIT(RTL819X_PORT_NUM_CPU);
...
static inline unsigned int to_rtl819x_portmask(unsigned int portmask)
{
	...
	if ((portmask & BIT(RTL819X_PORT_NUM_CPU)) == BIT(RTL819X_PORT_NUM_CPU))
		mask = ((mask) & (~(BIT(RTL819X_PORT_NUM_CPU)))) |
			(RTL819X_HW_CPUPORT_MASK);
...
static void rtk_init_swconfig_vconfig(void)
{
	...
	((struct dev_priv *)(netdev_priv(_rtl86xx_dev.dev[0])))->portmask =
		RTK_ETH_ALL_PORT_MASK;
	((struct dev_priv *)(netdev_priv(_rtl86xx_dev.dev[0])))->id =
		vlanconfig[0].vid;
	((struct dev_priv *)(netdev_priv(_rtl86xx_dev.dev[0])))->portnum =
		RTL819X_NUM_PORTS;
...
int rtk_set_hw_vlan(unsigned int vid, unsigned int member, unsigned int untag,
	unsigned int fid)
{
...
unsigned int rtk_get_819x_port_status(int port, struct switch_port_link *link)
{
	...
	regData = READ_MEM32(PSRP0 + ((port) << 2));
	speedData =
		(regData & PortStatusLinkSpeed_MASK) >>
		PortStatusLinkSpeed_OFFSET;
...
unsigned int rtk_get_mibCounter_val(int port, int mib_idx)
{
	...
	/* else */
	counter32 =
		rtl8651_returnAsicCounter(rtl819x_mib_counters[mib_idx].offset +
		addrOffset_fromP0);
	return counter32;
#else
...
{
	seq_printf(s, "%s %u %s %u\n", "hw_nat_lan_vid:", hw_nat_lan_vid,
		"hw_nat_wan_vid:", hw_nat_wan_vid);
	...
	unsigned long count, void *data)
{
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		int num =
			sscanf(tmp, "%u %u", &hw_nat_lan_vid, &hw_nat_wan_vid);
		if (num != 2) {
			printk("invalid vlan parameter!\n");
		}
...
static ssize_t hw_nat_vid_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
#else
	proc_create_data("hw_nat_vid", 0644, &proc_root, &hw_nat_vid_proc_fops,
		NULL);
#endif
...
static void rtl_set_vlanconfig(uint16 vid)
{
	...
	for (i = 0; i < count; i++) {
		if (strncmp(vlanconfig[i].ifname, RTL_DRV_WAN0_NETIF_NAME,
				MAX_IFNAMESIZE) == 0) {
			...
			vlanconfig[i].isWan = 1;	/* roll back wan config */
			vlanconfig[i].vid = vid;
			strncpy(vlanconfig[i].ifname,
				RTL_SWCONFIG_DRV_PPP_NETIF_NAME, IFNAMSIZ);
		}
	}
}
...
static int32 rtl_process_hw_netif(uint16 vid)
{
	...
	if (vid == hw_nat_lan_vid) {
		...
		if (rtl865x_getNetifVid(RTL_DRV_LAN_NETIF_NAME,
				&netif_vid) == SUCCESS) {
			if (netif_vid != hw_nat_lan_vid) {
				rtl865x_setNetifVid(RTL_DRV_LAN_NETIF_NAME,
					hw_nat_lan_vid);
			}
		}
		...
		for (i = 0; vlanconfig[i].vid != 0; i++) {
			...
			if (strncmp(vlanconfig[i].ifname, "ppp", 3)) {
				...
				strncpy(netif.name, hw_nat_wan_dev_name,
					MAX_IFNAMESIZE);
				if ((wan_dev = __dev_get_by_name(&init_net,
							hw_nat_wan_dev_name)) !=
					NULL)
					memcpy(netif.macAddr.octet,
						wan_dev->dev_addr,
					...
					memcpy(netif.macAddr.octet,
						vlanconfig[i].mac.octet,
						ETHER_ADDR_LEN);
			} else {
				memcpy(netif.name, vlanconfig[i].ifname,
					MAX_IFNAMESIZE);
				memcpy(netif.macAddr.octet,
					vlanconfig[i].mac.octet,
					ETHER_ADDR_LEN);
...
int rtl_get_bind_port_mask_by_dev_name(char *name)
{
	...
	for (i = 0; i < LAN_DEV_NUM_MAX; i++) {
		if (strncmp(name, lan_dev_bind_mask_mapping_drv[i].ifname,
				IFNAMSIZ) == 0) {
...
static int _rtl_update_vlan_table(int vlan_id)
{
	...
	for (i = 0; vlanconfig[i].ifname[0] != '\0'; i++) {
		if (vlanconfig[i].isWan && (vlanconfig[i].is_slave == 0)
			&& (vlanconfig[i].vid == vlan_id)) {
	...
	if (_the_num_of_this_vid_WAN > 0) {
		...
		if (ret == SUCCESS) {
			...
			if (rtl865x_setVlanFilterDatabase(vlan_id,
					RTL_WAN_FID) != SUCCESS) {
				printk("Leave %s @ %d \n", __FUNCTION__,
					__LINE__);
				return FAILED;
		...
		if (rtl865x_modVlanPortMember(vlan_id, memPort,
				untagSet) != SUCCESS) {
			printk("Leave %s @ %d \n", __FUNCTION__, __LINE__);
			return FAILED;
		}
	...
	for (j = 0; packedVlanConfig[j].vid != 0; j++) {
		if (packedVlanConfig[j].isWan) {
			if (packedVlanConfig[j].protocol == SMUX_PROTO_BRIDGE) {
				bridged_wan_exist = 1;
				if (packedVlanConfig[j].vid ==
					RTL_BRIDGE_WANVLANID)
				...
				for (j = 0; packedVlanConfig[j].vid != 0; j++) {
					if (packedVlanConfig[j].isWan
						...
						if (the_first_vid_exist_in_bridged_wan == -1)
							the_first_vid_exist_in_bridged_wan
								=
								packedVlanConfig
								[j].vid;
					}
...
int rtl_set_wanport_vlanconfig(char *ifname, int proto, int vid, int napt)
{
	...
	for (i = 0; vlanconfig[i].ifname[0] != '\0'; i++) {
		if (vlanconfig[i].isWan && (vlanconfig[i].is_slave == 0)
			&& !strcmp(vlanconfig[i].ifname, ifname)) {
	...
	if ((proto == SMUX_PROTO_BRIDGE) && (vid != -1)) {
		...
		if (rtl865x_add_acl(&rule, netif.name, RTL865X_ACL_ICBUG_PATCH,
				1, 1) != SUCCESS) {
			/* printk("Leave %s @ %d \n", __FUNCTION__, __LINE__); */
			return FAILED;
		}
	...
	if (vlan_id == RTL_BRIDGE_WANVLANID) {
		...
		rtl865x_get_master_netif_by_vid(RTL_WANVLANID,
			vid_8_netif_name);

		if (strcmp(vid_8_netif_name, "")) {
			rtl865x_netif_local_t *netif_vid_8 =
				_rtl865x_getNetifByName(vid_8_netif_name);
			...
			memcpy(netif_set_mac.macAddr.octet,
				netif_vid_8->macAddr.octet, ETHER_ADDR_LEN);
...
int rtl_set_wanport_portmapping(char *ifname, unsigned int member)
{
	...
	if (vlanconfig[idx].protocol == SMUX_PROTO_BRIDGE) {
		...
		if (member == 0xFFFFFFFF) {
			vlanconfig[idx].memPort |=
				(RTL_LANPORT_MASK | RTL_WANPORT_MASK);
			vlanconfig[idx].untagSet |=
				(RTL_LANPORT_MASK | RTL_WANPORT_MASK);
		} else {
...
#if defined(RTL_BRIDGE_WAN_SUPPORT_PORT_MAPPING)
			rtl865x_modVlanPortMember(vlanconfig[idx].vid,
				vlanconfig[idx].memPort,
				vlanconfig[idx].untagSet);
			/*jwj: Delete wan bridge bind lan port from br0 port mask.now all lan neitf
			 * port are untag. */
			lan_netif_port_mask =
				rtl865x_getVlanPortMask(RTL_LANVLANID);
			lan_netif_untag_port_mask =
				rtl865x_getVlanPortMask(RTL_LANVLANID);
			rtl865x_modVlanPortMember(RTL_LANVLANID,
				lan_netif_port_mask & (~member),
				lan_netif_untag_port_mask & (~member));
#endif
		}
	}

	if (((vlanconfig[idx].vid != RTL_BRIDGE_WANVLANID)
			&& (vlanconfig[idx].vid != RTL_WANVLANID))
...
int rtl865x_unregisterDev(char *ifname)
{
	...
	if ((vlanconfig[idx].protocol == SMUX_PROTO_BRIDGE)
		 ...
		 * port are untag. */
		unsigned int lan_netif_port_mask =
			rtl865x_getVlanPortMask(RTL_LANVLANID);
		unsigned int lan_netif_untag_port_mask =
			rtl865x_getVlanPortMask(RTL_LANVLANID);

		rtl865x_modVlanPortMember(RTL_LANVLANID,
			lan_netif_port_mask | (vlanconfig[idx].
				memPort & (~RTL_WANPORT_MASK)),
			lan_netif_untag_port_mask | (vlanconfig[idx].
				memPort & (~RTL_WANPORT_MASK)));
	}
#endif
...
int rtl865x_setNetifMacAddr(char *ifname, unsigned char *addr)
{
	...
	if (rtl865x_getNetifVid(ifname, &vid) == SUCCESS) {
		if (vid == RTL_WANVLANID) {
			...
			rtl865x_get_master_netif_by_vid(RTL_BRIDGE_WANVLANID,
				vid_7_netif_name);

			if (strcmp(vid_7_netif_name, "")) {
				memcpy(netif.name, vid_7_netif_name,
					MAX_IFNAMESIZE);
				if (rtl865x_setNetifMac(&netif) != SUCCESS) {
					/* printk("Leave %s @ %d \n",__FUNCTION__,__LINE__); */
					return FAILED;
...
int32 rtl_add_ppp_netif(char *ifname)
{
	...
	for (i = 0; vlanconfig[i].ifname[0] != '\0'; i++) {
		if (vlanconfig[i].isWan && vlanconfig[i].is_slave
			&& (!strcmp(vlanconfig[i].ifname, ifname))) {
			idx = i;
			break;
		}
	...
	int count, int *eof, void *data)
{
	...
	len = sprintf(page, "vlan=%d, tag=%d, vid=%d, priority=%d, cfi=%d\n",
		PPPOE_vlanInfo.vlan, PPPOE_vlanInfo.tag, PPPOE_vlanInfo.id,
		PPPOE_vlanInfo.pri, PPPOE_vlanInfo.cfi);
...
#if 0
		printk("===%s(%d), global_vlan(%d),is_lan(%d),vlan(%d),tag(%d),id(%d),pri(%d),cfi(%d)", __FUNCTION__, __LINE__, PPPOE_vlanInfo.global_vlan, PPPOE_vlanInfo.is_lan, PPPOE_vlanInfo.vlan, PPPOE_vlanInfo.tag, PPPOE_vlanInfo.id, PPPOE_vlanInfo.pri, PPPOE_vlanInfo.cfi);
#endif
	}
...
#if defined(CONFIG_RTD_1295_HWNAT)
int rtl_getPortlistByDevName(char *name, uint32 *portlist)
{
	int i;
	struct dev_priv *cp;

	if (name == NULL || portlist == NULL)
		return FAILED;

	for (i = 0; i < ETH_INTF_NUM; i++) {
		cp = ((struct dev_priv *)NETDRV_PRIV(_rtl86xx_dev.dev[i]));
		if (_rtl86xx_dev.dev[i] && cp
			&& (strcmp(name, cp->dev->name) == 0)) {
			*portlist = cp->portmask;
			return SUCCESS;
		}
	}
...
static int string_to_hex(char *string, unsigned char *key, int len)
{
	...
	for (idx = 0; idx < len; idx += 2) {
		...
		key[ii++] =
			(unsigned char)simple_strtol(tmpBuf, (char **)NULL, 16);
	}
	return 1;
}
...
/* hash function  */
int32 rtl_lag_hash_function(uint8 *src_mac, uint8 *dst_mac, uint32 *hash,
	int32 hash_sel)
{
	/* uint32 hash_sel = 0; */
	/* int32 ret = FAILED; */
	...
	if (src_mac) {
		...
		sid0h = ((src_mac[5] >> 7) ^ (src_mac[5] >> 5) ^ (src_mac[5] >>
				3) ^ (src_mac[5] >> 1) ^ (src_mac[4] >> 7) ^
			(src_mac[4] >> 5) ^ (src_mac[4] >> 3) ^ (src_mac[4] >>
				1) ^ (src_mac[3] >> 7) ^ (src_mac[3] >> 5) ^
			(src_mac[3] >> 3) ^ (src_mac[3] >> 1) ^ (src_mac[2] >>
				7) ^ (src_mac[2] >> 5) ^ (src_mac[2] >> 3) ^
			(src_mac[2] >> 1) ^ (src_mac[1] >> 7) ^ (src_mac[1] >>
				5) ^ (src_mac[1] >> 3) ^ (src_mac[1] >> 1) ^
			(src_mac[0] >> 7) ^ (src_mac[0] >> 5) ^ (src_mac[0] >>
				3) ^ (src_mac[0] >> 1)
			) & 0x01;
		sid1h = ((src_mac[5] >> 6) ^ (src_mac[5] >> 4) ^ (src_mac[5] >>
				2) ^ (src_mac[5]) ^ (src_mac[4] >> 6) ^
			(src_mac[4] >> 4) ^ (src_mac[4] >> 2) ^ (src_mac[4]) ^
			(src_mac[3] >> 6) ^ (src_mac[3] >> 4) ^ (src_mac[3] >>
				2) ^ (src_mac[3]) ^ (src_mac[2] >> 6) ^
			(src_mac[2] >> 4) ^ (src_mac[2] >> 2) ^ (src_mac[2]) ^
			(src_mac[1] >> 6) ^ (src_mac[1] >> 4) ^ (src_mac[1] >>
				2) ^ (src_mac[1]) ^ (src_mac[0] >> 6) ^
			(src_mac[0] >> 4) ^ (src_mac[0] >> 2) ^ (src_mac[0])
			) & 0x01;
		sidmah = ((src_mac[5] >> 4) ^ (src_mac[5] >> 3) ^ (src_mac[5] >>
				2) ^ (src_mac[4] >> 4) ^ (src_mac[4] >> 3) ^
			(src_mac[4] >> 2) ^ (src_mac[3] >> 4) ^ (src_mac[3] >>
				3) ^ (src_mac[3] >> 2) ^ (src_mac[2] >> 4) ^
			(src_mac[2] >> 3) ^ (src_mac[2] >> 2) ^ (src_mac[1] >>
				4) ^ (src_mac[1] >> 3) ^ (src_mac[1] >> 2) ^
			(src_mac[0] >> 4) ^ (src_mac[0] >> 3) ^ (src_mac[0] >>
				2)
			) & 0x01;
	}
	if (dst_mac) {
		/* panic_printk("%s %d dstmac=0x%x:%x:%x:%x:%x:%x \n", __FUNCTION__, __LINE__, dst_mac[0], dst_mac[1], dst_mac[2], dst_mac[3], dst_mac[4], dst_mac[5]); */
		did0h = ((dst_mac[5] >> 7) ^ (dst_mac[5] >> 5) ^ (dst_mac[5] >>
				3) ^ (dst_mac[5] >> 1) ^ (dst_mac[4] >> 7) ^
			(dst_mac[4] >> 5) ^ (dst_mac[4] >> 3) ^ (dst_mac[4] >>
				1) ^ (dst_mac[3] >> 7) ^ (dst_mac[3] >> 5) ^
			(dst_mac[3] >> 3) ^ (dst_mac[3] >> 1) ^ (dst_mac[2] >>
				7) ^ (dst_mac[2] >> 5) ^ (dst_mac[2] >> 3) ^
			(dst_mac[2] >> 1) ^ (dst_mac[1] >> 7) ^ (dst_mac[1] >>
				5) ^ (dst_mac[1] >> 3) ^ (dst_mac[1] >> 1) ^
			(dst_mac[0] >> 7) ^ (dst_mac[0] >> 5) ^ (dst_mac[0] >>
				3) ^ (dst_mac[0] >> 1)
			) & 0x01;

		did1h = ((dst_mac[5] >> 6) ^ (dst_mac[5] >> 4) ^ (dst_mac[5] >>
				2) ^ (dst_mac[5]) ^ (dst_mac[4] >> 6) ^
			(dst_mac[4] >> 4) ^ (dst_mac[4] >> 2) ^ (dst_mac[4]) ^
			(dst_mac[3] >> 6) ^ (dst_mac[3] >> 4) ^ (dst_mac[3] >>
				2) ^ (dst_mac[3]) ^ (dst_mac[2] >> 6) ^
			(dst_mac[2] >> 4) ^ (dst_mac[2] >> 2) ^ (dst_mac[2]) ^
			(dst_mac[1] >> 6) ^ (dst_mac[1] >> 4) ^ (dst_mac[1] >>
				2) ^ (dst_mac[1]) ^ (dst_mac[0] >> 6) ^
			(dst_mac[0] >> 4) ^ (dst_mac[0] >> 2) ^ (dst_mac[0])
			) & 0x01;
		didmah = ((dst_mac[5] >> 4) ^ (dst_mac[5] >> 3) ^ (dst_mac[5] >>
				2) ^ (dst_mac[4] >> 4) ^ (dst_mac[4] >> 3) ^
			(dst_mac[4] >> 2) ^ (dst_mac[3] >> 4) ^ (dst_mac[3] >>
				3) ^ (dst_mac[3] >> 2) ^ (dst_mac[2] >> 4) ^
			(dst_mac[2] >> 3) ^ (dst_mac[2] >> 2) ^ (dst_mac[1] >>
				4) ^ (dst_mac[1] >> 3) ^ (dst_mac[1] >> 2) ^
			(dst_mac[0] >> 4) ^ (dst_mac[0] >> 3) ^ (dst_mac[0] >>
				2)
			) & 0x01;
		didmbh = ((dst_mac[5] >> 7) ^ (dst_mac[5] >> 6) ^ (dst_mac[5] >>
				5) ^ (dst_mac[4] >> 7) ^ (dst_mac[4] >> 6) ^
			(dst_mac[4] >> 5) ^ (dst_mac[3] >> 7) ^ (dst_mac[3] >>
				6) ^ (dst_mac[3] >> 5) ^ (dst_mac[2] >> 7) ^
			(dst_mac[2] >> 6) ^ (dst_mac[2] >> 5) ^ (dst_mac[1] >>
				7) ^ (dst_mac[1] >> 6) ^ (dst_mac[1] >> 5) ^
			(dst_mac[0] >> 7) ^ (dst_mac[0] >> 6) ^ (dst_mac[0] >>
				5)
			) & 0x01;
	}
	if (hash_sel == LINK_TRUNK_HASH_TYPE_DA_SA) {
		...
		if (hash) {
			*hash = (tmp_hash[0] | (tmp_hash[1] << 1) | (tmp_hash[2]
					<< 2)) & 0x7;
...
void rtl_lag_check_trunk_port_state(void)
{
	...
	if (!up_trunk_msk) {
		...
		/* panic_printk("%s %d \n", __FUNCTION__, __LINE__); */
	} else if ((up_trunk_msk == trunk_mask)
		&& (up_trunk_msk == last_up_trunk_msk)) {
		/* all lag trunk ports up all the time. */
		/* panic_printk("%s %d \n", __FUNCTION__, __LINE__); */
	} else if ((up_trunk_msk == last_up_trunk_msk)
		&& (down_trunk_msk == last_down_trunk_msk)) {
		/* panic_printk("%s %d \n", __FUNCTION__, __LINE__); */
		...
		for (i = 0; i <= LINK_TRUNK_HASH_MAX_INDEX; i = i + cnt) {
			...
			for (j = 0; j <= RTL8651_PHY_NUMBER; j++) {
				if ((up_trunk_msk & (1 << j))) {
					rtl8651_setLAHashIndexToPortMapping(i +
						cnt, j);
					cnt++;
				}
			}
			...
			for (i = 0; i < ETH_INTF_NUM; i++) {
				...
				if (_rtl86xx_dev.dev[i] && cp && cp->opened
					&& !(memcmp(cp->dev->name,
							...
							(RTL_DRV_WAN0_NETIF_NAME))))
				{
					ret = rtl_lag_hash_function(cp->dev->
						dev_addr, NULL, &hash1,
						hash_sel);
					if (ret == SUCCESS) {
						rtl8651_setLAHashIndexToPortMapping
							(hash1, tmp_port);
					}
				}
			}
...
#endif
{
	...
	if (hash_sel == LINK_TRUNK_HASH_TYPE_DA_SA)
		len += sprintf(page + len,
			"Trunk Hash algorithm selection: DA+SA\n");
	else if (hash_sel == LINK_TRUNK_HASH_TYPE_DA)
		len += sprintf(page + len,
			"Trunk Hash algorithm selection: DA\n");
	else if (hash_sel == LINK_TRUNK_HASH_TYPE_SA)
		len += sprintf(page + len,
			"Trunk Hash algorithm selection: SA\n");
	else
		len += sprintf(page + len,
			"Trunk Hash algorithm selection: Unkown\n");
	...
	unsigned long len, void *data)
{
	...
	char *tokptr = NULL;
	uint8 srcmac_str[16] = { '\0' }, src_mac[6] = {
	'\0'};
	uint8 dstmac_str[16] = { '\0' }, dst_mac[6] = {
	'\0'};
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		if (!memcmp(cmd_addr, "trunkmask", strlen("trunkmask"))) {
		...
		} else if (!memcmp(cmd_addr, "hashselect",
				strlen("hashselect"))) {
		...
		} else if (!memcmp(cmd_addr, "hashmapping",
				strlen("hashmapping"))) {
			sscanf(strptr, "%d %d", &hash_index, &phy_port);
			/* panic_printk("%s %d hash_index=%d phy_port=%d \n", __FUNCTION__, __LINE__, hash_index, phy_port); */
			rtl8651_setLAHashIndexToPortMapping(hash_index,
				phy_port);
		} else if (!memcmp(cmd_addr, "hashcalculate",
				strlen("hashcalculate"))) {
			tokptr = strsep(&strptr, " ");
			if (tokptr == NULL) {
				goto errout;
			}
			...
			if (hash_type == LINK_TRUNK_HASH_TYPE_DA_SA) {
				...
				rtl_lag_hash_function(src_mac, dst_mac,
					&hash_value, hash_type);
				panic_printk
					("%s %d srcmacstr=%s dstmacstr=%s hash type =0x%x hash value = 0x%x \n",
					__FUNCTION__, __LINE__, srcmac_str,
					dstmac_str, hash_type, hash_value);
			} else if ((hash_type == LINK_TRUNK_HASH_TYPE_DA)) {
				sscanf(strptr, "%s", dstmac_str);
				string_to_hex(dstmac_str, dst_mac, 12);
				rtl_lag_hash_function(NULL, dst_mac,
					&hash_value, hash_type);
				panic_printk
					("%s %d srcmacstr=%s dstmacstr=%s hash type =0x%x hash value = 0x%x \n",
					__FUNCTION__, __LINE__, srcmac_str,
				...
				string_to_hex(srcmac_str, src_mac, 12);
				rtl_lag_hash_function(src_mac, NULL,
					&hash_value, hash_type);
				panic_printk
					("%s %d srcmacstr=%s dstmacstr=%s hash type =0x%x hash value = 0x%x \n",
					__FUNCTION__, __LINE__, srcmac_str,
...
extern int rtl_8367QosReadProc(char *page, char **start, off_t off,
	int count, int *eof, void *data);
#endif
extern int rtl_8367QosWriteProc(struct file *file, const char *buffer,
	unsigned long count, void *data);

#if definedCONFIG_RTL_PROC_NEW
int rtl_8367Qos_single_open(struct inode *inode, struct file *file)
{
	return (single_open(file, rtl_8367QosReadProc, NULL));
}

static ssize_t rtl_8367Qos_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
{
#ifdef CONFIG_RTL_PROC_NEW
	proc_create_data("rtl_8367Qos", 0, &proc_root, &rtl_8367Qos_proc_fops,
		NULL);
#else
...
#if definedCONFIG_RTL_8367_QOS_TEST
	rtl8367_qos_test();
#endif
	...
	for (i = 0; pVlanConfig[i].vid != 0; i++) {
		...
		if ((pVlanConfig[i].isWan == 0) && (_br0_ip_tmp != 0) && (_br0_mask_tmp != 0)) {	/*lan config */
			...
			if ((_br0_ip_bak != 0) && (_br0_mask_bak != 0)) {
				...
				rule.srcIpAddrStart_ =
					(_br0_ip_bak & _br0_mask_bak) +
					(~_br0_mask_bak) + 1;
...
inline void rtl865x_disableRunoutIrq(void)
{
	...
	REG32(CPUIIMR) =
		REG32(CPUIIMR) & (~(PKTHDR_DESC_RUNOUT_IP_ALL |
