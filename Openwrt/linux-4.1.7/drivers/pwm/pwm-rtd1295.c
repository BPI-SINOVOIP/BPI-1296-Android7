...
int set_real_freq_by_target_freq(struct rtd1295_pwm_chip *pc, int hwpwm, int target_freq) {
	...
	pr_info("--- %s debug : hwpwm=%d, target_freq=%d, real_freq=%d, osd=%d, ocd=%d\n",
...
int set_real_period_by_target_period(struct rtd1295_pwm_chip *pc, int hwpwm, int target_period_ns) {
	...
	pr_info("--- %s debug : hwpwm=%d, target_period_ns=%d (freq=%d), real_period_ns=%d (real_freq=%d), osd=%d, ocd=%d\n",
			__func__, hwpwm, target_period_ns, target_freq, real_period_ns, real_freq,
...
static void pwm_set_register(struct rtd1295_pwm_chip *pc, int hwpwm)
{
	...
	pr_info("--- debug : pwm_set_register -- set index %d OCD=%d,CD=%d,CSD=%d\n",
			    ...
			    int duty_ns, int period_ns)
{
	...
	pr_info("--- debug : rtd1295_pwm_config : hwpwm=%d, enable=%d duty_ns=%d period_ns=%d\n",
			pwm->hwpwm, pc->enable[pwm->hwpwm], duty_ns, period_ns);
...
{
	struct rtd1295_pwm_chip *pc = to_rtd1295_pwm_chip(chip);
	pr_info("--- debug : rtd1295_pwm_enable ---- hwpwm=%d, enable=%d \n", pwm->hwpwm, pc->enable[pwm->hwpwm]);
...
{
	struct rtd1295_pwm_chip *pc = to_rtd1295_pwm_chip(chip);
	pr_info("--- debug :  rtd1295_pwm_disable ---- disable PWM[%d] now !\n", pwm->hwpwm);
...
static ssize_t pwm_show_dutyRate(struct device *dev, struct device_attribute *attr, char *buf, int hwpwm)
{
	...
	pr_info("--- debug : pwm_show_dutyRate called : hwpwm=%d, dutyRate=> pc->dutyRate=[%d]--- \n", hwpwm, pc->duty_rate[hwpwm]);
...
static ssize_t pwm_store_dutyRate(struct device *dev, struct device_attribute *attr, const char *buf, size_t count, int hwpwm)
{
	...
	pr_info("--- debug : pwm_store_dutyRate called : hwpwm=%d, count:%ld ", hwpwm, count);
	...
	if( value < 0 || value > 100 ) {
		pr_err("--- debug : pwm_store_dutyRate  ====  input should between 0 ~ 100 \n");
		return count;
	}
...
static ssize_t pwm_show_enable(struct device *dev, struct device_attribute *attr, char *buf, int hwpwm)
{
	...
	pr_info("--- debug : pwm_show_enable called : hwpwm=%d, enable=%d---- \n", hwpwm, pc->enable[hwpwm]);
			 ...
			 const char *buf, size_t count, int hwpwm)
{
	...
	if( pc->enable[hwpwm] == value ) {
		pr_err("--- debug : rtd1295_pwm_enable	====  the same, do nothing (hwpwm=%d, value=%d) \n", hwpwm, value);
		return count;
	}
...
static ssize_t pwm_show_clksrcDiv(struct device *dev, struct device_attribute *attr, char *buf, int hwpwm)
{
	...
	pr_info("--- debug : pwm_show_clksrcDiv called : hwpwm=%d, clksrc_div=%d---- \n", hwpwm, pc->clksrc_div[hwpwm]);
			 ...
			 const char *buf, size_t count, int hwpwm)
{
	...
	if(value < 0 || value > 15) {
		pr_err("--- debug : pwm_store_clksrcDiv  ====  input should between 0 ~ 15 \n");
		return count;
	}

	if(pc->clksrc_div[hwpwm] == value) {
		pr_err("--- debug : pwm_store_clksrcDiv  ====  hwpwm=%d, input is the same=%d, do nothing! \n", hwpwm, value);
		return count;
	}
	...
	pr_info("--- debug : pwm_store_clksrcDiv  ====  done (hwpwm=%d, value=%d) \n", hwpwm, value);
	return count;
}
...
static ssize_t pwm_show_clkoutDiv(struct device *dev, struct device_attribute *attr, char *buf, int hwpwm)
{
	...
	pr_info("--- debug : pwm_show_clkoutDiv called : hwpwm=%d, clkout_div=%d---- \n", hwpwm, pc->clkout_div[hwpwm]);
			 ...
			 const char *buf, size_t count, int hwpwm)
{
	...
	if(value < 0 || value > 255) {
		pr_err("--- debug : pwm_store_clkoutDiv  ====  input should between 0 ~ 255 \n");
		return count;
	}

	if(pc->clkout_div[hwpwm] == value) {
		pr_err("--- debug : pwm_store_clkoutDiv  ====  hwpwm=%d, input is the same=%d, do nothing! \n", hwpwm, value);
		return count;
	}
	...
	pr_info("--- debug : pwm_store_clkoutDiv  ====  done (hwpwm=%d, value=%d) \n", hwpwm, value);
	return count;
}
...
static ssize_t pwm_show_out_freq(struct device *dev, struct device_attribute *attr, char *buf, int hwpwm)
{
	...
	pr_info("--- debug : pwm_show_out_freq called : hwpwm=%d, out_freq=%dHz---- \n", hwpwm, pc->out_freq[hwpwm]);
			 ...
			 const char *buf, size_t count, int hwpwm)
{
	...
	if(value < 0 || value > 27000000) {
		pr_err("--- debug : pwm_store_out_freq  ====  input should between 0 ~ 27MHz \n");
		return count;
	}

	if(pc->out_freq[hwpwm] == value) {
		pr_err("--- debug : pwm_store_out_freq  ====  hwpwm=%d, input is the same=%d, do nothing! \n", hwpwm, value);
		return count;
	}
	...
	pr_info("--- debug : pwm_store_out_freq  ====  done (hwpwm=%d, value=%d) \n", hwpwm, value);
	return count;
}
...
static int rtd1295_pwm_probe(struct platform_device *pdev)
{
	...
	pr_info("--- debug : rtd1295_pwm_probe - hwpwm=(%d) enable=(%d) duty_rate=(%d) clksrc_div=(%d) clkout_div=(%d)---  \n",
			0, pwm->enable[0], pwm->duty_rate[0], pwm->clksrc_div[0], pwm->clkout_div[0]);
	...
	pr_info("--- debug : rtd1295_pwm_probe - hwpwm=(%d) enable=(%d) duty_rate=(%d) clksrc_div=(%d) clkout_div=(%d)---  \n",
			1, pwm->enable[1], pwm->duty_rate[1], pwm->clksrc_div[1], pwm->clkout_div[1]);
	pr_info("--- debug : rtd1295_pwm_probe - defualt output frequence = %dHz ---  \n",
			pwm->out_freq[1]);
	...
	pr_info("--- debug : rtd1295_pwm_probe - hwpwm=(%d) enable=(%d) duty_rate=(%d) clksrc_div=(%d) clkout_div=(%d)---  \n",
			2, pwm->enable[2], pwm->duty_rate[2], pwm->clksrc_div[2], pwm->clkout_div[2]);
	pr_info("--- debug : rtd1295_pwm_probe - defualt output frequence = %dHz ---  \n",
			pwm->out_freq[2]);
	...
	pr_info("--- debug : rtd1295_pwm_probe - hwpwm=(%d) enable=(%d) duty_rate=(%d) clksrc_div=(%d) clkout_div=(%d)---  \n",
			3, pwm->enable[3], pwm->duty_rate[3], pwm->clksrc_div[3], pwm->clkout_div[3]);
	pr_info("--- debug : rtd1295_pwm_probe - defualt output frequence = %dHz ---  \n",
			pwm->out_freq[3]);
