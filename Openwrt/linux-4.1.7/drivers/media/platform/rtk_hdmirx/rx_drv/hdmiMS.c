...
int Hdmi_CRC_check(void)
{
	...
	hdmi_rx_reg_mask32(TMDS_CRCC, ~TMDS_CRCC_crcc_mask,0, HDMI_RX_MAC);//disable CRC check
	hdmi_rx_reg_mask32(TMDS_CRCC, ~TMDS_CRCC_crcc_mask, TMDS_CRCC_crcc(1), HDMI_RX_MAC);//enable CRC check

	while(!(hdmi_rx_reg_read32(TMDS_CRCC, HDMI_RX_MAC)&TMDS_CRCC_crc_done_mask))//wait CRC done bit
	{
...
bool Hdmi_GetInterlace(HDMI_MS_MODE_T mode)
{
	...
	switch (mode)
	{
		case HDMI_MS_MODE_ONESHOT:
			//hdmi_rx_reg_write32(HDMI_VCR, hdmi_rx_reg_read32(HDMI_VCR, HDMI_RX_MAC)|_BIT6, HDMI_RX_MAC); //clear status
			...
			if(hdmi_rx_reg_read32(HDMI_VCR, HDMI_RX_MAC) & HDMI_VCR_eot_mask)// interlace mode
			{
		...
		case HDMI_MS_MODE_CONTINUOUS_INIT:
			hdmi_rx_reg_mask32(HDMI_VCR, ~HDMI_VCR_eot_mask, HDMI_VCR_eot(1), HDMI_RX_MAC); //clear status
		...
		case HDMI_MS_MODE_CONTINUOUS_CHECK:
			...
			{
				if(hdmi_rx_reg_read32(HDMI_VCR, HDMI_RX_MAC) & _BIT6)// interlace mode
					progressive = 0;
				else 
					progressive = 1;
...
HDMI_3D_T Hdmi_Get3DInfo(HDMI_MS_MODE_T mode)
{
	...
	switch (mode)
	{
		case HDMI_MS_MODE_ONESHOT:
			...
			hdmi_rx_reg_write32(HDMI_GPVS, HDMI_GPVS_pis_6_mask, HDMI_RX_MAC); // Clear RSV2 indicator
			...
			for (timeout = 12; timeout >0; timeout--)
			{
				...
				{
					if (HDMI_VIDEO_FORMAT_get_hvf(hdmi_rx_reg_read32(HDMI_VIDEO_FORMAT, HDMI_RX_MAC)) != 0x02){ // if 3D format indication present is absent, return 2D anyway
				...
				{
				 // HDMI_PRINTF(" delay!!! Don't GET  Hdmi_Get3DInfo \n");
				}
			}
			if (timeout == 0 )	 HDMI_PRINTF("	Hdmi_Get3DInfo timeout \n");
			return HDMI3D_2D_ONLY;
		break;

		case HDMI_MS_MODE_ONESHOT_INIT:
			hdmi_rx_reg_mask32(HDMI_PTRSV1 , 0xffff00ff, 0x00008100, HDMI_RX_MAC);//Packet Type = 0x81(HDMI Vendor Specific InfoFrame)
			hdmi_rx_reg_write32(HDMI_GPVS, HDMI_GPVS_pis_6_mask, HDMI_RX_MAC);  /*Clear RSV2 indicator*/
		break;
...
void Hdmi_Get3DGenTiming(HDMI_TIMING_T *tx_timing, HDMI_TIMING_T *gen_timing)
{
	...
	switch (hdmi.path_policy)
	{
		...
		case HDMI_PATH_POLICY_3D_DMA:
			if (hdmi.hdmi3d_capability == 0) gen_timing->hdmi_3dformat = HDMI3D_2D_ONLY;
			else if (hdmi.force_all_3d_disable) gen_timing->hdmi_3dformat = HDMI3D_2D_ONLY;
	...
	{
		case HDMI3D_2D_ONLY:
			if (tx_timing->hdmi_3dformat != HDMI3D_2D_ONLY) {  // 3d convert to 2D , don't do DI
				gen_timing->progressive = 1;
			} else {
				gen_timing->progressive = tx_timing->progressive;
			}
	...
	if (hdmi.path_policy == HDMI_PATH_POLICY_3D_NODMA) {
		if (gen_timing->hdmi_3dformat == HDMI3D_FRAME_PACKING && tx_timing->hdmi_3dformat  == HDMI3D_FRAME_PACKING && gen_timing->progressive == 0) {
...
int hdmi_onms_measure(HDMI_TIMING_T *timing)
{
	...
	if((timing->h_act_len == 0) || (timing->v_act_len == 0) || (timing->h_total == 0) || (timing->v_total == 0))
...
bool Hdmi_MeasureTiming(HDMI_TIMING_T *timing, int b)
{
	...
	if (hdmi_onms_measure(timing) != HDMI_ERR_NO)  //to check
		return FALSE;

	// if down sample number change ---> measure again
...
#if HDMI2p0	
		if (bHDMI_420_Space==COLOR_YUV420)//cloud test 420 timing
