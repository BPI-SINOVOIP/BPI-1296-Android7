...
#if HDMI2p0
extern u_int8_t   bHDMI_6G_flag,b6G_detect_cnt ;
#endif
extern HDMI_INFO_T hdmi;

const HDMI_PHY_PARAM_T hdmi_phy_param[] = //QC 2016/1/18
{
//RD SD suggestion
...
void Forground_EQ(void)
{
	...
	HDMIRX_DEBUG("[%s] EQ manual 0x%08x",__FUNCTION__,hdmi_rx_reg_read32(P1_B1, HDMI_RX_PHY));
}

void Hdmi_TriggerbMeasurement(UINT8 bport)
{
	// mode dect set release (enable ??)
	...
	hdmi_rx_reg_mask32(CLKDETCR, ~_BIT0, 0, HDMI_RX_MAC);//PMM clk detect function disable
	hdmi_rx_reg_mask32(CLKDETSR, ~_BIT0, _BIT0, HDMI_RX_MAC);//clr PMM clk detect flag
	hdmi_rx_reg_mask32(CLKDETCR, ~_BIT0, _BIT0, HDMI_RX_MAC);//PMM clk detect function enable
}
...
void DEMUX_RESET(UINT8 benable)
{
	...
	//hdmiport_mask(DFE_HDMI_RX_PHY_RST_reg,~DFE_HDMI_RX_PHY_RST_dfe_rstn_n_mask,0);

	if (benable ==0)//release
	{
		...
		hdmi_rx_reg_mask32(RST, ~(RST_p0_ck_pllrstb_mask|RST_p0_ck_vcorstb_mask), 0, HDMI_RX_PHY);
		hdmi_rx_reg_mask32(RST, ~(RST_p0_r_demux_rstb_mask|RST_p0_g_demux_rstb_mask|RST_p0_b_demux_rstb_mask), 0, HDMI_RX_PHY);
	}
	//hdmiport_mask(DFE_HDMI_RX_PHY_RST_reg,~DFE_HDMI_RX_PHY_RST_dfe_rstn_n_mask,DFE_HDMI_RX_PHY_RST_dfe_rstn_n_mask);
}
...
void CDR_RESET(UINT8 benable)
{
	if(benable ==0)//release
	{
		hdmi_rx_reg_mask32(REGD0, ~(REGD0_p0_r_dig_rst_n_mask|REGD0_p0_g_dig_rst_n_mask|REGD0_p0_b_dig_rst_n_mask|REGD0_p0_r_cdr_rst_n_mask|REGD0_p0_g_cdr_rst_n_mask|REGD0_p0_b_cdr_rst_n_mask),
									(REGD0_p0_r_dig_rst_n_mask|REGD0_p0_g_dig_rst_n_mask|REGD0_p0_b_dig_rst_n_mask|REGD0_p0_r_cdr_rst_n_mask|REGD0_p0_g_cdr_rst_n_mask|REGD0_p0_b_cdr_rst_n_mask), HDMI_RX_PHY);
		//HDMI_PRINTF("[HDMI] DEMUX_release\n");
	}
	else// reset
	{
		hdmi_rx_reg_mask32(REGD0, ~(REGD0_p0_r_dig_rst_n_mask|REGD0_p0_g_dig_rst_n_mask|REGD0_p0_b_dig_rst_n_mask), 0, HDMI_RX_PHY);
		hdmi_rx_reg_mask32(REGD0, ~(REGD0_p0_r_cdr_rst_n_mask|REGD0_p0_g_cdr_rst_n_mask|REGD0_p0_b_cdr_rst_n_mask), 0, HDMI_RX_PHY);
...
void DIC_AFIFO_ENABLE(u_int8_t enable)
{
	if (enable == 1)
	{
		...
		hdmi_rx_reg_mask32(CH_CR, ~(CH_CR_r_ch_afifo_en_mask|CH_CR_g_ch_afifo_en_mask|CH_CR_b_ch_afifo_en_mask),
									(CH_CR_r_ch_afifo_en_mask|CH_CR_g_ch_afifo_en_mask|CH_CR_b_ch_afifo_en_mask), HDMI_RX_MAC);
	}
	else
	{
		hdmi_rx_reg_mask32(CH_CR, ~(CH_CR_r_ch_afifo_en_mask|CH_CR_g_ch_afifo_en_mask|CH_CR_b_ch_afifo_en_mask), 0, HDMI_RX_MAC);
	}
}
...
void CLOSE_RST_MAC(void)
{
	...
	//cloud modify dic fifio reset
	DIC_AFIFO_ENABLE(0);

	hdmi_rx_reg_mask32(TMDS_PWDCTL, ~(TMDS_PWDCTL_ebip_mask|TMDS_PWDCTL_egip_mask|TMDS_PWDCTL_erip_mask|TMDS_PWDCTL_ecc_mask), 0, HDMI_RX_MAC);//disable TMDS input
...
void OPEN_RST_MAC(void)
{
	...
	hdmi_rx_reg_mask32(TMDS_PWDCTL, ~(TMDS_PWDCTL_ebip_mask|TMDS_PWDCTL_egip_mask|TMDS_PWDCTL_erip_mask|TMDS_PWDCTL_ecc_mask),
								(TMDS_PWDCTL_ebip(1)|TMDS_PWDCTL_egip(1)|TMDS_PWDCTL_erip(1)|TMDS_PWDCTL_ecc(1)), HDMI_RX_MAC);//enable TMDS input
	usleep_range(1000, 1100);
...
void EQ_bias_band_setting(u_int32_t b)
{
	...
	if (b> clock_bound_3g ) // over  3G HBR
	{
#if 1//QC 2016/1/18
		/*B lane */
		// tap0_ISEL   LEQ_ISEL   EQ_bit_rate  tap_HBR
		hdmi_rx_reg_mask32(B2_addr, ~(P0_b_7_TAP0_ISEL|P0_b_7_LEQ_ISEL|P0_b_6_LEQ_BIT_RATE|P0_b_6_TAP0_HBR|P0_b_5_LEQ_CURRENT_ADJ),
									(P0_b_7_TAP0_ISEL_400uA|P0_b_7_LEQ_ISEL_400uA|P0_b_6_LEQ_BIT_RATE_HBR|P0_b_6_LEQ_BIT_RATE_HBR|P0_b_6_TAP0_HBR|P0_b_5_LEQ_CURRENT_ADJ_1p25X), HDMI_RX_PHY);//tap0
		/*G lane */
		// tap0_ISEL   LEQ_ISEL   EQ_bit_rate  tap_HBR
		hdmi_rx_reg_mask32(G2_addr, ~(P0_g_7_TAP0_ISEL|P0_g_7_LEQ_ISEL|P0_g_6_LEQ_BIT_RATE|P0_g_6_TAP0_HBR|P0_g_5_LEQ_CURRENT_ADJ),
									(P0_b_7_TAP0_ISEL_400uA|P0_b_7_LEQ_ISEL_400uA|P0_g_6_LEQ_BIT_RATE_HBR|P0_g_6_LEQ_BIT_RATE_HBR|P0_g_6_TAP0_HBR|P0_g_5_LEQ_CURRENT_ADJ_1p25X), HDMI_RX_PHY);//tap0
		
		/*R lane */
		// tap0_ISEL   LEQ_ISEL   EQ_bit_rate  tap_HBR
		hdmi_rx_reg_mask32(R2_addr, ~(P0_r_7_TAP0_ISEL|P0_r_7_LEQ_ISEL|P0_r_6_LEQ_BIT_RATE|P0_r_6_TAP0_HBR|P0_r_5_LEQ_CURRENT_ADJ),
									(P0_b_7_TAP0_ISEL_400uA|P0_b_7_LEQ_ISEL_400uA|P0_r_6_LEQ_BIT_RATE_HBR|P0_r_6_LEQ_BIT_RATE_HBR|P0_r_6_TAP0_HBR|P0_r_5_LEQ_CURRENT_ADJ_1p25X), HDMI_RX_PHY);//tap0
#else
		/*B lane */
		// tap0_ISEL   LEQ_ISEL   EQ_bit_rate  tap_HBR
		hdmi_rx_reg_mask32(B2_addr, ~(P0_b_7_TAP0_ISEL|P0_b_7_LEQ_ISEL|P0_b_6_LEQ_BIT_RATE|P0_b_6_TAP0_HBR|P0_b_5_LEQ_CURRENT_ADJ),
									(P0_b_7_TAP0_ISEL|P0_b_7_LEQ_ISEL|P0_b_6_LEQ_BIT_RATE_HBR|P0_b_6_LEQ_BIT_RATE_HBR|P0_b_6_TAP0_HBR|P0_b_5_LEQ_CURRENT_ADJ_1p25X), HDMI_RX_PHY);//tap0

		/*G lane */
		// tap0_ISEL   LEQ_ISEL   EQ_bit_rate  tap_HBR
		hdmi_rx_reg_mask32(G2_addr, ~(P0_g_7_TAP0_ISEL|P0_g_7_LEQ_ISEL|P0_g_6_LEQ_BIT_RATE|P0_g_6_TAP0_HBR|P0_g_5_LEQ_CURRENT_ADJ),
								(P0_g_7_TAP0_ISEL|P0_g_7_LEQ_ISEL|P0_g_6_LEQ_BIT_RATE_HBR|P0_g_6_LEQ_BIT_RATE_HBR|P0_g_6_TAP0_HBR|P0_g_5_LEQ_CURRENT_ADJ_1p25X), HDMI_RX_PHY);//tap0

		/*R lane */
		// tap0_ISEL   LEQ_ISEL   EQ_bit_rate  tap_HBR
		hdmi_rx_reg_mask32(R2_addr, ~(P0_r_7_TAP0_ISEL|P0_r_7_LEQ_ISEL|P0_r_6_LEQ_BIT_RATE|P0_r_6_TAP0_HBR|P0_r_5_LEQ_CURRENT_ADJ),
								(P0_r_7_TAP0_ISEL|P0_r_7_LEQ_ISEL|P0_r_6_LEQ_BIT_RATE_HBR|P0_r_6_LEQ_BIT_RATE_HBR|P0_r_6_TAP0_HBR|P0_r_5_LEQ_CURRENT_ADJ_1p25X), HDMI_RX_PHY);//tap0
#endif
		//need check
		//B
		hdmi_rx_reg_mask32(B3_addr,~(P0_b_11_ACDR_RATE_SEL_HALF_RATE|P0_b_10_DFE_SUMAMP_ISEL_mask|P0_b_10_DFE_PREAMP_ISEL_mask|P0_b_10_DFE_SUMAMP_LP_MANUALOFF),
									(P0_b_11_ACDR_RATE_SEL_HALF_RATE|P0_b_10_DFE_SUMAMP_ISEL_400uA|P0_b_10_DFE_PREAMP_ISEL_400uA|P0_b_10_DFE_SUMAMP_LP_MANUALOFF), HDMI_RX_PHY);
		//G
		hdmi_rx_reg_mask32(G3_addr,~(P0_g_11_ACDR_RATE_SEL_HALF_RATE|P0_g_10_DFE_SUMAMP_ISEL_mask|P0_g_10_DFE_PREAMP_ISEL_mask|P0_g_10_DFE_SUMAMP_LP_MANUALOFF),
									(P0_g_11_ACDR_RATE_SEL_HALF_RATE|P0_g_10_DFE_SUMAMP_ISEL_400uA|P0_g_10_DFE_PREAMP_ISEL_400uA|P0_g_10_DFE_SUMAMP_LP_MANUALOFF), HDMI_RX_PHY);
		//R
		hdmi_rx_reg_mask32(R3_addr,~(P0_r_11_ACDR_RATE_SEL_HALF_RATE|P0_r_10_DFE_SUMAMP_ISEL_mask|P0_r_10_DFE_PREAMP_ISEL_mask|P0_r_10_DFE_SUMAMP_LP_MANUALOFF),
									(P0_r_11_ACDR_RATE_SEL_HALF_RATE|P0_r_10_DFE_SUMAMP_ISEL_400uA|P0_r_10_DFE_PREAMP_ISEL_400uA|P0_r_10_DFE_SUMAMP_LP_MANUALOFF), HDMI_RX_PHY);
...
#else
		hdmi_rx_reg_mask32(B1_addr, ~(P1_b_2_LEQ_BIT_RATE_mask), P1_b_2_LEQ_BIT_RATE_HBR, HDMI_RX_PHY);
		hdmi_rx_reg_mask32(G1_addr, ~(P1_g_2_LEQ_BIT_RATE_mask), P1_g_2_LEQ_BIT_RATE_HBR, HDMI_RX_PHY);
		hdmi_rx_reg_mask32(R1_addr, ~(P1_r_2_LEQ_BIT_RATE_mask), P1_r_2_LEQ_BIT_RATE_HBR, HDMI_RX_PHY);
#endif
		...
		else if ((b>TMDS_4G)&&(b<TMDS_5G) )   //4 //  4G~5G
		{
			//B
	...
	else if (b>clock_bound_1p5g)  //3//  3G ~1.5G
	{
		/*B lane */
		// tap0_ISEL   LEQ_ISEL   EQ_bit_rate  tap_HBR
		hdmi_rx_reg_mask32(B2_addr, ~(P0_b_7_TAP0_ISEL|P0_b_7_LEQ_ISEL|P0_b_6_LEQ_BIT_RATE|P0_b_6_TAP0_HBR|P0_b_5_LEQ_CURRENT_ADJ),
								(P0_b_7_TAP0_ISEL_400uA|P0_b_7_LEQ_ISEL_200uA|P0_b_6_LEQ_BIT_RATE_MBR|P0_b_6_TAP0_HBR|P0_b_5_LEQ_CURRENT_ADJ_1X), HDMI_RX_PHY);//tap0

		/*G lane */
		// tap0_ISEL   LEQ_ISEL   EQ_bit_rate  tap_HBR
		hdmi_rx_reg_mask32(G2_addr, ~(P0_g_7_TAP0_ISEL|P0_g_7_LEQ_ISEL|P0_g_6_LEQ_BIT_RATE|P0_g_6_TAP0_HBR|P0_g_5_LEQ_CURRENT_ADJ),
								(P0_g_7_TAP0_ISEL_400uA|P0_g_7_LEQ_ISEL_200uA|P0_g_6_LEQ_BIT_RATE_MBR|P0_g_6_TAP0_HBR|P0_g_5_LEQ_CURRENT_ADJ_1X), HDMI_RX_PHY);//tap0


		/*R lane */
		// tap0_ISEL   LEQ_ISEL   EQ_bit_rate  tap_HBR
		hdmi_rx_reg_mask32(R2_addr, ~(P0_r_7_TAP0_ISEL|P0_r_7_LEQ_ISEL|P0_r_6_LEQ_BIT_RATE|P0_r_6_TAP0_HBR|P0_r_5_LEQ_CURRENT_ADJ),
								(P0_r_7_TAP0_ISEL_400uA|P0_r_7_LEQ_ISEL_200uA|P0_r_6_LEQ_BIT_RATE_MBR|P0_r_6_TAP0_HBR|P0_r_5_LEQ_CURRENT_ADJ_1X), HDMI_RX_PHY);//tap0

		//B
		hdmi_rx_reg_mask32(B3_addr, ~(P0_b_11_ACDR_RATE_SEL_HALF_RATE|P0_b_10_DFE_SUMAMP_ISEL_mask|P0_b_10_DFE_PREAMP_ISEL_mask|P0_b_10_DFE_SUMAMP_LP_MANUALOFF),
								(P0_b_11_ACDR_RATE_SEL_HALF_RATE|P0_b_10_DFE_SUMAMP_ISEL_400uA|P0_b_10_DFE_PREAMP_ISEL_400uA), HDMI_RX_PHY);
		//G
		hdmi_rx_reg_mask32(G3_addr, ~(P0_g_11_ACDR_RATE_SEL_HALF_RATE|P0_g_10_DFE_SUMAMP_ISEL_mask|P0_g_10_DFE_PREAMP_ISEL_mask|P0_g_10_DFE_SUMAMP_LP_MANUALOFF),
								(P0_g_11_ACDR_RATE_SEL_HALF_RATE|P0_g_10_DFE_SUMAMP_ISEL_400uA|P0_g_10_DFE_PREAMP_ISEL_400uA), HDMI_RX_PHY);
		//R
		hdmi_rx_reg_mask32(R3_addr, ~(P0_r_11_ACDR_RATE_SEL_HALF_RATE|P0_r_10_DFE_SUMAMP_ISEL_mask|P0_r_10_DFE_PREAMP_ISEL_mask|P0_r_10_DFE_SUMAMP_LP_MANUALOFF),
								(P0_r_11_ACDR_RATE_SEL_HALF_RATE|P0_r_10_DFE_SUMAMP_ISEL_400uA|P0_r_10_DFE_PREAMP_ISEL_400uA), HDMI_RX_PHY);
...
#else
		hdmi_rx_reg_mask32(B1_addr, ~(P1_b_2_LEQ_BIT_RATE_mask), P1_b_2_LEQ_BIT_RATE_MBR, HDMI_RX_PHY);
		hdmi_rx_reg_mask32(G1_addr, ~(P1_g_2_LEQ_BIT_RATE_mask), P1_g_2_LEQ_BIT_RATE_MBR, HDMI_RX_PHY);
		hdmi_rx_reg_mask32(R1_addr, ~(P1_r_2_LEQ_BIT_RATE_mask), P1_r_2_LEQ_BIT_RATE_MBR, HDMI_RX_PHY);
#endif
	...
	else // under 1.5G
	{
...
#if 1//QC 2016/1/18
		hdmi_rx_reg_mask32(B2_addr,~(P0_b_7_TAP0_ISEL|P0_b_7_LEQ_ISEL|P0_b_6_LEQ_BIT_RATE|P0_b_6_TAP0_HBR|P0_b_5_LEQ_CURRENT_ADJ),
									(P0_b_5_LEQ_CURRENT_ADJ_1X|P0_b_7_TAP0_ISEL_400uA), HDMI_RX_PHY);//tap0
		
		/*G lane */
		// tap0_ISEL   LEQ_ISEL   EQ_bit_rate  tap_HBR
		hdmi_rx_reg_mask32(G2_addr,~(P0_g_7_TAP0_ISEL|P0_g_7_LEQ_ISEL|P0_g_6_LEQ_BIT_RATE|P0_g_6_TAP0_HBR|P0_g_5_LEQ_CURRENT_ADJ),
									(P0_g_5_LEQ_CURRENT_ADJ_1X|P0_g_7_TAP0_ISEL_400uA), HDMI_RX_PHY);//tap0
		
		/*R lane */
		// tap0_ISEL   LEQ_ISEL   EQ_bit_rate  tap_HBR
		hdmi_rx_reg_mask32(R2_addr,~(P0_r_7_TAP0_ISEL|P0_r_7_LEQ_ISEL|P0_r_6_LEQ_BIT_RATE|P0_r_6_TAP0_HBR|P0_r_5_LEQ_CURRENT_ADJ),
									(P0_r_5_LEQ_CURRENT_ADJ_1X|P0_r_7_TAP0_ISEL_400uA), HDMI_RX_PHY);//tap0
#else

		hdmi_rx_reg_mask32(B2_addr, ~(P0_b_7_TAP0_ISEL|P0_b_7_LEQ_ISEL|P0_b_6_LEQ_BIT_RATE|P0_b_6_TAP0_HBR|P0_b_5_LEQ_CURRENT_ADJ),
									(P0_b_5_LEQ_CURRENT_ADJ_1X|P0_b_7_TAP0_ISEL_400uA|P0_b_6_TAP0_HBR), HDMI_RX_PHY);//tap0
		hdmi_rx_reg_mask32(G2_addr, ~(P0_g_7_TAP0_ISEL|P0_g_7_LEQ_ISEL|P0_g_6_LEQ_BIT_RATE|P0_g_6_TAP0_HBR|P0_g_5_LEQ_CURRENT_ADJ),
									(P0_g_5_LEQ_CURRENT_ADJ_1X|P0_g_7_TAP0_ISEL_400uA|P0_g_6_TAP0_HBR), HDMI_RX_PHY);//tap0
		hdmi_rx_reg_mask32(R2_addr, ~(P0_r_7_TAP0_ISEL|P0_r_7_LEQ_ISEL|P0_r_6_LEQ_BIT_RATE|P0_r_6_TAP0_HBR|P0_r_5_LEQ_CURRENT_ADJ),
									(P0_r_5_LEQ_CURRENT_ADJ_1X|P0_r_7_TAP0_ISEL_400uA|P0_r_6_TAP0_HBR), HDMI_RX_PHY);//tap0
#endif
		...
		//LEQ_BIT_RATE
		/*
		hdmiport_mask(B1_addr,~(P1_b_2_LEQ_BIT_RATE_mask),P1_b_2_LEQ_BIT_RATE_MBR);
		hdmiport_mask(G1_addr,~(P1_g_2_LEQ_BIT_RATE_mask),P1_g_2_LEQ_BIT_RATE_MBR);
		hdmiport_mask(R1_addr,~(P1_r_2_LEQ_BIT_RATE_mask),P1_r_2_LEQ_BIT_RATE_MBR);
		*/
		//	 hdmiport_mask(B1_addr,~(P1_b_2_LEQ_BIT_RATE_mask),P1_b_2_LEQ_BIT_RATE_MBR);
#if 1//QC 2016/1/18
...
#else
 		hdmi_rx_reg_mask32(B1_addr, ~(P1_b_2_LEQ_BIT_RATE_mask), 0, HDMI_RX_PHY);
 		hdmi_rx_reg_mask32(G1_addr, ~(P1_g_2_LEQ_BIT_RATE_mask), 0, HDMI_RX_PHY);
 		hdmi_rx_reg_mask32(R1_addr, ~(P1_r_2_LEQ_BIT_RATE_mask), 0, HDMI_RX_PHY);
#endif
...
void HDMI_Set_Phy_Table(u_int8_t mode)
{
	...
	//M
	hdmi_rx_reg_mask32(wReg1, ~REGD16_reg_p0_n_code_mask, hdmi_phy_param[mode].M_code<<REGD16_reg_p0_n_code_shift, HDMI_RX_PHY);//M code set
	//N code
	hdmi_rx_reg_mask32(wCk1Reg, ~(p1_ck_3_CMU_N_code|p1_ck_3_CMU_PREDIVN), hdmi_phy_param[mode].N_code<<P0_CK1_reg_p0_ck_3_CMU_N_code_shift, HDMI_RX_PHY);//N code set

	//PI_DIV
	hdmi_rx_reg_mask32(B2Reg,~P0_b_8_PR_RATE_SEL, hdmi_phy_param[mode].PI_DIV<<P0_B2_reg_p0_b_8_PR_RATE_SEL_shift, HDMI_RX_PHY);//PI_DIV code set
	hdmi_rx_reg_mask32(G2REG,~P0_b_8_PR_RATE_SEL, hdmi_phy_param[mode].PI_DIV<<P0_B2_reg_p0_b_8_PR_RATE_SEL_shift, HDMI_RX_PHY);//PI_DIV code set
	hdmi_rx_reg_mask32(R2REG,~P0_b_8_PR_RATE_SEL, hdmi_phy_param[mode].PI_DIV<<P0_B2_reg_p0_b_8_PR_RATE_SEL_shift, HDMI_RX_PHY);//PI_DIV code set
	//DEMUX
	hdmi_rx_reg_mask32(B2Reg,~P0_b_8_DEMUX, hdmi_phy_param[mode].DEMUX<<P0_B2_reg_p0_b_8_DEMUX_shift, HDMI_RX_PHY);//DEMUX code set
	hdmi_rx_reg_mask32(G2REG,~P0_b_8_DEMUX, hdmi_phy_param[mode].DEMUX<<P0_B2_reg_p0_b_8_DEMUX_shift, HDMI_RX_PHY);//DEMUX code set
	hdmi_rx_reg_mask32(R2REG,~P0_b_8_DEMUX, hdmi_phy_param[mode].DEMUX<<P0_B2_reg_p0_b_8_DEMUX_shift, HDMI_RX_PHY);//DEMUX code set

	// LDO enable,CMU VCO select
	//need check when different freq
	hdmi_rx_reg_mask32(wCk2Reg,~(P0_ck_5_CMU_SEL_R1_mask|P0_ck_5_CMU_SEL_PUMP_I_mask|P0_ck_6_CMU_BIG_KVCO_mask|P0_ck_7_CMU_BYPASS_PI_mask|P0_ck_8_LDO_EN_mask|P0_ck_8_VSEL_LDO_D_mask|P0_ck_8_VSEL_LDO_A_mask),
								((hdmi_phy_param[mode].CK_RS<<0)|(hdmi_phy_param[mode].CK_Icp<<4)|P0_ck_6_CMU_BIG_KVCO_mask|P0_ck_7_CMU_BYPASS_PI_mask|P0_ck_8_LDO_EN_mask|DFE_HDMI_RX_PHY_P0_VSEL_LDO_A(hdmi_phy_param[mode].vsel_ldo)), HDMI_RX_PHY);
	// KP set
	hdmi_rx_reg_mask32(wReg2, ~(REGD0_reg_p0_kp_mask|REGD0_reg_p0_bypass_sdm_int_mask|REGD5_reg_p1_rate_sel_mask),
								((hdmi_phy_param[mode].CDR_KP<<REGD0_reg_p0_kp_shift)|REGD0_reg_p0_bypass_sdm_int_mask|(REGD5_reg_p1_rate_sel(hdmi_phy_param[mode].RATE_SEL))), HDMI_RX_PHY);

	hdmi_rx_reg_mask32(ck_md_sel, ~REGD42_reg_p0_ck_md_sel_mask, REGD42_reg_p0_ck_md_sel(0), HDMI_RX_PHY);

#if 1//QC 2016/1/18
	hdmi_rx_reg_mask32(wCk2Reg, ~(P1_ck_6_CMU_SEL_CS_mask), hdmi_phy_param[mode].CK_CS<<8, HDMI_RX_PHY); //MHL setting suggested by Edward
...
{
	hdmi_rx_reg_mask32(REGD21, ~(REGD28_reg_p1_auto_mode_mask|REGD28_reg_p1_adp_en_manual_mask|REGD28_reg_p1_adp_time_mask|REGD28_reg_p1_calib_time_mask|REGD28_reg_p1_calib_late_mask|REGD28_reg_p1_vco_coarse_mask|REGD28_reg_p1_divide_num_mask|REGD28_reg_p1_init_time_mask),
								(REGD28_reg_p1_adp_en_manual_mask|_BIT25|_BIT20|_BIT18|_BIT16|_BIT15|_BIT9|_BIT7|_BIT1), HDMI_RX_PHY);

	//hdmi_rx_reg_mask32(REGD22, ~(REGD29_reg_p1_lock_up_limit_mask|REGD29_reg_p1_lock_dn_limit_mask), (REGD29_reg_p1_lock_up_limit(0x462)|REGD29_reg_p1_lock_dn_limit(0x44e)), HDMI_RX_PHY);
	//hdmi_rx_reg_mask32(REGD23, ~(REGD30_reg_p1_cdr_cp_mask|REGD30_reg_p1_calib_reset_sel_mask), (REGD30_reg_p1_cdr_cp(0x44332)|REGD30_reg_p1_calib_reset_sel_mask), HDMI_RX_PHY);
	...
	hdmi_rx_reg_mask32(REGD26, ~(REGD33_reg_p1_rxidle_bypass_mask|REGD33_reg_p1_slope_band_mask|REGD33_reg_p1_old_mode_mask|_BIT15),(REGD33_reg_p1_rxidle_bypass_mask|REGD33_reg_p1_slope_band(8)|_BIT15), HDMI_RX_PHY);

	//Digital part Reg setting
	hdmi_rx_reg_mask32(PHY_FIFO_CR,~(PHY_FIFO_CR_port0_b_afifo_en_mask|PHY_FIFO_CR_port0_b_flush_mask|PHY_FIFO_CR_port0_g_afifo_en_mask|PHY_FIFO_CR_port0_g_flush_mask|PHY_FIFO_CR_port0_r_afifo_en_mask|PHY_FIFO_CR_port0_r_flush_mask),
									(PHY_FIFO_CR_port0_b_afifo_en_mask|PHY_FIFO_CR_port0_b_flush_mask|PHY_FIFO_CR_port0_g_afifo_en_mask|PHY_FIFO_CR_port0_g_flush_mask|PHY_FIFO_CR_port0_r_afifo_en_mask|PHY_FIFO_CR_port0_r_flush_mask), HDMI_RX_MAC);
}

void DFE_second_flow(void)
{
	UINT8 bLan0_TAP0Max,bLan1_TAP0Max,bLan2_TAP0Max;
	...
	hdmi_rx_reg_mask32(REG_DFE_INIT0_L0, ~(REG_DFE_INIT0_L0_tap0_init_lane0_mask|REG_DFE_INIT0_L0_vth_init_lane0_mask), (REG_DFE_INIT0_L0_tap0_init_lane0(bLan0_TAP0Max-1))|REG_DFE_INIT0_L0_vth_init_lane0(0x3), HDMI_RX_DEF);
	hdmi_rx_reg_mask32(REG_DFE_INIT0_L1, ~(REG_DFE_INIT0_L0_tap0_init_lane0_mask|REG_DFE_INIT0_L0_vth_init_lane0_mask), (REG_DFE_INIT0_L0_tap0_init_lane0(bLan1_TAP0Max-1))|REG_DFE_INIT0_L0_vth_init_lane0(0x3), HDMI_RX_DEF);
	hdmi_rx_reg_mask32(REG_DFE_INIT0_L2, ~(REG_DFE_INIT0_L0_tap0_init_lane0_mask|REG_DFE_INIT0_L0_vth_init_lane0_mask), (REG_DFE_INIT0_L0_tap0_init_lane0(bLan2_TAP0Max-1))|REG_DFE_INIT0_L0_vth_init_lane0(0x3), HDMI_RX_DEF);

	//load initial data
	hdmi_rx_reg_mask32(REG_DFE_INIT1_L0, ~(load_in_init_lane0_tap0|load_in_init_lane0_vth), (load_in_init_lane0_tap0|load_in_init_lane0_vth), HDMI_RX_DEF);
	hdmi_rx_reg_mask32(REG_DFE_INIT1_L1, ~(load_in_init_lane0_tap0|load_in_init_lane0_vth), (load_in_init_lane0_tap0|load_in_init_lane0_vth), HDMI_RX_DEF);
	hdmi_rx_reg_mask32(REG_DFE_INIT1_L2, ~(load_in_init_lane0_tap0|load_in_init_lane0_vth), (load_in_init_lane0_tap0|load_in_init_lane0_vth), HDMI_RX_DEF);

	//HDMI_PRINTF("[HDMI][DFE open] DFE load initial value \n");
...
void PHY_FORE_KOFF(void)
{
	...
	UINT32 koff_r_result,koff_g_result,koff_b_result;
	...
	hdmi_rx_reg_mask32(w_b_addr_control, ~(P0_b_3_FORE_EN_FORE_KOFF|P0_b_4_FORE_KOFF_AUTOK|P0_b_4_FORE_POW_FORE_KOFF),(P0_b_3_FORE_EN_FORE_KOFF|P0_b_4_FORE_KOFF_AUTOK), HDMI_RX_PHY);
	hdmi_rx_reg_mask32(w_g_addr_control, ~(P0_b_3_FORE_EN_FORE_KOFF|P0_b_4_FORE_KOFF_AUTOK|P0_b_4_FORE_POW_FORE_KOFF),(P0_b_3_FORE_EN_FORE_KOFF|P0_b_4_FORE_KOFF_AUTOK), HDMI_RX_PHY);
	hdmi_rx_reg_mask32(w_r_addr_control, ~(P0_b_3_FORE_EN_FORE_KOFF|P0_b_4_FORE_KOFF_AUTOK|P0_b_4_FORE_POW_FORE_KOFF),(P0_b_3_FORE_EN_FORE_KOFF|P0_b_4_FORE_KOFF_AUTOK), HDMI_RX_PHY);
	// cut input off
	...
	for (i=0; i<Fore_Koff_Range+1; i++)
	{
		...
		while(!((hdmi_rx_reg_read32(w_b_addr_result, HDMI_RX_PHY))&P0_B4_reg_p0_b_koffok_mask))
		{
			...
			usleep_range(10000, 11000);//for wait koffok
			j++;
		}
		HDMI_PRINTF("2435----------\n");
		...
		//HDMI_PRINTF("PHY_FORE_KOFF range =%x  test =%x	 %x    \n", hdmi_in(w_b_addr_result),w_b_addr_result,DFE_HDMI_RX_PHY_P0_B4_get_reg_p0_b_koff_sel(hdmi_in(w_b_addr_result)));
		if (!(hdmi_rx_reg_read32(w_b_addr_result, HDMI_RX_PHY) & P0_B4_reg_p0_b_koff_bound_mask))
		{
		...
		{
			HDMI_PRINTF("PHY_FORE_KOFF range out of B range  \n");
		}
	}
	...
	for (i=0; i<Fore_Koff_Range+1; i++)
	{
		...
		//koff_g_result = 0x11;
		//HDMI_PRINTF("PHY_FORE_KOFF range =%x	test =%x   %x \n", hdmi_in(w_g_addr_result),w_g_addr_result,DFE_HDMI_RX_PHY_P0_B4_get_reg_p0_b_koff_sel(hdmi_in(w_g_addr_result)));
		if (!(hdmi_rx_reg_read32(w_g_addr_result, HDMI_RX_PHY) & P0_B4_reg_p0_b_koff_bound_mask))
		{
		...
		{
			HDMI_PRINTF("PHY_FORE_KOFF range out of G range  \n");
		}
	}
	...
	for (i=0;i<3+1;i++)
	{
		...
		//HDMI_PRINTF("PHY_FORE_KOFF range =%x  test =%x	 %x \n", hdmi_in(w_r_addr_result),w_r_addr_result,DFE_HDMI_RX_PHY_P0_B4_get_reg_p0_b_koff_sel(hdmi_in(w_r_addr_result)));
		if (!(hdmi_rx_reg_read32(w_r_addr_result, HDMI_RX_PHY) & P0_B4_reg_p0_b_koff_bound_mask))
		{
		...
		{
			HDMI_PRINTF("PHY_FORE_KOFF range out of  R range  \n");
		}
	}
...
{
	UINT8 bLan0_LEMax,bLan1_LEMax,bLan2_LEMax;
	...
	HDMI_PRINTF("[HDMI][DFE open] LE MAX lane0 =0x%x   \n",bLan0_LEMax);
	HDMI_PRINTF("[HDMI][DFE open] LE MAX lane1 =0x%x   \n",bLan1_LEMax);
	HDMI_PRINTF("[HDMI][DFE open] LE MAX lane2 =0x%x   \n",bLan2_LEMax);
...
{
	UINT8 bLEmax,bLEmin,bTap0min,bTap0max,bTap1;
	...
	HDMI_PRINTF("[HDMI] Lane%d Tap0 max = 0x%02x  ",lane,bTap0max);
	*/
	...
	//hdmiport_mask(DFE_REG_DFE_READBACK_reg,~limit_set_mask,limit_sel(2));
	//udelay(10);	//1000
	//HDMI_PRINTF("[HDMI] Lane%d Tap1 max = 0x%02x  ",lane,get_Tap1_max(hdmi_in(DFE_REG_DFE_READBACK_reg)));

	//hdmiport_mask(DFE_REG_DFE_READBACK_reg,~limit_set_mask,limit_sel(3));
	//udelay(10);	//1000
	//HDMI_PRINTF(" min = 0x%02x\n",get_Tap1_min(hdmi_in(DFE_REG_DFE_READBACK_reg)));

	hdmi_rx_reg_mask32(REG_DFE_READBACK,~limit_set_mask,limit_sel(4),HDMI_RX_DEF);
	//udelay(10);	//1000
	bLEmax = get_LEQ_max(hdmi_rx_reg_read32(REG_DFE_READBACK,HDMI_RX_DEF));
	//HDMI_PRINTF("[HDMI] Lane%d LE   max = 0x%02x  ",lane,bLEmax);
	...
	//udelay(10);	//5000
	//HDMI_PRINTF("[HDMI] Lane%d Vth COEF = 0x%x  ",lane,get_VTH_coef(hdmi_in(DFE_REG_DFE_READBACK_reg)));

	HDMI_PRINTF("[HDMI] ********************Lane%d******************\n",lane);
	HDMI_PRINTF("[HDMI] Lane%d Tap0 max = 0x%02x  ,min = 0x%02x \n",lane,bTap0max,bTap0min);
	hdmi_rx_reg_mask32(REG_DFE_READBACK,~coef_set_mask,coef_sel(1),HDMI_RX_DEF);
	//udelay(10);	//5000
	HDMI_PRINTF("[HDMI] Lane%d COEF Tap0=0x%02x \n",lane,get_TAP0_coef(hdmi_rx_reg_read32(REG_DFE_READBACK,HDMI_RX_DEF)));
	...
	//HDMI_PRINTF("Tap1=0x%02x  ", get_TAP1_coef(hdmi_in(DFE_REG_DFE_READBACK_reg)));

	hdmi_rx_reg_mask32(REG_DFE_READBACK,~coef_set_mask,coef_sel(3),HDMI_RX_DEF);
	//udelay(10);	//5000
	HDMI_PRINTF("Tap1=0x%02x  ,Tap2=%+d \n",bTap1, (get_TAP2_coef_sign(hdmi_rx_reg_read32(REG_DFE_READBACK,HDMI_RX_DEF)))? -get_TAP2_coef(hdmi_rx_reg_read32(REG_DFE_READBACK,HDMI_RX_DEF)): get_TAP2_coef(hdmi_rx_reg_read32(REG_DFE_READBACK,HDMI_RX_DEF)));

	//hdmiport_mask(DFE_REG_DFE_READBACK_reg,~coef_set_mask,coef_sel(4));
	//udelay(10);	//5000
	//HDMI_PRINTF("Tap3=%+d  ", (get_TAP3_coef_sign(hdmi_in(DFE_REG_DFE_READBACK_reg)))? -get_TAP3_coef(hdmi_in(DFE_REG_DFE_READBACK_reg)): get_TAP3_coef(hdmi_in(DFE_REG_DFE_READBACK_reg)));

	//hdmiport_mask(DFE_REG_DFE_READBACK_reg,~coef_set_mask,coef_sel(5));
	//udelay(10);	//5000
	//HDMI_PRINTF("Tap4=%+d\n", (get_TAP4_coef_sign(hdmi_in(DFE_REG_DFE_READBACK_reg)))? -get_TAP4_coef(hdmi_in(DFE_REG_DFE_READBACK_reg)): get_TAP4_coef(hdmi_in(DFE_REG_DFE_READBACK_reg)));

	//hdmiport_mask(DFE_REG_DFE_READBACK_reg,~coef_set_mask,coef_sel(6));
	//udelay(10);	//5000
	//HDMI_PRINTF("[HDMI] Lane%d Servo COEF = 0x%x\n",lane,get_SERVO_coef(hdmi_in(DFE_REG_DFE_READBACK_reg)));

	hdmi_rx_reg_mask32(REG_DFE_READBACK,~coef_set_mask,coef_sel(7),HDMI_RX_DEF);
	//udelay(10);	//5000
	HDMI_PRINTF("[HDMI] Lane%d LE   max = 0x%02x , min = 0x%02x \n",lane,bLEmax,bLEmin);
...
{
	HDMI_PRINTF("[HDMI][%s] b(%d) dfe_mode(%d) \n",__FUNCTION__,b,dfe_mode);
	//script file
	...
	hdmi_rx_reg_mask32(REG_DFE_INIT0_L0, ~(REG_DFE_INIT0_L0_tap1_init_lane0_mask|REG_DFE_INIT0_L0_tap0_init_lane0_mask|REG_DFE_INIT0_L0_servo_init_lane0_mask|REG_DFE_INIT0_L0_vth_init_lane0_mask),(REG_DFE_INIT0_L0_tap1_init_lane0(0xc)|REG_DFE_INIT0_L0_tap0_init_lane0(0xc)|REG_DFE_INIT0_L0_servo_init_lane0(0xf)|REG_DFE_INIT0_L0_vth_init_lane0(0x3)), HDMI_RX_DEF);
	hdmi_rx_reg_mask32(REG_DFE_INIT0_L1, ~(REG_DFE_INIT0_L0_tap1_init_lane0_mask|REG_DFE_INIT0_L0_tap0_init_lane0_mask|REG_DFE_INIT0_L0_servo_init_lane0_mask|REG_DFE_INIT0_L0_vth_init_lane0_mask),(REG_DFE_INIT0_L0_tap1_init_lane0(0xc)|REG_DFE_INIT0_L0_tap0_init_lane0(0xc)|REG_DFE_INIT0_L0_servo_init_lane0(0xf)|REG_DFE_INIT0_L0_vth_init_lane0(0x3)), HDMI_RX_DEF);
	hdmi_rx_reg_mask32(REG_DFE_INIT0_L2, ~(REG_DFE_INIT0_L0_tap1_init_lane0_mask|REG_DFE_INIT0_L0_tap0_init_lane0_mask|REG_DFE_INIT0_L0_servo_init_lane0_mask|REG_DFE_INIT0_L0_vth_init_lane0_mask),(REG_DFE_INIT0_L0_tap1_init_lane0(0xc)|REG_DFE_INIT0_L0_tap0_init_lane0(0xc)|REG_DFE_INIT0_L0_servo_init_lane0(0xf)|REG_DFE_INIT0_L0_vth_init_lane0(0x3)), HDMI_RX_DEF);
	//cloud teset
	...
	{
		hdmi_rx_reg_mask32(REG_DFE_LIMIT0,~(REG_DFE_LIMIT0_tap0_max_mask|REG_DFE_LIMIT0_tap1_min_mask|REG_DFE_LIMIT0_tap2_max_mask|REG_DFE_LIMIT0_tap2_min_mask),
											(REG_DFE_LIMIT0_tap0_max(0x18)|REG_DFE_LIMIT0_tap1_min(0x10)|REG_DFE_LIMIT0_tap2_max(0x0a)|REG_DFE_LIMIT0_tap2_min(0x19)), HDMI_RX_DEF);
	}
	else
	{
		hdmi_rx_reg_mask32(REG_DFE_LIMIT0,~(REG_DFE_LIMIT0_tap0_max_mask|REG_DFE_LIMIT0_tap1_min_mask|REG_DFE_LIMIT0_tap2_max_mask|REG_DFE_LIMIT0_tap2_min_mask),
											(REG_DFE_LIMIT0_tap0_max(0x18)|REG_DFE_LIMIT0_tap1_min(0x35)|REG_DFE_LIMIT0_tap2_max(0x0a)|REG_DFE_LIMIT0_tap2_min(0x19)), HDMI_RX_DEF);
	}
#else
	hdmi_rx_reg_mask32(REG_DFE_LIMIT0, ~(REG_DFE_LIMIT0_tap0_max_mask|REG_DFE_LIMIT0_tap1_min_mask|REG_DFE_LIMIT0_tap2_max_mask|REG_DFE_LIMIT0_tap2_min_mask),
										(REG_DFE_LIMIT0_tap0_max(0x18)|REG_DFE_LIMIT0_tap1_min(0x35)|REG_DFE_LIMIT0_tap2_max(0x0a)|REG_DFE_LIMIT0_tap2_min(0x19)), HDMI_RX_DEF);
	...
	hdmi_rx_reg_mask32(REG_DFE_LIMIT1, ~(REG_DFE_LIMIT1_vth_min_mask|REG_DFE_LIMIT1_vth_max_mask|REG_DFE_LIMIT1_vth_threshold_mask|REG_DFE_LIMIT1_tap0_threshold_mask|REG_DFE_LIMIT1_tap0_adjust_mask|REG_DFE_LIMIT1_tap1_trans_mask),
										(REG_DFE_LIMIT1_vth_min(0)|REG_DFE_LIMIT1_vth_max(0x7)|REG_DFE_LIMIT1_tap0_adjust(0x02)|REG_DFE_LIMIT1_tap1_trans_mask), HDMI_RX_DEF);
	// ENABLE  VTH & TAP0
	//hdmiport_mask(DFE_REG_DFE_READBACK_reg,~DFE_REG_DFE_READBACK_record_limit_en_mask,DFE_REG_DFE_READBACK_record_limit_en(1));		//limit value record
	...
	//HDMI_PRINTF("[HDMI][DFE open] ENABLE	VTH & TAP0	\n");
	//ENABLE TAP1 & LE
#if 1//QC 2016/1/18
	hdmi_rx_reg_mask32(REG_DFE_EN_L0, ~(REG_dfe_adapt_en_lane0_TAP1|REG_dfe_adapt_en_lane0_LE|REG_dfe_adapt_en_lane0_TAP2),(REG_dfe_adapt_en_lane0_TAP1|REG_dfe_adapt_en_lane0_LE|REG_dfe_adapt_en_lane0_TAP2), HDMI_RX_DEF);
	hdmi_rx_reg_mask32(REG_DFE_EN_L1, ~(REG_dfe_adapt_en_lane1_TAP1|REG_dfe_adapt_en_lane1_LE|REG_dfe_adapt_en_lane1_TAP2),(REG_dfe_adapt_en_lane1_TAP1|REG_dfe_adapt_en_lane1_LE|REG_dfe_adapt_en_lane1_TAP2), HDMI_RX_DEF);
	hdmi_rx_reg_mask32(REG_DFE_EN_L2, ~(REG_dfe_adapt_en_lane2_TAP1|REG_dfe_adapt_en_lane2_LE|REG_dfe_adapt_en_lane2_TAP2),(REG_dfe_adapt_en_lane2_TAP1|REG_dfe_adapt_en_lane2_LE|REG_dfe_adapt_en_lane2_TAP2), HDMI_RX_DEF);
#else
	hdmi_rx_reg_mask32(REG_DFE_EN_L0, ~(REG_dfe_adapt_en_lane0_TAP1|REG_dfe_adapt_en_lane0_LE),(REG_dfe_adapt_en_lane0_TAP1|REG_dfe_adapt_en_lane0_LE), HDMI_RX_DEF);
	hdmi_rx_reg_mask32(REG_DFE_EN_L1, ~(REG_dfe_adapt_en_lane1_TAP1|REG_dfe_adapt_en_lane1_LE),(REG_dfe_adapt_en_lane1_TAP1|REG_dfe_adapt_en_lane1_LE), HDMI_RX_DEF);
	hdmi_rx_reg_mask32(REG_DFE_EN_L2, ~(REG_dfe_adapt_en_lane2_TAP1|REG_dfe_adapt_en_lane2_LE),(REG_dfe_adapt_en_lane2_TAP1|REG_dfe_adapt_en_lane2_LE), HDMI_RX_DEF);
#endif
	//HDMI_PRINTF("[HDMI][DFE open] ENABLE TAP1 & LE \n");
	//HDMI_DELAYMS(1);
...
void HDMI_MAG2_SetDFE_MS_speed(int b, int dfe_mode)
{
	...
	hdmi_rx_reg_mask32(REG_DFE_INIT0_L0, ~(REG_DFE_INIT0_L0_tap1_init_lane0_mask|REG_DFE_INIT0_L0_tap0_init_lane0_mask|REG_DFE_INIT0_L0_servo_init_lane0_mask|REG_DFE_INIT0_L0_vth_init_lane0_mask),(REG_DFE_INIT0_L0_tap1_init_lane0(0x0)|REG_DFE_INIT0_L0_tap0_init_lane0(0xc)|REG_DFE_INIT0_L0_servo_init_lane0(0xf)|REG_DFE_INIT0_L0_vth_init_lane0(0x3)), HDMI_RX_DEF);
	hdmi_rx_reg_mask32(REG_DFE_INIT0_L1, ~(REG_DFE_INIT0_L0_tap1_init_lane0_mask|REG_DFE_INIT0_L0_tap0_init_lane0_mask|REG_DFE_INIT0_L0_servo_init_lane0_mask|REG_DFE_INIT0_L0_vth_init_lane0_mask),(REG_DFE_INIT0_L0_tap1_init_lane0(0x0)|REG_DFE_INIT0_L0_tap0_init_lane0(0xc)|REG_DFE_INIT0_L0_servo_init_lane0(0xf)|REG_DFE_INIT0_L0_vth_init_lane0(0x3)), HDMI_RX_DEF);
	hdmi_rx_reg_mask32(REG_DFE_INIT0_L2, ~(REG_DFE_INIT0_L0_tap1_init_lane0_mask|REG_DFE_INIT0_L0_tap0_init_lane0_mask|REG_DFE_INIT0_L0_servo_init_lane0_mask|REG_DFE_INIT0_L0_vth_init_lane0_mask),(REG_DFE_INIT0_L0_tap1_init_lane0(0x0)|REG_DFE_INIT0_L0_tap0_init_lane0(0xc)|REG_DFE_INIT0_L0_servo_init_lane0(0xf)|REG_DFE_INIT0_L0_vth_init_lane0(0x3)), HDMI_RX_DEF);
	//cloud teset
	hdmi_rx_reg_mask32(REG_DFE_INIT1_L0, ~(REG_DFE_INIT1_L0_init1_rev1_lane0_mask),(_BIT19|_BIT21|_BIT23), HDMI_RX_DEF);
	...
	HDMI_PRINTF("[HDMI][DFE open] ENABLE TAP1  \n");
	//HDMI_DELAYMS(1);
	//hdmiport_mask(DFE_REG_DFE_READBACK_reg,~DFE_REG_DFE_READBACK_record_limit_en_mask,DFE_REG_DFE_READBACK_record_limit_en(0));		//limit value record
	//hdmiport_mask(DFE_REG_DFE_EN_L0_reg,~(REG_dfe_adapt_en_lane0_TAP0|REG_dfe_adapt_en_lane0_Vth|REG_dfe_adapt_en_lane0_TAP1|REG_dfe_adapt_en_lane0_LE),0);
	//hdmiport_mask(DFE_REG_DFE_EN_L1_reg,~(REG_dfe_adapt_en_lane1_TAP0|REG_dfe_adapt_en_lane1_Vth|REG_dfe_adapt_en_lane1_TAP1|REG_dfe_adapt_en_lane1_LE),0);
	//hdmiport_mask(DFE_REG_DFE_EN_L2_reg,~(REG_dfe_adapt_en_lane2_TAP0|REG_dfe_adapt_en_lane2_Vth|REG_dfe_adapt_en_lane2_TAP1|REG_dfe_adapt_en_lane2_LE),0);
	//HDMI_PRINTF("[HDMI][DFE open] disable DFE\n");
	bDFE_Close_Flag = 1;
...
void DFE_Manual_Set(void)
{
	...
	hdmi_rx_reg_mask32(REG_DFE_EN_L0, ~REG_DFE_EN_L0_le_min_lane0_mask, REG_DFE_EN_L0_le_min_lane0(0xf), HDMI_RX_DEF);//lane0 LE coefficient min		//0x18
	hdmi_rx_reg_mask32(REG_DFE_EN_L1, ~REG_DFE_EN_L1_le_min_lane1_mask, REG_DFE_EN_L1_le_min_lane1(0xf), HDMI_RX_DEF);//lane1 LE coefficient min		//0x18
	hdmi_rx_reg_mask32(REG_DFE_EN_L2, ~REG_DFE_EN_L2_le_min_lane2_mask, REG_DFE_EN_L2_le_min_lane2(0xf), HDMI_RX_DEF);//lane2 LE coefficient min		//0x18
	hdmi_rx_reg_mask32(REG_DFE_INIT0_L0, ~REG_DFE_INIT0_L0_tap1_init_lane0_mask, REG_DFE_INIT0_L0_tap1_init_lane0(5), HDMI_RX_DEF);//lane0 tap1 initial
	hdmi_rx_reg_mask32(REG_DFE_INIT0_L1, ~REG_DFE_INIT0_L1_tap1_init_lane1_mask, REG_DFE_INIT0_L1_tap1_init_lane1(5), HDMI_RX_DEF);//lane1 tap1 initial
	hdmi_rx_reg_mask32(REG_DFE_INIT0_L2, ~REG_DFE_INIT0_L2_tap1_init_lane2_mask, REG_DFE_INIT0_L2_tap1_init_lane2(5), HDMI_RX_DEF);//lane2 tap1 initial

	hdmi_rx_reg_mask32(REG_DFE_INIT0_L0, ~REG_DFE_INIT0_L0_tap0_init_lane0_mask, REG_DFE_INIT0_L0_tap0_init_lane0(0xc), HDMI_RX_DEF);//lane0 tap0 initial	//0xc
	hdmi_rx_reg_mask32(REG_DFE_INIT0_L1, ~REG_DFE_INIT0_L1_tap0_init_lane1_mask, REG_DFE_INIT0_L1_tap0_init_lane1(0xc), HDMI_RX_DEF);//lane1 tap0 initial	//0xc
	hdmi_rx_reg_mask32(REG_DFE_INIT0_L2, ~REG_DFE_INIT0_L2_tap0_init_lane2_mask, REG_DFE_INIT0_L2_tap0_init_lane2(0xc), HDMI_RX_DEF);//lane2 tap0 initial	//0xc

	hdmi_rx_reg_mask32(REG_DFE_INIT1_L0, ~REG_DFE_INIT1_L0_load_in_init_lane0_mask, REG_DFE_INIT1_L0_load_in_init_lane0(3), HDMI_RX_DEF);//lane0  initial load
	hdmi_rx_reg_mask32(REG_DFE_INIT1_L1, ~REG_DFE_INIT1_L1_load_in_init_lane1_mask, REG_DFE_INIT1_L1_load_in_init_lane1(3), HDMI_RX_DEF);//lane1  initial load
	hdmi_rx_reg_mask32(REG_DFE_INIT1_L2, ~REG_DFE_INIT1_L2_load_in_init_lane2_mask, REG_DFE_INIT1_L2_load_in_init_lane2(3), HDMI_RX_DEF);//lane2  initial load
	HDMI_DELAYMS(1);	//10000
	hdmi_rx_reg_mask32(REG_DFE_INIT1_L0, ~REG_DFE_INIT1_L0_load_in_init_lane0_mask, 0, HDMI_RX_DEF);//lane0  initial load
	hdmi_rx_reg_mask32(REG_DFE_INIT1_L1, ~REG_DFE_INIT1_L1_load_in_init_lane1_mask, 0, HDMI_RX_DEF);//lane1  initial load
	hdmi_rx_reg_mask32(REG_DFE_INIT1_L2, ~REG_DFE_INIT1_L2_load_in_init_lane2_mask, 0, HDMI_RX_DEF);//lane2  initial load

	// maybe can remove to hdmipower
	hdmi_rx_reg_mask32(P0_B3, ~P0_B_DFE_TAPEN4321, P0_B_DFE_TAPEN4321, HDMI_RX_PHY);//R tap1~4 enable
	hdmi_rx_reg_mask32(P0_G3, ~P0_G_DFE_TAPEN4321, P0_G_DFE_TAPEN4321, HDMI_RX_PHY);//G tap1~4 enable
	hdmi_rx_reg_mask32(P0_R3, ~P0_R_DFE_TAPEN4321, P0_R_DFE_TAPEN4321, HDMI_RX_PHY);//B tap1~4 enable
...
void HDMI_MAG2_SetDFE_Close(void)
{
	u_int8_t lane;
#if 1//QC 2016/1/18
	hdmi_rx_reg_mask32(REG_DFE_EN_L0,~(REG_dfe_adapt_en_lane0_TAP0|REG_dfe_adapt_en_lane0_Vth|REG_dfe_adapt_en_lane0_TAP1|REG_dfe_adapt_en_lane0_TAP2|REG_dfe_adapt_en_lane0_LE),0, HDMI_RX_DEF);
	hdmi_rx_reg_mask32(REG_DFE_EN_L1,~(REG_dfe_adapt_en_lane1_TAP0|REG_dfe_adapt_en_lane1_Vth|REG_dfe_adapt_en_lane1_TAP1|REG_dfe_adapt_en_lane1_TAP2|REG_dfe_adapt_en_lane1_LE),0, HDMI_RX_DEF);
	hdmi_rx_reg_mask32(REG_DFE_EN_L2,~(REG_dfe_adapt_en_lane2_TAP0|REG_dfe_adapt_en_lane2_Vth|REG_dfe_adapt_en_lane2_TAP1|REG_dfe_adapt_en_lane2_TAP2|REG_dfe_adapt_en_lane2_LE),0, HDMI_RX_DEF);
#else
	hdmi_rx_reg_mask32(REG_DFE_EN_L0, ~(REG_dfe_adapt_en_lane0_TAP0|REG_dfe_adapt_en_lane0_Vth|REG_dfe_adapt_en_lane0_TAP1|REG_dfe_adapt_en_lane0_LE), 0, HDMI_RX_DEF);
	hdmi_rx_reg_mask32(REG_DFE_EN_L1, ~(REG_dfe_adapt_en_lane1_TAP0|REG_dfe_adapt_en_lane1_Vth|REG_dfe_adapt_en_lane1_TAP1|REG_dfe_adapt_en_lane1_LE), 0, HDMI_RX_DEF);
	hdmi_rx_reg_mask32(REG_DFE_EN_L2, ~(REG_dfe_adapt_en_lane2_TAP0|REG_dfe_adapt_en_lane2_Vth|REG_dfe_adapt_en_lane2_TAP1|REG_dfe_adapt_en_lane2_LE), 0, HDMI_RX_DEF);
#endif

	hdmi_rx_reg_mask32(REG_DFE_READBACK, ~REG_DFE_READBACK_record_limit_en_mask, REG_DFE_READBACK_record_limit_en(0), HDMI_RX_DEF);//limit value record
	//HDMI_PRINTF("[HDMI][DFE open] disable DFE\n");
	...
	bDFE_Close_Cnt = 0;
	hdmi_ioctl_struct.DEF_ready = 1;
	HDMI_PRINTF("[HDMI][DFE open] HDMI_MAG2_SetDFE_Close \n");
}
...
void SetupTMDSPhy(u_int32_t b, char force)
{
	...
	hdmi_rx_reg_mask32(P0_CK3, ~(P0_ck_9_CMU_PFD_RSTB|P0_ck_9_CMU_WDRST), (P0_ck_9_CMU_PFD_RSTB|P0_ck_9_CMU_WDRST), HDMI_RX_PHY);//PFD_RSTB	release
	hdmi_rx_reg_mask32(P1_CK3, ~(P1_ck_9_CMU_PFD_RSTB|P0_ck_9_CMU_WDRST), (P1_ck_9_CMU_PFD_RSTB|P0_ck_9_CMU_WDRST), HDMI_RX_PHY);//PFD_RSTB	release
	hdmi_rx_reg_mask32(P2_CK3, ~(P2_ck_9_CMU_PFD_RSTB|P0_ck_9_CMU_WDRST), (P2_ck_9_CMU_PFD_RSTB|P0_ck_9_CMU_WDRST), HDMI_RX_PHY);//PFD_RSTB	release
	...
	if(hdmi_ioctl_struct.hdcp_state<HDCPRX_STATE_2P2_DISCOVER)//Prevent influence HDCP2.2
...
{
	u_int32_t tout=60;
	hdmi_rx_reg_mask32(VIDEO_BIT_ERR_DET, ~VIDEO_BIT_ERR_DET_mode_mask, 0, HDMI_RX_MAC); //mode
	//hdmiport_mask(HDMI_VIDEO_BIT_ERR_DET_reg, ~HDMI_VIDEO_BIT_ERR_DET_ch_sel_mask, HDMI_VIDEO_BIT_ERR_DET_ch_sel(ch_sel)); //ch_sel
	hdmi_rx_reg_mask32(VIDEO_BIT_ERR_DET, ~VIDEO_BIT_ERR_DET_period_mask, VIDEO_BIT_ERR_DET_period(period), HDMI_RX_MAC); //period
	hdmi_rx_reg_mask32(VIDEO_BIT_ERR_DET, ~VIDEO_BIT_ERR_DET_thd_mask, VIDEO_BIT_ERR_DET_thd(thd), HDMI_RX_MAC); //thd

	hdmi_rx_reg_mask32(VIDEO_BIT_ERR_DET, ~VIDEO_BIT_ERR_DET_en_mask, 0, HDMI_RX_MAC); // disable
	hdmi_rx_reg_mask32(VIDEO_BIT_ERR_DET, ~VIDEO_BIT_ERR_DET_reset_mask, VIDEO_BIT_ERR_DET_reset_mask, HDMI_RX_MAC); //reset cnt
	hdmi_rx_reg_mask32(VIDEO_BIT_ERR_DET, ~VIDEO_BIT_ERR_DET_reset_mask, 0, HDMI_RX_MAC);
	HDMI_DELAYMS(1);
	//For clear
	hdmi_rx_reg_mask32(VIDEO_BIT_ERR_STATUS_B,	//wclr_out
		~(VIDEO_BIT_ERR_STATUS_B_no_dis_reset_b_mask|VIDEO_BIT_ERR_STATUS_B_bit_err_thd_b_mask|VIDEO_BIT_ERR_STATUS_B_bit_err_b_mask|VIDEO_BIT_ERR_STATUS_B_bit_err_cnt_of_b_mask),
		(VIDEO_BIT_ERR_STATUS_B_no_dis_reset_b_mask|VIDEO_BIT_ERR_STATUS_B_bit_err_thd_b_mask|VIDEO_BIT_ERR_STATUS_B_bit_err_b_mask|VIDEO_BIT_ERR_STATUS_B_bit_err_cnt_of_b_mask), HDMI_RX_MAC);
	//
	hdmi_rx_reg_mask32(VIDEO_BIT_ERR_STATUS_G,	//wclr_out
		~(VIDEO_BIT_ERR_STATUS_B_no_dis_reset_b_mask|VIDEO_BIT_ERR_STATUS_B_bit_err_thd_b_mask|VIDEO_BIT_ERR_STATUS_B_bit_err_b_mask|VIDEO_BIT_ERR_STATUS_B_bit_err_cnt_of_b_mask),
		(VIDEO_BIT_ERR_STATUS_B_no_dis_reset_b_mask|VIDEO_BIT_ERR_STATUS_B_bit_err_thd_b_mask|VIDEO_BIT_ERR_STATUS_B_bit_err_b_mask|VIDEO_BIT_ERR_STATUS_B_bit_err_cnt_of_b_mask), HDMI_RX_MAC);
	//
	hdmi_rx_reg_mask32(VIDEO_BIT_ERR_STATUS_R,	//wclr_out
		~(VIDEO_BIT_ERR_STATUS_B_no_dis_reset_b_mask|VIDEO_BIT_ERR_STATUS_B_bit_err_thd_b_mask|VIDEO_BIT_ERR_STATUS_B_bit_err_b_mask|VIDEO_BIT_ERR_STATUS_B_bit_err_cnt_of_b_mask),
		(VIDEO_BIT_ERR_STATUS_B_no_dis_reset_b_mask|VIDEO_BIT_ERR_STATUS_B_bit_err_thd_b_mask|VIDEO_BIT_ERR_STATUS_B_bit_err_b_mask|VIDEO_BIT_ERR_STATUS_B_bit_err_cnt_of_b_mask), HDMI_RX_MAC);
	...
	HDMI_PRINTF("[HDMI][BER] B bit_err_cnt=%d ,G bit_err_cnt=%d  ,R bit_err_cnt=%d \n ",
...
int HdmiMeasureVedioClock(void)
{
	...
	if(hdmi_rx_reg_read32(HDMI_GPVS, HDMI_RX_MAC) & _BIT5)//RSV1 received
	{
...
int rtd_hdmiRx_cmd(u_int8_t cmd,  void * arg)
{
	switch((HDMIRX_IOCTL_T) cmd) {
			...
			case IOCTRL_HDMI_PHY_START:
			{
				...
				//hdmiport_mask(HDMIPHY_REG_CK_5_8_reg ,~_BIT23,_BIT23);//Freq mode auto detect enable

				//port 0 auto mode
				hdmi_rx_reg_mask32(REGD42, ~REGD42_reg_p0_ck_md_auto_mask, REGD42_reg_p0_ck_md_auto_mask, HDMI_RX_PHY);	//Freq mode auto detect enable
				//port 1 auto mode
				hdmi_rx_reg_mask32(REGD44, ~REGD44_reg_p1_ck_md_auto_mask, REGD44_reg_p1_ck_md_auto_mask, HDMI_RX_PHY); //Freq mode auto detect enable
				//port2 auto mode
				hdmi_rx_reg_mask32(REGD46,~REGD46_reg_p2_ck_md_auto_mask, REGD46_reg_p2_ck_md_auto_mask, HDMI_RX_PHY);	//Freq mode auto detect enable

				hdmi_rx_reg_mask32(CLKDETCR , ~_BIT0, _BIT0, HDMI_RX_MAC);//PMM clk detect enable
			...
			case IOCTRL_HDMI_PHY_STOP:
				...
				hdmi_phy_idle_patch.idle_count = 0;
				hdmi_ioctl_struct.hotplug_delay_count = 0;
				//hdmiport_mask(HDMIPHY_REG_CK_5_8_reg ,~_BIT23,0);//Freq mode auto detect disable

				//port 0 auto mode
				hdmi_rx_reg_mask32(REGD42, ~REGD42_reg_p0_ck_md_auto_mask, 0, HDMI_RX_PHY); //Freq mode auto detect enable
				//port 1 auto mode
				hdmi_rx_reg_mask32(REGD44, ~REGD44_reg_p1_ck_md_auto_mask, 0, HDMI_RX_PHY);	//Freq mode auto detect enable
				//port2 auto mode
				hdmi_rx_reg_mask32(REGD46, ~REGD46_reg_p2_ck_md_auto_mask, 0, HDMI_RX_PHY);	//Freq mode auto detect enable

				hdmi_rx_reg_mask32(CLKDETCR , ~_BIT0, 0, HDMI_RX_MAC);//PMM clk detect disable
