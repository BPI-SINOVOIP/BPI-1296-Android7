...
HDMI_DVI_MODE_T IsHDMI(void)
{
	...
	if(HDMI_SCR_get_mode(scr_val))//Manual HDMI/DVI switch mode
	{
...
//	M/N = [8/5 , 4/3 , 1 ] * S
	...
	unsigned int large_ratio, Smean,Stest, m, n, o, fraction1, fraction2, fvco, tmds;
...
void drvif_Hdmi2p0_DetectMode(void)
{
	...
	if((bTMDSStatus&_BIT0) == _BIT0 )//Scrambling
	{
		...
		hdmi_rx_reg_mask32(HDMI_SCR,~(HDMI_SCR_mode_mask|HDMI_SCR_msmode_mask),(HDMI_SCR_mode_mask|HDMI_SCR_msmode_mask), HDMI_RX_MAC);
	}
	else
	{
