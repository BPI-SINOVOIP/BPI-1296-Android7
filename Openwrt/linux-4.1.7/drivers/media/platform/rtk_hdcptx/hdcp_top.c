...
static void hdcp_wq_authentication_failure(void)
{
        ...
        if (hdcp.av_mute_needed){
            #if(USED_TA_CMD)
                ta_hdcp_lib_set_av_mute(AV_MUTE_SET);
            #else
        ...
        #if(USED_TA_CMD)
            ta_hdcp_lib_set_encryption(HDCP_ENC_OFF);
        #else
...
static void hdcp_work_queue(struct work_struct *work)
{
	...
	#if(USED_TA_CMD)
            HDCP_DEBUG("[HDCP]*************USED_TA_CMD\n");
	if(init_ta_flag==0){
	HDCP_DEBUG("[HDCP]*************ta_hdcp14_init\n");
	ta_hdcp14_init();
	init_ta_flag=1;
	}
	#endif
...
static long hdcp_set_22_repeater_info_ctl(void __user *argp)
{
	...
	HDCP_DEBUG("Seq_num: %02x %02x %02x",repeater_info.Seq_num[0],repeater_info.Seq_num[1],repeater_info.Seq_num[2]);
	...
	HDCP_DEBUG("Receive ID list : %02x , %02x, %02x, %02x, %02x",repeater_info.Receiver_ID_LIST[0] ,repeater_info.Receiver_ID_LIST[1],repeater_info.Receiver_ID_LIST[2],repeater_info.Receiver_ID_LIST[3],repeater_info.Receiver_ID_LIST[4] );
	return 0;
}
...
static DEVICE_ATTR(hdcp1x_en, S_IRUGO, hdcp_hdcp1x_en_show, NULL);
/* /sys/class/misc/hdcp/hdcp2p2_en */
static DEVICE_ATTR(hdcp2p2_en, S_IRUGO, hdcp_hdcp2p2_en_show, NULL);
