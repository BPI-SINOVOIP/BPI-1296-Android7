...
static void gpio_irq_enable(struct irq_data *d)
{
    ...
    RTK_GPIO_DBG("[%s] irq number(%u), hwirq(%lu), group(%s)", __FUNCTION__, d->irq, d->hwirq, d->domain->of_node->name);
						   ...
						   const struct rtk_gpio_groups *p_rtk_gpio_grp)
{
	...
	irq_set_chained_handler(p_rtk_gpio_ctl->bank_deassert_irq, gpio_irq_handler);
	irq_set_handler_data(p_rtk_gpio_ctl->bank_deassert_irq, p_rtk_gpio_ctl);

	irq_set_chained_handler(p_rtk_gpio_ctl->bank_assert_irq, gpio_irq_handler);
...
static int rtk_gpio_get(struct gpio_chip *chip, unsigned offset)
{
	...
	if (temp & mask) {  /*direction out*/
		...
		value = ioread_reg_bit(((volatile void *)p_rtk_gpio_ctl->gpio_regs_base + dato_offset), GPIO_REG_BIT(offset));
	} else {  /*direction in*/
		dati_offset = gpio_get_reg_offset(p_rtk_gpio_ctl->group_index, GP_REG_DATI, offset);
		value = ioread_reg_bit(((volatile void *)p_rtk_gpio_ctl->gpio_regs_base + dati_offset), GPIO_REG_BIT(offset));
	}
	spin_unlock_irqrestore(&p_rtk_gpio_ctl->lock, flags);
